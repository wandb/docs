...
## Added
- The new package `wandb.sandbox` package and the `wandb beta sandbox` CLI for using [W&B Sandboxes](/sandboxes).
...
## Removed
- The vendored `wandb_orjson` extension and its `WANDB_DISABLE_ORJSON` and `WANDB_BUILD_SKIP_ORJSON` environment variables have been removed.
</Update>
