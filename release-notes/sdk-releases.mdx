...
description: "View release notes for the W&B Python SDK including new features, bug fixes, and breaking changes by version."
rss: true
---
...
<Update label="0.30.0" description="September 8, 2026">
## Notable changes
- `wandb.InternalApi`, the `wandb.PublicApi` and `wandb.apis.PublicApi` aliases, and the `wandb.apis.internal` and `wandb.sdk.internal.internal_api` modules are removed. Use `wandb.Api()` instead.
- `wandb.api` and `wandb.ensure_configured()` are deprecated. Use `wandb.login(prompt=False)` to check whether a user is logged in, and `wandb.Api()` for everything else.
- The legacy `wandb sync` options are removed. Run `wandb sync` with no arguments instead of `wandb sync --sync-all`, and use `--job-type` instead of `--job_type`. To list the current options, run `wandb sync --help`.

## Added
- `wandb.login(prompt=False)` uses the configured credentials if there are any and returns `False` otherwise, without asking for an API key or switching to offline mode. Use it to check whether a user is logged in without blocking.
- The Automations API supports sending a prompt to ARIA as an automation action with `SendPromptToAria`. For more information, see [Create an ARIA automation](/models/automations/create-automations/aria).
- W&B LEET TUI features:
  - Charts support custom x-axes that you set with `run.define_metric()`, including glob definitions such as `run.define_metric("train/*", step_metric="train/step")`. The chart header shows the axis name as `[x: train/step]`. This applies to runs that you open from local `.wandb` files.
  - To filter console logs, focus the logs pane, press `/`, then type a regular expression, or press **Tab** and type a glob pattern. Only matching lines are shown, and new matches are followed as they arrive. Press `Ctrl+/` to clear the filter.
  - The metrics, system metrics, and runs filters are saved for each `wandb` directory and applied again the next time you open it, in both the workspace and the single-run view. The filters are stored in `.wandb-leet.json` inside the directory. Clearing a filter removes it from the saved filters.
  - The border or separator line that you drag to resize a pane or sidebar is highlighted while you drag it.

## Changed
- `wandb status --settings` no longer prints the API key, and `wandb projects` no longer prints project descriptions.
- System metrics on Apple Silicon Macs start about 1.5 seconds sooner after a run starts.
- The W&B LEET TUI is faster on long runs. A run with 100 metrics and 400,000 steps opens 10 times faster, frames render about 30 percent faster, and live chart updates no longer redraw every point.

## Deprecated
- `wandb.api` and `wandb.ensure_configured()` are deprecated and will be removed in a later release. `wandb.api` now provides only `api_key`, `default_entity`, and `viewer()`. Use `wandb.login(prompt=False)` to check whether a user is logged in, and `wandb.Api()` for everything else.
- `wandb.sandbox` and the `wandb beta sandbox` commands are deprecated and will be removed in a later release. Use the `cwsandbox` package directly.

## Removed
- Removed `wandb.InternalApi`, the `wandb.PublicApi` and `wandb.apis.PublicApi` aliases, and the `wandb.apis.internal` and `wandb.sdk.internal.internal_api` modules. Use `wandb.Api()` instead.
- Removed the legacy `wandb sync` options. Run `wandb sync` with no arguments instead of `wandb sync --sync-all`, and use `--job-type` instead of `--job_type`.

## Fixed
- File uploads and downloads that take longer than 20 seconds no longer fail with `CommError: Failed to execute API request: the service process is busy and did not respond in time`. This was a regression in v0.29.0.
- `wandb.init()` and `wandb.Api()` no longer stall for up to 10 seconds when the W&B server is slow to respond. This was a regression in v0.29.0.
- Requests that are rejected with HTTP `402 Payment Required`, which indicates a plan or quota limit such as an artifact usage limit, are no longer retried. The error is reported immediately.
- Syncing a run whose artifacts were already uploaded no longer fails when the artifact files are no longer on disk. This was a regression in v0.29.0.
- A failed artifact upload no longer deletes the files it was uploading, so `wandb sync` can retry it. After a successful upload, only files in the artifact's staging directory are cleaned up.
- When a `.wandb` file ends with an incomplete record, which happens while a run is still writing or after a run is stopped abruptly mid-write, `wandb sync` now explains what to do instead of reporting an internal error.
- Passing `aliases` or `tags` to `Run.log_artifact()` in the public API no longer fails with a server error.
- `wandb.Image` masks that you provide as float arrays with values outside the range [0, 255] now raise `TypeError`, as integer arrays already did, instead of being saved with incorrect class IDs.
- System metrics are collected as soon as a run starts instead of one sampling interval later, which is 15 seconds by default. Short runs now record system metrics, and system charts no longer start with a gap.
- `wandb.init()` no longer waits up to 5 seconds for the GPU and TPU metrics collector to start.
- In Jupyter notebooks, system metrics are collected while a cell runs and paused between cells. Since v0.19.10 the two were swapped, so system metrics were collected only while the notebook was idle.
- On machines with AMD GPUs, system metrics monitoring no longer runs `rocm-smi` up front, which took 1 to 3 seconds, and no longer crashes when `rocm-smi` reports no GPUs.
- System metrics on Apple Silicon Macs:
    - CPU utilization, frequency, and power now cover the whole sampling interval instead of a snapshot of about 10 milliseconds. The snapshot made utilization and frequency noisy and overstated power by about 25 percent.
    - Macs with 256 GB or more of RAM no longer report a memory size of 0 GB.
    - CPU and GPU temperatures are no longer skewed by invalid sensor readings.
    - M5 Pro and M5 Max Macs no longer report 0 efficiency cores.
    - CPU frequencies on the MacBook Neo are no longer reported about 1,000 times too high.
    - CPU utilization and frequency are now collected for the efficiency cores of M5 Macs and the performance cores of M5 Pro and M5 Max Macs, which were missing.
- W&B LEET TUI:
    - `wandb leet` prints an error when it cannot start, for example when no terminal is available, instead of exiting with status 1 and no output. Debug logs (`WANDB_DEBUG=true`) are written next to the LEET config file instead of to the current directory.
    - `wandb leet` no longer sends usage telemetry in offline or disabled mode (`WANDB_MODE=offline` or `WANDB_MODE=disabled`), like the rest of the SDK.
    - In narrow terminals, the workspace hides the run overview sidebar, and then the runs list, when they would leave the charts fewer than 24 columns wide. Previously, an 80-column terminal showed the charts as a one-column sliver between the two sidebars.
    - The top y-axis label of a chart is no longer cut off when it is wider than the other labels. For example, the label now shows `1.09e+03` instead of `+03`.
</Update>

<Update label="0.29.0" description="August 26, 2026">
## Notable changes
- On macOS, `wandb` now requires macOS 13 (Ventura) or newer.
- `wandb.Image` no longer normalizes pixel values. Make sure your data already falls within the range [0, 255].
- `wandb.Video()` now requires the `format` argument when you initialize it from a NumPy array or an IO object.
- The `.length` property on paginated objects, such as `Api.runs()`, `Api.sweeps()`, and `run.files()`, is removed. Use `len(...)` instead.
- The following deprecated APIs are removed: the `start_method` setting, the `summary="best"` option and `goal` argument of `run.define_metric()`, the `quiet` argument of `run.finish()` and `wandb.finish()`, and the `run.project_name()`, `run.get_url()`, `run.get_project_url()`, and `run.get_sweep_url()` methods.

## Added
- The new public API method `run.console_logs()` returns the console output that W&B captured for a run, for both finished and active runs. To return only the last N lines, pass `last=N`. Reading from the beginning of the log requires W&B Server v0.77 or newer.
- The Automations API supports team and organization scopes.
- The Automations API supports creating and editing automations that are scoped to a `Registry` object.
- `wandb.Artifact` accepts a new `digest_algorithm` argument, either `"MD5"` (the default) or `"XXH128"`. Pass `digest_algorithm="XXH128"` to opt an artifact into XXH128 hashing where possible. For artifacts created with `digest_algorithm="XXH128"`, `artifact.verify()` fails on SDK versions older than v0.29.0.
- The new property `Artifact.linked_at` returns the time a version was linked to the relevant collection. It applies only to linked versions and returns `None` for source artifacts.
- Registry version queries (`Api.registries().versions()`, `Api.registries().collections().versions()`, `Registry.versions()`, and `Registry.collections().versions()`) accept an optional `order` string as a keyword argument for organizations with advanced search. You can order versions by `created_at`, `artifact_size`, or `linked_at`.
- W&B LEET TUI features:
  - The new command `wandb leet inspect [PATH]` browses the raw records in a run's `.wandb` transaction log, showing a filterable list of records alongside a text view of the selected record, and following live runs as they write. Press `i` in the single-run view to inspect the current run. Pipe or redirect the output to print the records instead, such as `wandb leet inspect | less`.
  - Press `g` to draw guides behind line charts, either a dotted background or horizontal lines aligned with the axis ticks. Your choice is saved, and you can also set it with `chart_guides` in `wandb leet config`.
  - Drag the separator lines between the run overview sections to resize them with the mouse. The proportions are saved for each view. Press `0` to reset them along with the other pane sizes.

## Changed
- The message format that the SDK uses to communicate between internal processes has changed slightly. If you use `wandb beta core`, restart the service after you upgrade `wandb`, because some operations can fail when the SDK and service versions differ.
- `wandb.sandbox` now allows GPU resource requests for sandboxes instead of rejecting `resources.gpu` on the client.
- Registry search methods `Api.registries()`, `.collections()`, and `.versions()` now validate filter field names and reject unsupported names.
- `Api.create_automation()` and `Api.update_automation()` now raise `UnsupportedError` instead of `CommError` when the server does not support the given automation.
- The macOS wheels now require macOS 13 (Ventura) or newer. macOS 12 reached end of life in 2024.
- In the W&B LEET TUI, the run overview sections (**Environment**, **Config**, and **Summary**) now share the sidebar height proportionally and use all available space. Previously, they stopped growing at fixed sizes, which left the bottom of tall terminals empty.

## Removed
- Removed `wandb.tensorboard.log()` and `wandb.tensorflow.log()`.
- Removed the deprecated `start_method` setting, which already had no effect.
- Removed the deprecated `.length` property from `wandb.apis.paginator.SizedPaginator`. Use `len(...)` instead.
...
- Removed the deprecated `normalize` argument from `wandb.Image`. Make sure pixel values in NumPy arrays and PyTorch tensors already fall within the range [0, 255].
...
- At certain pane widths, such as while dragging a sidebar edge, the W&B LEET TUI drew metric charts wider than the available space, which pushed the right sidebar off screen.
- W&B LEET TUI:
    - `wandb leet config` now works in short terminal windows. The field list scrolls with the selection, and **Space** toggles boolean settings.
    - Media panels no longer paint black bars around images in the ANSI view or flash black when you switch to the full-resolution view. The area around the image now shows the terminal background, and switching modes no longer flashes or moves the image.

## Security
- Downloading artifacts with `artifact.files()` or `artifact.checkout()` now rejects file and artifact names that contain relative paths. This fixes a path traversal vulnerability described in [CVE-2026-91771](https://www.tenable.com/cve/CVE-2026-91771).
</Update>

<Update label="0.28.2" description="August 12, 2026">
## Notable changes
- The `wandb sync --clean` command now exits with code 1 and prints a hint to use the new `wandb clean` command instead.
- `wandb login` now verifies credentials by default. To turn off verification, pass `--no-verify` on the command line or `verify=False` to `wandb.login()`.

## Added
- Filestream requests support gzip compression, which reduces network traffic when you log metrics. Compression is opt-in and requires server support. To turn it on, set `x_file_stream_no_gzip=False` in `wandb.Settings`. Compression becomes the default in a future release.
- The `wandb agent` command accepts a new `--term-timeout` flag.
- The new property `run.sync_dir` returns the run's sync directory.

## Changed
- The new `wandb clean` command replaces `wandb sync --clean`.
- `wandb.init()` now reports the error it was retrying, such as a network error, when it times out, instead of a generic timeout message. The `init_timeout` setting now also bounds the backend's retries during run initialization.
...
- Paginated artifact and registry query methods now validate pagination arguments on the client before they fetch any results. This applies to `Api.artifacts()`, `Api.artifact_collections()`, `Api.registries()`, `Registries.collections()`, `Registries.versions()`, `Collections.versions()`, `Registry.collections()`, `Registry.versions()`, and `Project.collections()`.
- Ordered registry search now scopes per-registry collection and version queries to the current registry's internal ID, which makes pagination and filtering more reliable.
- Response parsing is faster for many `wandb.Api` operations, including artifact and registry queries.
- Argument handling in `wandb launch` for the `local-process` resource is hardened so that job-supplied values are always shell-quoted.
- The launch agent now restricts a job's Git source URL to HTTPS and SSH remotes, and pins Git's protocol allowlist when it fetches the source and updates submodules.

## Deprecated
- `wandb sync --sync-tensorboard` is deprecated and will be removed in a later release.

## Removed
- Releases no longer include 32-bit Windows (`win32`) wheels. Use 64-bit Python on Windows.

## Fixed
- Registry search with `registries(order=...).collections(order=...).versions()` now returns artifact versions in registry order, collection order, or both.
- macOS x86_64 wheels now contain x86_64 builds of the `wandb-xpu` binary and the Rust parquet library. Previously, these shipped as arm64 builds, which could not run or be loaded on Intel Macs.
- `wandb.login(verify=True)` and `wandb login --verify` now verify federated identity (identity token) credentials, which were previously not verified.
- `wandb login` now validates API keys before it saves them to the `.netrc` file.
- `wandb login` no longer removes or corrupts credentials that belong to other machines in your `.netrc` file.
- `wandb login` and `wandb verify` no longer update the system host settings when a login attempt fails.
- `wandb verify` now reports a failed check instead of crashing when an operation still fails after retries.
- Fixed `wandb.Api(overrides={"base_url": ...})` failing to authenticate with federated identity (identity token) credentials when the specified server was not the default one, such as a Dedicated Cloud deployment, unless `WANDB_BASE_URL` was also set.
- When you use federated identity, the identity token file is now read again for each access token exchange instead of only at startup. Short-lived identity tokens that are re-minted to the same path now keep working for runs that outlive them.
- When you use federated identity and the server rejects the identity token exchange, such as for an invalid or expired token, requests now fail immediately with the server's error message. Previously, the rejected exchange was retried until requests timed out with a generic error.
- Sweep agents now respect a custom `WANDB_DIR` when they write JSON output.
- The `global_step` metric that is created when you sync TensorBoard files is no longer prefixed, such as `train/global_step`, which makes it easier to compare training and validation metrics.
- The TensorBoard integration now produces fewer W&B steps by merging data for the same `global_step` into one W&B step where possible.
...
- `del run.summary[key]` now removes metrics that were logged as nested values, which it previously ignored.

## Security
- Debug logs no longer contain CoreWeave cluster credentials.
- `wandb docker` and `wandb docker-run` no longer pass your API key on the command line, where other users of the same machine could read it.
</Update>

<Update label="0.28.1" description="July 16, 2026">
## Notable changes
- This version requires protobuf v5+ and drops support for protobuf v4.
- This version drops compatibility with W&B Server v0.69.x and older.

## Added
- `Api().project().sweeps()` accepts an optional `filters` argument, which uses the same filter format as `Api().runs()`.
- The new public API method `Run.stop()` (`wandb.Api().run(...).stop()`) requests that an active run stop gracefully, equivalent to the **Stop run** button in the W&B App.
- The new method `wandb.Api().organization()` returns an `Organization` by name. If you omit the name, it returns your current default organization.

## Changed
- `wandb sync` now routes to `wandb beta sync` for supported sets of parameters. To use the previous implementation instead, pass `--legacy`.
- `wandb beta sync` now skips online runs by default, matching `wandb sync`. To include online runs, pass `--no-skip-online`.
- `wandb.sandbox` now defaults serverless sandboxes to a maximum lifetime of 12 hours (`max_lifetime_seconds=43200`). To override the default, pass `max_lifetime_seconds` for an individual sandbox or set it in `SandboxDefaults`.
- `wandb.Api().runs()` and `wandb.Api().run()` now raise `RunNotFoundError` when a run's data fails to load, such as when the run is deleted before its data finishes loading.
- `run.save()` accepts a new `glob` argument. Pass `glob=False` to turn off glob expansion (`*`, `?`, and `[...]`) and treat the path as a literal path. This is useful when a filename contains glob metacharacters.
- Temporary Unix socket files and directories are now removed at shutdown.
- protobuf v4 is no longer supported.

## Removed
- Removed legacy fallback implementations for downloading artifact files from W&B Server releases older than v0.70.0. On these servers, `Artifact.files()` and `Artifact.download()` no longer work for any artifact, and `Artifact.get_entry()` and `Artifact.get_path()` file downloads no longer work for non-reference artifacts. To keep using these operations, upgrade W&B Server to v0.70.0 or newer.
- Removed legacy fallback implementations for fetching an artifact by name from W&B Server releases older than v0.70.0. On these servers, `wandb.Api().artifact()` and other methods that fetch artifacts by path no longer work. To keep using these operations, upgrade W&B Server to v0.70.0 or newer.
- Removed the `GitPython` dependency. Git metadata is now collected by invoking the `git` executable directly. The `GIT_PYTHON_GIT_EXECUTABLE` environment variable is still honored for locating the executable.

## Fixed
- Saving a linked registry artifact, such as when adding an alias, no longer fails when you lack write access to the source project.
- `np.float16` and `np.float32` `NaN` values logged with `Run.log()` are now recorded as `NaN` instead of being silently dropped, matching the behavior of `np.float64` and native `float`.
- `Run.upload_file()` (through `wandb.Api().run(...)`) now registers the uploaded file with the run on Self-Managed deployments. Previously, the file's bytes were uploaded but never committed, so the file didn't appear on the run in deployments without object-store notifications.
- Sweep agents now let an in-progress run finish before exiting when the sweep is deleted or `Api()` returns a 404 error.
- Fixed deadlocks introduced in SDK v0.25.1. These deadlocks were reported when using PyTorch with Python versions earlier than 3.14, which use the `fork` `multiprocessing` start method by default. They could also occur during garbage collection when using some `wandb.Api` functionality.
</Update>

<Update label="0.28.0" description="June 22, 2026">
## Notable changes
...
<Update label="0.27.2" description="June 6, 2026">
## Fixed
- `Run.scan_history()` no longer returns duplicate or missing steps when fetching run history.
</Update>

<Update label="0.27.1" description="June 4, 2026">
## Added
- The Automations API now supports artifact tag, collection tag, and artifact unlinking events.

## Changed
- `wandb.Api` GraphQL operations now route through `wandb-core` instead of the legacy Python GraphQL client.
    - Failures from `wandb.Api` GraphQL operations now raise `WandbApiFailedError` instead of `requests` HTTP exceptions.
- `Run.scan_history()` now reads from exported parquet history when available, which can significantly improve throughput for runs with large history. This capability was introduced under `beta_scan_history` in SDK `v0.23.1`.

## Removed
- Removed the unsupported `wandb.apis.importers` API.
- Removed stale OpenAI, Cohere, and LangChain LLM integrations, including legacy autologging and tracing APIs.
- Removed the deprecated Keras `WandbCallback` and the legacy `wandb.integration.yolov8` callback package.

## Fixed
- `wandb.sandbox` now rejects invalid argument values for placement, GPU, and egress.
</Update>

<Update label="0.27.0" description="May 13, 2026">
## Notable Changes
This version drops support for Python 3.9 and Pydantic v1. Pydantic v2.6 or newer is now required.

## Added
- The new package `wandb.sandbox` and the `wandb beta sandbox` CLI for using [Serverless Sandboxes](/sandboxes).
- The new setting `stop_on_fatal_error` and environment variable `WANDB_STOP_ON_FATAL_ERROR` stop a run after a fatal error that prevents it from uploading metrics.
...
- The new method `run.write_logs()` writes text directly to the run's **Logs** tab instead of relying on automatic capture of `stdout`/`stderr`.

## Changed
- System metrics for runs in Linux containers now report CPU and memory usage relative to the container's cgroup v2 limits instead of the host's totals.
- Python 3.9 is no longer supported.
- Pydantic 2.6 or above is now required. Pydantic v1 is no longer supported. 
...
- The SDK now correctly infers notebooks paths in Jupyter sessions, using the server's root directory, so code saving works in subdirectories (e.g. `code/nested/<notebook>.ipynb`).
...
- Dataclasses in a run's `config` no longer raise `Object of type ... is not JSON serializable` when containing real classes as fields to the dataclass.
...
- Don't swallow exceptions in pyagent.
...
- Gcp storage uri verification failed due to improper async wrapping.
...
- Fix channel swapping on ultralytics classification task.
