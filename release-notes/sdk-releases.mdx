...
- `wandb beta leet` TUI updates:
    ...
    - Cycle through the following display modes in an individual chart using the `y` keyboard shortcut:
        ...
        - Press again to turn off log-scale display and turn on bucketed heatmap mode, which displays system metrics like GPU utilization as percentages.
...
- On Google TPU infrastructure, the SDK attempts to collect new metricsk such as `tensorcore_util`, `duty_cycle_pct`, `hbm_capacity_total`, `hbm_capacity_usage`, and `tensorcore_util`, and latency distributions. See [TPU monitoring library: Supported metrics](https://docs.cloud.google.com/tpu/docs/tpu-monitoring-library#list-all-supported-metric-names) in the Google Cloud documentation.
- On Hopper+ GPUs (H100 and newer), NVML GPU Performance Monitoring profiling metrics are collected, providing visibility into SM utilization, tensor/FP pipeline activity, DRAM bandwidth, and PCIe/NVLink throughput without running the DCGM daemon.
