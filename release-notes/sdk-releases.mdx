...
- `Run.scan_history()` now reads from exported parquet history when available, which can significantly improve throughput for runs with large history.

    This capability was introduced under `beta_scan_history` in SDK `v0.23.1`.
