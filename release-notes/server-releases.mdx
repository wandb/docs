...
- Team admins can now view team-level usage and billing charts, including an overages banner and searchable team selection, and set spend limits.
...
- When you work with nested run groups, you can now color all the child groups under a parent group at once. Click the icon next to a parent run group's name to color every run in the hierarchy. You can distinguish parent groups at a glance while distinct colors flow down to all subgroups.
    <Frame>
...
- In a workspace, you can now hide the sidebar directly from the page. The **Hide sidebar** control is now visible without opening the overflow (**...**) menu.
- When viewing a large log file, click **Top** or press **Cmd+Home** to scroll to the oldest log, and click **Current** or press **Cmd+End** to scroll to the newest log.
...
These features are available in Dedicated Cloud only.

- **Automation Hub** centralizes the creation and management of automations across projects and registries. Find it in the **Core** section of the sidebar.
...
- Weave prompts (`StringPrompt` and `MessagesPrompt`) can now be added to the Registry, so you can version, alias, and reuse prompts across your organization alongside model artifacts.
- Improved artifact collection overview editing and version grid selection. Auto-generate rich collection cards with dynamic artifact panels for more context.
...
- Moved dataset manipulation to the **Evaluations** page and improved the experience of creating datapoints.
- Added an audio player to agent conversations, including a new **Play all** control that plays each file in sequence.
- Improved the **Agents** tab.
...
- Administrators can now disable the creation of team service accounts by setting the environment variable `GORILLA_DISABLE_TEAM_SERVICE_ACCOUNT_CREATION` to `true`.
