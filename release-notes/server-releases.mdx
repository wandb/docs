...
## Changes
- You can now add Weave panels to a W&B workspace and reference W&B Artifacts in a Weave trace. See [Use Weave with W&B training runs](/weave/guides/tools/weave-in-workspaces).

## Fixes
- Fixed a bug where a registry version query from the SDK can return duplicates when there is an alias filter.
