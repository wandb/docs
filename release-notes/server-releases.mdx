...
W&B v0.75 enhances Registry with a new Restricted Viewer role, SCIM API support, and the ability to star registries. The new automation execution history view helps you understand and debug your automations. Visualize config-driven effects with semantic coloring by config properties such as hyperparameters, view full-fidelity plots for system metrics, and more.
...
A W&B Server release is supported for 12 months from its initial release date. As a reminder, customers using [Self-managed](/platform/hosting/hosting-options/self-managed) are responsible to upgrade to a [supported release](/release-notes) in time to maintain support.
