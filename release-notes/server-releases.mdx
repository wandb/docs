...
- When [inviting members](/platform/hosting/iam/access-management/manage-organization#invite-a-user) to an organization, you can now set each member's Models seat and Weave role at the same time.
...
- Artifact collections now have an editable **Overview** tab with a tabbed layout, so you can document a collection alongside its versions.
- You can now visualize artifact version metrics as charts in the artifact panel. Choose which metrics to plot and the chart type using the metrics editor.
