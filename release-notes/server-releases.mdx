...
## Fixes
- Fixed a display bug that could occur on macOS when scroll bars are configured to always display scroll bars. If a workspace has a [Query panel](/models/app/features/panels/query-panels), zooming out in the workspace could trigger a resize loop where the query panel table size oscillates several times a second.
- Fixed a bug where W&B Inference erroneously appeared in Dedicated Cloud UI. Inference is not available in Dedicated Cloud.
</Update>
