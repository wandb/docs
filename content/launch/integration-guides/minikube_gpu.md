...
W&B recommends you use a [Launch Docker queue]({{< relref "/guides/launch/setup-launch-docker" >}}) if you want to set up a minikube cluster for scheduling GPUs on a machine that has a single GPU. You can still follow the tutorial for fun, but the GPU scheduling will not be very useful.
{{% /alert %}}
...
For more information about queue configurations, see the [Set up Launch on Kubernetes]({{< relref "/guides/launch/setup-launch-kubernetes.md" >}}) and the [Advanced queue setup guide]({{< relref "/guides/launch/setup-queue-advanced.md" >}}).   
