...
2. View, sort, filter, group, join, and explore these tables interactively.
...
The histograms compare per-class scores between two models. The top green bar in each histogram shows "CNN-2, 1 epoch" (id 0), trained for one epoch, while the bottom purple bar shows "CNN-2, 5 epochs" (id 1), trained for five epochs. The images focus on instances where the models disagree. For example, in the first row, the "4" received high scores across all potential digits after 1 epoch, but after 5 epochs, it scored highest on the correct label with low scores on the others.
...
View incorrect predictions by filtering to rows where "guess" is not equal to "truth" for the full test dataset. There are 229 wrong guesses after one training epoch, but only 98 after five epochs.
...
Filter out correct answers and group by guess to see examples of misclassified images along with the distribution of true labels for two models side by side. A model variant with double the layer sizes and learning rate is on the left, while the baseline is on the right. The baseline makes slightly more mistakes for each guessed class.

{{< img src="/images/tutorials/tables-3.png" alt="grouped errors for baseline vs double variant" >}}

## Sign up or login

[Sign up or login](https://wandb.ai/login) to W&B to view and interact with your experiments in the browser.

In this example, Google Colab serves as a convenient hosted environment, but training scripts can run from any location and visualize metrics using W&B's experiment tracking tool.
...
Set the number of epochs, where each epoch consists of a training step and a validation (test) step. Optionally configure the amount of data logged per test step. The number of batches and images per batch are set low for demonstration purposes. Define a simple convolutional neural network following [PyTorch-tutorial](https://github.com/yunjey/pytorch-tutorial) code. Load the training and test sets using PyTorch.
...
For each epoch, execute a training step followed by a test step. Create a `wandb.Table()` for each test step to store test predictions. These can be visualized, dynamically queried, and compared side by side in the browser.

```python
# Initialize a new run to track the model's training
...
# Function to log predictions for a batch of test images
def log_test_predictions(images, labels, outputs, predicted, test_table, log_counter):
    ...
    _id = 0
    for i, l, p, s in zip(log_images, log_labels, log_preds, scores.cpu().numpy()):
