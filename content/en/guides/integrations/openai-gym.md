...
If you're using [OpenAI Gym](https://github.com/openai/gym), W&B automatically logs videos of your environment generated by `gym.wrappers.Monitor`. Just set the `monitor_gym` keyword argument to [`wandb.init`]({{< relref "/ref/python/functions/init.md" >}}) to `True` or call `wandb.gym.monitor()`.
