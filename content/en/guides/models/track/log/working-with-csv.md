...
You can optionally log a final summary metric to define the outcome of the run. Use the W&B  [`define_metric`]({{< relref "/ref/python/sdk/classes/run#define_metric" >}}) API to accomplish this. In this example case, we will add the summary metrics to our run with `run.summary.update()`:
