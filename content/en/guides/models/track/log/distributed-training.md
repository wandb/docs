...
Within the rank 0 process, initialize a W&B run with [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init" >}}) and log experiments ([`wandb.log`]({{< relref "/ref/python/sdk/actions/legacy_functions/log" >}})) to that run. 
...
Within the primary node, initialize a W&B run with [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init.md" >}}). Pass in a `wandb.Settings` object to the `settings` parameter (`wandb.init(settings=wandb.Settings()`) with the following:
...
For each worker node, initialize a W&B run with [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init.md" >}}) and provide the following:
