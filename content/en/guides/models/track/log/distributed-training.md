...
Within the primary node, initialize a W&B run with [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init" >}}). Pass in a `wandb.Settings` object to the `settings` parameter (`wandb.init(settings=wandb.Settings()`) with the following:
...
For each worker node, initialize a W&B run with [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init" >}}) and provide the following:
