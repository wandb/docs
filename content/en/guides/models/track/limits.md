...
<!-- Enable batching in calls to `run.log` by passing `commit=False` to minimize the total number of API calls for a given step. See [the docs]({{< relref "/ref/python/sdk/experiments/run/#method-runlog" >}}) for `run.log` for more details. -->
