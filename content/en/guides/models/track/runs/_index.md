...
Initialize a W&B run with [`wandb.init()`]({{< relref "/ref/python/sdk/actions/functions/init.md" >}}). The proceeding code snippet shows how to import the W&B Python SDK and initialize a run. 
...
### Custom run IDs
You can specify your own run ID by passing the `id` parameter to the [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init.md" >}}) method. 
...
You can specify a name for your run by passing the `name` parameter to the [`wandb.init`]({{< relref "/ref/python/sdk/actions/functions/init.md" >}}) method. 
