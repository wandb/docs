...
This captures all python source code files in the current directory and all subdirectories as an [artifact]({{< relref "/ref/python/sdk/actions/classes/Artifact" >}}). For more control over the types and locations of source code files that are saved, see the [reference docs]({{< relref "/ref/python/sdk/actions/classes/run#log_code" >}}).
