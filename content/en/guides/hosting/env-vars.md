...
| `GORILLA_CUSTOMER_SECRET_STORE_SOURCE` | Sets the secret manager to be used by the Weave application when enabling Team Secrets. Can be set to the internal Kubernetes secret manager, `k8s-secretmanager://wandb-secret`, or to a cloud secret manger when deployed in a cloud provider. Currently supported cloud provider secret managers are `aws-secretmanager`, `az-secretmanger`, and `gcp-secretmanager`.  |
