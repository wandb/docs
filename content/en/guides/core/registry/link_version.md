...
{{% alert %}}
The term "type" refers to the artifact object's type. When you create an artifact object ([`wandb.Artifact`]({{< relref "/ref/python/sdk/classes/artifact" >}})), or log an artifact ([`wandb.init.log_artifact`]({{< relref "/ref/python/sdk/classes/run#log_artifact" >}})), you specify a type for the `type` parameter. 
...
Programmatically link an artifact version to a collection with [`wandb.init.Run.link_artifact()`]({{< relref "/ref/python/sdk/classes/run#link_artifact" >}}).
...
1. Specify the team as the entity when you initialize a run with [`wandb.init()`]({{< relref "/ref/python/sdk/functions/init" >}}). If you do not specify the `entity` when you initialize a run, the run uses your default entity which may or may not be your team entity. 
  ```python 
  import wandb   
