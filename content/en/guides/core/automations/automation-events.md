...
#### Run status change
Trigger a workflow when a [run's status]({{< relref "/guides/models/track/runs/#run-states" >}}) changes. Optionally, you can further limit the runs that can trigger an automation by filtering by the user that started a run or the run's name.
