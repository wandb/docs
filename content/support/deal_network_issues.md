...
2. If the network connection is unstable, operate in offline mode by setting the [optional environment variable](../guides/track/environment-variables.md#optional-environment-variables) `WANDB_MODE` to `offline`, and sync files later from a device with Internet access.
3. Consider using [W&B Private Hosting](../guides/hosting/intro/), which runs locally and avoids syncing to cloud servers.
