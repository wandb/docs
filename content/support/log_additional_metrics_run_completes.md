...
For simpler workflows, call `wandb.init` with `resume=True` and `id=UNIQUE_ID`, then call `wandb.init` again with the same `id=UNIQUE_ID`. Log normally with [`wandb.log`]({{< relref "../guides/track/log/" >}}) or `wandb.summary`, and the run values will update accordingly.
