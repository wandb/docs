...
W&B recommends you use a [Launch Docker queue]({{< relref path="../set-up-launch/setup-launch-docker" lang="ja" >}}) if you want to set up a minikube cluster for scheduling GPUs on a machine that has a single GPU. You can still follow the tutorial for fun, but the GPU scheduling will not be very useful.
{{% /alert %}}
...
For more information about queue configurations, see the [Set up Launch on Kubernetes]({{< relref path="../set-up-launch/setup-launch-kubernetes.md" lang="ja" >}}) and the [Advanced queue setup guide]({{< relref path="../set-up-launch/setup-queue-advanced.md" lang="ja" >}}).   
