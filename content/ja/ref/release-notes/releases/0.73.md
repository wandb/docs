...
- Over the next several releases, we will continue migrating existing team-level Model Registry to W&B Registry. For details and a reminder about the timeline, see the [W&B Registry GA announcement]({{< relref path="#registry_ga_announcement" lang="ja" >}}) below.
...
- Added a copy button to the [Run Logs console page]({{< relref path="/guides/models/app/console-logs/" lang="ja" >}}) and improved paste behavior.
- W&B now offers two new colorblind-safe color palettes for runs. Set these in the **Runs** section of the [Workspace settings]({{< relref path="/guides/models/app/features/cascade-settings/#workspace-settings" lang="ja" >}}).
- When [coloring runs by metrics or numeric configs]({{< relref path="/guides/models/track/runs/color-code-runs/" lang="ja" >}}), you can now choose from five new continuous color palettes.
- During a [W&B run]({{< relref path="/guides/models/track/runs/" lang="ja" >}}), [CoreWeave Mission Control](https://www.coreweave.com/mission-control) monitors your compute infrastructure. This integration is now available on Dedicated Cloud.
- BYOB with CoreWeave AI Object Storage now supports authentication with service principals in Multi-tenant Cloud, Dedicated Cloud, and Self-Managed deployments. This helps you to avoid using static credentials in your deployment. Learn more at [Bring your own bucket (BYOB)]({{< relref path="/guides/hosting/data-security/secure-storage-connector/" lang="ja" >}}).
...
- Improved the smoothness and accuracy of [dragging and dropping panels]({{< relref path="/guides/models/app/features/panels/#move-a-panel" lang="ja" >}}) within a section and between sections.
- In [full screen panel view]({{< relref path="/guides/models/app/features/panels/#view-a-panel-in-full-screen-mode" lang="ja" >}}), the **Esc** key now returns you to the workspace.
- Fixed a bug that could cause a panel in a workspace to zoom unexpectedly after returning from full screen.
- In [plot settings]({{< relref path="/guides/models/app/features/panels/line-plot/#edit-line-plot-settings" lang="ja" >}}), chart and legend settings have been consolidated to a single tab.
...
- [Overlays for panels]({{< relref path="/guides/models/app/features/panels/media/#overlays" lang="ja" >}}) without matching runs are now hidden.
