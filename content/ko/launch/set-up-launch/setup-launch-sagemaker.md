...
* [Decide if you want the Launch agent to build a Docker image for you.]({{< relref path="#decide-if-you-want-the-launch-agent-to-build-a-docker-images" lang="ko" >}})
* [Set up AWS resources and gather information about S3, ECR, and Sagemaker IAM roles.]({{< relref path="#set-up-aws-resources" lang="ko" >}})
* [Create an IAM role for the Launch agent]({{< relref path="#create-an-iam-role-for-launch-agent" lang="ko" >}}).
...
Make a note of the ARNs for these resources. You will need the ARNs when you define the [Launch queue configuration]({{< relref path="#configure-launch-queue-for-sagemaker" lang="ko" >}}). 
...
{{% alert %}}
* If you want the launch agent to build images, see the [Advanced agent set up]({{< relref path="./setup-agent-advanced.md" lang="ko" >}}) for additional permissions required.
...
- `RoleArn` : ARN of the SageMaker execution IAM role (see [prerequisites]({{< relref path="#prerequisites" lang="ko" >}})). Not to be confused with the launch **agent** IAM role.
...
There are [several options for how the Launch agent is deployed for a Amazon SageMaker]({{< relref path="#decide-where-to-run-the-launch-agent" lang="ko" >}}) queue: on a local machine, on an EC2 instance, or in an EKS cluster. [Configure your launch agent appropriately]({{< relref path="#configure-a-launch-agent" lang="ko" >}}) based on the where you deploy your agent.
...
{{% alert %}}
This section applies only if your launch agent uses existing Docker images that contain your training or inference logic. [There are two options on how your launch agent behaves.]({{< relref path="#decide-if-you-want-the-launch-agent-to-build-a-docker-images" lang="ko" >}})  
{{% /alert %}}
