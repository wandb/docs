...
| `init_kwargs`                   | Params to pass to `wandb.init()` such as your wandb `config` etc. See the [`wandb.init()` parameters]({{< relref path="/ref/python/sdk/functions/init.md" lang="ko" >}}) for parameters that `wandb.init()` accepts.
