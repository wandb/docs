...
- **`WandbMetricsLogger`** : Use this callback for [Experiment Tracking]({{< relref path="/guides/models/track" lang="ko" >}}). It logs your training and validation metrics along with system metrics to Weights and Biases.

- **`WandbModelCheckpoint`** : Use this callback to log your model checkpoints to Weight and Biases [Artifacts]({{< relref path="/guides/core/artifacts/" lang="ko" >}}).

- **`WandbEvalCallback`**: This base callback logs model predictions to Weights and Biases [Tables]({{< relref path="/guides/models/tables/" lang="ko" >}}) for interactive visualization.
...
{{% alert %}}
The W&B [Artifact page]({{< relref path="/guides/core/artifacts/explore-and-traverse-an-artifact-graph" lang="ko" >}}) includes Table logs by default, rather than the **Workspace** page.
{{% /alert %}}
...
Use the W&B library [`WandbCallback`]({{< relref path="/ref/python/integrations/keras/wandbcallback" lang="ko" >}}) Class to automatically save all the metrics and the loss values tracked in `model.fit`.
...
Check out [the reference documentation for the `keras.WandbCallback`]({{< relref path="/ref/python/integrations/keras/wandbcallback.md" lang="ko" >}}) for full details.
