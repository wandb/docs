...
* Passing a number to `--save_period` will turn on [model versioning]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}}). At the end of every `save_period` epochs, the model weights will be saved to W&B. The best-performing model on the validation set will be tagged automatically.
