...
If you are using W&B for the first time you might want to check out our [**quickstart**]({{< relref path="/guides/quickstart.md" lang="ko" >}})
...
Using W&B's [Artifacts]({{< relref path="/guides/core/artifacts/" lang="ko" >}}), you can store up to 100GB of models and datasets for free and then use the W&B [Model Registry]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}}) to register models to prepare them for staging or deployment in your production environment.
...
#### W&B Model Registry
Once you have logged your checkpoints to Artifacts, you can then register your best model checkpoints and centralize them across your team using the  **[Model Registry]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}})**. Here you can organize your best models by task, manage model lifecycle, facilitate easy tracking and auditing throughout the ML lifecyle, and [automate]({{< relref path="/guides/core/automations/project-scoped-automations/#create-a-webhook-automation" lang="ko" >}}) downstream actions with webhooks or jobs. 

See the [Model Registry]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}}) documentation for how to link a model Artifact to the Model Registry.
...
See the **[Custom logging section]({{< relref path="#custom-logging-log-and-view-evaluation-samples-during-training" lang="ko" >}})** below for a full guide on how to log evaluation outupts while training to log to a W&B Table like this:
...
Once you have logged your training results you can explore your results dynamically in the [W&B Dashboard]({{< relref path="/guides/models/track/workspaces.md" lang="ko" >}}). It's easy to compare across dozens of runs at once, zoom in on interesting findings, and coax insights out of complex data with flexible, interactive visualizations.
...
If you'd like to centralize all your best model versions across your team to organize them by ML task, stage them for production, bookmark them for further evaluation, or kick off downstream Model CI/CD processes then ensure you're saving your model checkpoints to Artifacts. Once logged to Artifacts, these checkpoints can then be promoted to the [Model Registry]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}}).
...
Further configuration of what is logged with `Trainer` is possible by setting environment variables. A full list of W&B environment variables [can be found here]({{< relref path="/guides/hosting/env-vars/" lang="ko" >}}).
...
An example of what you might want to pass to `init` is below. For more details on how to use `wandb.init`, [check out the reference documentation]({{< relref path="/ref/python/init.md" lang="ko" >}}).
