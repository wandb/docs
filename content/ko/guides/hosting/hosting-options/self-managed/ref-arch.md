...
To learn more about managed solutions for running W&B instead of managing it yourself, refer to [W&B Multi-tenant Cloud]({{< relref path="/guides/hosting/hosting-options/saas_cloud.md" lang="ko" >}}) and [W&B Dedicated Cloud]({{< relref path="/guides/hosting/hosting-options/dedicated_cloud.md" lang="ko" >}}).
...
### Kubernetes
The W&B Server application is deployed as a [Kubernetes Operator]({{< relref path="kubernetes-operator/" lang="ko" >}}) that deploys multiple pods. For this reason, W&B requires a Kubernetes cluster with:
...
To learn about air-gapped deployments, refer to [Kubernetes operator for air-gapped instances]({{< relref path="kubernetes-operator/operator-airgapped.md" lang="ko" >}}).
