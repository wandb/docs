...
2. Next,  start a new W&B Run to track and log to W&B with [`wandb.init()`]({{< relref path="/ref/python/init.md" lang="ko" >}}):
...
As an experiment runs, you might want to log every instance of your metrics so they are available to view, query, and analyze with W&B. Use the [`run.log()`]({{< relref path="/ref/python/log.md" lang="ko" >}}) command to accomplish this:

```python
run.log({key: val})
```

You can optionally log a final summary metric to define the outcome of the run. Use the W&B  [`define_metric`]({{< relref path="/ref/python/run.md#define_metric" lang="ko" >}}) API to accomplish this. In this example case, we will add the summary metrics to our run with `run.summary.update()`:
