...
Note that when you link a model, that model is not duplicated in the [Model Registry]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}}). That model is also not moved out of the project and intro the registry. A linked model is a pointer to the original model in your project.

Use the [Model Registry]({{< relref path="/guides/core/registry/model_registry/" lang="ko" >}}) to organize your best models by task, manage model lifecycle, facilitate easy tracking and auditing throughout the ML lifecyle, and [automate]({{< relref path="/guides/core/automations/model-registry-automations.md" lang="ko" >}}) downstream actions with webhooks or jobs.
{{% /alert %}}
