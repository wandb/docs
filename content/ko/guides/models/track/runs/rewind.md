...
* You must use monotonically increasing steps. You can not use non-monotonic steps defined with [`define_metric()`]({{< relref path="/ref/python/run#define_metric" lang="ko" >}}) because it disrupts the required chronological order of run history and system metrics.
{{% /alert %}}
...
Use `resume_from` with [`wandb.init()`]({{< relref path="/ref/python/init" lang="ko" >}}) to "rewind" a run’s history to a specific step. Specify the name of the run and the step you want to rewind from:
