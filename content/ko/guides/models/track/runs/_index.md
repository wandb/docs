...
W&B stores runs that you create into [*projects*]({{< relref path="/guides/models/track/project-page.md" lang="ko" >}}). You can view runs and their properties within the run's project workspace on the W&B App. You can also programmatically access run properties with the [`wandb.Api.Run`]({{< relref path="/ref/python/sdk/classes/run.md" lang="ko" >}}) object.
...
Initialize a W&B Run with [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init" lang="ko" >}}). The proceeding code snippet shows how to import the W&B Python SDK and initialize a run. 
...
Each run also has a human-readable, non-unique [run name]({{< relref path="#name-your-run" lang="ko" >}}). You can specify a name for your run or let W&B randomly generate one for you. You can rename a run after initializing it.
...
If you [group runs]({{< relref path="grouping.md" lang="ko" >}}) into experiments, you can move a run into or out of a group or from one group to another.
...
### Custom run IDs
You can specify your own run ID by passing the `id` parameter to the [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init" lang="ko" >}}) method. 
...
You can specify a name for your run by passing the `name` parameter to the [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init" lang="ko" >}}) method. 
...
You can also rename a run from a run set in a [report]({{< relref path="/guides/core/reports/edit-a-report.md" lang="ko" >}}):
...
See the [**Runs** tab]({{< relref path="/guides/models/track/project-page.md#runs-tab" lang="ko" >}}).
...
* **Config**: List of config parameters saved with [`wandb.Run.config`]({{< relref path="/guides/models/track/config.md" lang="ko" >}}).
* **Summary**: List of summary parameters saved with [`wandb.Run.log()`]({{< relref path="/guides/models/track/log/" lang="ko" >}}). By default, W&B sets this value to the last value logged.
...
{{% alert %}}
The **Group by** feature is distinct from a [run's run group]({{< relref path="grouping.md" lang="ko" >}}). You can group runs by run group. To move a run to a different run group, refer to [Assign a group or job type to a run]({{< relref path="#assign-a-group-or-job-type-to-a-run" lang="ko" >}}).
{{% /alert %}}
...
View [example artifact graphs]({{< relref path="/guides/core/artifacts/explore-and-traverse-an-artifact-graph.md" lang="ko" >}}).
