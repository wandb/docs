...
At the beginning of your script call, the [`wandb.init()`]({{< relref path="/ref/python/init.md" lang="ko" >}}) API to generate a background process to sync and log data as a W&B Run. 
...
```
A [Run]({{< relref path="/ref/python/run.md" lang="ko" >}}) object is returned when you initialize W&B with `wandb.init()`. Additionally, W&B creates a local directory where all logs and files are saved and streamed asynchronously to a W&B server.
...
Log metrics during each `for` loop (epoch), the accuracy and loss values are computed and logged to W&B with [`wandb.log()`]({{< relref path="/ref/python/log.md" lang="ko" >}}). By default, when you call wandb.log it appends a new step to the history object and updates the summary object.
...
```
For more information about Artifacts, see the [Artifacts Chapter]({{< relref path="/guides/core/artifacts/" lang="ko" >}}). For more information about versioning models, see [Model Management]({{< relref path="/guides/models/registry/model_registry/" lang="ko" >}}).
...
For more information about available parameters when defining a W&B Experiment, see the [`wandb.init`]({{< relref path="/ref/python/init.md" lang="ko" >}}) API docs in the [API Reference Guide]({{< relref path="/ref/python/" lang="ko" >}}).