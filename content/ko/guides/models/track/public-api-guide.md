...
`api.runs`를 호출하면 iterable이고 목록처럼 작동하는 `Runs` 오브젝트가 반환됩니다. 기본적으로 오브젝트는 필요에 따라 한 번에 50개의 Runs을 순서대로 로드하지만 `per_page` 키워드 인수를 사용하여 페이지당 로드되는 수를 변경할 수 있습니다.
...
이 코드 조각은 특정 스윕과 연결된 모든 Runs을 다운로드합니다.
