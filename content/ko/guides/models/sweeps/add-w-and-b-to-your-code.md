...
3. Pass the sweep configuration dictionary to [`wandb.sweep()`]({{< relref path="/ref/python/sdk/functions/sweep.md" lang="ko" >}}). This initializes the sweep. This returns a sweep ID (`sweep_id`). For more information, see [Initialize sweeps]({{< relref path="./initialize-sweeps.md" lang="ko" >}}).
4. Use the [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init.md" lang="ko" >}}) API to generate a background process to sync and log data as a [W&B Run]({{< relref path="/ref/python/sdk/classes/run.md" lang="ko" >}}).
5. (Optional) define values from `wandb.config` instead of defining hard coded values.
6. Log the metric you want to optimize with [`wandb.Run.log()`]({{< relref path="/ref/python/sdk/classes/run.md/#method-runlog" lang="ko" >}}). You must log the metric defined in your configuration. Within the configuration dictionary (`sweep_configuration` in this example), you define the sweep to maximize the `val_acc` value.
7. Start the sweep with the [`wandb.agent`]({{< relref path="/ref/python/sdk/functions/agent.md" lang="ko" >}}) API call. Provide the sweep ID and the name of the function the sweep will execute (`function=main`), and specify the maximum number of runs to try to four (`count=4`). For more informationp, see [Start sweep agents]({{< relref path="./start-sweep-agents.md" lang="ko" >}}).
...
[`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init.md" lang="ko" >}}) API within a `with`
context manager statement to generate a background process to sync and log data
as a [W&B Run]({{< relref path="/ref/python/sdk/classes/run.md" lang="ko" >}}). This ensures the run is
...
3. Use the [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init.md" lang="ko" >}}) API to generate a background process to sync and log data as a [W&B Run]({{< relref path="/ref/python/sdk/classes/run.md" lang="ko" >}}). We pass the config object to the config parameter.
4. Define hyperparameter values from `wandb.Run.config` instead of using hard coded values.
5. Log the metric we want to optimize with [`wandb.Run.log()`]({{< relref path="/ref/python/sdk/classes/run.md/#method-runlog" lang="ko" >}}). You must log the metric defined in your configuration. Within the configuration dictionary (`sweep_configuration` in this example) we defined the sweep to maximize the `val_acc` value.
...
the sweep job with the [`wandb agent`]({{< relref path="/ref/cli/wandb-agent.md" lang="ko" >}})
command:
...
For more information, see [Start sweep jobs]({{< relref path="./start-sweep-agents.md" lang="ko" >}}).
