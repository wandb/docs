...
-  Use [reference artifacts]({{< relref path="/guides/core/artifacts/track-external-files.md" lang="ko" >}}) to track files saved outside the W&B system, instead of uploading them to W&B storage.
- Use an [external cloud storage bucket]({{< relref path="teams.md" lang="ko" >}}) for storage. *(Enterprise only)*
...
- [Set a TTL policy]({{< relref path="/guides/core/artifacts/manage-data/ttl.md" lang="ko" >}}) on Artifacts so they are automatically deleted.