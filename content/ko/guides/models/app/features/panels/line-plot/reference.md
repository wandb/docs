...
You can set the [smoothing coefficient]({{< relref path="/support/kb-articles/formula_smoothing_algorithm.md" lang="ko" >}}) to be between 0 and 1 where 0 is no smoothing and 1 is maximum smoothing.
...
- For plots that use [random sampling mode]({{< relref path="sampling.md#random-sampling" lang="ko" >}}), when you enable **Ignore outliers**, only points from 5% to 95% are shown. When outliers are shown, they are not formatted differently from other points.
- For plots that use [full fidelity mode]({{< relref path="sampling.md#full-fidelity" lang="ko" >}}), all points are always shown, condensed down to the last value in each bucket. When **Ignore outliers** is enabled, the minimum and maximum bounds of each bucket are shaded. Otherwise, no area is shaded.
