...
{{% alert %}}
The term "type" refers to the artifact object's type. When you create an artifact object ([`wandb.Artifact`]({{< relref path="/ref/python/sdk/classes/artifact.md" lang="ko" >}})), or log an artifact ([`wandb.init.log_artifact`]({{< relref path="/ref/python/sdk/classes/run.md#log_artifact" lang="ko" >}})), you specify a type for the `type` parameter. 
...
Programmatically link an artifact version to a collection with [`wandb.init.Run.link_artifact()`]({{< relref path="/ref/python/sdk/classes/run.md#link_artifact" lang="ko" >}}).
...
1. Specify the team as the entity when you initialize a run with [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init.md" lang="ko" >}}). If you do not specify the `entity` when you initialize a run, the run uses your default entity which may or may not be your team entity.
...
2. Log the artifact to the run either with run.log_artifact or by creating an Artifact object and then adding files to it with:
    ...
    ```
    To log artifacts, see [Construct artifacts]({{< relref path="/guides/core/artifacts/construct-an-artifact.md" lang="ko" >}}).
