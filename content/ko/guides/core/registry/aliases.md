...
Reference a specific [artifact version]({{< relref path="guides/core/artifacts/create-a-new-artifact-version" lang="ko" >}}) with one or more aliases. [W&B automatically assigns aliases]({{< relref path="aliases#default-aliases" lang="ko" >}}) to each artifact you link with the same name. You can also [create one or more custom aliases]({{< relref path="aliases#custom-aliases" lang="ko" >}}) to reference a specific artifact version.

Aliases appear as rectangles with the name of that alias in the rectangle in the Registry UI. If an [alias is protected]({{< relref path="aliases#protected-aliases" lang="ko" >}}), it appears as a gray rectangle with a lock icon. Otherwise, the alias appears as an orange rectangle. Aliases are not shared across registries.
...
When you add an alias to an artifact version, you can optionally start a [Registry automation]({{< relref path="/guides/core/automations/automation-events/#registry" lang="ko" >}}) to notify a Slack channel or trigger a webhook.
...
Any user with a [Member or Admin registry role]({{< relref path="guides/core/registry/configure_registry/#registry-roles" lang="ko" >}}) on a registry can add or remove a custom alias from a linked artifact in that registry. If appropriate, use [protected aliases]({{< relref path="aliases/#protected-aliases" lang="ko" >}}) to label and identify which artifact versions to protect from modification or deletion.
...
### Protected aliases
Use a [protected alias]({{< relref path="aliases/#protected-aliases" lang="ko" >}}) to both label and identify artifact versions that should not be modified or deleted. For example, consider using a `production` protected alias to label and identify artifact versions that are in used in your organization's machine learning production pipeline.

[Registry admins]({{< relref path="/guides/core/registry/configure_registry/#registry-roles" lang="ko" >}}) and [service accounts]({{< relref path="/support/kb-articles/service_account_useful" lang="ko" >}}) with the Admin role can create protected aliases and add or remove protected aliases from an artifact version. Members and Viewers cannot unlink a protected version or delete a collection that contains a protected . See [Configure registry access]({{< relref path="/guides/core/registry/configure_registry.md" lang="ko" >}}) for details.
...
You can find existing aliases with the [global search bar in the W&B Registry]({{< relref path="/guides/core/registry/search_registry/#search-for-registry-items" lang="ko" >}}). To find a protected alias:
