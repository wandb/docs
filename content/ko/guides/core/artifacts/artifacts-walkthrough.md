...
Use the [`wandb.init()`]({{< relref path="/ref/python/sdk/functions/init.md" lang="ko" >}}) API to generate a background process to sync and log data as a W&B Run. Provide a project name and a job type:
...
Create an artifact object with the [`wandb.Artifact()`]({{< relref path="/ref/python/sdk/classes/artifact.md" lang="ko" >}}) API. Provide a name for the artifact and a description of the file type for the `name` and `type` parameters, respectively.
...
2. Second, use the run objects [`use_artifact()`]({{< relref path="/ref/python/sdk/classes/run.md#use_artifact" lang="ko" >}}) method to tell W&B what artifact to use. This returns an artifact object.
3. Third, use the artifacts [`download()`]({{< relref path="/ref/python/sdk/classes/artifact.md#download" lang="ko" >}}) method to download the contents of the artifact.
