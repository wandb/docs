...
Bring your own bucket (BYOB) allows you to store W&B artifacts and other related sensitive data in your own cloud or on-prem infrastructure. In case of [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}) or [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}), data that you store in your bucket is not copied to the W&B managed infrastructure.

{{% alert %}}
* Communication between W&B SDK / CLI / UI and your buckets occurs using [pre-signed URLs]({{< relref "./presigned-urls.md" >}}).
* W&B uses a garbage collection process to delete W&B Artifacts. For more information, see [Deleting Artifacts]({{< relref "../../artifacts/delete-artifacts.md" >}}).
...
{{% alert %}}
Team level storage bucket provides the same benefits for [Self-managed]({{< relref "../hosting-options/self-managed.md" >}}) instances, especially when different business units and departments share an instance to efficiently utilize the infrastructure and administrative resources. This also applies to firms that have separate project teams managing AI workflows for separate customer engagements.
{{% /alert %}}
...
You can connect to a cloud-native storage bucket in another cloud or to an S3-compatible storage bucket like [MinIO](https://github.com/minio/minio) for team-level BYOB in your [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}) or [Self-managed]({{< relref "../hosting-options/self-managed.md" >}}) instance.
...
{{% alert %}}
Connectivity to S3-compatible storage for team-level BYOB is not available in [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}). Also, connectivity to an AWS bucket for team-level BYOB is cross-cloud in [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}), as that instance resides in GCP. That cross-cloud connectivity doesn't use the access key and environment variable based mechanism as outlined previously for [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}) and [Self-managed]({{< relref "../hosting-options/self-managed.md" >}}) instances.
{{% /alert %}}
...
1. Provision the KMS Key
    ...
    If you are using [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}) or [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}), replace `<aws_principal_and_role_arn>` with the corresponding value:

    * For [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}): `arn:aws:iam::725579432336:role/WandbIntegration`
    * For [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}): `arn:aws:iam::830241207209:root`
...
2. Provision the S3 Bucket
  ...
  For more details, see the [AWS self-managed hosting guide]({{< relref "../self-managed/aws-tf.md" >}}).
{{% /tab %}}
...
2. If you are using [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}) or [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}), grant the `Storage Admin` role to the GCP service account linked to the W&B Platform:

    * For [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}}), the account is: `wandb-integration@wandb-production.iam.gserviceaccount.com`
    * For [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}) the account is: `deploy@wandb-production.iam.gserviceaccount.com`
...
2. Generate a storage account access key, and keep a record of that along with the storage account name. If you are using [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}), share the storage account name and access key with your W&B team using a secure sharing mechanism.

    For the team level BYOB, W&B recommends that you use [Terraform](https://github.com/wandb/terraform-azurerm-wandb/tree/main/modules/secure_storage_connector) to provision the Azure Blob Storage bucket along with the necessary access mechanism and permissions. If you use [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}), provide the OIDC issuer URL for your instance. Make a note of details that you need to [configure the bucket while creating the team]({{< relref "#configure-byob-in-wb" >}}):
...
5. Provide the name of your storage bucket for the **Name** field. If you have a [Dedicated cloud]({{< relref "../hosting-options/dedicated_cloud.md" >}}) or [Self-managed]({{< relref "../hosting-options/self-managed.md" >}}) instance on Azure, provide the values for **Account name** and **Container name** fields.
...
9. (Optional on [SaaS Cloud]({{< relref "../hosting-options/saas_cloud.md" >}})) Optionally invite team members when creating the team.
