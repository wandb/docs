...
Simply by installing `wandb`, you'll activate the built-in W&B [logging features]({{< relref "../track/log/intro.md" >}}): system metrics, model metrics, and media logged to interactive [Dashboards]({{< relref "../track/workspaces.md" >}}).
...
* Passing a number to `--save_period` will turn on [model versioning]({{< relref "../model_registry/intro.md" >}}). At the end of every `save_period` epochs, the model weights will be saved to W&B. The best-performing model on the validation set will be tagged automatically.
* Turning on the `--upload_dataset` flag will also upload the dataset for data versioning.
* Passing a number to `--bbox_interval` will turn on [data visualization]({{< relref "../intro.md" >}}). At the end of every `bbox_interval` epochs, the outputs of the model on the validation set will be uploaded to W&B.
