...
| `model_log_interval`   | `Optional int`. `None` by default. If set, [model versioning]({{< relref "../model_registry/" >}}) with [Artifacts]({{< relref "../artifacts/" >}})will be enabled. Pass in the number of steps to wait between logging model checkpoints. `None` by default. |
...
When training begins, a link to your training run's [W&B page]({{< relref "../runs/" >}}) will be output which will take you to this run's experiment tracking [dashboard]({{< relref "../track/workspaces.md" >}}) in the Weights & Biases web UI.