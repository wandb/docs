...
| `model_log_interval`   | `Optional int`. `None` by default. If set, [model versioning](../model_registry/) with [Artifacts](../artifacts/)will be enabled. Pass in the number of steps to wait between logging model checkpoints. `None` by default. |
...
When training begins, a link to your training run's [W&B page](../runs/) will be output which will take you to this run's experiment tracking [dashboard](../track/workspaces/) in the Weights & Biases web UI.