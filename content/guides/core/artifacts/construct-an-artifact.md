...
Use the W&B Python SDK to construct artifacts from [W&B Runs]({{< relref "../../ref/python/run.md" >}}). You can add [files, directories, URIs, and files from parallel runs to artifacts]({{< relref "#add-files-to-an-artifact" >}}). After you add a file to an artifact, save the artifact to the W&B Server or [your own private server]({{< relref "../hosting/hosting-options/self-managed.md" >}}).
