...
This is in contrast to automations created in the W&B Model Registry. Automations created in the model registry are in scope of the Model Registry. They are triggered when events are performed on model versions linked to the [Model Registry]({{< relref "/guides/core/registry/model_registry/" >}}). For information on how to create an automations for model versions, see the [Automations for Model CI/CD]({{< relref "/guides/core/automations/model-registry-automations.md" >}}) page in the [Model Registry chapter]({{< relref "/guides/core/registry/model_registry/" >}}).
{{% /alert %}}
