...
When you add an alias to an artifact version, you can optionally trigger a workflow step such a Slack notification or webhook. For more information, see the [Registry section]({{< relref  "/guides/core/automations/automation-events/#registry" >}}) in [Automation event and scopes]({{< relref "/guides/core/automations/automation-events/" >}}).
...
Only [registry admins]({{< relref "/guides/core/registry/configure_registry/#registry-roles" >}}) can create protected aliases and add or remove protected aliases from an artifact version. Users (Member and Viewer roles) can not delete a collection that contains a version with a protected alias, and they also cannot unlink it from the registry. See [Configure registry access]({{< relref "/guides/core/registry/configure_registry.md" >}}) for information on how to manage users and assign roles in a registry.
...
The following code snippet demonstrates how to both link an artifact version to a collection and add aliases to that artifact version with the Python SDK. Replace values within `<>` with your own:
...
# Link the artifact version to the collection
# Add one or more protected aliases to this artifact version
run.link_artifact(
    artifact = artifact, 
    target_path = target_path, 
    aliases = ["<alias_1>", "<alias_2>"]
