...
* Source version: the artifact version inside a team's project that is logged to a [run]({{< relref "../runs/intro.md" >}}).
...
2. Log the artifact to the run either with run.log_artifact or by creating an Artifact object and then adding files to it with  :
    ...
    ```
    For more information on how to log artifacts, see [Construct artifacts]({{< relref "../artifacts/construct-an-artifact.md" >}}).
