...
This is in contrast to automations created in the W&B Model Registry. Automations created in the model registry are in scope of the Model Registry. They are triggered when events are performed on model versions linked to the [Model Registry](../model_registry/). For information on how to create an automations for model versions, see the [Automations for Model CI/CD](../model_registry/model-registry-automations/) page in the [Model Registry chapter](../model_registry/).
{{% /alert %}}
...
* Secrets are also available if you use [W&B Server](../hosting/) in an Azure, GCP, or AWS deployment. Connect with your W&B account team to discuss how you can use secrets in W&B if you use a different deployment type.
{{% /alert %}}
