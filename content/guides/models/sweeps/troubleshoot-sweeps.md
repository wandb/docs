...
Make a note of the W&B Sweep ID that is returned. Next, start the Sweep job with [`wandb agent`](../../ref/cli/wandb-agent/) with the CLI instead of the Python SDK ([`wandb.agent`](../../ref/python/agent/)). Replace `sweep_ID` in the code snippet below with the Sweep ID that was returned in the previous step:
