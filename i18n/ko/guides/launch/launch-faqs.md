...
“다음 Kubernetes 매니페스트는 `wandb` 네임스페이스에 `wandb-launch-agent`라는 역할을 생성합니다. 이 역할은 에이전트가 `wandb` 네임스페이스에서 파드, configmaps, 비밀, 및 파드/로그를 생성할 수 있도록 허용합니다. `wandb-cluster-role`은 에이전트가 선택한 네임스페이스의 모든 네임스페이스에서 파드, 파드/로그, 비밀, 작업, 및 작업/상태를 생성할 수 있도록 허용합니다.”

### Launch는 병렬화를 지원하나요? 작업에 의해 소비되는 리소스를 어떻게 제한할 수 있나요?
   
네, Launch는 여러 GPU 및 여러 노드에 걸쳐 작업을 확장하는 것을 지원합니다. 자세한 내용은 [이 가이드](https://docs.wandb.ai/tutorials/volcano)를 참조하세요.

작업 간 수준에서, 개별 런치 에이전트는 에이전트가 동시에 실행할 수 있는 작업의 수를 결정하는 `max_jobs` 파라미터로 구성됩니다. 또한, 해당 에이전트가 실행할 수 있는 인프라에 연결된 한, 특정 큐를 가리키는 여러 에이전트를 원하는 만큼 가리킬 수 있습니다.
  
CPU/GPU, 메모리 및 기타 요구 사항을 런치 큐 또는 작업 실행 수준에서 제한할 수 있으며, 리소스 구성에서 설정할 수 있습니다. Kubernetes에서 리소스 제한이 있는 큐를 설정하는 방법에 대한 자세한 내용은 [여기](https://docs.wandb.ai/guides/launch/kubernetes#queue-configuration)를 참조하세요.
...
### Docker 큐를 사용하여 동일한 아티팩트를 `use_artifact`로 다운로드하는 여러 작업을 실행할 때, 작업의 각 실행에 대해 아티팩트를 다시 다운로드하나요, 아니면 내부적으로 캐싱이 이루어지나요?
...
### 작업/자동화를 위해 비밀을 지정할 수 있나요? 예를 들어, 사용자에게 직접 보이지 않기를 원하는 API 키와 같은 것입니다?
...
### 관리자가 ML 엔지니어가 수정할 수 있는 내용을 어떻게 제한할 수 있나요? 예를 들어, 이미지 태그를 변경하는 것은 괜찮지만 다른 작업 설정은 그렇지 않을 수 있습니다.
...
이미지를 빌드하는 단계는 실행되는 작업의 출처와 리소스 구성에 지정된 가속기 베이스 이미지의 여부에 따라 다릅니다.

:::note
큐 구성을 지정하거나 작업을 제출할 때, 큐 또는 작업 리소스 구성에서 베이스 가속기 이미지를 제공할 수 있습니다:
```json
{
    "builder": {
        "accelerator": {
...
빌드 프로세스 중 다음 작업이 수행되며, 작업 유형과 제공된 가속기 베이스 이미지에 따라 다릅니다:

|                                                     | apt를 사용하여 파이썬 설치 | 파이썬 패키지 설치 | 사용자 및 작업 디렉토리 생성 | 코드를 이미지에 복사 | 엔트리포인트 설정 |
|-----------------------------------------------------|:------------------------:|:-------------------:|:-------------------------:|:-------------------:|:----------------:|
| git에서 출처를 가진 작업                              |                          |            X         |             X              |           X         |        X          |
| 코드에서 출처를 가진 작업                             |                          |            X         |             X              |           X         |        X          |
| git에서 출처를 가진 작업 및 제공된 가속기 이미지     |             X            |            X         |             X              |           X         |        X          |
| 코드에서 출처를 가진 작업 및 제공된 가속기 이미지    |             X            |            X         |             X              |           X         |        X          |
| 이미지에서 출처를 가진 작업                           |                          |                      |                            |                     |                   |

### 가속기 베이스 이미지에 대한 요구 사항은 무엇인가요?
가속기를 사용하는 작업의 경우, 필요한 가속기 구성 요소가 설치된 가속기 베이스 이미지를 제공할 수 있습니다. 제공된 가속기 이미지에 대한 기타 요구 사항은 다음과 같습니다:
...
GPU에서 tensorflow를 사용하는 작업의 경우, 실행이 GPU를 제대로 활용할 수 있도록 에이전트가 수행할 컨테이너 빌드에 대한 사용자 지정 베이스 이미지를 지정해야 할 수도 있습니다. 이는 리소스 구성의 `builder.accelerator.base_image` 키 아래에 이미지 태그를 추가함으로써 수행될 수 있습니다. 예를 들어:
