...
config = Path("hyperparameters.yaml").read_bytes()
...
with Sandbox.run(
    ...
    sandbox.write_file("hyperparameters.yaml", config).result()
    ...
    process = sandbox.exec(["python", "train.py", "--config", "hyperparameters.yaml"])
