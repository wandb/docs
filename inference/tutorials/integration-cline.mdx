...
[Cline](https://cline.bot) is an AI-powered coding assistant. This tutorial demonstrates how to configure Cline to use models provided by Serverless Inference, so that you can use open-weight models hosted by CoreWeave for your coding tasks.
...
### Use the native CoreWeave provider
...
The provider's command-line ID is `wandb`. Cline versions that use the older branding can display the provider as **Weights & Biases** or **W&B**. Cline uses that provider's configured Serverless Inference endpoint. If Cline responds with an answer, the connection is working.
