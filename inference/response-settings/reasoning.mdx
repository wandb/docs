...
The following table lists the models on Serverless Inference that can return reasoning output.
Some reasoning models don't provide a user option to disable reasoning. Other models allow the user to control reasoning output, with the default behavior when unspecified listed below.
Adaptive models determine on a per-request basis whether to return reasoning output, but allow the user to control output also.
