...
Compare two W&B Tables logged as artifact versions to analyze changes in your data or model performance. Use the merged view or side-by-side view to inspect differences between tables. The following sections describe two common scenarios: comparing the same model over time and comparing different model variants.
...
This section describes how W&B preserves table state across sessions and how to export a table snapshot to a report. Tables in a run workspace, project workspace, or report automatically save their view state. When you apply table operations and return to the table later, the table restores your last viewed configuration.
...
The following reports highlight the different use cases of W&B Tables:
