...
```
View recorded metrics such as accuracy and loss, and observe how they changed during each training step. You can view and monitor runs using the W&B App or the `wandb beta leet` terminal UI.
<Tabs>
<Tab title="App">
Visit [wandb.ai/home](https://wandb.ai/home). The following image shows the loss and accuracy tracked from each run. Each run object appears in the **Runs** column with generated names.
...
<Tab title="LEET">
In your terminal, start the `wandb beta leet` terminal UI:
```bash
wandb beta leet
```
...
See the [`wandb beta leet`](/models/ref/cli/wandb-beta/wandb-beta-leet) reference for details.
