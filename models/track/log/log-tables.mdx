...
After you create a table, populate it with the rows or columns you want to track. As your script executes, you can add more data to your table, up to 200,000 rows. You can add data to a table in two ways:
...
After data is in a table, you can access the data by column or by row:

1. Row iteration: Use Table.iterrows() to iterate over the data's rows. For example, `for ndx, row in table.iterrows(): ...`.
2. Retrieve a column: Retrieve a column of data using `table.get_column("col_name")`. As a convenience, you can pass `convert_to="numpy"` to convert the column to a NumPy NDArray of primitives. This is useful if your column contains media types such as `wandb.Image`, so that you can access the underlying data directly.
...
Use `wandb.Run.log()` to save your table to the run. For example, the following code snippet first creates a table object before logging the table to the run with `wandb.Run.log()`:
...
However, this can cause performance issues, such as slower queries, in the UI.
</Note>
...
For more information about artifacts, see [Artifacts](/models/artifacts/).

### Visualize tables

View tables you log in your project's workspace or run page. For more information, see [Visualize and Analyze Tables](/models/tables/visualize-tables/).
...
Join tables you have locally constructed or tables you have retrieved from other artifacts with `wandb.JoinedTable(table_1, table_2, join_key)`.
...
To join two tables you logged previously in an artifact context, fetch them from the artifact and join the result into a new table.

For example, the following code reads one table of original songs called `'original_songs'` and another table of synthesized versions of the same songs called `'synth_songs'`. It joins the two tables on `"song_id"` and uploads the result as a new W&B Table:
...
[Read this tutorial](https://wandb.ai/stacey/cshanty/reports/Whale2Song-W-B-Tables-for-Audio--Vmlldzo4NDI3NzM) for an example of combining two tables stored in different artifact objects.
