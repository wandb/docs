...
Access a run’s summary in W&B, during an active run, from an existing run, or from the local file system.

<Tabs>
<Tab title="Run Overview">
1. Navigate to W&B.
1. Select the **Workspace** tab from the project sidebar.
1. Click the run that logged the summary values. The run page opens with the **Overview** tab shown by default.
1. View the summary values in the **Summary** section.
...
1. Within the runs table, you can view the summary values within the columns based on the name of the summary value.
</Tab>
<Tab title="During a run">
