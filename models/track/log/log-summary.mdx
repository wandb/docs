...
In addition to values that change over time during training, it is often important to track a single value that summarizes a model or a preprocessing step. Log this information in run's `summary` dictionary. A run's summary dictionary can handle numpy arrays, PyTorch tensors or TensorFlow tensors.

<Info>
The last value logged with `wandb.Run.log()` is automatically set as the summary dictionary in a run. If a summary metric dictionary is modified, the previous value is lost.
</Info>

The following code snippet demonstrates how to provide a summary metric for the best accuracy value during training:
...
Use `wandb.Run.define_metric()` to specify how metrics should be aggregated in your run's summary instead of using the last logged value. For example, you might want to capture the maximum accuracy or the minimum loss value, instead of the final value.

To customize summary metrics, pass the `summary` argument in `define_metric`. It accepts the following values:
