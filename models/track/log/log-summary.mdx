...
In addition to values that change over time during training, it's often important to track a single value that summarizes a model or a preprocessing step. Log this information in a W&B run's `summary` dictionary. A run's summary dictionary can handle numpy arrays, PyTorch tensors, TensorFlow tensors, and more. When a value is one of these types, W&B persists the entire tensor in a binary file and stores high-level metrics in the summary object, such as min, mean, variance, and percentiles.
