...
In this approach you use a primary node and one or more worker nodes. Before you launch the nodes, generate a run ID and configure every node to use that same run ID. During training, each node logs to the same run ID. W&B aggregates metrics from all nodes and displays them in the W&B App UI.

Generate the run ID in your launcher, scheduler, or orchestration framework, then set the `WANDB_RUN_ID` environment variable for every node before the training process starts. W&B reads `WANDB_RUN_ID` automatically when you call `wandb.init()`. Do not read `WANDB_RUN_ID` in your training code or use `WANDB_` environment variables for custom coordination logic.
...
2. Optionally set [`x_update_finish_state`](https://github.com/wandb/wandb/blob/main/wandb/sdk/wandb_settings.py#L772) to `False`. This prevents non-primary nodes from updating the [run's state](/models/runs/run-states#run-states) to `finished` prematurely, ensuring the run state remains consistent and managed by the primary node.

<Note>
* Use the same entity and project for all nodes. This helps ensure the correct run ID is found.
* Set `WANDB_RUN_ID` for all nodes before each process starts. The exact mechanism depends on your launcher, scheduler, or orchestration framework.
* Do not pass the primary node's `WANDB_SERVICE` environment variable to worker nodes. Each worker must start its own W&B SDK backend. Configure this in the environment you use to launch workers instead of modifying `os.environ` after the process has started.
</Note>

The following sample code demonstrates the high level requirements for tracking multiple processes to a single run. It assumes that your launcher sets the same `WANDB_RUN_ID` for every node before starting the training process:
...
rank = 0  # Set this from your launcher, scheduler, or framework.
...
# W&B automatically uses WANDB_RUN_ID from the process environment.
run = wandb.init(
    entity=entity,
    project=project,
    settings=settings,
