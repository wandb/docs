...
* **Track multiple processes**: For multiple processes, you can either:
   * Track each process separately using one run per process. You can optionally group them together in W&B.
...
1. A `wandb.Settings` object to the `settings` parameter (`wandb.init(settings=wandb.Settings()`) with:
   ...
   * A unique label for `x_label`. You use the value you specify for `x_label` to identify which node the data is coming from in logs and system metrics in W&B. If left unspecified, W&B creates a label for you using the hostname and a random hash.
...
W&B aggregates system metrics from all nodes and displays them in the W&B UI. For example, the following image shows a sample dashboard with system metrics from multiple nodes. Each node possesses a unique label (`rank_0`, `rank_1`, `rank_2`) that you specify in the `x_label` parameter.
