...
This page shows you how to log rich media, including images, video, audio, and more, so you can explore your results and visually compare your runs, models, and datasets. Use these examples as a reference when you want to attach media objects to a W&B run.
...
To log media objects with the W&B Python SDK, you may need to install additional dependencies that support the media types you plan to use. Install them by running the following command.
...
W&B converts arrays to PNG using [Pillow](https://pillow.readthedocs.io/en/stable/index.html).
...
* `class_labels`: (optional) a dictionary mapping `class_id`s to strings. By default, W&B generates class labels such as `class_0` and `class_1`.
...
If you provide a sequence of numbers, such as a list, array, or tensor, as the first argument, W&B constructs the histogram automatically by calling `np.histogram()`. W&B flattens all arrays/tensors. Use the optional `num_bins` keyword argument to override the default of `64` bins. The maximum number of bins supported is `512`.
...
Logged point clouds are fully interactive in the W&B app. For example, the [LIDAR Point Clouds of Driving Scenes report](https://wandb.ai/stacey/lyft/reports/LIDAR-Point-Clouds-of-Driving-Scenes--Vmlldzo2MzA5Mg) embedded below shows scenes from the Lyft dataset with 3D bounding boxes. Drag to rotate and zoom within the viewer.
...
W&B supports three different formats of NumPy arrays for flexible color schemes.
...
If you supply a NumPy array, W&B assumes the dimensions are, in order: time, channels, width, and height. By default, W&B creates a 4 fps GIF image ([`ffmpeg`](https://www.ffmpeg.org) and the [`moviepy`](https://pypi.org/project/moviepy/) Python library are required when you pass NumPy objects). Supported formats are `"gif"`, `"mp4"`, `"webm"`, and `"ogg"`. If you pass a string to `wandb.Video`, W&B asserts the file exists and is a supported format before uploading to W&B. If you pass a `BytesIO` object, W&B creates a temporary file with the specified format as the extension.
...
Use `wandb.Table` to log text in tables that appear in the UI. By default, the column headers are `["Input", "Output", "Expected"]`. To ensure optimal UI performance, the default maximum number of rows is set to 10,000. However, you can explicitly override the maximum by setting `wandb.Table.MAX_ROWS`.
...
Log custom HTML at any key to expose an HTML panel on the run page. By default, W&B injects default styles. To turn off default styles, pass `inject=False`.
