...
Most Public API calls identify a run by its run path, which has the form `<entity>/<project>/<run_id>`. To find the run path, open a run page in the app UI and click the [Overview tab](/models/track/public-api-guide/#overview-tab).
...
If errors occur while communicating with W&B servers, a `wandb.CommError` is raised. To introspect the original exception, use the `exc` attribute.
