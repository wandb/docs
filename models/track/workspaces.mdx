...
Use _workspace templates_ to create workspaces using the same settings as an existing workspace instead of the [default settings for new workspaces](#default-workspace-settings).
...
1. Click on the **Settings** button at the top right corner of the workspace.
...
### Clone a saved workspace view
Use the Reports and Workspaces API to clone a saved workspace view and use its layout and settings as the basis for a new view.

Before you begin, identify the following:

- The entity and project that contain the source workspace view.
- The workspace view ID.
      - To find the view ID, open the saved workspace view in W&B and copy the value of the nw query parameter from the URL. For example, in `https://wandb.ai/awesome-team/mnist?nw=54ro5zcn9d4`, the view ID is `54ro5zcn9d4`.
- The destination entity and project.
- The name for the new workspace view.

The following example clones a saved workspace view:

```python
import wandb_workspaces.workspaces as ws

# Specify the source workspace view
...
# Load the source workspace view
