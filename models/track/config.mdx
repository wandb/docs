...
The following JSON object shows an example run configuration:
...
Use the W&B Public API to access a previously logged run. Access the run's configuration with the [`wandb.Api.Run.config`](/models/ref/python/public-api/run#property-config) property. Provide your entity, project name, and the run's ID to the `wandb.Api.run()` method.

Copy and paste the following code snippet into your Python script or notebook. Replace `"<entity>"`, `"<project>"`, and `"<run_id>"` with your W&B entity, project name, and run ID.
...
To access configuration values for multiple runs in a project, use the `wandb.Api.runs()` method to get a list of runs in a project, and then iterate over the runs returned by `api.runs()`:
