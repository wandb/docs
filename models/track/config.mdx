...
Some W&B App views display nested config keys in dotted form (for example, `EPISODE_BUDGET.cramped_room`). Public API filters prefix these paths with `config.` (for example, `config.EPISODE_BUDGET.cramped_room`). In Python, access nested values with normal dictionary indexing: `run.config["EPISODE_BUDGET"]["cramped_room"]`.
