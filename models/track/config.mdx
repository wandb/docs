...
To store nested values in a run config, pass a nested dictionary to `wandb.init()`. The following example defines nested `optimizer` and `training` dictionaries in `config`:
...
Each top-level key must contain a `value` field. Assign the nested dictionary to value:
...
In Python, use dictionary indexing to access nested values.

The following example retrieves the `learning_rate` value from the nested optimizer dictionary in the run config:
...
In contexts that refer to config values by path, such as Public API filters or line plot expressions, use dot notation. For example, use `config.optimizer.learning_rate` in a Public API filter and `${config:optimizer.learning_rate}` in a line plot expression.

The following example creates a run with a nested config and logs loss for each training epoch. It then uses the Public API to find runs with a learning rate of `1e-4` and prints each run's name and learning rate.
...
<Info>
In practice, you typically use separate scripts to create runs and query them later with filters.
</Info>
