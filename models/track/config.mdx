...
You must provide the API with your entity, project name and the run's ID. You can find these details in the Run object or in the [W&B UI](/models/track/workspaces/):
...
# current script or notebook, or you can copy them from the W&B UI.
username = run.entity
project = run.project
run_id = run.id
...
Access your config values in W&B, during a run, or from existing runs. You can also access config values from your local filesystem.

<Tabs>
<Tab title="W&B UI">

View a run configuration in W&B:
