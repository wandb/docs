...
- Use manual syncing: W&B stores your run data locally if you hit a rate limit. You can sync your data with the command `wandb sync <run-file-path>`. For more details, see the [`wandb sync`](/models/ref/cli/wandb-sync) reference.

### GraphQL API rate limits

The W&B Models App and [Public API](/models/ref/python/public-api/api) make GraphQL to query and modify data. W&B enforces rate limits for GraphQL requests on SaaS Cloud to protect backend performance.
