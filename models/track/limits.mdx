...
* Metrica cardinality: The number of distinct metric keys logged, including keys in nested dictionaries. For example, the following would count as 4 metric keys:
    ```python
    import wandb
...
* Logged points: The total number of metric values recorded. For example, both of the following would count as 3 logged points:
    ...
    with wandb.init() as run:
        ...
        run.log({"a": 3})
    ```
* Log frequency: The number of wandb.log() calls per minute.
    ```text
    log frequency = wandb.log() / minute 
    ```
