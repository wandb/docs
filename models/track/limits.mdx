...
* **Steps**: The number of times you call [`wandb.Run.log()`](/models/ref/python/experiments/run#method-run-log) with `commit=True` (or allow an implicit commit).
...
W&B suggests that you use separate `wandb.Run.log()` calls with `commit=False` to associate less frequent metrics with an existing step without logging them at every step.
