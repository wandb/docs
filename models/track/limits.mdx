...
The following sections describe recommended limit and performance considerations. In this page the following terms are used:

* Steps: The number of logical steps in a run. A step identifies a single row of metrics (such as those that belong to a particular training step or epoch) and is finalized when `wandb.Run.log()` is called with `commit=True`, or implicitly when neither `commit` nor `step` are specified.
* Metric cardinality: The number of distinct metric keys logged, including keys in nested dictionaries. For example, the following would count as 4 metric keys:
    ```python
    import wandb
    ...
    ```
* Logged points: The total number of metric values recorded. For example, both of the following would count as 3 logged points:
    ```python
    import wandb
    with wandb.init() as run:
        run.log({"a": 1, "b": 2, "c": 3})
    ...
    with wandb.init() as run:
        ...
        run.log({"a": 3})
    ```
* Log frequency: The number of `wandb.Run.log()` calls per minute.
    ```text
    log frequency = wandb.Run.log() / minute 
    ```
* Throughput: A single unified metric for logging scale to simplify how users think about metrics across dimensions. The unit is logged points per minute.
    ```text
...
W&B logging capacity is defined primarily in terms of number of steps, metric cardinality, and total number of logged points. Generally, performance issues are due to logging too many distinct metrics, not from logging too many steps.

### Summary of recommendations for MTSaaS
...
| Log frequency (rows per minute) | 1,000 |
| Throughput (values per minute) | 100,000 |
| Video (MB/min)| 40 |
