...
Use the search box in the workspace to filter panels by title.

<Note>
By default, W&B uses the metric name as the panel title. To change the title, click the **gear** (<Icon icon="gear" iconType="solid"/>) icon in the upper-right corner of the panel, and then enter a title in the **Chart title** field.
</Note>
...
To exclude panels from the search results, click **Hide panels** (<Icon icon="sliders" iconType="solid"/>) next to the search box, and then enter a regular expression. W&B applies the filter immediately. The filter remains active until you remove it.

To remove the filter, click **Hide panels** (<Icon icon="sliders" iconType="solid"/>) again and clear the regular expression.
...
Refer to the [`wandb.init()` reference](/models/ref/python/functions/init/#examples).
</Tab>
</Tabs>
