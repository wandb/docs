...
Start a sweep agent.
    ...
    the sweep and launch a run for each configuration.
...
Maximum number of runs this agent will execute. Runs until the sweep completes if not set.                
...
Usage: agent [OPTIONS] SWEEP_ID
  ...
  launch a run for each configuration.
...
Options:
  ...
  --count INTEGER        Maximum number of runs this agent will execute. Runs
                         until the sweep completes if not set.
