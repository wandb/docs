---
title: "wandb launch"
description: "CLI reference for the `wandb launch` command, which launches or queues a W&B job."
---
{/*
...
| `--async` | Flag to run the job asynchronously. Defaults to false, in other words, unless --async is set, wandb launch will wait for the job to finish. This option is incompatible with --queue; asynchronous options when running with an agent should be set on wandb launch-agent. (default: False) |
