---
title: "wandb init"
---
{/*
  To add introductory content for this command:
  1. Create the snippet file: /snippets/en/_includes/cli/wandb-init.mdx
  2. Add your intro content to that file
  3. Delete this entire comment block and keep only the two lines below:

import WandbInit from "/snippets/en/_includes/cli/wandb-init.mdx";

<WandbInit/>

  The snippet will be auto-detected on the next regeneration.
*/}

Configure a directory with Weights & Biases

## Usage

```bash
wandb init [OPTIONS]
```

## Options

| Option | Description |
| :--- | :--- |
| `--project`, `-p` | The project to use. (default: Sentinel.UNSET) |
| `--entity`, `-e` | The entity to scope the project to. (default: Sentinel.UNSET) |
| `--reset` | Reset settings (default: False) |
| `--mode`, `-m` | Can be "online", "offline" or "disabled". Defaults to online. (default: Sentinel.UNSET) |
