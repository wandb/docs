...
- `summary`: Specify aggregate metrics added to summary. Supported aggregations include "min", "max", "mean", "last", "first", "copy" and "none". "none" prevents a summary from being generated. "copy" is deprecated and should not be used.
...
- `glob`: Whether to treat `glob_str` as a glob pattern. Defaults to `True` for backward compatibility. Set to `False` to treat `glob_str` as a literal path, e.g. when its name contains glob metacharacters like `[`, `]`, `*`, or `?` that you don't want interpreted as a pattern.
