---
title: Run
description: "Represents a W&B run for logging metrics, artifacts, and experiment data."
...
**Args:**
 ...
 - `overwrite`:  If false, then this call is merged with previous  `wandb.Run.define_metric()` calls for the same metric by using their  values for any unspecified parameters. If true, then  unspecified parameters overwrite values specified by  previous calls. 
...
By default, each call to `wandb.Run.log()` creates a new "step". The step must always increase, and it is not possible to log to a previous step. You can use any metric as the X axis in charts. See [Custom log axes](https://docs.wandb.ai/guides/track/log/customize-logging-axes/) for more details. 
...
Note that you must call wandb.Run.finish_artifact() to finalize the artifact. This is useful when distributed jobs need to all contribute to the same artifact. 
