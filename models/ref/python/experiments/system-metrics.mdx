...
## View system metrics
You can view and monitor system metrics using the W&B App or the `wandb beta leet` terminal UI in your terminal.
...
To view system metrics locally in your terminal using the `wandb beta leet` terminal UI:

1. Navigate to the directory containing your run or `.wandb` file.
2. Run one of the following commands:

   ```bash
   # Look for the latest run in current directory
   wandb beta leet
...
- `]` - Toggle the right sidebar on/off
- `alt+n` / `alt+pgdown` - Navigate to next system metrics page
- `alt+N` / `alt+pgup` - Navigate to previous system metrics page
- `C` - Configure the number of columns in the system metrics grid
- `R` - Configure the number of rows in the system metrics grid
