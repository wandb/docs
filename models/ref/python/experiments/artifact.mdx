...
**Args:**
 ...
 - `name` (str):  A human-readable name for the artifact. Use the name to identify  a specific artifact in W&B or programmatically. You can  interactively reference an artifact with the `use_artifact` Public API.  A name can contain letters, numbers, underscores, hyphens, and dots.  The name must be unique across a project. 
...
Aliases are mutable references that you can programmatically reference. Change an artifact's alias with W&B or programmatically. See [Create new artifact versions](https://docs.wandb.ai/models/artifacts/create-a-new-artifact-version) for more information. 
