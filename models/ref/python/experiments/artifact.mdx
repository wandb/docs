---
title: Artifact
description: "W&B Artifact class for versioning datasets, models, and other files."
...
Existing files located within `root` are not modified. Explicitly delete `root` before you call `wandb.Artifact.download()` if you want the contents of `root` to exactly match the artifact. 
