...
- `name`: A human-readable name for the artifact. Use the name to identify a specific artifact in the W&B UI or programmatically. You can interactively reference an artifact with the `use_artifact` Public API. A name can contain letters, numbers, underscores, hyphens, and dots. The name must be unique across a project.
...
- `description`: A description of the artifact. For Model or Dataset Artifacts, add documentation for your standardized team model or dataset card. View an artifact's description programmatically with the `Artifact.description` attribute or in the W&B UI. W&B renders the description as markdown.
...
Change an artifact's alias with the W&B UI or programmatically.
...
- `name`: The subdirectory name within an artifact. The name you specify appears in W&B nested by artifact's `type`. Defaults to the root of the artifact.
