...
For a product overview, launch paths, and limitations compared to the W&B App, see [LEET terminal UI](/models/app/leet-tui). For the complete shortcut tables, configuration keys, and filtering syntax, see [`wandb beta leet`](/models/ref/cli/wandb-beta/wandb-beta-leet).

Press `h` or `?` inside LEET to open the in-app help overlay for the current view.
