...
1. In **Regex** enter an expression in [JavaScript regular expression](https://www.w3schools.com/js/js_regexp.asp) format. The plot name will show the regular expression the plot uses, and the plot will include lines for all metrics that match the expression, including metrics logged in the future. As you type, the UI updates to show which metrics match the expression.
...
Use alternation to match only the metrics you want. For example, `(layer_0|layer_10)_loss` matches just the first and tenth layer losses, excluding intermediate layers.
