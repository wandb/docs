...
1. In **Regex** enter an expression in [JavaScript regular expression](https://www.w3schools.com/js/js_regexp.asp) format. The plot will include lines for all metrics that match the expression, including metrics logged in the future. As you type, the UI updates to show which metrics match the expression.
