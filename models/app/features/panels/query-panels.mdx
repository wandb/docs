...
When you stack compatible row lists (for example with a `concat` operation in the expression or through the **Concat** configuration option), rows from the second source follow rows from the first. That stacking is one common form of **appending** rows for analysis.
...
In tables and plots built from `runs` or `runs.history`, the app can show a **step** control (for example a slider) so you move through logged steps and inspect metrics, text, or media over the course of your runs.  Once the expression is changed, open up the configuration and change the 'Render As' to 'Stepper.' The control can follow a metric other than `_step` when that better matches how you logged data. For sample expressions, see the [Query panel examples report](https://wandb.ai/luis_team_test/weave_example_queries/reports/Query-Panel-Examples---Vmlldzo1NzIxOTY2).
