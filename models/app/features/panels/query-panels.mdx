...
In tables and plots built from `runs` or `runs.history`, the app can show a **step** control (for example a slider) so you move through logged steps and inspect metrics, text, or media over the course of your runs. The control can follow a metric other than `_step` when that better matches how you logged data. For sample expressions, see the [Query panel examples report](https://wandb.ai/luis_team_test/weave_example_queries/reports/Query-Panel-Examples---Vmlldzo1NzIxOTY2).
