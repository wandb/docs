...
- **Standard deviation**: For each X-axis point, calculate the variability of the values using standard deviation, and shade the resulting area.
    ...
    $$
- **Standard error**: For each X-axis point, calculate the likelihood of a sampling error by dividing the standard deviation by the square root of the sample size:
    $$
    SE = \frac{'{'}SD{'{'}}{'{'}\sqrt{'{'}n{'{'}}{'}'}{'}'}
    $$
