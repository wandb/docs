...
When you use full fidelity mode, W&B breaks the x-axis into dynamic buckets based on the number of data points. It calculates the minimum and maximum values within each bucket (used for optional shading) and uses the last value in each bucket to draw the line.
...
W&B dynamically divides the x-axis into 1000 buckets by default. For each bucket, W&B calculates the following values:
...
W&B plots values in buckets in a way that preserves full data representation and includes extreme values in every plot. The line is drawn from the last value in each bucket. When zoomed in to 1,000 points or fewer, full fidelity mode renders every data point without additional aggregation.
