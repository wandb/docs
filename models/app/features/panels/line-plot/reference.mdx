...
Set the y-axis variables to any value logged with `wandb.log()` as a number, array of numbers, or histogram of numbers. If you logged more than 1500 points for a variable, W&B samples down to 1500 points.

<Note>
Customize the color of your y-axis lines by the changing run color in the runs table.
</Note>
...
* **Full fidelity mode**: All points are always shown, condensed to the last value in each bucket. When enabled, the minimum and maximum bounds of each bucket are shaded. Otherwise, no area is shaded.
