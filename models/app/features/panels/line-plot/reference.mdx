...
Add custom calculated expressions to create derived metrics. Expressions are evaluated per step and can reference logged metrics, config values, and summary statistics to produce a new line plot.

This is useful when you want to:

* Transform a metric (scale, normalize, log-transform)
* Combine multiple metrics into a single signal
...
If your metric name includes special characters or spaces that could be misinterpreted as part of the expression (such as `/`, `-`, or spaces), escape it using `${...}`.

For example, if you log a metric named `val/accuracy`, reference it as `${val/accuracy}` or `${"val/accuracy"}`.
