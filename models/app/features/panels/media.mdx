...
- Configure how W&B fits and scales media items. These settings have no effect in [**Compare** mode](#compare-mode).
    - **Images**: Set **Image fit and resolution** to control how W&B displays images. The available options depend on your deployment type.

        On W&B Multi-tenant Cloud, the following options are available:
        - **Thumbnails** (default): Use thumbnails to optimize loading speed.
        - **Original image**: Load full-resolution images for detailed inspection.
        - **Fit to available space**: Resize full-resolution images to fit within the panel.
        - **Original size**: Load full-resolution images in their original dimensions.

        On W&B Dedicated Cloud or W&B Self-Managed, the following options are available:
        - **Default** (default): Use the default image dimensions for the panel mode.
        - **Fit to available space**: Resize full-resolution images to fit within the panel.
        - **Original size**: Load full-resolution images in their original dimensions.

        On Multi-tenant Cloud, W&B always loads full-resolution images in the full-screen view, regardless of this setting.
    - **Videos**: In **Gallery** mode, turn on **Use original size** to display videos at their original dimensions. If off (default), W&B scales videos to fill as much panel space as possible while maintaining the image's aspect ratio. In **Grid** mode, each video fills the grid width.
