...
- Configure how W&B fits and scales media items. These settings have no effect in **Compare** mode.
    ...
    - **Videos**: In **Gallery** mode, turn on **Use original size** to display videos at their original dimensions. If off (default), W&B scales videos to take up as much room as possible, so that each video fills the panel width. In **Grid** mode, each video fills the grid width.
