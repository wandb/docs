...
The custom chart editor runs a GraphQL query over the runs you selected in the project workspace or report. In the **Query** section of the panel, add the fields you need. The UI lists **config**, **summary**, **history**, **`summaryTable`**, and **`historyTable`** so you do not need to write the query from scratch unless you choose **Edit query** for advanced edits.
...
* **`historyTable`** loads a [`wandb.Table`](/models/ref/python/data-types/table) from run history. Each time you log a table with `wandb.Run.log()`, you add another step to run history that includes that table. Use **`historyTable`** when the table changes over time or when you need the **Show step selector** option in **Other settings** (see [How do you show a step slider in a custom chart?](/support/models/articles/how-do-you-show-a-step-slider-in-a-custo)).
