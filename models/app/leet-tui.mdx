...
| **[W&B UI](/models/runs/view-logged-runs)** | Full project workspaces, team collaboration, reports, sweeps, artifacts, and rich panel types across synced cloud runs. |
...
LEET and the W&B UI show the same underlying run data when that data exists locally and has been synced. LEET does not replace a project workspace in the browser. It is a different interface over local run files and live updates from an active training process.
...
For console log behavior in the W&B UI, see [Console logs](/models/app/console-logs).
...
## Limitations compared to the W&B UI

LEET complements the W&B UI but does not replicate every workspace capability.

| Capability | LEET | W&B UI |
| --- | --- | --- |
...
If you need artifacts, sweep analysis, or panel types that LEET does not provide, open the run in the [W&B UI](/models/runs/view-logged-runs) after syncing.
...
- [View a specific run](/models/runs/view-logged-runs) in the W&B UI or LEET.
- [Keyboard shortcuts](/models/app/keyboard-shortcuts) for the W&B UI and LEET.
