...
<Info> The sweep agent selects a value from the `values` list and passes it to the run config in the training script. For example, if you define the `batch_size` parameter with the values `[16, 32, 64]`, the sweep agent selects one of those values and passes it to the training script as `run.config.batch_size`. </Info>
...
3. Access the hyperparameter values from `run.config` so that your script uses the suggested arguments for each run instead of hard-coded values.
...
The following code snippet shows how to integrate W&B into your training script. When the sweep agent runs this script, it passes the selected hyperparameter values to `run.config` for that run.
...
<Note>
Do not read the sweep configuration file in your training script. The YAML file is the sweep configuration that you pass to `wandb sweep`. When a sweep agent starts a run, it automatically sends the selected hyperparameter values to `run.config`. If you run the training script directly with `python train.py`, the sweep agent does not set these values and keys such as `run.config["lr"]` are not available.
</Note>
...
2. Copy the sweep ID and replace `sweepID` in the following command to start the sweep job with the [`wandb agent`](/models/ref/cli/wandb-agent) command. Replace `your-entity` with your W&B entity name.

    ```bash
    wandb agent --count $NUM your-entity/project_name/sweepID
    ```

For more information, see [Start a sweep agent](./start-sweep-agents).
