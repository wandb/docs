...
Use top-level keys within your sweep configuration to define qualities of your sweep search. These qualities include the name of the sweep ([`name`](/models/sweeps/sweep-config-keys) key), the parameters to search through ([`parameters`](/models/sweeps/sweep-config-keys#parameters) key), the methodology to search the parameter space ([`method`](/models/sweeps/sweep-config-keys#method) key), and more.
...
The following example shows a sweep configuration with nested parameters `nested_category_1`, `nested_category_2`, and `nested_category_3`. Each nested parameter includes the additional parameters `momentum` and `weight_decay`.
