...
keywords: ["hyperparameter search", "Bayesian", "grid search", "Hyperband", "YAML"]
---


A sweep configuration consists of nested key-value pairs. Use top-level keys within your sweep configuration to define qualities of your sweep search, such as the parameters to search through ([`parameter`](./sweep-config-keys#parameters) key) and the methodology to search the parameter space ([`method`](./sweep-config-keys#method) key).
...
For more information about how to structure your sweep configuration, see the [Sweep configuration structure](./sweep-config-keys).
...
If you choose random search (`method: random`), specify the distribution space with the metric key.
