...
A sweep configuration consists of nested key-value pairs. Use top-level keys within your sweep configuration to define qualities of your sweep search, such as the parameters to search through ([`parameters`](./sweep-config-keys#parameters) key) and the methodology to search the parameter space ([`method`](./sweep-config-keys#method) key).
