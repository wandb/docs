...
description: "Reference for all sweep configuration keys including method, metric, parameters, early termination, and command."
---
...
{/* Agents created with [`wandb agent`](/models/ref/cli/wandb-agent) receive a command in the following format by default: */}

Use the `command` top-level key to control how the sweep agent invokes your training script and passes arguments to it. Specify the invocation as a YAML list. Each list item is either a fixed string (such as a filename or literal flag) or a macro that the agent expands at run time.

If you omit the `command` key, the sweep agent uses the following default invocation:
...
The default invocation passes each hyperparameter as `--name=value`. Override `command` when you need a different argument format, want to inject fixed flags, or need to pass values as JSON or environment variables.

The following example runs `train.py` with the Python interpreter, passes a fixed `--train` flag, and expands the swept `learning_rate` parameter as a custom `--lr=<value>` argument:
...
For a full walkthrough that includes the matching training script, see [Macro and custom command arguments example](/models/sweeps/define-sweep-configuration#macro-and-custom-command-arguments-example).
