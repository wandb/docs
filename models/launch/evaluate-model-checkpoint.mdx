...
1. Click **Evaluations**, then select up to four benchmarks to run.
...
1. Some benchmarks also require a HuggingFace token, in which case a **Hugging Face token** field is displayed. These datasets are typically gated and require you to navigate to the dataset on Hugging Face and [request access as a user](https://huggingface.co/docs/hub/en/datasets-gated#access-gated-datasets-as-a-user).
...
1. To track the status of evaluation jobs, click the recent runs list (circular arrows icon) at the top of the page. Evaluation jobs appear with other runs in the project. Click the name of a finished run to open it in the project, where you can view the results, leaderboard, configuration, and logs. For details, see [View the results](#view-the-results).
