...
[W&B LLM Evaluation Jobs](/models/launch) is a benchmarking framework for evaluating an LLM model's performance, using infrastructure managed by CoreWeave. Choose from a comprehensive suite of modern, industry-standard [model evaluation benchmarks](/models/launch/evaluations), then view, analyze, and share the results using automatic leaderboards and charts in W&B. LLM Evaluation Jobs removes the complexity of deploying and maintaining GPU  infrastructure yourself.
...
- Certain benchmarks use OpenAI models for scoring. To run these benchmarks, an organization or team admin must configure a team secret with the API key. This requirement is documented in [Evaluation benchmark catalog](/models/launch/evaluations).
