...
Rewind a run to modify the history of a run without losing the original data. When you rewind a run, W&B resets the state of the run to the specified step, preserves the original data, and maintains the same run ID. 

<Warning>
The option to rewind a run is in private preview and in active development. Contact W&B Support at support@wandb.com to request access to this feature.
...
At a later time, you want to rewind the run from step 200 and you want to log a new metric called `additional_metric` that logs `i*1.1` from step 200 to step 300. From step 250 you want to log a new subtle wavey pattern (`i**2 + 2*sin(i/3)`) instead of a linear line:
...
with wandb.init(project="<project>", entity="wandb", resume_from=f"{run_ID}?_step=200") as run:
    ...
    # After step 250, start logging the wavey pattern
    for i in range(200, 300):
        if i < 250:
            run.log({"metric": i, "step": i})  # Continue logging from run without waves
        else:
            # Introduce the wavey behavior starting from step 250
            subtle_wave = i + (2 * math.sin(i / 3.0))  # Apply a subtle wavey pattern
...
- The line plot shows the original linear line from step 0 to step 200 and the new subtle wavey pattern starts from step 250 (left image).
