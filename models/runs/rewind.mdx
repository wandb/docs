...
Use `resume_from` with [`wandb.init()`](/models/ref/python/functions/init) to "rewind" a run’s history to a specific step. Specify the name of the run and the step you want to rewind from:
...
After you rewind a run, you can explore archived run with the W&B App. Follow these steps to view archived runs:
