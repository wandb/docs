...
You must use the W&B App to move runs between projects or teams. Neither the W&B Python SDK nor the W&B Public API provides a method to move runs between projects or teams. W&B doesn't support copying runs.
...
To delete a run with the Python Public API, use [`Run.delete()`](/models/ref/python/public-api/run#method-run-delete). The Public API also supports other run-management operations, but you must use the W&B App to move runs between projects.
