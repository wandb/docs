...
The [Run state](/models/runs/run-states#run-states) indicates the current status of a W&B run. You can [view the state](/models/runs/run-states#view-the-state-of-a-run) of a run in the W&B UI or programmatically using the W&B Python SDK.
...
Programmatically or interactively view a run's state with the Python SDK or the W&B UI.
...
View the state of a run from W&B:
