...
The number of runs that match the filter expression appears underneath the **Filter** button, which looks like a funnel, above the runs table.

For example, in the folloing image, there are `6 of 18 listed` runs that match the filter expression (see the text highlighted within the yellow box):
