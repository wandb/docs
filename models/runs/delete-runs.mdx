...
Individual files attached to a run (for example logged media) can be removed with [`File.delete()`](/models/ref/python/public-api/file#method-file-delete) after you obtain file handles from [`Run.files()`](/models/ref/python/public-api/run#method-run-files).
