...
description: "Add, update, and remove tags on W&B runs using the Python SDK, Public API, or the W&B UI for organization."
---

Add tags to label runs with particular features that might not be obvious from the logged metrics or artifact data. 

For example, you can add a tag to a run to indicate that a run's model is `in_production`, that run is `preemptible`, this run represents the `baseline`, and so forth.
...
Follow these steps to remove tags from a run in W&B.

<Tabs>
<Tab title="Project page">
This method is best suited to removing tags from a large number of runs.
