...
By default, W&B assumes each Python process has only one active run at a time when you call `wandb.init()`. If you call `wandb.init()` again, W&B will either return the same run or finish the old run before starting a new one, depending on the configuration.  To manage multiple active runs in the same process, see [W&B Run Contexts](/models/runs/initialize_run).

## Single run per process

For example, consider the following code snippet:

{/* The following code snippet shows how to import the W&B Python SDK and initialize a run. 
