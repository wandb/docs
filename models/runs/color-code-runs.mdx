...
After you configure the settings above and close the drawer, line plots in your **Workspace** tab update to use the new colors. In this example plot, **Key** is set to `loss` and **Y value** is set to `Latest`, so runs are colored with a gradient where darker colors represent higher loss values and lighter colors represent lower loss values.
