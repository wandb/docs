...
with wandb.init() as run:
    run.mark_preempting()
```
The following table outlines how W&B handles runs based on the exit status of the sweep run.
...
Termination via UNIX signals like `SIGTERM` or `SIGINT` usually produces a non-zero exit code when the training script handles the signal and exits. The requeue behavior described in the table above also applies when you use `mark_preempting()`. For details on Sweeps handle signals and ensuring that the training process receives them, see [Signal handling and sweep runs](/models/sweeps/signal-handling-sweep-runs).
