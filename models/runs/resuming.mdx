...
<Note>
W&B suggests that you provide the name of the W&B Project where you want to store the run.
</Note> */}
...
W&B recommends using `resume="allow"` with a stored run ID. `resume="allow"` provides control over which specific run to resume. The `resume="auto"` option does not require you to specify a run ID but it has the limitation of requiring the same directory structure.
</Note>
