...
keywords: ["baseline run", "runs table", "line plot", "metric delta", "run selector"]
---
...
When you set a run as the baseline, by default every other run that logs the same summary metric shows the delta from the baseline. The delta (amount of change) appears to the right of the metric's value in the run's row in the runs table.

By default, the delta appears as dark gray text on a dark gray background. To turn on semantic coloring for quick visual reference, set the **Metric directionality** for a column. With directionality set:
...
By default, a workspace with a baseline run always displays summary metric deltas. To reduce visual clutter or focus on absolute values, you can hide them for a workspace:
...
The following example illustrates how pinned and baseline runs help you compare runs.

1. Run this example code, which simulates a hyperparameter-tuning scenario with a series of runs. Replace bracketed uppercase placeholders (for example, `[TEAM-NAME]`) with your own values.
    ...
    After you run this code, your workspace has three runs.
