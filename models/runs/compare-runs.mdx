...
- Hover over the baseline run's label in the legend to display a tooltip with its value. Lines for other visible runs appear with reduced saturation.
  {/*![Demo showing details for the baseline run](TODO)*/}
  Tooltips show the baseline value at a given point on the y-axis.
- Hover over another run's label in the legend to display a tooltip with values for both that run and the baseline. Lines for other visible runs appear with reduced saturation.
