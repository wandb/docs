...
---
import PreviewLink from '/snippets/en/_includes/pinned-and-baseline-runs/preview.mdx';
...
- Tracking multiple candidate models during hyperparameter tuning.
- Keeping failed experiments visible as reminders of what not to try.
...
Pin up to 5 runs to keep them easily accessible at the top of your workspace. Pinned runs remain visible regardless of sorting or filtering applied to other runs.
...
Pinned runs appear at the top of the run selector with a pin icon, separated from other runs by a visual divider.

{/*![Runs table with pinned runs](TODO)*/}

To unpin a run, follow the same procedure, but select **Unpin run** instead.
...
In line plots, lines for the baseline run appear bolder than other lines. Hover over the baseline run's line to see details. Hover over another run's line to visually compare it with the baseline. When comparing a run with the baseline, other pinned and visible runs appear with reduced saturation.
...
The baseline run appears at the top of the run selector, separated from other runs by a visual divider. The baseline run has a bookmark icon instead of a pin.

{/*![Runs table with a baseline run and pinned runs](TODO)*/}
...
The baseline run is always visible. In line plots, lines for the baseline run appear bolder than other lines.

- Hover over a part of the plot that does not have a line to display a tooltip with values for all visible runs, including the baseline run and pinned runs.
  {/*![Demo showing details for all visible runs at a given point]*/}
- Hover over the baseline run's line to display a tooltip with its value. Lines for other visible runs appear with reduced saturation.
  {/*![Demo showing details for the baseline run](TODO)*/}
  Tooltips show the baseline value at a given point on the y-axis.
- Hover over another run's line to display a tooltip with values for both that run and the baseline. Lines for other visible runs appear with reduced saturation.
  {/*![Demo of comparing another run with the baseline](TODO)*/}

## Use cases
This section describes some scenarios where pinned and baseline runs can help guide your experiments.

- **Track production models**: Ensure new models meet your quality bar before deployment.
  1. Pin your current production model.
  2. Set it as the baseline.
  3. Compare all experiments against your deployed model to identify candidates that outperform production.
...
- **Document failed experiments**: Keep track of approaches that don't work, to help your team avoid repeating mistakes.
  1. Pin runs that performed poorly.
  2. Add notes explaining why the experiment failed.
