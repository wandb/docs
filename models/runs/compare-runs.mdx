...
W&B shows pinned runs in your project’s run selector and runs table. To see which project a pinned run comes from, hover over the run name. Runs from other projects have an open circle icon next to their name.

Artifacts, logs, and other details for pinned runs link back to the original run. W&B does not import this data into the current project or workspace.

To view the details for a run pinned from another project, click the run. W&B opens the original run details in a new browser tab.
