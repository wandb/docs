...
---
import PinRunsCondensed from '/snippets/_includes/pinned-and-baseline-runs/pin-runs-condensed.mdx';

In a workspace with many runs, it's hard to keep track of your best performers, production models, failed experiments, or important reference points. The W&B App provides features to help you organize and compare runs:

- **Pinned runs**: Pin up to six runs to keep them visible in the workspace and at the top of the runs list. If you have a baseline run, you can pin up to five runs because the baseline is implicitly pinned.
...
Pin a run to keep it visible at the top of the runs list and in the workspace.
...
- If the other run **underperforms** (is directionally better than) the baseline, the delta appears as dark teal text with a light teal background.
