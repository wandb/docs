...
For **Run metrics z-score threshold met** events, the automation triggers when a metric's z-score (standard score) exceeds a specified threshold. A z-score indicates how many standard deviations a value is from the mean of all values for that metric across runs in the project.
...
For example, trigger an automation when `accuracy` has a z-score above 2, indicating the run is performing significantly better than typical runs in the project.

**Understanding z-0score values:**
