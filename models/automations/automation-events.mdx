...
An automation starts when a specific event occurs within a project, registry, or collection. For example, when an artifact version is created, aliased, tagged, or linked, when a run's status changes, or when a run metric meets or changes by a given threshold. Learn more about automations in the [Automations overview](/models/automations/) or [Create an automation](/models/automations/create-automations/).
