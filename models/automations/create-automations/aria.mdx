...
This page shows how to use an [automations](/models/automations) to send a prompt to [ARIA](/aria/overview) when a specific event occurs in W&B. Each time the automation runs, ARIA starts a new conversation and works on the prompt on your behalf. Use an ARIA automation to kickstart exploration of an experiment, summarize or report on a run, or investigate a metric that crossed a threshold.

At a high level, an ARIA automation defines the [event](/models/automations/automation-events) to watch for and the prompt that ARIA receives each time that event occurs.
...
You can ask ARIA to create and manage automations of all kinds on your behalf. For details, see [Create and manage automations with ARIA](#create-and-manage-automations-with-aria).
...
<AriaChatBubbles
  prompt="CCreate a new automation: When a run succeeds, trigger ARIA to write a report for your team that summarizes the run, metrics, and outcome."
