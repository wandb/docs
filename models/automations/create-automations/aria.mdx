---
title: Use automations with ARIA
...
1. Choose the [event](/models/automations/automation-events/#registry-events) to watch for.
   ...
   Click **Next step**.
1. Select the team that hosts the ARIA conversation. Registries belong to your organization, not a team, but ARIA conversations must belong to one. See [View conversations created by automations](#view-conversations-created-by-automations).
...
{/* VERIFY: "delete, enable, and disable" — ARIA's automation skill exposes only list/get/integrations/template-vars/create subcommands, no update or delete. It also has a generic execute_graphql escape hatch, so these may still work. Test in product before trusting this sentence. */}
ARIA can create, delete, enable, and disable automations on demand.

Ask ARIA to create an automation:

<AriaChatBubbles
  prompt="Create a new automation: When a run succeeds, trigger ARIA to write a report for your team that summarizes the run, metrics, and outcome."
  response="I can create an enabled project-wide automation named **Summarize successful run**. It will fire whenever any run reaches `FINISHED` and ask ARIA to write a report to summarize the run's configuration, key metrics, outcome, and notable observations."
/>
...
Ask ARIA to rename an automation:

<AriaChatBubbles
  prompt="Rename old-name to production-alert."
  response="Proceed with the rename?"
/>

After you confirm, ARIA renames the automation.

### View your ARIA automations in chat

Open ARIA, click **Reveal chat history** in the chat header, then open the **Customize** section at the bottom of the chat history sidebar and select **My automations**.

**My automations** replaces the conversation view in ARIA's main panel rather than opening beside it. Selecting a conversation or starting a new chat returns you to the conversation. The pane isn't persisted across reloads, so refreshing the page also returns you to your conversation.

The pane lists only the automations you created that send prompts to ARIA. It omits automations that use other actions, and your teammates' ARIA automations.

The pane is read-only. You can see whether an automation is enabled or disabled, but to enable, disable, edit, or delete one, click **View all automations** to open the full Automations page in a new tab. See [View and manage automations](#view-and-manage-automations).

Search by name, or page through results 20 at a time with **Previous** and **Next**. The list shows each automation's name, **Event type**, and **Scope**.

Click an automation to open its detail view, which shows its **Description**, **Scope**, **Created by**, **Created**, **Status** (**Enabled** or **Disabled**), **Event type**, and the **Prompt ARIA** it sends.

The **Automation history** section lists each execution's **Triggered** time and **Status**, plus a **Conversation** column. Click **Open thread** to open the conversation an execution created. A **No thread** value means the execution didn't create one. Executions are shown for up to 6 months.

Use **Back to chat** or **Back to My automations** to return from the list or a detail view.

Click **Create automation** to start one. This doesn't open a form. Instead, it starts a new ARIA chat with the following prompt already sent on your behalf:

<AriaChatBubbles
  prompt="Guide me through creating an automation that prompts ARIA. Ask what I want to automate, help me choose its scope and trigger, and draft the prompt with me."
  response="Happy to help. What event should trigger this, and should the automation live in a project or a registry? Once we settle that, I'll help you word the prompt."
/>

**Create automation** is disabled if you have read-only billing access. **My automations** isn't available for personal accounts. Select an organization to view your automations. The pane requires no separate feature flag. It's available under the same access checks described in [Requirements](#requirements).
...
The automation creates the ARIA conversation under the account of the user who created the automation, not the user whose action triggered the event. The conversation appears in that person's ARIA chat history sidebar, alongside conversations they started themselves. To identify the user that caused the event, include `${event_author}` in the prompt.
...
To confirm that the automation ran, open its [execution history](/models/automations/view-automation-history). A successful ARIA execution shows the prompt that W&B sent and a response body containing the conversation's `thread_id`. To open that conversation, use the **Open thread** link in ARIA's [My automations](#view-your-aria-automations-in-chat) pane, which is built from the same `thread_id`. An automation-created conversation isn't marked differently in your chat history.
...
ARIA automations have the following limitations:
...
- The in-chat [My automations](#view-your-aria-automations-in-chat) pane is read-only. Enable, disable, edit, or delete an automation from the **Automations** tab or hub instead.
