...
If a webhook isn't working as expected, you can troubleshoot it interactively with W&B or programmatically with a shell script. You can troubleshoot a webhook during creation or afterward.

For details about the format W&B uses for the `POST` request, refer to the **Shell script** tab.

<Tabs>
<Tab title="W&B">
A team admin can test a webhook interactively with W&B.
...
1. Click **Test webhook**. Within W&B, W&B posts the response from your endpoint.
    <Frame>
