...
If a webhook isn't working as expected, you can troubleshoot it interactively with the W&B UI or programmatically with a shell script. You can troubleshoot a webhook during creation or afterward.

For details about the format W&B uses for the `POST` request, refer to the **Shell script** tab.

<Tabs>
<Tab title="W&B UI">
A team admin can test a webhook interactively with the W&B UI.
...
1. Click **Test webhook**. W&B displays the response from your endpoint.
    <Frame>
