...
Anything you log with `wandb.Run.log()` is recorded to that run.
...
You can use a run's unique ID to directly navigate to the run's overview page in the W&B App. The following cell shows the URL path for a specific run:
...
The following [example workspace](https://wandb.ai/stacey/model_iterz?workspace=user-stacey), trains a baseline model using increasing amounts of data from the Fashion-MNIST dataset. The workspace uses colors to represent the amount of data used:
