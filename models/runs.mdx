...
Each run object has a [unique identifier known as a *run ID*](/models/runs/run-identifiers#run-id). [You can specify a unique ID](/models/runs/run-identifiers#run-id) or let [W&B randomly generate one for you](/models/runs/run-identifiers#run-id). Each run object also has a human-readable, non-unique [run name](/models/runs/run-identifiers#run-name). You can specify a name for your run or let W&B randomly generate one for you. You can rename a run after initializing it.
...
Use a run’s [`summary`](/models/ref/python/experiments/run#attributes) attribute to store values that summarize a model, training process, or preprocessing step. Unlike metrics that change over time, summary values typically represent a final or aggregate result.

You can log summary values during an active run or update them after the run finishes. Access them programmatically with the W&B Python SDK or view them in the W&B App.

For more information, see [Log summary metrics](/models/track/log/log-summary).

## Run configuration

Use a run’s configuration to store the input values and settings for an experiment, such as hyperparameters, model architecture, and dataset name. Configuration values help you track the conditions used to produce a run’s results.

You can log configuration values during an active run or update them after the run finishes. Access them programmatically with the W&B Python SDK or view them in the W&B App.
