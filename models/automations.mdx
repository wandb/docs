...
Automations can be triggered by events related to these W&B entities:

- **Runs**: Trigger when a run meets or changes by a threshold on a specific metric. For example, trigger when a run's `loss` is less than 0.01 or when a run's `accuracy` improves by 5%.
- **Artifacts**: Trigger when a new artifact version is created or when a specific alias is added to an artifact version. For example, trigger when a new model version is created or when the `production` alias is added to a model.
- **Registries**: Trigger when a new model version is linked to a registry or when an alias is added to a model version in the registry. For example, trigger when a new model version is linked to the `Model Registry` or when the `production` alias is added to a model version in the registry.
...
- User information
- Timestamp
- Custom payload data you configure
