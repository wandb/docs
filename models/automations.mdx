...
Automations can be triggered by events related to runs or artifacts in a collection, project, or registry. For example, an automation can start:

- When a run's status changes to **Running**, **Finished**, **Failed**.
...
- When an artifact version is created, linked, or tagged in a project, registry, or collection. For example, trigger when a new version of a model artifact is created or when the `production` alias is added to a model artifact.
