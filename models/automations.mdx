...
_Automations_ send Slack notifications, run webhooks, or send a prompt to ARIA in response to specific events in W&B. You can scope an automation to an organization, a team, project, registry, or collection. An automation observes the event across everything within its scope, including projects, registries, and collections created later. The steps to create an automation and the available events depend on the scope. For event types by scope, see [Automation events and scopes](/models/automations/automation-events).
...
- Click an automation's name to view its full configuration details.
...
The **Automations** hub complements the **Automations** tab in each project and registry; those scoped tabs remain available for quick targeted access.
...
- **Automated analysis**: When a run fails or a new model version is linked, send a prompt to [ARIA](/aria/overview) to diagnose the failure or compare the new version against production.
...
For implementation details, see [Create a Slack automation](/models/automations/create-automations/slack), [Create a webhook automation](/models/automations/create-automations/webhook), or [Create an ARIA automation](/models/automations/create-automations/aria).
...
1. Configure team-level webhook or Slack integrations to authorize W&B to post to Slack or run the webhook on your behalf. A single automation action (webhook or Slack notification) can be used by multiple automations. These actions are defined in your **Team Settings**. An ARIA action requires no integration.
1. In the project or registry, create the automation:
    ...
    1. Define the action to take when the event occurs, such as posting to a Slack channel, running a webhook, or sending a prompt to ARIA. For a webhook, specify a secret to use for the access token and/or a secret to send with the payload, if required.
