...
If you run a sweep within the command line, you must specify your sweep config with a [YAML file](/models/sweeps/define-sweep-configuration/).
</Note>

### Pick a search method

First, specify a hyperparameter search method within your configuration dictionary. [There are three hyperparameter search strategies to choose from: grid, random, and Bayesian search](/models/sweeps/sweep-config-keys/#method).
...
For a full list of configuration options, see [Sweep configuration options](/models/sweeps/sweep-config-keys/). 
...
For more information on how to create W&B Sweeps in a terminal, see the [W&B Sweep walkthrough](/models/sweeps/walkthrough/).
...
* [`wandb.init()`](/models/ref/python/functions/init/): Initialize a new W&B Run. Each run is a single execution of the training function.
* [`run.config`](/models/track/config/): Pass sweep configuration with the hyperparameters you want to experiment with.
* [`run.log()`](/models/ref/python/experiments/run/#method-runlog): Log the training loss for each epoch.
