...
decription: Learn best practices for integrating W&B into your own Python library for experiment tracking, model checkpointing, and more.
---
...
<Tip>
If you are still learning how to use W&B, we recommend exploring the other W&B Guides in these docs, such as [Experiment Tracking](/models/track/), before reading further.
</Tip>
...
Add the W&B Python SDK (`wandb`) to your `requirements.txt` dependency file:
...
Make the W&B Python SDK an optional dependency by using a `try and except` block to import `wandb` or with `pyproject.toml`.

<Tabs>
<Tab title="Python">

Raise an error when a user tries to use `wandb` functionality without installing it manually and show an appropriate error message:
...
Add `wandb` as an optional dependency to your `pyproject.toml` file:
...
For more information on how to define a config file, see [Configure experiments](/models/track/config/).

## Log to W&B

### Log metrics

Create a dictionary where the key value is the name of the metric. Pass this dictionary object to [`run.log`](/models/):
...
See the [`wandb.Run.log()` reference](/models/).
...
See the [logging guide](/models/) for media, objects, plots, and more.
...
For information on how to create a custom alias, see [Create a Custom Alias](/models/artifacts/create-a-custom-alias/).
...
To download an Artifact without creating a `wandb` run (through `wandb.init`), for example in distributed environments or for simple inference, you can instead reference the artifact with the [wandb API](/models/ref/python/public-api/):
...
If your library would like to leverage W&B hyper-parameter tuning, [W&B Sweeps](/models/sweeps/) can also be added to your library.
