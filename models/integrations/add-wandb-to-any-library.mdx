...
Wrap the training loop in a context manager, initialize a run, pass a config dictionary as an argument to `wandb.init(config=)`, and log metrics to the run (`wandb.Run.log()`):
...
The [next section](#log-configuration-and-metrics) describes in detail how to log configuration values, metrics, and artifacts from your library.

## Log configuration and metrics

Log configuration values and metrics to W&B so you, and members of your team, can [compare](/models/runs/compare-runs), [filter](/models/runs/filter-runs#example-filter-run-configuration-values-with-contains), group, and [reproduce](/models/track/reproduce_experiments) experiments.
