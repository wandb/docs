...
Configure W&B so users can run your library without sending run data to W&B. Choose a mode based on whether you want to discard the data or save it locally for later upload.
...
Pass a [configuration](/models/track/config/) dictionary to `wandb.init()` to record a run’s hyperparameters and other metadata. Configuration values help you [compare](/models/runs/compare-runs), [filter](/models/runs/filter-runs), group, and [reproduce](/models/track/reproduce_experiments) runs.

In the following image, `batch_size` is logged as a configuration parameter and displayed in the first column of the **Runs** table. You can use this value to filter and compare runs by batch size:
...
The following code snippet shows an example of how to define a run config in your library. The `config` dictionary contains hyperparameters and other metadata that you want to log for the run. Pass this dictionary to `wandb.init()` when you initialize the run:
...
Some configuration values, such as the number of model parameters, might not be available when you initialize a run with `wandb.init()`. Add or update these values during the run or after the run finishes.

<Tabs>
<Tab title="During an active run">

Use `wandb.Run.config.update()` to update the configuration of an active run. For example, add the number of model parameters after you instantiate the model:

```python icon="python" title="Python"
config = {
    "batch_size": 32, 
    # Add other configuration values heres
    }
...
<Tab title="After the run finished">

Use the [W&B Public API](/models/ref/python/public-api/) to retrieve a run logged to W&B, modify its configuration, and save the changes:
...
This workflow updates the existing run without creating a new run or requiring an active run context.
