...
| `init_kwargs`                   | Params to pass to `wandb.init()` such as your wandb `config` etc. See the [`wandb.init()` parameters](/models/ref/python/functions/init) for parameters that `wandb.init()` accepts.
