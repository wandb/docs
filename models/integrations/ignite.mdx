...
Ignite supports a W&B handler to log metrics, model and optimizer parameters, and gradients during training and validation. You can also use it to log model checkpoints to the W&B cloud. This class wraps the W&B Python SDK (`wandb`), so you can call any `wandb` function using this wrapper. See examples on how to save model parameters and gradients.
