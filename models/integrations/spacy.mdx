...
If you use spaCy, this page explains how to use W&B to monitor training runs, compare experiments, and version the models and datasets produced by `spacy train`.
...
| `model_log_interval`   | `Optional int`. `None` by default. If set, enables [model versioning](/models/registry/) with [artifacts](/models/artifacts/). Pass in the number of steps to wait between logging model checkpoints. |
...
When training begins, spaCy outputs a link to your training run's [W&B page](/models/runs/), which takes you to this run's experiment tracking [dashboard](/models/track/workspaces/) in the W&B web UI.
