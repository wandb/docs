...
* [`add_dirs`](/models/ref/python/experiments/artifact#method-artifact-add_dir): (list[dict[str, Any]]): An array containing configuration for each local directory to include in the Artifact. 
* [`add_files`](/models/ref/python/experiments/artifact#method-artifact-add_file): (list[dict[str, Any]]): An array containing configuration for each local file to include in the Artifact. 
* [`add_references`](/models/ref/python/experiments/artifact#method-artifact-add_reference): (list[dict[str, Any]]): An array containing configuration for each external reference to include in the Artifact. 
...
Avoid using [`Artifact.download()`](/models/ref/python/experiments/artifact#method-artifact-download) or [`Run.log_artifact()`](/models/ref/python/experiments/run#method-run-log_artifact) directly. The integration handles those methods. Instead, return the data you want to store in the Artifact and let the integration do the rest. This approach provides better lineage for the Artifact.
