...
Use the [global search bar in the W&B Registry App](./search_registry#search-for-registry-items) to find a registry, collection, artifact version tag, collection tag, or alias. You can use MongoDB-style queries to [filter registries, collections, and artifact versions](./search_registry#query-registry-items-with-mongodb-style-queries) based on specific criteria using the W&B Python SDK.
...
To use the `wandb.Api().registries()` method, first import the W&B Python SDK ([`wandb`](/ref/)) library:
```python
import wandb
