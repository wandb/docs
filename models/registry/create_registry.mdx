...
<Info>
See the summary table in [Registry types](/models/registry/registry_types/) for a complete comparison of core and custom registries.
</Info>


## Create a custom registry

Create a custom registry either programmatically using the W&B Registry UI or the W&B Python SDK.
...
Use the [`wandb.Api().create_registry()`](/models/ref/python/#method-apicreate_registry) method to create a custom registry programmatically. Provide a name and [visibility](#visibility-types) for the registry for the `name` and `visibility` parameters, respectively.
...
See the  [`wandb.Api().create_registry()`](/models/ref/python/#method-apicreate_registry) method reference for a full list of parameters that you can provide when you create a custom registry.
</Tab>
</Tabs>
...
See [Create a custom registry](./create_registry#create-a-custom-registry) for more information on how assign the visibility of a custom registry when a team administrator creates it.
