...
Next, upload the dataset to W&B. To do this, create an [artifact](/models/artifacts/) object and add the dataset to that artifact. 
...
Use the [`link_model`](/models/ref/python/experiments/run#link_model) API to log model one ore more files to a W&B run and link it to the [W&B Model Registry](./).
...
Download the [model version](./model-management-concepts#model-version) from W&B that you want to evaluate. Use the `use_model` API to access and download your model.
...
Mark a model version ready for the next stage of your machine learning workflow with a [*model alias*](./model-management-concepts#model-alias). Each registered model can have one or more model aliases. A model alias can only belong to a single model version at a time.
...
You can add an alias to a model version interactively with the W&B App UI or programmatically with the Python SDK. The following steps show how to add an alias with the W&B Model Registry:
