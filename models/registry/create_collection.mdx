...
<Note>
You specify an artifact's type when you create that artifact object. See the `type` field in `wandb.Artifact()`. Replace values enclosed in `<>` with your own values:
...
The following code snippet shows how to programmatically create a collection. Replace values enclosed in `<>` with your own values:
