...
Use aliases to reference a specific [artifact version](/models/artifacts/create-a-new-artifact-version/) in W&B Registry. Aliases provide stable, human-readable labels for versions that are current, in production, or otherwise important. [W&B automatically assigns aliases](/models/registry/aliases/#default-aliases) to each artifact you link with the same name. You can also [create custom aliases](/models/registry/aliases/#custom-aliases) and use protected aliases to safeguard important versions.
...
The following code snippet demonstrates how to link an artifact version to a collection and add aliases to that artifact version with the Python SDK. Replace values enclosed in `<>` with your own values:
...
Use a [protected alias](/models/registry/aliases/#protected-aliases) to identify an artifact version and prevent it from being modified or deleted. Protected aliases are useful for versions that downstream systems depend on. For example, use a `production` protected alias for artifact versions used in your organization's production ML pipeline.
...
After an admin creates a protected alias, it can be added to an artifact version either in the UI or programmatically with the Python SDK. See [Custom aliases](#custom-aliases).

## Find existing aliases

You can find aliases with the [global search bar in the W&B Registry](/models/registry/search_registry/#search-for-registry-items). To find a protected alias:
