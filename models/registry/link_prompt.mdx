---
title: Link an AI prompt to a collection
---

Track, version, and manage your AI prompts in the W&B Registry by linking them to a collection. *Linking* a prompt to a [collection](/models/registry/create_collection)  moves the version from a [private, project-level scope to a shared, organization-level scope](/models/registry/create_registry#visibility-types).
...
To track and manage your AI prompts, use Weave Python SDK. The Weave Python SDK enables you to create, publish, and link prompt objects to a collection in a registry.
...
2. Create a Weave-friendly prompt object with [`weave.StringPrompt()`](/weave/reference/python-sdk#class-stringprompt).
    <Tip>
...
To fetch a prompt by its alias, use the following URI path:
...
W&B autogenerates the URI path and sample code to fetch your prompt. To view the sample code:
