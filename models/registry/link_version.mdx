...
<Note>
The term "type" refers to the artifact object's type. When you create an artifact object ([`wandb.Artifact`](/models/ref/python/experiments/artifact)), or log an artifact ([`wandb.Run.log_artifact()`](/models/ref/python/experiments/run#log_artifact)), you specify a type for the `type` parameter. 
...
Programmatically link an artifact version to a collection with [`wandb.Run.link_artifact()`](/models/ref/python/experiments/run#link_artifact).
