...
<Note>
Use `wandb.Run.link_artifact()` to [link an artifact version during an active run](#link-an-artifact-version-within-the-context-of-a-run). Use `wandb.Artifact.link()` to [link an *existing artifact version* without creating a run](#link-an-artifact-version-outside-the-context-of-a-run).
</Note>

For either method, specify the artifact name and type when you create the `Artifact` object. Pass the destination collection path to the `target_path` parameter of the linking method.

The target path consists of the `"wandb-registry"` prefix, the name of the registry, and the name of the collection separated by a forward slashes:

```text
wandb-registry-{REGISTRY_NAME}/{COLLECTION_NAME}
```

##### Link an artifact during a run

Use `wandb.Run.link_artifact()` to link an artifact version during an active run. W&B associates the artifact version with the run and records the run in your project.

To link an artifact version:

1. Initialize a run with `wandb.init()`.
2. Create an Artifact object and add one or more files.
3. Call `wandb.Run.link_artifact()` and specify the target collection.
...
##### Link a committed artifact directly

Use `wandb.Artifact.link()` when you already have a committed artifact version and want to link it without initializing a run.

1. Create an `Artifact` object and add one or more files.
2. Call `wandb.Artifact.link()` and specify the target collection.
