...
You can [link an artifact version to a collection](https://docs.wandb.ai/guides/core/registry/link_version.md#link-an-artifact-version-to-a-collection) programmatically with the W&B Python SDK or interactively with the W&B App.
...
<Note>
The term "type" refers to the artifact object's type. When you create an artifact object ([`wandb.Artifact`](/models/ref/python/experiments/artifact)), or log an artifact ([`wandb.init.log_artifact`](/models/ref/python/experiments/run#log_artifact)), you specify a type for the `type` parameter. 
...
* The types of artifacts that collection permits. For more information about collection types, see "Collection types" within [Create a collection](./create_collection.md).
...
For both approaches, specify the name of the artifact (`wandb.Artifact(name="<name>"`), the type  of artifact (`wandb.Artifact(type="<type>"`), and the `target_path` of the collection and registry you want to link the artifact version to.
...
You can [view a linked artifact's metadata, version data, usage, lineage information](/guides/core/registry/link_version.md#view-linked-artifacts-in-a-registry") and more in the Registry App.
