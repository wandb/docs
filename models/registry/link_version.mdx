...
Programmatically link an artifact version to a collection with [`wandb.init.Run.link_artifact()`](/models/ref/python/experiments/run#link_artifact).

<Note>
Before you link an artifact to a collection, ensure that the registry that the collection belongs to already exists. To check that the registry exists, navigate to the W&B Registry and search for the name of the registry.
</Note>
...
View a linked artifact's metadata, version data, usage, lineage information and more in the W&B Registry.
