...
For example, the following image shows artifacts created and used throughout an ML experiment:
...
Mark an artifact as an input or dependency of a run with the `wandb.Run.use_artifact()` method

The following code snippet shows how to use the `wandb.Run.use_artifact()` method. Replace values enclosed in angle brackets (`< >`) with your values:
...
Use ([`wandb.Run.log_artifact`](/models/ref/python/experiments/run#log_artifact)) to declare an artifact as an output of a run.

The following code snippet shows how to use the `wandb.Run.log_artifact()` method. Ensure to replace values enclosed in angle brackets (`< >`) with your values:
