...
| API Compatibility | Uses `wandb.Run.link_model()` and MR-specific patterns | Modern SDK APIs (`link_artifact()`, `use_artifact()`) with auto-redirection |
...
- Explore enhanced functionality in the Registry such as:
    - [Organization-level access](/models/registry/create_registry/#visibility-types)
    - [Role-based access control](/models/registry/configure_registry/)
    - [Registry-level lineage tracking](/models/registry/lineage/)
