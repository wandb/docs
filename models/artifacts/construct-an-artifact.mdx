...
W&B uses the `name` and `type` you provide to create a directed acyclic graph in the W&B App. See [Explore and traverse artifact graphs](./explore-and-traverse-an-artifact-graph) for more information.
...
See [Add files to an artifact](/models/artifacts/construct-an-artifact#add-files-to-an-artifact), for more information about adding different file types to an artifact.

### Save your artifact to the W&B server

Use the run object's [`wandb.Run.log_artifact()`](/models/ref/python/experiments/run#log_artifact) method to save the artifact.
...
The following sections show how to add different types of objects to an artifact. Assume you have a directory with the following structure as you read through the examples:
...
To point an artifact to content stored outside of W&B (such as in an object store) without copying the underlying bytes, add an external URI reference with the [`wandb.Artifact.add_reference()`](/models/ref/python/experiments/artifact#method-artifact-add-reference) method. Artifacts track checksums and other information for reproducibility if the URI has a scheme that the W&B library supports. Replace the `'uri'` string with your own URI. Optionally pass the desired path within the artifact for the name parameter.
