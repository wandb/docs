...
Pass desired values to update the `description`, `metadata`, and `alias` of an artifact. Call the `save()` method to persist changes on the W&B servers. You can do this while a run is active, or with [`wandb.Api`](/models/ref/python/public-api/api) without calling `wandb.init()`.
