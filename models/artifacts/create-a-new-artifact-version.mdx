...
<Note>
Create an incremental artifact when you need to apply changes to a subset of files in an artifact, where the size of the original artifact is larger.
</Note>
...
Allow a collection of runs to collaborate on a version before committing it. This is in contrast to single-run mode described previously, where one run provides all the data for a new version.
...
The previous code examples combined look like the following: 
