...
<Tabs>
<Tab title="Associate with a run">
...
Add, modify, or remove a subset of files from a previous artifact version without re-indexing files that didn't change. Adding, modifying, or removing a subset of files from a previous artifact version creates a new artifact version known as an *incremental artifact*.

<Frame>
    <img src="/images/artifacts/incremental_artifacts.png" alt="Incremental artifact versioning"  />
</Frame>

There are three common use cases for creating an incremental artifact:
...
To create an incremental artifact, you need to create a draft of the previous artifact version. You can create a draft from any committed artifact version.



<Tabs>
    <Tab title="Use as run input">
...
Use the Public API when you want to fetch the artifact without recording a run input relationship.
