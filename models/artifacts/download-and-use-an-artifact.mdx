...
After downloading an artifact, call `Artifact.verify()` to check that the files on disk match the artifact manifest.

For example, the following code snippet downloads an artifact as part of a run and verifies the downloaded files:
...
[`wandb.Artifact.verify()`](/models/ref/python/experiments/artifact#method-artifact-verify) recomputes checksums for local files and compares them with the checksums stored in the artifact manifest. It raises a `ValueError` if a file is missing, has changed, or if the directory contains files that are not part of the artifact.

`wandb.Artifact.verify()` does not verify [reference entries](/models/artifacts/track-external-files).

<Note>
Artifacts created with `digest_algorithm="XXH128"` require W&B SDK 0.29.0 or newer for `Artifact.verify()`. Older SDK versions do not understand XXH128 manifest entries and verification fails.
</Note>
