---
description: Delete artifacts interactively with the W&B UI or programmatically with
  the W&B Python SDK.
title: Delete an artifact
---

Delete artifacts interactively with the W&B UI or programmatically with the W&B Python SDK. When you delete an artifact, W&B marks that artifact as a *soft-delete*. In other words, the artifact is marked for deletion but files are not immediately deleted from storage. 
...
graph TB
    ...
    DeleteMethod -->|UI| UIDelete[Delete via W&B UI]
...
<Note>
When W&B deletes an artifact due to a TTL policy, from the W&B Python SDK, or from the W&B UI, the artifact is initially *soft-deleted*. Soft-deleted artifacts are permanently deleted during garbage collection.
</Note>
...
Delete an artifact version interactively with the W&B UI or programmatically with the W&B Python SDK.

<Tabs>
<Tab title="W&B UI" value="ui">
