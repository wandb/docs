# Serverless RL API Reference Documentation Scripts

This directory contains scripts to manage and update the Serverless RL API reference documentation. The upstream service hostname remains `api.training.wandb.ai`; only the product branding and docs paths have changed.

## Overview

The Serverless RL API documentation system manages:
...
Downloads the latest OpenAPI spec from the Serverless RL API service and saves it locally.
...
Updates the Serverless RL API landing page with the current list of endpoints from the OpenAPI spec.
...
Master script that runs all Serverless RL API generation steps in order.
...
{
  "group": "API Reference",
  "openapi": "serverless-rl/api-reference/openapi.json",
  "pages": ["serverless-rl/api-reference"]
}
```
...
1. **Check the generated files:**
   ```bash
   ls -la serverless-rl/api-reference/openapi.json
   cat serverless-rl/api-reference.mdx | grep "## Available Endpoints" -A 20
   ```
...
2. **Test locally with Mintlify:**
   ...
   mint dev
   ```
   Navigate to `/serverless-rl/api-reference` to verify the endpoints are listed correctly.

3. **Validate configuration:**
   ```bash
   grep "serverless-rl/api-reference" docs.json
   ```
...
**Issue**: The upstream Serverless RL API's OpenAPI spec was missing `tags` fields on the health endpoints (`/v1/health` and `/v1/system-check`). This caused Mintlify to place these endpoints in an incorrect navigation hierarchy with an extra "API Reference" layer in the breadcrumb.
