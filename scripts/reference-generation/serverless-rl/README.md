# Serverless Training API Reference Documentation Scripts

This directory contains scripts to manage and update the Serverless Training API reference documentation. The upstream service hostname remains `api.training.wandb.ai`; only the product branding and docs paths have changed.

## Overview

The Serverless Training API documentation system manages:
...
Downloads the latest OpenAPI spec from the Serverless Training API service and saves it locally.
...
```json
{
  ...
  "pages": ["serverless-training/api-reference"]
}
```
...
1. **Check the generated files:**
   ```bash
   ls -la serverless-training/api-reference/openapi.json
   cat serverless-training/api-reference.mdx | grep "## Available Endpoints" -A 20
   ```
...
2. **Test locally with Mintlify:**
   ...
   mint dev
   ```
   Navigate to `/serverless-training/api-reference` to verify the endpoints are listed correctly.

3. **Validate configuration:**
   ```bash
   grep "serverless-training/api-reference" docs.json
   ```
...
**Issue**: The upstream Serverless Training API's OpenAPI spec was missing `tags` fields on the health endpoints (`/v1/health` and `/v1/system-check`). This caused Mintlify to place these endpoints in an incorrect navigation hierarchy with an extra "API Reference" layer in the breadcrumb.
