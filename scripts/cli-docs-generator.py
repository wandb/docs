...
def generate_index_markdown(cmd_info: Dict[str, Any], subcommands_only: bool = False) -> str:
...
def write_command_docs(cmd_info: Dict[str, Any], output_dir: Path, parent_path: str = ""):
    ...
    if is_root:
        ...
        with open(index_path, 'w') as f:
            f.write(generate_index_markdown(cmd_info))
...
def main():
    ...
    if output_dir.exists() and any(output_dir.iterdir()):
        if args.interactive:
            print(f"⚠️  Warning: This will clear and regenerate ALL documentation in {output_dir}")
            response = input("Continue? (y/N): ")
            if response.lower() != 'y':
                print("Aborted.")
                sys.exit(0)
        
        # Clear entire directory to remove any vestigial files from removed commands
        print("Clearing existing documentation directory...")
        files_removed = 0
        dirs_removed = 0
