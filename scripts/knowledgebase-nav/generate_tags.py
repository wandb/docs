...
Phase 2. Generate tag pages (group-generator pattern):
    For each (product, tag) pair, renders a tag page MDX file containing
    Mintlify Card components that link to the tagged articles.
...
def _extract_body(body_and_footer: str) -> str:
    """
    Separate the article body from the trailing keyword footer.

    Uses ``str.rfind("\\n---")`` on the text after front matter: everything
    before the last match is the body, and the match begins the footer (the
    horizontal rule and Mintlify Badges). Repository articles should use only
    one such break before the footer so previews and sync stay correct.
    ...
    """
    # Last "\n---" marks the start of the auto-managed footer (see README).
    last_sep = body_and_footer.rfind("\n---")
    if last_sep != -1:
        body = body_and_footer[:last_sep]
    else:
...
def plain_text(body: str) -> str:
    ...
    Steps (order matters): fenced code blocks, HTML comments, reference links,
...
def _split_main_body_and_trailing_footer(body_and_footer: str) -> Tuple[str, str]:
    """
    Split ``body_and_footer`` into main article text and the trailing block.

    Uses the same rule as ``_extract_body``: the last ``\\n---`` substring
    starts the footer. If none is found, the entire string is main body and
    the footer is empty.
    """
    last_sep = body_and_footer.rfind("\n---")
    if last_sep == -1:
        return body_and_footer, ""
    return body_and_footer[:last_sep], body_and_footer[last_sep:]
