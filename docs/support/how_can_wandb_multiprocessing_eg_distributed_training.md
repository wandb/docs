...
1. Call `wandb.init` in all your processes, using the [group](../guides/runs/grouping.md) keyword argument to define a shared group. Each process will have its own wandb run and the UI will group the training processes together.
...
:::info
Check out the [Distributed Training Guide](../guides/track/log/distributed-training.md) for more detail on these two approaches, including code examples with Torch DDP.
:::