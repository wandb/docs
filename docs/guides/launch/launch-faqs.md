...
### Are there best practices for using Launch effectively?
  ...
  3. Use `wandb.config` to read and write your hyperparameters, so that they can be overwritten when re-running a job. Check out [this guide](/guides/launch/create-launch-job#making-your-code-job-friendly) if you use argsparse.
...
You can limit the CPU/GPU, memory, and other requirements at the launch queue or job run level, in the resource config. For more information about setting up queues with resource limits on Kubernetes see [here](/guides/launch/kubernetes#queue-configuration). 
