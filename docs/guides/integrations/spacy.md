...
| `model_log_interval`   | `Optional int`. `None` by default. If set, [model versioning](../data-and-model-versioning/model-versioning.md) with [Artifacts](../artifacts/intro.md)will be enabled. Pass in the number of steps to wait between logging model checkpoints. `None` by default. |
