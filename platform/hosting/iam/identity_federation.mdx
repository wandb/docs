...
<Note>
Identity federation is available in preview for Multi-tenant Cloud, Dedicated Cloud, and Self-Managed. An [Enterprise license](/platform/hosting-enterprise-license) is required. For details or assistance, contact your AISE or [support](mailto:support@wandb.com).
</Note>
...
* Typically, the `aud` claim in the JWT should be equal to the name of the W&B organization which houses the project that you are accessing as part of your AI workflow.
  - Some organizations have different requirements for customizing the `aud` value. Specify a custom audience using the environment variable `FEDERATED_AUTH_AUDIENCES` to a string with a comma-separated list of values.
  - To skip audience valudation on [Dedicated Cloud](/platform/hosting/hosting-options/dedicated-cloud) or [Self-Managed](/platform/hosting/hosting-options/self-managed) instances, you can set  the environment variable `FEDERATED_AUTH_AUDIENCES` to `wandb`.
