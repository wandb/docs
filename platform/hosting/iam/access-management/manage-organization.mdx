...
Organization roles control administrative actions at the organization scope. They're separate from [team roles](#assign-or-update-a-team-members-role), which control what a user can do inside a specific team, and from [Models seat and Weave access](#assign-or-update-a-users-access), which control product access and billing.
...
In organizations marked as hidden, members can view profiles only for themselves, for users who share a team with them, or if they're an organization admin.
...
| Add themselves to a team as a member or viewer               | Admin           |        |   X   |

<Note>
On Self-Managed deployments, non-admin users might be allowed to create teams when your instance administrator enables that option.
</Note>
...
| Configure organization alert thresholds                      | Admin           |        |   X   |
...
| View billing alert thresholds                                | Read            |        |   X   |

Users designated as **billing admins** can perform billing actions even if their organization role is **Member**. Billing admins can't perform other organization admin actions such as inviting users or creating teams.
...
| Manage organization user secrets (Dedicated Cloud and Self-Managed) | Admin     |        |   X   |
...
3. Select the **Users** tab.
4. Hover your mouse next to the name of the user you want to delete. Select the **action (<Icon icon="ellipsis" iconType="solid"/>)** menu when it appears. 
