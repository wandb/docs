...
keywords: ["invite users", "org admin", "add team members", "user seats", "JIT provisioning", "just-in-time provisioning", "auto provisioning"]
---
...
Auto-provisioning streamlines onboarding by letting users join your organization automatically through SSO, so admins don't need to send individual invitations. This is W&B's equivalent of just-in-time (JIT) provisioning: an account is created the first time a user with a matching email domain signs in through SSO.
...
Domain capture lets you automatically add people with a company email address, such as `@example.com`, to W&B. This helps all your employees join the right organization and ensures that new users don't create assets outside your company jurisdiction. 

<Note>Domain capture is not available on free or trial plans.</Note>

Domains must be unique; you can't use a domain that's already in use by another organization.
...
<Tab title="Dedicated Cloud or Self-Managed">
Contact your W&B Account Team to configure domain capture. Afterward, when a user creates a W&B account with a company email address, your deployment automatically prompts them to contact your admin to request access.
