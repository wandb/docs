...
- **Public Client (no secret)**: Uses PKCE without a client secret. Simpler to configure, suitable for most deployments.
- **Confidential Client (with secret)**: Uses PKCE with a client secret for enhanced security. Required if you need to set the `GORILLA_OIDC_SECRET` environment variable.
