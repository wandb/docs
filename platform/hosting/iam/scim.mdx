...
- **Required**: `name` (bucket name), `provider` (one of `AWS`, `GCP`, `AZURE`, `MINIO`, `COREWEAVE`).
...
These examples show how to create a team without custom storage and with BYOB storage on a specific provider. Select a tab for the request format.
