...
<Note>
This operation removes a user from a specific registry. It doesn't revoke organization-level Registry access. To revoke Registry access entirely, use [Update Registry access](#update-registry-access) on **Multi-tenant Cloud**. {/* and on **Dedicated Cloud** and **Self-Managed** v0.83.0+, or [Update Models seat](#update-models-seat) on **Dedicated Cloud** and **Self-Managed**. {/* v0.82.0 and older. */}
</Note>
...
| Set `registryAccess` on create/update | &check;   | -            |
| Revoke Registry access with `modelsSeat: none` | - | check;      |
| Revoke Registry access with `registryAccess: none` | &check; | - |
