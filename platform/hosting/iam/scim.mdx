...
In your Entra enterprise application's provisioning settings, set **Tenant URL** to your W&B SCIM base URL with the `aadOptscim062020` query parameter appended:
...
<Note>
When you append `aadOptscim062020` to the tenant URL, user deactivation from Entra's **Provision on demand** UI in the Microsoft Entra admin center might not work because that UI still sends string boolean values. To test deactivation manually, send a `PATCH` request with `{"active": false}`. See [Deactivate user](#deactivate-user).
</Note>
...
| Source attribute (Entra) | Target attribute (W&B) | Apply this mapping | Match objects using this attribute |
| --- | --- | --- | --- |
| `mail` | `emails[type eq "work"].value` | Always | Yes |
| `Not([IsSoftDeleted])` | `active` | Always | |
| `displayName` | `displayName` | Always | |
| `displayName` | `userName` | Only on object creation | |
