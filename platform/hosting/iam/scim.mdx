...
<Tabs>
<Tab title="Provision Service Account Response">
```bash
(Status 201)
```
...
A duplicate `userName` returns `409 Conflict`. A missing or unknown `defaultTeam` returns `400 Bad Request`.
...
| Provision service accounts (`POST` with `accountType`) | &check; | &check; (v0.81.0+) |
| Deprovision service accounts (`DELETE` by id) | &check; | &check; (v0.81.0+) |
