...
Deletion in W&B or [Public API](/models/ref/python/public-api/api) updates W&B metadata first. Removing a run, artifact, or file from the product doesn't guarantee an immediate drop in reported bucket usage. Object storage cleanup runs as background work that can lag, especially on busy instances.
...
- [Delete runs in W&B](/models/runs/delete-runs#ui) or [with Python](/models/runs/delete-runs#python) when you no longer need them.
...
If deletions don't appear correctly in W&B after you use the Public API, upgrade the W&B Python SDK to a current release and retry. Large per-run file counts can increase how long background cleanup takes across the instance.
