...
W&B Weave on Self-Managed deployments uses ClickHouse. ClickHouse is not required for every Self-Managed installation.

Before you upgrade W&B Server or change your ClickHouse version, see [ClickHouse compatibility for upgrades](/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) in the Operator guide and the compatibility notes in [Supported W&B Server releases](/release-notes/server-releases).
