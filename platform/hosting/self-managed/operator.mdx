...
Self-Managed deployments that use an external ClickHouse cluster must verify ClickHouse compatibility before upgrading W&B Server or enabling Models OLAP features.

### When this applies

W&B Server 0.81.0+ introduces schema migrations that require ClickHouse 26.2 or newer when any of the following Models OLAP features are enabled in your Helm values:
...
1. Confirm whether Registry Search, Run Store Accelerator, or History is enabled in your deployment.
2. If any are enabled, upgrade **both** ClickHouse Server and ClickHouse Keeper to 26.2 or newer **before** upgrading W&B Server to 0.81 or newer. On older ClickHouse versions, the migration can fail on startup and prevent `api` and `glue` pods from starting.
3. If your deployment uses **Weave** with ClickHouse, coordinate your target ClickHouse version with [Weave self-managed guidance](/weave/guides/platform/weave-self-managed#upgrade). Models OLAP and Weave can have different ClickHouse version constraints. Contact [support](mailto:support@wandb.com) before upgrading if you use both.
