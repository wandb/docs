...
- **Restart the affected deployments manually.** Deployment names vary by version and install (the app deployment usually carries a `-bc` suffix, e.g. `<release>-app-bc`), so list them first, then restart the ones that consume the changed config — the `app` component covers most auth/SSO changes:
