...
Before you upgrade MySQL, you can run the [MySQL Shell upgrade checker](https://dev.mysql.com/doc/mysql-shell/8.4/en/mysql-shell-utilities-upgrade.html) against your database to surface compatibility issues for your target version. Resolve any errors or warnings in the checker output before you proceed, following the MySQL documentation for your distribution.
...
2. Drain and shut down your W&B deployment so the application is not writing to the database during the upgrade.
    ```bash
    # Scale down the `wandb-operator` deployment to 0 replicas
    kubectl scale deployment wandb-operator -n <NAMESPACE> --replicas=0

    # Scale down the `wandb-server` deployment to 0 replicas
    kubectl scale deployment wandb-server -n <NAMESPACE> --replicas=0
    ```
3. Take a full backup of your MySQL database according to your MySQL distribution's documentation.
...
6. When MySQL is up, scale your W&B deployment to its original size. In each command, replace `[N]` with the desired number of replicas. These commands are similar to those in step 2, run in reverse order.
    ```bash
    # Scale the `wandb-server` deployment to the original number of replicas
    kubectl scale deployment wandb-server -n <NAMESPACE> --replicas=[N]

    # Scale the `wandb-operator` deployment to the original number of replicas
    kubectl scale deployment wandb-operator -n <NAMESPACE> --replicas=[N]
    ```
7. Run `wandb verify` to validate your deployment. The command runs a series of checks and reports results to `STDOUT`. If it reports problems, make adjustments and run it again. For setup and login steps, see [Verify the installation](#verify-the-installation).
8. After validation completes, users can resume normal operations.
