...
1. To sort the list by user status, click the **Last Active** column label. Each user's status is one of the following:
    ...
    * `-`: A hyphen indicates that the user has not yet been active within the organization.
    ...
    A user is _active_ if they perform any auditable action scoped to the organization _after May 8, 2025_. For a full list, refer to [Actions](/platform/hosting/monitoring-usage/audit-logging#actions) in the Audit Logging page.
</Tab>
</Tabs>
