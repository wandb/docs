...
- **Track experiments**: View run status, metrics, and line plots for your experiments in real time.
    ...
    - **View console logs**: View [console logs](/models/runs/view-logged-runs#logs) for active or completed runs, or download logs for completed runs. ANSI color output from your scripts renders faithfully, and colors adapt to light and dark mode. The app polls active runs for new log lines every 5 seconds. Even for runs with hundreds of thousands of log lines, the most recent output appears immediately, and older lines load in the background. Log search stays responsive even for large outputs.
