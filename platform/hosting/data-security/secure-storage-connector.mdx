...
Azure workload identity is in preview for Dedicated Cloud and Self-Managed. These steps configure instance-level Azure Blob Storage authentication with a user-delegation shared access signature (SAS). For Dedicated Cloud, coordinate the rollout with your W&B team. For Self-Managed, the deployment administrator performs the setup.
...
1. Configure the `azure.workload.identity/client-id` annotation on each of those service accounts. In `component` mode, set each component's `serviceAccount.annotations` in your Helm values. If infrastructure creates the account, manage its annotations there. For example, these values configure the API and Console accounts. Repeat the configuration for each enabled storage consumer:
    ...
    Replace `[CLIENT-ID]` with the identity's client ID. `component` mode preserves component accounts and doesn't annotate all of them from the global identity block. Verify the rendered service account names against the federation subjects.
...
1. Merge the following values into your existing configuration. Replace `[TENANT-ID]`, `[CLIENT-ID]`, and `[PRINCIPAL-ID]` with the managed identity's values. Preserve the existing storage account name, container path, and other deployment settings.
    ...
    For Self-Managed, these values can come from your deployment configuration or the user spec. For Dedicated Cloud, your W&B team manages the default bucket through infrastructure configuration. Use Console to configure a BYOB override, rather than to opt the infrastructure default bucket into workload identity.
