...
1. <a id="coreweave-org-id"></a>**Multi-tenant Cloud**: Obtain your organization ID, which is required for your bucket policy.
    1. Log in to [W&B](https://wandb.ai/site).
...
1. <a id="coreweave-customer-namespace" aria-label="CoreWeave customer namespace"></a>**Dedicated Cloud** / **Self-Managed**: Obtain your customer namespace, which is required for your bucket policy.
    1. In W&B, click your user profile icon, then click **System Console**.
...
For **Self-Managed**, you can configure instance level BYOB using the W&B UI:
...
You can configure team level BYOB when you create a team in the W&B UI or using the [SCIM API](/platform/hosting/iam/scim#create-team) (POST Groups with optional `storageBucket`). You have two options:
