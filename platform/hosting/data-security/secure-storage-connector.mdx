...
1. Choose an authentication method:
    ...
    - **Workload identity**: Grant an Azure managed identity access to your storage account and configure W&B to use it. You don't need to share a storage account key. Follow the [workload identity instructions for your deployment type](#set-up-azure-workload-identity).
...
For **Dedicated Cloud**: Share the bucket details with your W&B team, who configures your instance. For Azure workload identity, [grant access to your storage account](#set-up-azure-workload-identity) and provide the storage account name, container name, and optional path.
...
1. Ensure that the identity or credentials for the selected provider have access to the bucket. For Azure, select an **Authentication** method:
    - **Storage account key**: Enter the **Storage account key**.
    - **Workload identity (user-delegation SAS)**: Use the identity configured by your deployment administrator. If this option is unavailable, complete the [identity setup](#set-up-azure-workload-identity).
1. Click **Save**.
...
If a setting isn't editable, contact your deployment administrator.

#### Set up Azure workload identity

Workload identity lets W&B access your Azure bucket without a storage account key. Select your deployment type for the setup instructions.

<Tabs>
<Tab title="Dedicated Cloud">
W&B configures the deployment. You configure access to your Azure storage account:

1. Contact your W&B team with your storage account's tenant ID to confirm which managed identity to use.
1. If the identity is in your Azure tenant, use its principal ID for the role assignments below. Otherwise, create or select a managed identity in your tenant and configure its federated credentials using the issuer, subject, and audience values provided by W&B. A managed identity [can't directly access storage in another tenant](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/managed-identities-faq#can-i-use-a-managed-identity-to-access-a-resource-in-a-different-directorytenant).
1. Grant the identity's principal **Reader** and **Storage Blob Data Contributor** at storage-account scope on your BYOB storage account.
1. Share the storage account name, container name, optional path, and the identity's tenant ID and client ID with your W&B team.

After W&B configures the connection, verify access by uploading and downloading a run file or artifact.
</Tab>
<Tab title="Self-Managed">
As the deployment administrator, configure workload identity in Azure and Kubernetes, then select it in System Console.
...
1. Create or select an Azure managed identity and record its tenant ID, client ID, and principal ID. Grant its principal **Reader** and **Storage Blob Data Contributor** at storage-account scope on your BYOB storage account.
1. Configure `global.azureStorageIdentity` in your Helm values with the identity's tenant ID, client ID, and principal ID. Set up federation and the `azure.workload.identity/client-id` annotation for the Kubernetes service accounts that access storage. For Helm configuration details, see [Azure workload identity configuration](https://github.com/wandb/helm-charts/blob/operator-wandb-0.44.13/charts/operator-wandb/README.md#azure-managed-storage-workload-identity).
1. Follow the [Console procedure](#instance-level-byob) to select workload identity and save your bucket settings. Verify access by uploading and downloading a run file or artifact.
</Tab>
</Tabs>
