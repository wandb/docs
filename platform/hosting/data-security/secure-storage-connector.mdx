...
1. Choose an authentication method:
    ...
    - **Workload identity**: Don't generate or share a storage account key. Instead, the W&B System Console shows the principal ID of the deployment's managed identity. Grant that principal the **Reader** and **Storage Blob Data Contributor** roles on the storage account. W&B uses one Azure managed identity per instance. Kubernetes service accounts are configured according to each component's permissions. <Note>For Self-Managed deployments, requires Operator Helm chart [v0.44.0+](https://github.com/wandb/helm-charts/releases/tag/operator-wandb-0.44.0).</Note>

    Workload identity is an opt-in migration. Enabling it for W&B-managed storage preserves the authentication method of existing BYOB configurations, including storage account keys and bucket-specific identities.
