...
1. Choose an authentication method:
    ...
    - **Workload identity**: Use the deployment's Azure managed identity instead of a storage account key. Complete the [Azure workload identity setup](#set-up-azure-workload-identity) before selecting this method. Grant the identity's principal ID the **Reader** and **Storage Blob Data Contributor** roles at storage-account scope. The principal ID is different from the client ID.

    Workload identity is an opt-in migration. Enabling it for W&B-managed default storage preserves existing BYOB keys and bucket-specific identities. Changing the instance bucket doesn't change the authentication of separately configured team buckets.
...
For **Dedicated Cloud**: Share the bucket details with your W&B team, who configures your instance. For Azure, include the storage account, container, optional path, and preferred authentication method. Your W&B team provides workload identity and federation details and manages the deployment configuration. You grant the required roles on your storage account. If you use key authentication, share the key through a secure mechanism.

For **Self-Managed**, configure instance level BYOB in the W&B System Console. For Azure workload identity, first complete the [identity setup](#set-up-azure-workload-identity).
...
1. In **Bucket Storage**, select the **Provider** and enter the bucket details. For Azure, select **Azure Blob Storage (az)**, enter the **Storage account**, and enter the **Blob container** separately.
1. Optional: Enter the **Path** to use in the new bucket.
1. Ensure that the identity or credentials for the selected provider have access to the bucket. For Azure, select an **Authentication** method:
    - **Storage account key**: Enter a nonempty **Storage account key**. An available managed identity doesn't prevent you from using a key.
    - **Workload identity (user-delegation SAS)**: Use the configured deployment identity. This option is available when the effective configuration contains the Azure tenant and client IDs. The **Identity** information is read-only. Selecting this option doesn't create an identity, grant roles, or configure federation.
1. If you change authentication for the same storage account and container, review the warning. Saving creates a bucket override that takes precedence over the default bucket, including future authentication changes.
1. Click **Save**.
...
If infrastructure sets a named `global.bucket` in the resource spec, Console locks the corresponding fields. Update the configuration through its infrastructure owner. A configured `global.defaultBucket` remains editable through a user-spec bucket override.

#### Set up Azure workload identity

Azure workload identity is in preview for Dedicated Cloud and Self-Managed. It authenticates to Azure Blob Storage with a user-delegation shared access signature (SAS). Select your deployment type for the steps you need to perform.

<Tabs>
<Tab title="Dedicated Cloud">
W&B manages the deployment's software versions, managed identity, Kubernetes service-account annotations, and cluster federation. W&B also controls authentication for its managed default bucket through infrastructure configuration. You don't need to apply Helm values or configure Kubernetes service accounts.

For an Azure BYOB bucket that uses the deployment identity:

1. Obtain the deployment's managed identity principal ID from your W&B team or the read-only identity information in System Console.
1. Grant that principal **Reader** and **Storage Blob Data Contributor** at storage-account scope on your BYOB storage account.
1. Share the storage account name, container name, and optional path with your W&B team, and request workload identity authentication for that bucket.

If your bucket requires a different identity or Azure tenant, coordinate its bucket-specific federation with your W&B team. Enabling workload identity for the W&B-managed default bucket doesn't migrate existing BYOB keys or bucket-specific identities.
</Tab>
<Tab title="Self-Managed">
The deployment administrator configures the identity, federation, and Helm values. These steps apply to Self-Managed deployments, including deployments in your cloud account or on-premises.

Use the following versions for this workflow:

| Component | Required version |
| --- | --- |
| W&B Server | v0.84.0 or later, including every component that accesses storage |
| Operator Helm chart | [v0.44.13](https://github.com/wandb/helm-charts/releases/tag/operator-wandb-0.44.13) or later |
| W&B System Console | [v2.20.0](https://github.com/wandb/console/releases/tag/v2.20.0) or later for the authentication selector |

Check component image overrides before the rollout. Upgrading the chart alone doesn't add workload identity support to an older server image.

1. Enable OpenID Connect (OIDC) token federation and Azure workload identity for the Kubernetes cluster. For Azure Kubernetes Service (AKS), follow [Configure Microsoft Entra Workload ID](https://learn.microsoft.com/en-us/azure/aks/workload-identity-deploy-cluster).
1. Create or select the Azure managed identity for the instance. Record its tenant ID, client ID, and principal ID. Grant the principal **Reader** and **Storage Blob Data Contributor** on the target storage account. Storage-account scope permits the identity to request the [user-delegation key](https://learn.microsoft.com/en-us/azure/storage/blobs/storage-blob-user-delegation-sas-create-cli#assign-permissions-with-azure-rbac) used to sign blob URLs.
1. Configure a federated identity credential on that identity for each Kubernetes service account used by a storage-consuming component. Use these values:

    | Credential field | Value |
    | --- | --- |
    | Issuer | The Kubernetes cluster's OIDC issuer URL |
    | Subject | `system:serviceaccount:[NAMESPACE]:[SERVICE-ACCOUNT]`, with the actual namespace and service account name |
    | Audience | `api://AzureADTokenExchange` |

    One Azure managed identity can trust multiple Kubernetes service accounts. For an initial rollout, use `component` mode to retain the existing accounts and their Kubernetes permissions. Include Console and any optional storage consumers, such as the parquet metadata cache. Keep existing federation mappings during the rollout and rollback window. The chart doesn't create Azure federated credentials or role assignments.

1. Configure the `azure.workload.identity/client-id` annotation on each of those service accounts. In `component` mode, set each component's `serviceAccount.annotations` in your Helm values. If infrastructure creates the account, manage its annotations there. For example, these values configure the API and Console accounts. Replace `[CLIENT-ID]` with the identity's client ID, and repeat the configuration for each enabled storage consumer:
    ...
    `component` mode preserves component accounts and doesn't annotate all of them from the global identity block. Verify the rendered service account names against the federation subjects.

1. Merge the following values into your existing configuration. Replace `[TENANT-ID]`, `[CLIENT-ID]`, and `[PRINCIPAL-ID]` with the managed identity's values. Preserve the existing storage account name, container path, and other deployment settings.
    ...
    This example selects workload identity for an Azure `defaultBucket`. To keep using the default bucket's key while you prepare federation, explicitly set `defaultBucket.azureAuthMethod: accessKey` and retain its key. Configuring the identity makes tokens available independently of the selected authentication method.

    These values can come from your deployment configuration or the user spec. Use Console to configure a BYOB override. Set the default bucket's authentication method in its deployment configuration.

1. For an instance-level BYOB override, use the [Console procedure](#instance-level-byob), or configure a named `global.bucket` with its own `azureAuthMethod`. A named bucket overrides the entire default bucket, including authentication. It doesn't inherit `defaultBucket.azureAuthMethod` or the default bucket's key. Existing named buckets continue to use their key or existing bucket-specific identity unless you change their configuration.
1. Apply the configuration through your deployment process. Confirm that all replacement pods are ready and that the old replicas have terminated. Test run-file and artifact uploads and downloads, an existing file, and any separately configured BYOB projects. A successful pod rollout alone doesn't verify storage authentication.

When you omit `defaultBucket.azureAuthMethod`, the chart preserves legacy selection: a configured deployment or default-bucket identity selects workload identity, otherwise it uses the key. Set the method explicitly during migration to control when the switch occurs.

The Console selector uses the configured deployment identity. Existing buckets with their own Azure tenant and client IDs retain that identity. If a bucket requires a different identity or tenant, configure its bucket-specific federation separately before changing authentication.

Selecting workload identity stops using the key for that bucket. Remove unused keys from that bucket's configuration after validation. This doesn't revoke or delete Azure storage account keys, turn off Shared Key access in Azure, or remove copies from backups and prior configuration records. To restore key authentication, select `accessKey` and supply a valid key. Weave storage is configured separately.
</Tab>
</Tabs>
