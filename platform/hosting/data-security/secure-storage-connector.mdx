...
1. Choose an authentication method:
    ...
    - **Workload identity**: Don't generate or share a storage account key. The W&B System Console shows the principal ID of the deployment's managed identity. Grant that principal the **Reader** and **Storage Blob Data Contributor** roles on the storage account. W&B configures the identity federation for the deployment.

    Existing instance-level BYOB configurations that use a storage account key continue to work and don't require migration.
