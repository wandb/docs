...
Use the admin UI to rotate your license key from within W&B, without changing your deployment configuration. This method only works for updating licenses that aren't set with an environment variable in the W&B server container, typically in self-managed Docker installations. This method updates the license only. It doesn't change the running W&B Server version.
...
2. Access the **License** page in W&B. Click **Settings** > **License** or browse to `HOST_URL/console/settings/license`, where `HOST_URL` is your W&B Server host URL.
