...
<Note>
This section shows only your personal API keys. To see every key you can manage in your organization, including organization API keys and service account keys, use the **API Keys** tab described in [Manage API keys](/platform/hosting/iam/api-keys).
</Note>
...
- **Name**: An optional descriptive name for the key.
...
To change an API key's display name without modifying the key material:

1. Find the key you want to delete in the API keys table.
1. Click the **action (<Icon icon="ellipsis" iconType="solid"/>)** menu and choose **Edit**.
1. Update the key's **Name**, then click **Save**.
...
1. Find the key you want to delete in the API keys table.
1. Click the **action (<Icon icon="ellipsis" iconType="solid"/>)** menu and choose **Delete**.
...
<Note>
You can't delete a user account that is the **Billing admin** for an organization that has more than one member. Before deleting such an account, transfer the Billing admin role to another user from the [Billing settings](/platform/app/settings-page/billing-settings#billing-admin) page. If you are the organization's only member, deleting your account also deletes the organization.
</Note>

To delete an entire organization account rather than a user account, see [How do I delete my organization account?](/support/models/articles/how-do-i-delete-my-organization-account).
