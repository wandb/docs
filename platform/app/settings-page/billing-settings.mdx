...
Organization admins can visualize usage trends for each category in the **Usage & Alerts** tab. To open it:
...
Usage charts are available on both W&B Multi-tenant Cloud and Dedicated Cloud. The categories in the sidebar depend on which products are active in your deployment, and can include Storage, Tracked Hours, Weave, Inference, Training, WB Agent, and Sandboxes.
...
- **Team**: Choose **All teams** to see organization-wide usage, or select an individual team to see only its usage. The dropdown lists teams from highest to lowest usage and is searchable.
...
When a category has a defined allowance, the chart displays two threshold lines: a gold line at 85% of the allowance and a red line at 100%. These lines mark where [usage alerts](#usage-alerts) fire. Usage can exceed these thresholds. The lines are visual guides only.
