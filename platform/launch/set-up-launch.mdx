...
<Note>
Agents are highly flexible and can be configured to support a wide variety of use cases. The required configuration for your agent will depend on your specific use case. See the dedicated page for [Docker](/guides/launch/setup-launch-docker/), [Amazon SageMaker](/guides/launch/setup-launch-sagemaker/), [Kubernetes](/guides/launch/setup-launch-kubernetes/), or [Vertex AI](/guides/launch/setup-vertex/).
</Note>
...
The launch agent can be configured to build images. You must configure the agent to use a container builder if you intend to use launch jobs created from git repositories or code artifacts. See the [Create a launch job](/guides/launch/create-launch-job/) for more information on how to create a launch job. 
...
See the [Set up Kubernetes](/guides/launch/setup-launch-kubernetes/) for details about the Kaniko builder.
</Note>
...
To learn more about how to configure the agent to interact with a container registry, see the [Advanced agent set](/guides/launch/setup-agent-advanced/) up page.
...
In some use cases, you might want to have a launch agent polling queues from within a Kubernetes cluster. See the [Advanced queue set up page](/guides/launch/setup-queue-advanced/) for more information.