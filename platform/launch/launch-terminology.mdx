...
This page defines the core terms and concepts used throughout the W&B Launch documentation. Use it as a reference when you encounter unfamiliar terms in setup guides, tutorials, or W&B.
...
Find jobs you created in W&B under the **Jobs** tab of your project workspace. From there, you can configure jobs and send them to a [Launch queue](#launch-queue) to execute on different [target resources](#target-resources).

## Launch queue

Launch *queues* are ordered lists of jobs to execute on a specific target resource. Launch queues are first-in, first-out (FIFO). No practical limit applies to the number of queues you can have, but a good guideline is one queue per target resource. You can enqueue jobs with W&B, W&B CLI, or Python SDK. You can then configure one or more Launch agents to pull items from the queue and execute them on the queue's target resource.
