...
The following examples show common ways to launch LEET against local run data.
...
LEET has three main views: workspace, single-run, and SYMON. Each view is tailored to a different exploration task, from comparing many runs to focusing on a single run or monitoring system resources in isolation.

### Workspace view

Workspace view is the default LEET experience. It's built to compare multiple runs from the same local `wandb/` directory.
...
Single-run view focuses on one run. It uses the same chart engine as workspace view, but the layout centers on that run:
...
`wandb beta leet symon` opens a standalone system monitor that isn't tied to a W&B run.

Use SYMON to watch local CPU, memory, disk, network, GPU, TPU, IPU, and Trainium metrics while another process runs. It uses the same system metrics chart engine as the workspace and single-run views.
...
The following sections describe LEET's main capabilities and how to use them.

### Multi-run comparison

Workspace mode lets you select multiple local runs and compare their metric series on the same charts. Each selected run receives a stable color. LEET also avoids base color collisions so visible runs are easier to distinguish.

The runs list uses distinct markers for unselected, selected, and pinned runs. Pinning is useful when you select many runs and want one run's metric series to render on top.
...
While you edit a filter, press `tab` to switch between regex and glob modes. Regex mode behaves like a case-insensitive substring search unless the query contains regex metacharacters. Glob mode supports `*` for any sequence and `?` for any single character.
...
Use the mouse wheel to zoom the focused chart. On live system metric charts, LEET defaults to a rolling 10-minute tail window. Zoom out to reveal more history.
...
The media pane renders `wandb.Image` data as ANSI thumbnails directly in the terminal. It's available in both workspace and single-run views.

Open or close the media pane with `3`. When the media pane is focused:

- `w`, `a`, `s`, `d` move between media tiles.
...
LEET reads its settings from a JSON config file. Use the built-in editor to change defaults such as startup view, grid sizes, visible panes, and color palettes.
...
LEET stores configuration in `wandb-leet.json`. By default, LEET writes the file to:
...
Because LEET is keyboard-driven, the following tables list every shortcut grouped by view. Press `h` or `?` inside LEET to open the in-app help screen. Shortcuts vary slightly by view.
...
| `0.26.0` | Metadata-aware run filtering, tags and notes in overview, per-chart log Y, SYMON, bucketed heatmaps, colorblind-friendly palettes, media pane, improved system metrics (overlays, inspection, and zoom), and workspace run color collision prevention. |
