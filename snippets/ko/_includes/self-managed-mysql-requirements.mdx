...
사이징 권장 사항과 설정 파라미터를 포함한 전체 MySQL 요구 사항은 [레퍼런스 아키텍처](/ko/platform/hosting/self-managed/ref-arch/#mysql)를 참조하세요. 데이터베이스 생성 SQL은 [bare-metal guide](/ko/platform/hosting/self-managed/bare-metal/#mysql-database)를 참조하세요. 배포 환경의 데이터베이스 설정 관련 문의는 [지원팀](mailto:support@wandb.com) 또는 담당 AISE에게 연락하세요.