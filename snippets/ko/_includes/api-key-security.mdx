...
API 키는 W&amp;B 계정에 대한 액세스 권한을 제공하므로 비밀번호처럼 보호해야 합니다. 다음 모범 사례를 따르세요:
...
API 키가 노출되었다면, 즉시 W&amp;B 계정에서 해당 API 키를 삭제하고 [지원팀](mailto:support@wandb.ai)로 연락하거나 담당 AISE에게 문의하세요.
...
SDK를 즉시 업그레이드할 수 없는 경우, 임시 방편으로 `WANDB_API_KEY` 환경 변수를 사용하여 API 키를 설정하세요.