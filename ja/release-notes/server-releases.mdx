...
description: "新機能、改善点、バグ修正、アップグレード手順を含む W&B Server のリリースノートを参照できます。"
sidebarTitle: "サポート対象リリース"
rss: true
---

import EolReminder from '/snippets/ja/_includes/release-notes-support-eol-reminder.mdx';
import SelfManagedClickhouseUpgradeNote from '/snippets/ja/_includes/self-managed-clickhouse-upgrade-note.mdx';
...
<Update label="v0.84.1" description="September 2, 2026">
  ...
  * チーム内の最初の 500 件の タグ のみが UI に表示されるバグを修正しました。現在は、1 チームあたり最大 1000 件まですべての タグ が表示されます。
  * パネルグリッドを含む Reports で、report を短時間に複数回操作するとページがクラッシュすることがあるバグを修正しました。
  * `GORILLA_GRAPHQL_DISABLE_INTROSPECTION` が `true` に設定されていても、Tables と アーティファクト で GraphQL のイントロスペクションが引き続き使用されるバグを修正しました。
</Update>

<Update label="v0.84.0" description="2026年8月25日">
  W&amp;B Server 0.84 では、一元管理と一括削除に対応し、組織およびチームの APIキー 管理が簡素化されました。Weave には、BYOB ストレージからのインラインレンダリングや、刷新された Agents ビューと Signals ビューなどが追加されています。このリリースではさらに、散布図で設定可能な参照線、BYOB ストレージ向け Azure ワークロード ID (プレビュー) 、疎なメトリクスを多数含む runs でのクエリパネルの読み込み高速化にも対応しました。

  <div id="support-and-end-of-life">
    ## サポートとサポート終了 (EOL)
  </div>

  * W&amp;B Server v0.72 はサポート終了となりました。

  <div id="features">
    ## 機能
  </div>

  * 折れ線グラフでは、プロット内のすべての Runs で単一のスケールを使用するようになりました。run にカーソルを合わせると、同じ step でメトリクスをログしたすべての Runs で、同じ X-axis の値が表示されます。
    * run が同じ間隔でログする場合、ホバーすると一貫した X 軸の値が表示されます。
    * Distributed runs のように run ごとにログの間隔が異なる場合、最も近いログされたポイント自体が異なるため、異なる X 値が表示されることがあります。値は引き続き同じバケット境界に基づいているため、比較には意味があります。
  * Workspace、セクション、またはパネルの **Data** 設定タブでパネルのモードを指定すると、折れ線グラフでフルフィデリティのデータを表示する方法をカスタマイズできます。

    * **Band モード**では、データが密集している領域の値の範囲を表示し、大規模なデータにおけるばらつきや外れ値の把握に役立ちます。
    * **Line モード**は現在一般提供されており、完全忠実度のトレースを直接表示することで、run の履歴の生データ本来の形状を保持します。

    <Frame>![BandモードとLineモードのチャートを示すアニメーション](/images/release-notes/v0-84-0/chart-line-mode.gif)</Frame>
  * 散布図に参照線を追加して、チャート上の重要なしきい値、ベンチマーク、目標値を強調できます。プロットの設定にある**Reference lines**タブでは、カラーをカスタマイズした最大 5 本の線を設定できます。
    <Frame>![参照線付きの 2 つの散布図](/images/release-notes/v0-84-0/scatter-plot-reference-lines.png)</Frame>
  * 組織とチーム向けに強化されたAPIキー管理：
    * **Organization dashboard** の **API Keys** タブから、管理者は組織内の任意のAPIキーを編集または削除できるようになりました。
    * **Organization dashboard** とチームの **Settings** の両方で、管理者は管理権限を持つAPIキーを一括削除できるようになりました。
  * プレビュー中の BYOB storage 向け [Azure workload identities](/ja/platform/hosting/data-security/secure-storage-connector#provision-your-bucket)。試すには、W&amp;B をアップグレードする前に Operator Helm chart を [v0.44.0+](https://github.com/wandb/helm-charts/releases/tag/operator-wandb-0.44.0) にアップグレードしてください。
  * Content-Encoding: gzip を使用する filestream request のサポートを追加しました。
  * Weave data を含む W&amp;B project の overview page に、アクティブなトレースと評価が表示されるようになりました。

  <div id="weave">
    ### Weave
  </div>

  * Weave が BYOB ストレージ参照 (`s3://`、`gs://`、`gcs://`、`az://`) を有効期限の短い署名付き URL に解決するようになり、次の機能強化が可能になりました。
    * リモートメディアを Weave に取り込む必要がなくなり、コスト効率とデータの機密性が向上します。
    * メディアが、チャットメッセージのテキスト、評価比較の詳細ペイン、トレース比較グリッド、raw-call のカスタムビューにインライン表示されるようになりました。評価比較やデータセットのセルなど、高密度なグリッドでは、インライン表示の代わりにコンパクトな署名付きダウンロードリンクが表示されます。
    * オーディオの参照は、インライン波形プレーヤーで表示されます。
    * エージェントのトレースとスパンで、参照先のメディアが表示・可視化されるようになりました。
  * [Agent views](/ja/weave/guides/tracking/view-agent-activity) で検索とフィルタリングがサポートされるようになりました。同じフィールドに複数の `OR` フィルターを指定して `OR` 式を作成したり、異なるフィールドにフィルターを指定して `AND` 条件を作成したりできます。さらに、列をドラッグしてサイズ変更できるようになりました。
  * コンパクトで所要時間を考慮したターンのタイムラインと、より使いやすい詳細レイアウトにより、Agent view の会話詳細表示を改善しました。エラーが会話内に直接表示されるようになり、会話から trace view の対応する span に直接リンクできるようになりました。また、スコアリングされたターンにはターンごとのドットが表示されるようになりました。
  * [シグナルタグの管理](/ja/weave/guides/tracking/view-agent-signals#tags-presets)の方が、迅速かつ簡潔です。
    * システムプロンプトや出力スキーマなどを定義せずに、タグの説明から直接シグナルを作成できます。変更は自動的に保存されます。
    * 各バッジには削除コントロールが組み込まれています。バッジにカーソルを合わせると、ツールチップにタグの由来に関する詳細が表示されます。
    * タグが適用されていない場合は不要なセクションが非表示になり、Workspaceをすっきりと保てます。
  * シグナルの作成または編集時に、**Inference provider** セレクターでは、サードパーティプロバイダーごとにモデル単位のエントリが表示されるようになりました。
  * シグナルから親会話へのリンク、およびスパンから親評価へのリンクが追加されました。
  * 評価比較表では、複数の数値のリストを含むセルがデフォルトでヒストグラムとして表示されるようになりました。セルのツールバーを使用して、表示モードを **Histogram**、**Bar Chart**、**Text**、または **Code** に変更できます。セルをクリックすると、要約統計量を含む拡大チャートが表示されます。
  * string セルで選択した表示モード (**Text**、**Code**、または **Markdown**) が、クリックしたセルだけでなく、その列内のすべての string セルに適用されるようになりました。明示的にモードを選択するまで、セルには JSON の **Code** などの自動デフォルトが適用されます。
  * Custom providers は、UI を強化した [custom runtimes](/ja/weave/guides/tools/custom-runtimes) に名称変更されました。実験の変化に応じて custom runtimes を編集または削除できます。
  * Weave が inference provider の設定状況を報告する際、チームシークレットに加えてユーザースコープのシークレットも考慮されるようになりました。独自のキーで設定した provider は、**Playground** のモデルドロップダウン、**Providers** タブ、および **Agents** inference で利用可能として表示されるようになりました。
  * セキュリティを強化するため、Weave scoring worker のリモート Scorer ホスト許可リストで、ワイルドカードを含む接尾辞パターンを指定できるようになりました。`WF_SCORING_WORKER_REMOTE_SCORER_ALLOWED_HOSTS` 環境変数に、パターンをカンマ区切りのリストとして設定します。
    * `*.corp.example.com` のようなエントリは、任意の階層のサブドメインに一致しますが、apex ドメインには一致しません。したがって、`a.corp.example.com` と `a.b.corp.example.com` には一致しますが、`corp.example.com` には一致しません。
    * 安全性のため、ワイルドカードは少なくとも 2 つの DNS ラベルを対象とする必要があります。そのため、`*.com` や `*.internal` のようなパターンは拒否されます。

  <div id="performance-improvements">
    ## パフォーマンス改善
  </div>

  * スパースなメトリクスが多数ある Runs のクエリパネルの読み込みパフォーマンスを改善しました。
  * Weave のスコアリングワーカーは、推論エンドポイントの呼び出し時に project ごとに同時実行数を適応的に調整するようになり、プロバイダーのクォータが低い組織で発生するレート制限エラーを削減します。

  <div id="fixes-2">
    ## 修正
  </div>

  * パネルを report にインポートすると、ホバー時だけでなく常にパネル名全体が表示されるようになりました。
  * サポートされていなかったため、Weave の **Signals** タブで `NOT` 演算子を提供しなくなりました。
  * Weave ダッシュボードのフィルターチップに不要な不等号が表示される問題を修正しました。
</Update>

<Update label="v0.83.2" description="2026年8月6日">
  <div id="security">
    ## セキュリティ
  </div>

  * Weave API endpointの悪用を可能にする可能性がある重大なセキュリティバグを修正しました。

  <div id="performance-improvements-2">
    ## パフォーマンス改善
  </div>

  * アーティファクト コレクションのサイドバーの更新効率を改善しました。

  <div id="fixes-3">
    ## 修正
  </div>

  * チームの **Users** 表に **ステータス** 列が追加され、各チームメンバーがアクティブか、招待が保留中か、組織管理者によってアカウントが無効化されているかが表示されるようになりました。[Add and manage teams](/ja/platform/hosting/iam/access-management/manage-organization#add-and-manage-teams)を参照してください。
  * run の最新の記録済み step より前の step で最初にログされたメトリクスが、run のメトリクスおよび以降の履歴 エクスポートから除外され、影響を受ける run のライブ履歴クリーンアップがブロックされる可能性があったバグを修正しました。既存のエクスポートは再処理されません。
  * 複数の書き込みに分割されるほど大きい run ロギングリクエストで、データの一部が保存されなかったにもかかわらず成功と報告される可能性があったバグを修正しました。
  * 単一の アーティファクト 更新操作でタグの追加と削除の両方を試行した場合に、Registry のバージョンタグが検索インデックスで上書きされる可能性があったバグを修正しました。
  * Registry のパージが同期的に実行される可能性があったバグを修正しました。
</Update>

<Update label="v0.83.1" description="2026年7月28日">
  <div id="fixes-4">
    ## 修正
  </div>

  * run メトリクスを読み込む UI リクエストがキャンセルされた場合や失敗した場合のキャッシュ処理を改善しました。
</Update>

<Update label="v0.83.0" description="2026年7月22日">
  W&amp;B Server 0.83.0 では、Registry をプラットフォーム横断で利用できるようになりました。専用クラウドでは、Weave ユーザーが Models のシートなしで Registry を使用できるようになり、Weave のプロンプトを Registry に公開して組織全体でバージョン管理および再利用できます。また、組織レベルの Registry アクセスは、新しい SCIM エンタイトルメントで管理します。本リリースでは、チームの可視性、サービスアカウント、使用状況に対する管理者向けの制御も拡張されました。

  <div id="support-and-end-of-life-2">
    ## サポートとサポート終了(EOL)
  </div>

  * W&amp;B Server v0.71 はサポート終了となりました。

  <div id="features-2">
    ## 機能
  </div>

  * チーム管理者は、超過分を示すバナーやチームを検索して選択する機能を含む、チームレベルの[使用状況と請求](/ja/platform/app/settings-page/billing-settings#plan-usage)チャートを表示し、支出制限を設定できるようになりました。
  * パネルを検索する際に、名前が正規表現パターンに一致するパネルを検索結果から除外できるようになりました。フィルターは削除するまで有効なため、重要なパネルだけが表示されるように Workspace を絞り込むことができます。
    <Frame>
      ![正規表現パターンを使用して検索結果からパネルを除外](/images/release-notes/v0-83-0/exclude-panels-by-regex.png)
    </Frame>
  * ネストされた[run groups](/ja/models/runs/grouping#organize-runs-into-groups)を使用する場合、親グループ配下のすべての子グループに一度に色を設定できるようになりました。親run groupの名の横にあるアイコンをクリックすると、階層内のすべてのrunに色が設定されます。親グループをひと目で見分けられ、異なるカラーがすべてのサブグループに適用されます。
    <Frame>
      ![ネストした run group 階層内のすべての run をワンクリックで色分け](/images/release-notes/v0-83-0/color-nested-run-groups.gif)
    </Frame>
    * **チャートで Runs table のグループ化を使用する**をオフにすると、親グループの配色を維持したまま、各 run を個別の線として確認できます。
    * **グループの色をリセット**をクリックすると、デフォルトの色分けに戻ります。
  * Workspace では、action menu (<Icon icon="ellipsis" iconType="solid" />) を開かなくても、ページ上の **Hide sidebar** コントロール (<Icon icon="angle-double-right" iconType="solid" />) をクリックして、直接サイドバーを非表示にできるようになりました。
  * 大きな[ログ file](/ja/models/app/console-logs)を表示している場合、**Top** をクリックするか **Cmd+Home** を押すと最も古いログにスクロールし、**Current** をクリックするか **Cmd+End** を押すと最新のログにスクロールします。
  * 組織のメンバーシップ表、チームの project 表、sweep 表の応答性を改善しました。
  * メトリクスパネルエディタの応答性と使いやすさを改善しました。
  * フォーク履歴の再計算用にベースラインメトリクスを追加しました。
  * 仮想バージョンのエイリアスでartifactをフィルターおよび検索できるようになりました。あいまい一致はまだサポートされていません。
  * 組織ごとのレジストリ数とartifact数を追加しました。

  <div id="automations">
    ### オートメーション
  </div>

  v0.83.x では、以下の Automations の機能は専用クラウドでのみ利用できます。

  * **Automation Hub**では、project とレジストリにまたがる[オートメーション](/ja/models/automations#automations-overview)を一元的に作成・管理できます。サイドバーの**Core**セクションにあります。
  * オートメーションの適用範囲を、組織、チーム、project、レジストリ、またはコレクションに設定できるようになりました。これにより、広範囲をカバーするためにオートメーションを複製する必要がなくなります。オートメーションを作成する際は、**Edit scopes** をクリックしてください。

  <div id="registry">
    ### Registry
  </div>

  v0.83.x では、以下の Registry 機能は専用クラウドでのみ利用可能です。

  * 組織内のすべての Models および Weave ユーザーに、デフォルトで Registry アクセスが付与されるようになりました。既存の Registry ごとの権限は変更されません。管理者は、SCIM を使用してユーザーの組織レベルの Registry アクセスを取り消すことができます。[Registry アクセスの更新](/ja/platform/hosting/iam/scim#update-registry-access)を参照してください。
  * Weave [プロンプト](/ja/weave/guides/core-types/prompts#create-prompt-objects) (`StringPrompt` および `MessagesPrompt`) を Registry に追加できるようになりました。これにより、モデルアーティファクトとともに、組織全体でプロンプトのバージョン管理、エイリアスの設定、再利用を行えます。
  * [アーティファクト コレクション](/ja/models/registry/create_collection)の概要の編集機能とバージョングリッドでの選択機能を改善しました。動的なアーティファクト パネルを備えたリッチなコレクションカードを自動生成し、より多くのコンテキストを提供できます。

  <div id="weave-2">
    ### Weave
  </div>

  * **New agent** シグナルドロワーに、事前設定されたシグナルメニューを追加しました。
  * トークンコストが UI に表示されるようになりました。
  * エージェントのリアクション (高評価/低評価) は、絵文字リアクションではなくタグとして保存されるようになりました。Scorer タグと並んで表示され、エージェント別にフィルタリングやチャート化ができます。
  * シグナルと Playground エージェントは、デフォルトで非表示になりました。
  * データセットの操作を [**Evaluations** ページ](/ja/weave/guides/core-types/evaluations)に移動し、データポイントの作成操作を改善しました。
  * エージェントの会話に [audio](/ja/weave/cookbooks/audio_with_weave#audio-with-weave)プレーヤーを追加しました。各 file を順番に再生する新しい **Play all** コントロールも追加されています。
  * [**Agents**](/ja/weave/custom-agents-quickstart) タブを改善しました。
  * エージェントの会話に **Spans** ミニマップを追加し、turn に紐付くフィードバックデータを表示できるようにしました。
  * ユーザーメッセージに埋め込まれた JSON にコピーコントロールを追加しました。
  * タグをデフォルトのエージェントシグナルタイプとして復元しました。
  * スコアリングワーカーのスパンに、entity、project、Scorer、モニターの詳細がアノテーションされるようになりました。
  * **Agents** ビューで、会話内の個々のスパンにリンクできるようになりました。

  <div id="performance-improvements-3">
    ## パフォーマンス改善
  </div>

  * パネルのホバーおよびハイライト時のパフォーマンスと効率を改善しました。
  * Workspace の履歴キー集約の効率を改善しました。
  * **Artifacts** サイドバーで多数のアーティファクトを読み込む際のパフォーマンスを改善しました。
  * Weave のスコアリングコンシューマーループのスループットを改善しました。
  * Weave が多数のワーカージャッジコールスパンをバッチ挿入する際のパフォーマンスとスレッドプールのサイズ設定を改善しました。
  ...
  * Registry アクセスは、Models シートとは独立したエンタイトルメントになりました。管理者は、ユーザーの Models シートや Weave ロールを変更せずに、組織レベルの Registry アクセスを付与または取り消すことができます。ユーザーに Weave ロールがない場合、`modelsSeat` を `none` に設定しても Registry アクセスは取り消されなくなりました。[Registry アクセスを更新する](/ja/platform/hosting/iam/scim#update-registry-access)を参照してください。
  * 管理者は、環境変数 `GORILLA_DISABLE_TEAM_SERVICE_ACCOUNT_CREATION` を `true` に設定することで、チームのサービスアカウントの作成を無効にできるようになりました。[チームスコープのサービスアカウント](/ja/platform/hosting/iam/service-accounts#team-scoped-service-accounts)を参照してください。

  **チームと組織の可視性**

  * **チームの可視性制限を適用**が有効になっている場合：
    * チームメンバーのユーザープロフィールを表示しようとした場合、閲覧者がそのチームのメンバー、組織管理者、またはインスタンス管理者でない限り、HTTP `404` レスポンスが返されるようになりました。
    * **Organization dashboard** の **Users** および **Teams** タブへのアクセスは、組織管理者のみに制限されるようになりました。
  * サービスアカウントの表示は、チームおよび組織内のロールに基づいて制限されるようになりました。
    * 組織のサービスアカウントは、組織管理者にのみ表示されます。チームのサービスアカウントは、チームメンバーと組織管理者にのみ表示されます。
    * チームメンバー数からサービスアカウントが除外されるようになりました。

  **アクセス制御とセキュリティ強化**

  * 機密性の高いLicenseの詳細とオブジェクトストレージの認証情報が、起動時の設定ログでマスクされるようになりました。

  <div id="fixes-5">
    ## 修正
  </div>

  * report リストのフィルターが、すべての report ではなく各ページに個別に適用されるバグを修正しました。
  * 非表示の Runs が平行座標プロットに表示されることがあるバグを修正しました。
  * entity スコープのオートメーションに run オートメーションのオプションが表示されることがあるバグを修正しました。
  * API を介して完了済みの run にアップロードされた file が、run のページに表示されないことがあるバグを修正しました。W&amp;B SDK v0.28.1 以降が必要です。
</Update>

<Update label="v0.82.2" description="2026年7月7日">
  <div id="fixes-6">
    ## 修正
  </div>
...
<Update label="v0.82.1" description="2026年7月1日">
  **2026年7月1日**

  <SelfManagedClickhouseUpgradeNote />

  <Warning>
    W&amp;B Server v0.82.1 のリグレッションにより、新しい W&amp;B デプロイで初期セットアップを完了できません。W&amp;B は、新しいデプロイで v0.82.1 を使用しないことを推奨します。v0.82.1 にアップグレードする前に最初のユーザーを作成済みのインスタンスは影響を受けません。
  </Warning>
  ...
  * 大量のメトリクスがログされた project におけるチャートと Workspace の読み込みを改善しました。メディアパネルの読み込みの高速化や、履歴キーのクエリの効率化が含まれます。

  <div id="fixes-7">
    ## 修正
  </div>

  * 大量のメトリクスがログされた project で、カスタム Vega チャートに履歴キーを追加すると、Workspace の読み込み時に停止することがあるバグを修正しました。
  * 非対数軸の分割を使用して折れ線グラフをレンダリングすると発生することがある無限ループを修正しました。
  * スムージングを無効にした折れ線グラフで **Show original** を切り替えると、網掛けされた最小値/最大値バンドが変わることがあるバグを修正しました。
  * Models および Weave project で、**Assets** を開くと分類されたアセットピッカーではなくアセットの完全なリストが表示されるバグを修正しました。
</Update>

<Update label="v0.82.0" description="2026年6月23日">
  **2026年6月23日**

  W&amp;B 0.82.0 では、編集可能なコレクション概要とバージョン単位のメトリクスチャートにより、アーティファクトに関する分析情報がさらに充実しました。加えて、Workspace と run テーブルの使い勝手も改善されています。エンタープライズ管理をサポートするため、メンバーを招待する際に Models のシートと Weave ロールを割り当てられるようになり、監査ログには APIキー のアクティビティも記録されるようになりました。
  ...
  * W&amp;B v0.70 は提供終了となりました。

  <div id="features-3">
    ## 機能
  </div>

  * 異なる project の Runs をピン留めして比較したり、別の project の run をベースライン run として指定したりできるようになりました。[Compare runs across projects](/ja/models/runs/compare-runs#compare-runs-across-projects)を参照してください。
    <Frame>
      ![別の project の pinned runs が表示された Runs table](/images/runs/compare_runs_ui_element.png)
    </Frame>
  * run のステータスの変更、または run のメトリクス値の変更に基づいてオートメーションを設定できるようになりました。[Run events](/ja/models/automations/automation-events#run-events)を参照してください。専用クラウド v0.81+ で利用できます。セルフマネージドではまだ利用できません。
  * W&amp;B UI に最新の利用可能な W&amp;B SDK バージョンが表示されるようになり、新しいリリースが利用可能かどうかを確認できます。
  * Workspace のブラウザータブにアクティブなビュー名がページタイトルとして含まれるようになり、ブラウザータブ間で保存済みビューを区別しやすくなりました。
  * 組織にメンバーを招待する際に、各メンバーの Models のシートと Weave ロールを同時に設定できるようになりました。

  <div id="registry-2">
    ### Registry
  </div>

  * Registry の **Advanced** search タブで、Registry、コレクション、バージョンを効率的に検索してフィルターできるようになりました。オブジェクト名ですばやく検索し、名、タグ、作成日または更新日、親Registry などでオブジェクトをフィルターできます。検索パフォーマンスと効率も改善されました。[Find registry items](/ja/models/registry/search_registry)を参照してください。

  <div id="weave-3">
    ### Weave
  </div>

  * Weave LLM プロキシで、コストが入力トークンと出力トークンの内訳に分けてレポートされるようになり、より詳細なコスト分析が可能になりました。
  * Weave Playground で、Gemini 3.1 preview モデルの動画入力がサポートされるようになりました。

  <div id="security-3">
    ## セキュリティ
  </div>

  * [監査ログ](/ja/platform/hosting/monitoring-usage/audit-logging)に、関連イベントの APIキー 情報が含まれるようになりました。
  * 以下の CVE に対処するため、Redis を 8.6.3 にアップグレードしました。
  ...
  * Workspace の panel search の効率を改善しました。

  <div id="fixes-8">
    ## 修正
  </div>

  * 線形軸を使用するラインプロットパネルで発生する無限ループを修正しました。
  * ダークモードでログされた動画を表示しているときに、全画面表示へ切り替えるとライトモードのカラーで動画が表示されることがあるバグを修正しました。
  * Workspace 内の Weave パネルで、自動更新中に適用済みのフィルターが失われることがあるバグを修正しました。
</Update>
...
<Update label="v0.81.3" description="2026年6月9日">
  ...
  * サポート対象外の内部 API endpoint を削除しました。
</Update>
...
<Update label="v0.81.2" description="2026年6月8日">
  ...
  * 専用クラウドで、v0.81.1 へのアップグレード後に、オプトインの preview 機能が誤って無効化されることがあるバグを修正しました。デプロイは自動的にアップグレードされるため、対応は不要です。問題が見られる場合は [サポート](mailto:support@wandb.com) にお問い合わせください。
</Update>
...
<Update label="v0.81.1" description="2026年6月3日">
  ...
  * アーティファクトが複数のレジストリにリンクされている場合に、ユーザーが現在アクセス可能なコンテキストを超えて権限チェックが評価されることで、アーティファクトのダウンロードが失敗することがあるバグを修正しました。
  * すでにリンク済みのアーティファクトを再リンクすると、アーティファクト-link オートメーションが予期せずトリガーされることがあるバグを修正しました。
  * project の名前変更後に、`project [storage_key] not found` というエラーでストレージエンジンの読み取りが失敗することがあるバグを修正しました。
  * サポートされていない response format が原因で、OpenAI の音声対応モデルに対する Weave のスコアリング call が失敗することがあるバグを修正しました。
</Update>
...
<Update label="v0.80.0" description="2026年4月28日">
  ...
  * [Compare mode](/ja/models/app/features/panels/media#compare-mode) が、メディアパネルで一般提供になりました。Compare mode では、Runs、ステップ、インデックスを任意に組み合わせて 2 ～ 4 個の画像または動画を選択し、ファイルをダウンロードしたりタブを切り替えたりすることなく、1 つのグリッドで表示できます。
    ...
    <Frame>
      ![Media パネルの Compare mode グリッド](/images/release-notes/v0-80-0/media-compare-mode.png)
    </Frame>
  ...
  * W&amp;B App のホーム画面が改善されました。
    ...
    * スターを付けた Projects はホームページには表示されなくなりましたが、Projects ページの上部と左側のナビゲーションには表示されます。
    ...
    <Frame>
      ![最近のアクティビティが表示されたホームページ](/images/release-notes/v0-80-0/home-page-improved.png)
    </Frame>
  ...
  * 新しいメソッド `run.pin_config_keys()` を使用すると、選択した設定キーを run の Overview タブ上部にプログラムからピン留めできます。ピン留めしたキーは、**メモ** の上にある新しい **References** セクションに表示され、ハイパーパラメーター、外部リンク、識別子など、run の重要な詳細をひと目で確認できます。ピン留めした各キーは値とともに個別に表示され、リンクはクリックできます。[W&amp;B SDK v0.26.0](/ja/release-notes/sdk-releases#0-26-0)+ が必要です。
    ...
    <Frame>
      ![References セクションにピン留めされた設定キー](/images/release-notes/v0-80-0/pin-config-keys-references.png)
    </Frame>
  ...
  * **Project overview** ページの新しい **Scorer dashboard** ビューでは、スコアラーごとの合格/不合格率やヒストグラム分布、フィルター可能な例のリスト、さらに各例の試行スコアを含む詳細を表示できます。
    ![Scorer dashboard の動画デモ](/images/release-notes/v0-80-0/weave_scorer_dashboards.gif)
...
<Update label="v0.78.0" description="2026年3月3日">
  ...
  * 重要な run を把握し、モデルの実験を効率的に評価するには、[ピン留めされた run とベースライン run](/ja/models/runs/compare-runs)を使用します。
    ...
    * 比較の基準としてベースラインrunを指定できます。ベースラインrunは、Workspace 内とRunsリストの先頭に常に表示されます。
    ...
    <Frame>![ベースラインと比較した Runs 表の summary メトリクス差分](/images/release-notes/v0-78-0/baseline-deltas-runs-table.png)</Frame>

    折れ線グラフでは、比較しやすいように、ベースラインは視覚的に区別しやすいスタイルで表示されます。

    <Frame>![折れ線グラフで異なるスタイルを適用したベースライン run](/images/release-notes/v0-78-0/baseline-line-plot-styling.png)</Frame>
  ...
  * Registry UI では、よりすばやく移動でき、内容を把握しやすくなりました。
    ...
    * Registry アイコンは初期状態では自動生成され、Registry 管理者がいつでもカスタマイズできます。
    ...
    <Frame>![Registry のアイコンとリデザインされたカード](/images/release-notes/v0-78-0/registry-icons-and-cards.gif)</Frame>
  ...
  * **プロジェクト概要**ページに、リクエスト数、レイテンシのパーセンタイル (P50/P95/P99) 、モデルごとのトークン使用量、コストの内訳を確認できる**Trace Analytics**ダッシュボードが追加されました。どのモデルが予算を大きく圧迫しているか、またはどの低速なリクエストがP95を押し下げているかを把握できます。時間バケットをクリックすると関連するトレースに直接移動できるため、レイテンシ急増の原因をすばやく特定できます。
    ...
    <Frame>![プロジェクト概要の Trace Analytics ダッシュボード](/images/release-notes/v0-78-0/weave-trace-analytics-dashboard.png)</Frame>
