...
<Update label="v0.82.0" description="2026年6月23日">
  **2026年6月23日**
  ...
  W&amp;B 0.82.0 では、編集可能なコレクション概要とバージョン単位のメトリクスチャートにより、アーティファクトに関する分析情報がさらに充実しました。加えて、Workspace と run テーブルの使い勝手も改善されています。エンタープライズ管理を支援するため、メンバー招待時に Models のシートと Weave のロールを割り当てられるようになり、監査ログには APIキー のアクティビティも記録されるようになりました。
  ...
  * Registry の **Advanced** search タブで、レジストリ、コレクション、バージョンを効率的に検索してフィルターできるようになりました。オブジェクト名ですばやく検索し、名、タグ、作成日または更新日、親レジストリなどでオブジェクトをフィルターできます。検索パフォーマンスと効率も改善されました。[Find registry items](/ja/models/registry/search_registry)を参照してください。
