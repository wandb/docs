...
# Agent プロンプト: W&amp;B ドキュメント向けの Locadex AI コンテキストを設定する (韓国語、後に日本語) {#agent-prompt-configure-locadex-ai-context-for-wb-docs-korean-and-later-japanese}
...
## Agent の事前確認事項 {#agent-prerequisites}
...
## コンテキストと制約事項 {#context-and-constraints}

### レガシーツール (`wandb_docs_translation`) {#legacy-tooling-wandb_docs_translation} 
...
### Locadex/GT プラットフォームの挙動 {#locadexgt-platform-behavior}
...
### 1. 用語ソースを収集する {#1-gather-terminology-sources}
...
### 2. Glossary CSV を作成または入手する {#2-build-or-obtain-a-glossary-csv}
...
### 3. コンソールでLocadex プロジェクトを設定する {#3-configure-the-locadex-project-in-the-console}
...
### 4. Glossary 用語をアップロードまたは追加する {#4-upload-or-add-glossary-terms}
...
### 5. 韓国語のLocale Contextを設定する {#5-set-locale-context-for-korean}
...
### 6. Style Controls を行う (プロジェクト全体) {#6-set-style-controls-project-wide} 
...
### 7. 必要に応じて再翻訳する {#7-retranslate-if-needed}
...
### 問題: CSV upload が Glossary にマッピングされない {#issue-csv-upload-does-not-map-to-glossary}
...
### 問題: 英語のままにすべき用語まで翻訳されてしまう {#issue-terms-still-translated-when-they-should-stay-in-english}
...
### 問題: 日本語 (または他のロケール) には別のルールが必要 {#issue-japanese-or-another-locale-needs-different-rules}
...
### 今後の生成に向けたCSVのフォーマット {#csv-formatting-for-future-generation}
