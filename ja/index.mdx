...
<div className="grid grid-cols-1 md:grid-cols-3 gap-4" style={{ marginTop: '2rem' }}>
  ...
  <ProductCard title="Serverless Training" iconSrc="icons/Training.svg" href="/ja/serverless-training" subtitle="モデルを事後トレーニングしてファインチューニング" iconSize={24}>
    現在、パブリックプレビューで提供中です。Serverless RL と Serverless SFT、マネージド GPU インフラストラクチャー、ART と RULER のインテグレーション、マルチターンのエージェント型タスク向けオートスケーリングにより、LLM の事後トレーニングとファインチューニングを行えます。
  </ProductCard>
