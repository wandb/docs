---
description: "W&B を使って OpenAI モデルをファインチューニングし、トレーニングメトリクスをログし、ジョブを監視し、モデルのパフォーマンスを経時的に比較します。"
...
OpenAI GPT-3.5 または GPT-4 モデルのファインチューニングメトリクスと構成を W&B にログしましょう。W&B エコシステムを活用して、ファインチューニング実験、モデル、データセットを追跡し、その結果を同僚と共有できます。
...
ファインチューニングのために W&B を OpenAI と統合する方法に関する補足情報については、OpenAI ドキュメントの [W&B Integration](https://developers.openai.com/cookbook/examples/third_party/gpt_finetuning_with_wandb) セクションを参照してください。
