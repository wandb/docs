...
コンテナに対して明示的に [環境変数](/ja/models/track/environment-variables/) を渡す必要がある場合があります。双方向のリンクを有効にするには、 `WANDB_KUBEFLOW_URL` 環境変数 に Kubeflow Pipelines インスタンスのベース URL（例: `https://kubeflow.mysite.com`）を設定してください。
...
ウェブアプリ UI には、 Kubeflow Pipelines の `Visualizations` タブと同じ内容が表示されますが、より広い画面で操作できます。ウェブアプリ UI についての詳細は [こちら](/ja/models/app) をご覧ください。
...
* プログラムによる アクセス については、 [Public API ドキュメント](/ja/models/ref/python/public-api/) を参照してください。
...
| ------------------ | --- | --------------- |
| Input Scalar | [`config`](/ja/models/) | [Overviewタブ](/ja/models/runs/#overview-tab) |
| Output Scalar | [`summary`](/ja/models/) | [Overviewタブ](/ja/models/runs/#overview-tab) |
| Input Artifact | Input Artifact | [Artifactsタブ](/ja/models/runs/#artifacts-tab) |
| Output Artifact | Output Artifact | [Artifactsタブ](/ja/models/runs/#artifacts-tab) |
...
[サポートされているフレームワークインテグレーション](/ja/models/integrations) を使用している場合は、 コールバック を直接渡すことも可能です。
