---
description: "W&B を Kubeflow Pipelines と統合して、ML パイプライン コンポーネント全体で実験を追跡し、メトリクスを可視化します。"
title: Kubeflow Pipelines (kfp)
---

import ApiKeyCreateStreamlined from "/snippets/ja/_includes/api-key-create-streamlined.mdx";

[Kubeflow Pipelines (kfp) ](https://www.kubeflow.org/docs/components/pipelines/overview/)は、Docker コンテナに基づく、移植性とスケーラビリティに優れた機械学習 (ML) ワークフローを構築およびデプロイするためのプラットフォームです。

この統合により、ユーザーは kfp の Python 関数コンポーネントにデコレーターを適用して、パラメーターとアーティファクトを自動的に W&B にログできます。
