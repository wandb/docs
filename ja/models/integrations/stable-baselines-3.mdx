...
SB3のトレーニングをW&amp;Bにログするには、モデルの `learn` メソッドに `WandbCallback` を渡します:
...
次の表では、`WandbCallback` に渡せる引数について説明します。

| Argument             | Usage                                         |
| :------------------- | :-------------------------------------------- |
| `verbose`            | SB3 の出力の詳細レベル。                                |
| `model_save_path`    | モデルを保存するフォルダーのパス。デフォルトは `None` で、モデルはログされません。 |
| `model_save_freq`    | モデルを保存する頻度。                                   |
| `gradient_save_freq` | 勾配をログする頻度。デフォルトは `0` で、勾配はログされません。            |

<div id="basic-example">
  ## 基本的な例
</div>

W&amp;B SB3 インテグレーションでは、TensorBoard が出力するログを使用して、メトリクスをログします。
