...
| `model_log_interval` | `Optional int` 。 デフォルトは `None` 。設定すると、 [Artifacts](/ja/models/artifacts/) による [モデルのバージョン管理](/ja/models/registry/) が有効になります。モデルの チェックポイント をログに記録する間隔（ステップ数）を渡します。 |
...
トレーニングが開始されると、トレーニング Run の [W&B ページ](/ja/models/runs/) へのリンクが出力されます。このリンクから、 W&B ウェブ UI 上の該当する Run の 実験管理 [ダッシュボード](/ja/models/track/workspaces/) にアクセスできます。