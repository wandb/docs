...
1. 可視化設定から `visualizer` を定義します。
   ...
   <Note>`init_kwargs` には、[W&amp;B run の初期化](/ja/models/ref/python/functions/init)の入力パラメーターとして使用する引数の辞書を渡します。</Note>
...
`WandbVisBackend`は、[MMDetection](https://mmdetection.readthedocs.io/) などのOpenMMLabのコンピュータビジョンライブラリで実験を管理する際にも、簡単に使用できます。