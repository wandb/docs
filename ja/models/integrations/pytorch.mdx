...
## このノートブックで学ぶこと {#what-this-notebook-covers}
...
## インストール、インポート、ログイン {#install-import-and-log-in}
...
### Step 0: W&amp;B をインストール {#step-0-install-wb}
...
### Step 1: W&amp;B をインポートしてログイン {#step-1-import-wb-and-log-in}
...
## 実験とパイプラインを定義する {#define-the-experiment-and-pipeline}

W&amp;B をインストールし、セッションの認証が完了したら、実験の設定と、その設定を使用するトレーニング パイプラインを定義します。

### `wandb.init()` でメタデータとハイパーパラメーターをトラッキングする {#track-metadata-and-hyperparameters-with-wandbinit}
...
### データの読み込みとモデルを定義する {#define-the-data-loading-and-model}
...
### トレーニングロジックを定義する {#define-training-logic}
...
### 勾配は `run.watch()` でトラッキングし、それ以外はすべて `run.log()` でログする {#track-gradients-with-runwatch-and-everything-else-with-runlog}
...
### トレーニングを実行し、wandb.ai でメトリクスをライブで確認する {#run-training-and-watch-your-metrics-live-on-wandbai}
...
### Sweeps でハイパーパラメーターを試す {#test-hyperparameters-with-sweeps}
