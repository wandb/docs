...
勾配 を自動的に ログ 記録するには、 [`wandb.Run.watch()`](/ja/models/ref/python/experiments/run.md/#method-runwatch) を呼び出し、 PyTorch モデル を渡します。
...
同じ スクリプト 内で複数の モデル を追跡する必要がある場合は、各 モデル に対して個別に [`wandb.Run.watch()`](/ja/models/ref/python/experiments/run/#method-runwatch) を呼び出すことができます。
...
画像 データを含む PyTorch の `Tensors` を [`wandb.Image`](/ja/models/ref/python/data-types/image) に渡すと、 [`torchvision`](https://pytorch.org/vision/stable/index.html) のユーティリティが使用され、自動的に画像に変換されます。
...
PyTorch やその他の フレームワーク でリッチメディアを W&B に ログ 記録する方法の詳細については、 [メディアロギングガイド](/ja/models/track/log/media/) を参照してください。
...
データセット や モデル の ログ 記録と可視化の詳細については、 [W&B Tables ガイド](/ja/models/tables/) を参照してください。
...
W&B は [PyTorch Kineto](https://github.com/pytorch/kineto) の [Tensorboard プラグイン](https://github.com/pytorch/kineto/blob/main/tb_plugin/README.md) と直接連携し、 PyTorch コード のプロファイリング、 CPU および GPU 通信の詳細な検査、ボトルネックの特定と最適化のための ツール を提供します。
