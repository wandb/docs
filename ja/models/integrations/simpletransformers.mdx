...
このライブラリは Hugging Face の Transformers ライブラリをベースにしています。Simple Transformers を使うと、Transformer モデルをすばやくトレーニングおよび評価できます。モデルの初期化、トレーニング、評価は、わずか 3 行のコードで実行できます。Sequence Classification、Token Classification (NER)、質問応答、Language Model Fine-Tuning、Language Model Training、Language Generation、T5 Model、Seq2Seq Tasks、Multi-Modal Classification、Conversational AI をサポートしています。
...
このライブラリは、各 NLP タスクに対して個別のクラスを持つように設計されています。類似した機能を提供するクラスは、まとめてグループ化されています。
...
SimpleTransformers は、一般的な自然言語処理タスク向けのクラスとトレーニング用スクリプトを提供します。以下に、このライブラリでサポートされているグローバル引数の完全な一覧と、そのデフォルト値を示します。
...
広く利用されている GLUE ベンチマークのデータセットのいくつかで Transformer をトレーニングする方法を解説した [この W&amp;B レポート](https://app.wandb.ai/cayush/simpletransformers/reports/Using-simpleTransformer-on-common-NLP-applications---Vmlldzo4Njk2NA) をご覧ください。[Colab でご自身でも試してみてください](https://colab.research.google.com/drive/1oXROllqMqVvBFcPgTKJRboTq96uWuqSz?usp=sharing)。
