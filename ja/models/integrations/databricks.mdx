---
description: "W&B を Databricks と統合し、Spark クラスターで実験の追跡、メトリクスのログ記録、モデル管理を行います。"
title: Databricks
---

W&B は、Databricks 環境で W&B Jupyter ノートブックのエクスペリエンスをカスタマイズすることで、[Databricks](https://www.databricks.com/) と統合します。
...
wandb.sweep() または wandb.agent() を使用するノートブックでは、 (一時的に) セットアップが必要です:
