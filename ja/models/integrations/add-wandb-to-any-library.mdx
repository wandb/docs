...
<Note>
W&Bの使用方法をまだ学習中の場合は、読み進める前に [実験管理](/ja/models/track/) など、このドキュメント内の他の W&B ガイドを先に確認することをお勧めします。
</Note>
...
config ファイルの定義方法の詳細については、[実験の設定](/ja/models/track/config/) を参照してください。

### W&Bへのログ記録

#### メトリクスのログ記録

キーの値がメトリクスの名前である辞書を作成します。この辞書オブジェクトを [`run.log`](/ja/models/) に渡します。
...
[`wandb.Run.log()` リファレンス](/ja/models/) を参照してください。
...
メディア、オブジェクト、プロットなどの [ログ記録ガイド](/ja/models/) を参照してください。
...
詳細は [分散トレーニング実験のログ記録](/ja/models/track/log/distributed-training/) を参照してください。
...
カスタムエイリアスの作成方法については、[カスタムエイリアスの作成](/ja/models/artifacts/create-a-custom-alias/) を参照してください。
...
分散環境や単純な推論などで、`wandb` run を作成せずに（`wandb.init` を通さずに）アーティファクトをダウンロードするには、[wandb API](/ja/models/ref/python/public-api/) を使用してアーティファクトを参照できます。
...
ライブラリで W&B のハイパーパラメータチューニングを利用したい場合は、[W&B Sweeps](/ja/models/sweeps/) をライブラリに追加することもできます。
