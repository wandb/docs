...
W&amp;B では ユーザーおよびマシンを認証するために APIキー を使用します。

<div id="create-an-api-key">
  ### APIキーを作成する
</div>

APIキーは、クライアントまたはマシンからのアクセスを W&amp;B に対して認証します。APIキーはユーザープロファイルから生成できます。
...
ライブラリにアーティファクトを統合する際は、次の点を検討してください。

* モデル チェックポイントやデータセットをアーティファクトとしてログするかどうか（オプションにしたい場合など）。
...
モデルのチェックポイントを W&amp;B にログします。よくある方法として、W&amp;B が生成する一意の run ID をアーティファクト名の一部として使用して、チェックポイントをアーティファクトとしてログするやり方があります。
...
直前のコードスニペットは、モデル チェックポイントをアーティファクトとしてログし、評価精度やトレーニングステップなどのメタデータを追加する方法を示しています。アーティファクトには一意の run ID を含む名前が付けられ、参照しやすいように [カスタムエイリアス](/ja/models/artifacts/create-a-custom-alias/) がタグとして付けられています。
...
前のコードスニペットでは、&quot;flowers&quot; という名前のデータセット用アーティファクトを作成し、その中にファイルを追加します。その後、`run.use_artifact()` を使ってそのアーティファクトを現在の run に関連付けます。こうすることで、その run で使用されたデータセットの来歴を W&amp;B が追跡できるようになります。

<div id="download-artifacts">
  ### アーティファクトをダウンロードする
</div>

W&amp;B に以前にログしたアーティファクトをダウンロードして、トレーニングまたは推論コードで使用します。

run コンテキストがある場合は、[`wandb.Run.use_artifact()`](/ja/models/ref/python/experiments/run) を使用して W&amp;B のアーティファクトを参照してから、[`wandb.Artifact.download()`](/ja/models/ref/python/experiments/artifact) を呼び出してローカルディレクトリにダウンロードします。
...
[W&amp;B Public API](/ja/models/ref/python/public-api/) を使用すると、run を初期化せずにアーティファクトを参照したりダウンロードしたりできます。これは、分散環境での利用時や推論を実行する際など、新しい run を作成したくない場合に有用です。
...
詳しくは、[アーティファクトのダウンロードと利用](/ja/models/artifacts/download-and-use-an-artifact/) を参照してください。
