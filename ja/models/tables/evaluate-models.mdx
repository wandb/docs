...
## Weave でモデルを評価する {#evaluate-models-with-weave}
...
### モデル評価の主な機能 {#key-features-for-model-evaluation}
...
### はじめに: W&amp;B Registry のモデルを評価する {#getting-started-evaluate-a-model-from-wb-registry}
...
### Weave の評価を W&amp;B Models と統合する {#integrate-weave-evaluations-with-wb-models}

[Models と Weave のインテグレーション例](/ja/weave/cookbooks/Models_and_Weave_Integration_Demo) では、次の一連のワークフロー全体を確認できます。
...
### Weave の高度な機能 {#advanced-weave-features}

#### カスタムスコアラーとジャッジ {#custom-scorers-and-judges}
...
## Tablesでモデルを評価する {#evaluate-models-with-tables}
...
### 基本例: 評価結果をログする {#basic-example-log-evaluation-results}
...
### 表を使った高度なワークフロー {#advanced-table-workflows}
...
<Frame>
  ![トレーニングの各エポックにおけるモデル予測の並列比較](/images/data_vis/table_comparison.png)
</Frame>

#### 予測の推移をトラッキングする {#track-predictions-over-time}
...
### W&amp;B UI でのインタラクティブな分析 {#interactive-analysis-in-the-wb-ui}
...
### 例: 拡張したTablesを使ったエラー分析 {#example-error-analysis-with-enriched-tables}

```python
# 分析列を追加するためのミュータブルな表を作成する
eval_table = wandb.Table(
    columns=["id", "image", "label", "prediction"],
    log_mode="MUTABLE"  # 後から列を追加できるようにする
)
