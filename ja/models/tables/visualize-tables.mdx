---
description: "分析のために、W&B Tables のデータを比較、フィルタリング、グループ化、並べ替えし、マージ ビューまたは並列ビューで可視化します。"
title: 表を可視化して分析する
---
...
* [2 つの W&B Tables を比較](#table-comparison-options)して、アーティファクト バージョンとしてログされた表のデータやモデル性能の変化を分析する。
* データ内のより高いレベルのパターンを理解する
* [表にログした値が run 全体でどのように変化するかを確認](#visualize-how-values-change-throughout-your-runs)する。

<Note>
W&B Tables は次のような動作をします:
1. **アーティファクト コンテキストではステートレス**: アーティファクト バージョンとともにログされた表は、ブラウザー ウィンドウを閉じるとデフォルトの状態にリセットされます
2. **ワークスペースまたはレポート コンテキストではステートフル**: 単一 run ワークスペース、複数 run プロジェクト ワークスペース、またはレポートで表に加えた変更は保持されます。
...
2 つの表を並列ビューで表示するには、最初のドロップダウンを "Merge Tables: Table" から "List of: Table" に変更し、その後それぞれの "Page size" を更新します。ここでは、最初に選択した表が左側に、2 つ目の表が右側に表示されます。また、"Vertical" チェックボックスをクリックすると、これらの表を縦方向に並べて比較することもできます。

<Frame>
    <img src="/images/data_vis/side_by_side.png" alt="Side-by-side table view" max-width="90%"  />
</Frame>

* **表をひと目で比較する**: 任意の操作（並べ替え、フィルタリング、グループ化）を両方の表に同時に適用し、変更点や違いをすばやく見つけられます。たとえば、予測ラベルごとにグループ化した誤予測、全体で最も難しいネガティブ例、正解ラベルごとの信頼度スコア分布などを表示できます。
* **2 つの表を個別に確認する**: スクロールしながら、注目したい側や行に集中できます
...
run ワークスペース、プロジェクト ワークスペース、またはレポート内で操作した Tables では、ビューの状態が自動的に保存されます。表に何らかの操作を適用したあとでブラウザーを閉じても、次回その表を開いたときに最後に表示していた設定が保持されます。 

<Note>
アーティファクト コンテキストで操作した Tables は、ステートレスのままです。
</Note>

ワークスペース内の表を特定の状態で保存するには、W&B レポートにエクスポートします。表をレポートにエクスポートするには、次の手順に従います:

1. ワークスペースの可視化パネルの右上隅にある kebab アイコン（縦に並んだ 3 つの点）を選択します。
