...
| `${alias}`                    | 自動処理が **An artifact alias is added** イベントによってトリガーされた場合に、アーティファクト の alias を含みます。その他の自動処理では、この変数は空です。 |
| `${tag}`                      | 自動処理が **An artifact tag is added** イベントによってトリガーされた場合に、アーティファクト の tag を含みます。その他の自動処理では、この変数は空です。 |
| `${artifact_collection_name}` | アーティファクト バージョンがリンクされているアーティファクト コレクションの名前。 |
...
<Tabs>
  <Tab title="GitHub リポジトリ ディスパッチ">
    ...
    テンプレート文字列を使用して、W&amp;B から GitHub Actions やその他のツールへコンテキストを動的に渡します。これらのツールが Python スクリプトを呼び出せる場合、[W&amp;B API](/ja/models/artifacts/download-and-use-an-artifact/) を通じて登録済みモデルのアーティファクトを利用できます。
