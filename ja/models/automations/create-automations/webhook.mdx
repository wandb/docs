...
<Tabs>
  ...
  <Tab title="Bash script">
    このシェルスクリプトは、トリガーされたときに W&amp;B が webhook オートメーションに送信するリクエストに類似した `POST` リクエストを生成する 1 つの方法を示します。
    ...
    * `API_ENDPOINT`
    ...
    # HMAC 署名を生成
    # セキュリティのため、W&B はペイロードと webhook に関連付けられた共有 secret から
    # HMAC SHA-256 で計算した X-Wandb-Signature をヘッダーに含めます。
