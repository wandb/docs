...
このページでは、webhook [automation](/ja/models/automations/) の作成方法について説明します。Slack のオートメーションを作成する場合は、代わりに [Create a Slack automation](/ja/models/automations/create-automations/slack/) を参照してください。

ハイレベルな手順として、webhook オートメーションを作成するには以下のステップを踏みます：
1. 必要に応じて、アクセストークン、パスワード、SSH キーなど、オートメーションで必要となる機密文字列ごとに [W&B secret を作成](/ja/platform/secrets/) します。Secrets は **Team Settings** で定義されます。
2. [webhook を作成](#webhook-を作成する) して、エンドポイントと認証の詳細を定義し、インテグレーションが必要な secrets へのアクセスを許可します。
3. [オートメーションを作成](#オートメーションを作成する) して、監視する [event](/ja/models/automations/automation-events/) と W&B が送信するペイロードを定義します。オートメーションに、ペイロードに必要な secrets へのアクセスを許可します。

## webhook を作成する
チーム管理者は、チームに webhook を追加できます。

<Note>
webhook が Bearer トークンを必要とする場合、またはそのペイロードが機密文字列を必要とする場合は、webhook を作成する前に [それを含む secret を作成](/ja/platform/secrets/#add-a-secret) してください。1 つの webhook に対して、最大 1 つのアクセストークンと 1 つのその他の secret を設定できます。webhook の認証および認可要件は、webhook のサービスによって決定されます。
</Note>
...
1. webhook が Bearer トークンを必要とする場合は、**Access token** をそのトークンを含む [secret](/ja/platform/secrets/) に設定します。webhook オートメーションを使用すると、W&B は `Authorization: Bearer` HTTP ヘッダーにアクセストークンを設定し、`${ACCESS_TOKEN}` [ペイロード変数](#ペイロード変数) でトークンにアクセスできます。W&B が webhook サービスに送信する `POST` リクエストの構造の詳細については、[webhook のトラブルシューティング](#webhook-のトラブルシューティング) を参照してください。
...
1. 監視する [event](/ja/models/automations/automation-events/#registry-events) を選択します。
...
1. サイドバーで **Automations** をクリックし、**Create automation** をクリックします。
    ...
    または、ワークスペースのラインプロットから、表示されているメトリクスに対する [run metric automation](/ja/models/automations/automation-events/#run-events) を素早く作成できます。パネルの上にカーソルを置き、パネル上部のベルアイコンをクリックします。
    <Frame>
    <img src="/images/automations/run_metric_automation_from_panel.png" alt="オートメーションのベルアイコンの場所"  />
    </Frame>
1. アーティファクトのエイリアスが追加されたときや、run のメトリクスが特定の閾値に達したときなど、監視する [event](/ja/models/automations/automation-events/#project) を選択します。
...
| `${artifact_version}`         | アクションをトリガーしたアーティファクトバージョンの [`Wandb.Artifact`](/ja/models/ref/python/experiments/artifact.md/) 表現。 |
...
テンプレート文字列を使用して、W&B から GitHub Actions やその他のツールに動的にコンテキストを渡します。それらのツールが Python スクリプトを呼び出せる場合、[W&B API](/ja/models/artifacts/download-and-use-an-artifact/) を通じて Registered Models のアーティファクトを利用できます。
