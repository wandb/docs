...
このページでは、W&amp;B の[オートメーション](/ja/models/automations)を作成および管理する方法を説明します。オートメーションは、W&amp;B 内の特定のイベントに応じて開始されます。組織、チーム、project、Registry、またはコレクション内で特定のイベントが発生すると、オートメーションは Slack チャンネルに通知するか、webhook を実行するか、ARIA にプロンプトを送信します。まずこの概要で前提条件と大まかなワークフローを把握し、その後、[Slack オートメーションを作成する](/ja/models/automations/create-automations/slack)、[webhook オートメーションを作成する](/ja/models/automations/create-automations/webhook)、または [ARIA オートメーションを作成する](/ja/models/automations/create-automations/aria) の詳細な手順に従ってください。
...
左側のナビゲーションにあるグローバル **Automations** ハブ、project または Registry の **Automations** タブ、または Workspace のラインプロットパネルから直接オートメーションを作成できます。ハブからオートメーションを作成する場合は、そのスコープ (組織、チーム、Registry、Registry コレクション、または project) を選択します。[Automations ハブ](/ja/models/automations#automations-hub)を参照してください。

### Automations タブから作成する {#create-from-the-automations-tab}
...
### Workspace のパネルから run メトリクス オートメーションを作成する {#create-a-run-metric-automation-from-a-workspace-panel}
...
* [ARIA オートメーションを作成する](/ja/models/automations/create-automations/aria)
