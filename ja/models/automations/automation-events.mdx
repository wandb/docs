...
自動化は、プロジェクトまたはレジストリ内で特定のイベントが発生したときに開始されます。このページでは、各スコープで自動化のトリガーとなるイベントについて説明します。自動化について詳しくは、[Automations の概要](/ja/models/automations) または [自動化の作成](/ja/models/automations/create-automations) を参照してください。
...
<Note>
  ...
  * ステータスが **Killed** の run では自動化をトリガーできません。このステータスは、その run が管理者ユーザーによって強制的に停止されたことを示します。
</Note>

run の [ステータス](/ja/models/runs/run-states) が **Running**、**Finished**、または **Failed** に変わったときにワークフローをトリガーします。必要に応じて、ユーザーまたは run 名のフィルターを指定して、自動化をトリガーできる run をさらに絞り込むこともできます。

<Frame>
  ![Screenshot showing a run status change automation](/images/automations/run_status_change.png)
</Frame>

run ステータスは run 全体のプロパティであるため、run ステータスの自動化はワークスペースからではなく、**Automations** ページからのみ作成できます。
...
* [Slack の自動化を作成](/ja/models/automations/create-automations/slack)
* [Webhook の自動化を作成](/ja/models/automations/create-automations/webhook)