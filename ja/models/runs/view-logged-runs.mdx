...
特定の run の現在の状態、Artifacts、メトリクスなどの情報を表示できます。W&amp;B App または `wandb beta leet` ターミナル UI を使って run を表示およびモニタリングできます。
...
<Tabs>
  ...
  <Tab title="LEET">
    ...
    * **中央ペイン**: ログしたメトリクスを Braille 形式の折れ線グラフで表示するメトリクスグリッド
    * **右ペイン**: GPU/CPU/RAM の使用率を含むシステムメトリクス
...
* **Host name**: W&amp;B が run を計算する場所。ローカルマシン上で run を初期化した場合は、そのマシン名が表示されます。
* **Name**: run の名前。
...
* **Tags**: 文字列のリスト。タグは、関連する run をまとめて整理したり、`baseline` や `本番` のような一時的なラベルを付けるのに役立ちます。
...
Config と Summary の各セクションの上にある検索ボックスを使用すると、特定のパラメーターでフィルターできます。たとえば、検索ボックスに `acc` と入力すると、W&amp;B は `accuracy` や `val_acc` のように、名前に `acc` を含むすべてのパラメーターをフィルターします。

<Note>
  W&amp;B はネストされた config パラメーターや summary パラメーターを検索しません。たとえば、`wandb.Run.config.update({"model": {"learning_rate": 0.01}})` を使ってネストされた config パラメーターをログした場合、検索ボックスで `learning_rate` を検索しても、W&amp;B は `learning_rate` パラメーターを返しません。`learning_rate` パラメーターを見つけるには、`model` を検索する必要があります。
</Note>
...
ログ タブの例は [こちら](https://app.wandb.ai/stacey/deep-drive/runs/pr0os44x/logs) を参照してください。

<div id="files">
  ## Files
</div>

モデル チェックポイントや検証用データセットのサンプルなど、特定の run に関連付けられたファイルを表示するには **Files** タブを使用します
