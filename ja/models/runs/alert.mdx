...
<Note>
W&B Alerts を使用するには、コードに `run.alert()` を追加する必要があります。コードを変更せずに Slack に通知する別の方法として [Automations](/ja/models/automations/) があります。これは、[Artifacts](/ja/models/artifacts/) の バージョン が作成されたときや、[run metric](/ja/models/runs/) がしきい値に達した、あるいは変化したときなど、W&B 上のイベントに基づいて通知を行います。
...
プライベートクラウド または W&B 専用クラウド で [W&B Server](/ja/platform/hosting/) を使用している場合は、[W&B Server での Slack アラートの設定](/ja/platform/hosting/monitoring-usage/slack-alerts) を参照して Slack アラートを設定してください。
</Note>
