...
description: "整理のために、Python SDK、Public API、または W&B App UI を使用して W&B Runs のタグを追加、更新、削除します。"
---

ログされたメトリクスやアーティファクトデータからは分かりにくい特定の特徴で Runs にラベルを付けるために、タグを追加します。

たとえば、Run にタグを追加して、その Run のモデルが `in_production` であること、その Run が `preemptible` であること、この Run が `baseline` を表していることなどを示すことができます。
