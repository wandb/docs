...
<Frame>
  ![ピン留めされた run を含む Runs table](/images/models/pinned-and-baseline-runs/runs-table-with-pinned-runs.png)
</Frame>
...
## project をまたいで Runs を比較する {#compare-runs-across-projects}
...
<Frame>
  ![別の project からピン留めされた Runs が表示された Runs table](/images/runs/compare_runs_ui_element.png)
</Frame>
...
## ベースライン run を管理して比較する {#manage-and-compare-a-baseline-run}
...
### ベースライン run を設定する {#set-a-baseline-run}
...
### ベースライン run を変更する {#change-the-baseline-run}
...
### ベースライン指定を解除する {#remove-the-baseline-designation}
...
### run をベースラインと比較する {#compare-runs-to-the-baseline}
...
### summary メトリクスの差分 {#summary-metric-deltas}
...
## Workspace で summary メトリクスの差分を非表示にする {#hide-summary-metric-deltas-in-a-workspace}
...
## 例: ハイパーパラメーター調整実験の run を比較する {#example-compare-runs-from-hyperparameter-tuning-experiments}

このセクションでは、ピン留めした run とベースライン run を使って run を比較する方法を示します。

たとえば、次のコードは一連の run によるハイパーパラメーター調整のシナリオをシミュレートします。
...
def train_model(learning_rate, batch_size, run_name, tags=None):
    ...
    with wandb.init(
      # ご自身のチーム名とプロジェクト名に置き換えてください
        ...
        entity="your-team-name", # ご自身のチーム名に置き換えてください
        name=run_name,
        config=config,
        tags=tags or []
    ) as run:
...
run の完了後、W&amp;B App にアクセスして、Workspace で run を表示します。有望な実験をピン留めし、ベースライン run を設定して、新しい run のパフォーマンスを比較できます。

1. W&amp;B project にアクセスします。
2. **Runs** タブを選択します。
3. `baseline` タグを持つ run を[フィルター](/ja/models/runs/filter-runs)し、[ベースライン](#set-a-baseline-run) run として設定します。
4. `baseline-config` を[ピン留め](#pin-runs)して、Runs table の上部に表示します。
5. 実験 run をベースラインと比較します。

   * `accuracy` 列の横にある縦の省略記号メニューを選択し、**Metric directionality** を **Higher values are best** に設定します。各 run の accuracy の差分が Runs table に表示されます。
   * `loss` 列の横にある縦の省略記号メニューを選択し、**Metric directionality** を **Lower values are best** に設定します。各 run の loss の差分が Runs table に表示されます。

   <Frame>
     <img src="/images/runs/compare_metric_deltas.png" />
   </Frame>
6. 折れ線グラフを使用して、run をベースラインと視覚的に比較します。ベースライン run は太い破線で表示され、グラフにカーソルを合わせると、特定のエポックで各 run がベースラインとどのように比較されるかを確認できます。

   たとえば、次の画像は、run `lr-experiment-0.003` および `lr-experiment-0.0001` がベースライン run `baseline-config` とどのように比較されるかを示しています。
