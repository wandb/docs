...
<Note>
キーベースの色設定を使用する場合、 [Run の色をカスタマイズする](/ja/models/runs/run-colors) オプションは利用できません。
</Note>
...
**Key** ドロップダウンのメトリクスオプションは、 [W&B にログ記録した](/ja/models/runs/color-code-runs/#custom-metrics) キーと値のペア、および W&B によって定義された [デフォルトメトリクス](/ja/models/runs/color-code-runs/#default-metrics) から派生します。
