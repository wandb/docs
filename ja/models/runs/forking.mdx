...
* Run のフォークには、単調増加するステップが必要です。[`define_metric()`](/ja/models/ref/python/experiments/run#define_metric) で定義された非単調なステップを使用している Run からはフォークできません。非単調なステップは、Run の履歴やシステム メトリクス の時系列順序を損なうためです。
</Note>

既存の Run からフォークするには、[`wandb.init()`](/ja/models/ref/python/functions/init) の `fork_from` パラメータ を使用します。ソースとなる Run の一意な `run ID` と、フォークを開始したい `step` を指定してください。

<Note>
Run ID の詳細や確認方法については、[Unique run identifiers](/ja/models/runs/run-identifiers#unique-run-identifiers) を参照してください。
</Note>
...
フォークは、Run の管理と試行に柔軟性を持たせることで [`rewind`](/ja/models/runs/rewind/) を補完します。
