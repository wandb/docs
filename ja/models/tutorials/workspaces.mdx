...
`wandb-workspaces` は、 W&B [Workspaces](/ja/models/track/workspaces/) および [Reports](/ja/models/reports/) をプログラムで作成・カスタマイズするための Python ライブラリです。
...
<Note>
Workspace のプログラムによる操作は、現在 [Saved workspaces views](/ja/models/track/workspaces/#saved-workspace-views) でサポートされています。 Saved workspaces views は、 Workspace の共同作業用スナップショットです。チームの誰でも、保存された Workspace ビューを表示、編集、保存できます。
</Note>
