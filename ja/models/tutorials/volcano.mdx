...
この設定ブロックには、Kubernetes ジョブ仕様、Volcano ジョブ仕様、または Launch したいその他のカスタムリソース定義（CRD）を指定できます。[設定ブロック内のマクロ](/ja/platform/launch/set-up-launch/) を使用して、このスペックの内容を動的に設定することも可能です。
