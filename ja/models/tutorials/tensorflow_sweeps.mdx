...
詳細については、 [Sweeps の概要](/ja/models/sweeps/) を参照してください。
...
詳細は [sweep configuration ガイド](/ja/models/sweeps/define-sweep-configuration/) を参照してください。
...
## 高度なセットアップ
1. [環境変数](/ja/platform/hosting/env-vars/): 管理された クラスター での トレーニング 用に APIキー を設定します。
2. [オフラインモード](/ja/models/support/run_wandb_offline/)
3. [オンプレミス](/ja/platform/hosting/hosting-options/self-managed): インフラストラクチャー 内の プライベートクラウド やエアギャップのある サーバー に W&B をインストールします。 ローカルインストールは、研究者や企業の チーム に適しています。