...
レジストリを作成すると、利用可能なアーティファクト タイプを柔軟に管理したり、レジストリの公開範囲を制限したりできます。
...
<Tabs>
  <Tab title=" W&B Registry UI">
    ...
    7. （**Specify types** を選択した場合）レジストリで受け入れる 1 つ以上のアーティファクトタイプを追加します。
...
<Note>
  アーティファクトタイプは、レジストリの設定に保存された後はレジストリから削除できません。
</Note>
