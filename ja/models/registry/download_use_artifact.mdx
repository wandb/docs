...
`wandb.Run.use_artifact()` メソッドは、[run](/ja/models/runs/) を作成すると同時に、ダウンロードした Artifact をその run のインプットとしてマークします。
...
パラメータと戻り値の型については、API リファレンスの [`wandb.Run.use_artifact()`](/ja/models/ref/python/experiments/run#use_artifact) および [`Artifact.download()`](/ja/models/ref/python/experiments/artifact#download) を参照してください。
