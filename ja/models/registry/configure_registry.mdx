...
| 使用：Registry 内で use&#95;artifact を使ってアーティファクトを利用                                                       | Read             |                                                            |   ✓    |   ✓    |   ✓   |
...
| リンクされたアーティファクトの alias を追加または削除                                                                 | Update           |                                                            |        |   ✓    |   ✓   |
| リンクされたアーティファクトの [保護された alias](/ja/models/registry/aliases#protected-aliases) を追加または削除       | Update           |                                                            |        |        |   ✓   |
...
このロールでは、コレクション、オートメーション、その他の registry リソースの作成・更新・削除はできませんが、registry アーティファクトへの読み取り専用アクセス権が付与されます。

**Viewer** と異なり、**Restricted Viewer** は次の操作ができません:

* アーティファクト ファイルをダウンロードしたり、ファイルの内容にアクセスしたりすること。
...
<Note>
  ...
  **Restricted Viewer** として W&amp;B SDK を使用してアーティファクトにアクセスするには、W&amp;B SDK バージョン 0.19.9 以上を使用する必要があります。それより前のバージョンを使用している場合、一部の SDK コマンドで権限エラーが発生します。
</Note>
...
次のメソッドはアーティファクトのメタデータにのみアクセスできます:
...
* 両方のレジストリにリンクされた同じアーティファクトでも、アクセスレベルは異なります
* Registry A では、ユーザーは **Restricted Viewer** であり、ファイルのダウンロードやアーティファクトの利用はできません
* Registry B では、ユーザーは **Viewer** であり、ファイルのダウンロードやアーティファクトの利用ができます
* つまり、アクセス権は、どのレジストリからそのアーティファクトにアクセスするかによって決まります