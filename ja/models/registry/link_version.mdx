...
* コレクションが属する Registry が既に存在していること。Registry が存在するかどうかを確認するには、[Registry App に移動して](/ja/models/registry/search_registry) Registry 名を検索してください。
</Note>
...
[`wandb.Run.link_artifact()`](/ja/ref/python/experiments/run#link_artifact") または [`wandb.Artifact.link()`](/ja/ref/python/experiments/artifact#method-artifactlink") を使用して、Artifact バージョンをプログラムでコレクションにリンクします。
...
* Source version: the artifact version inside a team's project that is logged to a [run](/ja/models/track/runs/).
...
Registry App では、[リンクされた Artifact のメタデータ、バージョンデータ、使用状況、リネージ情報などを表示](/ja/models/registry/link_version#view-linked-artifacts-in-a-registry) できます。
...
1. [`wandb.init()`](/ja/models/ref/python/functions/init) で Run を初期化する際に、チームを entity として指定します。Run の初期化時に `entity` を指定しない場合、Run はデフォルトの entity（チーム entity である場合とそうでない場合があります）を使用します。
...
2. `wandb.Run.log_artifact()` を使用するか、Artifact オブジェクトを作成して以下の方法でファイルを追加して、Run に Artifact をログ記録します。
    ...
    ```
    Artifact のログ記録については、[Artifact の構築](/ja/models/artifacts/construct-an-artifact/) を参照してください。
