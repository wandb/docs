...
## artifact をコレクションにリンクする {#link-an-artifact-to-a-collection}
...
##### run からアーティファクトをリンクする {#link-an-artifact-from-a-run}
...
##### アーティファクトを作成してリンクする {#create-and-link-an-artifact}
...
##### 既存のアーティファクトをリンクする {#link-an-existing-artifact}
...
## コレクションから アーティファクト バージョンのリンクを解除する {#unlink-an-artifact-version-from-a-collection}

W&amp;B Python SDK または W&amp;B Registry を使用して、コレクションから アーティファクト バージョンのリンクを解除します。

リンクを解除すると、アーティファクト バージョンはコレクションから削除されますが、ソース アーティファクト バージョンやそのファイルは削除されません。

<Warning>
  アーティファクト バージョンを Registry のコレクションにリンクすると、存続期間 (TTL) ポリシーが無効化され、TTL が `None` に設定されます。後でバージョンのリンクを解除しても、W&amp;B は以前の TTL を復元せず、元の期間も保持しません。バージョンに引き続き TTL が必要な場合は、リンク解除後に TTL を再設定してください。

  TTL ポリシーの詳細については、[アーティファクト データ保持を管理する](/ja/models/artifacts/ttl)を参照してください。
</Warning>

<Tabs>
  <Tab title="Python SDK">
    W&amp;B Python SDK で アーティファクト バージョンのリンクを解除するには、次の手順に従います。

    1. `wandb.Api()` を使用して Public API クライアントを作成します。
    2. `wandb.Api.artifact()` を使用してリンク済みの アーティファクト バージョンを取得します。
    3. [`wandb.Artifact.unlink()`](/ja/models/ref/python/experiments/artifact#method-artifactunlink) を呼び出して、コレクションから アーティファクト バージョンのリンクを解除します。

    ```text
    wandb-registry-<registry_name>/<collection_name>:<version_or_alias>
    ```

    プレースホルダーをご自身の値に置き換えます。

    ```python
    import wandb

    registry_name = "your-registry-name"  # Registry の名前に置き換えます
    collection_name = "your-collection-name"  # コレクションの名前に置き換えます
    version = "version-or-alias"  # リンクを解除する アーティファクト のバージョンに置き換えます
    ...
    # コレクションから アーティファクト バージョンのリンクを解除します
  ...
  <Tab title="W&B Registry">
    1. W&amp;B Registry にアクセスします。
    2. コレクションを含む Registry の名前を選択します。
    3. コレクションを選択します。
    4. リンクを解除する アーティファクト バージョンを選択します。
    5. **Version** タブで、**Unlink version** を選択します。
  </Tab>
</Tabs>

## Registry 内のリンクされた アーティファクト を表示する {#view-linked-artifacts-in-a-registry}

W&amp;B Registry で、メタデータ、リネージ、利用状況情報など、リンクされた アーティファクト に関する情報を表示します。

1. W&amp;B Registry にアクセスします。
2. アーティファクト をリンクした registry の名を選択します。
3. collection の名を選択します。
4. collection の アーティファクト がメトリクスをログしている場合は、**Show metrics** をクリックして、バージョン間でメトリクスを比較します。
5. アーティファクト バージョンの一覧から、アクセスしたいバージョンを選択します。バージョン番号は `v0` から始まり、リンクされた各 アーティファクト バージョンに段階的に割り当てられます。
6. アーティファクト バージョンの詳細を表示するには、そのバージョンをクリックします。このページのタブから、そのバージョンのメタデータ (ログされたメトリクスを含む) 、リネージ、利用状況情報を表示できます。

**Version** タブ内の **Full Name** フィールドに注目してください。リンクされた アーティファクト の完全名は、registry、collection 名、および アーティファクト バージョンの alias またはインデックスで構成されます。

```text title="Full name of a linked artifact"
wandb-registry-{REGISTRY_NAME}/{COLLECTION_NAME}:v{INTEGER}
```

アーティファクト のバージョンにプログラムからアクセスするには、リンクされた アーティファクト のフルネームが必要です。
...
<div id="logging-artifacts-from-a-personal-account">
  ### 個人アカウントから Artifacts をログする {#logging-artifacts-from-a-personal-account}
</div>
...
#### チーム entity を確認する {#find-your-team-entity}
...
<div id="log-from-a-team-entity">
  #### チーム entity でログする {#log-from-a-team-entity}
</div>
...
### W&amp;B App UI で Registry のパスを確認する {#confirm-the-path-of-a-registry-in-the-wb-app-ui}
