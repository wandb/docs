...
各コレクションでは、1種類のartifactの*タイプ*のみを受け入れます。指定したタイプによって、あなたや同じ組織の他のメンバーがそのコレクションにリンクできるartifactの種類が制限されます。
...
コレクションを作成するときは、あらかじめ定義されたartifactタイプの一覧から選択できます。利用できるartifactタイプは、そのコレクションが属するレジストリによって異なります。
