...
<Note>
  アーティファクト バージョンがメトリクスをログしている場合 (たとえば `wandb.Run.log_artifact()` を使用する場合) 、そのバージョンの詳細ページからメトリクスを確認でき、コレクションのページから アーティファクト バージョン間のメトリクスを比較できます。詳しくは、[レジストリ内のリンクされた artifact を表示する](/ja/models/registry/link_version/#view-linked-artifacts-in-a-registry)を参照してください。
</Note>
