...
このレポートの&quot;リンクを知っている全員が閲覧できる&quot;共有リンクを無効にします。

このレポートの有効な公開アクセストークンをすべて失効させます。

**戻り値:**

* `bool`: リンクが失効した場合は True、アクティブなリンクが存在しなかった場合は False。
...
このレポートで「リンクを知っている人は誰でも閲覧可能」の共有設定を有効にします。

レポートの公開アクセストークンを作成します。共有リンクがすでに存在する場合は、既存のリンクを返します。

**戻り値:**

* `str`:  共有可能なマジックリンクの URL。
...
このレポートのマジックリンク URL を取得します。共有が有効でない場合は None を返します。

有効なアクセストークンがあるか確認するために、ネットワーク通信を行います。アクセストークンが付加されたレポート URL を返すため、リンクを知っている人は誰でも、プロジェクトが非公開でもレポートを閲覧できます。
