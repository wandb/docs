...
Receiver Operating Characteristic (ROC) 曲線チャートを構築します。

**引数:**

* `y_true`:  目的変数の真のクラスラベル (正解ラベル) 。形状は (num&#95;samples,) である必要があります。
* `y_probas`:  各クラスに対する予測確率または決定スコア。形状は (num&#95;samples, num&#95;classes) である必要があります。
* `labels`:  `y_true` のクラスインデックスに対応する人間が読めるラベル。例えば、`labels=['dog', 'cat']` の場合、クラス 0 はプロット内で &#39;dog&#39;、クラス 1 は &#39;cat&#39; として表示されます。None の場合、`y_true` の生のクラスインデックスが使用されます。デフォルトは None です。
* `classes_to_plot`:  ROC 曲線に含める一意のクラスラベルのサブセット。None の場合、`y_true` 内のすべてのクラスがプロットされます。デフォルトは None です。
* `title`:  ROC 曲線プロットのタイトル。デフォルトは &quot;ROC Curve&quot; です。
* `split_table`:  表を W&amp;B UI の別セクションに分割するかどうか。`True` の場合、表は &quot;Custom Chart Tables&quot; という名前のセクションに表示されます。デフォルトは `False` です。
...
# 3つの疾患を対象とした医療診断の分類問題をシミュレートする
n_samples = 200
n_classes = 3

# 真のラベル: 各サンプルに "Diabetes"、"Hypertension"、または "Heart Disease" を割り当てる
disease_labels = ["Diabetes", "Hypertension", "Heart Disease"]
# 0: Diabetes、1: Hypertension、2: Heart Disease
y_true = np.random.choice([0, 1, 2], size=n_samples)

# 予測確率: 各サンプルの合計が1になるように予測をシミュレートする
