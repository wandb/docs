...
W&amp;B Automations API は、ML パイプライン内のイベントに応答する自動化ワークフローをプログラムから作成および管理できるようにします。モデル パフォーマンスのしきい値やアーティファクトの作成など、特定の条件が満たされたときに実行されるアクションを設定できます。
...
| [`OnCreateArtifact`](/ja/models/ref/python/automations/oncreateartifact/) | コレクション内で新しいアーティファクトが作成されたときにトリガーされます。 |
...
* アーティファクトにタグが付けられたときにデプロイ ワークフローをトリガーする
