...
<div id="class-wandbautomationsoncreateartifact">
  ## <Badge color="yellow" size="lg" shape="rounded">クラス</Badge> wandb.automations.OnCreateArtifact
</div>
...
<ResponseField name="scope" type="Annotated">
  イベントのスコープです。アーティファクト コレクションである必要があります。
</ResponseField>

<ResponseField name="filter" type="Annotated">
  このイベントをトリガーするために必要な追加条件がある場合は、それを指定します。
</ResponseField>
