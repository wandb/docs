...
例:
プロジェクト &quot;my-project&quot; 内の任意の run について、その状態が
&quot;finished&quot; (つまり成功) または &quot;failed&quot; に変化したときにトリガーされるイベントを定義します:
...
* `scope` (ProjectScope): イベントのスコープです。プロジェクトである必要があります。
* `filter` (RunStateFilter): このイベントがトリガーされるために満たす必要があるRun 状態条件。
