...
`wandb.init()`の使い方や詳細な例については、[ガイドとFAQ](/ja/models/track/launch)をご覧ください。
...
| 戻り値 | Description |
| :--- | :--- |
|  現在のRunのハンドルである`Run`オブジェクト。このオブジェクトを使用して、データをログしたり、ファイルを保存したり、Runを終了するなどの操作を行うことができます。[Run API](/ja/models/ref/python/experiments/run/)の詳細をご覧ください。 |
