...
description: W&B Python SDK の基盤クラスを使用して実験をトラッキングし、アーティファクトを管理する
module: wandb
no_list: true
---

これらのクラスは、機械学習実験のトラッキング、アーティファクトの管理、SDK の動作設定のための中核的な構成要素です。これらの基盤クラスにより、メトリクスのログ記録、モデル チェックポイントの保存、データセットのバージョン管理、および実験設定の管理を、高い再現性とコラボレーション機能とともに行うことができます。
...
メタデータ付きの モデル アーティファクトを作成し、バージョンとしてログに記録します：
