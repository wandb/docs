...
| クラス | 説明 |
|-------|-------------|
| [`Run`](/ja/models/ref/python/experiments/run/) | W&amp;B によってログされる計算の主要な単位であり、メトリクス、設定、および出力を伴う単一の ML 実験を表します。 |
| [`Artifact`](/ja/models/ref/python/experiments/artifact/) | データセットとモデルのバージョン管理のための柔軟で軽量な構成要素であり、自動重複排除とリネージ追跡を備えています。 |
| [`Settings`](/ja/models/ref/python/experiments/settings/) | W&amp;B SDK の設定管理であり、ログから API とのやり取りまでの動作を制御します。 |
