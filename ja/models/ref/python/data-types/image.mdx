...
* `data_or_path`:  画像データの NumPy 配列 / PyTorch テンソル、PIL 画像オブジェクト、または画像ファイルへのパスを受け入れます。NumPy 配列または PyTorch テンソルが渡された場合、画像データは指定されたファイル形式で保存されます。値が [0, 255] の範囲になかったり、すべての値が [0, 1] の範囲にある場合、`normalize` が `False` に設定されていない限り、画素値は [0, 255] の範囲に正規化されます。
