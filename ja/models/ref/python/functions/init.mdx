...
`wandb.init()` は、run にデータをログするための新しいバックグラウンドプロセスを起動します。また、デフォルトでデータを [https://wandb.ai](https://wandb.ai) に Sync するため、結果をリアルタイムで確認できます。データのログが完了したら、`wandb.Run.finish()` を呼び出して run を終了します。`run.finish()` を呼び出さない場合、スクリプトの終了時に run も終了します。
