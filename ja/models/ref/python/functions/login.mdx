...
## <Badge color="blue" size="lg" shape="rounded">関数</Badge> wandb.login() {#function-wandblogin}
...
* 対話型プロンプト (利用可能で、かつ `prompt` が True の場合)
...
<ResponseField name="prompt" type="bool">
  APIキーが設定されていない場合に、インタラクティブに入力を求めるかどうか。False の場合は False を返し、セッションと設定は変更されません。
</ResponseField>
...
この call の後にセッションが認証情報を保持している場合は True。

## Raises {#raises}

* `AuthenticationError`: 認証情報のサーバー検証に失敗した場合。
* `UsageError`: 認証情報が設定されておらず、`prompt` が True で、APIキーを入力するためのターミナルがない場合。