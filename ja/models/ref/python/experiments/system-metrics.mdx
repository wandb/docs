---
description: W&B SDK が Runs 中に自動的にログする CPU、GPU、メモリ、ディスク、ネットワークのシステムメトリクスのリファレンス。
...
<div id="view-system-metrics">
  ## システムメトリクスを表示する {#view-system-metrics}
</div>

W&amp;B App または `wandb leet` のターミナル UI を使用して、システムメトリクスを表示および監視できます。
...
<Tabs>
  ...
  <Tab title="LEET">
    `wandb leet` のターミナル UI を使用してターミナルでローカルの run のシステムメトリクスを表示するには、次の手順に従います。
    ...
    2. 次のいずれかのコマンドを使用して `wandb leet` を起動します。
    ...
    run パスを指定しない場合、LEET はデフォルトで Workspace ビューを開きます。`2` を押すと、ハイライトされている run のシステムメトリクスが表示されます。run ディレクトリまたは `.wandb` ファイルを指定すると単一 run ビューが開き、システムメトリクスはデフォルトで右側のサイドバーに表示されます。

    システムメトリクスのペインには、次の内容が表示されます。
    ...
    * `y`: フォーカス中のチャートの表示を、線形、対数、および割合のメトリクスの場合はバケット化されたヒートマップの各モードで切り替えます。
    * `n` および `N`: フォーカス中のペイン内でページ間を移動します。
    * `q` または `ctrl+c`: 終了します。

    run なしで現在のマシンを監視するには、スタンドアロンのシステムモニター (SYMON) を起動します。

    ```bash
    wandb leet symon --interval 2s
    ```

    詳細なガイドについては [LEET terminal UI](/ja/models/app/leet-tui) を、コマンド構文については [`wandb leet`](/ja/models/ref/cli/wandb-leet) の CLI リファレンスを参照してください。
  </Tab>
</Tabs>
...
<div id="process-cpu-percent-cpu">
  ### プロセス CPU 使用率 (CPU) {#process-cpu-percent-cpu}
</div>
...
<div id="process-cpu-threads">
  ### プロセスの CPU スレッド数 {#process-cpu-threads}
</div>
...
[Disk In](#disk-in) と同様に、最初のサンプル取得時に初期のディスク書き込みバイト数が記録されます。以降のサンプルでは、現在の書き込みバイト数と初期値との差分が計算されます。
...
<div id="process-memory-rss">
  ### プロセスメモリ RSS {#process-memory-rss}
</div>
...
<div id="process-memory-percent">
  ### プロセスのメモリ使用率 {#process-memory-percent}
</div>
...
[ネットワーク送信](#network-sent) と同様に、受信バイト数の初期値はメトリクスが最初に初期化された時点で記録されます。以降のサンプルでは、現在の受信バイト数と初期値との差分が計算されます。
...
<div id="gpu-memory-allocated">
  ### GPU メモリ割り当て率 {#gpu-memory-allocated}
</div>
...
<div id="gpu-memory-allocated-bytes">
  ### GPU メモリ割り当て量 (バイト) {#gpu-memory-allocated-bytes}
</div>
...
<div id="gpu-power-usage-watts">
  ### GPU 電力使用量 (ワット) {#gpu-power-usage-watts}
</div>
...
<div id="gpu-power-usage-percent">
  ### GPU 電力使用率 {#gpu-power-usage-percent}
</div>
...
<div id="gpu-sm-clock-speed">
  ### GPU SM クロック速度 {#gpu-sm-clock-speed}
</div>
...
<div id="gpu-memory-clock-speed">
  ### GPUメモリクロック速度 {#gpu-memory-clock-speed}
</div>
...
<div id="gpu-graphics-clock-speed">
  ### GPU グラフィックスクロック速度 {#gpu-graphics-clock-speed}
</div>
...
<div id="gpu-corrected-memory-errors">
  ### GPU 訂正済みメモリエラー {#gpu-corrected-memory-errors}
</div>
...
<div id="gpu-uncorrected-memory-errors">
  ### GPU の訂正不能メモリエラー {#gpu-uncorrected-memory-errors}
</div>
...
<div id="gpu-encoder-utilization">
  ### GPU エンコーダー使用率 {#gpu-encoder-utilization}
</div>
...
<div id="amd-gpu-utilization">
  ### AMD GPU 使用率 {#amd-gpu-utilization}
</div>
...
<div id="amd-gpu-memory-allocated">
  ### AMD GPU メモリ割り当て率 {#amd-gpu-memory-allocated}
</div>
...
<div id="amd-gpu-temperature">
  ### AMD GPU 温度 {#amd-gpu-temperature}
</div>
...
<div id="amd-gpu-power-usage-watts">
  ### AMD GPU 電力使用量 (ワット) {#amd-gpu-power-usage-watts}
</div>
...
<div id="amd-gpu-power-usage-percent">
  ### AMD GPU 電力使用率 {#amd-gpu-power-usage-percent}
</div>
...
<div id="apple-arm-mac-gpu">
  ## Apple ARM MacのGPU {#apple-arm-mac-gpu}
</div>

<div id="apple-gpu-utilization">
  ### Apple GPU 使用率 {#apple-gpu-utilization}
</div>
...
<div id="apple-gpu-memory-allocated">
  ### Apple GPU メモリ割り当て率 {#apple-gpu-memory-allocated}
</div>
...
<div id="apple-gpu-temperature">
  ### Apple GPU 温度 {#apple-gpu-temperature}
</div>
...
<div id="apple-gpu-power-usage-watts">
  ### Apple GPU 電力使用量 (ワット) {#apple-gpu-power-usage-watts}
</div>
...
<div id="apple-gpu-power-usage-percent">
  ### Apple GPU 電力使用率 {#apple-gpu-power-usage-percent}
</div>
...
<div id="ipu-device-metrics">
  ### IPU デバイスのメトリクス {#ipu-device-metrics}
</div>
...
<div id="tpu-compute-metrics">
  ### TPU Compute メトリクス {#tpu-compute-metrics}
</div>
...
<div id="tpu-memory-metrics">
  ### TPU メモリメトリクス {#tpu-memory-metrics}
</div>
...
<div id="tpu-interconnect-health">
  ### TPU インターコネクトの健全性 {#tpu-interconnect-health}
</div>
...
<div id="tpu-transfer-latency">
  ### TPU 転送レイテンシ {#tpu-transfer-latency}
</div>
...
<div id="tpu-collective-communication">
  ### TPU 集合通信 {#tpu-collective-communication}
</div>
...
<div id="tpu-network-metrics">
  ### TPU ネットワーク メトリクス {#tpu-network-metrics}
</div>
...
<div id="tpu-hlo-execution-metrics">
  ### TPU HLO 実行メトリクス {#tpu-hlo-execution-metrics}
</div>
...
<div id="trainium-neuron-core-utilization">
  ### Trainium Neuron Core 使用率 {#trainium-neuron-core-utilization}
</div>
...
<div id="trainium-host-memory-usage-total">
  ### Trainium ホストの合計メモリ使用量 {#trainium-host-memory-usage-total}
</div>
...
<div id="trainium-neuron-device-total-memory-usage">
  ### Trainium Neuron デバイスの総メモリ使用量 {#trainium-neuron-device-total-memory-usage}
</div>
...
<div id="trainium-host-memory-usage-breakdown">
  ### Trainium ホストメモリ使用量の内訳: {#trainium-host-memory-usage-breakdown}
</div>
...
<div id="trainium-neuron-core-memory-usage-breakdown">
  ### Trainium NeuronCore のメモリ使用量の内訳 {#trainium-neuron-core-memory-usage-breakdown}
</div>
