...
```python
with wandb.init() as run:
     # 例: X軸として使用する「epoch」メトリクスをログする。
     run.log({"epoch": 40, "train-loss": 0.5})
```

`step` パラメーターと `commit` パラメーターを使用すると、複数回の `wandb.Run.log()` 呼び出しで同じ `step` にログできます。以下はすべて同じ意味です。
...
* `data`:  `str` キーと、シリアライズ可能な値を持つ `dict`
* `Python objects including`:  `int`、`float`、`string`、`wandb.data_types` のいずれか、シリアライズ可能な Python オブジェクトのリスト、タプル、NumPy 配列、および同じ構造の他の `dict`
...
**例:**
さらに詳しい例については、[logging に関するガイド](https://docs.wandb.com/guides/track/log)を参照してください。
...
# => runの "run123/" フォルダにファイルを保存します。下記の注を参照。
...
# 名前とエイリアスでartifactを使用する
artifact_a = run.use_artifact(artifact_or_name="<name>:<alias>")

# 名前とバージョンでartifactを使用する
artifact_b = run.use_artifact(artifact_or_name="<name>:v<version>")

# entity/プロジェクト/名前:エイリアスでartifactを使用する
artifact_c = run.use_artifact(
    artifact_or_name="<entity>/<project>/<name>:<alias>"
)

# entity/プロジェクト/名前:バージョンでartifactを使用する
artifact_d = run.use_artifact(
    artifact_or_name="<entity>/<project>/<name>:v<version>"
)

# コンテキストマネージャーを使用していないため、runを明示的に終了する。
run.finish()
```
