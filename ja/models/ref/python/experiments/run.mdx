...
この run に関連付けられている設定オブジェクト。

### <kbd>プロパティ</kbd> config_static

この run に関連付けられている静的な設定オブジェクト。

### <kbd>プロパティ</kbd> dir

run の file が保存されるディレクトリです。

これは、`run.save()` で保存される file を指します。`run.log()` に渡された特定のデータタイプに対して自動的に
作成される file も含まれます。run のすべてのデータが含まれる
ディレクトリについては、`run.sync_dir` を参照してください。

### <kbd>プロパティ</kbd> disabled

run が無効な場合は True、そうでない場合は False。

### <kbd>プロパティ</kbd> entity

run に関連付けられている W&amp;B entity の名前。
...
### <kbd>プロパティ</kbd> job_type

run に関連付けられているジョブタイプの名。

W&amp;B App の run の Overview ページで、run のジョブタイプを確認できます。

これを使用すると、&quot;training&quot;、&quot;evaluation&quot;、&quot;inference&quot; などのジョブタイプごとに run を分類できます。これは、特に同じプロジェクト内に異なるジョブタイプの複数の run がある場合に、W&amp;B UI で run を整理したりフィルターしたりする際に役立ちます。詳細は [Organize Runs](https://docs.wandb.ai/models/runs#organize-runs) を参照してください。

### <kbd>プロパティ</kbd> 名

run の表示名。

表示名は一意であるとは限らず、内容を説明するものの場合があります。

デフォルトでは、ランダムに生成されます。

### <kbd>プロパティ</kbd> メモ

run に関連付けられているメモです (存在する場合) 。

メモには複数行の文字列を指定でき、`$$` 内では Markdown や LaTeX の数式 (`$x + 3$` など) も使用できます。

### <kbd>プロパティ</kbd> offline
...
Run のパスには entity、プロジェクト、run ID が含まれ、形式は
...
### <kbd>プロパティ</kbd> resumed

run が再開された場合は True、そうでない場合は False を返します。

### <kbd>プロパティ</kbd> settings

run の Settings オブジェクトを凍結したコピー。

### <kbd>プロパティ</kbd> start_time

run が開始された時刻の Unix タイムスタンプ (秒単位) 。

### <kbd>プロパティ</kbd> step

次の `log()` 呼び出しで使用される W&amp;B の step です。

mode=&quot;shared&quot; の runs ではエラーになります。

### <kbd>プロパティ</kbd> sweep_id

run に関連付けられている sweep の識別子 (存在する場合) 。

### <kbd>プロパティ</kbd> sweep_url

run に関連付けられている sweep の URL (ある場合) 。

オフラインの run には sweep URL はありません。

### <kbd>プロパティ</kbd> sync_dir

run のすべてのデータが含まれるディレクトリです。

このディレクトリを `wandb sync` に渡すと、run をアップロードまたは再アップロードできます。
...
* `summary`: summary に追加する集約メトリクスを指定します。サポートされる集約には &quot;min&quot;、&quot;max&quot;、&quot;mean&quot;、&quot;last&quot;、&quot;first&quot;、&quot;copy&quot;、&quot;none&quot; が含まれます。&quot;none&quot; を指定すると summary は生成されません。&quot;copy&quot; も非推奨のため使用しないでください。
...
* `name`: アーティファクト 名。entity/プロジェクト を接頭辞として付けることもできます。
  有効な名前の形式は次のとおりです:
  * name:version
  * name:alias
  * digest
...
* `data`: `str` キーと、`int`、`float`、`string`、任意の `wandb.data_types`、シリアライズ可能な Python オブジェクトのリスト、タプル、NumPy 配列、および同じ構造の他の `dict` を含む、シリアライズ可能な Python オブジェクトの値を持つ `dict` です。
...
* `artifact_or_path`: (str または Artifact) この アーティファクト の内容へのパス。
  指定できる形式は次のとおりです:
  * `/local/directory`
  * `/local/directory/file.txt`
  * `s3://bucket/path`
...
* `include_fn`: ファイルパスと (必要に応じて) ルートパスを受け取り、含める場合は True、そうでない場合は False を返す callable。デフォルトは `lambda path, root: path.endswith(".py")` です。
...
### <kbd>method</kbd> Run.pin_config_keys()
...
* `glob`: `glob_str` を glob パターンとして扱うかどうか。後方互換性のため、デフォルトでは `True` です。`glob_str` の名前に `[`、`]`、`*`、`?` などの glob メタ文字が含まれており、それらをパターンとして解釈したくない場合は、`False` に設定して `glob_str` をリテラルパスとして扱います。
...
* `artifact_or_name`: 使用する アーティファクト の名前。アーティファクト がログされたプロジェクト名
...
* `name`: モデル アーティファクト の名です。`name` は、既存のログ済み
...
* `log`: `"gradients"`、`"parameters"`、または `"all"` をログするかどうかを指定します。ロギング を無効にするには None に設定します。 (デフォルト=`"gradients"`) 。
...
* `text`: 書き込むテキストです。末尾に改行がない場合は、改行が追加されます。