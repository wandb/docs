...
* `allow_offline_artifacts` (bool): オフライン モードでテーブル Artifacts の同期を許可するかどうかを制御するフラグです。
...
* `api_key` (Optional[str]): W&amp;B の API キー。
...
* `disable_job_creation` (bool): W&amp;B Launch の job Artifacts の作成を無効にするかどうかを指定します。
