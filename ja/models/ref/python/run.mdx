...
[こちらのガイド](/ja/models/track/launch)をご覧ください。
...
[こちらのガイド](/ja/models/track/log/distributed-training)をご覧ください。
...
[ログ記録ガイド](/ja/models/track/log)を参照し、
...
ワークスペースでログしたデータを[wandb.ai](https://wandb.ai)で可視化したり、自分でホストするW&Bアプリのインスタンスでローカルに可視化したり、データをエクスポートしてローカルで可視化、探索したり、例えば、Jupyter ノートブックで[公開API](/ja/models/track/public-api-guide)を使用して行うことができます。
...
さまざまにサポートされているタイプについては、[参照ドキュメント](/ja/models/ref/python/data-types/)を参照するか、[logging ガイド](/ja/models/track/log)で、3D分子構造やセグメンテーションマスク、PR曲線、ヒストグラムの例を確認してください。
`wandb.Table`を使用して構造化データをログできます。詳細については、[テーブルログのガイド](/ja/models/tables/tables-walkthrough)をご覧ください。
...
[define_metric](/ja/models/ref/python/experiments/run/#method-rundefine_metric)も参照してください。
