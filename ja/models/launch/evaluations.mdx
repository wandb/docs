...
このページでは、 [LLM Evaluation Jobs](/ja/models/launch) が提供する評価ベンチマークをカテゴリ別にリストアップしています。

特定のベンチマークを実行するには、チーム管理者が [チームスコープの secret](/ja/platform/secrets#add-a-secret) として必要な API キーを追加する必要があります。チームメンバーは、評価ジョブを設定する際にその secret を指定できます。
