...
run の **Overview** タブにある **Config** セクションで設定値を確認できます。設定値をピン留めすると、**References** セクションでも確認できます。

1. W&amp;B App で対象の project にアクセスします。
2. 設定値を表示したい run をクリックします。
3. **Overview** タブを選択します。
4. **Config** セクションまでスクロールします。
5. (任意) **View raw data** をクリックすると、JSON 形式で設定値を確認できます。

raw JSON 形式は、設定値、ログ済みのメトリクス、またはサマリー値から折れ線グラフを作成または変換する式を記述する際に便利です。詳細は [式](/ja/models/app/features/panels/line-plot/reference#expressions) を参照してください。
