...
次のコード例では、設定値を含む Python の辞書を定義する方法と、W&amp;B run を初期化する際にその辞書を引数として渡す方法を示します。
...
def main(args):
    ...
    with wandb.init(project="config_example", config=args) as run:
        # 設定 dict から値を取得し、可読性のために変数に格納する
...
`config` オブジェクトには、スクリプト全体を通してさらにパラメーターを追加できます。次のコード例は、`config` オブジェクトに新しいキーと値のペアを追加する方法を示しています。
...
# 以下の値は、現在のスクリプトまたはノートブックから開始した場合はRunオブジェクトから取得できます。
# W&B App UIからコピーすることも可能です。
username = run.entity
project = run.project
run_id = run.id
...
```python
flags.DEFINE_string("model", None, "model to run")  # 名, デフォルト, ヘルプ

run.config.update(flags.FLAGS)  # absl flagsを設定に追加
```
...
# カスタム値を渡してconfig-defaults.yamlの設定を上書きする
...
with wandb.init() as run:
    ...
    run.config.update(flags.FLAGS)  # TensorFlowのフラグを設定として追加
```
