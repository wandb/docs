...
`config-defaults.yaml` は、ネストされたマッピングがトップレベルキーの `value` になっている場合に、ネストされた辞書をサポートします。また、ネストされた辞書を `wandb.init()` に直接渡すこともできます。
...
W&amp;B App の一部のビューでは、ネストされた設定キーがドット区切り形式で表示されます (例: `EPISODE_BUDGET.cramped_room`)。Public API のフィルターでは、これらのパスの先頭に `config.` が付きます (例: `config.EPISODE_BUDGET.cramped_room`)。Python では、通常の 辞書 のインデックス指定でネストされた値にアクセスします (例: `run.config["EPISODE_BUDGET"]["cramped_room"]`)。
