...
<Frame>
  <img src="/images/app_ui/metadata_panel.png" alt="Artifacts のメタデータ パネル" />
</Frame>
...
Usage パネルには、Web アプリの外部、たとえばローカル マシンで使用するためにアーティファクトをダウンロードするコード スニペットが表示されます。また、このセクションには、そのアーティファクトを出力した Run と、そのアーティファクトを入力として使用する Runs が示され、それぞれへのリンクも表示されます。

アーティファクトの使用状況を表示するには:
...
<Note>
  **Runs を比較するための説明的なメモは W&amp;B レポートで作成する**

  W&amp;B レポートを作成して、プロットと Markdown を横に並べて追加することもできます。異なるセクションを使って別々の Runs を表示し、自分が取り組んだ内容についてストーリーを伝えましょう。
</Note>
