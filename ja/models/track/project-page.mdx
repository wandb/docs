...
* 最近削除した Runs を復元するには、右上の **...** をクリックし、**Undelete recently deleted runs** をクリックします。
...
* **ドットメニュー**: サイドバーの行にカーソルを合わせると、左側にメニューが表示されます。このメニューから、run の名前変更、削除、アクティブな run の停止を行えます。
...
<Tabs>
  <Tab title="列をカスタマイズ">
    ...
    * 列を固定するには、列名にカーソルを合わせ、表示されるアクションメニュー `...` をクリックしてから、**Pin column** をクリックします。固定された列は、**Name** 列の後、ページの左側に表示されます。固定を解除するには、**Unpin column** を選択します。
    * 列を非表示にするには、列名にカーソルを合わせ、表示されるアクションメニュー `...` をクリックしてから、**Hide column** をクリックします。現在非表示の列をすべて確認するには、**Columns** をクリックします。
  ...
  <Tab title="並べ替え">
    ...
    1. 任意の列ヘッダーにカーソルを合わせると表示されるケバブメニュー (縦に並んだ3つの点) をクリックすると、その列の値で表をすばやく並べ替えられます。
  ...
  <Tab title="フィルター">
    ダッシュボードの左上にある **Filter** ボタンを使って、式に基づいてすべての行をフィルタリングします。
...
プロジェクトにスターを付けると、そのプロジェクトを重要なものとしてマークできます。あなたやチームがスターを付けた重要なプロジェクトは、組織のホームページの上部に表示されます。

たとえば、次の画像は、重要なプロジェクトとしてマークされた 2 つのプロジェクト `zoo_experiment` と `registry_demo` を示しています。どちらのプロジェクトも、組織のホームページ上部にある **Starred projects** セクションに表示されます。
...
アプリの左上にある組織名をクリックして、プロジェクトが組織のランディングページに表示されていることを確認します。

<div id="delete-a-project">
  ## プロジェクトを削除する
</div>

Overview タブの右側にある三点メニューをクリックすると、プロジェクトを削除できます。

1. W&amp;B のプロジェクトにアクセスします。
2. プロジェクトのサイドバーから **Overview** タブを選択します。
3. 右上にある三点メニューをクリックします。
