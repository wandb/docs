---
description: "認証、プロジェクト設定、ログモードなどのために、環境変数を使用して W&B SDK の動作を設定します。"
title: 環境変数
---
...
AWS でバッチジョブを実行している場合は、W&amp;B の認証情報でマシンを簡単に認証できます。[ユーザー設定](https://wandb.ai/settings) で APIキー を作成し、[AWS バッチジョブの仕様](https://docs.aws.amazon.com/batch/latest/userguide/job_definition_parameters.html#parameters) で `WANDB_API_KEY` 環境変数を設定します。