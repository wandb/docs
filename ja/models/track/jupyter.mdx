...
2. **コード保存**: モデルを再現する際、ノートブックのどのセルが実行されたか、どの順序で実行されたかを知るのは難しいです。各実験のセル実行の記録を保存するために、[設定ページ](/ja/platform/app/settings-page/)でコード保存をオンにしてください。
...
<Note>
W&B でできることについてもっと知りたいですか？[データとメディアのログガイド](/ja/models/track/log/)をチェックし、[お気に入りの ML ツールキットとのインテグレーション方法](/ja/guides/integrations/)を学ぶか、[リファレンスドキュメント](/ja/models/ref/python/)または私たちの[例のレポジトリ](https://github.com/wandb/examples)に直接飛び込んでください。
</Note>
