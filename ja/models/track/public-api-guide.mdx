---
description: W&B に保存したデータをエクスポートまたは更新する
title: データのエクスポートと更新
---
...
### run データをエクスポート {#export-run-data}
...
### 複数の Runs をクエリする {#querying-multiple-runs}
...
### アクティブな run の実行中に run 名と ID を取得する {#get-a-runs-name-and-id-during-an-active-run}
...
## Public APIの例 {#public-api-examples}

### run のメトリクスを読み取る {#read-metrics-from-a-run}
...
### run から特定のメトリクスを読み取る {#read-specific-metrics-from-a-run}
...
### 2 つの run を比較する {#compare-two-runs}
...
### 完了した run のメトリクスを更新する {#update-metrics-for-a-run-after-the-run-has-finished}
...
### 完了したrun内のメトリクス名を変更する {#rename-a-metric-in-a-completed-run}
...
### 既存のrunの設定を更新する {#update-config-for-an-existing-run}
...
詳細については、[実験の設定](/ja/models/track/config#existing-run)を参照してください。

### システムリソースの使用量をCSVファイルにエクスポートする {#export-system-resource-consumptions-to-a-csv-file}
...
### サンプリングされていないメトリクスデータを取得する {#get-unsampled-metric-data}
...
### history からページ単位でデータを取得する {#get-paginated-data-from-history}
...
### プロジェクト内のすべての Runs のメトリクスを CSV ファイルにエクスポートする {#export-metrics-from-all-runs-in-a-project-to-a-csv-file}
...
### run の開始時刻を取得する {#get-the-starting-time-for-a-run}
...
### 完了済みの run にファイルをアップロードする {#upload-files-to-a-finished-run}
...
### run からファイルをダウンロードする {#download-a-file-from-a-run}
...
### run からすべてのファイルをダウンロードする {#download-all-files-from-a-run}
...
### 特定の sweep から run を取得する {#get-runs-from-a-specific-sweep}
...
### sweep から最適な run を取得する {#get-the-best-run-from-a-sweep}
...
### sweep から最良のモデルファイルをダウンロードする {#download-the-best-model-file-from-a-sweep}
...
### run から指定した拡張子のファイルをすべて削除する {#delete-all-files-with-a-given-extension-from-a-run}
...
### システムメトリクスデータをダウンロードする {#download-system-metrics-data}
...
### summaryメトリクスを更新する {#update-summary-metrics}
...
### run の実行に使用されたコマンドを取得する {#get-the-command-that-ran-the-run}
...
{/* ### サンプリング
  ...
  デフォルトの history メソッドでは、メトリクスを固定数のサンプルにサンプリングします（デフォルトは 500 で、`samples` 引数で変更できます）。大規模な run のすべてのデータをエクスポートする場合は、`run.scan_history()` メソッドを使用できます。詳細については、[API リファレンス](/models/ref/python/public-api) を参照してください。  */}