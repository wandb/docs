---
description: "Python SDK を使用して run の初期化、ハイパーパラメーター、メトリクスのログをトラッキングする W&B Experiment を作成します。"
...
W&amp;B Dashboard を、機械学習モデルの結果を整理して可視化するための中心的な場所として活用しましょう。数回クリックするだけで、[並列座標プロット](/ja/models/app/features/panels/parallel-coordinates/)、[パラメーター重要度分析](/ja/models/app/features/panels/parameter-importance/)、[その他のチャートタイプ](/ja/models/app/features/panels/) などの、リッチでインタラクティブなチャートを作成できます。
