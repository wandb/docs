...
### メトリクスのカーディナリティ {#metric-cardinality}
...
## 大規模運用時の推奨事項 {#recommendations-at-scale}
...
## スループットの例 {#throughput-examples}

ログの記録方法が異なっていても、同じスループットになることがあります。

### スカラーのログの例 {#scalar-logging-examples}
...
### 動画のログ記録の例 {#video-logging-examples}
...
## Logging に関する考慮事項 {#logging-considerations}

実験のメトリクスをトラッキングするには、`wandb.Run.log()` を使用します。

### メトリクスのカーディナリティ {#metric-cardinality-2}
...
### ログ頻度とスループット {#log-frequency-and-throughput}
...
## Workspace のパフォーマンス {#workspace-performance}

Workspace のパフォーマンスは、基盤となる project のデータと Workspace の設定の両方に左右されます。

### project ごとの Runs {#runs-per-project}
...
### 1 run あたり多数のメトリクス {#many-metrics-per-run}
...
### Reports と Workspace {#reports-and-workspaces}
...
## Python スクリプトのパフォーマンス {#python-script-performance}
...
### レート制限の HTTP ヘッダー {#rate-limit-http-headers}
...
### メトリクス logging API のレート制限 {#metric-logging-api-rate-limits}
...
### GraphQL API のレート制限 {#graphql-api-rate-limits}
...
## 動作が遅い project のトラブルシューティング {#troubleshooting-slow-projects}
...
## ブラウザに関する注意事項 {#browser-considerations}
...
## W&amp;B へのパフォーマンス問題の報告 {#reporting-performance-issues-to-wb}
