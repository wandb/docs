...
* `points` は、表示するポイントの座標と色を含む NumPy 配列です。形式には、[上で示した simple point cloud renderer と同じ形式](#python-object)を使用します。
...
点群の表示中は、Controlキーを押したままマウスを使って空間内を移動できます。
