---
description: "実験の run からの構造化データを可視化してクエリするために、W&B 表を作成し、データを追加してログします。"
title: 表をログする
---
...
データが表に入ったら、列または行単位でアクセスできます。

1. **Row Iterator**: `for ndx, row in table.iterrows(): ...` などの表の行イテレータを使って、データの各行を効率的に反復処理できます。
2. **Get a Column**: `table.get_column("col_name")` を使って、データの 1 列を取得できます。便利な機能として、`convert_to="numpy"` を渡すと、その列をプリミティブ型の NumPy NDArray に変換できます。これは列に `wandb.Image` などのメディア型が含まれている場合に、基になるデータへ直接アクセスするのに役立ちます。
