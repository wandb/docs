...
状況によっては、対数スケールの x 軸など、別の x 軸に対してメトリクスをログ記録する方が適切な場合があります。[`define_metric()`](/ja/models/ref/python/experiments/run/#define_metric) メソッドを使用すると、ログ記録する任意のメトリクスをカスタム x 軸として使用できます。
