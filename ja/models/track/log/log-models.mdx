---
title: モデルをログする
description: "モデルのアーティファクトを W&B の run にログし、後で log_model と use_model の SDK メソッドを使って取得します。"
---
...
<details>
  ...
  `log_model` を呼び出すと、`MNIST` という名前のモデル Artifacts が作成され、ファイル `model.h5` がそのモデル Artifacts に追加されます。ターミナルまたはノートブックには、モデルがログされた run に関する情報の確認場所が出力されます。
