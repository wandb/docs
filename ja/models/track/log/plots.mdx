...
`matplotlib` のプロットまたは図オブジェクトを `wandb.Run.log()` に渡すだけです。デフォルトでは、プロットは [Plotly](https://plot.ly/) のプロットに変換されます。プロットを画像としてログしたい場合は、`wandb.Image` にプロットを渡してください。Plotly の chart も直接受け付けています。
