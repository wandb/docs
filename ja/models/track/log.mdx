...
W&amp;B Python SDK を使用して、メトリクス、メディア、またはカスタムオブジェクトの辞書を step にログします。W&amp;B は各 step でキーと値のペアを収集し、`wandb.Run.log()` でデータをログするたびに、それらを1つの単一辞書に保存します。スクリプトからログされたデータは、まず `wandb` というディレクトリにローカル保存され、その後 W&amp;B クラウドまたは[プライベートサーバー](/ja/platform/hosting/)に同期されます。

<Note>
  キーと値のペアが1つの単一辞書に保存されるのは、各 step に同じ値を渡した場合のみです。`step` に異なる値をログすると、W&amp;B は収集したすべてのキーと値をメモリに書き込みます。
</Note>

デフォルトでは、`wandb.Run.log()` の各 call は新しい `step` です。W&amp;B は チャート や panels の作成時に、steps をデフォルトの x-axis として使用します。必要に応じて、カスタム x-axis を作成して使用したり、カスタム summary メトリクスを記録したりできます。詳細は、[ログ軸のカスタマイズ](/ja/models/track/log/customize-logging-axes/)を参照してください。

{/* [より良い例を挿入] */}

{/* コード内のさまざまな場所から同じ history step にログしたい場合は、次のように `run.log()` に step インデックスを渡します。
...
<Warning>
  各`step`に対して連続する値 (0、1、2、…) をログするには、`wandb.Run.log()`を使用してください。特定の history step に書き込むことはできません。W&amp;B が書き込むのは、&quot;現在の&quot; step と &quot;次の&quot; step のみです。
</Warning>

{/* メトリクスを蓄積するには、`run.log` で `commit=False` を設定できます。メトリクスを保存するには、最終的に `commit=True`（デフォルト）を指定して `run.log` を呼び出してください。

  ```python
  run.log({'loss': 0.2}, commit=False)
  # この step を記録する準備ができたら、別の場所で:
