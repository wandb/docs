...
`wandb.Run.log()` を呼び出すたびに、デフォルトでは新しい `step` が作成されます。W&amp;B はチャートとパネルの作成時に、steps をデフォルトの x-axis として使用します。必要に応じて、カスタム x-axis を作成して使用したり、カスタム summary メトリクスを取得したりすることもできます。詳細は, [Customize log axes](/ja/models/track/log/customize-logging-axes/) を参照してください。
...
## 自動的にログされるデータ {#automatically-logged-data}
...
## 特定の W&amp;B API 呼び出しではどのようなデータがログされますか？ {#what-data-is-logged-with-specific-wb-api-calls}
...
## メトリクス名の命名制約 {#metric-naming-constraints}
