...
<Tabs>
  ...
  <Tab title="複数メトリクスの折れ線グラフ">
    [自動 Workspace](/ja/models/app/features/panels#workspace-modes) では、W&amp;B がログされた各メトリクスに対して単一メトリクスの折れ線グラフを自動的に作成します。

    このセクションでは、JavaScript の正規表現で選択した複数のメトリクスを表示する折れ線グラフを作成する方法を説明します。複数メトリクスの折れ線グラフを使用すると、複数のパネルにまたがって確認する代わりに、関連するメトリクスを 1 つのパネルで比較できます。
    ...
    4. **Regex** に、[Google&#39;s RE2](https://github.com/google/re2/wiki/Syntax) 形式の式を入力します。入力に合わせて UI が更新され、どのメトリクスがその式に一致するかが表示されます。デフォルトでは、グラフ名にはそのグラフで使用している正規表現が表示されます。このグラフには、今後ログされるメトリクスも含め、その式に一致するすべてのメトリクスの線が表示されます。
...
### 複数メトリクスの正規表現 {#multi-metric-regular-expressions}

    複数メトリクスの折れ線グラフでは、メトリクス名の一致に [Google&#39;s RE2](https://github.com/google/re2/wiki/Syntax) 構文を使用します。以下のセクションでは、一般的なユースケースを説明し、キャプチャグループが W&amp;B が作成するパネルにどのような影響を与えるかなど、正規表現の仕組みについてさらに詳しく説明します。
...
#### キャプチャグループ {#capture-groups}

    <LinePlotCaptureGroups />
  </Tab>
</Tabs>

## 折れ線グラフの設定を編集する {#edit-line-plot-settings}
...
### 個別の折れ線グラフ {#individual-line-plot}
...
### セクション内のすべての折れ線グラフ {#all-line-plots-in-a-section}
...
### Workspace 内のすべての折れ線グラフ {#all-line-plots-in-a-workspace}
...
## プロット上で平均値を可視化する {#visualize-average-values-on-a-plot}
...
## プロット上でNaN値を可視化する {#visualize-nan-value-on-a-plot}
...
## 1つのチャートで複数のメトリクスを比較する {#compare-multiple-metrics-on-one-chart}
...
## 線の色を変更する {#change-the-colors-of-the-lines}
...
## 異なる x 軸で可視化する {#visualize-on-different-x-axes}
...
## チャートの凡例を非表示にする {#hide-chart-legend}
...
## run メトリクス通知を作成する {#create-a-run-metrics-notification}
