...
keywords: ["EMA スムージング", "ガウシアン スムージング", "full fidelity", "凡例テンプレート", "式"]
---
...
* **フルフィデリティ** (デフォルト) : [フルフィデリティ](/ja/models/app/features/panels/line-plot/sampling/#full-fidelity)を参照してください。
* **ランダムサンプリング**: [ランダムサンプリング](/ja/models/app/features/panels/line-plot/sampling/#random-sampling)を参照してください。
...
Runs table がグループ化されている場合、各折れ線グラフではグループ内の runs が1本の線にまとめられます。このグループ化をグラフにどのように適用するかを設定します。

* **チャートで Runs table のグループ化を使用**: Runs table のグループ化を折れ線グラフに適用します。デフォルトでオンです。Runs table がグループ化されているかどうかにかかわらず、すべての run を個別にプロットするにはオフにします。
* **グループごとに最新の run のみを使用**: グループ内の runs をまとめず、グループごとに1つの run をプロットします。デフォルトでオフです。Workspace レベルでのみ使用できます。
* **集約**: 各グループで結合された線の値を指定します。オプションは **Mean**、**Min**、**Max** です。**グループごとに最新の run のみを使用** がオンの場合は使用できません。

同じグループで experiment を繰り返し実行する場合は、**グループごとに最新の run のみを使用** をオンにします。各チャートには最新の試行のみが表示され、以前の runs は project に残ります。

グループ内の最新の run とは、run selector でそのグループの先頭に表示される run です。Runs table の並べ替え順序を変更すると、グループごとに異なる run をプロットできます。

<Note>
  **グループごとに最新の run のみを使用** は、デフォルトの **Point aggregation method** が **フルフィデリティ** の場合にのみ有効です。**ランダムサンプリング** モードでは効果がありません。
</Note>
...
* **Group runs**: プロットで Runs のグループ化を有効にします。プロットの網掛け範囲を設定するには必須です。Runs の集約を 1 本の線として表示しないようにするには、これを無効にします。グループ化を有効にしたまま集約だけを無効にする個別のスイッチはありません。
* **Group by**: 必要に応じて column を選択します。その column で同じ値を持つすべての Runs が同じグループにまとめられます。
* **集約**: グラフ上の線の値です。オプションは、グループの平均、中央値、最小値、最大値です。このコントロールは **Group runs** が有効な場合にのみ適用されます。
* **Range**: フルフィデリティの折れ線グラフの網掛け領域を設定します。オプションは Min/Max、Std Dev、Std Err、None です。
