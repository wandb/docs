...
## コードの保存を有効にする {#enable-code-saving}

チームまたは組織のコードの保存を設定します。チームおよび組織での設定方法については、[プライバシー設定を構成する](/ja/platform/hosting/privacy-settings)を参照してください。

<Info>
  デフォルトでは、W&amp;B はすべての チーム でコードの保存を無効にしています。チームでこれを有効にするには、組織管理者が組織の **Disable default code saving** をオフのままにしておく必要があります。[組織](#organization)セクションをご覧ください。
</Info>

### チーム {#team}

チーム管理者としてチームのコードの保存を有効にするには、チームの **Settings** ページを開き、**Privacy** セクションに移動して、そのチームの Runs に対する **Enable code saving by default** を設定します。このオプションは、組織管理者が組織全体に対して **Disable default code saving** を有効にしていない場合にのみ利用できます。画面の移動手順については、[チームのプライバシー設定を構成する](/ja/platform/hosting/privacy-settings#configure-privacy-settings-for-a-team)をご覧ください。

### 組織 {#organization}

組織管理者として **Settings** を開き、**Privacy** を選択して、**Project and content security** セクションの **Disable default code saving** を使用することで、すべてのチームでコードの保存をデフォルトでオフにするかどうかを制御できます。この設定が有効な間、チーム管理者はチームに対して **Enable code saving by default** をオンにできません。組織で利用できる制御項目の一覧については、[Enforce privacy settings for all teams](/ja/platform/hosting/privacy-settings#enforce-privacy-settings-for-all-teams) を参照してください。
...
### コードを自動取得する {#automatically-capture-code}
...
## ローカルの変更を表示する {#view-local-changes}
...
## Jupyter セッション履歴 {#jupyter-session-history}
...
## Runs 間でコードを比較 {#compare-code-across-runs}
