---
title: コードの保存と差分
description: "コードの保存を有効にし、コード比較を使って W&B の run 間でコードを比較し、Jupyter セッション履歴を記録します。"
---
...
コードの保存をプログラムで設定するだけでなく、W&B アカウントの Settings でこの機能を切り替えることもできます。なお、これを有効にすると、あなたのアカウントに関連付けられているすべての Teams でコードの保存が有効になります。

> デフォルトでは、W&B はすべての Teams でコードの保存を無効にしています。
