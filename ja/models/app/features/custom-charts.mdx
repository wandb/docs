---
title: カスタム チャートの概要
description: Vega の可視化機能を使って、W&B プロジェクトでカスタム チャートを作成する
keywords: ["Vega", "wandb.plot_table", "適合率-再現率曲線", "ROC 曲線", "GraphQL クエリ"]
---
...
## スクリプトからチャートをログする {#log-charts-from-a-script}
...
<AriaChatBubbles prompt="表示中のRunsについて、検証 accuracy とトレーニング損失の散布図を作成し、点を学習率ごとに色分けしてください。" response="カスタム チャート パネルを追加し、X-axis に検証 accuracy、Y 軸にトレーニング損失をプロットして、config 内の各 run の学習率に応じて点を色分けします。" />

### GraphQL クエリを作成する {#build-the-graphql-query}
...
<Frame>
  <img src="/images/app_ui/customize_chart.gif" alt="カスタム チャートの作成" max-width="90%" />
</Frame>

### カスタム可視化 {#custom-visualizations}
...
### チャートプリセットを保存する {#save-chart-presets}
...
## Reports とガイド {#reports-and-guides}
