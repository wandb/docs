...
以下の例では、既に設定ファイルとPythonスクリプトまたはJupyterノートブックで定義されたトレーニングループがあることを前提としています。設定ファイルの定義方法についての詳細は、[Define sweep configuration](/ja/models/sweeps/define-sweep-configuration/) を参照してください。
...
スイープを初期化した後、[`wandb controller`](/ja/models/ref/python/controller) を使用してコントローラを開始します：
...
ローカルコントローラを使用することを指定した後、スイープを実行するために1つ以上のSweepエージェントを開始します。通常のW&Bスイープを開始するのと同じようにしてください。[Start sweep agents](/ja/models/sweeps/start-sweep-agents/) を参照してください。
...
Python SDKでコントローラを使用する最も簡単な方法は、[`wandb.controller`](/ja/models/ref/python/controller) メソッドにスイープIDを渡すことです。次に、戻りオブジェクトの `run` メソッドを使用してスイープジョブを開始します：
