...
## 終了ステータスとシグナル {#exit-status-and-signals}
...
## 応答のないRunsとサーバー側のタイムアウト {#stale-runs-and-server-side-timeouts}
...
## 捕捉可能なシグナルとプリエンプション {#catchable-signals-and-preemption}
...
## `SIGKILL` (捕捉不可) {#sigkill-uncatchable}
...
## エージェントから子プロセスへのシグナル転送 {#signal-forwarding-from-agent-to-child}
...
## SLURM のようなプリエンプト可能なクラスター {#preemptible-clusters-like-slurm}
...
## エージェントへのシグナルと run へのシグナル {#signals-to-the-agent-versus-signals-to-the-run}
...
## 参照: `mark_preempting()` と最終的な run の状態 {#reference-mark_preempting-and-final-run-state}
