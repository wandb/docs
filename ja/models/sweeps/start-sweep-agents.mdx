...
<Tabs>
  <Tab title="CLI">
    ...
    エージェントを中断したとき (たとえば Ctrl+C) に正常にシャットダウンするには、`wandb agent --forward-signals sweep_id` を使用して、現在の run がシグナルを受け取り正常にシャットダウンできるようにします。詳細は [Signal handling and sweep runs](/ja/models/sweeps/signal-handling-sweep-runs) を参照してください。
  </Tab>
  ...
  <Tab title="Python script or notebook">
    ...
    エージェントからトレーニング run へのシグナル転送は、CLI (`wandb agent --forward-signals`) を使用する場合にのみサポートされます。Python の `wandb.agent()` では、トレーニング関数が子プロセスではなくスレッドで実行されるため、サポートされていません。詳細は [Signal handling and sweep runs](/ja/models/sweeps/signal-handling-sweep-runs) を参照してください。
