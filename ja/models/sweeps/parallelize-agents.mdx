...
<Tabs>
  <Tab title="CLI">
    ...
    2. 以下の코드 스니펫をコピー＆ペーストし、`sweep_id` を自分の sweep ID に置き換えます:
  ...
  <Tab title="Jupyter Notebook">
    ...
    2. 複数の Jupyter Notebook 上で同じ W&amp;B Sweep ID をコピー＆ペーストすることで、W&amp;B Sweep を並列化します。たとえば、`sweep_id` という変数に sweep ID が格納されており、関数名が `function_name` の場合は、以下の코드 스니펫を複数の Jupyter Notebook に貼り付けることで sweep を並列化できます:
...
CUDA Toolkit を使用して、ターミナルから複数の GPU にわたって W&amp;B Sweep エージェントを並列化するには、次の手順に従います。

1. ローカルマシンで複数のターミナル ウィンドウを開きます。
2. W&amp;B Sweep ジョブ ([`wandb agent`](/ja/models/ref/cli/wandb-agent)) を開始するときに、`CUDA_VISIBLE_DEVICES` を使って使用する GPU インスタンスを指定します。使用する GPU インスタンスに対応する整数値を `CUDA_VISIBLE_DEVICES` に割り当てます。
...
2 つ目のターミナル ウィンドウを開きます。`CUDA_VISIBLE_DEVICES` を `1` (`CUDA_VISIBLE_DEVICES=1`) に設定します。次の코드 스니펫に記載されている `sweep_ID` に、同じ W&amp;B Sweep ID を貼り付けます。
