...
[sweep 設定で使用できる、あらかじめ定義されたマクロの一覧については、[sweep 設定オプション](./sweep-config-keys) の [コマンドマクロ](./sweep-config-keys#command-macros) セクションを参照してください。](./sweep-config-keys#command-macros)
