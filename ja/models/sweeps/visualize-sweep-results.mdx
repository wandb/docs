...
sweep リストには、各 sweep の状態 (**State**) 、作成時刻 (**Created**) 、開始したユーザー (**Creator**) 、完了した run 数 (**Run count**) 、および合計の **Compute time** が表示されます。離散的な探索空間に対するグリッドサーチでは、W&amp;B は **Est. Runs** (予想される run 数) も表示します。リストから sweep を開くと、アプリから一時停止、再開、停止、または強制終了できます。CLI で同じ操作を行うには、[Manage sweeps](/ja/models/sweeps/pause-resume-and-cancel-sweeps/) を参照してください。
