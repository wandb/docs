...
## アーティファクトのガベージコレクションのワークフロー {#artifact-garbage-collection-workflow}
...
## アーティファクトバージョンを削除する {#delete-an-artifact-version}
...
## 複数のアーティファクトバージョンを削除する {#delete-multiple-artifact-versions}
...
## 特定の エイリアス を持つ複数の アーティファクトバージョン を削除する {#delete-multiple-artifact-versions-with-a-specific-alias}
...
## アーティファクト コレクション を削除する {#delete-an-artifact-collection}
...
## 保護されたエイリアスと削除権限 {#protected-aliases-and-deletion-permissions}
...
## W&amp;B のホスティング形態に応じてガベージコレクションを有効にする {#enable-garbage-collection-based-on-how-wb-is-hosted}
