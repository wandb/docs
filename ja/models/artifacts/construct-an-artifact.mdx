...
## アーティファクト を作成する {#construct-an-artifact}
...
### `wandb.Artifact()` を使って アーティファクト Python オブジェクトを作成する {#create-an-artifact-python-object-with-wandbartifact}
...
次の コードスニペット をコピー＆ペーストして アーティファクト オブジェクトを作成します。`"artifact-name"` と `"artifact-type"` のプレースホルダーを自分の値に置き換えてください。
...
<Info>
  Artifacts は、タイプが異なっていても同じ名前を共有することはできません。たとえば、`dataset` タイプの `cats` という名前の アーティファクト と、`model` タイプの `cats` という名前の アーティファクト を作成することはできません。
</Info>

アーティファクト オブジェクトを初期化する際に、必要に応じて説明、メタデータ、ダイジェスト アルゴリズム、その他のパラメーターを指定することもできます。

たとえば、次の コードスニペット は、名前、タイプ、説明、メタデータ、ダイジェスト アルゴリズムを指定して アーティファクト オブジェクトを作成します。
...
<Note>
  デフォルトの ダイジェスト アルゴリズムは引き続き MD5 です。W&amp;B Python SDK 0.29.0 以降では、`digest_algorithm="XXH128"` を設定すると、新しい アーティファクト でより高速なハッシュ処理を利用できます。MD5 を使用している既存の アーティファクト コレクション は引き続き MD5 を使用します。0.29.0 より前の SDK バージョンでは、XXH128 のエントリを含む アーティファクト を `Artifact.verify()` で検証できません。
</Note>

使用可能な属性とパラメーターの詳細については、Python SDK Reference Guide の [`wandb.Artifact`](/ja/models/ref/python/experiments/artifact) クラス定義を参照してください。

### アーティファクト に 1 つ以上のファイルを追加する {#add-one-or-more-files-to-the-artifact}
...
### アーティファクトをW&amp;B Serverに保存する {#save-your-artifact-to-the-wb-server}
...
## アーティファクト にファイルを追加する {#add-files-to-an-artifact}
...
次の表は、異なる API Call によって アーティファクト に格納される内容がどのように変わるかを示しています。
...
### 複数のファイルを追加する {#add-multiple-files}
...
### URI referenceを追加する {#add-a-uri-reference}
...
### 並列 run からアーティファクトにファイルを追加する {#add-files-to-artifacts-from-parallel-runs}
...
## ログ済みの アーティファクト やその他のメタデータのパスを検索する {#find-path-for-logged-artifacts-and-other-metadata}
