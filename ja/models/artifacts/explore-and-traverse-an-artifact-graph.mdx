...
W&B は、_リネージ グラフ_ と呼ばれる有向非巡回グラフ（DAG）を使用して、run の入力と出力を追跡します。リネージ グラフは、ML 実験における artifacts と run の関係を視覚的に表現したものです。生データの取り込みからモデルのトレーニングや評価に至るまで、データやモデルが ML ライフサイクルのさまざまな段階をどのように流れていくかを示します。
