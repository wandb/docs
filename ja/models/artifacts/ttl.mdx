---
description: "W&B アーティファクトに time-to-live (TTL) ポリシーを設定し、自動削除をスケジュールして、ストレージ消費を管理します。"
...
Python SDK または W&B App UI を使用して、artifact の TTL ポリシーを表示します。

<Tabs>
<Tab title="Python SDK">
`print` 文を使用して、artifact の TTL ポリシーを表示します。次の例は、artifact を取得してその TTL ポリシーを表示する方法を示しています。
...
<Tab title="W&B App">
W&B App UI を使用して artifact の TTL ポリシーを表示します。
...
コレクションビューでは、選択したコレクション内のすべての artifact を確認できます。`Time to Live` 列には、その artifact に割り当てられている TTL ポリシーが表示されます。
