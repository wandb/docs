...
次の コードスニペット は、artifact を作成して TTL ポリシーを設定する方法を示しています。
...
<Note>artifact の TTL を変更しても、artifact が期限切れになるまでの時間は、artifact の `createdAt` timestamp に基づいて計算されます。</Note>
...
<Tabs>
  <Tab title="Python SDK">
    ...
    次の コードスニペット は、artifact に TTL ポリシーを設定する方法を示しています。
