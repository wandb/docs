...
1. [W&B run](/ja/models/runs/) を初期化し、追跡したいハイパーパラメーターを渡します。
...
W&B run では自動で [メトリクス](/ja/models/runs/#workspace-tab)、システム情報、[ハイパーパラメーター](/ja/models/runs/#overview-tab)、[ターミナル出力](/ja/models/runs/#logs-tab) をログし、モデルの入力と出力を含む [インタラクティブテーブル](/ja/models/tables/) が表示されます。
...
W&B は、保存したモデルやモデルのチェックポイントをチームや組織の他のメンバーと容易に共有することも可能です。チーム外のメンバーとモデルやモデルのチェックポイントを共有する方法は、[W&B Registry](/ja/models/core/registry/) をご覧ください。
...
[W&B アラート](/ja/models/runs/alert/) を作成して、Python コードからあなたの Slack やメールにアラートを送信します。
...
[W&B アラートの完全なドキュメントはこちらで見つけることができます](/ja/models/runs/alert/)。
