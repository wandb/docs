...
ARIA は学習した内容を記憶します。作業中、ARIA は project にスコープされた&#95;メモリ&#95;を書き込みます。これにより、新しいセッションごとに setup、結論、設定を最初から確認し直す必要がなくなります。チームが対数スケールのチャートを好むことを一度伝えれば、その内容を覚えておきます。

各メモリには、ARIA がいつ呼び出すかを判断するための名と説明に加え、メモリ本文が含まれます。

project にアクセスできるユーザーは、メモリの表示、編集、削除を行えます。

1. ARIA のチャットウィンドウで、**<Icon icon="bars" iconType="solid" />** をクリックします。
2. **Customization** セクションで、**Project memory** をクリックします。ARIA が project について記憶しているメモリがすべて表示されます。
...
<AriaChatBubbles prompt="チャートにはセマンティックカラーを使用することを覚えておいてください。" response="まず既存の project メモリを確認し、この設定が重複せず適切なトピックとして記録されるようにします... 新しい project メモリを保存しました: chart-coloring。チャートでは任意のパレット順ではなく、系列やカテゴリの意味、またはステータスに基づく一貫性のあるセマンティックカラーを使用する必要があることを記録しています。 " />
