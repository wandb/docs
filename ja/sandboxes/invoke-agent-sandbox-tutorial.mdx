---
title: "チュートリアル: サンドボックスでエージェントを呼び出す"
description: "Serverless サンドボックス 環境内で OpenAI エージェントを呼び出す方法を学びます"
---

<Warning>
  Serverless Sandboxes は現在プライベートプレビュー中で、招待制でのみご利用いただけます。利用を希望する場合は、[サポート](mailto:support@wandb.com) または担当の AISE にお問い合わせください。
</Warning>

このチュートリアルでは、Serverless サンドボックス 環境内でエージェントを呼び出します。そのために、適切な環境変数を設定して サンドボックス を起動し、必要な依存関係をインストールしたうえで、ツール呼び出しを使用して特定の場所の天気を取得し、しゃれの効いた天気予報を返すシンプルな OpenAI エージェントを作成して呼び出す Python スクリプトを実行します。
