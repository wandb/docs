...
description: "このステップごとのチュートリアルでは、Serverless Sandbox 環境で PyTorch モデルをトレーニングする方法を学びます。"
---

<Warning>
  Serverless Sandboxes は現在プライベートプレビュー中で、招待制でのみご利用いただけます。利用登録をリクエストするには、[サポート](mailto:support@wandb.com) または担当の AISE にお問い合わせください。
</Warning>

このチュートリアルでは、Serverless Sandbox 環境で PyTorch モデルをトレーニングします。これを行うには、適切な環境変数を設定して サンドボックス を起動し、必要な依存関係をインストールして、UCI Zoo データセットでシンプルなニューラルネットワークをトレーニングする Python スクリプトを実行します。
...
このチュートリアルに必要なコードにアクセスするには、以下のドロップダウンを展開してください。コードをコピー＆ペーストして、このチュートリアルと同じディレクトリ内にある 3 つの別々のファイルに保存します。次のセクションでは、これらのファイルを読み込み、Serverless Sandbox 環境内で PyTorch モデルをトレーニングするスクリプトを実行します。
