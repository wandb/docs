---
title: ファイル操作
description: Serverless Sandboxes でファイルを読み込み、書き込み、マウントする方法を学びます。
keywords: ["Sandbox.write_file", "Sandbox.read_file", "ファイルのマウント", "サンドボックスへのアップロード"]
---
...
ファイル操作を使用すると、ローカル環境とサンドボックスの間でデータを共有できます。サンドボックスからファイルを読み込んだり、サンドボックスにファイルを書き込んだり、ローカルのファイルやディレクトリをサンドボックスにマウントしたりできます。
...
<Tip>
  **適切なファイルアクセス方法を選択してください**
  ...
  ローカル環境とサンドボックスの間で小さなファイルを転送する場合や、サンドボックスの出力をローカルに保存する場合は、ファイルの読み込みと書き込みを使用します。
</Tip>

## サンドボックスにファイルを書き込む {#write-a-file-to-the-sandbox}
...
## サンドボックス内のファイルを読み込む {#read-a-file-from-the-sandbox}
...
## ファイルまたはディレクトリをマウントする {#mount-a-file-or-directory}
