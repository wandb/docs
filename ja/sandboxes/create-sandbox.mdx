...
description: 独自のファイルシステム、ネットワーク、プロセス空間を備えた分離コンテナー内でコードを実行する W&B Serverless Sandboxes を作成します。
---
...
分離環境でコードを実行するための W&amp;B Serverless Sandboxes を作成します。単一のサンドボックスを起動してその中でコマンドを実行し、セッション を使って複数のサンドボックスを管理できます。各サンドボックスは、独自の[ファイルシステム](/ja/sandboxes/file-access)、ネットワーク、プロセス空間を持つ専用のコンテナー内で実行されます。
...
<Info>
  ...
  使用する前に、イメージとタグがパブリックレジストリに存在することを確認してください。サンドボックスがイメージをプルできない場合、起動せず、クライアントは `SandboxFailedError` を発生させます。

  <Info>
    `SandboxFailedError` は複数の理由で発生する可能性があり、利用できないイメージを特定するものではありません。このエラーが発生した場合は、イメージとタグが利用可能であることを確認してください。
  </Info>

  次のコードスニペットは、`python:3.14` イメージでサンドボックスを作成し、その中で `python --version` を実行します。
...
### メイン コマンドを指定せずにサンドボックスを開始する {#start-a-sandbox-without-a-main-command}
...
### メインコマンドを指定して サンドボックス を開始する {#start-a-sandbox-with-a-main-command}

`Sandbox.run()` にコマンドを渡すこともできます。サンドボックス が 1 つのジョブを開始から終了まで実行する場合は、このパターンを使用してください。メインプロセスが終了すると、サンドボックス は [COMPLETED or FAILED](/ja/sandboxes/lifecycle) などの終了状態に入ります。
...
## セッションで複数のサンドボックスを作成する {#create-multiple-sandboxes-with-a-session}

同時に複数のサンドボックスが必要な場合や、複数のサンドボックスで設定を共有したい場合は、セッションを使用します。
...
次のコードスニペットは、デフォルト設定 (`SandboxDefaults`) を共有する 2 つのサンドボックスを持つセッションを作成します。
