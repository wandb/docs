...
ドリフトが成立するには、*ずれる元となる*ドキュメントが必要です。どのページも言及していないラベルの名称変更は、何も不正確にしません。したがってそれは findings ではなく、せいぜい統計値にすぎません。そうしたものは件数を数えて出力するだけにしてください。個別に列挙すると、本当に重要な行が埋もれてしまいます。これはこの report にとって唯一許されない失敗です。本物の findings を読み飛ばしたレビュー担当者は、二度と戻ってきません。

これは一方向ルールが禁じている抑制ではありません。ドキュメントが存在しないことによって*実在する findings が隠される*ことは決してあってはなりませんが、同時に*存在しない findings を作り出す*こともあってはならないのです。

同じ検討から、関連する 2 つのパターンが見えてきました。

* **surface ごとに新規コピーを集約する。** 新しい設定パネルには、見出し、説明、2 つのフィールドラベル、ボタンが追加されます。これは 5 つではなく 1 つのドキュメントタスクです。
* **findings のキーは、コードの surface ではなくドキュメントタスクに置く。** 3 つのメンバー表が同じ列を表示していても、ドキュメントページ側でその名前が登場するのは 1 回だけです。findings の id に surface を含めていたために、1 件の編集が 3 行として表示されてしまいました。
...
レビュアーと担当チームは、一見すると検出項目ごとのルックアップに見えますが、実際はそうではありません。CODEOWNERS は run の途中で変わらない 1 つの file であり、作成者情報は UI ルートに対する 1 回の `git log --name-only` から取得し、パス → 作成者のインデックスとしてメモリ上に展開します。素朴な実装 — CODEOWNERS のための `git show` に加えて、検出項目ごとに 1〜2 回の `git log` 呼び出し — では、スキャン全体で同一のデータのために 1 行あたりおよそ 3 つのサブプロセスを起動することになります。
