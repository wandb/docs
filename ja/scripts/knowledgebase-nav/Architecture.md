...
* **`parse_frontmatter`**、**`_extract_body`** は、YAML フロントマターと本文を分割します。`_extract_body` は `_BADGE_START` を境界として使用し、見た目を整えるために末尾の `---` 行を削除します。
...
* **`_tab_badge_pattern`**、**`build_tab_badges_mdx`**、**`build_keyword_footer_mdx`**、**`_replace_tab_badges_in_body`** は、tab Badge のピンポイントな Sync を実装します。管理対象の Badges は `_BADGE_START` / `_BADGE_END` のマーカーコメントで囲まれます。この関数は、マーカーがある場合はそれにマッチし、マーカー導入前の記事では正規表現にフォールバックします。新しいフッターには、空行、マーカー、Badges が追加されます。
...
* **`update_support_index`** は、ルートの `support.mdx` にあるプロダクトCardの件数行を更新します。`{/* auto-generated counts */}` マーカーを優先し、移行時は正規表現にフォールバックします。
* **`update_support_featured`** は、ルートの `support.mdx` 内で `_FEATURED_START` / `_FEATURED_END` マーカーに挟まれた注目記事セクションを再生成します。
...
* **`BODY_PREVIEW_MAX_LENGTH`** と **`BODY_PREVIEW_SUFFIX`** は、Cardプレビューの長さと省略記号を制御します。
* **`DOCS_JSON_NAV_LANGUAGE`** は `"en"` で、navigation の編集範囲を英語ツリーのみに限定します。
* **`_BADGE_START`** / **`_BADGE_END`** は、各記事ページで管理対象のタブBadgeを囲む MDX コメントマーカーです。
* **`_FEATURED_START`** / **`_FEATURED_END`** は、ルートの `support.mdx` にある注目記事セクションを囲む MDX コメントマーカーです。
...
* **Tab Badge の管理範囲**: `/support/<product>/tags/...` にリンクする `<Badge>` 要素だけが `keywords` から導出されます。これらはマーカーコメントで囲まれているため、移行後はジェネレーターで正規表現によるマッチングを行う必要がありません。本文とBadgeの間にある `---` 行は見た目のためのもので、`_extract_body` は境界として `_BADGE_START` を使用し、末尾の `---` はクリーンアップとしてのみ削除します。
* **古いタグのクリーンアップ**: どの記事キーワードにも対応しなくなったタグページは、`docs.json` の更新前に、生成後の処理として削除されます。これにより、tags ディレクトリとナビゲーションに孤立したエントリが残りません。
* **マーカーベースの編集**: 自動生成されるすべてのセクション (記事タブの Badges、`support.mdx` の件数行、注目の記事) では、MDX コメントマーカーを使用します。これにより、管理対象の領域が執筆者にも分かりやすくなり、壊れやすい正規表現アンカーに頼らずにジェネレーターが内容を正確に置き換えられます。各マーカーペアには、初回実行時に素のコンテンツを囲む移行パスがあります。
