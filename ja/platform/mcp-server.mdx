...
keywords: ["MCP ツール", "Cursor MCP", "Claude MCP", "AI アシスタント インテグレーション", "自然言語クエリ"]
---
...
<CardGroup cols={2}>
  <Card title="ホスト型サーバー（推奨）">
    W&amp;B が管理するMCP サーバーです。クライアントは W&amp;B APIキー を使用して HTTP 経由で接続します。インストールは不要で、維持するローカルプロセスもありません。

    [ホスト型サーバーを使用する](#use-the-hosted-server)
  </Card>

  <Card title="ローカルインストール">
    MCP サーバー をご自身のマシン上で STDIO または HTTP 経由で実行します。外部ネットワークから隔離された運用、特定のリリースへの固定、カスタムのサーバー動作、サーバーのアクティブな開発、または STDIO のみに対応するクライアントのサポートが必要な場合に使用します。
...
* そのキーを `WANDB_API_KEY` 環境変数に設定するか、bearer tokenとしてクライアントに渡します。
* 専用クラウド、セルフマネージド、またはデフォルト以外のインスタンスを対象とするローカルインストールでは、`WANDB_BASE_URL` 環境変数にインスタンスの URL を設定します。
* W&amp;B は `mcp` SDK をリリース `2024-11-05` の [1.14.0](https://pypi.org/project/mcp/1.14.0/) に固定しています。クライアントは `mcp` SDK 1.14.x を使用して接続する必要があります。W&amp;B Dedicated Cloud でストリーミング可能な HTTP をサポートするには、リリース `2025-03-26` 以降の `mcp` SDK 1.14.x が必要です。
...
### サーバーをインストールする {#install-the-server}
...
### クライアントを設定する {#configure-your-client}
...
### HTTP トランスポートでサーバーを実行する {#run-the-server-with-http-transport}
...
## W&amp;B MCP Server の機能 {#wb-mcp-server-capabilities}
...
### スキーマファーストのトレースクエリ {#schema-first-trace-queries}
...
### 一般的なベスト プラクティス {#general-best-practices}
...
### トレースが多いワークフロー向け {#for-trace-heavy-workflows}
...
### run が多いワークフロー向け {#for-run-heavy-workflows}
...
### 専用クラウドおよびセルフマネージド向け {#for-dedicated-cloud-and-self-managed}
...
### ローカル環境にインストールする場合 {#for-local-installs}
...
### 不案内な project を調べる {#explore-an-unfamiliar-project}
...
### 失敗した LLM calls をトリアージする {#triage-failing-llm-calls}
...
### 不調なトレーニング run を診断する {#diagnose-a-bad-training-run}
...
### 評価を要約してモデルバージョンを比較する {#summarize-evals-and-compare-model-versions}
