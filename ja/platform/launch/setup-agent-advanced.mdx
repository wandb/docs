...
description: "Docker および Kaniko ビルダー、コンテナーレジストリ設定など、W&B Launch の高度なエージェントオプションを設定します。"
keywords: ["Kaniko builder", "ECR push", "GCR registry", "agent builder", "イメージレジストリ"]
---

# 高度なエージェント設定 {#advanced-agent-setup}
...
## コンテナーレジストリへのプッシュ {#push-to-a-container-registry}
...
### Kaniko のストレージ権限 {#storage-permissions-for-kaniko}
...
## Kaniko ビルドをカスタマイズする {#customize-the-kaniko-build}
...
## Launch エージェントを CoreWeave にデプロイする {#deploy-launch-agent-into-coreweave}
