...
keywords: ["SageMaker トレーニング", "CreateTrainingJob", "AWS Launch", "ECR Registry", "SageMaker キュー"]
---
...
### Launch エージェントに Docker イメージ をビルドさせるかどうかを決める {#decide-if-you-want-the-launch-agent-to-build-a-docker-image}
...
### AWS リソースをセットアップする {#set-up-aws-resources}
...
### Launch エージェント用の IAM ポリシーを作成する {#create-an-iam-policy-for-the-launch-agent}
...
### Launch エージェント用の IAM ロールを作成する {#create-an-iam-role-for-the-launch-agent}
...
## SageMaker 用に Launch キューを設定する {#configure-the-launch-queue-for-sagemaker}
...
## Launch エージェントを設定する {#set-up-the-launch-agent}
...
### Launch エージェント をどこで実行するかを決める {#decide-where-to-run-the-launch-agent}
...
### Launch エージェントを設定する {#configure-a-launch-agent}
...
## 任意: Launch ジョブ の Docker イメージを Amazon ECR にプッシュする {#optional-push-your-launch-job-docker-image-to-amazon-ecr}

<Note>
  このセクションは、Launch エージェントがトレーニングまたは推論のロジックを含む既存の Docker イメージを使用する場合にのみ該当します。[Launch エージェントは 2 つの動作オプションをサポートします。](#decide-if-you-want-the-launch-agent-to-build-a-docker-image)
</Note>
