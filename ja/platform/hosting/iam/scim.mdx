...
SCIM の user リソースは、W&amp;B Users とサービスアカウントに対応します。これらの endpoints を使用して、組織内のユーザーとサービスアカウントを管理します。
...
組織内に、チームスコープまたは組織スコープのサービスアカウントを作成します。通常のユーザーを作成する場合は、`accountType` を省略します。[Create user](#create-user) を参照してください。
...
レスポンスでこれらのいずれかのエラーが返された場合は、リクエストに次のような一般的な問題がないか確認してください。
...
サービスアカウントと、その組織へのメンバーシップを完全に削除します。これはハード削除であり、SCIM を介してアカウントを再有効化することはできません。
...
| サービスアカウントをプロビジョニング (`accountType` を指定した `POST`)  | ✓                  | ✓ (v0.81.0+)    |
| サービスアカウントのプロビジョニング解除 (ID を指定した `DELETE`)         | ✓                  | ✓ (v0.81.0+)    |
