...
[User SCIM API](/ja/platform/hosting/iam/automate_iam/#user-scim-api) を使用すると、ユーザーの作成、無効化、詳細の取得、または W&B 組織内の全ユーザーの一覧を表示できます。この API は、組織内のユーザーに事前定義されたロールまたはカスタムロールを割り当てることもサポートしています。

<Note>
`DELETE User` エンドポイントを使用して、W&B 組織内のユーザーを無効化します。無効化されたユーザーはサインインできなくなります。ただし、無効化されたユーザーは引き続き組織のユーザーリストに表示されます。

無効化されたユーザーをユーザーリストから完全に削除するには、 [ユーザーを組織から削除](/ja/platform/hosting/iam/access-management/manage-organization/#remove-a-user) する必要があります。
...
[Group SCIM API](/ja/platform/hosting/iam/automate_iam/#group-scim-api) は、組織内での W&B チームの作成や削除を含めた管理を行うことができます。 `PATCH Group` を使用して、既存のチームにユーザーを追加または削除します。
...
[Custom role SCIM API](/ja/platform/hosting/iam/scim/#role-resource) は、組織内でのカスタムロールの作成、一覧表示、更新を含めた管理を行うことができます。
...
SCIM API がユーザーとチームの管理を自動化するのと同様に、一部のメソッドを使用して [W&B Python SDK API](/ja/models/ref/python/public-api/api) も同様の目的で使用できます。以下のメソッドに注意してください：
