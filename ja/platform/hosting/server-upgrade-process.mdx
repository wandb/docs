...
<Warning>
`wandb` Helm チャートは非推奨となり、サポートが終了しました。このチャートは単一の Pod をデプロイするものであり、[W&B Kubernetes Operator](/platform/hosting/self-managed/operator) に置き換えられています。このチャートを使用している場合は、[移行ガイド](/platform/hosting/self-managed/operator#migrate-self-managed-instances-to-wb-operator) に従って Operator に移行してください。
</Warning>
