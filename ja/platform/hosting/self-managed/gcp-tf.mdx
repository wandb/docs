...
<Note>
W&Bでは、[W&B Multi-tenant Cloud](/ja/platform/hosting/hosting-options/multi_tenant_cloud) や [W&B Dedicated Cloud](/ja/platform/hosting/hosting-options/dedicated_cloud/) デプロイメントタイプのような、フルマネージドなデプロイオプションを推奨しています。W&Bのフルマネージドサービスはシンプルかつ安全に利用でき、設定は最小限、あるいは全く必要ありません。
</Note>
