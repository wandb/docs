...
<Note>
W&B は、[W&B Multi-tenant Cloud](/ja/platform/hosting/hosting-options/saas_cloud) または [W&B 専用クラウド](/ja/platform/hosting/hosting-options/dedicated_cloud/) のデプロイメントタイプなどの完全管理されたデプロイメントオプションを推奨しています。W&B の完全管理サービスはシンプルで安全に使用でき、設定はほとんど必要ありません。
</Note>
