...
<Note>
W&Bでは、[W&B Multi-tenant Cloud](/ja/platform/hosting/hosting-options/multi_tenant_cloud) や [W&B Dedicated Cloud](/ja/platform/hosting/hosting-options/dedicated_cloud/) などのフルマネージドなデプロイメントオプションを推奨しています。W&Bのフルマネージドサービスは、設定をほとんど、あるいは全く必要とせず、シンプルかつ安全に使用できます。
</Note>
...
<Note>
Kubernetes [オペレーター](https://kubernetes.io/docs/concepts/extend-kubernetes/operator/) パターンへの移行は、W&B アーキテクチャーにとって必要です。詳細な説明については、[アーキテクチャー移行の理由](/ja/platform/hosting/operator/#reasons-for-the-architecture-shift) を参照してください。
</Note>
