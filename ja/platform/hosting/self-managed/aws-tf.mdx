...
<Note>
W&B は [W&B マルチテナントクラウド](/ja/platform/hosting/hosting-options/saas_cloud) または [W&B 専用クラウド](/ja/platform/hosting/hosting-options/dedicated_cloud/) デプロイメントタイプのようなフルマネージドデプロイメントオプションを推奨しています。 W&B のフルマネージドサービスはシンプルで安全に利用でき、設定が最小限または不要です。
</Note>
