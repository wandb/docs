...
<Note>
W&B は、[W&B Multi-tenant Cloud](/ja/platform/hosting/hosting-options/saas_cloud) または [W&B Dedicated Cloud](/ja/platform/hosting/hosting-options/dedicated_cloud/) デプロイメント タイプのような完全に管理されたデプロイメント オプションをお勧めします。W&B の完全管理サービスは簡単で安全に使用でき、設定がほとんどまたは全く必要ありません。
</Note>
