...
ネットワークやロードバランサーの設定を含むインフラストラクチャーの完全な要件については、[リファレンスアーキテクチャー](/ja/platform/hosting/self-managed/ref-arch/#infrastructure-requirements)を参照してください。

<Note>
W&B はエアギャップ環境の OpenShift Kubernetes クラスターにデプロイ可能です。詳細は [リファレンスアーキテクチャー](/ja/platform/hosting/self-managed/ref-arch/#kubernetes) を参照し、エアギャップ環境の OpenShift デプロイに適応可能な具体的な設定手順については、Operator ガイドの [OpenShift セクション](/ja/platform/hosting/operator/#openshift-kubernetes-clusters) を確認してください。
</Note>
...
MySQL の設定パラメータについては、[リファレンスアーキテクチャーの MySQL 設定セクション](/ja/platform/hosting/self-managed/ref-arch/#mysql-configuration-parameters)を参照してください。
