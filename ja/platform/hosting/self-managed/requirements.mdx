...
keywords: ["ハードウェア要件", "Kubernetes バージョン", "MySQL バージョン", "CPU とメモリの要件", "Redis 要件"]
---
...
## ソフトウェアのバージョン要件 {#software-version-requirements}
...
### MySQL 設定パラメーター {#mysql-configuration-parameters}
...
セルフマネージドのデプロイでは、W&amp;B Weave で ClickHouse を使用します。ClickHouse は、すべてのセルフマネージド環境で必須ではありません。

W&amp;B Server をアップグレードする前、または ClickHouse のバージョンを変更する前に、Operator ガイドの [アップグレード時の ClickHouse 互換性](/ja/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) と、[サポートされる W&amp;B Server リリース](/ja/release-notes/server-releases) の互換性に関するメモを参照してください。

デプロイ手順については、[セルフマネージド環境で Weave をデプロイする](/ja/weave/guides/platform/weave-self-managed) を参照してください。
...
### W&amp;B でバケットを使用するように設定する {#configure-wb-to-use-your-bucket}
...
### ロードバランサーと ingress {#load-balancer-and-ingress}
