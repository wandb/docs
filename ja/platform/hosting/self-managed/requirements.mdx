...
セルフマネージドのデプロイでは、Weave と一部の Models の OLAP 機能で ClickHouse を使用します。ClickHouse は、すべてのセルフマネージド環境で必須ではありません。

必要な ClickHouse の最小バージョンは、有効にする W&amp;B 機能と実行中の W&amp;B Server のバージョンによって異なります。W&amp;B Server をアップグレードする前、または ClickHouse のバージョンを変更する前に、Operator ガイドの [アップグレード時の ClickHouse 互換性](/ja/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) と、[サポートされる W&amp;B Server リリース](/ja/release-notes/server-releases) の互換性に関するメモを参照してください。
