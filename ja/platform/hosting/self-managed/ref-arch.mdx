...
keywords: ["インフラストラクチャーのサイジング", "MySQL の要件", "Redis のセットアップ", "本番アーキテクチャ", "ハードウェア仕様"]
---
...
## インフラストラクチャー要件 {#infrastructure-requirements}
...
#### MySQL データベースの作成 {#mysql-database-creation}
...
#### MySQL の設定パラメーター {#mysql-configuration-parameters}
...
#### ingress controller の要件 {#ingress-controller-requirements}
...
#### W&amp;B サービスのルーティング {#wb-service-routing}
...
#### ingress の設定例 {#example-ingress-configuration}
...
### サポート対象のCPUアーキテクチャ {#supported-cpu-architectures}
...
### Helm を使用した W&amp;B Kubernetes Operator {#wb-kubernetes-operator-with-helm}
...
### インフラストラクチャーのプロビジョニング {#infrastructure-provisioning}
...
### Kubernetes のサイジング {#kubernetes-sizing}
...
## クラウドプロバイダごとの推奨インスタンス {#cloud-provider-instance-recommendations}
