...
MySQL データベースとユーザーを手動で作成する手順については、[bare-metal ガイドの MySQL データベース セクション](/ja/platform/hosting/self-managed/bare-metal/#mysql-database)を参照してください。
...
W&amp;B Self-Managed の推奨インストール方法は、Helm 経由で **W&amp;B Kubernetes Operator** をデプロイすることです。この方法には、次の利点があります。
...
* [Kubernetes 上で W&amp;B Server を実行する (Operator) ](/ja/platform/hosting/operator/) - メインのインストール ガイド
* [オンプレミス環境に W&amp;B Platform をデプロイする](/ja/platform/hosting/self-managed/bare-metal/) - データセンター / ベアメタル固有の考慮事項について
* [air-gapped インスタンス向け Kubernetes operator](/ja/platform/hosting/self-managed/operator-airgapped/) - 外部接続のない環境向け
