...
自分のバケットを持ち込む (BYOB) を使用することで、W&B Artifacts やその他の機密性の高いデータを、自分のクラウドやオンプレミスのインフラストラクチャーに保存することができます。[専用クラウド](/ja/platform/hosting/hosting-options/dedicated_cloud)または[SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud)の場合、バケットに保存したデータは W&B が管理するインフラストラクチャーにコピーされません。
...
<Note>
チームレベル BYOB 用の S3 互換ストレージへの接続は [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) では利用できません。また、[SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) の場合、チームレベル BYOB 用の AWS バケットへの接続はクロスクラウドで行われます。これは、そのインスタンスが GCP に存在するためです。そのクロスクラウドの接続は、以前説明した専用クラウドやセルフマネージドインスタンスのようにアクセキーおよび環境変数ベースのメカニズムを使用しません。
</Note>
...
1. KMS キーのプロビジョニング
    ...
    [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) または [専用クラウド](/ja/platform/hosting/hosting-options/dedicated_cloud) を使用している場合は、`<aws_principal_and_role_arn>` を次の値で置き換えてください：

    * [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) の場合: `arn:aws:iam::725579432336:role/WandbIntegration`
...
2. S3 バケットのプロビジョニング
    ...
    5. W&B プラットフォームのホスティング先の AWS アカウントに必要な S3 権限を付与します。これにより、AI ワークロードがクラウドインフラストラクチャーやユーザーブラウザーでバケットにアクセスするための[事前署名された URL](/ja/platform/hosting/data-security/presigned-urls/)が生成されます。
        ...
        [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) または [専用クラウド](/ja/platform/hosting/hosting-options/dedicated_cloud) を使用している場合、`<aws_principal_and_role_arn>` を次の値で置き換えてください。

        * [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud): `arn:aws:iam::725579432336:role/WandbIntegration`
...
2. [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) または [専用クラウド](/ja/platform/hosting/hosting-options/dedicated_cloud) を使用している場合、W&B プラットフォームにリンクされた GCP サービスアカウントに `Storage Admin` ロールを付与します：

    * [SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud) の場合、アカウントは: `wandb-integration@wandb-production.iam.gserviceaccount.com`
...
9. ([SaaS クラウド](/ja/platform/hosting/hosting-options/saas_cloud)でオプション) チームの作成時にオプションでチームメンバーを招待します。
10. **チームを作成** ボタンを押します。
