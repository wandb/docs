...
description: W&B がオブジェクトストレージをどのように使用するか、削除がバケット使用量（バイト数）にどのように反映されるか、またストレージの使用量とコストを削減する方法を理解します。
...
## バケット容量を消費するもの {#what-uses-bucket-space}
...
## W&amp;B がストレージからデータを削除する仕組み {#how-wb-removes-data-from-storage}
...
### runデータとrunファイル {#run-data-and-run-files}
...
## バックグラウンド クリーンアップで想定されること {#what-to-expect-from-background-cleanup}
...
## バケット使用量を減らす {#reduce-bucket-usage}
