...
[リファレンスアーキテクチャ](/ja/platform/hosting/self-managed/ref-arch/#infrastructure-requirements)を参照してください。また、[有効な W&B サーバーライセンスを取得](/ja/platform/hosting/hosting-options/self-managed#obtain-your-wb-server-license)します。

セルフマネージドインストールのセットアップと構成方法についての詳細な説明は、こちらの[ガイド](/ja/guides/)を参照してください。
...
エアギャップ環境での W&B Kubernetes オペレーターのインストール方法については、[Deploy W&B in airgapped environment with Kubernetes](/ja/platform/hosting/self-managed/operator-airgapped/) チュートリアルを参照してください。
...
これらのモジュールの使用方法の詳細な説明については、[セルフマネージドインストールセクション](/ja/platform/hosting/hosting-options/self-managed#deploy-wb-server-within-self-managed-cloud-accounts)のドキュメントを参照してください。

### インストールを検証する

インストールを検証するには、W&B は [W&B CLI](/ja/models/ref/cli/) を使用することを推奨しています。検証コマンドは、すべてのコンポーネントと設定を検証するいくつかのテストを実行します。
...
移行プロセスの詳細な説明については、こちら [here](/ja/platform/hosting/self-managed/aws-tf/#migrate-to-operator-based-aws-terraform-modules)を参照してください。
...
ドキュメントには環境変数が2つのリストに分かれています：[basic](/ja/platform/hosting/env-vars/) および [advanced](/ja/platform/hosting/iam/advanced_env_vars/)。必要な設定オプションが Helm Chart を使用して公開されていない場合にのみ環境変数を使用してください。
