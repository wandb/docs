...
title: ユーザー設定の管理
keywords: ["APIキー", "GitHub インテグレーション", "ベータ機能", "アカウント削除"]
---
...
#### ソーシャルバッジを追加する {#add-social-badges}
...
<Note>
  このセクションには、個人用APIキーのみが表示されます。組織のAPIキーやサービスアカウントキーを含め、組織内で管理できるすべてのキーを確認するには、[APIキーを管理する](/ja/platform/hosting/iam/api-keys)で説明されている**APIキー**タブを使用してください。
</Note>

### APIキーを表示する {#view-your-api-keys}

APIキーの表には、次の項目が表示されます。

* **キーID**: 各APIキーの先頭部分で、識別に使用されます。
* **名**: キーに付ける説明用の名前です (省略可)。
* **作成日時**: キーが作成された日時です。
* **最終使用日時**: 直近で使用された日時です。
...
### 新しいAPIキーを作成 {#create-a-new-api-key}

<ApiKeyCreate />

### APIキーをリネームする {#rename-an-api-key}

キーの内容を変更せずに、APIキーの display name を変更するには、次の手順を実行します。

1. APIキーの表で、削除するキーを探します。
2. **アクション (<Icon icon="ellipsis" iconType="solid" />)** メニューをクリックし、**Edit** を選択します。
3. キーの **名** を更新し、**Save** をクリックします。
...
1. APIキーの表で、削除するキーを探します。
2. **アクション (<Icon icon="ellipsis" iconType="solid" />)** メニューをクリックし、**Delete** を選択します。
3. 削除を確認します。
...
## 個人用 GitHub インテグレーション {#personal-github-integration}
...
## アカウントを削除する {#delete-your-account}
...
<Note>
  複数のメンバーが所属する組織で **Billing admin** となっているユーザーアカウントは削除できません。このようなアカウントを削除する前に、[Billing settings](/ja/platform/app/settings-page/billing-settings#billing-admin) ページで Billing admin ロールを別のユーザーに移譲してください。ご自身がその組織の唯一のメンバーである場合、アカウントを削除すると組織も削除されます。
</Note>
