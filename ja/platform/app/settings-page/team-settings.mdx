...
Registry や プロジェクト でイベントが発生したときに、たとえば新しい アーティファクト が作成された場合や run のメトリクスが定義済みのしきい値に達した場合に、チームの [automations](/ja/models/automations/) から通知を送信する Slack 通知の送信先を設定します。[Create a Slack automation](/ja/models/automations/create-automations/slack/) を参照してください。
...
チームの [automations](/ja/models/automations/) が Registry やプロジェクトでイベント発生時に実行されるように webhooks を設定します。たとえば、新しいアーティファクトが作成されたときや、run のメトリクスが定めたしきい値に達したときなどです。詳しくは、[Create a webhook automation](/ja/models/automations/create-automations/slack/) を参照してください。
...
* 将来のプロジェクトを公開したり、レポートを公開で共有したりする機能を無効にできます。
