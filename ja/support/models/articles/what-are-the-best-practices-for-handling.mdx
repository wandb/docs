...
API Callを try-except ブロックで囲みます:
...
* デバッグしやすくするため、Timestamp 付きでエラーをログする
* 並行処理を改善するため、非同期処理を使用する
* 本番システムにはサーキットブレーカーを実装する
* 必要に応じて応答をキャッシュし、API Call を減らす
