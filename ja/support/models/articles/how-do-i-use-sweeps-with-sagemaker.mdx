...
Amazon SageMaker で W&amp;B Sweeps を使用すると、SageMaker のトレーニングジョブでハイパーパラメーター探索を実行できます。組み込みの Amazon SageMaker estimator を使用する際に W&amp;B を認証するには、`requirements.txt` ファイルを作成してください。
