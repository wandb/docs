...
スコアラーの結果（例: スコアラーの出力）を call に追加します。  
命令型の evaluation で、predict call にスコアラー結果を紐づけるために使用します。
...
| :------ | :------ | :------ |
| `predictCallId` | `string` | フィードバックを紐付ける predict 呼び出しの ID |
| `scorerCallId` | `string` | フィードバックを生成した scorer 呼び出しの ID |
| `runnableRefUri` | `string` | scorer（Op または Object ref）の URI |
