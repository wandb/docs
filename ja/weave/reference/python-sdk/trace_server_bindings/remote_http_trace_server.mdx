...
基底の call processor を公開するための、正式な TraceServerInterface で定義されていない独自の methodです。クライアント側のインターフェースで正式に定義する必要があります。
...
内部のフィードバックプロセッサを公開するための、正式な TraceServerInterface では定義されていない独自の method。クライアント側のインターフェースとして正式化する必要があります。
...
`calls/batch_upsert` と同様に、表の更新は一連の更新に分解できるというプロパティがあるため、ペイロード サイズを動的に調整できます。
