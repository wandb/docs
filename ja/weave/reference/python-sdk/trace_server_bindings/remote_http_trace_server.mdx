...
description: "weave.trace_server_bindings.remote_http_trace_server に関する Python SDK リファレンス"
---
...
内部の呼び出しプロセッサーを公開するためのカスタムメソッドであり、正式に定義された TraceServerInterface には含まれていません。クライアント側インターフェースで正式に定義されるべきです。
...
これは、内部のフィードバックプロセッサーにアクセスするためのカスタムメソッドであり、正式な TraceServerInterface には定義されていません。クライアント側のインターフェースとして正式に定義すべきです。
...
実際の行データではなく、行のダイジェストを指定してテーブルを作成します。
...
### <kbd>メソッド</kbd> `table_query_stream`
...
`calls/batch_upsert` と同様に、テーブルの更新が一連の更新に分解できるという特性があるため、ペイロード サイズを動的に調整できます。
...
### <kbd>メソッド</kbd> `threads_query_stream`
