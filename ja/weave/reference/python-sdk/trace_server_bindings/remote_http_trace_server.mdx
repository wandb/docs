...
正式な TraceServerInterface には定義されていないカスタムメソッドで、基盤となる call processor を公開します。クライアント側のインターフェースで正式化されるべきものです。
...
正式な TraceServerInterface には定義されていないカスタムメソッドで、基盤となる feedback processor を公開します。クライアント側のインターフェースで正式化されるべきものです。
...
実際の行の代わりに、行のダイジェストを指定してテーブルを作成します。
...
`calls/batch_upsert` と同様に、テーブルの更新が一連の更新に分解可能であるという特性を利用して、ペイロードサイズを動的に調整できます。
