...
threading.Thread のドロップイン置き換えであり、スレッド内部での呼び出しが期待どおりに動作することを保証します。Weave は特定の contextvars が設定されている必要があります（call&#95;context.py を参照）が、新しいスレッドは親からコンテキストを自動的にはコピーしないため、呼び出しコンテキストが失われてしまうことがあり、これは望ましくありません。このクラスは contextvars のコピーを自動化することで、このスレッドを使えばユーザーがおそらく期待するように「そのまま動作する」ようにします。
...
意味的な役割はありません。複数のスレッドに同じ name を与えることができます。name の初期値はコンストラクタによって設定されます。
...
同じ種類のメッセージを 1 度だけログに記録し、以降の同種メッセージは抑制します。これは、ログを大量の出力で埋め尽くすことなくエラーをユーザーに通知するのに役立ちます。

これは、同じエラーメッセージが何度も発生する可能性があるケースで特に有用です。たとえば、ある op の保存に失敗した場合、その op が呼び出されるたびに同じことが起こる可能性が高くなります。また、パッチを当てたイテレーターにエラーがある場合、その結果を反復処理するたびに同じエラーが発生する可能性があります。この機能により、ログをあふれさせることなく、エラーについてユーザーに通知できます。
