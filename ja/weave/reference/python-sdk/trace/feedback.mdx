...
サーバーからフィードバックを取得するための遅延ロード用オブジェクト。
...
Feedback オブジェクトのコレクションおよび操作用ユーティリティ。
...
これらのフィードバックに関連付けられている ref のうち、一意なものを返します。
...
ref に対してフィードバックを追加します。

feedback&#95;type: フィードバックの種類を識別する文字列。&quot;wandb.&quot; プレフィックスは予約されています。creator: フィードバックの作成者として表示する名前。
