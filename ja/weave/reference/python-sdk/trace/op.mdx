...
@weave.op でデコレートされた関数はすでに Op のインスタンスなので、この関数は実行時には実質 no-op です。ただし、OpDef の属性に型安全に 액세스する必要がある場合に、型チェッカーを満足させるために使用できます。
...
この method は `@weave.op` でデコレートされた任意の関数に自動的にバインドされるため、次のように利用できます。
...
この op へのすべての Call に対するイテレーターを取得します。
...
Op ライクな method または partial から、プレーンな Op 関数へのバインドを外します。

対象:

* method: 設定されている `self` パラメーターを削除します
...
関数や method を Weave の op としてラップするためのデコレーターです。同期処理・非同期処理のどちらにも対応します。イテレータ関数を自動的に検出し、適切な動作を適用します。
...
WeaveKwargs を ops に渡すための &#95;&#95;weave dict を設定します。

**引数:**

* <b>`d`</b>: 更新する既存の WeaveKwargs dict (省略可) 。
