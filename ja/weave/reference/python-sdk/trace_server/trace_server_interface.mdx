...
アノテーションキューに複数の call を一括で追加するためのリクエスト。 

内部 API 向けに queue_id を追加することで AnnotationQueueAddCallsBody を拡張したもの。 
...
内部 API で使用するために、`queue_id` を追加して `AnnotationQueueItemsQueryBody` を拡張したものです。
...
annotation queue アイテムをクエリした際のレスポンス。 
...
特定の annotation キューを読み取るためのリクエスト。 
...
プロジェクトの annotation queue を照会するためのリクエスト。
...
複数のアノテーションキューの統計情報を取得するためのリクエストです。
...
現在のアノテーターのキュー項目のアノテーション状態を更新するためのリクエスト。

有効な状態遷移:

- (absence) -> 'in_progress': レコードが存在しない場合に限り、その項目を進行中としてマークする
- (absence) -> 'completed' または 'skipped': 項目を直接完了またはスキップする
- 'in_progress' または 'unstarted' -> 'completed' または 'skipped': 進行中または未開始の項目を完了またはスキップする
...
REST API 経由で evaluation run を完了させるためのリクエストボディ。 

このモデルには `project_id` と `evaluation_run_id` は含まれません。これらは RESTful エンドポイントの URL パスから取得されるためです。 
...
V1 API と Object API の両方をサポートする完全な trace サーバー インターフェース。

このプロトコルは、API の完全なセット――レガシーな V1 エンドポイントと新しい Object エンドポイントの両方――をサポートする trace サーバー 実装を表します。両方の API バージョンをサポートする必要がある実装には、この型を使用してください。

---

<SourceLink url="https://github.com/wandb/weave/blob/v0.52.24/weave/trace_server/trace_server_interface.py#L2211" />

### <kbd>メソッド</kbd> `actions_execute_batch`
...
### <kbd>メソッド</kbd> `annotation_queue_add_calls`
...
### <kbd>メソッド</kbd> `annotation_queue_read`
...
### <kbd>メソッド</kbd> `annotation_queues_stats`
...
### <kbd>メソッド</kbd> `completions_create`
...
### <kbd>メソッド</kbd> `ensure_project_exists`
...
### <kbd>メソッド</kbd> `evaluation_create`
...
### <kbd>メソッド</kbd> `evaluation_delete`
...
### <kbd>メソッド</kbd> `evaluation_status`
...
### <kbd>メソッド</kbd> `prediction_create`
...
### <kbd>メソッド</kbd> `prediction_delete`
...
### <kbd>メソッド</kbd> `table_create_from_digests`
...
### <kbd>メソッド</kbd> `table_query_stats_batch`
...
Trace Server 用の Object API エンドポイント。

このプロトコルは、よりクリーンで RESTful なインターフェースを提供するオブジェクト管理 API を含みます。後方互換性を維持するため、実装ではこのプロトコルと TraceServerInterface の両方をサポートする必要があります。 
...
### <kbd>メソッド</kbd> `evaluation_run_read`
...
### <kbd>メソッド</kbd> `prediction_finish`
...
内部 API 用に `project_id` を追加することで `OpCreateBody` を拡張したもの。 
...
`code` フィールドには、その Op の実際のソースコードが格納されます。 
...
REST API 経由で Prediction を作成するためのリクエストボディです。 

このモデルには `project_id` フィールドは含まれません。`project_id` は RESTful エンドポイントの URL パスから取得されます。 
...
Prediction を作成するためのリクエスト用モデル。

内部 API での使用のために、PredictionCreateBody に project_id を追加したものです。
...
このモデルには `project_id` は含まれません。RESTful エンドポイントでは URL パスから取得されるためです。 
...
内部 API での使用のため、ScoreCreateBody に project_id を追加して拡張したものです。
...
ターン呼び出しに対する集約された統計情報に基づいてスレッドをクエリします。

ターン呼び出しはスレッドコンテキストの直下の子（call.id == turn_id のもの）です。これにより、すべてのネストされた実装上の詳細を含めるのではなく、会話単位で意味のある統計情報を得ることができます。
...
### <kbd>メソッド</kbd> `calls_query_stream`
