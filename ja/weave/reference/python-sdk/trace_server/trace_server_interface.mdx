...
複数のアノテーションキューの統計情報を取得するリクエストです。
...
* (absence) -&gt; &#39;completed&#39; or &#39;skipped&#39;: 項目を直接完了またはスキップする
* &#39;in&#95;progress&#39; or &#39;unstarted&#39; -&gt; &#39;completed&#39; or &#39;skipped&#39;: 進行中または未開始の項目を完了またはスキップする
...
REST API 経由で評価 run を終了するためのリクエストボディです。
...
V1 API と Object API の両方をサポートする完全なトレースサーバーインターフェースです。

このプロトコルは、API の完全なセット (従来の V1 エンドポイントと最新の Object エンドポイントの両方) をサポートするトレースサーバー実装を表します。両方の API バージョンをサポートする必要がある実装では、このタイプを使用します。
...
Trace Server のオブジェクト API エンドポイントです。

このプロトコルには、より簡潔で RESTful なインターフェイスを提供するオブジェクト管理 API が含まれています。後方互換性を維持するため、実装ではこのプロトコルと TraceServerInterface の両方をサポートする必要があります。
...
REST API 経由で Op オブジェクトを作成するためのリクエストボディです。
...
内部 API で使用する `project_id` を追加して OpCreateBody を拡張したものです。
...
このモデルでは、`project_id` は RESTful endpoints の URL パスから取得されるため、含まれていません。
...
Prediction を作成するためのリクエストモデルです。

内部 API 用に project_id を追加した PredictionCreateBody の拡張です。
...
REST API で Score を作成するためのリクエストボディです。

このモデルでは、`project_id` は RESTful エンドポイントの URL パスから取得されるため、除外されています。
...
内部 API で使用する `project_id` を追加し、ScoreCreateBody を拡張したものです。
...
ターン Call のみを基準に、集計統計付きでスレッドをクエリします。

ターン Call はスレッドコンテキストの直接の子です (`call.id == turn_id` の場合) 。これにより、ネストされた実装の詳細をすべて含めるのではなく、会話レベルで意味のある統計を取得できます。
