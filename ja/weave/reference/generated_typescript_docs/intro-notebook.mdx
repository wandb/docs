...
[`weave.Dataset`](/ja/weave/guides/core-types/datasets) クラスを使用して、Weave で Datasets を作成・管理できます。[Weave `Models`](/ja/weave/guides/core-types/models) と同様に、`weave.Dataset` は以下のことに役立ちます。
...
Weave は [`Evaluation` クラス](/ja/weave/guides/core-types/evaluations) による評価駆動開発をサポートしています。評価 (Evaluations) は、GenAI アプリケーションを確実に反復改善するのに役立ちます。`Evaluation` クラスは以下のことを行います。
