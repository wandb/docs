---
title: "weave"
description: "Agent クラス、trace ユーティリティ、インテグレーションを含む Weave Python SDK の API リファレンス。"
---
...
分類器 monitor は、同じモデルを対象とする複数の LLMAsAJudgeScorers の プロンプト を、1 回のスコアリング call にまとめます。
...
1 回の LLM API 呼び出しです。chat OTel span にマッピングされます。
...
この LLM 呼び出しにメディアを添付します。

content、uri、またはfile&#95;idのいずれか 1 つを必ず指定してください。明示的に設定されていない場合、Modality は mime&#95;type から推定されます。
...
output&#95;messages に アシスタントメッセージ を追加します。
...
会話セッションです。conversation&#95;id ごとにターンをグループ化します (span はありません) 。
...
これは、スレッド内で Call が期待どおりに動作するようにする `threading.Thread` のドロップイン置換実装です。Weave では特定の contextvars が設定されている必要があります (`call&#95;context.py` を参照) が、新しいスレッドは親からコンテキストを自動ではコピーしないため、Call コンテキストが失われることがあります。これは好ましくありません。このクラスは contextvar のコピーを自動化するので、このスレッドを使うだけで、ユーザーの期待どおりに動作します。
...
LLM の呼び出しを開始します (この ターン の子である chat span) 。
...
ツールの実行を開始します (この ターン の子である execute&#95;tool span) 。
...
contextvar からアクティブな LLM 呼び出しを返します。なければ None を返します。
...
各 Turn の `.spans` 属性には、その子 span が含まれます。空の場合は `session_id` を自動生成します。デフォルトでは、各 turn はそれぞれ独自の OTel trace を持ちます。
...
命令的に 1 つの turn とその子 span を OTel に送出します。

コンテキストマネージャーを使用できない場合 (ステートレスなコンテナー、コールバック、キューワーカー) に使用します。渡す各子 span には `started_at` / `ended_at` を設定しておく必要があります。送出される OTel span の Timestamp は、これらのフィールドから取得されます。turn 自身で値が指定されていない場合は、最も早い/遅い子の Timestamp が使用され、それもない場合は `now()` にフォールバックします。
...
Tool の OTel span は、OTel コンテキスト内の現在の span の子として自動的に設定されます。通常、アクティブな Turn span がある場合は、その子になります。turn を明示的に委譲する必要はありません。親子関係の伝播は Session SDK の contextvars ではなく、OTel コンテキストを通じて行われます。
