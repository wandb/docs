...
* ナレッジベースを構築する
* 関連ドキュメントを検索する取得 step を含む RAG アプリケーションを作成する
...
次に、retrieval 関数 `get_most_relevant_document` を `weave.op()` デコレーターでラップし、`Model` クラスを作成します。`weave.init('<team-name>/rag-quickstart')` を呼び出して、後から確認できるように関数のすべての入力と出力のトラッキングを開始します。チーム名を指定しない場合、出力はあなたの [W&amp;B default team or entity](/ja/platform/app/settings-page/user-settings#default-team) に記録されます。
...
アプリケーションを評価する簡単な方法がない場合、一つのアプローチとして、LLM を使用してその側面を評価する方法があります。以下は、LLM ジャッジを使用してコンテキストの精度を測定しようとする例です。この例では、与えられた回答に至る過程でコンテキストが有用だったかどうかを検証するよう LLM にプロンプトします。このプロンプトは、人気の [RAGAS フレームワーク](https://docs.ragas.io/en/stable/) を基に拡張したものです。

<div id="defining-a-scoring-function">
  ### スコアリング関数の定義
</div>

[Build an Evaluation pipeline tutorial](/ja/weave/tutorial-eval) と同様に、アプリをテストするためのサンプル行の集合とスコアリング関数を定義します。スコアリング関数は 1 行を受け取り、その行を評価します。入力引数は行内の対応するキーと一致している必要があるため、ここでの `question` はその行のdictから取得されます。`output` はモデルの出力です。モデルへの入力は、その入力引数を기준으로サンプルから取得されるため、ここでも `question` が使われます。この例では `async` 関数を使うことで、並列に高速に実行できるようにしています。async への簡単な入門が必要な場合は、[こちら](https://docs.python.org/3/library/asyncio.html) を参照してください。
...
2. `score` 関数をオーバーライドし、その関数の各呼び出しをトラッキングしたい場合は `@weave.op()` を追加する
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    const articles: Article[] = [
        { 
            text: `Novo Nordisk と Eli Lilly のライバル企業、有望な減量薬の試験結果を受けて株価が32%急騰 デンマークの Zealand Pharma の株価は、肝疾患治療薬 survodutide の試験で良好な結果が示されたことを受け、午前の取引で32%上昇した。同薬は肥満治療薬としても治験中である。「この試験は、6mg用量が安全であることを示している。これは現在進行中の[Phase 3]肥満試験でも使われている最高用量だ」と、あるアナリストはメモで述べた。この結果は、減量に使用できる薬に対する投資家の熱狂的な関心が高まる中で発表された。`
        },
        { 
            text: `Berkshire、大幅増益で株価上昇――Buffett のコングロマリットが時価総額1兆ドルに迫る Warren Buffett のコングロマリットが週末に第4四半期の好決算を発表したことを受け、Berkshire Hathaway の株価は月曜日に上昇した。Berkshire のクラスA株とクラスB株は、それぞれ1.5%以上上昇した。クラスA株は今年に入って17%以上、クラスB株は18%以上上昇している。FactSet によると、Berkshire の時価総額は直近で9301億ドルとなり、金曜日終値時点の9055億ドルから増加した。Berkshire は土曜日、第4四半期の営業利益が84億8100万ドルだったと発表した。これは前年同期の66億2500万ドルから約28%増で、保険事業の大幅な利益増が押し上げた。営業利益とは、保険、鉄道、公益事業全体にわたる事業からの利益を指す。一方で、Berkshire の現金保有額も過去最高を更新した。同コングロマリットは第4四半期に1676億ドルの現金を保有し、前四半期の記録である1572億ドルを上回った。`
        },
        { 
            text: `Highmark Health、Google と Epic の技術を組み合わせて医師が情報によりアクセスしやすくなるようにすると発表 Highmark Health は、Google Cloud とヘルスケアソフトウェア企業 Epic Systems の技術を統合すると発表した。同社によると、このインテグレーションは、情報が複数の場所や形式にまたがって保存されている場合でも、保険者と医療提供者の双方が必要な重要情報にアクセスしやすくすることを目的としている。Highmark は、700万人の会員を持つ医療保険プランと、14の病院およびその他の組織から成る医療提供ネットワークの親会社である。`
        }
    ];
        ...
        async predict(question: string): Promise<{
            ...
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    ...
                    { 
                        role: "user", 
                        content: `以下の情報を使用して、次の質問に答えてください。答えが見つからない場合は「わかりません」と書いてください。
                        コンテキスト:
                        """
                        ${context}
                        """
    ...
    function createQuestionDataset(): weave.Dataset<QuestionRow> {
        return new weave.Dataset<QuestionRow>({
            id: 'rag-questions',
            rows: [
                { question: "Zealand Pharma の肥満治療試験について、どのような重要な結果が報告されましたか？" },
                { question: "Berkshire Hathaway の第4四半期における現金保有額はどれだけ増加しましたか？" },
    ...
    const contextPrecisionScore = weave.op(async function(args: {
        ...
        const prompt = `質問、回答、コンテキストが与えられた場合、そのコンテキストが回答を導き出すのに役立ったかどうかを検証してください。役立った場合は「1」、役立たなかった場合は「0」を JSON 形式で出力してください。
        有効な JSON 形式のみで出力してください。
...
このチュートリアルでは、この例で示した検索 step のように、アプリケーションのさまざまな step に可観測性を組み込む方法を紹介しました。また、アプリケーションの応答を自動で評価する LLM judge のような、より複雑なスコアリング関数の構築方法も学びました。

<div id="next-steps">
  ## 次のステップ
</div>

エンジニア向けの実践的な RAG 手法をさらに深く掘り下げる [RAG++ コース](https://www.wandb.courses/courses/rag-in-production?utm_source=wandb_docs\&utm_medium=code\&utm_campaign=weave_docs) をご覧ください。このコースでは、パフォーマンスの最適化、コスト削減、アプリケーションの精度と関連性の向上を図るために、Weights &amp; Biases、Cohere、Weaviate による本番環境ですぐに使えるソリューションを学べます。