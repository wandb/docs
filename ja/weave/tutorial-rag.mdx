...
アプリケーションによっては、カスタム評価クラスを作成したい場合があります。たとえば、特定のパラメーター (たとえば、チャットモデルとプロンプト)、各行に対する特定のスコアリング、集計スコアの特定の計算方法を備えた標準化済みの `LLMJudge` クラスを作成したいことがあります。Weave にはそのまま使用できる `Scorer` クラスがいくつか用意されており、カスタム `Scorer` も簡単に作成できます。以下の例では、カスタム `class CorrectnessLLMJudge(Scorer)` を作成する方法を示します。
