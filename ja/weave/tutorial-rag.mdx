...
次に、検索関数 `get_most_relevant_document` を `@weave.op()` デコレータでラップし、 `Model` クラスを作成します。 `weave.init('<team-name>/rag-quickstart')` を呼び出して、後で確認できるように関数のすべての入力と出力の追跡を開始します。チーム名を指定しない場合、出力は [W&B デフォルトチームまたは Entity](/ja/platform/app/settings-page/user-settings#default-team) に記録されます。
