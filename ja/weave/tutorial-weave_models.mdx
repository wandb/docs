...
[入力、出力、メタデータ](/ja/weave/quickstart) だけでなく、[アプリケーションを流れるデータ](/ja/weave/tutorial-tracing_2) を追跡することは、システムのパフォーマンスを理解する上で不可欠です。しかし、コードやアプリケーションの パラメータ の変更が出力にどのように影響するかを理解するためには、**時間の経過に伴うアプリケーションの バージョン管理** も同様に重要です。Weave の `Model` クラスを使用すると、これらの変更を Weave で追跡できるようになります。
...
- [評価 パイプライン の構築チュートリアル](/ja/weave/tutorial-eval) に従って、アプリケーションの反復的な改善を開始しましょう。