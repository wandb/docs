...
W&B Weave は、一般的な LLM プロバイダーおよびオーケストレーションフレームワーク向けのロギングインテグレーションを提供します。これらのインテグレーションにより、さまざまなライブラリを通じて行われる呼び出しをシームレスにトレースでき、AI アプリケーションを監視および分析する能力を高められます。

アプリケーションで LLM プロバイダーのライブラリ（OpenAI、Anthropic、Cohere、Mistral など）を使用している場合、それらの API 呼び出しを、入力、出力、レイテンシ、トークン使用量、コストを含むトレースされた Call として W&B Weave に表示させたいはずです。これを使わない場合、すべての `client.chat.completions.create()`（または同等の呼び出し）を `@weave.op` または手動計測でラップする必要があり、これは面倒なうえ、何かを見落としやすくなります。

Weave は、サポートされている LLM クライアントライブラリを自動的にインターセプト（パッチ適用）します。アプリケーションコードはそのままで、通常どおりプロバイダーの SDK を使用するだけで、各リクエストが Weave の Call として記録されます。最小限のセットアップで完全なトレースが得られます。
...
* **[W&amp;B Inference Service](/ja/inference/)**
