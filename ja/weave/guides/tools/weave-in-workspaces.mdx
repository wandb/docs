...
# モデルのトレーニング run中にトレースをログする {#log-traces-during-model-training-runs}
...
## Weave パネルを使用する {#use-weave-panels}
...
## 特定の W&amp;B run にトレースを関連付ける {#associate-traces-with-a-specific-wb-run}
...
## Weave トレースで W&amp;B アーティファクト を参照する {#reference-wb-artifacts-in-a-weave-trace}
...
### アーティファクトをトレースの属性として追加する {#add-an-artifact-as-an-attribute-of-a-trace}
...
### アーティファクトを `Model` の属性として追加する {#add-an-artifact-as-an-attribute-of-a-model}
