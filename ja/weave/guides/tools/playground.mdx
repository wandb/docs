...
- **マルチ LLM 対応:** モデルを切り替えられ、チーム単位で APIキー を管理できます。
...
- **API key**: OpenAI の APIキー など、プロバイダー用の APIキー。
...
[以前に作成したカスタムプロバイダー](#add-a-custom-provider) を削除するには、次の手順を実行する。

1. プロンプトヘッダーで **Select a model** ドロップダウンをクリックし、**+Configure providers** を選択する。

* または、サイドバーのメニューで **Project** を選択し、**AI Providers** タブを選択することもできる。

2. **Custom providers** テーブルで、削除したいカスタムプロバイダーを探す。
3. カスタムプロバイダーのエントリの **Last Updated** 列で、削除ボタン（ごみ箱アイコン）をクリックする。
4. ポップアップ モーダルで、そのプロバイダーを削除することを確認する。この操作は元に戻せない。
5. **Delete** をクリックする。
