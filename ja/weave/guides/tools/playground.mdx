...
新しいメッセージを モデル に送信するには、**Message composer** を使用します。
...
既存の [保存済みモデル](#save-a-model) を編集してパラメーターを微調整したり、設定を更新したりできます。これにより、保存済みモデルをユースケースに合わせて進化させ続けることができます。
...
5. **Update model** をクリックします。これによりモデルが更新され、**Select a model** ドロップダウンの **Saved Models** からアクセスできるようになります。保存済みモデルのバージョンは自動的に繰り上がります。