...
2. ドロップダウンで、必要に応じてパラメータを調整します。Weave の Call 追跡のオン/オフを切り替えたり、[関数の追加](#anthropicintegrationsanthropic) を行うこともできます。
...
2. 2 つ目のチャットでは、以下が可能です。
   - [比較する LLM の選択](#playground-へのアクセス)
   - [パラメータの調整](#amazon-bedrockintegrationsbedrock)
   - [関数の追加](#anthropicintegrationsanthropic)
