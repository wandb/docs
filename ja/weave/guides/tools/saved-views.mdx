...
### 保存済みビューを作成する {#create-a-saved-view}
...
### 保存済みビューを読み込む {#load-a-saved-view}
...
### 保存済みビューの名前を変更する {#rename-a-saved-view}
...
### 保存済みビューを編集する {#edit-a-saved-view}

    1. [保存済みビューを読み込む](#load-a-saved-view)で説明されている手順に従います。
    2. 表の設定を調整します。
    3. 右上隅にある **Save view** をクリックします。

### 保存済みビューを削除する {#delete-a-saved-view}
...
### デフォルトビューに戻る {#return-to-the-default-view}
...
### `SavedView` を初期化する {#initialize-a-savedview}
...
### `SavedView` をグリッドとして可視化する {#visualize-the-savedview-as-a-grid}
...
### 表示する列を設定する {#set-displayed-columns}
...
### op 名でフィルターする {#filter-by-operation-name}
...
### 演算子と条件でフィルターする {#filter-by-operator-and-condition}
...
### `SavedView` を保存する {#save-the-savedview}
...
### 関数の Call を取得する {#retrieve-function-calls}
