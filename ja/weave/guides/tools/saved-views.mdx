...
Weave では、*保存済みビュー* を使うことで、トレースされた関数呼び出しや評価の操作方法をカスタマイズできます。保存済みビューを定義すると、フィルター、並び替え、列の表示/非表示を設定して、関連するデータにすばやくアクセスできます。

ビューは Weave の Python SDK と UI のどちらからでも作成、変更、保存できます。Python SDK では、プログラムによるフィルタリングやクエリをきめ細かく制御でき、UI では **Traces** タブと **Evals** タブでさまざまな表の設定を簡単に確認して保存できます。

このガイドでは次の内容を扱います。

* [Python SDK で保存済みビューを作成および変更する方法](#saved-views-in-the-python-sdk)。
* [Weave UI で保存済みビューを作成および操作する方法](#saved-views-in-the-ui)。

<div id="saved-views-in-the-python-sdk">
  ## Python SDK における SavedView
</div>

Weave の `SavedView` クラスは、トレース と evals のデータに対するビューを保存・フィルター・ソートし、カスタマイズするための手段を提供します。 
...
`.to_grid()` を使用して、`SavedView` をグリッドとして表示します。`limit` で表示する最大行数を指定します。
...
Weave では、すべての トレース または eval はオペレーション名に紐付けられています。
`.filter_op()` を使用して、その特定のオペレーションが実行されたCallだけが含まれるように `SavedView` をフィルタリングします。
...
`.add_filter()` を使用して、ビューにカスタムフィルターを適用します。フィルターは、[サポートされるフィルター演算子](#filter-operators) のいずれかと条件を組み合わせて定義します。
...
Weave UI で保存済みビューを作成、読み込み、名前変更、編集できます。より細かく制御するには、[Python SDK](#saved-views-in-the-python-sdk) を使用します。
...
1. **Traces** または **Evals** タブにアクセスします。
2. 削除するビューを[読み込み](#load-a-saved-view)ます。
