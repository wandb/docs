...
keywords: ["オブジェクト", "バージョン管理", "refs", "weave.publish", "weave.ref"]
---
...
* `[OBJECT-NAME]`: オブジェクト名
* `[OBJECT-VERSION]`: バージョンハッシュ、`v0` や `v1` のような string、または `:latest` のような alias のいずれかです。すべてのオブジェクトには `:latest` alias があります。
