...
2. Entity: トレースをログできるのは、自分がアクセス権を持つ entity 配下のプロジェクトのみです。entity 名は、[https://wandb.ai/home] の W&amp;B ダッシュボードにアクセスし、左サイドバーの **Teams** フィールドを確認するとわかります。
3. Project Name: 好きな名前を付けてください。
4. `OPENAI_API_KEY`: [OpenAI dashboard](https://platform.openai.com/api-keys) から取得できます。
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    * `BatchSpanProcessor` は非同期でフラッシュされるため、スパンが確実にフラッシュされるよう、シャットダウン前に遅延を入れたうえで `provider.shutdown()` を明示的に呼び出す必要があります。
    ...
    // 重要: インストルメンテーションがパッチを適用できるよう、OpenAI を最初にインポートすること
...
<Tabs>
  <Tab title="Python">
    次のコードを、`openllmetry_example.py` などの Python ファイルに貼り付けます。上記のコードと同じですが、`OpenAIInstrumentor` のインポート元が `openinference.instrumentation.openai` ではなく `opentelemetry.instrumentation.openai` である点が異なります。
  ...
  <Tab title="TypeScript">
    次のコードを、`openllmetry_example.ts` などの TypeScript ファイルに貼り付けてください。なお、これは Traceloop の OpenAI インストルメンテーション パッケージを使用しています。
...
インストルメンテーションパッケージの代わりに OTel を直接使用することもできます。スパン属性は、[https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-spans/) で説明されている OpenTelemetry のセマンティック規約に従って解析されます。
...
<Accordion title="深くネストされたオペレーションをトレースし、最も外側の span のみをターンとしてマークする">
  <Tabs>
    <Tab title="Python">
      ```python lines
...
<Accordion title="スレッドに属しているがターンではないバックグラウンド操作をトレースする">
  <Tabs>
    <Tab title="Python">
      ```python lines
