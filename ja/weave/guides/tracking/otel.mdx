...
`wandb-api-key` ヘッダーに W&amp;B API キーを渡し、`TracerProvider` クラス内で次のキーを OpenTelemetry の Resource 属性として指定します：
...
* `WANDB_OTLP_ENDPOINT` からエンドポイント URL を、`WANDB_API_KEY` から API キーを読み取り、`wandb-api-key` ヘッダーを使って Weave の OTLP エンドポイントにトレースをエクスポートします。
