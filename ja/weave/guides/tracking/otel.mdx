...
<Warning>
  OTel スパンを Weave の新しい Agents ビューに送信するには、[Weave の新しい OTel ネイティブ ワークフロー](/ja/weave/guides/tracking/trace-agents-otel)を使用することをお勧めします。生の OTLP トレースデータを Weave の標準 Traces ビューに取り込む場合にのみ、このガイドを使用してください。
</Warning>
...
<Note>
  このエンドポイントは **Traces** ビューと **スレッド** ビューにデータを送ります。multi-turn エージェント から **Agents** ビューに OTel スパンを送信するには、代わりに agents エンドポイントを使用してください。詳細は、[OpenTelemetry スパンを Agents ビューに送信する](/ja/weave/guides/tracking/trace-agents-otel)を参照してください。
</Note>

## エンドポイント の詳細 {#endpoint-details}
...
## 認証とルーティング {#authentication-and-routing}
...
### OpenInference インストルメンテーション {#openinference-instrumentation}
...
### OpenLLMetry インストルメンテーション {#openllmetry-instrumentation}
...
### インストルメンテーションを使用しない場合 {#without-instrumentation}
...
## OpenTelemetry Collector を使用する {#use-an-opentelemetry-collector}
...
### collector を設定する {#set-up-a-collector}
...
## OTel トレースをスレッドに整理する {#organize-otel-traces-into-threads}
