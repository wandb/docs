...
keywords: ["LLM コスト", "トークン コスト", "add_cost", "query_costs", "purge_costs"]
---
...
## 自動コストトラッキングを使用する {#use-automatic-cost-tracking}
...
### カスタムコストをクエリする {#query-custom-costs}
...
### カスタムコストをパージする {#purge-a-custom-cost}
...
### project のカスタムコストを計算する {#calculate-custom-costs-for-a-project}
...
### カスタムコスト付きカスタムモデルの設定 {#set-up-a-custom-model-with-custom-costs}
