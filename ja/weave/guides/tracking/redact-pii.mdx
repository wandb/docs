...
## PII のマスキングを有効にする {#enable-pii-redaction}
...
## デフォルトでマスクされるエンティティ {#entities-redacted-by-default}
...
## `REDACT_KEYS` を使用した機密キーのマスキング {#redact-sensitive-keys-with-redact_keys}
...
### 事前定義された `REDACT_KEYS` {#pre-defined-redact_keys}
