---
title: "スレッドをトレースする"
description: "スレッドを使用して、LLM アプリケーション内のマルチターン会話をトレースおよび分析します。"
---

W&amp;B Weave の *スレッド* を使用すると、LLM アプリケーション内のマルチターン会話をトラッキングして分析できます。スレッドでは、関連する Call を共通の `thread_id` の下にグループ化するため、セッション全体を可視化し、ターンをまたいだ会話レベルのメトリクスをトラッキングできます。スレッドはプログラムから作成でき、Weave UI で可視化できます。

スレッドの使用を開始するには、次の手順を実行します。
...
スレッド は、次のような内容を整理・分析したいときに役立ちます。

* 複数ターンの会話
* セッションベースのワークフロー
* 関連する一連の処理

スレッド を使うと、Call をコンテキストごとにまとめられるため、システムが複数ステップにわたってどのように応答しているかを理解しやすくなります。たとえば、単一のユーザー セッション、エージェントの一連の意思決定、インフラストラクチャー層とビジネスロジック層にまたがる複雑なリクエストなどをトラッキングできます。

アプリケーションを スレッド と ターン で構造化することで、メトリクスがよりクリーンになり、Weave UI での公開範囲も向上します。すべての低レベルな op を確認する代わりに、重要な高レベルの step に集中できます。
...
*スレッド* は、共通の会話コンテキストを共有する関連するCallを論理的にまとめたものです。スレッドには次の特徴があります。
...
*ターン* は、スレッド内の高レベルな操作であり、UI ではスレッドビュー内の個々の行として表示されます。各ターン:

* 会話またはワークフローにおける 1 つの論理的な step を表します
* ターンはスレッドコンテキストの直接の子であり、ネストされた下位レベルの Call を含む場合があります (これらはスレッドレベルの統計には表示されません) 。
...
* *ターン Call* は、新しいターンを開始するトップレベルのオペレーションです。
* *Nested Call* は、ターン内で行われる下位レベルのオペレーションです。

<div id="trace">
  ### トレース
</div>

*トレース* は、1 回の操作における完全な Call スタックを取得します。スレッドは、同じ論理的な会話またはセッションの一部であるトレースをまとめてグループ化します。言い換えると、1 つのスレッドは複数のターンで構成され、それぞれが会話の 1 つの部分を表します。トレースの詳細については、[トレースの概要](/ja/weave/guides/tracking/tracing)を参照してください。

<div id="ui-overview">
  ## UI 概要
</div>

Weave のサイドバーで **スレッド** を選択して、[スレッド一覧ビュー](#threads-list-view) にアクセスします。

<Frame>
  ![Weave サイドバー内の スレッド アイコン](/weave/guides/tracking/imgs/threads-sidebar.png)
</Frame>
...
* 行のいずれかをクリックすると、その行の詳細ドロワーが開きます。
* スレッド内のすべてのターンを表示します。
* ターンは開始順 (開始時刻に基づく。継続時間や終了時刻によるものではない) に一覧表示されます。
* Callレベルのメタデータ (レイテンシ、入力、出力) を含みます。
* ログされている場合、メッセージ内容または構造化データをオプションで表示します。
* ターンの実行全体を確認するには、スレッド詳細ドロワーからそのターンを開くことができます。これにより、その特定のターン中に発生したすべてのネストされたオペレーションを掘り下げて確認できます。
* ターンに LLM Call から抽出されたメッセージが含まれている場合、それらは右側のチャットペインに表示されます。これらのメッセージは、通常、サポートされているインテグレーション (例: `openai.ChatCompletion.create`) によって行われた Call から得られ、表示するためには特定の基準を満たす必要があります。詳細は [Chat view behavior](#chat-view-behavior) を参照してください。

<div id="chat-view-behavior">
  ### Chat ビューの動作
</div>

チャット ペインには、各ターンで行われた LLM Call から抽出された構造化されたメッセージ データが表示されます。このビューでは、やり取りの内容が会話形式で表示されます。
...
Weave は、1 つのターン内にある Call のうち、LLM プロバイダーとの直接的なやり取り (たとえば、プロンプトを送信してレスポンスを受信する処理) を表すものからメッセージを抽出します。ほかの Call の内部にさらにネストされていない Call のみが、メッセージとして表示されます。これにより、中間 step や集約された内部ロジックが重複して表示されるのを防げます。

通常、メッセージは自動パッチ適用されたサードパーティ SDK から生成されます。たとえば、次のようなものです。
...
あるターンでメッセージが一切出力されない場合、そのターンに対応するチャットペインには空のメッセージセクションが表示されます。チャットペインには、同じスレッド内の別のターンのメッセージは、引き続き表示される場合があります。
...
<div id="navigate-to-and-from-the-trace-view">
  #### Trace view にアクセスしたり戻ったりする
</div>

ターンをクリックすると、その完全なトレースを開くことができます。

左上隅に戻るボタンが表示され、スレッド詳細ビューに戻れます。Weave は、この遷移をまたいで UI の状態 (スクロール位置など) を保持しません。
...
このセクションの各例では、アプリケーション内でターンとスレッドを整理するための異なる戦略を示しています。ほとんどの例では、スタブ関数内に独自の LLM Call やシステムの動作を実装してください。

* セッションや会話をトラッキングするには、`weave.thread()` コンテキストマネージャを使用します。
* 論理的なオペレーションを `@weave.op` でデコレートして、ターンまたはネストされた Call としてトラッキングします。
* `thread_id` を渡すと、Weave はそのブロック内のすべてのオペレーションを同じスレッドにグループ化します。`thread_id` を省略すると、Weave が一意の ID を自動生成します。

`weave.thread()` の戻り値は、`thread_id` プロパティを持つ `ThreadContext` オブジェクトです。これをログしたり、再利用したり、他のシステムに渡したりできます。

ネストされた `weave.thread()` コンテキストは、同じ `thread_id` を再利用しない限り、常に新しいスレッドを開始します。子コンテキストを終了しても、親コンテキストが中断されたり上書きされたりすることはありません。これにより、アプリケーションのロジックに応じて、フォークしたスレッド構造や階層化されたスレッドオーケストレーションが可能になります。

<div id="basic-thread-creation">
  ### 基本的なスレッド作成
</div>

次のコードサンプルは、`weave.thread()` を使用して、共通の `thread_id` を持つ 1 つ以上のオペレーションをまとめる方法を示します。これは、アプリケーションでスレッドの利用を始める最も簡単な方法です。
...
この例では、`@weave.op` デコレーターと `weave.thread()` のコンテキスト管理を使用して、会話型エージェントを手動で定義する方法を示します。`process_user_message` を呼び出すたびに、スレッドに新しいターンが作成されます。独自のエージェントループを構築し、コンテキストやネストの扱いを完全に制御したい場合は、このパターンを使用できます。

短時間のやり取りには自動生成されたスレッド ID を使用し、セッションをまたいでスレッドコンテキストを保持したい場合は、`user_session_123` のようなカスタムセッション ID を渡してください。
...
<div id="manual-agent-with-unbalanced-call-depth">
  ### 不均衡なCall深度を持つ手動エージェント
</div>

この例では、スレッドのコンテキストをどのように適用するかに応じて、ターンをCallスタック内の異なる深さで定義できることを示します。このサンプルでは、2 つのプロバイダー (OpenAI と Anthropic) を使用しており、それぞれでターン境界に到達する前のCall深度が異なります。

すべてのターンは同じ `thread_id` を共有しますが、ターン境界はプロバイダーのロジックに応じてスタック内の異なるレベルに現れます。これは、Callを異なるバックエンドごとに別々にトレースする必要がありつつ、同時にそれらを同じスレッドにグループ化したい場合に便利です。
...
すでに開始したセッションを再開し、同じスレッドに Calls を追加し続ける必要がある場合があります。一方で、既存のセッションを再開できず、代わりに新しいスレッドを開始しなければならない場合もあります。

オプションのスレッド再開を実装する際は、`thread_id` パラメーターを **決して** `None` のままにしないでください。そうすると、スレッドのグループ化が完全に無効になります。代わりに、常に有効なスレッド ID を指定してください。新しいスレッドを作成する必要がある場合は、`generate_id()` のような関数を使用して一意の識別子を生成してください。

`thread_id` が指定されていない場合、Weave の内部実装ではランダムな UUID v7 が自動的に生成されます。この動作は独自の `generate_id()` 関数で再現することも、任意の一意な string 値を使用することもできます。
...
この例では、複数の連携するスレッドを使用して、複雑なアプリケーションをどのように構成するかを示します。

各レイヤーはそれぞれ専用のスレッドコンテキスト内で実行されるため、関心事をきれいに分離できます。親アプリケーションスレッドは、共有される `ThreadContext` を使用してスレッド ID を設定することで、これらのレイヤーを調整します。このパターンは、システムの異なる部分を個別に分析・監視しつつ、共通のセッションに関連付けておきたい場合に使用してください。
...
class ThreadSchema:
    ...
    turn_count: int          # このスレッド内のターン Call 数
    start_time: datetime     # このスレッド内のターン Call の最も早い開始時刻
    last_updated: datetime   # このスレッド内のターン Call の最も遅い終了時刻
