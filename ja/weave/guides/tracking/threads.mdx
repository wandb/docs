...
Threads を使うと、Calls をコンテキストごとにまとめられるため、システムが複数ステップにわたってどのように応答しているかを理解しやすくなります。たとえば、単一のユーザー セッション、エージェントの一連の意思決定、インフラストラクチャ層とビジネスロジック層にまたがる複雑なリクエストなどを追跡できます。

アプリケーションを Threads と turns で構造化することで、メトリクスがよりクリーンになり、Weave UI での 公開範囲 の把握もしやすくなります。すべての低レベルな Op を見る代わりに、重要な高レベルのステップに集中できます。
...
Weave は、1 回のターン内で行われる呼び出しのうち、LLM プロバイダーとの直接のやり取り (例: プロンプトの送信とレスポンスの受信) を表すものからメッセージを抽出します。ほかの呼び出しの内部にさらにネストされていない呼び出しのみがメッセージとして表示されます。これにより、中間ステップや集約された内部ロジックが重複して表示されるのを防ぎます。

通常、メッセージは自動でパッチが適用されたサードパーティ SDK から出力されます。たとえば、次のようなものです:
...
ある turn でメッセージが一切出力されない場合、その turn に対応するチャットペインには空のメッセージセクションが表示されます。チャットペインには、同じスレッド内の別の turn のメッセージは、引き続き表示される場合があります。
...
左上に戻るボタンが表示され、thread の詳細ビューに戻れます。Weave は画面遷移の際に UI の状態 (スクロール位置など) を保持しません。
...
以前に開始したセッションを再開して、同じスレッドに Calls を追加し続ける必要がある場合があります。一方で、既存のセッションを再開できず、新しいスレッドを開始しなければならないこともあります。
