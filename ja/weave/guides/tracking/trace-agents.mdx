...
| セッション        | `Session`     | *(no span, turns are grouped by the `conversation_id` attribute)* | 1 つ以上のターンを含む会話または run                               | [Python](/ja/weave/reference/typescript-sdk/classes/session) <br /> [TypeScript](/ja/weave/reference/typescript-sdk/classes/session) |
