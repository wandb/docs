...
spec:
    ...
    podTemplates:
      - name: keeper
        ...
        spec:
          ...
          # ノード間に Keeper を分散させるためのアンチアフィニティ（HA に推奨）
          ...
          containers:
            - name: clickhouse-keeper
              ...
              # リソースリクエスト - サンプル値です。ワークロードに応じて調整してください
              resources:
                requests:
                  memory: "256Mi"
                  cpu: "0.5"
  ...
  configuration:
    clusters:
      - name: keeper # Keeper クラスター名 - サービスの DNS 名前解決に使用されます
        layout:
          replicasCount: 3
        templates:
          podTemplate: keeper
...
* **StorageClass**: `storageClassName: gp3` を、使用中のクラスタで利用可能な StorageClass に合わせて更新します。
* **Security Context**: 組織のセキュリティポリシーに準拠するように、`runAsUser`、`fsGroup` の値を調整します。
* **Anti-Affinity**: クラスタのトポロジーと HA 要件に基づいて、`affinity` セクションをカスタマイズまたは削除します。
* **Resources**: CPU/メモリの値はサンプルです。適切なリソースサイズについては W&amp;B Solutions Architects に相談してください。
...
spec:
    ...
    podTemplates:
      - name: clickhouse
        ...
        spec:
          ...
          # アンチアフィニティルール - サーバーが異なるノードで実行されることを保証します（省略可能ですが推奨）
          ...
          containers:
            - name: clickhouse
              ...
              # AWS 認証情報（IRSA を使用する場合はこのセクションを削除してください）
              env:
  ...
  configuration:
    # Keeper（ZooKeeper）設定
    # 重要: これらのホスト名はステップ 3 の Keeper デプロイのホスト名と一致している必要があります
    zookeeper:
      nodes:
        ...
        - host: chk-wandb-keeper-0-2.clickhouse.svc.cluster.local
          port: 2181
      # 省略可能: 必要に応じてタイムアウトを調整する場合はコメントを解除してください
    ...
    # クラスター設定
    clusters:
      - name: weavecluster # クラスター名 - カスタマイズ可能ですが wandb-cr.yaml と一致している必要があります
        layout:
          shardsCount: 1
          replicasCount: 3 # レプリカ数 - HA 要件に応じて調整してください
        templates:
...
6. **Anti-Affinity Rules**: クラスター構成や高可用性要件に応じてカスタマイズするか削除してください
7. **Keeper Hostnames**: Keeper ノードのホスト名は、ステップ 3 の Keeper デプロイメントの命名と **一致している必要があります**（下記の「Keeper 名前付けの理解」を参照）
8. **Cluster Naming**: クラスター名 `weavecluster` は変更可能ですが、ステップ 5 の `WF_CLICKHOUSE_REPLICATED_CLUSTER` の値と一致させる必要があります
9. **Credentials**:
   - IRSA を使用する場合: `<use_environment_credentials>true</use_environment_credentials>` を維持するか、環境変数にマッピングされたシークレット キーへアクセスしてください。
...
Weave の動作を確認するために、簡単な Python テストを作成します。
...
これを実行した後、組織内の W&amp;B UI の traces ページを開き、traces が表示されていることを確認してください。
...
異なる ClickHouse バージョンを使用するには、`ch-keeper.yaml` と `ch-server.yaml` の両方の image タグを更新してください。

```yaml
image: clickhouse/clickhouse-keeper:25.3.5.42   # Keeper バージョン
image: clickhouse/clickhouse-server:25.3.5.42   # Server バージョン
```
