...
W&amp;B Weave をセルフホストすると、その環境や設定をより細かく制御できます。これにより、より分離された環境を構築し、追加のセキュリティ要件やコンプライアンス要件に対応しやすくなります。このドキュメントでは、Altinity ClickHouse Operator を使用して、[W&amp;B Self-Managed](/ja/platform/hosting/hosting-options/self-managed) デプロイで W&amp;B Weave を実行するために必要なコンポーネントをデプロイする方法を説明します。最終的には、レプリケーションされた ClickHouse データベースと S3互換オブジェクトストレージを基盤とする、本番環境対応の Weave インスタンスを独自の Kubernetes クラスター上で稼働させられるようになります。このガイドは、組織内で W&amp;B のデプロイと運用を担当する Kubernetes 管理者およびプラットフォームエンジニアを対象としています。
...
## 重要なセットアップ上の注意 {#important-setup-notes}
...
## セルフマネージド Weave インスタンスをデプロイする {#deploy-your-self-managed-weave-instance}
...
### Altinity ClickHouse Operator をデプロイする {#deploy-the-altinity-clickhouse-operator}
...
#### Altinity Helmリポジトリを追加する {#add-the-altinity-helm-repository}
...
#### Operator の設定を作成 {#create-the-operator-configuration}
...
#### operatorをインストールする {#install-the-operator}
...
#### operator がインストールされていることを確認する {#verify-the-operator-installation}
...
#### S3認証情報を設定する {#configure-s3-credentials}
...
##### オプション A: AWS IAM ロールを使用する (IRSA、AWS では推奨) {#option-a-use-aws-iam-roles-irsa-recommended-for-aws}
...
##### オプション B: アクセスキーを使用する {#option-b-use-access-keys}
...
#### Keeper の設定を作成する {#create-the-keeper-configuration}
...
#### ClickHouse Keeperをデプロイする {#deploy-clickhouse-keeper-resources}

```bash
kubectl apply -f ch-keeper.yaml
```

#### Keeper のデプロイを確認する {#verify-the-keeper-deployment}
...
### ClickHouse クラスターをデプロイする {#deploy-the-clickhouse-cluster}
...
#### ClickHouse サーバー設定を作成する {#create-the-clickhouse-server-configuration}
...
#### S3 の設定を更新する {#update-the-s3-configuration}
...
#### 認証情報を設定する (Option B のみ) {#configure-credentials-option-b-only}
...
#### ClickHouse クラスターのリソースをデプロイする {#deploy-the-clickhouse-cluster-resources}

```bash
kubectl apply -f ch-server.yaml
```

#### ClickHouse デプロイを確認する {#verify-the-clickhouse-deployment}
...
### W&amp;B Platform で Weave を有効にする {#enable-weave-in-the-wb-platform}
...
#### ClickHouse の接続情報を確認する {#gather-clickhouse-connection-information}
...
#### W&amp;B Custom Resource を更新する {#update-the-wb-custom-resource}
...
#### 更新した設定を適用する {#apply-the-updated-configuration}

```bash
kubectl apply -f wandb-cr.yaml
```

#### Weave Trace のデプロイを確認する {#verify-the-weave-trace-deployment}
...
#### データベースが作成されていることを確認します {#verify-database-creation}
...
### Weave が有効になっていることを確認する {#verify-that-weave-is-enabled}

この最後の手順では、Weave にライセンスが付与されており、W&amp;B Console からアクセスでき、クライアント SDK からのトレースを記録できることを確認します。

#### W&amp;B Console にアクセスする {#access-the-wb-console}

Webブラウザで W&amp;B インスタンスの URL にアクセスします。

#### Weave ライセンスの状態を確認する {#check-the-weave-license-status}
...
#### Weave の機能をテストする {#test-weave-functionality}
...
### ClickHouse Keeper の問題 {#clickhouse-keeper-issues}
...
### ClickHouse サーバーの問題 {#clickhouse-server-issues}
...
### Weave Trace の問題 {#weave-trace-issues}
...
### 本番環境の最小構成 {#minimum-production-setup}
...
### 推奨される本番構成 {#recommended-production-setup}
...
### ClickHouse をスケールする {#scale-clickhouse}
...
### 異なるバージョンの ClickHouse を使用する {#use-a-different-clickhouse-version}
...
<Warning>
  ClickHouse Server をアップグレードする際は、ClickHouse Keeper も互換性のあるバージョンにアップグレードしてください。W&amp;B セルフマネージド デプロイメントで ClickHouse バージョンを変更する前に、[アップグレード時の ClickHouse 互換性](/ja/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades)と[サポートされる W&amp;B Server リリース](/ja/release-notes/server-releases)のページを確認してください。
</Warning>

### ClickHouse を監視する {#monitor-clickhouse}
...
<Note>
  Weave では、サポートされる ClickHouse バージョンが必要です。ClickHouse または W&amp;B Server をアップグレードする前に、[アップグレード時の ClickHouse 互換性](/ja/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades)と[サポートされる W&amp;B Server リリース](/ja/release-notes/server-releases)を参照してください。ClickHouse Server と ClickHouse Keeper は一緒にアップグレードしてください。
</Note>

### ClickHouse Operator のアップグレード {#upgrade-the-clickhouse-operator}
...
### ClickHouse Server のアップグレード {#upgrade-clickhouse-server}

`ch-keeper.yaml` と `ch-server.yaml` の両方でイメージのバージョンを更新してから、サーバーのマニフェストを適用します:

```bash
# ch-keeper.yaml と ch-server.yaml を編集してイメージタグを変更する
kubectl apply -f ch-keeper.yaml
kubectl apply -f ch-server.yaml
...
### Weave Trace のアップグレード {#upgrade-weave-trace}
...
* [取り込みサンプリングを設定する](/ja/weave/guides/platform/ingest-sampling): トレース量が多い場合に、ストレージと LLM スコアリングのコストを抑えるため、受信したトレースの一部のみを保持します。
