...
Weave が正しく動作していることを確認するために、簡単な Python テストを作成します。
...
これを実行したら、組織内の W&amp;B UI の traces ページでトレースを確認してください。
...
接続に失敗する場合は、`ch-server.yaml` 内の Keeper ホスト名が実際の Keeper デプロイと一致していない可能性があります。命名規則については、ステップ 4 の &quot;Keeper の命名規則を理解する&quot; を参照してください。
