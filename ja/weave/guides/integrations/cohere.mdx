...
Weave の op を使うと、実験を進めながらコードが自動的にバージョン管理され、入力と出力も記録されるため、結果を&#95;再現可能&#95;にできます。Cohere の chat method を呼び出す [`@weave.op()`](/ja/weave/guides/tracking/ops) でデコレートした関数を作成すると、Weave が入力と出力をトラッキングしてくれます。以下はその例です。
