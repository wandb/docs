...
## TypeScript project で Weave インテグレーションを使用する {#use-weave-integrations-with-your-typescript-project}
...
### どのタイプのプロジェクトかわからない場合 {#if-youre-unsure-which-type-of-project-you-have}
...
### CommonJS プロジェクトを設定する {#set-up-a-commonjs-project}
...
### ESM プロジェクトを設定する {#set-up-an-esm-project}
...
## 高度な使い方とトラブルシューティング {#advanced-usage-and-troubleshooting}
...
### `NODE_OPTIONS` を使用する (ESM の場合のみ) {#use-node_options-only-for-esm}
...
### 手動パッチ適用 (フォールバックオプション) {#manual-patching-fallback-option}
