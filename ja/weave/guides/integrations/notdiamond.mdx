...
![API キーを作成](/weave/guides/integrations/imgs/notdiamond/api-keys.png)
...
[評価][evals] で独自の [カスタム ルーター] をトレーニングすることで、特定のユースケースに応じて Not Diamond が
評価結果に基づいてプロンプトをルーティングできるようになります。
...
この preference ID を任意の `model_select` リクエストで再利用することで、プロンプトをルーティングし、
評価データでのパフォーマンスを最大化し、コストを最小限に抑えることができます:
