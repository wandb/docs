...
Weave は、デフォルトでこのライブラリに対して [暗黙的なパッチ適用（implicit patching）](/ja/weave/guides/integrations#automatic-implicit-patching) を有効にします。これにより、パッチ関数を明示的に呼び出すことなく、Verifiers で Weave を使用できます。
...
トレーニングが開始されると、UI で run 中にログ記録された [トレースを確認](/ja/weave/guides/tools/weave-in-workspaces) できます。
