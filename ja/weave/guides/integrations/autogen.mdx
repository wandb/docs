...
始める前に、AutoGen と Weave がインストールされていることを確認してください。また、使用する予定の LLM プロバイダ (たとえば OpenAI または Anthropic) 向けの SDK も必要です。
...
AutoGen のメモリ コンポーネントは Weave でトレースできます。`@weave.op()` を使用すると、メモリ操作を 1 つのトレースにまとめて、可読性を高められます。
...
Retrieval Augmented Generation (RAG) ワークフローは、`ChromaDBVectorMemory` のようなメモリシステムを使用したドキュメントのインデックス作成や取得を含めて、トレース可能です。RAG プロセスを `@weave.op()` でデコレートすると、フロー全体を可視化しやすくなります。
...
Weave では、`SingleThreadedAgentRuntime` など、AutoGen のエージェントランタイム内のオペレーションをトレースできます。ランタイムの実行関数を `@weave.op()` でラップすると、関連するトレースをグループ化できます。
