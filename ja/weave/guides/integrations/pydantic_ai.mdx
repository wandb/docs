...
### Weave で OTEL トレースを設定する {#configure-otel-tracing-in-weave}
...
## OTEL で PydanticAI エージェントをトレースする {#trace-pydanticai-agents-with-otel}
...
## OTEL で PydanticAI のツールをトレースする {#trace-pydanticai-tools-with-otel}
...
## デフォルトですべてのエージェントにインストルメンテーションを適用する {#instrument-all-agents-by-default}
