...
<Warning>
  APIキーやプロジェクト情報などの機密性の高い環境変数は、環境ファイル (たとえば `.env`) に保存し、`os.environ` を使って読み込むことをおすすめします。こうすることで、認証情報を安全に保ち、コードベースに含めずに済みます。
</Warning>
...
次のコードスニペットでは、OTLP span exporter と tracer provider を設定して、PydanticAI アプリケーションから Weave に OTEL トレースを送信する方法を示します。
