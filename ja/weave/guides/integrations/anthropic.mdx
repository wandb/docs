---
title: "Anthropic"
description: "Weave を使用して、Anthropic SDK 経由で行われる LLM Call を自動的にトラッキングしてログする"
---
...
<Tabs>
  ...
  <Tab title="TypeScript">
    Weave は、モジュールローダーフックを介して `@anthropic-ai/sdk` を自動的にインストルメントします。必要なセットアップは、モジュールシステムによって若干異なります。CommonJS と ESM の違いや、Weave のローダーフックの仕組みの詳細については、[TypeScript SDK サードパーティインテグレーションガイド](/ja/weave/guides/integrations/js)を参照してください。

    * **CommonJS プロジェクト**: 追加の設定は不要です。自動インストルメンテーションが先に実行されるよう、`@anthropic-ai/sdk` より前に `weave` を require してください。
    * **ESM プロジェクト**: インストルメンテーションが他のモジュールより先に読み込まれるよう、`--import=weave/instrument` フラグを指定して Node を起動してください。
...
## 独自の op でラップする {#wrap-with-your-own-ops}
...
## 実験をしやすくするために `Model` を作成する {#create-a-model-for-easier-experimentation}
...
## ツール (関数呼び出し) {#tools-function-calling}
