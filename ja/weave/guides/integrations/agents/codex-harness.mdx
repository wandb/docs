...
keywords: ["OpenAI Codex", "OTEL", "agent tracing", "Stop フック", "rollout"]
---

Weave Codex プラグインは、Codex の各ターンを自動的にトレースし、その構造化データを W&amp;B Weave に送信します。Codex のワークフローを変更することなく、すべてのモデル Call、ツール実行、推論ステップがログされます。これらのトレースを使用して、セッションのデバッグ、ツール使用状況の監査、Runs 全体のコストとレイテンシの監視を行えます。

このプラグインは、Codex 自身のロールアウトセッションファイル (`~/.codex/sessions/**/rollout-*.jsonl`) を読み取り、スパンを再構築します。fire-and-forget の Stop フック を通じて Codex のクリティカルパスとは完全に切り離して実行されるため、Codex がネットワークを待つことはありません。

<Warning>
  デフォルトでは、このプラグインはスパンの内容、つまりプロンプト、モデルの応答と推論、ツール呼び出しの引数、ツールの結果を取得します。ツールの結果には、シェルコマンド、コマンド出力、file の内容が含まれます。このデータは、お使いの Weave インスタンスに送信されます。
...
* hooks システムを備えた [OpenAI Codex CLI](https://github.com/openai/codex) 。
...
<div id="view-codex-traces-in-weave">
  ## Weave で Codex トレースを表示する {#view-codex-traces-in-weave}
</div>
...
<div id="wb-dedicated-cloud-or-self-hosted-instances">
  ### W&amp;B 専用クラウドまたはセルフホスト型インスタンス {#wb-dedicated-cloud-or-self-hosted-instances}
</div>
...
<div id="check-plugin-status">
  ## プラグインのステータスを確認する {#check-plugin-status}
</div>
...
<div id="no-traces-appear-after-running-codex">
  ### Codex を実行してもトレースが表示されない {#no-traces-appear-after-running-codex}
</div>
...
<div id="turns-appear-but-inputoutput-text-is-empty">
  ### ターンは表示されるが、入力/出力テキストが空です {#turns-appear-but-inputoutput-text-is-empty}
</div>
...
<div id="errors-sending-traces-to-weave">
  ### Weave へのトレース送信時のエラー {#errors-sending-traces-to-weave}
</div>
...
<div id="hook-locked-environments">
  ### フックが制限された環境 {#hook-locked-environments}
</div>
