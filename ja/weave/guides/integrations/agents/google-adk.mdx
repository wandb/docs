...
スクリプトの実行後、Weave が会話を取得できていることを確認するため、project の **Agents** ビューを開きます。`weave.init()` は実行時に、project へのリンクを出力します。**Agents** ビューで、次の内容を確認してください。

* **Agents** タブに `research_assistant` の行が 1 つ表示されること。
* 3 つのターンを含む 1 つの会話があること。
* 各ターンが、ネストされたモデルの Call とツール呼び出しを含む `invoke_agent` span として表示されること。
* 各ステップで、入力、モデル、出力、トークン使用量、Wikipedia の結果のすべてを確認できること。

Weave で Agents データを表示する方法について詳しくは、[エージェントのアクティビティを表示する](/ja/weave/guides/tracking/view-agent-activity) を参照してください。