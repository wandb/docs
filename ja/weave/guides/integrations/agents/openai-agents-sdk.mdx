...
[OpenAI Agents SDK](https://github.com/openai/openai-agents-python) は、OpenAI の API 上でエージェントやマルチエージェント ワークフローを構築するための軽量なフレームワークです。Weave は、OpenAI Agents SDK で構築されたエージェントを自動的にトレースし、各エージェントの呼び出し、サブエージェントへのハンドオフ、モデルの Call、ツール呼び出しを含むデータを取得します。Weave は、取得したデータを project の **Agents** ビューに表示します。

<Note>
  Weave TypeScript SDK は、OpenAI Agents SDK のパッチ適用をサポートしていません。
</Note>
...
スクリプトの実行後、Weave で取得されたアクティビティを確認できます。`weave.init()` は実行時に、使用中の project へのリンクを出力します。**Agents** ビューを開くと、次の内容を確認できます。

* **Agents** タブに `Research assistant` の行が表示されます。
* 3 つのターンを含む会話が表示されます。
* 各ターンは、ネストされたモデルの Call とツール呼び出しを含む `invoke_agent` span として表示されます。
* 各ステップで、完全な input、モデル、output、トークン使用量、Wikipedia の結果を確認できます。
