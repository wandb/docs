...
複数ターンの会話は、サーバー側でセッション ID によって関連付けられるため、複数のトレースにまたがって会話全体を追跡できます。各スパンには、トークン使用量、モデル名、tool の入力と出力、タイミング、およびプロンプトと応答のテキスト内容が含まれます。トレースされるデータの詳細については、GitHub の [What Gets Traced](https://github.com/wandb/weave-claude-code/blob/main/README.md#what-gets-traced) を参照してください。
