...
keywords: ["音声対音声", "音声アシスタント", "WebSockets", "Agents SDK", "オーディオトレース", "TypeScript"]
---
...
このガイドでは、アプリケーションで Realtime API のトレースを有効にする方法を説明し、ローカルで実行できる Python と TypeScript のエンドツーエンドの例を紹介します。

## リアルタイムトレースを統合する {#integrate-realtime-traces}
...
<Tabs>
  ...
  <Tab title="TypeScript">
    TypeScript では、Weave は [OpenAI Agents SDK](https://openai.github.io/openai-agents-js/) を通じて Realtime API をトレースします。Weave をインポートすると、`@openai/agents-realtime` のすべての `RealtimeSession` が自動的にインストルメントされるため、パッチ関数を呼び出す必要はありません。必要なのは Weave プロジェクトの初期化だけです。
    ...
    動的インポートや特定のバンドラーなど、Weave が自動インストルメンテーションに使用する Node.js モジュールローダーフックをセットアップでバイパスする場合は、アプリケーションがセッションを作成する前に、起動時に一度 [`patchRealtimeSession()`](/ja/weave/reference/typescript-sdk/functions/patchrealtimesession) を呼び出します。
...
### OpenAI Agents SDK を使用してリアルタイムエージェントを実行する {#run-a-realtime-agent-with-the-openai-agents-sdk}

<Tabs>
  <Tab title="Python">
    この例では、マイクのオーディオを OpenAI の Realtime API にストリーミングし、AI の音声応答をローカルマシンのスピーカーで再生するリアルタイム音声アシスタントを実行します。このアプリケーションは、OpenAI Agents SDK の `RealtimeAgent` と `RealtimeRunner` を使用し、`patch_openai_realtime()` でパッチを適用して Weave tracing を有効化します。より高レベルな Agents SDK に Realtime セッションの管理を任せたい場合は、この方法を使用してください。

    この例を実行するには、次の手順を実施します:
    ...
    アシスタントの実行中は、キーボードの `T` を押すとマイクのミュート/ミュート解除を切り替えられます。アシスタントはサーバー側の音声アクティビティ検出により、発話の切り替わりや割り込みを処理します。

    アシスタントに話しかけると、Weave はセッションのオーディオを含むトレースを取得し、Weave UI で確認できます。
  </Tab>

  <Tab title="TypeScript">
    この例では、`RealtimeAgent` と `RealtimeSession` を使用してリアルタイムエージェントを作成し、天気ツールを追加してテキストメッセージを送信します。Weave はインポート時にセッションを自動的にインストルメントし、セッション、生成、ツール呼び出し、オーディオ出力をトレースとして取得します。

    例を実行するには、次の手順を実行します:

    1. 新しいディレクトリで、TypeScript を使用する Node.js project を設定し、次のライブラリをインストールします:
    ...
    2. `weave_realtime_agent.ts` という名前のファイルを作成し、次のコードを追加します。

       ハイライトされた行は、アプリケーションにおける Weave のインテグレーションを示しています。残りのコードでは、基本的なリアルタイムエージェントを作成します。
       ...
       <Accordion title="weave_realtime_agent.ts">
         ...
         // エージェントが呼び出す、シミュレートされた天気ルックアップツール。
         ...
         async function main() {
           ...
           // Weave はインポート時に `@openai/agents-realtime` を自動的にインストルメントします。
           // モジュールローダーフックをバイパスする設定の場合は、次の行のコメントを解除します:
    ...
    3. `WEAVE_PROJECT` の値をチーム名とプロジェクト名に更新します。

    4. `OPENAI_API_KEY` と `WANDB_API_KEY` の環境変数を設定します。

    5. エージェントをコンパイルして実行します:
    ...
    最初の `--import` フラグは Weave のインストルメンテーションをプリロードし、2 番目の `--import` フラグは Node で TypeScript を実行できるように `tsx` をプリロードします。

    エージェントは Realtime API に接続し、San Francisco の天気について質問して天気ツールを呼び出した後、切断します。Weave UI では、セッション、生成、ツール呼び出し、アシスタントのオーディオ出力を含むトレースを確認できます。
  </Tab>
</Tabs>

### WebSockets を使用してリアルタイム音声アシスタントを実行する {#run-a-realtime-voice-assistant-with-websockets}

次の Python の例では、WebSockets 経由で OpenAI Realtime API に直接接続します。マイクのオーディオを API にストリーミングし、音声応答を再生し、ツール呼び出し (天気の検索、数式の評価、コードの実行、ファイルの書き込み) をサポートします。Weave は `weave.init()` と `patch_openai_realtime()` を使用してセッションをトレースします。Agents SDK に依存せずに Realtime セッションを完全に制御したい場合は、この方法を使用してください。
