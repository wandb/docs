...
`Module` は、プロンプト手法を抽象化する、学習可能なパラメーターを備えた DSPy プログラムの構成要素です。`Signature` は、DSPy Module の入出力の動作を宣言的に記述する仕様です。Weave は、DSPy プログラム内の組み込みおよびカスタムの Signature と Module をすべて自動的にトラッキングします。
