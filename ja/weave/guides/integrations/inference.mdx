...
利用可能な主要な API method は `/chat/completions` です。これは、サポートされるモデルにメッセージを送信して補完を受け取るための OpenAI 互換の Request 形式をサポートします。Weave で W&amp;B Inference サービスを使用する方法を示す使用例については、[API usage examples](#usage-examples)を参照してください。
