...
## 独自のopでCallをラップする {#wrap-calls-with-your-own-ops}
...
## より簡単に試行錯誤できるように `Model` を作成する {#create-a-model-for-easier-experimentation}
...
### Weave Playground で Bedrock を試す {#try-bedrock-in-the-weave-playground}

設定なしで Weave UI から Amazon Bedrock モデルを試すには、[LLM Playground](../tools/playground) をお試しください。

### レポート: Weave で Bedrock 上の LLM をテキスト要約タスクで比較する {#report-compare-llms-on-bedrock-for-text-summarization-with-weave}
