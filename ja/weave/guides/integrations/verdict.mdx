...
始めるには、スクリプトの冒頭で `weave.init(project=...)` を呼び出すだけです。`project` 引数を使って、`team-name/project-name` のように指定すれば特定の W&amp;B Team 名にログでき、`project-name` とすればデフォルトの team/entity にログできます。
...
ここでは、Weave がネストされたパイプライン処理をどのようにトレースするかを示す、より複雑な例を紹介します。
...
Weave は、すべてのパイプライン処理のタイミング情報を自動的に取得します。これを使って、パフォーマンスのボトルネックを特定できます。
