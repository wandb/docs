...
`weave.init()` によって提供される自動ログは、LangChain アプリケーション内のすべてのコンポーネントにコンストラクターコールバックを渡すのとよく似ています。つまり、プロンプトテンプレート、チェーン、LLM 呼び出し、ツール、エージェントステップなどを含むすべてのやり取りが、アプリケーション全体にわたってグローバルにトラッキングされます。
