...
この重複を避けるには、評価を実行する前にデータセットを Weave に公開し、公開済みデータセットの行を `inputs` として渡してください。Weave は、データを再取り込みする代わりに、内部参照を使用して公開済みの行への参照を解決します。これにより、標準の [Evaluation framework](../core-types/evaluations) と同様に、各予測が Weave UI 内の特定のデータセット行にリンクされる形で扱えます。
