...
keywords: ["EvaluationLogger", "log_prediction", "log_score", "段階的な評価", "リッチメディアのログ記録"]
---
...
<div id="simplified-logging-with-log_example">
  ## `log_example()` を使用した簡易ログ記録 {#simplified-logging-with-log_example}
</div>
...
<div id="use-context-managers">
  ### コンテキストマネージャーを使用する {#use-context-managers}
</div>
...
* 予測コンテキスト内で、モデル実行後に output を設定できる。
...
<Tabs>
  <Tab title="Python">
    ...
    このパターンにより、ネストされたすべての操作が親予測にひも付けられてトラッキングされるため、Weave UI で正確なトークン使用量 と cost data を確認できます。
  </Tab>

  <Tab title="TypeScript">
    TypeScript には、コンテキストマネージャー向けの Python の `with` 文のようなパターンはありません。代わりに、`finish()` を明示的に呼び出す fire-and-forget パターンを使用します。

    次の例では、予測をログし、スコアと LLM judge score を追加した後、`finish()` で予測を完了します。
    ...
    <Note>
      TypeScript にはコンテキストマネージャーによる自動クリーンアップはありませんが、`logSummary()` は結果を集計する前に、未完了の予測を自動的に finish します。`finish()` を明示的に呼び出したくない場合は、この動作に任せることもできます。
    </Note>
  </Tab>
</Tabs>

<div id="link-agent-traces-to-evaluations">
  ### エージェント トレースを評価にリンクする {#link-agent-traces-to-evaluations}
</div>
...
<div id="link-spans-you-instrument-yourself">
  #### 自分でインストルメントしたスパンをリンクする {#link-spans-you-instrument-yourself}
</div>
...
<div id="link-an-agent-that-runs-in-a-separate-service">
  #### 別のサービスで実行されるエージェントをリンクする {#link-an-agent-that-runs-in-a-separate-service}
</div>
...
<div id="view-linked-agent-spans-from-your-evaluations">
  #### 評価からリンクされたエージェント スパンを表示する {#view-linked-agent-spans-from-your-evaluations}
</div>
...
### 既存のデータセットにリンクする {#link-to-an-existing-dataset}
...
<div id="get-outputs-before-logging">
  ### ログする前に出力を取得する {#get-outputs-before-logging}
</div>
...
### 複数の評価をログして比較する {#log-and-compare-multiple-evaluations}
...
<Frame>
  ![評価 run のリストを表示する Evals タブ](/weave/guides/evaluation/img/evals_tab.png)
</Frame>

<Frame>
  ![複数の評価 run にわたるメトリクスを表示する Compare ビュー](/weave/guides/evaluation/img/comparison.png)
</Frame>
