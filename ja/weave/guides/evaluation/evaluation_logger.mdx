...
説明したワークフローを示す Python コードについては、[基本例](#basic-example)を参照してください。

<div id="basic-example">
  ## 基本例
</div>

次の例では、`EvaluationLogger` を使用して、既存のコード内で予測とスコアを直接ログする方法を示します。
...
<Tabs>
  <Tab title="Python">
    ...
    * `finish()` はその予測のログ記録を完了します。
    ...
    def user_model(a: int, b: int) -> int:
        ...
        # レスポンスを何らかの形で使用する（ここでは簡略化のためa + bを返すだけ）
        return a + b

    # サンプルを反復処理し、予測してログする
    for sample in eval_samples:
        inputs = sample["inputs"]
        model_output = user_model(**inputs) # 引数をkwargsとして渡す
    ...
    # 評価全体の最終サマリーをログする。
    # Weaveは上記でログされた'correctness'スコアを自動集計する。
    summary_stats = {"subjective_overall_score": 0.8}
    eval_logger.log_summary(summary_stats)

    print("評価のロギングが完了しました。Weave UIで結果を確認してください。")
    ```
  </Tab>
  ...
  <Tab title="TypeScript">
    ...
    * **シンプル**: ほとんどの評価シナリオで、定型コードが少なく、構文もすっきりします
    ...
    // サンプルを反復処理し、fire-and-forgetパターンで予測とログを実行する
...
`EvaluationLogger` は、予測とスコアの両方でコンテキストマネージャー (`with` 文) をサポートしています。これにより、コードをよりすっきり書けるほか、リソースの自動クリーンアップや、LLM judge の call のようなネストされた処理をより適切にトラッキングできます。

このコンテキストで `with` 文を使用すると、次の利点があります。

* コンテキストを抜けるときに `finish()` が自動的に呼び出される
* ネストされた LLM call のトークン数やコストをより適切にトラッキングできる
...
<Tabs>
  <Tab title="Python">
    ...
    # prediction でコンテキストマネージャーを使用 - finish() を呼び出す必要はありません
    with ev.log_prediction(inputs={"user_prompt": user_prompt}) as pred:
        ...
        # LLM call が必要なスコアには、ネストしたコンテキストマネージャーを使用
    ...
    このパターンにより、ネストされたすべての処理がトラッキングされて親の予測に紐付けられるため、Weave UI で正確なトークン使用量とコストデータを確認できます。
  </Tab>

  <Tab title="TypeScript">
    TypeScript には、コンテキストマネージャー向けの Python の `with` 文パターンはありません。代わりに、明示的に `finish()` を呼び出す fire-and-forget パターンを使用します。

    次の例では、予測をログし、シンプルなスコアと LLM judge スコアを追加してから、`finish()` で予測を終了します。
    ...
    // LLM judge スコアについては、call を行って結果をログする
    ...
    <Note>
      TypeScript にはコンテキストマネージャーによる自動クリーンアップはありませんが、`logSummary()` は結果を集計する前に、未完了の予測を自動的にすべて終了します。`finish()` を明示的に呼び出したくない場合は、この動作を利用できます。
    </Note>
  </Tab>
</Tabs>
...
この重複を避けるには、評価を実行する前にデータセットを Weave に公開し、その公開済みデータセットの行を `inputs` として渡してください。Weave はデータを再取り込みする代わりに、公開済みの行への参照を内部参照として解決します。これにより、標準の [評価フレームワーク](../core-types/evaluations) と同様に、各予測が Weave UI 内の特定のデータセット行にリンクされるようになります。
...
まずモデルの出力を計算し、その後で予測とスコアを別々にログできます。これにより、評価ロジックとログするロジックをより明確に分離できます。

<Tabs>
  <Tab title="Python">
    ```python lines
    # トークンをトラッキングできるよう、モデルを呼び出す前に EvaluationLogger を初期化します
    ev = EvaluationLogger(
        model="example_model",
        dataset="example_dataset"
    )

    # トークンをトラッキングするため、モデルの出力（例: OpenAI の call）はロガーの初期化後に行う必要があります
  ...
  <Tab title="TypeScript">
    複数の予測を並列に処理する場合、fire-and-forget パターンは特に効果的です。

    次の例では、`EvaluationLogger` の複数の同時実行インスタンスを作成し、評価を並列にバッチ処理します。
    ...
    // トークンをトラッキングするため、モデルの出力（OpenAI の call など）はロガーの初期化後に行う必要があります
...
入力、出力、スコアには、画像、動画、オーディオ、構造化テーブルなどのリッチメディアを含めることができます。`log_prediction` または `log_score` メソッドに `dict` またはメディアオブジェクトを渡すだけです。
...
<Tabs>
  ...
  <Tab title="TypeScript">
    TypeScript SDK は、`weaveImage` 関数と `weaveAudio` 関数を使って画像とオーディオをログすることをサポートしています。次の例では、画像ファイルとオーディオファイルを読み込み、それらをモデルで処理し、スコア付きで結果をログします。
  ...
  <Tab title="TypeScript">
    ...
    for (const model of models) {
      ...
      for (const inputs of yourDataset) {
        ...
        // シンプルで効率的にログするための fire-and-forget パターン
        const pred = ev.logPrediction(inputs, output);
...
<Frame>
  ![Compare ビュー](/weave/guides/evaluation/img/comparison.png)
</Frame>
