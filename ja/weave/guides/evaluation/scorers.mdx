...
## 独自のスコアラーを作成する {#create-your-own-scorers}
...
### 関数ベースのスコアラー {#function-based-scorers}
...
### クラスベースの Scorer {#class-based-scorers}
...
### Scorer のキーワード引数 {#scorer-keyword-arguments}
...
### `column_map` を使用した列名のマッピング {#mapping-column-names-with-column_map}
...
### `columnMapping` を使用した列名のマッピング {#mapping-column-names-with-columnmapping}
...
### スコアリングプロンプトで op の変数にアクセスする {#access-variables-from-your-ops-in-scoring-prompts}
...
### Scorer の最終集計 {#final-summarization-of-the-scorer}
...
### Call に Scorer を適用する {#apply-scorers-to-a-call}
...
独自のインフラストラクチャー上で実行される Scorer で本番の Call をスコアリングするには、[リモート Scorer](/ja/weave/guides/evaluation/remote-scorers) をモニターに関連付けます。
...
### `preprocess_model_input` を使用する {#use-preprocess_model_input}
...
### 単一のcallのスコアを分析する {#analyze-a-single-calls-scores}

#### 単一Call API {#single-call-api}
...
### 複数のCallのスコアを分析する {#analyze-multiple-calls-scores}

#### 複数のCalls API {#multiple-calls-api}
...
#### 複数の Call の UI {#multiple-calls-ui}
...
### 特定のScorerでスコア付けされたすべてのCallを分析する {#analyze-all-calls-scored-by-a-specific-scorer}

#### ScorerごとのすべてのCall API {#all-calls-by-scorer-api}
...
#### Scorer ごとのすべての Call を UI で確認する {#all-calls-by-scorer-ui}
