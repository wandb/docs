...
<Tip>
  **すぐに使える Scorer**
  このガイドではカスタム Scorer の作成方法を説明しますが、Weave にはすぐに使えるさまざまな [事前定義済み Scorer](./builtin_scorers) や [ローカル SLM Scorer](./weave_local_scorers) が用意されています。たとえば次のようなものがあります:
...
単一の Call に対する Scores は、Call 詳細パネルの「Scores」タブに表示されます。
