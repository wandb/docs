...
<Note>
  カスタムモニターは、本番トラフィックをモニタリングする従来の方法です。新しい実装では、Weave for Agents の **シグナル** を使用してください。[エージェントのシグナルを表示する](/ja/weave/guides/tracking/view-agent-signals) を参照してください。
</Note>
...
LLM ジャッジモデルではなく、自身でホストする HTTP エンドポイントで Call をスコアリングする場合は、[リモート Scorer で Call をスコアリングする](/ja/weave/guides/evaluation/remote-scorers) を参照してください。

## シグナル と カスタムモニター {#signals-and-custom-monitors}
...
## Weave でモニターを作成する {#create-a-monitor-in-weave}
...
### 例: 真実性モニターを作成する {#example-create-a-truthfulness-monitor}
