...
<Tab title="Python">
  ...
  詳細については、[RAG アプリケーションのモデルベース評価](/ja/weave/tutorial-rag#optional-defining-a-scorer-class) の `Scorer` クラスの定義に関するチュートリアルを参照してください。
</Tab>
<Tab title="TypeScript">
...
<Tab title="Python">
  ...
  現在のアプローチについては、[評価での HuggingFace データセットの使用クックブック](/ja/weave/cookbooks/hf_dataset_evals) を参照してください。
</Tab>
<Tab title="TypeScript">
  ```plaintext
...
Evals テーブルの設定、フィルタ、ソートを _保存済みビュー_ として保存し、好みのセットアップに素早くアクセスできます。保存済みビューは UI および Python SDK で設定・アクセスできます。詳細については、[保存済みビュー](/ja/weave/guides/tools/saved-views) を参照してください。
