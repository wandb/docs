...
* *object&#95;name*: オブジェクト名
* *object&#95;version*: バージョンハッシュ、`v0` や `v1` のような文字列、または `:latest` のようなエイリアスのいずれかです。すべてのオブジェクトには `:latest` エイリアスがあります。
...
本番システムにプロンプトをデプロイする場合は、最新バージョンを参照するのではなく、特定のプロンプトのバージョンを固定してください。本番アプリケーションでは、`weave.ref(<name>:<version>).get()` を使ってそのバージョンを読み込みます。

バージョンを固定しておくと、あとから新しいプロンプトのバージョンが公開されても、本番環境の動作を安定した状態に保てます。

一般的なワークフローは次のとおりです。

1. 新しいプロンプトのバージョンを開発してテストします。
2. 新しいプロンプトをデータセットまたは評価スイートに対して評価します。
3. 本番システムが新しいバージョンを参照するように更新します。

この方法により、チームは本番環境の動作を意図せず変えることなく、安全にプロンプトを反復的に改善できます。
...
2. Weave プロジェクトのサイドバーで **Assets** をクリックします。Assets ページが開きます。
...
5. (任意) 一覧に表示されたプロンプトの横にあるチェックボックスを選択し、表のツールバーにある **Compare** ボタンをクリックすると、プロンプトのバージョンを比較できます。これにより、プロンプト間の差分を確認できます。
