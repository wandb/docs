...
<Tip>
[Audio Logging](/ja/weave/cookbooks/audio_with_weave) のクックブックをお試しください。このクックブックには、Weave と統合されたリアルタイムオーディオ API ベースのアシスタントの高度な例も含まれています。
</Tip>
...
この例では、[W&B Inference](/ja/inference/) を使用して自己完結型 HTML ページを生成し、そのページを Weave にログ記録します：
...
クラスの属性とメソッドの包括的なリストについては、[Content リファレンスドキュメント](/ja/weave/reference/python-sdk#class-content) をご覧ください。
