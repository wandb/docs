---
title: "モデルを比較して順位付けする"
description: "評価メトリクスに基づいて異なるモデルバージョンを比較・順位付けする"
---

Weave _Leaderboards_ を使用して、複数のメトリクスにわたって複数のモデルを評価および比較し、精度、生成品質、レイテンシ、またはカスタム評価ロジックを測定します。リーダーボードを使うと、モデル性能を一元的な場所で可視化し、時間の経過に伴う変化を追跡し、チーム全体でのベンチマークを統一できます。
...
<Note>
リーダーボードの作成は Weave UI と Weave Python SDK でのみ利用できます。TypeScript ユーザーは [Weave UI](#using-the-ui) を使用してリーダーボードを作成および管理できます。
</Note>
...
[Weave UI](#ui) から、または [プログラムから](#python) 作成できます。
...
<Tip>
完全な実行可能コードサンプルをお探しの場合は、[End-to-end Python example](#end-to-end-python-example) を参照してください。
</Tip>

リーダーボードを作成して公開するには、次の手順を実行します。

1. テストデータセットを定義します。組み込みの [`Dataset`](./datasets) を使用するか、入力とターゲットのリストを手動で定義します。
...
2. 1つ以上の [scorer](../evaluation/scorers) を定義します。
...
次の例では、Weave Evaluations を使用してカスタムメトリクスに基づき、共通のデータセット上で 3 つの要約モデルを比較するためのリーダーボードを作成します。小さなベンチマークを作成し、各モデルを評価して、[Jaccard similarity](https://www.learndatasci.com/glossary/jaccard-similarity/) を用いてスコアリングし、その結果を Weave のリーダーボードに公開します。
...
Leaderboard テーブルでは、各行が特定のモデル（`model_humanlike`、`model_vanilla`、`model_messy`）を表します。`mean` 列には、そのモデルの出力と参照要約との平均 Jaccard 類似度が表示されます。

<Frame>
![Weave UI の Leaderboard](/weave/guides/core-types/imgs/leaderboard-example.png)
</Frame>
