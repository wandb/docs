...
description: "W&B Inference を使ってモデル呼び出しをトレースし、出力を比較し、評価を実行しながら、Weave の基本を学びます。"
---
...
このガイドでは、W&amp;B Weave を [W&amp;B Inference](/ja/inference/) と組み合わせて使用する方法を説明します。W&amp;B Inference を使うと、自前のインフラストラクチャを用意したり、複数のプロバイダーの API キーを管理したりすることなく、稼働中のオープンソースのモデルを使って LLM アプリケーションを構築およびトレースできます。W&amp;B API キーがあれば、[W&amp;B Inference によってホストされているすべてのモデル](/ja/inference/models/) と対話できます。
...
利用可能なモデルの完全な一覧については、W&amp;B Inference ドキュメントの [Available Models セクション](/ja/inference/models/) を参照してください。
...
<details>
  ...
  * 上限を増やすためにプランのアップグレードを検討する
  ...
  詳細については、[W&amp;B Inference の制限に関するドキュメント](/ja/inference/usage-limits/)を参照してください。
</details>

<details>
  <summary>クレジット不足</summary>

  無料プランには利用可能なクレジット数に制限があります。詳しくは、[使用量と制限に関するドキュメント](/ja/inference/usage-limits/)を参照してください。
</details>