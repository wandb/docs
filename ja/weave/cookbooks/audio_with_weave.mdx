...
<div id="how-to-use-weave-with-audio-data-an-openai-example">
  # Weave を音声データと組み合わせて使う方法: OpenAI の例
</div>

このデモでは、OpenAI chat completions API と GPT 4o Audio Preview を使用して、テキストプロンプトに対する音声応答を生成し、それらを Weave で追跡します。

<img src="https://i.imgur.com/OUfsZ2x.png" alt="GPT 4o Audio Preview インテグレーションと音声応答生成ワークフローを備えた OpenAI chat completions API インターフェース" />

より高度なユースケースでは、OpenAI Realtime API を利用して音声をリアルタイムにストリーミングします。ビデオ デモを見るには次のサムネイルをクリックするか、[こちら](https://www.youtube.com/watch?v=lnnd73xDElw) をクリックしてください。
...
まず、OpenAI (`openai`) と Weave (`weave`) の依存関係に加え、APIキー を管理するための依存関係である `set-env` をインストールします。
...
次に、OpenAI と Weave 用の必要な APIキー を読み込みます。ここでは set&#95;env を使用します。これは Google Colab のシークレットキー マネージャーと互換性があり、Colab 固有の `google.colab.userdata` の代替となります。使い方については [こちら](https://pypi.org/project/set-env-colab-kaggle-dotenv/) を参照してください。
...
ここでは、OpenAI completions のリクエストを定義し、Weave のデコレーター（op）を追加します。

ここで、関数 `prompt_endpont_and_log_trace` を定義します。この関数は主に 3 つのステップから成ります。

1. テキストおよび音声の入力と出力をサポートする `GPT 4o Audio Preview` モデルを使って completion オブジェクトを作成します。

   * モデルに対して、さまざまなアクセントでゆっくり 13 まで数えるように指示します。
   * completion を `"stream"` に設定します。

2. ストリーミングされたデータを書き込む新しい出力ファイルを開き、チャンクごとに書き込みます。

3. Weave が trace に音声データをログできるように、その音声ファイルに対するオープンなファイルハンドラーを返します。
...
次のセルを実行します。システム プロンプトとユーザー プロンプトは、出力された音声とともに Weave trace に保存されます。
セルを実行したら、「🍩」絵文字の横にあるリンクをクリックして trace を表示します。
...
<img src="https://i.imgur.com/ZiW3IVu.png" alt="Weave と Realtime Audio API のインテグレーションおよびストリーミング音声会話インターフェース" />
...
利用可能なすべてのオーディオデバイスを確認するには、次のセルを実行します。その後、表示されたデバイス一覧に基づいて `INPUT_DEVICE_INDEX` と `OUTPUT_DEVICE_INDEX` を設定します。入力デバイスには入力チャネルが少なくとも 1 つ、出力デバイスには出力チャネルが少なくとも 1 つあります。
...
python
INPUT_DEVICE_INDEX = 3  # @param                                                 # 上記のデバイスリストに基づいて選択する。デバイスの入力チャンネル数が 0 より多いことを確認すること。
OUTPUT_DEVICE_INDEX = 12  # @param                                                # 上記のデバイスリストに基づいて選択する。デバイスの出力チャンネル数が 0 より多いことを確認すること。
...
SAMPLE_WIDTH = p.get_sample_size(pyaudio.paInt16)  # このフォーマットにおけるフレームあたりのサンプル数
CHUNK_DURATION = 0.3  # OAI API に送信する 1 チャンクあたりの音声の長さ（秒）
OAI_SAMPLE_RATE = (
    24000  # OAI のサンプルレートは 24kHz。アシスタントの音声を再生または保存する際に必要
)
...
OpenAI Python SDK は現時点では Realtime API をサポートしていません。可読性を高めるために Pydantic で OAI Realtime API スキーマ全体を実装しており、公式サポートがリリースされ次第、この実装は非推奨とする可能性があります。
...
リアルタイム (RT) オーディオ モデルは websocket を使用してイベントを OpenAI の Realtime audio API に送信します。処理の流れは次のとおりです。

1.  **init:** ローカル バッファ（入力オーディオ）とストリーム（アシスタント再生ストリーム、ユーザー音声ディスク書き込みストリーム）を初期化し、Realtime API への接続を開きます。
2.  **receive_messages_thread**: スレッドが API からのメッセージ受信を処理します。4 つの主要なイベントタイプを処理します: - RESPONSE_AUDIO_TRANSCRIPT_DONE:

            サーバーはアシスタントのレスポンスが完了したことを示し、文字起こし結果を提供します。

        - CONVERSATION_ITEM_INPUT_AUDIO_TRANSCRIPTION_COMPLETED:

            サーバーはユーザーの音声が文字起こしされたことを示し、ユーザー音声の文字起こし結果を送信します。この文字起こしを Weave にログとして記録し、ユーザー向けに表示します。

        - RESPONSE_AUDIO_DELTA:

            サーバーは新しいアシスタントのレスポンス音声のチャンクを送信します。これをレスポンス ID を用いて進行中のレスポンス データに追加し、再生用に出力ストリームに追加します。

        - RESPONSE_DONE:

            サーバーはアシスタントのレスポンスが完了したことを示します。レスポンスに関連付けられたすべての音声チャンクと文字起こしを取得し、これらを Weave にログとして記録します。

    3.**send_audio**: ハンドラーがユーザー音声のチャンクをバッファに追加し、オーディオ バッファが一定サイズに達したときに音声チャンクを送信します。
...
メインスレッドは、Weave を組み込んだ Realtime Audio モデルを初期化します。次に録音を開始し、ユーザーからのキーボード割り込みを待ちます。
