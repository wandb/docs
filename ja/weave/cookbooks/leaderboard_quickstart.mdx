---
title: "リーダーボード クイックスタート"
description: "W&B Weave のリーダーボード クイックスタートの使い方を学ぶ"
---
...
- [GitHub でソースコードを表示](https://github.com/wandb/docs/blob/main/weave/cookbooks/source/leaderboard_quickstart.ipynb)
</Note>
...
このノートブックでは、Weave の Leaderboard を使って、異なるデータセットやスコアリング関数にまたがるモデルのパフォーマンスを比較する方法を学びます。具体的には次のことを行います。

1. 架空の郵便番号データのデータセットを生成する
2. いくつかのスコアリング関数を作成し、ベースラインとなるモデルを評価する
3. これらの手法を使って、モデルと評価（スコアリング）の組み合わせを評価する
...
1. `check_concrete_fields`: モデルの出力が、期待される都市名と州名に一致しているかを確認します。
2. `check_value_fields`: モデルの出力が、期待される人口と世帯収入中央値の 10% 以内に収まっているかを確認します。
3. `check_subjective_fields`: モデルの出力が、期待される「known for」フィールドに一致しているかを LLM を使って確認します。
...
次に、ダミーデータとスコアリング関数を使って簡単な評価を定義します。
...
ここでは、静的なレスポンスを返すベースラインモデルを評価します。
...
ここでは、ベースラインと比較するためのモデルを 2 つ追加で作成します。
...
ここでは、モデルと評価の組み合わせ（マトリクス）に対して評価を実行します。
...
UI の「Leaderboard」タブに移動し、「Create Leaderboard」をクリックすると、新しいリーダーボードを作成できます。

Python からリーダーボードを直接生成することもできます。
