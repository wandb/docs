...
> このガイドでは、HuggingFace Datasets を Weave の評価で使用するための回避策を説明します。<br /><br/>
...
> **Note:**<br />
> インデックス内では、各行が一意の識別子を持つようにするために、`hf_id` とともに `hf_hub_name` をエンコードします。
> この一意のダイジェスト値は、評価時にデータセット内の特定のエントリを追跡・参照するために使用されます。
