...
### `weave.op` を使用して入出力のロギングをカスタマイズする {#customize-input-and-output-logging-using-weaveop}
...
### PII データで Weave を使用する際のベストプラクティス {#best-practices-for-weave-with-pii-data}
...
#### 本番環境では {#in-production}

* 未加工のPIIは絶対にログしない。
* ログする前に機微なフィールドを暗号化する。

#### 暗号化のポイント {#encryption-tips}
...
## マスキング方法の概要 {#redaction-methods-overview}
...
### 方法 1: 正規表現を使用してフィルタリングする {#method-1-filter-using-regular-expressions}
...
### 方法 2: Microsoft Presidio を使用してマスクする {#method-2-redact-using-microsoft-presidio}
...
### 方法 3: Faker と Presidio を使用して置換で匿名化する {#method-3-anonymize-with-replacement-using-faker-and-presidio}
...
### Method 4: `autopatch_settings` を使用する {#method-4-use-autopatch_settings}
...
## Weave Call にメソッドを適用する {#apply-the-methods-to-weave-calls}
...
### Presidio を使用したマスキング方法 {#presidio-redaction-method}
...
### Faker と Presidio の置換方法 {#faker-and-presidio-replacement-method}
...
### `autopatch_settings` メソッド {#autopatch_settings-method}
...
### オプション: データを暗号化する {#optional-encrypt-your-data}
