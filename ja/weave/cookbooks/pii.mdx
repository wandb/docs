...
さらに、_`weave.op` の入力/出力ログのカスタマイズ_ と _`autopatch_settings`_ を使用して、PII の墨消しと匿名化をワークフローに統合する方法についても学びます。詳細については、[ログに記録される入力と出力のカスタマイズ](/ja/weave/guides/tracking/ops/#customize-logged-inputs-and-outputs) を参照してください。
