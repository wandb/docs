...
description: "W&B Weave でマルチエージェント構造化出力の使い方を学ぶ"
---
...
<div id="structured-outputs-for-multi-agent-systems">
  # マルチエージェントシステムにおける構造化出力
</div>

OpenAI は、強く制約したプロンプトを使わなくても、指定した JSON Schema に常に準拠した応答をモデルが生成するように保証できるよう、[Structured Outputs](https://openai.com/index/introducing-structured-outputs-in-the-api/) をリリースしました。Structured Outputs を使うことで、形式が正しくない応答を検証したり再試行したりする必要がなくなります。

新しいパラメータ `strict: true` を使用することで、応答が与えられたスキーマに従うことを保証できます。

マルチエージェントシステムで構造化出力を使用すると、エージェント間で一貫性があり処理しやすいデータを確保できるため、コミュニケーションが向上します。また、明示的な拒否を可能にすることで安全性が高まり、再試行や検証の必要がなくなることでパフォーマンスも向上します。これにより、インタラクションが簡素化され、システム全体の効率が高まります。

このチュートリアルでは、マルチエージェントシステムで構造化出力を活用し、それらを [Weave](/ja/weave) でトレースする方法を説明します。

<Tip>
  **出典**: このクックブックは、[OpenAI の Structured Outputs のサンプルコード](https://cookbook.openai.com/examples/structured_outputs_multi_agent) をベースにしており、Weave を用いた可視化を改善するためにいくつかの変更を加えています。
</Tip>
...
* LLMワークフローを追跡し、プロンプト戦略を評価するための [Weave](/ja/weave)。
...
`WANDB_API_KEY` を環境変数に設定しておくことで、wandb.login() で簡単にログインできるようにします（これは Colab にはシークレットとして渡す必要があります）。

W&amp;B でログを記録したいプロジェクトを `name_of_wandb_project` に設定します。

**NOTE**: `name_of_wandb_project` は、トレースを記録するチームを指定する場合、`{team_name}/{project_name}` という形式にすることもできます。
その後、weave.init() を呼び出して Weave クライアントを取得します。

[OpenAI API](https://openai.com/index/openai-api/) を使用するので、OpenAI の APIキー も必要です。OpenAI プラットフォーム上で[サインアップ](https://platform.openai.com/signup)することで、自分の APIキー を取得できます（これも Colab にはシークレットとして渡す必要があります）。
...
ここでは、ユースケースとしてデータ分析タスクを扱います。
まず、4 つのエージェントからなるシステムをセットアップします。

* トリアージエージェント: どのエージェントを呼び出すかを決定する
* データ前処理エージェント: たとえばデータのクリーニングなどを行って、分析用にデータを準備する
* データ分析エージェント: データの分析を実行する
* Data Visualization エージェント: 分析結果を可視化してインサイトを抽出する
  まずは、これら各エージェントに対するシステムプロンプトを定義します。
...
次に、各エージェント用のツールを定義します。

トリアージエージェントを除き、各エージェントにはその役割に特化したツールを与えます。

**データ前処理エージェント** : 1. データのクレンジング, 2. データ変換, 3. データ集約
...
* ユーザーからのクエリをマルチエージェントシステムに渡す処理
* マルチエージェントシステムの内部動作を扱う処理
* ツール呼び出しを実行する処理
...
まず、ツール呼び出しの実行を担う関数を定義します。

Python 関数に `@weave.op()` を付与することで、言語モデルの入力・出力・トレースをログに記録し、デバッグできます。

マルチエージェントシステムを作成する際には多くの関数が出てきますが、それらの関数の定義の上に `@weave.op()` を追加するだけで十分です。
...
次に、各サブエージェントごとにツールハンドラーを作成します。それぞれに、モデルに渡される固有のプロンプトとツールセットを持たせます。生成された出力は、ツール呼び出しを実行する関数に渡されます。
...
最後に、ユーザーのクエリ全体の処理を担う統括的なツールを作成します。この関数はユーザーのクエリを受け取り、モデルからレスポンスを取得し、それを他のエージェントに渡して実行させます。
...
最後に、ユーザーの入力を使って主要な `handle_user_message` 関数を実行し、その結果を確認します。

```python lines
handle_user_message(user_query)
```

Weave の URL をクリックすると、実行が次のようにトレースされていることがわかります。Traces ページでは、入力と出力を確認できます。わかりやすくするために、それぞれの出力をクリックしたときに表示される結果のスクリーンショットを図に追加しています。Weave は OpenAI の API とのインテグレーションを提供しており、コストを自動的に計算できます。そのため、右端にコストとレイテンシも表示されていることが確認できます。
![1-1.png](/media/multi-agent-structured-output/1-1.png)

各行をクリックすると、マルチエージェントシステム内で実行された中間処理を確認できます。たとえば、`analysis_agent` の入力と出力を見ることで、それが構造化出力形式になっていることがわかります。OpenAI の構造化出力は、エージェント間の連携を容易にしますが、システムが複雑になるにつれて、これらのやり取りがどのような形式で行われているのか把握しづらくなります。Weave を使うことで、これらの中間処理とその入出力を、まるで手に取るように理解できます。

<Frame>
  ![3.png](/media/multi-agent-structured-output/3.png)
</Frame>

ぜひ Weave でのトレースがどのように行われているか、じっくり確認してみてください。

<div id="conclusion">
  ## まとめ
</div>

このチュートリアルでは、構造化出力と Weave を活用し、入力・最終出力・中間的な出力フォーマットを追跡するために OpenAI が提供する機能を用いて、マルチエージェントシステムを手軽に開発する方法を学びました。