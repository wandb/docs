...
次のコードは、前の step で開始した複雑な親トレースに子スパンを追加する方法を示しています。この step では、全体の ワークフロー における RAG ドキュメント検索のサブ 操作 を表現しています。
...
次のコードは、親トレースを完了してワークフロー全体の終了を示す方法を示しています。この step では、すべての子 スパン (たとえば、RAG lookup や LLM completion) の結果を集約し、最終的な出力とメタデータをログします。
