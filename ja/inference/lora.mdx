...
<Tabs>
  <Tab title="他の環境でトレーニングした LoRA をアップロードする">
    ...
    * LoRA は、[サポートされているベースモデル セクション](#supported-base-models) に記載されているいずれかのモデルを用いてトレーニングされている必要があります。
  ...
  <Tab title="W&B で新しい LoRA をトレーニングする">
    [W&amp;B Training (serverless RL)](/ja/training) を使用して新しい LoRA をトレーニングします。トレーニングした LoRA は自動的に W&amp;B アーティファクト になり、そのまま利用できます。
...
LoRA がアーティファクトとしてプロジェクトに追加されたら、次のように推論呼び出しでそのアーティファクト の URI を使用します。
...
Inference は現在、以下の LLM をサポートしています (`wandb.base_model` ではこれらの文字列をそのまま使用してください)。今後さらに多くのモデルが追加される予定です。

{/* takeru lora-base-models - この表は自動生成されるため、手動で編集しないでください。 */}

| モデル ID (API 使用時)                    | 最大 LoRA ランク |
| ----------------------------------- | ----------- |
