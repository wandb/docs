...
description: >
  Serverless Inference でファインチューニングしたモデルを提供するために、独自の LoRA を持ち込みます。
---
...
以下は、Serverless Inference を使用してカスタム LoRA モデルを呼び出す例です。
...
<Tabs>
  ...
  <Tab title="W&B で新しい LoRA をトレーニングする">
    [Serverless RL](/ja/serverless-rl) を使用して新しい LoRA をトレーニングします。LoRA は自動的に W&amp;B アーティファクト となり、そのまま直接使用できます。
