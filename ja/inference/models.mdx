...
次のモデルは[非推奨](/ja/inference/lifecycle#model-lifecycle-stages)です。

{/* takeru inference-models-deprecated - この表は自動生成されています。手動で編集しないでください。 */}

| モデル                          | モデル ID (API で使用)                               | タイプ  | コンテキストウィンドウ | パラメーター                  | 説明                                                                             |
| ---------------------------- | ---------------------------------------------- | ---- | ----------- | ----------------------- | ------------------------------------------------------------------------------ |
| MiniMax M2.5                 | `MiniMaxAI/MiniMax-M2.5`                       | テキスト | 197k        | 10B-230B (Active-Total) | 高スループット、低レイテンシ、優れたコーディング能力を実現するために設計された、非常にスパースなアーキテクチャを持つ MoE モデル。            |
| NVIDIA Nemotron 3 Super 120B | `nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-FP8` | テキスト | 262k        | 12B-120B (Active-Total) | Nemotron 3 は、優れたエージェント機能、推論、会話能力を提供するよう設計された LatentMoE モデルです。                  |
| Qwen3 Coder 480B A35B        | `Qwen/Qwen3-Coder-480B-A35B-Instruct`          | テキスト | 262k        | 35B-480B (Active-Total) | 関数呼び出し、ツールの使用、長いコンテキストでの推論などのエージェント型コーディングタスク向けに最適化された Mixture-of-Experts モデル。 |
| Z.AI GLM 5.1                 | `zai-org/GLM-5.1`                              | テキスト | 203k        | 40B-744B (Active-Total) | 長期にわたるエージェント型エンジニアリングと高度な推論に対応する強力な MoE モデル。                                   |
