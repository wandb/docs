...
次の表は、Serverless Inference で 推論 output を返すモデルを示しています。サポートされる各モデルでは、推論 が常に含まれるか、既定で 推論 が無効または有効になっています。

{/* takeru reasoning-models - この表は自動的に生成されるため、手動で編集しないでください。 */}
