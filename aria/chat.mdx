...
description: "Start and organize ARIA chats, queue messages, add context, grant network access, and give feedback."
---
...
Attach runs, workspace panels, and Weave traces and evaluations to a chat so ARIA knows exactly what you're asking about. Each item appears in the message box as a removable tag. You can attach several items to a message. Adding context doesn't send the message on its own. When you're ready, click **Send**.
...
ARIA asks you for network access when a task needs a domain that its sandbox blocks, such as `pypi.org` and `files.pythonhosted.org` to install a Python package, and waits for your answer. By default, the sandbox where ARIA runs code can reach W&B services but blocks most other domains on the internet.

The request appears in the chat under **The agent is requesting network access**. It shows why ARIA needs access and lists the domains it wants to reach, all selected. Respond to the request in one of these ways:
...
After you grant access in a conversation, **<Icon icon="link" iconType="solid"/>** (**Manage network access**) appears in the message box, next to **+**.
