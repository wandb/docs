...
Pinned conversations appear first, under a single **Pinned** header, not grouped by date. Unpinned conversations follow, grouped **Today**, **Yesterday**, **Last 7 Days**, **Last 30 Days**, then by month, then by month and year for conversations older than a year. Within each group, the most recently active conversation appears first. {/* VERIFY: are conversations scoped/grouped by project? not expressed in grouping code */}
...
There's no limit on how many conversations you can pin, and pin changes sync across your open browser tabs. Pinning affects only your own conversation list, not what teammates with access to the same project see. {/* VERIFY: pins private to user — inferred from data model, confirm with Chris LoCasto */}
