...
* **Pencil icon**: Move the message back into the message box so you can change it. ARIA removes it from the queue, so send it again to requeue it. This control is unavailable while the message box already holds a draft.
* **Up arrow icon**: Stop what ARIA is working on and run this message next. The other queued messages keep their order.
