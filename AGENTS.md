...
- **Naming the web app — "W&B" or "the W&B UI", never "W&B App"**: There is no "W&B App" product. The umbrella product name is **W&B**, and "W&B App" is also ambiguous with the W&B iOS app. Which of the two terms to use depends on context:

  - Use **"the W&B UI"** when the sentence contrasts the browser UI with another interface — the Python SDK, the CLI, the Public API, or the LEET terminal UI — and in any `<Tab title="...">` that sits beside a `Python SDK`, `W&B CLI`, or `Python` tab. Plain "W&B" reads wrong there: "Use W&B or the W&B Python SDK to define a TTL policy."
  - Use **plain "W&B"** everywhere else, dropping the article so the grammar stays clean: "in the W&B App" → "in W&B"; "Navigate to the [W&B App](url)" → "Navigate to [W&B](url)".
  - **Compound nouns** keep the real noun and drop only `App`: "W&B App URL" → "W&B URL". Same for workspace, dashboard, navigation, tab, and system.
  - The bare variants **`App UI`** and **`the App`** mean the same thing and follow the same rules.
  - **Exception**: the Registry-specific **"W&B Registry UI"** stays as-is.

  **Reread the whole sentence after each replacement.** The mechanical swap reliably produces redundancies that need a rewrite, not just a term swap: "Navigate to your W&B project in W&B" (drop the trailing phrase), "Within W&B, W&B posts the response" (name the actor once), "Visualize your W&B Sweeps with W&B" (say "with the W&B UI"). A sweep that leaves these behind will come back in review.

  Changing this term in a heading changes its anchor. Grep for inbound links to the old anchor before you commit. History: owner ruling on #2721, refined into the two-term rule on #2737.
- **English sources only**: Never edit content under `ja/`, `ko/`, or `fr/`. Those pages are regenerated by the translation pipeline, so edits there are overwritten and only add review noise. Fix the English source. When a grep for a change turns up localized hits, report them and leave them alone.
- **Don't reword text inside `{/* ... */}` comments**: Commented-out MDX is unpublished, so rewording it produces a diff a reviewer has to check for no reader-facing benefit. Leave it as-is even during a terminology sweep, and mention in the PR that hidden blocks will need updating when someone un-hides them. Live markup wrapping a comment (a `<Tab title="...">` just outside it, for example) is fair game.
