...
- **Callout components**: This docset uses `<Note>`, `<Warning>`, `<Tip>`, and `<Info>`. `<Caution>` and `<Important>` are not Mintlify components — they parse without error and then render nothing.
- **`keywords:` frontmatter**: In use on roughly a third of English pages, heaviest in `platform/`, `support/`, and `weave/`. Whether to add, keep, or remove `keywords` is unsettled here — treat it as contested rather than normalizing either way.
