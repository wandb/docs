...
* [Répondre automatiquement aux events dans W&amp;B](/fr/models/automations/create-automations/aria), par exemple en diagnostiquant un run en échec ou en examinant une nouvelle version de modèle.

## Démarrage rapide {#quick-start}

Pour [chat avec ARIA](/fr/aria/chat) :
...
## Lancer ARIA là où vous travaillez {#start-aria-from-where-you-work}

En plus de **Ask ARIA** en haut de la page, vous pouvez lancer ARIA depuis différents boutons répartis dans W&amp;B. La plupart de ces boutons ouvrent un nouveau chat avec une suggestion dans la zone de message. Vous décrivez alors ce que vous souhaitez avant d’envoyer le message.

| Emplacement                                              | Bouton                                                          | Aide apportée par ARIA                                                                                                                                                                 |
| -------------------------------------------------------- | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Page d’un run planté ou en échec                         | **Debug with ARIA**                                             | Expliquer pourquoi le run a planté ou échoué. Ce bouton lance un nouveau chat et interroge ARIA immédiatement.                                                                         |
...
| Menu **Filter** du tableau **Runs**                      | Champ de texte **Describe the filters you want ARIA to create** | Filtrer le tableau **Runs**. Décrivez les runs recherchés, puis appuyez sur **Enter**. ARIA propose un **Run Filter**. Cliquez dessus dans le chat pour l’appliquer à votre Workspace. |
| Menu des vues enregistrées                               | **Ask ARIA**                                                    | Créer une vue enregistrée de votre Workspace.                                                                                                                                          |
| Onglet **Reports** du projet                             | **Draft report with ARIA**                                      | Créer un rapport.                                                                                                                                                                      |
...
Pour fournir à ARIA du contexte sur un run, un panneau de Workspace, ou une trace ou une évaluation W&amp;B Weave précise, consultez [Ajouter du contexte à une conversation](/fr/aria/chat#add-context-to-a-conversation).

## Suivre la progression d’ARIA {#follow-arias-progress}

Pendant qu’ARIA travaille, elle transmet ses réponses en continu afin que vous puissiez suivre sa progression étape par étape, au lieu d’attendre une réponse complète. Selon la complexité de votre question, ARIA peut mettre quelques instants à terminer. Vous pouvez fermer la fenêtre de chat pendant qu’ARIA travaille et revenir consulter la réponse plus tard, ou [mettre votre prochain message en file d’attente](/fr/aria/chat#queue-messages) sans attendre. Si ARIA a besoin de plus d’informations, ARIA vous demande des précisions.

Pour vous aider à garder le contexte, ARIA affiche des graphiques en courbes et des graphiques à barres directement dans le chat, avec prise en charge de la vue plein écran. Survoler un run dans l’onglet **Runs** du workspace met en évidence les graphiques de ce run dans ARIA, et survoler un panel dans ARIA met en évidence le run dans l’onglet **Runs**.
...
Pour des exemples concrets, voir [Exemples de conversations](/fr/aria/overview#example-chats). Pour plus d’informations sur la gestion des chats, voir [Chat with ARIA](/fr/aria/chat).

## Afficher et gérer les mémoires du projet {#view-and-manage-project-memories}

ARIA mémorise ce qu’elle apprend. Au fil de votre travail, ARIA enregistre des *mémoires* limitées à votre projet afin de ne pas avoir à redécouvrir votre configuration, vos conclusions ou vos préférences à chaque nouvelle session. Dites-lui une seule fois que votre équipe préfère les graphiques à échelle logarithmique, et elle s’en souviendra.

Chaque mémoire possède un nom, une description permettant à ARIA de savoir quand la rappeler, ainsi que le texte de la mémoire elle-même.

Toute personne de l’équipe propriétaire du projet peut afficher, modifier ou supprimer des mémoires :

1. Dans la fenêtre de chat ARIA, cliquez sur **<Icon icon="sidebar-reveal" iconType="solid" />** (**Reveal chat history**) pour ouvrir la barre latérale de l’historique de conversation.
2. Dans la section **Customize**, cliquez sur **Project memory**. Toutes les mémoires qu’ARIA possède sur le projet s’affichent.
3. Pour modifier une mémoire, cliquez sur **<Icon icon="pencil" iconType="solid" />**. Mettez à jour le **Nom**, la **Description** ou la **Mémoire**, puis cliquez sur **enregistrer**.
4. Pour supprimer une mémoire, cliquez sur **<Icon icon="trash" iconType="solid" />**.

Sans attendre qu’ARIA apprenne de nouvelles mémoires, vous pouvez créer une mémoire de projet de manière interactive. Par exemple :

<AriaChatBubbles prompt="N’oublie pas que nous préférons une coloration sémantique pour les graphiques." response="Je vais d’abord vérifier les mémoires de projet existantes afin que cette préférence soit enregistrée sous le bon sujet plutôt que dupliquée… Nouvelle mémoire de projet enregistrée : chart-coloring. Elle indique que les graphiques doivent utiliser des couleurs cohérentes et sémantiques basées sur la signification ou le statut des séries/catégories, plutôt que sur un ordre arbitraire de la palette. " />

## Exemples de conversations {#example-chats}
...
<Accordion title="Exécuter des expériences à partir des recommandations d’ARIA">
  ...
  ARIA utilise W&amp;B Launch pour exécuter des expériences dans un environnement sandbox. Voir [Activer ARIA pour exécuter des expériences](/fr/aria/autoresearch) pour plus d’informations.
</Accordion>
...
<Accordion title="Obtenez des recommandations pour le réglage des hyperparamètres">
  ...
  ARIA identifie également que tous les run utilisent l’optimizer SGD. Sur la base de cette observation, ARIA recommande d’essayer un autre optimizer, comme Adam (Adaptive Moment Estimation), dans les prochains run pour voir si cela aide à améliorer les performances du modèle.
