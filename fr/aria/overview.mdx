...
ARIA se souvient de ce qu’elle apprend. Au fil de votre travail, ARIA enregistre des *mémoires* associées à votre projet afin de ne pas avoir à redécouvrir votre configuration, vos conclusions ou vos préférences à chaque nouvelle session. Dites-lui une seule fois que votre équipe préfère les graphiques à échelle logarithmique, et elle s’en souviendra.

Chaque mémoire comporte un nom et une description afin qu’ARIA sache quand la rappeler, ainsi que le texte de la mémoire.

Toute personne ayant accès au projet peut afficher, modifier ou supprimer des mémoires :
...
3. Pour modifier une mémoire, cliquez sur **<Icon icon="pencil" iconType="solid" />**. Mettez à jour le **Name**, la **Description** ou la **Memory**, puis cliquez sur **Save**.
...
<AriaChatBubbles prompt="Souviens-toi que nous préférons une coloration sémantique pour les graphiques." response="Je vais d’abord vérifier les mémoires de projet existantes afin que cette préférence soit enregistrée sous le bon thème plutôt que dupliquée ... Nouvelle mémoire de projet enregistrée : chart-coloring. Elle indique que les graphiques doivent utiliser des couleurs sémantiques cohérentes, basées sur la signification ou le statut des séries/catégories plutôt que sur un ordre arbitraire de la palette. " />
