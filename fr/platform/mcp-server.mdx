...
<Tabs>
  <Tab title="Claude Code">
    ...
    Pour plus d’informations, voir [la documentation de Claude](https://docs.anthropic.com/en/docs/claude-code/mcp).
  </Tab>

  <Tab title="Claude Code Desktop">
    L’interface intégrée des connecteurs personnalisés de Claude Code Desktop ne prend pas en charge l’authentification par clé API pour les serveurs MCP distants. Pour contourner cette limitation, utilisez le proxy npm [`mcp-remote`](https://www.npmjs.com/package/mcp-remote) pour connecter Claude Code Desktop au serveur MCP distant de W&amp;B. Le proxy s’exécute en local et transfère les requêtes vers `https://mcp.withwandb.com/mcp` avec votre jeton Bearer.

    Vous devez avoir [Node.js](https://nodejs.org/) installé sur votre système.

    Ouvrez votre fichier de configuration Claude Code Desktop dans un éditeur de texte. Vous trouverez le fichier de configuration à l’emplacement suivant selon votre système d’exploitation :
    ...
    * **Windows** : `%APPDATA%\Claude\claude_desktop_config.json`
    ...
    La valeur complète de l’en-tête est définie via le champ `env`, plutôt que directement dans `args`, afin de contourner un problème lié à l’échappement des espaces dans certaines versions de Claude Desktop.

    Redémarrez Claude Code Desktop pour activer la nouvelle configuration. Vérifiez la connexion en saisissant le prompt &quot;List the projects in my W&amp;B account.&quot;
  </Tab>
...
<Tabs>
  ...
  <Tab title="Claude Code Desktop">
    Ouvrez votre fichier de configuration Claude Code Desktop dans un éditeur de texte. Vous trouverez le fichier de configuration aux emplacements correspondant à votre système d’exploitation :

    * **macOS**: ~/Library/Application\ Support/Claude/claude&#95;desktop&#95;config.json
    * **Windows**: %APPDATA%\Claude\claude&#95;desktop&#95;config.json

    Ajoutez ce qui suit à l’objet JSON de votre fichier de configuration Claude Code Desktop. Utilisez le chemin complet vers `uvx`, car sinon Claude Code Desktop risque de ne pas trouver votre installation de `uvx`.
