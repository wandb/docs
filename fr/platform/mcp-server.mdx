...
keywords: ["outil MCP", "MCP Cursor", "MCP Claude", "intégration d’assistant d’IA", "requêtes en langage naturel"]
---
...
## Types de déploiement {#deployment-types}
...
- W&amp;B fixe la version du SDK `mcp` à [1.14.0](https://pypi.org/project/mcp/1.14.0/), version `2024-11-05`. Les clients doivent se connecter à l’aide du SDK `mcp` 1.14.x. Pour la prise en charge de HTTP avec diffusion en continu dans le Cloud dédié de W&amp;B, la version `2025-03-26` ou une version ultérieure du SDK `mcp` 1.14.x est requise.
...
### URL de connexion {#connection-url}
...
### Prérequis locaux {#local-prerequisites}
...
### Exécutez le serveur avec le transport HTTP {#run-the-server-with-http-transport}
...
## Capacités du serveur MCP de W&amp;B {#wb-mcp-server-capabilities}
...
### Requêtes de traces axées sur le schéma {#schema-first-trace-queries}
...
### Pour les flux de travail avec de nombreuses traces {#for-trace-heavy-workflows}
...
### Pour les flux de travail comportant beaucoup de runs {#for-run-heavy-workflows}
...
### Pour Cloud dédié et Autogéré {#for-dedicated-cloud-and-self-managed}
...
### Pour les installations en local {#for-local-installs}
...
## Flux de travail recommandés {#recommended-workflows}
...
### Explorer un projet inconnu {#explore-an-unfamiliar-project}
...
### Analyser les appels LLM en échec {#triage-failing-llm-calls}
...
### Diagnostiquer un run d&#39;entraînement défaillant {#diagnose-a-bad-training-run}
...
### Résumer les évaluations et comparer les versions du modèle {#summarize-evals-and-compare-model-versions}
