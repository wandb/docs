...
keywords: ["W&B Operator", "chart Helm", "CR Kubernetes", "installation de l’opérateur", "ressource personnalisée"]
---
...
2. **Obtenir une licence serveur W&amp;B** : Voir la section [License](/fr/platform/hosting/self-managed/requirements#license) sur la page des exigences.
...
### clusters Kubernetes OpenShift {#openshift-kubernetes-clusters}
...
#### Exécutez le conteneur avec un utilisateur non privilégié {#run-the-container-as-an-un-privileged-user}
...
## Déployer l’application W&amp;B Server {#deploy-wb-server-application}
...
#### Module Terraform Helm {#helm-terraform-module}
...
#### Modules Terraform pour le cloud {#cloud-terraform-modules}
...
## Activer le serveur MCP {#enable-the-mcp-server}
...
### Activez le sous-chart {#enable-the-subchart}
...
### Vérifier le serveur MCP {#verify-the-mcp-server}
...
### Connecter un client {#connect-a-client}
...
## Considérations spécifiques à l’environnement {#environment-specific-considerations}
...
### sur site et bare metal {#on-premises-and-bare-metal}
...
#### Gestion du DNS et des certificats {#dns-and-certificate-management}
...
#### Déploiements OpenShift {#openshift-deployments}
...
#### Stockage d’objets pour les environnements sur site et compatibles S3 {#object-storage-for-on-premises-and-s3-compatible}
...
### Sur cloud public avec Terraform {#public-cloud-with-terraform}
...
## Déployer avec Terraform sur un cloud public {#deploy-with-terraform-on-public-cloud}
...
### Autorisations préalables {#prerequisite-permissions}
...
### Activer le courtier de messages (file d&#39;attente) {#enable-message-broker-queue}
...
### Déploiement recommandé {#recommended-deployment-2}
...
### Activer le courtier de messages (file d&#39;attente) {#enable-message-broker-queue-2}
...
### Ressources supplémentaires {#additional-resources-2}
...
### Autorisations préalables {#prerequisite-permissions-3}
...
### Étapes générales {#general-steps-3}
...
### Déploiement recommandé {#recommended-deployment-3}
...
### Activer le courtier de messages (file d&#39;attente) {#enable-message-broker-queue-3}
...
### Ressources supplémentaires {#additional-resources-3}
...
### Autres options de déploiement {#other-deployment-options}
...
## Accéder à la console de gestion W&amp;B {#access-the-wb-management-console}
...
## Mettre à jour l’opérateur Kubernetes W&amp;B {#update-the-wb-kubernetes-operator}
...
## Mettre à jour l’application W&amp;B Server {#update-the-wb-server-application}
...
## Mettre à niveau MySQL vers 8.4.x {#upgrade-mysql-to-84x}
...
## Compatibilité de ClickHouse pour les mises à niveau {#clickhouse-compatibility-for-upgrades}
...
W&amp;B Weave nécessite une version prise en charge à la fois de ClickHouse Server et de ClickHouse Keeper.

* Weave prend en charge ClickHouse 25.8 à 25.12, ainsi que 26.3 ou ultérieure.
* Weave ne fonctionne pas avec ClickHouse 26.1 ou 26.2.

***Avant* de mettre à niveau W&amp;B Autogéré**, vérifiez que ClickHouse Server et ClickHouse Keeper exécutent tous deux une version prise en charge par Weave. Si vous devez modifier les versions de ClickHouse, mettez à niveau les deux composants ensemble.
...
Pour les notes de version propres à chaque version, voir [Versions du serveur W&amp;B prises en charge](/fr/release-notes/server-releases).

## Migrer des instances Autogéré vers W&amp;B Operator {#migrate-self-managed-instances-to-wb-operator}
...
### Migrer vers les modules Terraform Google Cloud basés sur l’opérateur {#migrate-to-operator-based-google-cloud-terraform-modules}
...
### Migrer vers des modules Terraform Azure basés sur l’opérateur {#migrate-to-operator-based-azure-terraform-modules}

Contactez [assistance client](mailto:support@wandb.com) ou votre équipe W&amp;B si vous avez des questions ou besoin d’assistance.

### Migrer vers le chart Helm basé sur l’opérateur {#migrate-to-operator-based-helm-chart}
...
### Migrer vers le chart Helm Terraform basé sur l’opérateur {#migrate-to-operator-based-terraform-helm-chart}
...
## Référence de configuration pour serveur W&amp;B {#configuration-reference-for-wb-server}
...
### Stockage d’objets (bucket) {#object-storage-bucket}
...
### Comptes de service Kubernetes personnalisés {#custom-kubernetes-service-accounts}
...
### Définir les limites de débit {#set-rate-limits}
...
### Contexte de sécurité personnalisé {#custom-security-context}
...
## Référence de configuration pour l’opérateur W&amp;B {#configuration-reference-for-wb-operator}
...
### Objectif et rôle de chaque pod {#purpose-and-role-of-each-pod}
...
### Comment obtenir le mot de passe de la console de l’opérateur W&amp;B {#how-to-get-the-wb-operator-console-password}

Voir [Accéder à la console de gestion W&amp;B](#access-the-wb-management-console).

### Comment accéder à la console de l’opérateur W&amp;B si l’ingress ne fonctionne pas {#how-to-access-the-wb-operator-console-if-ingress-doesnt-work}
...
### Comment consulter les journaux de W&amp;B Server {#how-to-view-wb-server-logs}
...
### Comment identifier la classe d’ingress Kubernetes {#how-to-identify-the-kubernetes-ingress-class}
