...
* **Version du chart** : `operator-wandb` `0.42.3` ou ultérieure. Le sous-chart `mcp-server` a été introduit dans `0.42.1`, mais les champs Datadog et Confidentialité utilisés dans l’exemple ci-dessous ont été ajoutés plus tard.
...
Ajoutez les éléments suivants au bloc `spec.values` de votre ressource personnalisée `WeightsAndBiases` (CR) existante, aux côtés de vos valeurs `global`, `ingress` et autres redéfinitions existantes. Le bloc Datadog est facultatif, mais recommandé si un DaemonSet Datadog Agent collecte déjà les journaux et les traces des pods dans votre cluster.
...
<Note>
  W&amp;B Operator est la méthode d’installation par défaut et recommandée pour W&amp;B Server. Contactez [assistance client](mailto:support@wandb.com) ou votre équipe W&amp;B si vous avez des questions.
</Note>
