...
keywords: ["installation en air gap", "déploiement hors ligne", "cluster déconnecté", "registre privé", "OpenShift"]
---
...
### Versions requises {#version-requirements}

<SelfManagedVersionRequirements />

### Prérequis SSL/TLS {#ssltls-requirements}

<SelfManagedSslTlsRequirements />

### Prérequis matériels {#hardware-requirements}
...
### Exigences spécifiques aux environnements isolés du réseau {#air-gapped-specific-requirements}
...
## Préparez votre environnement isolé du réseau {#prepare-your-air-gapped-environment}
...
### Étape 1 : Configurer le registre de conteneurs interne {#step-1-set-up-internal-container-registry}
...
#### Conteneurs des composants W&amp;B essentiels {#core-wb-component-containers}
...
#### Conteneurs de dépendances {#dependency-containers}
...
#### Obtenir la liste complète des images {#get-the-complete-image-list}
...
#### Transférer des images vers un registre isolé du réseau {#transfer-images-to-air-gapped-registry}
...
### Étape 2 : Configurer le dépôt interne de charts Helm {#step-2-set-up-internal-helm-chart-repository}
...
### Étape 3 : Configurer l’accès au dépôt Helm {#step-3-configure-helm-repository-access}
...
## Déployer W&amp;B dans un environnement isolé du réseau {#deploy-wb-in-air-gapped-environment}
...
### Étape 4 : Installer l’opérateur Kubernetes {#step-4-install-the-kubernetes-operator}
...
### Étape 5 : Configurer la base de données MySQL {#step-5-set-up-mysql-database}
...
### Étape 6 : Configurer la Ressource personnalisée W&amp;B {#step-6-configure-wb-custom-resource}
...
### Étape 7 : Déployer la plateforme W&amp;B {#step-7-deploy-the-wb-platform}
...
### Contraintes de contexte de sécurité d’OpenShift {#openshift-security-context-constraints}
...
#### Option 1 : Utiliser la SCC restricted (recommandée) {#option-1-use-restricted-scc-recommended}
...
#### Option 2 : Créer une SCC personnalisée (si nécessaire) {#option-2-create-custom-scc-if-required}
...
### Routes d’OpenShift {#openshift-routes}
...
### Configuration de récupération d’image pour OpenShift {#openshift-image-pull-configuration}
...
### Exemple complet pour OpenShift {#openshift-complete-example}
...
## Vérifiez votre installation {#verify-your-installation}
...
### Vérifications supplémentaires pour un environnement isolé du réseau {#additional-air-gapped-verification}
...
### Erreurs de récupération d’image {#image-pull-errors}
...
### Erreurs SCC d’OpenShift {#openshift-scc-errors}
...
### Chart Helm introuvable {#helm-chart-not-found}
...
### Puis-je utiliser une autre classe d’ingress ? {#can-i-use-a-different-ingress-class}
...
### Comment puis-je gérer des bundles de certificats contenant plusieurs certificats ? {#how-do-i-handle-certificate-bundles-with-multiple-certificates}
...
### Comment puis-je empêcher les mises à jour automatiques ? {#how-do-i-prevent-automatic-updates}
...
### Le déploiement fonctionne-t-il sans connexion à des dépôts publics ? {#does-the-deployment-work-with-no-connection-to-public-repositories}
...
### Comment puis-je mettre à jour W&amp;B dans un environnement isolé du réseau ? {#how-do-i-update-wb-in-an-air-gapped-environment}
