...
La ressource utilisateur SCIM correspond aux utilisateurs W&amp;B et aux comptes de service. Utilisez ces points de terminaison pour gérer les utilisateurs et les comptes de service dans votre organisation.
...
Crée un compte de service limité à une équipe ou à l’organisation dans votre organisation. Omettez `accountType` pour créer à la place un utilisateur standard. Voir [Create user](#create-user).

<Note>
  Disponible dans **Cloud dédié**, **Autogéré** v0.81.0+ et **Cloud mutualisé**.

  * Définissez `userName` sur le nom du compte de service. L’API utilise `userName` comme nom d’affichage du compte ; `displayName` dans le corps de la requête est ignoré.
...
Si la réponse renvoie l’une de ces erreurs, vérifiez la requête pour repérer ces problèmes courants :
...
Supprime définitivement un compte de service ainsi que son appartenance à l’organisation. Il s’agit d’une suppression définitive ; le compte ne peut pas être réactivé via SCIM.
