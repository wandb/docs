...
| `registryAccess` | string | Non    | Droit d’accès au registre. L’une des valeurs suivantes : `enabled` ou `none`. Lorsqu’il n’est pas indiqué, l’accès effectif est déterminé à partir de `modelsSeat` et `weaveRole` au moment de la lecture. Disponible dans **Cloud mutualisé**{/* and in **Cloud dédié** and **Autogéré** v0.83.0+ */}. |
...
<Note>
  ...
  Sur **Cloud dédié** et **Autogéré**, {/* v0.82.0 and older */} l&#39;accès à Registry reste lié à `modelsSeat`. Définissez `modelsSeat` sur `none` pour révoquer l&#39;accès à Registry dans ces versions.
</Note>
...
{/* À corriger avec la version 0.83+ */}
