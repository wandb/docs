...
description: "Stockez les artefacts W&B et les données dans vos propres buckets de stockage cloud grâce à la fonctionnalité Bring Your Own Bucket (BYOB)."
keywords: ["bucket personnalisé", "bucket S3", "bucket GCS", "stockage de blobs Azure"]
---

import ByobContextNote from "/snippets/fr/_includes/byob-context-note.mdx";
import WeaveByob from "/snippets/fr/_includes/weave-byob.mdx";
...
### Données stockées dans la base de données centrale ou dans les buckets {#data-stored-in-the-central-database-vs-buckets}
...
### Portées des buckets {#bucket-scopes}
...
### Matrice de disponibilité {#availability-matrix}
  ...
  <Tab title="Azure">
    ...
    **BYOB au niveau de l’instance**:
    ...
    2. Choisissez une méthode d’authentification :
       ...
       * **Clé de compte de stockage** : générez une clé d’accès au compte de stockage et notez la clé ainsi que le nom du compte de stockage. Pour les organisations sur Cloud dédié, partagez ces valeurs avec votre équipe de compte W&amp;B via un mécanisme de partage sécurisé.
       * **Workload identity** : accordez à une identité managée Azure l’accès à votre compte de stockage et configurez W&amp;B pour qu’il l’utilise. Vous n’avez pas besoin de partager de clé de compte de stockage. Suivez les [instructions relatives à Workload Identity correspondant à votre type de déploiement](#set-up-azure-workload-identity).
...
## Déterminer l’adresse de stockage {#determine-the-storage-address}
...
### BYOB au niveau de l’instance {#instance-level-byob}

<Note>
  Pour CoreWeave AI Object Storage au niveau de l’instance, contactez l’[assistance W&amp;B](mailto:support@wandb.com) plutôt que de suivre ces instructions. La configuration en libre-service n’est pas encore prise en charge.
</Note>

Pour le **Cloud dédié** : communiquez les informations du bucket à votre équipe W&amp;B, qui se charge de configurer votre instance. Pour Azure workload identity, [accordez l’accès à votre compte de stockage](#set-up-azure-workload-identity) et fournissez le nom du compte de stockage, le nom du conteneur ainsi que le chemin facultatif.

Pour le mode **Autogéré**, configurez le BYOB au niveau de l’instance dans la W&amp;B System Console. Pour Azure workload identity, effectuez au préalable la [configuration de l’identité](#set-up-azure-workload-identity).

1. Connectez-vous à W&amp;B en tant qu’utilisateur doté du rôle `admin`.
2. Cliquez sur l’icône utilisateur en haut, puis sur **System Console**.
3. Accédez à **Settings** &gt; **System Connections**.
4. Dans **Bucket Storage**, sélectionnez le **Provider** et saisissez les informations du bucket. Pour Azure, sélectionnez **Azure Blob Storage (az)**, saisissez le **Storage account**, puis renseignez séparément le **Blob container**.
5. Facultatif : saisissez le **Path** à utiliser dans le nouveau bucket.
6. Assurez-vous que l’identité ou les identifiants d’authentification du provider sélectionné disposent d’un accès au bucket. Pour Azure, sélectionnez une méthode d’**Authentication** :
...
Une fois l’enregistrement effectué, W&amp;B utilise le bucket configuré comme destination de stockage par défaut pour les nouveaux artifacts et les fichiers de run au niveau de l’instance.

Si un paramètre n’est pas modifiable, contactez l’administrateur de votre déploiement.

#### Configurer Azure workload identity {#set-up-azure-workload-identity}

Workload Identity permet à W&amp;B d&#39;accéder à votre bucket Azure sans clé de compte de stockage. Sélectionnez votre type de déploiement pour obtenir les instructions de configuration.

<Tabs>
  <Tab title="Cloud dédié">
    W&amp;B configure le déploiement. De votre côté, vous configurez l&#39;accès à votre compte de stockage Azure :

    1. Contactez votre équipe W&amp;B en indiquant l&#39;ID de tenant de votre compte de stockage afin de confirmer l&#39;identité gérée à utiliser.
    2. Si l&#39;identité se trouve dans votre tenant Azure, utilisez son ID de principal pour les attributions de rôles ci-dessous. Sinon, créez ou sélectionnez une identité gérée dans votre tenant, puis configurez ses identifiants d&#39;authentification fédérés à l&#39;aide des valeurs d&#39;issuer, de subject et d&#39;audience fournies par W&amp;B. Une identité gérée [ne peut pas accéder directement au stockage d&#39;un autre tenant](https://learn.microsoft.com/en-us/entra/identity/managed-identities-azure-resources/managed-identities-faq#can-i-use-a-managed-identity-to-access-a-resource-in-a-different-directorytenant).
    3. Accordez au principal de l&#39;identité les rôles **Reader** et **Storage Blob Data Contributor** au niveau du compte de stockage, sur votre compte de stockage BYOB.
    4. Communiquez à votre équipe W&amp;B le nom du compte de stockage, le nom du conteneur, le chemin facultatif, ainsi que l&#39;ID de tenant et l&#39;ID de client de l&#39;identité.

    Une fois que W&amp;B a configuré la connexion, vérifiez l&#39;accès en téléversant puis en téléchargeant un fichier de run ou un artifact.
  </Tab>

  <Tab title="Autogéré">
    En tant qu&#39;administrateur du déploiement, configurez Workload Identity dans Azure et Kubernetes, puis sélectionnez-le dans la System Console.
    ...
    1. Activez la fédération de jetons OpenID Connect (OIDC) et Azure workload identity pour le cluster Kubernetes. Pour Azure Kubernetes Service (AKS), suivez [Configure Microsoft Entra Workload ID](https://learn.microsoft.com/en-us/azure/aks/workload-identity-deploy-cluster).
    2. Créez ou sélectionnez une identité gérée Azure et notez son ID de tenant, son ID de client et son ID de principal. Accordez à son principal les rôles **Reader** et **Storage Blob Data Contributor** au niveau du compte de stockage, sur votre compte de stockage BYOB.
    3. Renseignez `global.azureStorageIdentity` dans vos Helm values avec l&#39;ID de tenant, l&#39;ID de client et l&#39;ID de principal de l&#39;identité. Configurez la fédération et l&#39;annotation `azure.workload.identity/client-id` pour les comptes de service Kubernetes qui accèdent au stockage. Pour plus de détails sur la configuration Helm, voir [Azure workload identity configuration](https://github.com/wandb/helm-charts/blob/operator-wandb-0.44.13/charts/operator-wandb/README.md#azure-managed-storage-workload-identity).
    4. Suivez la [procédure dans la Console](#instance-level-byob) pour sélectionner Workload Identity et enregistrer vos bucket settings. Vérifiez l&#39;accès en téléversant puis en téléchargeant un fichier de run ou un artifact.
  </Tab>
</Tabs>

### BYOB au niveau de l’équipe {#team-level-byob}
