...
## Créer une équipe collaborative {#create-a-collaborative-team}

Suivez ces étapes pour créer une nouvelle équipe et inviter vos premiers collaborateurs. Une fois l’équipe créée, vous pouvez affiner les membres, les alertes et la confidentialité dans [Configuration de l’équipe](#team-configuration).
...
## Configuration de l&#39;équipe {#team-configuration}

Vous pouvez configurer différents paramètres pour votre équipe, notamment ses membres, ses alertes et ses paramètres de confidentialité. Les sections suivantes décrivent chaque catégorie de paramètres.

Pour gérer les paramètres de votre équipe, accédez à la section **Teams** dans le menu de gauche, puis cliquez sur l&#39;équipe dont vous souhaitez modifier les paramètres.
...
Dans les déploiements Cloud dédié et Autogéré, la liste inclut également une colonne **Statut** qui indique si chaque membre est actif, a une invitation en attente ou possède un compte qu’un administrateur de l’organisation a désactivé. Voir [Afficher le statut des membres de l’équipe](/fr/platform/hosting/iam/access-management/manage-organization#view-team-member-status).
...
Dans la section **Team alerts**, basculez l’interrupteur à côté du type d’événement pour lequel vous souhaitez recevoir des alertes. W&amp;B propose les types d’événement suivants :
...
Pour modifier ces paramètres, ouvrez le tableau de bord de l’équipe à l’adresse `https://wandb.ai/[TEAM-NAME]`, sélectionnez **Paramètres de l’équipe** dans la navigation de gauche, puis ouvrez la section **Confidentialité**. Remplacez `[TEAM-NAME]` par le nom de votre équipe.

### Utilisation {#usage}

La section **Utilisation** décrit la quantité totale de mémoire utilisée par l’équipe sur les serveurs de W&amp;B. Le plan de stockage par défaut est de 100 Go. Pour en savoir plus sur le stockage et la tarification, voir la page [Tarification](https://wandb.ai/site/pricing).

### Stockage {#storage}

La section **Stockage** décrit la configuration du bucket de stockage cloud qu’utilisent les données de l’équipe. Pour plus d&#39;informations, voir [Connecteur de stockage sécurisé](#secure-storage-connector) ou consultez la documentation [W&amp;B Server](/fr/platform/hosting/data-security/secure-storage-connector) si vous l’hébergez vous-même.

## Créer un profil d’équipe {#create-a-team-profile}

Un profil d’équipe est une page publique qui aide votre équipe à attirer des collaborateurs et à mettre en valeur son travail. Vous pouvez personnaliser la page de profil de votre équipe pour y afficher une présentation et mettre en avant des Reports et des Projects visibles par le public ou par les membres de l’équipe. Présentez des Reports, des Projects et des liens externes.

Utilisez un profil d’équipe pour effectuer les actions suivantes :
...
## Rôles et autorisations d’équipe {#team-roles-and-permissions}
...
### paramètres de l’équipe {#team-settings}
...
| Télécharger des Artifacts | X         | X           | X          |
...
### Paramètres système (W&amp;B Server uniquement) {#system-settings-wb-server-only}
...
| Créer ou supprimer des Teams      |           |             |            | X                      |

### Comportement des comptes de service d&#39;équipe {#team-service-account-behavior}
