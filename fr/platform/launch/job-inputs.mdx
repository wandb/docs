...
L’appel à `launch.manage_config_file` ajoute le fichier `config.yaml` comme entrée du job, ce qui le rend reconfigurable lors d’un lancement depuis la CLI ou l’UI de W&amp;B.
