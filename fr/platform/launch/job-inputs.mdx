...
keywords: ["configuration du job", "redéfinitions d'hyperparamètres", "entrée de fichier de configuration", "schéma JSON", "schéma Pydantic"]
---
...
## Reconfigurer l’objet `Run` {#reconfigure-the-run-object}
...
### Accéder aux entrées de configuration du run {#access-run-config-inputs}
...
## Reconfigurer un fichier {#reconfigure-a-file}
...
### Accéder aux entrées du fichier de configuration {#access-config-file-inputs}
...
## Personnaliser l’interface du volet de lancement d’un job {#customize-a-jobs-launch-drawer-ui}
