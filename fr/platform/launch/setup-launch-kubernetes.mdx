...
Vous pouvez utiliser W&amp;B Launch pour déployer des charges de travail de ML sur un cluster Kubernetes et offrir aux ingénieurs ML une interface simple, directement dans W&amp;B, pour utiliser les ressources que vous gérez déjà avec Kubernetes.
...
W&amp;B s’appuie sur l’outil de build [Kaniko](https://github.com/GoogleContainerTools/kaniko) pour permettre à l’agent Launch de créer des images Docker dans un cluster Kubernetes. Pour en savoir plus sur la configuration de Kaniko pour l’agent Launch, ou sur la manière de désactiver la création d’images pour les jobs afin d’utiliser uniquement des images Docker préconstruites, voir [Configuration avancée de l’agent](./setup-agent-advanced).
...
Vous pouvez contrôler n’importe quel aspect de la spécification de la ressource de charge de travail Kubernetes lorsque vous créez une file d’attente Launch.
...
Créez une file d&#39;attente dans W&amp;B App en utilisant Kubernetes comme ressource de calcul :
...
6. Dans le champ **Configuration**, indiquez la spécification de flux de travail d’un job Kubernetes ou la spécification d’une **Ressource personnalisée** que vous avez [configurée dans la section précédente](#configure-a-queue-for-kubernetes).
...
Dans votre fichier `values.yaml`, cela peut ressembler à ceci :
