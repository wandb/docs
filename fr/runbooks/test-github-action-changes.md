...
* **Fork GitHub** : un fork personnel de wandb/docs pour tester les modifications du flux de travail. Dans ce fork, vous devez être autorisé à pousser vers la branche par défaut et à contourner les règles de protection de la branche.
...
1. **Nom d&#39;utilisateur GitHub** - Vérifiez d&#39;abord `git remote -v` pour trouver le remote du fork, puis `git config` pour le nom d&#39;utilisateur. Ne le demandez à l&#39;utilisateur que s&#39;il est introuvable dans l&#39;un ou l&#39;autre emplacement.
2. **Statut du fork** - Confirmez qu&#39;il dispose d&#39;un fork de wandb/docs avec les autorisations nécessaires pour pousser vers la branche par défaut et contourner la protection de branche.
3. **Portée du test** - Demandez quelles modifications précises sont testées (mise à niveau de dépendance, changement de fonctionnalité, etc.).

<div id="task-overview">
  ## Aperçu de la tâche
</div>

Testez les modifications apportées aux flux de travail GitHub Actions dans le dépôt wandb/docs.
...
* **Important** : les GitHub Actions dans les PR s’exécutent toujours à partir de la branche de base (main), et non de la branche de la PR.
* **Limitation de déploiement Mintlify** : les déploiements Mintlify et la vérification `link-rot` ne sont générés que pour le dépôt principal `wandb/docs`, pas pour les forks. Dans un fork, la GitHub Action `validate-mdx` vérifie le statut des commandes `mint dev` et `mint broken-links` dans une PR du fork.

**Note de l’agent** : vous devez :

1. Vérifier dans `git remote -v` si un remote de fork existe déjà et extraire le nom d’utilisateur à partir de l’URL, le cas échéant.
...
Pour tester les modifications apportées au flux de travail, vous devez :
...
3. Créer une PR de test sur le `main` du fork avec des modifications de contenu afin de déclencher les flux de travail.
...
# Ne demander le nom d'utilisateur GitHub ou les détails du fork que s'ils sont introuvables dans les remotes ou la configuration
...
<div id="3-apply-workflow-changes">
  ### 3. Appliquer les modifications au flux de travail
</div>

Apportez vos modifications aux fichiers de flux de travail. Pour les mises à niveau de dépendances :
...
Si vous ne pouvez pas pousser directement vers le fork :
...
2. Fournissez à l’utilisateur la commande suivante :
   ...
   ```
3. Demandez-lui de créer la PR à l’adresse suivante : `https://github.com/<username>/docs/compare/main...test-pr-[description]`
...
# Apporter une petite modification de contenu pour déclencher les flux de travail
...
Créez ensuite une PR via l’interface utilisateur GitHub, de `<username>:test-pr-[description]` vers `<username>:main`
...
* ❌ Toute erreur dans les journaux GitHub Actions
...
* Rappel : les flux de travail s’exécutent depuis la branche de base (`main`), et non depuis la branche de PR
...
* [ ] Appliquer les modifications du flux de travail aux deux fichiers concernés
