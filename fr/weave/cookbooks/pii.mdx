---
title: "Gestion des données PII"
description: "Gérez les données PII dans les applications LLM à l'aide des outils de masquage et de filtrage des données de W&B Weave."
---
...
Dans ce guide, vous apprendrez à utiliser W&amp;B Weave tout en veillant à ce que vos données PII restent privées. Le guide présente les méthodes suivantes pour identifier, masquer et anonymiser les données PII :
...
De plus, vous apprendrez à utiliser la *personnalisation de la journalisation des entrées/sorties de `weave.op`* et *`autopatch_settings`* pour intégrer le masquage et l’anonymisation des données PII dans le flux de travail. Pour plus d&#39;informations, voir [Customize logged inputs and outputs](/fr/weave/guides/tracking/ops/#customize-logged-inputs-and-outputs).
...
Dans l’exemple suivant, deux fonctions de post-traitement sont définies et passées comme arguments à `weave.op()`.
...
* Ne journalisez jamais de PII brutes
...
Les [expressions régulières (regex)](https://docs.python.org/3/library/re.html) constituent la méthode la plus simple pour identifier et masquer des données PII. Elles vous permettent de définir des motifs correspondant à différents formats d’informations sensibles, comme les numéros de téléphone, les adresses e-mail et les numéros de sécurité sociale. À l’aide de regex, vous pouvez parcourir de grands volumes de texte et remplacer ou masquer des informations sans avoir recours à des techniques de NLP plus complexes.
...
La méthode suivante consiste à supprimer complètement les données PII à l’aide de [Microsoft Presidio](https://microsoft.github.io/presidio/). Presidio masque les PII et les remplace par un espace réservé représentant le type de PII. Par exemple, Presidio remplace `Alex` dans `"My name is Alex"` par `<PERSON>`.

Presidio prend en charge de manière native les [entités courantes](https://microsoft.github.io/presidio/supported_entities/). Dans l’exemple ci-dessous, nous masquons toutes les entités de type `PHONE_NUMBER`, `PERSON`, `LOCATION`, `EMAIL_ADDRESS` ou `US_SSN`. Le processus Presidio est encapsulé dans une fonction.

```python lines
from presidio_analyzer import AnalyzerEngine
from presidio_anonymizer import AnonymizerEngine

# Configurer l'Analyzer, qui charge un module NLP (modèle spaCy par défaut) et d'autres reconnaisseurs de PII.
...
Au lieu de masquer le texte, vous pouvez l’anonymiser à l’aide de MS Presidio pour remplacer des PII, comme les noms et les numéros de téléphone, par de fausses données générées avec la bibliothèque Python [Faker](https://faker.readthedocs.io/en/master/). Par exemple, supposons que vous disposiez des données suivantes :
...
1. La logique de gestion des PII est centralisée et délimitée dans la portée de l’initialisation, ce qui réduit le besoin d’une logique personnalisée dispersée.
...
Pour utiliser `autopatch_settings` afin de configurer la gestion des PII, définissez `postprocess_inputs` et/ou `postprocess_output` dans `op_settings` pour n’importe laquelle des intégrations LLM prises en charge.
...
Dans les exemples suivants, nous allons intégrer nos méthodes de masquage des PII et d&#39;anonymisation dans des modèles Weave et prévisualiser les résultats dans Weave Traces.

Tout d&#39;abord, nous allons créer un [modèle Weave](https://docs.wandb.ai/weave/guides/core-types/models). Un modèle Weave est une combinaison d&#39;informations telles que des paramètres de configuration, des poids de modèle et du code qui définit le fonctionnement du modèle.

Dans notre modèle, nous inclurons notre fonction `predict` où l&#39;API Anthropic sera appelée. Claude Sonnet d&#39;Anthropic est utilisé pour effectuer une analyse de sentiment tout en traçant les appels LLM à l&#39;aide de [Traces](https://docs.wandb.ai/weave/quickstart). Claude Sonnet recevra un bloc de texte et produira l&#39;une des classifications de sentiment suivantes : *positive*, *negative* ou *neutral*. De plus, nous inclurons nos fonctions de post-traitement pour nous assurer que nos données PII sont masquées ou anonymisées avant d&#39;être envoyées au LLM.
...
Dans le cas le plus simple, vous pouvez utiliser des expressions régulières pour identifier et masquer des données PII dans le texte original.
...
<div id="presidio-redaction-method">
  ### Méthode de masquage des PII avec Presidio
</div>

Ensuite, nous allons utiliser Presidio pour identifier et masquer les données PII dans le texte original.

<Frame>
  ![Processus de masquage des PII avec Presidio, avec les entités PII identifiées et la sortie de texte masquée](/media/pii/redact.png)
</Frame>
...
Dans cet exemple, nous utilisons Faker pour générer des données PII de remplacement anonymisées et Presidio pour identifier et remplacer les données PII dans le texte original.

<Frame>
  ![Processus de remplacement de données PII avec Faker et Presidio, avec le texte original, les PII identifiées et les valeurs de remplacement anonymisées](/media/pii/replace.png)
</Frame>
...
Dans l’exemple suivant, nous affectons `postprocess_inputs` pour `anthropic` à la fonction `postprocess_inputs_regex()` () lors de l’initialisation. La fonction `postprocess_inputs_regex` applique la méthode `redact_with_regex` définie dans [Méthode 1 : filtrage par expression régulière](#method-1-regular-expression-filtering). Désormais, `redact_with_regex` sera appliquée à toutes les entrées de n’importe quel modèle `anthropic`.
