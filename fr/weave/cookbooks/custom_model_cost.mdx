...
description: "Définissez et suivez les coûts personnalisés des modèles dans W&B Weave afin de surveiller les dépenses entre différents fournisseurs et modèles de LLM."
---
...
Nous définissons `WANDB_API_KEY` dans notre environnement afin de pouvoir nous connecter facilement avec `wandb.login()` (il doit être fourni à Colab comme secret).

Nous définissons dans `name_of_wandb_project` le projet W&amp;B dans lequel nous voulons journaliser ceci.

***REMARQUE :*** `name_of_wandb_project` peut également être au format `{team_name}/{project_name}` pour spécifier une équipe dans laquelle journaliser les traces.
...
Ici, nous ajoutons un coût personnalisé. Maintenant que nous en avons un et que nos appels ont une utilisation, nous pouvons récupérer les appels avec `include_cost`, et leurs coûts apparaîtront sous `summary.weave.costs`.
