...
description: "Suivez et évaluez des données audio dans vos applications LLM grâce aux fonctionnalités de journalisation audio et de Tracing de W&B Weave."
---
...
<div id="how-to-use-weave-with-audio-data-an-openai-example">
  # Comment utiliser Weave avec des données audio : un exemple avec OpenAI
</div>
...
Pour ce cas d’utilisation avancé, nous exploitons l’API Realtime d’OpenAI afin de diffuser l’audio en temps réel. Cliquez sur la vignette ci-dessous pour voir la démonstration vidéo, ou cliquez [ici](https://www.youtube.com/watch?v=lnnd73xDElw).
...
Commencez par installer les dépendances OpenAI (`openai`) et Weave (`weave`), ainsi que la dépendance `set-env` pour gérer les clés API.
...
<div id="audio-streaming-and-storage-example">
  ## Exemple de streaming Audio et de stockage
</div>

Nous allons maintenant configurer un appel au point de terminaison de complétion d’OpenAI avec la modalité audio activée. Commencez par créer le client OpenAI et initialisez un projet Weave.
...
2. Nous ouvrons un nouveau fichier de sortie dans lequel les données du flux sont écrites bloc par bloc.

3. Nous renvoyons un descripteur de fichier ouvert vers le fichier audio afin que Weave enregistre les données audio dans la trace.
...
def prompt_endpoint_and_log_trace(system_prompt=None, user_prompt=None):
    ...
    with wave.open("./output.wav", "wb") as wav_file:
        ...
        # Écrire les segments au fur et à mesure de leur réception depuis l'API
        for chunk in completion:
            if (
                ...
                # Écrire le segment courant dans le fichier wave
...
Exécutez la cellule suivante. Les prompts système et utilisateur seront enregistrés dans une trace Weave, ainsi que l’audio de sortie.
...
<div id="advanced-usage-realtime-audio-api-with-weave">
  ## Utilisation avancée : API audio Realtime d’OpenAI avec Weave
</div>

<img src="/images/weave/cookbooks/realtime-audio-api.png" alt="intégration de l’API audio Realtime d’OpenAI avec Weave et interface de conversation audio en continu" />

L’API temps réel d’OpenAI est une API conversationnelle riche en fonctionnalités et fiable pour créer des assistants audio et textuels en temps réel.

Veuillez noter :

* Consultez les cellules de [Configuration du microphone](#microphone-configuration)
* En raison des limitations de l’environnement d’exécution de Google Colab, **ce notebook doit être exécuté sur votre machine locale** en tant que notebook Jupyter. Il ne peut pas être exécuté dans le navigateur.
...
* Le paramètre `enable_audio_playback` active la lecture de l’audio généré par l’assistant. Veuillez noter qu’**un casque est requis si cette option est activée**, car la détection d’écho nécessite une implémentation très complexe.
...
Exécutez la cellule suivante pour trouver tous les périphériques Audio disponibles. Renseignez ensuite `INPUT_DEVICE_INDEX` et `OUTPUT_DEVICE_INDEX` en fonction des périphériques répertoriés. Votre périphérique d’entrée doit comporter au moins 1 canal d’entrée, et votre périphérique de sortie doit comporter au moins 1 canal de sortie.
...
CHUNK_DURATION = 0.3  # Secondes d'audio par segment envoyé à l'API OAI
OAI_SAMPLE_RATE = (
    24000  # Le taux d'échantillonnage OAI est de 24 kHz, nécessaire pour lire ou sauvegarder l'audio de l'assistant
)
...
Le SDK Python d’OpenAI ne fournit pas encore d’assistance pour l’API Realtime. Nous implémentons le schéma complet de l’API Realtime d’OAI dans Pydantic pour une meilleure lisibilité, et pourrons le déprécier une fois l’assistance officielle disponible.
...
def parse_server_event(event_data: dict) -> ServerEvent:
    ...
    if not event_type:
        raise ValueError("Les données d'événement sont manquantes dans le champ 'type'")
    ...
    try:
        ...
        raise ValueError(f"Échec de l'analyse de l'événement de type {event_type} : {str(e)}") from e
```
...
class StreamingWavWriter:
    """Écrit des segments audio entiers ou tableaux d'octets dans un fichier WAV."""
    ...
    def get_wav_buffer(self):
        assert self.in_memory, "Le buffer n'est disponible que si le flux est en mémoire."
        return self.buffer
```
...
2. **receive&#95;messages&#95;thread**: Un thread gère la réception des messages de l’API. Quatre principaux types d’événements sont traités : - RESPONSE&#95;AUDIO&#95;TRANSCRIPT&#95;DONE:
   ...
   * RESPONSE&#95;AUDIO&#95;DELTA:
     ...
     Le serveur envoie un nouveau segment audio de la réponse de l’assistant. Nous l’ajoutons aux données de réponse en cours via l’ID de réponse, puis au flux de sortie pour la lecture.

   * RESPONSE&#95;DONE:

     Le serveur indique qu’une réponse de l’assistant est terminée. Nous récupérons tous les segments audio associés à la réponse, ainsi que la transcription, et les journalisons dans Weave.

   3.**send&#95;audio**: Un gestionnaire ajoute les segments audio de l’utilisateur à un tampon et envoie des segments audio lorsque le tampon audio atteint une certaine taille.
...
class RTAudioModel(weave.Model):
    ...
    user_wav_writer: Optional[StreamingWavWriter] = (
        None  # Flux pour écrire la sortie utilisateur dans un fichier
    )
    ...
    playback_stream: Optional[pyaudio.Stream] = (
        None  # Flux de lecture pour diffuser les réponses de l'assistant
    )
        ...
        try:
            ...
            config_event = SessionUpdate(
                session=Session(
                    ...
                    ),  # VAD serveur pour détecter le silence
                )
            )
            ...
            print(f"Erreur lors de la connexion au WebSocket : {e}")
    ...
    def send_audio(self, audio_chunk):
        if self.ws and self.ws.connected:
            ...
            if len(self.input_audio_buffer) >= SAMPLE_RATE * CHUNK_DURATION:
                try:
                    # Rééchantillonner l'audio au taux d'échantillonnage OAI
                    resampled_audio = (
                        resampy.resample(
                    ...
                    audio_event = InputAudioBufferAppend(
                        ...
                        ).decode("utf-8")  # Convertir le tableau audio en octets b64
                    )
...
Nous utilisons un flux d’entrée PyAudio avec un gestionnaire lié à la méthode `send_audio` du modèle RTAudio. Le flux est renvoyé au thread principal afin de pouvoir être arrêté en toute sécurité à la fin du programme.

```python lines
# Flux de capture audio
def record_audio(realtime_model: RTAudioModel) -> pyaudio.Stream:
    """Configure un flux d'entrée Pyaudio et utilise RTAudioModel comme rappel pour la diffusion des données."""
...
<div id="main-thread-run-me">
  ## Thread principal (Exécutez-moi !)
</div>

Le thread principal initialise un modèle Audio en temps réel avec intégration de Weave. Ensuite, il démarre un enregistrement et attend une interruption clavier de l’utilisateur.
...
if realtime_model.ws and realtime_model.ws.connected:
    ...
    print(
        "Échec de la connexion WebSocket. Veuillez vérifier votre clé API et votre connexion Internet."
    )
```
