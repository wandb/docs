...
description: "Suivez et évaluez des données audio dans vos applications LLM grâce aux fonctionnalités de journalisation audio et de traçage de W&B Weave."
---
...
Pour ce cas d’utilisation avancé, nous exploitons l’API OpenAI Realtime afin de diffuser l’audio en temps réel. Cliquez sur la vignette ci-dessous pour voir la démonstration vidéo, ou cliquez [ici](https://www.youtube.com/watch?v=lnnd73xDElw).
...
Nous allons maintenant configurer un appel au point de terminaison `completions` d’OpenAI avec la modalité audio activée. Commencez par créer le client OpenAI et initialisez un projet Weave.
...
1. Nous créons un objet de complétion à l’aide du modèle `GPT 4o Audio Preview`, qui prend en charge les entrées et sorties textuelles et audio.

   * Nous demandons au modèle de compter lentement jusqu’à 13 avec des accents variés.
   * Nous configurons la complétion sur &quot;stream&quot;.

2. Nous ouvrons un nouveau fichier de sortie dans lequel les données transmises en flux sont écrites segment par segment.

3. Nous renvoyons un descripteur de fichier ouvert vers le fichier audio afin que Weave journalise les données audio dans la trace.
...
Exécutez la cellule suivante. Les prompts système et utilisateur seront stockés dans une trace Weave, ainsi que l’audio de sortie.
Après avoir exécuté la cellule, cliquez sur le lien à côté de l’emoji &quot;🍩&quot; pour voir votre trace.
...
<img src="/images/weave/cookbooks/realtime-audio-api.png" alt="intégration de l’API Audio en temps réel avec Weave et interface de conversation audio en streaming" />

L’API en temps réel d’OpenAI est une API conversationnelle très complète et fiable pour créer des assistants audio et texte en temps réel.

Veuillez noter :

* Consultez les cellules de [Configuration du microphone](#microphone-configuration)
* En raison des limitations de l’environnement d’exécution de Google Colab, **ce notebook doit être exécuté sur votre machine hôte** en tant que notebook Jupyter. Il ne peut pas être exécuté dans le navigateur.
...
* Le paramètre `enable_audio_playback` active la lecture de l’audio généré par l’assistant. Veuillez noter que **des écouteurs sont requis si cette option est activée**, car la détection d’écho nécessite une implémentation très complexe.
...
Exécutez la cellule suivante pour trouver tous les périphériques Audio disponibles. Renseignez ensuite `INPUT_DEVICE_INDEX` et `OUTPUT_DEVICE_INDEX` en fonction des périphériques répertoriés. Votre périphérique d’entrée doit avoir au moins 1 canal d’entrée, et votre périphérique de sortie doit avoir au moins 1 canal de sortie.
...
Le SDK Python d’OpenAI ne fournit pas encore d’assistance pour l’API Realtime. Nous implémentons l’intégralité du schéma de l’API Realtime d’OAI dans Pydantic pour en améliorer la lisibilité, et pourrons le marquer comme obsolète une fois l’assistance officielle disponible.
...
2. **receive&#95;messages&#95;thread**: Un thread gère la réception des messages de l’API. Quatre principaux types d’événements sont traités : - RESPONSE&#95;AUDIO&#95;TRANSCRIPT&#95;DONE:
   ...
   * RESPONSE&#95;AUDIO&#95;DELTA:
     ...
     Le serveur envoie un nouveau segment de l’audio de réponse de l’assistant. Nous l’ajoutons aux données de réponse en cours via l’ID de réponse, puis au flux de sortie pour la lecture.
...
Le thread principal initialise un modèle Audio en temps réel avec Weave intégré. Ensuite, il ouvre un enregistrement et attend une interruption clavier de l’utilisateur.
