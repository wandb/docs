...
2. **Évaluer les sorties du LLM** : Créez et comparez des évaluations de votre code généré à l’aide d’outils de débogage riches et de visualisations.
...
<Note>
  Weave assure automatiquement le suivi des appels à l’API OpenAI, y compris les entrées, les sorties et les métadonnées. Cela signifie que vous n’avez pas besoin d’ajouter de code de journalisation supplémentaire pour vos interactions avec OpenAI – Weave s’en charge de manière transparente en arrière-plan.
</Note>
...
1. **Sécurité des types** : en définissant des modèles Pydantic pour nos sorties attendues, nous appliquons une structure stricte au code généré, aux exécuteurs de programme et aux tests unitaires.
...
Voici un exemple de la manière dont nous définissons nos modèles Pydantic et les utilisons avec les sorties structurées d’OpenAI :
...
Pour garantir un code généré cohérent et propre, nous implémentons une classe `CodeFormatter` à l’aide des opérations Weave. Ce formateur applique diverses règles de linting et de style au code généré, au moteur d’exécution du programme et aux tests unitaires.
...
Cette classe `CodeFormatter` fournit plusieurs opérations Weave pour nettoyer et formater le code généré :
...
Nous utilisons un `weave.Model` afin qu’il soit automatiquement versionné à chaque modification. Nous conservons également `model_name` comme attribut afin de pouvoir expérimenter avec et voir facilement les différences, puis le comparer dans Weave. Nous suivons les appels de fonction avec `@weave.op` afin que les entrées et les sorties soient journalisées pour faciliter le suivi des erreurs et le débogage.
...
Cette classe `CodeGenerationPipeline` encapsule notre logique de génération de code dans un modèle Weave, offrant plusieurs avantages clés :
...
5. Intégration avec l’écosystème Weave : l’utilisation de `weave.Model` permet une intégration transparente avec d’autres outils Weave, comme les évaluations et les fonctionnalités de serving.

<div id="implement-evaluation-metrics">
  ## Implémenter des métriques d&#39;évaluation
</div>

Pour évaluer la qualité du code généré, nous allons implémenter des métriques d&#39;évaluation simples à l&#39;aide d&#39;une sous-classe de `weave.Scorer`. Cette sous-classe exécutera `score` sur chaque `model_output` de notre jeu de données. `model_output` provient de la sortie de la fonction `predict` de notre `weave.Model`. `prompt` est extrait de notre jeu de données `human-eval`.
...
Ce code crée un jeu de données à partir de nos exemples de prompts, définit notre scorer de test HumanEval et lance une évaluation de notre pipeline de génération de code.
...
Pour en savoir plus sur Weave et ses fonctionnalités, consultez la [documentation Weave](/fr/weave). Vous pouvez étendre cet exemple pour gérer des jeux de données plus volumineux, implémenter des métriques d’évaluation plus sophistiquées ou intégrer d’autres flux de travail LLM.