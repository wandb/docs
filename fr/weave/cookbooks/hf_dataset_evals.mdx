...
> Ce guide présente une solution de contournement pour utiliser HuggingFace Datasets dans des évaluations Weave.<br /><br />
...
* Créer un mappage d’index pour référencer les lignes du jeu de données.
...
* `preprocess_example` : Transforme la référence d’index en données effectives nécessaires à l’évaluation
* `hf_eval` : Définit comment évaluer les sorties du modèle
...
def hf_eval(hf_id: int, output: dict) -> dict:
    """
    Fonction de score pour évaluer les sorties du modèle.
    Arguments :
...
* Pour chaque index de hf&#95;index :
  1. `preprocess_example` récupère les données correspondantes dans le jeu de données HF.
  2. Les données prétraitées sont transmises à `function_to_evaluate`.
  3. La sortie reçoit un score à l’aide de `hf_eval`.
