...
description: "Importez des jeux de données à partir de fichiers CSV dans W&B Weave afin de les utiliser dans des évaluations, le Tracing et des flux de travail de comparaison de modèles."
---
...
Il n’est parfois pas possible d’instrumenter votre code Python ou JavaScript avec l’intégration simple de Weave pour obtenir des traces en temps réel de votre application GenAI. Il arrive souvent que vous disposiez ensuite de ces traces au format `csv` ou `json`.

Dans ce cookbook, nous explorons l’API Python de bas niveau de Weave pour extraire des données d’un fichier CSV et les importer vers Weave afin d’en tirer des enseignements et de mener des évaluations rigoureuses.
...
Nous utilisons donc `conversation_id` comme identifiant parent, et `turn_index` comme identifiant enfant, afin d’obtenir une journalisation complète des conversations.
...
Nous chargeons les données dans un dataframe Pandas et veillons à les trier par `conversation_id` et `turn_index` afin de garantir que les éléments parents et enfants sont correctement ordonnés.
...
* Nous parcourons la liste des tours pour créer des appels enfants triés selon leur `turn_index`
...
* Un appel Weave peut avoir d’autres éléments qui lui sont associés : Feedback, métadonnées, etc. Ici, nous ne lui associons que des entrées et des sorties, mais vous pouvez ajouter ces éléments lors de l’import si les données les fournissent.
...
* La valeur `op` d’un appel correspond à la manière dont Weave catégorise les appels de même structure. Dans cet exemple, tous les appels parents sont de type `Conversation`, et tous les appels enfants sont de type `Turn`. Vous pouvez modifier cela comme bon vous semble.
* Un appel peut avoir des `inputs` et un `output`. Les `inputs` sont définis à la création, et l’`output` est défini lorsque l’appel est terminé.
...
<div id="result-traces-are-logged-to-weave">
  # Résultat : les traces sont enregistrées vers Weave
</div>
...
Une fois nos traces dans Weave et après avoir compris à quoi ressemblent les conversations, nous pourrons vouloir les exporter vers un autre processus afin d’exécuter des évaluations dans Weave.

<Frame>
  ![image.png](/images/screenshots/csv-3.png)
</Frame>

Pour ce faire, nous récupérons toutes les conversations depuis W&amp;B via notre API de requête simple et en créons un jeu de données.
...
# accédant aux Opérations de votre projet dans l'interface utilisateur, en cliquant sur l'objet
