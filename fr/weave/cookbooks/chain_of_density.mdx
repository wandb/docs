...
2. **Évaluer les sorties LLM** : créez des évaluations rigoureuses et réellement comparables de vos résumés à l&#39;aide des outils intégrés de Weave.
...
Nous allons créer une simple classe `ArxivPaper` pour représenter nos données :
...
Cette classe encapsule les métadonnées et le contenu d’un article ArXiv, qui serviront d’entrée à notre pipeline de synthèse.
...
* `summarize_current_summary` : génère une itération unique du résumé à partir de l’état actuel.
...
En utilisant les décorateurs `@weave.op()`, nous nous assurons que Weave suit les entrées, les sorties et l’exécution de ces fonctions.
...
<Frame>
  ![Interface de configuration du modèle Weave pour la synthèse Chain of Density, avec les réglages et paramètres du modèle](/media/summarization/model.png)
</Frame>
...
Pour évaluer la qualité de nos résumés, nous allons implémenter de simples métriques d’évaluation :
...
<div id="create-a-weave-dataset-and-run-evaluation">
  ## Créer un jeu de données Weave et exécuter une évaluation
</div>

Pour évaluer notre pipeline, nous allons créer un jeu de données Weave et exécuter une évaluation :
