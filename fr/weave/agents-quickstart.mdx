...
[Essayer dans Colab](https://colab.research.google.com/github/wandb/weave/blob/master/docs/weave/cookbooks/source/agents-quickstart.ipynb) · [Code source sur GitHub](https://github.com/wandb/weave/blob/master/docs/weave/cookbooks/source/agents-quickstart.ipynb)

Le SDK Weave vous permet de tracer des agents créés avec des SDK populaires ou des harnesses personnalisés. Ce Démarrage rapide vous montre comment intégrer manuellement Weave à un agent multi-tour personnalisé afin d’émettre et de capturer des spans OpenTelemetry.

Si vous cherchez à intégrer Weave à des SDK ou à des harnesses populaires, comme le Claude Agent SDK ou Codex, voir la [section des intégrations Weave](/fr/weave/guides/integrations). Weave applique automatiquement des patchs à plusieurs SDK populaires de création d’agents ainsi qu’à des harnesses d’agents pour une intégration rapide.
...
* Consultez la [section d’intégration](/fr/weave/guides/integrations) pour découvrir d’autres options permettant d’intégrer Weave à vos agents.