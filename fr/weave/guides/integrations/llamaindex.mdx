...
Dans l’exemple ci-dessus, nous créons un moteur de chat LlamaIndex simple qui, en coulisses, effectue un appel OpenAI. Consultez la trace ci-dessous :
...
Vous trouverez ci-dessous un exemple de pipeline RAG simple tiré du [Starter Tutorial (OpenAI)](https://docs.llamaindex.ai/en/stable/getting_started/starter_example/) de LlamaIndex :
...
# En supposant que vous avez un fichier `.txt` dans le répertoire `data`
...
Notre intégration exploite cette fonctionnalité de LlamaIndex et assigne automatiquement [`WeaveCallbackHandler()`](https://github.com/wandb/weave/blob/master/weave/integrations/llamaindex/llamaindex.py) à `llama_index.core.global_handler`. Ainsi, en tant qu’utilisateur de LlamaIndex et de Weave, il vous suffit d’initialiser un run Weave : `weave.init(<name-of-project>)`

<div id="create-a-model-for-easier-experimentation">
  ## Créez un `Model` pour faciliter l&#39;expérimentation
</div>

Organiser et évaluer des LLM dans des applications pour divers cas d&#39;usage est difficile avec des composants multiples, comme les prompts, les configurations de modèle et les paramètres d&#39;inférence. En utilisant [`weave.Model`](/fr/weave/guides/core-types/models), vous pouvez capturer et organiser des détails expérimentaux comme les prompts système ou les modèles que vous utilisez, ce qui facilite la comparaison entre différentes itérations.

L&#39;exemple suivant montre comment construire un moteur de requête LlamaIndex dans un `WeaveModel`, en utilisant des données disponibles dans le dossier [weave/data](https://github.com/wandb/weave/tree/master/data) :
...
Cette classe `SimpleRAGPipeline`, qui est une sous-classe de `weave.Model`, organise les paramètres importants de ce pipeline RAG. Décorer la méthode `query` avec `weave.op()` permet le Tracing.
...
Cette évaluation s’appuie sur l’exemple de la section précédente. L’évaluation avec `weave.Evaluation` nécessite un jeu de données d’évaluation, une fonction scorer et un `weave.Model`. Voici quelques nuances concernant les trois composants clés :

* Assurez-vous que les clés des dicts des échantillons d’évaluation correspondent aux arguments de la fonction scorer ainsi qu’à ceux de la méthode `predict` du `weave.Model`.
