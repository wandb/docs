...
Cet exemple exécute un assistant vocal en temps réel qui transmet en flux l’audio du microphone à l’API Realtime d’OpenAI et restitue les réponses orales de l’IA via le haut-parleur de votre machine locale. L’application utilise le SDK OpenAI Agents avec `RealtimeAgent` et `RealtimeRunner`, et active le traçage Weave en appliquant `patch_openai_realtime()`.
...
2. Créez un fichier nommé `weave_voice_assistant.py` et ajoutez-y le code suivant.
   ...
   Les lignes surlignées indiquent l’intégration de Weave dans l’application. Le reste du code crée l’application de base d’assistant vocal.
   ...
   <Accordion title="weave_voice_assistant.py">
     ...
     def parse_args():
         ...
         parser.add_argument(
             ...
             type=int,
             default=None,
             help="Index du périphérique de sortie PyAudio (haut-parleur). Par défaut, utilise le périphérique système par défaut. Exécutez mic_detect.py pour obtenir la liste des périphériques.",
             dest="output_device"
         )
...
Une fois en cours d’exécution, appuyez sur **T** sur votre clavier pour couper ou réactiver le micro. L’assistant utilise la détection d’activité vocale côté serveur pour gérer les prises de parole et les interruptions.

Lorsque vous parlez à l’assistant, Weave capture des traces que vous pouvez explorer dans l’interface utilisateur de Weave, y compris l’audio de la session.

<div id="run-a-realtime-voice-assistant-using-websockets">
  ### Exécuter un assistant vocal en temps réel avec WebSockets
</div>

L’exemple suivant se connecte directement à l’API Realtime d’OpenAI via WebSockets. Il diffuse en continu l’audio du microphone vers l’API, restitue les réponses vocales et prend en charge l’appel d’outils (consultation de la météo, évaluation mathématique, exécution de code et écriture de fichiers). Weave effectue le traçage de la session à l’aide de `weave.init()` et `patch_openai_realtime()`.
...
2. Créez un fichier intitulé `tool_definitions.py` et ajoutez-y les définitions d’outils suivantes. L’application principale importe son contenu depuis ce module.
   ...
   <Accordion title="tool_definitions.py">
     ...
     async def write_file(file_path: str, content: str) -> str:
         """Écrire du contenu dans un fichier sur le disque.
...
3. Créez un fichier nommé `weave_ws_voice_assistant.py` dans le même répertoire, puis ajoutez-y le code suivant.
   ...
   <Accordion title="weave_ws_voice_assistant.py">
     ...
     def play_audio(output_stream: pyaudio.Stream, audio_output_queue: queue.Queue):
         """S'exécute dans un thread séparé car la méthode write() de pyaudio est
         bloquante jusqu'à ce que la carte son consomme les échantillons. Découpler
         la lecture de la boucle d'événements asynchrone permet de vider la file
         d'attente lors d'une interruption sans attendre la fin des écritures en cours."""
         while True:
             data = audio_output_queue.get()
             if data is None:
                 break
     ...
     async def main():
         ...
         # Le nombre de canaux doit correspondre aux capacités du périphérique, sinon pyaudio génère une erreur à l'ouverture.
         input_info = p.get_device_info_by_index(input_device_index)
         output_info = p.get_device_info_by_index(output_device_index)
         ...
         print("Connexion à l'API temps réel d'OpenAI...")
...
4. Mettez à jour l’appel à `weave.init()` avec le nom de votre équipe et celui de votre projet.
