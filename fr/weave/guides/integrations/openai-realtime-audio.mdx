...
keywords: ["de voix à voix", "assistant vocal", "WebSockets", "SDK Agents", "traçage audio", "TypeScript"]
---
...
Ce guide vous montre comment activer le traçage de l’API Realtime dans votre application et vous présente des exemples complets en Python et TypeScript que vous pouvez exécuter localement.

## Intégrer les traces en temps réel {#integrate-realtime-traces}

Cette section présente le code minimal nécessaire pour activer le tracing Weave pour l’API Realtime d’OpenAI dans toute application.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    En TypeScript, Weave trace l’API Realtime via le [SDK OpenAI Agents](https://openai.github.io/openai-agents-js/). L’importation de Weave instrumente automatiquement chaque `RealtimeSession` de `@openai/agents-realtime`, vous n’avez donc pas besoin d’appeler une fonction de patch. L’initialisation de votre projet Weave est la seule étape requise :
    ...
    Si votre configuration contourne les hooks du chargeur de modules Node.js que Weave utilise pour l’instrumentation automatique, par exemple avec des imports dynamiques ou certains bundlers, appelez [`patchRealtimeSession()`](/fr/weave/reference/typescript-sdk/functions/patchrealtimesession) une fois au démarrage, avant que votre application ne crée des sessions :
...
### Exécutez un agent en temps réel avec le SDK OpenAI Agents {#run-a-realtime-agent-with-the-openai-agents-sdk}

<Tabs>
  <Tab title="Python">
    Cet exemple exécute un assistant vocal en temps réel qui diffuse l’audio du microphone vers l’API Realtime d’OpenAI et restitue les réponses vocales de l’IA sur le haut-parleur de votre machine locale. L’application utilise le SDK OpenAI Agents avec `RealtimeAgent` et `RealtimeRunner`, et active le tracing Weave via le patching avec `patch_openai_realtime()`. Adoptez cette approche lorsque vous souhaitez que le SDK Agents, de plus haut niveau, gère la session Realtime à votre place.
    ...
    2. Créez un fichier nommé `weave_voice_assistant.py` et ajoutez-y le code suivant.
       ...
       Les lignes surlignées montrent l’intégration de Weave dans l’application. Le reste du code crée l’application de base de l’assistant vocal.
    ...
    Une fois l’assistant lancé, appuyez sur la touche `T` de votre clavier pour couper ou réactiver le micro. L’assistant utilise la détection d’activité vocale côté serveur pour gérer la prise de parole à tour de rôle et les interruptions.
  ...
  <Tab title="TypeScript">
    Cet exemple crée un agent en temps réel avec `RealtimeAgent` et `RealtimeSession`, lui fournit un outil météo et lui envoie un message texte. Weave instrumente automatiquement la session lors de son importation et capture la session, les générations, les appels d’outil et la sortie audio sous forme de traces.

    Pour exécuter l’exemple, effectuez les étapes suivantes :

    1. Dans un nouveau répertoire, configurez un projet Node.js avec TypeScript et installez les bibliothèques suivantes :
    ...
    2. Créez un fichier nommé `weave_realtime_agent.ts` et ajoutez-y le code suivant.

       Les lignes surlignées montrent l’intégration de Weave dans l’application. Le reste du code crée l’agent de base en temps réel.

       <Accordion title="weave_realtime_agent.ts">
         ```typescript lines {1,32,34-36}
         import * as weave from 'weave';
         import {RealtimeAgent, RealtimeSession, tool} from '@openai/agents/realtime';
         import {z} from 'zod';

         const WEAVE_PROJECT = '<your-team-name/your-project-name>';

         // Outil de recherche météo simulé que l’agent peut appeler.
         ...
         const agent = new RealtimeAgent({
           name: 'Assistant',
           instructions:
             'You are a helpful voice assistant. Keep responses concise because this is a voice conversation.',
         ...
         async function main() {
           ...
           // Weave instrumente automatiquement `@openai/agents-realtime` lors de l’importation.
           // Si votre configuration contourne les hooks du chargeur de modules, décommentez la ligne suivante :
           ...
           // Affiche les réponses textuelles de l’assistant à mesure que l’historique de la conversation est mis à jour.
           ...
           // Attend la réponse, puis se déconnecte.
    ...
    3. Mettez à jour la valeur de `WEAVE_PROJECT` avec le nom de votre équipe et celui de votre projet.

    4. Définissez les variables d’environnement `OPENAI_API_KEY` et `WANDB_API_KEY`.

    5. Compilez et exécutez l’agent :

       ```bash
       npx tsc
       node --import=weave/instrument --import=tsx  weave_realtime_agent.js
       ```

    La première option `--import` précharge l’instrumentation de Weave et la seconde précharge `tsx` afin que Node puisse exécuter TypeScript.

    L’agent se connecte à l’API Realtime, interroge l’outil météo sur la météo à San Francisco, puis se déconnecte. Dans l’interface Weave, vous pouvez explorer la trace obtenue, y compris la session, la génération, l’appel d’outil et la sortie audio de l’assistant.
  </Tab>
</Tabs>

### Exécuter un assistant vocal en temps réel avec WebSockets {#run-a-realtime-voice-assistant-with-websockets}

L’exemple Python suivant se connecte directement à l’API Realtime d’OpenAI via WebSockets. Il transmet l’audio du microphone à l’API, lit les réponses vocales et prend en charge l’appel d’outils (recherches météo, évaluation mathématique, exécution de code et écriture de fichiers). Weave trace la session à l’aide de `weave.init()` et `patch_openai_realtime()`. Utilisez cette approche lorsque vous souhaitez un contrôle total sur la session Realtime sans dépendre du SDK Agents.
...
Une fois que l’assistant est en cours d’exécution, appuyez sur la touche `T` de votre clavier pour couper ou réactiver le micro. L’assistant utilise la détection de l’activité vocale côté serveur pour gérer la prise de parole à tour de rôle et les interruptions.
