...
<Warning>
  Actuellement, l&#39;intégration capture séparément les opérations côté client et côté serveur, mais n&#39;offre pas de visibilité de bout en bout sur leur interaction. Une proposition est en cours pour ajouter la prise en charge des traces OpenTelemetry à MCP afin de permettre une observabilité de bout en bout. Pour plus d&#39;informations, voir [la discussion GitHub #269](https://github.com/modelcontextprotocol/modelcontextprotocol/discussions/269).
</Warning>

L&#39;intégration Weave trace automatiquement les composants clés du Model Context Protocol (MCP) en patchant les méthodes principales avec le décorateur [`weave.op()`](../tracking/ops). Plus précisément, elle patche les méthodes des classes [`mcp.server.fastmcp.FastMCP`](https://github.com/modelcontextprotocol/python-sdk/blob/b4c7db6a50a5c88bae1db5c1f7fba44d16eebc6e/src/mcp/server/fastmcp/server.py#L109) et [`mcp.ClientSession`](https://github.com/modelcontextprotocol/python-sdk/blob/b4c7db6a50a5c88bae1db5c1f7fba44d16eebc6e/src/mcp/client/session.py#L84).
...
L’intégration Weave fonctionne aussi bien avec le serveur MCP qu’avec le client. Une fois installée, vous pouvez activer le traçage avec seulement deux lignes de code supplémentaires : l’une pour importer `weave`, l’autre pour l’initialiser.
...
* `MCP_TRACE_LIST_OPERATIONS` : définissez cette variable sur `true` pour activer la trace des opérations de liste (`list_tools`, `list_resources` et `list_prompts`) côté serveur comme côté client.
...
Côté client, le traçage ne nécessite lui aussi que deux changements : importer Weave et l’initialiser. Tous les appels d’outil, les accès aux ressources et les requêtes de prompt seront automatiquement tracés.
...
L’exemple `mcp_demo` illustre une intégration entre le Model Context Protocol (MCP) et Weave pour le traçage. Il montre comment instrumenter à la fois les composants client et serveur afin de capturer des traces détaillées de leurs interactions. 
...
3. Exécutez la démo :
   ...
   Cette commande lance à la fois le client et le serveur. Le client démarre une CLI interactive dans laquelle vous pouvez tester différentes fonctionnalités.
...
| `demo`                  | Exécuter une démonstration complète de toutes les fonctionnalités disponibles. Cela exécute chaque fonctionnalité successivement et produit une chronologie complète des traces d’interaction dans l’interface Weave. |
...
* Illustrer l’utilisation de [`weave.op()`](../tracking/ops) avec des méthodes/fonctions personnalisées.
...
<div id="why-is-mcp-tracing-needed">
  ### Pourquoi le traçage MCP est-il nécessaire ?
</div>
...
* *Développeur côté client MCP* : vous souhaitez connecter votre application côté client à plusieurs serveurs MCP. Une part essentielle de votre logique côté client consiste à effectuer des appels à un LLM pour décider quel outil appeler ou quelle ressource récupérer.

* *Développeur serveur et client MCP* : vous développez à la fois le serveur et le client.

Si vous appartenez à l’une des deux premières catégories, vous voulez savoir quand chaque outil est appelé, à quoi ressemble le flux d’exécution, ainsi que le nombre de jetons et la latence des différents composants de votre logique côté serveur ou côté client.
