...
# les appels d'ajout en mémoire ainsi que les appels de récupération sous une seule trace
# c'est entièrement facultatif mais fortement recommandé de l'utiliser
...
async def run_rag_agent(model_name="gpt-4o"):
    ...
    async def index_autogen_docs() -> None:
        ...
        print(f"Indexé {chunks} fragments depuis {len(sources)} documents AutoGen")
    ...
    # Cet exemple essaiera d'indexer à chaque run. Envisagez d'ajouter une vérification.
    ...
    # Pour une sortie en streaming vers la console :
...
<Warning>
  **Docker requis**
  Cet exemple implique l&#39;exécution de code avec Docker et peut ne pas fonctionner dans tous les environnements (par ex. directement dans Colab). Assurez-vous que Docker est en cours d’exécution en local si vous essayez cet exemple.
</Warning>
