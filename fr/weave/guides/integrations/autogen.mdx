...
{/* TODO : Ajouter un notebook Colab pour ce guide, avec un lien de badge ici. */}
...
Weave s’intègre à [AutoGen](https://microsoft.github.io/autogen/stable/index.html) pour vous aider à tracer et à visualiser l’exécution de vos applications multi-agents. En initialisant simplement Weave, vous pouvez suivre automatiquement les interactions dans `autogen_agentchat`, `autogen_core` et `autogen_ext`. Ce guide vous guide à travers divers exemples de comment utiliser Weave avec AutoGen.

<div id="prerequisites">
  ## Prérequis
</div>

Avant de commencer, assurez-vous qu’AutoGen et Weave sont installés. Vous aurez également besoin des SDK des fournisseurs de LLM que vous comptez utiliser (par exemple, OpenAI, Anthropic).
...
Initialisez Weave au début de votre script pour commencer à capturer des traces.
...
Weave peut tracer les appels effectués directement aux clients de modèle dans AutoGen.

<div id="tracing-a-client-create-call">
  ### Tracing d’un appel de création du client
</div>
...
async def simple_client_call(model_name = "gpt-4o"):
    ...
    # Alternativement, vous pouvez utiliser Anthropic ou d'autres clients de modèles
...
<div id="tracing-a-client-create-call-with-streaming">
  ### Tracing d&#39;un appel de création de client avec streaming
</div>

Weave prend également en charge le tracing des réponses en flux.
...
Vous pouvez utiliser le `ChatCompletionCache` d’AutoGen, et Weave trace ces interactions, en indiquant si une réponse provient du cache ou d’un nouvel appel.
...
async def run_cache_client(model_name = "gpt-4o"):
      ...
      print(response)  # Doit afficher la réponse en cache
...
Weave trace les agents et leur utilisation des outils, offrant une visibilité sur la façon dont les agents sélectionnent et exécutent les outils.
...
Les interactions au sein des conversations de groupe, comme `RoundRobinGroupChat`, sont tracées par Weave, ce qui vous permet de suivre le fil de la conversation entre les agents.
...
# nous ajoutons cette op Weave ici parce que nous voulons tracer l'ensemble de la conversation de groupe
# c'est complètement facultatif mais il est fortement recommandé de l'utiliser
...
async def run_round_robin_group_chat(model_name="gpt-4o"):
    ...
    await team.reset()
    # Pour la sortie en streaming vers la console :
...
Les composants de mémoire d’AutoGen peuvent être tracés avec Weave. Vous pouvez utiliser `@weave.op()` pour regrouper les opérations de mémoire sous une seule trace afin d&#39;améliorer la lisibilité.
...
# les appels d'ajout à la mémoire ainsi que les appels d'obtention de la mémoire sous une seule trace
...
async def run_memory_agent(model_name="gpt-4o"):
    ...
    # Pour afficher la sortie en streaming vers la console :
...
Les flux de travail de génération augmentée par récupération (RAG), y compris l&#39;indexation et la récupération de documents avec des systèmes de mémoire comme `ChromaDBVectorMemory`, sont traçables. Décorer le processus RAG avec `@weave.op()` aide à visualiser l&#39;ensemble du flux.
...
async def run_rag_agent(model_name="gpt-4o"):
    ...
    # À des fins de démonstration, vous pouvez l'indexer à chaque fois ou vérifier s'il est déjà indexé.
...
<div id="tracing-agent-runtimes">
  ## Tracing des runtimes d&#39;agents
</div>

Weave peut tracer des opérations au sein des runtimes d&#39;agents d&#39;AutoGen, comme `SingleThreadedAgentRuntime`. Utiliser `@weave.op()` autour de la fonction d&#39;exécution du runtime permet de regrouper les traces associées.
...
# nous ajoutons cet op Weave ici car nous voulons tracer 
# l'ensemble de l'appel du runtime de l'agent sous une unique trace
# c'est totalement facultatif mais il est fortement recommandé de l'utiliser
...
<div id="tracing-workflows-sequential">
  ## Tracing des flux de travail (séquentiels)
</div>

Vous pouvez tracer des flux de travail d’agents complexes qui définissent des séquences d’interactions entre agents. Utilisez `@weave.op()` pour fournir une trace de haut niveau pour l’ensemble du flux de travail.
...
@weave.op(call_display_name="Flux de travail d'agent séquentiel")
...
<Warning>
  **Docker requis**
  Cet exemple implique l&#39;exécution de code à l&#39;aide de Docker et peut ne pas fonctionner dans tous les environnements (par exemple, directement dans Colab). Assurez-vous que Docker s&#39;exécute localement si vous essayez cela.
</Warning>

Weave trace la génération et l&#39;exécution du code par les agents AutoGen.
...
# nous ajoutons cette op Weave ici parce que nous voulons tracer
...
async def run_codegen(model_name="gpt-4o"): # Modèle mis à jour
    ...
    # Assurez-vous que Docker est en cours d'exécution pour cet exemple
    try:
...
Ce guide fournit un point de départ pour intégrer Weave avec AutoGen. Explorez l’interface Weave pour voir des traces détaillées de vos interactions avec les agents, des appels de modèle et de l’utilisation des outils.