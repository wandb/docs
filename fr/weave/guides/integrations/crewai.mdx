---
title: "CrewAI"
description: "Intégrez CrewAI avec Weave pour la surveillance et le tracing des applications multi-agents"
---
...
CrewAI est un framework Python léger et ultra-rapide, entièrement développé de zéro — complètement indépendant de LangChain ou d&#39;autres frameworks d&#39;agents. CrewAI offre aux développeurs à la fois une simplicité de haut niveau ([Crews](https://docs.crewai.com/guides/crews/first-crew)) et un contrôle précis de bas niveau ([Flows](https://docs.crewai.com/guides/flows/first-flow)), idéal pour créer des agents d&#39;IA autonomes adaptés à tous les scénarios. En savoir plus sur [CrewAI ici](https://docs.crewai.com/introduction).

Lorsque vous travaillez avec des agents d&#39;IA, le débogage et la surveillance de leurs interactions sont essentiels. Les applications CrewAI se composent souvent de plusieurs agents travaillant ensemble, ce qui rend indispensable de comprendre comment ils collaborent et communiquent. Weave simplifie ce processus en capturant automatiquement les traces de vos applications CrewAI, ce qui vous permet de surveiller et d&#39;analyser les performances et les interactions de vos agents.

L&#39;intégration prend en charge à la fois Crews et Flows.

<div id="getting-started-with-crew">
  ## Premiers pas avec Crew
</div>

Vous devez installer CrewAI ([plus d’informations](https://docs.crewai.com/installation)) et Weave pour exécuter cet exemple :

```
pip install crewai weave
```

Nous allons maintenant créer un crew CrewAI et tracer l’exécution avec Weave. Pour commencer, appelez simplement `weave.init()` au début de votre script. L’argument passé à `weave.init()` est le nom du projet dans lequel les traces seront enregistrées.
...
Weave tracera et consignera tous les appels effectués via la bibliothèque CrewAI, y compris les interactions entre agents, les exécutions de tâches et les appels LLM. Vous pouvez consulter les traces dans l’interface web de Weave.
...
<Note>
  CrewAI fournit plusieurs méthodes pour mieux contrôler le processus de lancement : `kickoff()`, `kickoff_for_each()`, `kickoff_async()` et `kickoff_for_each_async()`. L’intégration prend en charge la journalisation des traces pour toutes ces méthodes.
</Note>
...
Les outils CrewAI offrent aux agents des capacités allant de la recherche sur le Web et de l’analyse de données à la collaboration et à la délégation de tâches entre collègues. L’intégration peut également les tracer.

Nous allons améliorer la qualité du rapport généré dans l’exemple ci-dessus en lui donnant accès à un outil capable de rechercher sur Internet et de renvoyer les résultats les plus pertinents.
...
Dans cet exemple, nous utilisons `SerperDevTool` pour permettre à notre agent &#39;Research Analyst&#39; de rechercher des informations pertinentes sur Internet. Pour en savoir plus sur cet outil et les prérequis de l’API, consultez [cette page](https://docs.crewai.com/tools/serperdevtool).
...
L’exécution de ce Crew avec un agent ayant accès à Internet produit de meilleurs résultats, plus pertinents. Nous traçons automatiquement l’utilisation des outils, comme le montre l’image ci-dessous.
...
class CustomerFeedbackFlow(Flow):
    ...
    def fetch_feedback(self):
        print("Récupération des retours clients")
...
<Note>
  L’intégration instrumente automatiquement le point d’entrée `Flow.kickoff` ainsi que tous les décorateurs disponibles : `@start`, `@listen`, `@router`, `@or_` et `@and_`.
</Note>


<div id="crew-guardrail-track-your-own-ops">
  ## Crew Guardrail - Suivre vos propres ops
</div>

Les garde-fous de tâche permettent de valider et de transformer les sorties d’une tâche avant qu’elles ne soient transmises à la tâche suivante. Vous pouvez utiliser une simple fonction Python pour valider à la volée l’exécution de l’agent.

En décorant cette fonction avec `@weave.op`, vous commencez à capturer les entrées, les sorties et la logique applicative, ce qui vous permet de déboguer la façon dont les données sont validées par vos agents. Cela active également automatiquement la gestion des versions du code pendant vos expériences, afin de capturer des détails ad hoc qui n’ont pas encore été enregistrés dans Git.
...
writing_task = Task(
    ...
    expected_output='Le rapport doit être facile à lire et à comprendre. Utilisez des puces là où c\'est approprié.',
    agent=writer,
...
En décorant simplement la fonction de garde-fou avec `@weave.op`, vous pouvez suivre les entrées et les sorties de cette fonction, ainsi que son temps d’exécution, les informations sur les jetons si un LLM est utilisé en arrière-plan, la version du code, et bien plus encore.
...
N’hésitez pas à nous dire ce que nous devrions améliorer dans cette intégration. Si vous rencontrez un problème, veuillez ouvrir une issue [ici](https://github.com/wandb/weave/issues/new/choose).

Pour en savoir plus sur la manière de build des systèmes multiagents puissants avec CrewAI, consultez leurs [nombreux exemples](https://github.com/crewAIInc/crewAI-examples) et leur [documentation](https://docs.crewai.com/introduction).