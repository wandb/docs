...
Une fois le script exécuté, vous pouvez examiner l’activité capturée dans Weave. `weave.init()` affiche un lien vers votre projet à l’exécution. Ouvrez la vue **Agents** pour examiner :

* Une ligne dans l’onglet **Agents** pour `Research assistant`.
* Une conversation contenant trois tours de conversation.
* Chaque tour de conversation affiché sous la forme d’un span `invoke_agent`, avec des appels de modèle et des appels d’outil imbriqués.
* L’entrée complète, le modèle, la sortie, l’utilisation des jetons et les résultats de Wikipédia à chaque étape.

Pour en savoir plus sur l’affichage des données d’agent dans Weave, voir [Voir l’activité de l’agent](/fr/weave/guides/tracking/view-agent-activity).