...
Une fois le script exécuté, ouvrez la vue **Agents** de votre projet pour confirmer que Weave a bien capturé la conversation. `weave.init()` affiche un lien vers votre projet lors de son exécution. Dans la vue **Agents**, examinez les éléments suivants :

* Une ligne dans l’onglet **Agents** pour `research_assistant`.
* Une seule conversation contenant trois tours de conversation.
* Chaque tour de conversation est affiché sous la forme d’un span `invoke_agent`, avec des appels de modèle et des appels d’outil imbriqués.
* L’entrée complète, le modèle, la sortie, l’utilisation des jetons et les résultats de Wikipédia à chaque étape.

Pour en savoir plus sur l’affichage des données Agents dans Weave, voir [Voir l’activité de l’agent](/fr/weave/guides/tracking/view-agent-activity).