...
Une fois le script exécuté, ouvrez la vue **Agents** de votre projet pour vérifier que Weave a bien capturé la conversation. `weave.init()` affiche un lien vers votre projet lors de son exécution. Dans la vue **Agents**, examinez les éléments suivants :
...
Pour en savoir plus sur l’affichage des données des agents dans Weave, voir [Afficher l’activité de l’agent](/fr/weave/guides/tracking/view-agent-activity).