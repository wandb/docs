---
title: "Anthropic"
description: "Utilisez Weave pour suivre et consigner automatiquement les appels LLM effectués avec le SDK Anthropic"
---
...
<Tabs>
  ...
  <Tab title="TypeScript">
    Weave instrumente automatiquement `@anthropic-ai/sdk` au moyen de hooks du chargeur de modules. La configuration requise varie légèrement selon le système de modules utilisé. Pour plus d’informations sur les différences entre CommonJS et ESM et sur le fonctionnement des hooks du chargeur de Weave, consultez le [guide d’intégration tierce du SDK TypeScript](/fr/weave/guides/integrations/js).

    * **Projets CommonJS** : aucune configuration supplémentaire n’est nécessaire. Importez `weave` avec `require` avant `@anthropic-ai/sdk` afin que l’instrumentation automatique s’exécute en premier.
...
En incluant `weave.init()` dans le code, Weave capture automatiquement les informations de traçage et affiche des liens. Vous pouvez consulter les traces dans l’interface Weave en cliquant sur ces liens.
...
## Encapsuler avec vos propres ops {#wrap-with-your-own-ops}
...
## Outils (appel de fonctions) {#tools-function-calling}
