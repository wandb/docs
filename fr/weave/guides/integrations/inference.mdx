...
*Serverless Inference* donne accès à des modèles fondamentaux open source de premier plan via W&amp;B Weave et une API compatible avec OpenAI. Avec Serverless Inference, vous pouvez :
...
<Warning>
  Des crédits Serverless Inference sont inclus avec les plans Free, Pro et Academic pendant une durée limitée. La disponibilité peut varier pour Enterprise. Une fois les crédits consommés :

  * Les comptes Free doivent passer à un plan Pro pour continuer à utiliser Inference.
  * Les utilisateurs du plan Pro seront facturés mensuellement pour les dépassements de Serverless Inference, selon la tarification propre à chaque modèle.

  Pour en savoir plus, voir la [page de tarification](https://wandb.ai/site/pricing/) et les [coûts des modèles Serverless Inference](https://wandb.ai/site/pricing/inference).
</Warning>

Avec Weave, vous pouvez tracer, évaluer, surveiller et faire évoluer vos applications basées sur Serverless Inference.
...
Les prérequis suivants sont nécessaires pour accéder au service Serverless Inference via l’API ou l’interface W&amp;B Weave.
...
Cette section présente plusieurs exemples montrant comment utiliser Serverless Inference avec Weave :
...
Une fois l’exemple de code exécuté, vous pouvez consulter la trace dans Weave en cliquant sur le lien affiché dans le terminal (par exemple, `https://wandb.ai/<your-team>/<your-project>/r/call/01977f8f-839d-7dda-b0c2-27292ef0e04g`), ou :
