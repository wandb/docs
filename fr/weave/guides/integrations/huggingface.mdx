...
keywords: [Hugging Face Hub, InferenceClient, huggingface_hub, traçage LLM, expérimentation de modèles]
---
...
Hugging Face Hub est une plateforme de machine learning pour les créateurs et les collaborateurs, qui propose une grande collection de modèles préentraînés et de Datasets pour de nombreux types de projets.
...
2. Pour utiliser l&#39;inférence avec un modèle sur le Hugging Face Hub, configurez votre [User Access Token](https://huggingface.co/docs/hub/security-tokens). Vous pouvez définir ce token depuis votre [page Settings de Hugging Face Hub](https://huggingface.co/login?next=%2Fsettings%2Ftokens) ou par programmation. L&#39;exemple de code suivant vous invite à saisir votre `HUGGINGFACE_TOKEN` et définit ce token comme variable d&#39;environnement.
...
## Traçage de base {#basic-tracing}

Stocker les traces des applications basées sur des modèles de langage dans un emplacement centralisé est essentiel pendant le développement comme en production. Ces traces facilitent le débogage et constituent des Datasets pour améliorer votre application.

Weave capture automatiquement les traces de l&#39;`InferenceClient`. Pour commencer le suivi, initialisez Weave en appelant `weave.init()`, puis utilisez la bibliothèque comme d&#39;habitude.
...
# Initialiser le Hugging Face Inference Client
...
## Tracer une fonction {#trace-a-function}
...
Weave journalise automatiquement les modèles et suit leurs différentes versions afin que vous puissiez analyser les performances et l’historique des expériences.

<Frame>
  ![Weave journalise automatiquement les modèles et suit leurs différentes versions afin que vous puissiez analyser les performances et l’historique des expériences.](/weave/guides/integrations/imgs/huggingface/trace_model.png)
</Frame>