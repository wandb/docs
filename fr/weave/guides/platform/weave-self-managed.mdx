...
L’hébergement autogéré de W&amp;B Weave vous permet de mieux contrôler son environnement et sa configuration. Cela peut vous aider à créer un environnement plus isolé et à répondre à des exigences supplémentaires de conformité et de sécurité. Ce document vous guide dans le déploiement de tous les composants requis pour exécuter W&amp;B Weave dans un environnement autogéré à l’aide de l’Altinity ClickHouse Operator.
...
<Tip>
  Pour une architecture de référence détaillée, voir [Architecture de référence W&amp;B Autogéré](https://docs.wandb.ai/guides/hosting/self-managed/ref-arch/#models-and-weave).
</Tip>
...
Les exemples de configuration de ce guide sont fournis uniquement à titre de référence. Comme l’environnement Kubernetes de chaque organisation est unique, votre instance auto-hébergée vous obligera probablement à ajuster :
...
* **Dimensionnement des ressources** : les allocations de ressources indiquées sont des points de départ. **Consultez votre équipe d’architectes solutions W&amp;B** pour dimensionner correctement l’infrastructure en fonction du volume de traces attendu et des exigences de performances.
* **Spécificités de l’infrastructure** : mettez à jour les classes de stockage, les sélecteurs de nœuds et les autres paramètres propres à l’infrastructure pour qu’ils correspondent à votre environnement.

Les configurations de ce guide doivent être considérées comme des modèles, et non comme des solutions prêtes à l’emploi.
...
* **Nœuds Kubernetes** : cluster multinœud (au moins 3 nœuds recommandés pour la haute disponibilité)
* **Classe de stockage** : une StorageClass fonctionnelle pour les volumes persistants (par exemple, `gp3`, `standard`, `nfs-csi`)
* **Bucket S3** : bucket S3 ou compatible S3 préconfiguré avec les autorisations d&#39;accès appropriées
* **Plateforme W&amp;B** : déjà installée et en service (voir le [Guide de déploiement W&amp;B Autogéré](https://docs.wandb.ai/guides/hosting/hosting-options/self-managed/))
* **License W&amp;B** : License W&amp;B avec Weave activé, fournie par l&#39;assistance W&amp;B

<Warning>
  Ne prenez pas de décisions de dimensionnement en vous appuyant uniquement sur cette liste de prérequis. Les besoins en ressources varient considérablement selon le volume de traces et les modèles d&#39;utilisation. Voir la section détaillée [Exigences en ressources](#resource-requirements) pour obtenir des recommandations précises sur le dimensionnement du cluster.
</Warning>
...
* `kubectl` configuré pour l’accès au cluster
* `helm` v3.0+
* Identifiants AWS (si vous utilisez S3) ou accès à un stockage compatible avec S3
...
* Les pods de l’espace de noms `clickhouse` doivent pouvoir communiquer avec ceux de l’espace de noms `wandb`
* Les nœuds ClickHouse doivent communiquer entre eux sur les ports 8123, 9000, 9009 et 2181
...
<div id="step-1-deploy-altinity-clickhouse-operator">
  ### Étape 1 : Déployer Altinity ClickHouse Operator
</div>

Altinity ClickHouse Operator gère les installations de ClickHouse sur Kubernetes.
...
<div id="12-create-the-operator-configuration">
  #### 1.2 Créer la configuration de l’opérateur
</div>
...
operator:
  ...
  containerSecurityContext:
    runAsGroup: 0
    runAsNonRoot: true
    runAsUser: 10001 # À mettre à jour selon les politiques de sécurité OpenShift/Kubernetes de votre cluster
...
Les valeurs `containerSecurityContext` indiquées ici fonctionnent avec la plupart des distributions Kubernetes. Pour **OpenShift**, vous devrez peut-être ajuster `runAsUser` et `fsGroup` pour qu’ils correspondent à la plage d’UID attribuée à votre projet.
...
<div id="step-2-prepare-s3-storage">
  ### Étape 2 : Préparez le stockage S3
</div>
...
Créez un bucket S3 dans votre compte AWS ou auprès d’un fournisseur de stockage compatible S3 :
...
<div id="22-configure-s3-credentials">
  #### 2.2 Configurer les identifiants S3
</div>

Vous avez deux options pour fournir les identifiants d’accès à S3 :

<div id="option-a-using-aws-iam-roles-irsa-recommended-for-aws">
  ##### Option A : Utilisation des rôles IAM AWS (IRSA - recommandé pour AWS)
</div>

Si vos nœuds Kubernetes disposent d’un rôle IAM avec accès à S3, ClickHouse peut utiliser les métadonnées d’instance EC2 :
...
**Politique IAM requise** (attachée au rôle IAM de votre nœud) :
...
<div id="option-b-using-access-keys">
  ##### Option B : Avec des clés d&#39;accès
</div>
...
Configurez ensuite ClickHouse pour utiliser le secret (voir la configuration de ch-server.yaml ci-dessous).
...
[ClickHouse Keeper](https://clickhouse.com/docs/guides/sre/keeper/clickhouse-keeper) fournit le système de coordination pour la réplication des données et l’exécution des requêtes DDL distribuées.

<div id="31-create-the-keeper-configuration">
  #### 3.1 Créer la configuration de Keeper
</div>
...
* **StorageClass** : Mettez à jour `storageClassName: gp3` pour qu&#39;il corresponde à la StorageClass disponible sur votre cluster
...
* **Anti-affinité** : Personnalisez ou supprimez la section `affinity` basée sur la topologie de votre cluster et vos exigences en haute disponibilité
* **Ressources** : Les valeurs de CPU et de mémoire sont des exemples - consultez les architectes solutions de W&amp;B pour un dimensionnement adapté
* **Nommage** : Si vous modifiez `metadata.name` ou `configuration.clusters[0].name`, vous **devez mettre à jour** les noms d&#39;hôte de Keeper dans ch-server.yaml (Étape 4) pour qu&#39;ils correspondent
...
**Mises à jour critiques de la configuration requises** :

1. **StorageClass** : Mettez à jour `storageClassName: gp3` pour qu’il corresponde à la StorageClass de votre cluster
2. **Point de terminaison S3** : Remplacez `YOUR-BUCKET-NAME` et `YOUR-REGION` par vos valeurs réelles
3. **Taille du cache** : `<max_size>40Gi</max_size>` doit être **inférieure** à la taille du volume persistant (50Gi)
4. **Contexte de sécurité** : Ajustez `runAsUser`, `fsGroup` et les autres paramètres de sécurité afin qu’ils respectent les politiques de votre organisation
5. **Allocation des ressources** : Les valeurs de CPU/mémoire sont fournies uniquement à titre d’exemple - **consultez votre Solutions Architect W&amp;B** pour dimensionner correctement en fonction du volume de trace attendu
6. **Règles d’anti-affinité** : Personnalisez-les ou supprimez-les selon la topologie de votre cluster et vos exigences en matière de haute disponibilité
...
<Warning>
  **Ne supprimez pas `{replica}`.** Cela garantit que chaque réplique ClickHouse écrit dans son propre dossier au sein du bucket.
</Warning>


<div id="43-configure-credentials-option-b-only">
  #### 4.3 Configurer les identifiants (Option B uniquement)
</div>

Si vous utilisez **l’option B (Clés d’accès)** à l’étape 2, assurez-vous que la section `env` de `ch-server.yaml` fait référence au secret :
...
Si vous utilisez **l’option A (IRSA)**, supprimez entièrement la section `env`.
...
Les noms d’hôte des nœuds Keeper dans la section `zookeeper.nodes` suivent un schéma précis, basé sur votre déploiement de Keeper à l’étape 3 :
...
* `{installation-name}` = le `metadata.name` de ch-keeper.yaml (par exemple, `wandb`)
* `{cluster-name}` = le `configuration.clusters[0].name` de ch-keeper.yaml (par exemple, `keeper`)
...
* `{namespace}` = espace de noms Kubernetes (par exemple, `clickhouse`)
...
**Si vous personnalisez le nom de l’installation Keeper** (p. ex., `metadata.name: myweave`) :
...
**Si vous personnalisez le nom du cluster Keeper** (p. ex., `clusters[0].name: coordination`) :
...
**Pour vérifier les noms d’hôte effectifs de Keeper** :
...
<Note>
  Les noms d’hôte Keeper dans `ch-server.yaml` **doivent correspondre exactement** aux noms de service effectivement créés par le déploiement de Keeper, sinon les serveurs ClickHouse ne parviendront pas à se connecter au service de coordination.
</Note>
...
Configurez maintenant la plateforme W&amp;B pour utiliser le cluster ClickHouse pour les traces de Weave.
...
* **Port** : `8123`
* **Utilisateur** : `weave` (comme configuré dans ch-server.yaml)
* **Mot de passe** : `weave123` (comme configuré dans ch-server.yaml)
* **Base de données** : `weave` (sera créée automatiquement)
* **Nom du cluster** : `weavecluster` (comme configuré dans ch-server.yaml)

Le nom d’hôte suit ce format : `clickhouse-{installation-name}.{namespace}.svc.cluster.local`

<div id="52-update-wb-custom-resource">
  #### 5.2 Mettre à jour la Ressource personnalisée W&amp;B
</div>

Modifiez la Ressource personnalisée (CR) de votre plateforme W&amp;B afin d’ajouter la configuration Weave :
...
<Note>
  Les contextes de sécurité, les allocations de ressources et les autres configurations propres à Kubernetes présentés ci-dessus sont des exemples de référence. Personnalisez-les en fonction des exigences de votre organisation et consultez votre équipe d’architectes solutions W&amp;B pour dimensionner correctement les ressources.
</Note>
...
Le service weave-trace créera automatiquement le schéma de base de données requis lors du premier démarrage.
...
# Vérifier les tableaux dans la base de données weave
kubectl exec -n clickhouse chi-wandb-weavecluster-0-0-0 -- \
  clickhouse-client --user weave --password weave123 --query \
...
Accédez à l’URL de votre instance W&amp;B dans votre navigateur web.
...
1. Accédez à **Top Right Menu** → **tableau de bord de l’organisation**
2. Vérifiez que l’**accès à Weave** est activé

<div id="73-test-weave-functionality">
  #### 7.3 Tester le fonctionnement de Weave
</div>
...
Après avoir exécuté cette commande, vérifiez dans l’interface utilisateur W&amp;B que des traces sont bien présentes sur la page des traces de votre organisation.
...
* Réduisez les requêtes de ressources si les nœuds sont contraints
...
**Solution** : vérifiez les journaux et la configuration :
...
# Vérifier le point de terminaison S3 dans la configuration de stockage
...
Si la connexion échoue, les noms d’hôte de Keeper dans ch-server.yaml ne correspondent probablement pas à votre déploiement Keeper. Voir &quot;Comprendre le nommage de Keeper&quot; à l’étape 4 pour connaître le schéma de nommage.


<div id="weave-trace-issues">
  ### Problèmes de traces Weave
</div>
...
1. Vérifiez que la License inclut Weave :
...
<Warning>
  Les allocations de ressources ci-dessous sont **uniquement des exemples de départ**. Les besoins réels varient considérablement en fonction des éléments suivants :
  ...
  **Consultez toujours votre équipe d’architectes solutions W&amp;B** pour déterminer le dimensionnement adapté à votre cas d’usage. Des ressources sous-dimensionnées peuvent entraîner des problèmes de performances, tandis qu’un surdimensionnement augmente inutilement les coûts d’infrastructure.
</Warning>
...
| Composant         | Réplicas   | Requête CPU / Limite | Requête mémoire / Limite | Stockage    |
| ----------------- | ---------- | -------------------- | ------------------------ | ----------- |
...
Pour les charges de travail de production avec un volume élevé de traces :
...
*Convient pour : environnements de production à fort volume*

Pour les déploiements à très haut volume, contactez votre équipe d’architectes solutions W&amp;B pour obtenir des recommandations de dimensionnement personnalisées basées sur votre volume spécifique de traces et vos exigences de performances.

<div id="advanced-configuration">
  ## Configuration avancée
</div>

Cette section présente les options de personnalisation des déploiements Weave autogérés, notamment la mise à l’échelle de la capacité de ClickHouse, verticalement ou horizontalement, la mise à jour des versions de ClickHouse en modifiant les tags d’image dans les configurations keeper et server, ainsi que la surveillance de l’état de santé de ClickHouse.

Nous vous recommandons de consulter l’équipe d’architectes solutions W&amp;B avant d’apporter des modifications avancées à votre instance, afin de vous assurer qu’elles répondent à vos exigences en matière de performances et de fiabilité.
...
1. **Mise à l’échelle verticale** : augmente les ressources par pod (approche la plus simple)
   ...
   **Recommandation** : surveillez l’utilisation réelle des ressources et adaptez la mise à l’échelle en conséquence. Pour les déploiements à très grand volume, contactez l’équipe d’architectes solutions W&amp;B.

2. **Mise à l’échelle horizontale** : ajoute davantage de réplicas (nécessite une planification rigoureuse)
   * Augmenter le nombre de réplicas nécessite un rééquilibrage des données
   * Consultez la documentation de ClickHouse pour la gestion des shards
   * **Contactez un W&amp;B Solutions Architect** avant de mettre en œuvre une mise à l’échelle horizontale en production

<div id="using-different-clickhouse-versions">
  ### Utiliser une version différente de ClickHouse
</div>

Pour utiliser une version différente de ClickHouse, mettez à jour le tag de l’image dans les fichiers `ch-keeper.yaml` et `ch-server.yaml` :
...
Accédez aux tables système de ClickHouse pour la surveillance :
...
Les données ClickHouse sont stockées dans S3, ce qui offre des capacités de sauvegarde intégrées grâce à la gestion des versions S3 et aux fonctionnalités de réplication des buckets. Pour les stratégies de sauvegarde propres à votre déploiement, consultez votre équipe d’architectes solutions W&amp;B et référez-vous à la [documentation de sauvegarde de ClickHouse](https://clickhouse.com/docs/en/operations/backup).

<div id="security-considerations">
  ## Considérations de sécurité
</div>

1. **Identifiants** : stockez les mots de passe ClickHouse dans des secrets Kubernetes, et non en clair
2. **Politiques réseau** : envisagez de mettre en place des NetworkPolicies afin de restreindre l’accès à ClickHouse
3. **RBAC** : assurez-vous que les comptes de service disposent uniquement des autorisations minimales requises
...
Mettez à jour la version de l’image dans `ch-server.yaml`, puis appliquez :
...
* [Documentation de W&amp;B Weave](https://docs.wandb.ai/weave)
...
* **Assistance W&amp;B** : `support@wandb.com`
* **Architectes solutions** : Pour les déploiements à très grand volume, le dimensionnement personnalisé et la planification du déploiement
* **Incluez dans les requêtes d’assistance** :
  ...
  * Informations sur le cluster et le volume de traces
...
A : Oui, mais ce n’est pas recommandé en production. Mettez à jour `replicasCount: 1` dans ch-server.yaml et définissez `clickhouse.replicated: false` dans wandb-cr.yaml.

**Q : Puis-je utiliser une autre base de données à la place de ClickHouse ?**

A : Non, Weave Trace nécessite ClickHouse pour ses capacités de stockage colonnaire hautes performances.

**Q : De combien de stockage S3 aurai-je besoin ?**

A : Les besoins en stockage S3 dépendent de votre volume de traces, de votre durée de rétention et de la compression des données. Surveillez votre utilisation réelle après le déploiement et ajustez en conséquence. Le format colonnaire de ClickHouse offre une excellente compression pour les données de trace.

**Q : Dois-je configurer le nom de `database` dans ClickHouse ?**

A : Non, la base de données `weave` sera créée automatiquement par le service weave-trace lors du démarrage initial.

**Q : Que faire si le nom de mon cluster n’est pas `weavecluster` ?**

A : Vous devez définir la variable d’environnement `WF_CLICKHOUSE_REPLICATED_CLUSTER` pour qu’elle corresponde au nom de votre cluster, sinon les migrations de base de données échoueront.

**Q : Dois-je utiliser exactement les contextes de sécurité indiqués dans les exemples ?**

A : Non. Les contextes de sécurité (runAsUser, fsGroup, etc.) fournis dans ce guide sont des exemples de référence. Vous devez les ajuster pour respecter les politiques de sécurité de votre organisation, en particulier pour les clusters OpenShift, qui imposent des exigences spécifiques concernant les plages d’UID/GID.

**Q : Comment puis-je savoir si mon cluster ClickHouse est correctement dimensionné ?**

A : Contactez l’équipe d’architectes solutions W&amp;B en indiquant votre volume de traces attendu et vos profils d’utilisation. Elle vous fournira des recommandations de dimensionnement précises. Surveillez l’utilisation des ressources de votre déploiement et ajustez si nécessaire.

**Q : Puis-je personnaliser les conventions de nommage utilisées dans les exemples ?**

A : Oui, mais vous devez rester cohérent sur l’ensemble des composants :
...
Voir la section « Comprendre le nommage de Keeper » à l’étape 4 pour plus de détails sur la convention de nommage et sur la façon de vérifier vos noms réels.

**Q : Que se passe-t-il si mon cluster utilise des exigences d’anti-affinité différentes ?**

A : Les règles d’anti-affinité indiquées sont des recommandations pour la haute disponibilité. Ajustez-les ou supprimez-les en fonction de la taille de votre cluster, de sa topologie et de vos exigences de disponibilité. Pour les petits clusters ou les environnements de développement, vous n’aurez peut-être pas besoin de règles d’anti-affinité.