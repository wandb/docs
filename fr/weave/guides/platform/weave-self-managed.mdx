...
## Notes importantes sur la configuration {#important-setup-notes}
...
## Déployez votre instance Weave autogérée {#deploy-your-self-managed-weave-instance}
...
### Déployer l’Altinity ClickHouse Operator {#deploy-the-altinity-clickhouse-operator}
...
#### Ajouter le dépôt Helm d’Altinity {#add-the-altinity-helm-repository}
...
#### Créez la configuration de l’opérateur {#create-the-operator-configuration}
...
#### Installez l’opérateur {#install-the-operator}
...
#### Vérifier l’installation de l’opérateur {#verify-the-operator-installation}
...
### Préparer le stockage S3 {#prepare-s3-storage}
...
#### Créer un bucket S3 {#create-an-s3-bucket}
...
#### Configurer les identifiants d’accès à S3 {#configure-s3-credentials}
...
##### Option A : utiliser des rôles IAM AWS (IRSA, recommandé pour AWS) {#option-a-use-aws-iam-roles-irsa-recommended-for-aws}
...
##### Option B : Utiliser des clés d’accès {#option-b-use-access-keys}
...
#### Créez la configuration de Keeper {#create-the-keeper-configuration}
...
#### Déployer les ressources ClickHouse Keeper {#deploy-clickhouse-keeper-resources}

```bash
kubectl apply -f ch-keeper.yaml
```

#### Vérifier le déploiement de Keeper {#verify-the-keeper-deployment}
...
### Déployer le cluster ClickHouse {#deploy-the-clickhouse-cluster}
...
#### Créer la configuration du serveur ClickHouse {#create-the-clickhouse-server-configuration}
...
#### Mettre à jour la configuration S3 {#update-the-s3-configuration}
...
#### Configurez les identifiants d’accès (option B uniquement) {#configure-credentials-option-b-only}
...
#### Noms d’hôte de Keeper {#keeper-naming}
...
#### Déployer les ressources du cluster ClickHouse {#deploy-the-clickhouse-cluster-resources}

```bash
kubectl apply -f ch-server.yaml
```

#### Vérifier le déploiement de ClickHouse {#verify-the-clickhouse-deployment}
...
### Activer Weave dans la plateforme W&amp;B {#enable-weave-in-the-wb-platform}
...
#### Rassemblez les informations de connexion à ClickHouse {#gather-clickhouse-connection-information}
...
#### Mettre à jour la ressource personnalisée W&amp;B {#update-the-wb-custom-resource}
...
#### Appliquez la configuration mise à jour {#apply-the-updated-configuration}

```bash
kubectl apply -f wandb-cr.yaml
```

#### Vérifier le déploiement de Weave Trace {#verify-the-weave-trace-deployment}
...
### Initialiser la base de données Weave {#initialize-the-weave-database}
...
#### Surveiller la migration de la base de données {#monitor-the-database-migration}
...
#### Vérifier la création de la base de données {#verify-database-creation}
...
### Vérifier que Weave est activé {#verify-that-weave-is-enabled}
...
#### Accéder à la console W&amp;B {#access-the-wb-console}

Accédez à l’URL de votre instance W&amp;B depuis un navigateur web.

#### Vérifier le statut de la licence Weave {#check-the-weave-license-status}
...
#### Tester le fonctionnement de Weave {#test-weave-functionality}
...
### Problèmes liés à ClickHouse Keeper {#clickhouse-keeper-issues}
...
### Problèmes du serveur ClickHouse {#clickhouse-server-issues}
...
### Problèmes Weave Trace {#weave-trace-issues}
...
## Exigences de ressources {#resource-requirements}
...
### Configuration minimale de production {#minimum-production-setup}
...
### Configuration de production recommandée {#recommended-production-setup}
...
### Mettre à l’échelle ClickHouse {#scale-clickhouse}
...
### Utiliser une autre version de ClickHouse {#use-a-different-clickhouse-version}
...
<Warning>
  Mettez à niveau ClickHouse Keeper vers une version compatible lorsque vous mettez à niveau le serveur ClickHouse. Avant de modifier les versions de ClickHouse pour un déploiement W&amp;B Autogéré, consultez [la compatibilité de ClickHouse pour les mises à niveau](/fr/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) ainsi que la page [versions du serveur W&amp;B prises en charge](/fr/release-notes/server-releases).
</Warning>

### Surveiller ClickHouse {#monitor-clickhouse}
...
<Note>
  Weave nécessite une version de ClickHouse prise en charge. Voir [la compatibilité de ClickHouse pour les mises à niveau](/fr/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) et [les versions du serveur W&amp;B prises en charge](/fr/release-notes/server-releases) avant de mettre à niveau ClickHouse ou le serveur W&amp;B. Mettez à niveau le serveur ClickHouse et ClickHouse Keeper ensemble.
</Note>

### Mise à niveau de l’opérateur ClickHouse {#upgrade-the-clickhouse-operator}
...
### Mise à niveau du serveur ClickHouse {#upgrade-clickhouse-server}
...
### Mettre à niveau Weave Trace {#upgrade-weave-trace}
