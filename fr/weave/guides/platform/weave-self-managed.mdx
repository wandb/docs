...
spec:
    ...
    podTemplates:
      - name: clickhouse
        ...
        spec:
          ...
          # À ajuster ou supprimer selon la taille de votre cluster et vos exigences
          affinity:
            podAntiAffinity:
              requiredDuringSchedulingIgnoredDuringExecution:
                - labelSelector:
  ...
  configuration:
    ...
    clusters:
      - name: weavecluster # Nom du cluster - peut être personnalisé mais doit correspondre à wandb-cr.yaml
        layout:
          shardsCount: 1
...
```bash
# Lister les services Keeper pour voir les noms effectifs
kubectl get svc -n clickhouse | grep keeper

# Lister les pods Keeper pour confirmer le format de nommage
kubectl get pods -n clickhouse -l app=clickhouse-keeper
```
...
os.environ["WANDB_BASE_URL"] = "https://your-wandb-host"  # Remplacez par votre URL W&B
...
Après avoir exécuté cela, vérifiez dans l’interface utilisateur W&amp;B les traces sur la page Traces de votre organisation.
...
Si la connexion échoue, les noms d’hôte Keeper dans ch-server.yaml ne correspondent probablement pas à votre déploiement Keeper. Voir « Comprendre le nommage de Keeper » à l’étape 4 pour la convention de nommage.
...
```bash
# Modifiez ch-server.yaml, modifiez le tag d’image
kubectl apply -f ch-server.yaml
