...
Les attributs dans Weave vous permettent d’attacher des métadonnées personnalisées à vos traces et évaluations. Ces métadonnées peuvent inclure des informations telles que des noms d’environnement, des versions de modèle, des ID d’expérience, des ID d’utilisateur ou d’autres informations contextuelles pour vous aider à organiser, filtrer et analyser vos données Weave.

Weave fournit deux façons d’ajouter des attributs :
...
Vous pouvez voir tous les attributs enregistrés pendant les traces et l’évaluation dans l’interface utilisateur. Vous pouvez ensuite les utiliser pour filtrer et regrouper les données.

<Tip>
  `call.attributes` ne peut pas être modifié une fois que l’appel commence. Utilisez ce
  gestionnaire de contexte pour définir toute métadonnée avant d’invoquer l’op.
</Tip>
...
Le gestionnaire de contexte `weave.attributes()` vous permet d’ajouter des métadonnées à des opérations tracées spécifiques. Vous pouvez ainsi associer des informations contextuelles à certains appels de fonction ou à des runs d’évaluation spécifiques.
...
Vous pouvez utiliser ensemble des attributs globaux et des attributs par appel. Les attributs par appel qui ont la même clé redéfinissent les attributs globaux :
...
Renvoie l&#39;ensemble des attributs actuellement enregistrés pour votre appel. Cela peut vous aider à déboguer vos appels ou à exploiter le contexte pour une logique conditionnelle.

<Tabs>
  <Tab title="Python">
    L&#39;exemple suivant applique le décorateur Weave à la fonction `process_data` pour l&#39;enregistrer, définit les attributs à enregistrer, puis les renvoie à l&#39;exécution.
