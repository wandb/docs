...
keywords: ["playground LLM", "prompt engineering", "comparaison de modèles", "environnements d’exécution personnalisés", "fournisseurs personnalisés", "modèles enregistrés"]
---
...
### Ajuster les paramètres du LLM {#adjust-llm-parameters}
...
<Frame>
  ![Le Weave Playground montrant la liste déroulante « Select a model » ouverte et le panneau Chat settings ouvert à droite.](/weave/guides/tools/imgs/playground_settings.png)
</Frame>
...
### Zone de définition du prompt {#prompt-definition-area}
...
### Panneau Messages {#messages-panel}
...
### champ de saisie des messages (champ de saisie) {#message-composer-input-field}
...
<Frame>
  ![Weave Playground montrant la zone de définition du prompt en haut, l’historique des messages au centre et un champ de saisie des messages en bas pour envoyer de nouveaux messages.](/weave/guides/tools/imgs/playground_chat_input.png)
</Frame>

## Voir l’historique des messages {#view-message-history}
...
Outre les fournisseurs intégrés, le Playground prend en charge les environnements d’exécution personnalisés : des points de terminaison d’API compatibles avec OpenAI que vous enregistrez dans votre projet. Les environnements d’exécution personnalisés étaient auparavant appelés *fournisseurs personnalisés*. Utilisez un environnement d’exécution personnalisé pour tester des modèles, par exemple un prototype d’agent hébergé par votre équipe, un modèle local ou une version antérieure du modèle d’un fournisseur pris en charge.

Pour enregistrer un environnement d’exécution personnalisé dans l’interface utilisateur ou avec le SDK Python, voir [Environnements d’exécution personnalisés](/fr/weave/guides/tools/custom-runtimes). Après avoir enregistré un environnement d’exécution personnalisé, sélectionnez ses modèles dans la liste déroulante **Select a model** et testez-les à l’aide des fonctionnalités standard du Playground.

### Utiliser un modèle local comme environnement d’exécution personnalisé {#use-a-local-model-as-a-custom-runtime}
...
4. Une fois ngrok lancé, une URL publique s’affiche, par exemple `https://xxxx-xxxx.ngrok-free.app`. Utilisez cette URL comme **Base URL** lorsque vous [enregistrez un environnement d’exécution personnalisé](/fr/weave/guides/tools/custom-runtimes#register-a-custom-runtime-in-the-ui).
...
### Enregistrer un modèle {#save-a-model}
...
### Utiliser un modèle enregistré {#use-a-saved-model}
...
### Mettre à jour un modèle enregistré {#update-a-saved-model}
