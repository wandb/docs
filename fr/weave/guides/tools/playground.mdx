...
<Tip>
  Pendant une durée limitée, W&amp;B Inference est inclus dans votre offre gratuite. W&amp;B Inference vous donne accès à des modèles fondamentaux open source de premier plan via l’API et le Playground de Weave.

  * [Documentation développeur](../integrations/inference)
  * [Page produit](https://wandb.ai/site/inference)
</Tip>

L’évaluation des prompts et des réponses de LLM peut s’avérer complexe. Le Playground de Weave est conçu pour simplifier l’itération sur les prompts et les réponses de LLM, afin de faciliter les expérimentations avec différents modèles et prompts. Grâce à des fonctionnalités comme la modification des prompts, la relance des messages et la comparaison de modèles, Playground vous aide à tester et à améliorer rapidement vos applications LLM. Playground prend actuellement en charge des modèles de fournisseurs tels qu’OpenAI, Anthropic et Google, ainsi que des [fournisseurs personnalisés](#add-a-custom-provider).
...
* **Comparaison de modèles :** Comparez la manière dont différents modèles répondent aux prompts.
...
Commencez à utiliser le Playground pour optimiser vos interactions avec les LLM et simplifier votre processus d’ingénierie des prompts ainsi que le développement d’applications LLM.
...
La **zone de définition du prompt** vous permet de définir les instructions qui façonnent le comportement du modèle tout au long d&#39;une interaction.

Utilisez cette zone pour fournir un contexte qui s&#39;applique de façon cohérente avant tout échange de messages. Cela inclut la définition du rôle, les consignes de ton et de style, les contraintes de comportement et les exigences de sortie. Les modifications apportées ici affectent toutes les interactions suivantes, sauf modification ultérieure.
...
* **Texte du prompt** : saisissez le texte d&#39;instruction qui définit la manière dont le modèle doit répondre.
...
Elle permet de sélectionner le rôle du message et de soumettre des messages à l&#39;exécution. La plupart des interactions sont rédigées sous forme de messages **Utilisateur**. Vous pouvez ajouter des messages **Système** ou **Assistant** lorsque vous testez des modifications d&#39;instructions.
...
Playground vous permet de comparer des LLM. Pour effectuer une comparaison, cliquez sur le bouton **Add Chat (<Icon icon="plus" iconType="solid" />)** dans la barre d’outils de droite du Playground. Une deuxième conversation s’ouvre à côté de la conversation d’origine.
Dans cette deuxième conversation, vous disposez des mêmes fonctionnalités que dans la conversation d’origine, comme le choix du modèle, le réglage des paramètres et l’ajout de fonctions.
...
* **Provider name** : Un nom pour le fournisseur, tel que `openai` ou `ollama`.
* **API key** : La clé API du fournisseur, par exemple une clé API OpenAI.
* **Base URL** : L’endpoint de base du fournisseur, tel que `https://api.openai.com/v1/` ou une URL ngrok comme `https://e452-2600-1700-45f0-3e10-2d3f-796b-d6f2-8ba7.ngrok-free.app`.
* **Headers** : (Facultatif) Une ou plusieurs paires clé-valeur d’en-têtes HTTP personnalisés.
* **Models** : Un ou plusieurs modèles pour le fournisseur, tels que `deepseek-r1` ou `qwq`.
...
<Warning>
  En raison des restrictions CORS, vous ne pouvez pas appeler directement des URL localhost ou 127.0.0.1 depuis le Playground. Si vous exécutez un serveur de modèles local (comme Ollama), utilisez un service de tunneling comme ngrok pour l’exposer de manière sécurisée. Pour plus de détails, voir [Use ngrok with Ollama](#use-a-local-model-as-a-custom-provider).
</Warning>
...
* Vous pouvez aussi, dans le menu latéral, sélectionner **Projet**, puis l’onglet **Fournisseurs d’IA**.
