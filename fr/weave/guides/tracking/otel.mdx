...
2. Entité : Vous pouvez uniquement journaliser des traces dans le projet relevant d’une entité à laquelle vous avez accès. Vous pouvez trouver le nom de votre entité en consultant votre tableau de bord W&amp;B à l’adresse [https://wandb.ai/home], puis en vérifiant le champ **Teams** dans la barre latérale gauche.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    * OpenAI doit être importé avant d’enregistrer l’instrumentation (les modules ESM l’exigent).
    ...
    * Nécessite d’appeler explicitement `provider.shutdown()` avec un délai avant l’arrêt pour garantir que les spans sont bien exportés, car `BatchSpanProcessor` effectue le vidage de façon asynchrone.
...
<Tabs>
  <Tab title="Python">
    Collez le code suivant dans un fichier Python, par exemple `openllmetry_example.py`. Notez qu’il s’agit du même code que ci-dessus, à ceci près que `OpenAIInstrumentor` est importé depuis `opentelemetry.instrumentation.openai` au lieu de `openinference.instrumentation.openai` :
...
* Configurer un fichier de configuration Docker qui déploie un serveur local (collecteur) à l’écoute des traces OTLP, les regroupe par lots et les transmet vers Weave.
  ...
  resource:
    attributes:
      - key: wandb.entity # Champ des attributs de ressource
        ...
        action: insert # Injecter uniquement si absent
      - key: wandb.project
...
* Exporte les traces vers le point de terminaison OTLP de Weave à l’aide de l’en-tête `wandb-api-key`, en lisant l’URL du point de terminaison à partir de `WANDB_OTLP_ENDPOINT` et la clé API à partir de `WANDB_API_KEY`.
* Définit `wandb.entity` et `wandb.project` comme attributs de ressource à l’aide du processeur `resource`, en lisant les valeurs à partir de `DEFAULT_WANDB_ENTITY` et `DEFAULT_WANDB_PROJECT`. L’action `insert` n’injecte ces attributs que si le code de votre application ne les définit pas déjà.
* Active la `sending_queue` intégrée de l’exportateur avec traitement par lots afin de réduire la charge réseau.
...
Une fois le collecteur démarré, configurez votre application pour y exporter des traces en définissant la variable d’environnement `OTEL_EXPORTER_OTLP_ENDPOINT`. Le SDK OTel lit automatiquement cette variable, vous n’avez donc pas besoin de fournir le point de terminaison à l’exportateur.
...
Pour envoyer des traces vers d’autres backends, ajoutez des exportateurs supplémentaires et incluez-les dans la liste `service.pipelines.traces.exporters`. Par exemple, vous pouvez exporter vers Weave et Jaeger depuis une même instance du collecteur.

<div id="organize-otel-traces-into-threads">
  ## Organiser les traces OTel en threads
</div>

Ajoutez des attributs de span spécifiques pour organiser vos traces OpenTelemetry en [threads Weave](/fr/weave/guides/tracking/threads), puis utilisez l&#39;interface utilisateur des threads de Weave pour analyser des opérations liées, comme des conversations à plusieurs tours ou des sessions utilisateur.
    ...
    <Tab title="Python">
      ...
      def example_2_multiple_turns():
          ...
          with tracer.start_as_current_span("process_message_turn2") as turn2_span:
              ...
              turn2_span.set_attribute("input.value", "Pouvez-vous expliquer les différences entre Python et JavaScript ?")
    ...
    <Tab title="TypeScript">
      ...
      function example_2_multiple_turns() {
        ...
        tracer.startActiveSpan("process_message_turn2", (turn2Span) => {
          ...
          turn2Span.setAttribute("input.value", "Pouvez-vous expliquer les différences entre Python et JavaScript ?");
...
<Accordion title="Tracer des opérations profondément imbriquées et marquer uniquement le span le plus externe comme tour">
  <Tabs>
    <Tab title="Python">
      ```python lines
      def example_3_complex_nested_structure():
          ...
          # Tour avec plusieurs niveaux d'imbrication
...
| :-------------------------------- | :---------------------------- | :-------------------------------------------------------- | :-------------------------------- | :--------------------------------------------- |
| `ai.prompt`                       | `inputs`                      | Texte du prompt utilisateur ou messages.                  | Chaîne de caractères, liste, dict | `"Écrivez un court haïku sur l’été."`          |
| `gen_ai.prompt`                   | `entrée`                      | Prompt du modèle d’IA ou liste de messages.               | Liste, dict, chaîne               | `[{"role":"user","content":"abc"}]`            |
| `input.value`                     | `inputs`                      | Valeur d’entrée pour l’appel du modèle.                   | Chaîne, liste, dict               | `{"text":"Raconte une blague"}`                |
| `mlflow.spanInputs`               | `entrée`                      | Données d’entrée du span.                                 | Chaîne, liste, dict               | `["texte du prompt"]`                          |
| `traceloop.entity.input`          | `entrée`                      | Données d’entrée de l’entité.                             | Chaîne, liste, dict               | `"Traduisez ceci en français"`                 |
| `gcp.vertex.agent.tool_call_args` | `entrée`                      | Arguments d’appel de l’outil.                             | Dict                              | `{"args":{"query":"météo à SF"}}`              |
...
| `gen_ai.completion`               | `outputs`                     | Résultat de la complétion de l’IA.                        | Chaîne, liste, dict               | `"Texte de complétion"`                        |
...
| `gen_ai.system`                   | `attributes.system`           | Prompt système ou instructions.                           | Chaîne                            | `"Vous êtes un assistant serviable."`          |
| `llm.system`                      | `attributes.system`           | Prompt système ou instructions.                           | Chaîne                            | `"Vous êtes un assistant utile."`              |
| `weave.span.kind`                 | `attributes.kind`             | Type ou catégorie du span.                                | Chaîne                            | `"llm"`                                        |
...
| `wandb.display_name`              | `display_name`                | Display name personnalisé pour l’interface utilisateur.   | String                            | `"Message utilisateur"`                        |
...
| `langfuse.startTime`              | `start_time` (redéfinit)      | Remplace l’horodatage de début du span.                   | Horodatage (ISO8601/unix ns)      | `"2024-01-01T12:00:00Z"`                       |
| `langfuse.endTime`                | `end_time` (redéfinit)        | Remplace l’horodatage de fin du span.                     | Horodatage (ISO8601/unix ns)      | `"2024-01-01T12:00:01Z"`                       |
