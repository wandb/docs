...
description: "Importer des données de trace compatibles OpenTelemetry via un point de terminaison dédié"
keywords: ["OTLP", "OpenTelemetry Collector", "OpenInference", "OpenLLMetry", "threads"]
---

<Warning>
  Nous vous recommandons d’utiliser le [nouveau flux de travail natif OTel de Weave](/fr/weave/guides/tracking/trace-agents-otel) pour envoyer des spans OTel vers la nouvelle vue Agents de Weave. Utilisez ce guide uniquement si vous souhaitez ingérer des traces OTLP brutes dans la vue Traces standard de Weave.
</Warning>
...
Cette page présente les détails du point de terminaison, l’authentification, des exemples complets en Python et TypeScript, comment transférer des traces via un OpenTelemetry Collector, comment organiser les traces en threads Weave, ainsi que la correspondance des attributs que Weave applique aux spans entrants.
...
## Détails du point de terminaison {#endpoint-details}
...
## Authentification et acheminement {#authentication-and-routing}
...
### Instrumentation OpenInference {#openinference-instrumentation}
...
### Instrumentation OpenLLMetry {#openllmetry-instrumentation}
...
### Sans instrumentation {#without-instrumentation}
...
## Utiliser un collecteur OpenTelemetry {#use-an-opentelemetry-collector}
...
### Configurer un collecteur {#set-up-a-collector}
...
## Organiser les traces OTel en threads {#organize-otel-traces-into-threads}
...
## Correspondance des attributs {#attribute-mappings}
...
* **Logfire / Pydantic AI**: instrumentation de Pydantic AI par Logfire (`gen_ai.input.messages`, `gen_ai.output.messages`, `pydantic_ai.all_messages`, `final_result`).
* **Langfuse**: attributs de tracing de Langfuse (`langfuse.startTime`, `langfuse.endTime`).

### Référence des attributs {#attribute-reference}
...
| `gen_ai.input.messages`           | `inputs`                      | Messages d’entrée émis par Logfire.                       | Liste, dict, chaîne               | `[{"role":"user","parts":[...]}]`              |
| `pydantic_ai.all_messages`        | `inputs`                      | Tous les messages d’un run d’agent Pydantic AI.           | Liste                             | `[{"role":"user","content":"abc"}]`            |
...
| `gen_ai.output.messages`          | `outputs`                     | Messages de sortie émis par Logfire.                      | Liste, dict, chaîne               | `[{"role":"assistant","parts":[...]}]`         |
| `mlflow.spanOutputs`              | `outputs`                     | Données de sortie du span.                                | String, liste, dict               | `["answer"]`                                   |
| `final_result`                    | `outputs`                     | Sortie finale d’un run d’agent Pydantic AI.               | Chaîne, dict                      | `"Texte de réponse"`                           |
...
| `gen_ai.system_instructions`      | `attributes.system`           | Prompt système ou consignes.                              | String                            | `"You are a helpful assistant."`               |
