...
keywords: ["sur plusieurs tours de conversation", "sessions", "OTel", "invoke_agent", "conversation_id"]
---
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    Appelez `weave.init()` avant tout appel à `startSession()`, `startTurn()`, `startLLM()`, `startTool()` ou `startSubagent()`. Toutes les fonctions de traçage des agents ne font silencieusement rien lorsque le traçage est désactivé ou que l’appel à init est absent. Vous pouvez donc laisser l’instrumentation dans le code de production et la contrôler via la configuration.
  </Tab>
</Tabs>
...
| -------------------- | ------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Agent                | *(aucune classe)*   | *(aucun span, regroupé par l’attribut `agent_name`)*                                      | Une application agentique dans l’onglet Agents qui contient une ou plusieurs sessions. |                                                                                                                                |
| Session              | `Session`           | *(aucun span, les tours de conversation sont regroupés par l’attribut `conversation_id`)* | Une conversation ou une exécution qui contient un ou plusieurs tours de conversation.  | [Python](/fr/weave/reference/typescript-sdk/classes/session) <br /> [TypeScript](/fr/weave/reference/typescript-sdk/classes/session) |
...
Les sections suivantes décrivent chaque fonction de traçage de premier niveau ainsi que les arguments qu’elle accepte. Utilisez-les pour instrumenter les couches session, tour de conversation, appel LLM et appel d’outil du modèle de données décrit dans la section précédente.
...
`start_session()` (Python) ou `startSession()` (TypeScript) définit un attribut `conversation_id` sur tous les spans enfants afin de regrouper les tours de conversation dans l’onglet Agents. Si vous fournissez un `session_id` / `sessionId`, il doit rester stable tout au long de la conversation. Réutilisez le même ID pour ajouter de nouveaux tours de conversation à une session existante. Si vous ne l’indiquez pas, le SDK génère automatiquement un UUID.
...
<Tabs>
  <Tab title="Python">
    ...
    session = weave.start_session(
        agent_name="my-agent",    # Facultatif : identifie l’agent dans l’interface utilisateur. Si vous l’omettez, la session n’est pas regroupée sous un agent nommé.
  ...
  <Tab title="TypeScript">
    ...
    const session = weave.startSession({
      agentName: 'my-agent',  // Facultatif : identifie l’agent dans l’interface utilisateur. Si vous l’omettez, la session n’est pas regroupée sous un agent nommé.
...
* **Comme fonction autonome** (`weave.start_turn(...)` / `weave.startTurn(...)`), comme illustré dans les exemples ci-dessous. Elle détermine la session active à partir du contexte et hérite de son ID de conversation. Si aucune session n’est active, le tour de conversation est créé sans `conversation_id` et n’est pas regroupé avec d’autres tours de conversation.
* **Comme méthode d’instance** sur une session dont vous détenez une référence (`session.start_turn(...)` / `session.startTurn(...)`). Cela est utile lorsque vous avez un objet session explicite dans la portée, par exemple à l’intérieur d’un bloc de gestionnaire de contexte. L’exemple « Gestionnaire de contexte ou schéma try-finally » ci-dessous utilise cette forme. Voir le tableau du modèle de données ci-dessus pour des liens directs vers les pages de référence `Session`, `Turn`, `LLM`, `Tool` et `SubAgent` dans les deux SDK.
