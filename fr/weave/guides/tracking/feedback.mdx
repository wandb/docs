...
<div id="query-a-projects-feedback">
  ### Consulter les retours d&#39;un projet
</div>

Vous pouvez interroger les retours de votre projet Weave à l&#39;aide du SDK. Le SDK prend en charge les opérations de requête de retour suivantes :

* `client.get_feedback()` : Renvoie tous les retours d&#39;un projet.
* `client.get_feedback("<feedback_uuid>")` : Renvoie, sous forme de collection, un objet de retour spécifique désigné par `<feedback_uuid>`.
* `client.get_feedback(reaction="<reaction_type>")` : Renvoie tous les objets de retour pour un type de réaction donné.

Vous pouvez également obtenir des informations supplémentaires pour chaque objet de retour dans `client.get_feedback()` :

* `id` : l&#39;ID de l&#39;objet de retour.
* `created_at` : la date et l&#39;heure de création de l&#39;objet de retour.
* `feedback_type` : le type de retour (`reaction`, `note`, `custom`).
* `payload` : le payload du retour.
...
<div id="add-feedback-to-a-call">
  ### Ajouter du retour à un Appel
</div>

Vous pouvez ajouter du retour à un Appel à l’aide de l’UUID de l’Appel. Pour utiliser l’UUID afin d’obtenir un Appel précis, [récupérez-le pendant ou après l’exécution de l’Appel](#retrieve-the-call-uuid). Le SDK prend en charge les opérations suivantes pour ajouter du retour à un Appel :
...
* `call.feedback.add("<label>", <object>)`: ajoute un `<object>` de retour personnalisé spécifié par `<label>`.

<Warning>Le nombre maximal de caractères dans une note de retour est de 1024. Si une note dépasse cette limite, elle ne sera pas créée.</Warning>
...
<Frame>![Historique de l’évaluateur d’annotation humaine](/weave/guides/tracking/imgs/human-annotation-scorer-history.png)</Frame>
