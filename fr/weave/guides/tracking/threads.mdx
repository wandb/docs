...
<Frame>
  ![Vue en liste des Threads](/weave/guides/tracking/imgs/threads-list.png)
</Frame>
...
Cet exemple montre comment définir manuellement un agent conversationnel à l’aide des décorateurs `@weave.op` et de la gestion du contexte `weave.thread()`. Chaque appel à `process_user_message` crée un nouveau tour dans le thread. Vous pouvez utiliser ce modèle lorsque vous créez votre propre boucle d’agent et que vous souhaitez garder un contrôle complet sur la gestion du contexte et de l’imbrication.
...
<div id="manual-agent-with-unbalanced-call-depth">
  ### Agent manuel avec profondeur d&#39;appel déséquilibrée
</div>

Cet exemple montre que les tours peuvent être définis à différentes profondeurs dans la pile d&#39;appels, selon la façon dont le contexte de thread est appliqué. L&#39;exemple utilise deux fournisseurs (OpenAI et Anthropic), chacun avec une profondeur d&#39;appel différente avant d&#39;atteindre la limite du tour.

Tous les tours partagent le même `thread_id`, mais la limite du tour apparaît à différents niveaux de la pile selon la logique du fournisseur. Cela est utile lorsque les appels doivent être tracés différemment selon les backends, tout en étant regroupés dans le même thread.
...
class MultiProviderAgent:
    """Agent principal qui achemine les requêtes entre les fournisseurs avec différentes profondeurs de chaîne d'appels"""
    ...
    def handle_conversation_turn(self, user_input: str, thread_id: str) -> str:
        """
        Achemine vers différents fournisseurs avec des profondeurs de chaîne d'appels déséquilibrées.
...
Il est parfois nécessaire de reprendre une session déjà démarrée et de continuer à ajouter des Appels au même thread. Dans d’autres cas, il se peut que vous ne puissiez pas reprendre une session existante et que vous deviez à la place démarrer un nouveau thread.

Lors de l’implémentation de la reprise facultative d’un thread, ne laissez **jamais** le paramètre `thread_id` à `None`, car cela désactivera entièrement le regroupement des threads. Fournissez toujours un ID de thread valide. Si vous devez créer un nouveau thread, générez un ID unique à l’aide d’une fonction comme `generate_id()`.

Lorsqu’aucun `thread_id` n’est spécifié, l’implémentation interne de Weave génère automatiquement un UUID v7 aléatoire. Vous pouvez reproduire ce comportement dans votre propre fonction `generate_id()` ou utiliser n’importe quelle valeur de chaîne unique de votre choix.
...
# - Pas de passage des ID de thread par paramètre
...
Cet exemple récupère les 50 Threads dont la mise à jour est la plus récente. Remplacez `my-project` par l’ID réel de votre projet.
...
for thread in response.threads:
    print(f"Thread {thread.thread_id}: {thread.turn_count} tours, dernier actif {thread.last_updated}")
```

<div id="query-threads-by-activity-level">
  ### Interroger les threads selon leur niveau d’activité
</div>

Cet exemple récupère les 20 threads les plus actifs, triés par nombre de tours.
