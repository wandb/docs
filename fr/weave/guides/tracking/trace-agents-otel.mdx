...
L’observabilité des agents W&amp;B Weave repose sur [OpenTelemetry (OTel)](https://opentelemetry.io/docs/concepts/) et fournit un point de terminaison OTLP dédié qui ingère les spans dans la vue **Agents**. Vous pouvez ainsi rediriger rapidement les spans de votre application instrumentée avec OTel vers Weave en remplaçant la configuration actuelle du point de terminaison de votre exportateur OTel par la configuration du point de terminaison de Weave. Vous n’avez pas besoin d’installer le SDK Weave pour cela.
...
## Acheminement vers un projet {#project-routing}
...
## Configurez uniquement à l’aide de variables d’environnement {#configure-with-environment-variables-only}
...
## Exemple : émettre des spans d’agent sans le SDK Weave {#example-emit-agent-spans-without-the-weave-sdk}
