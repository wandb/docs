...
## Créez vos propres évaluateurs {#create-your-own-scorers}
...
### évaluateurs basés sur des fonctions {#function-based-scorers}
...
### évaluateurs basés sur des classes {#class-based-scorers}
...
## Fonctionnement des évaluateurs {#how-scorers-work}
...
### Arguments nommés des évaluateurs {#scorer-keyword-arguments}
...
### Mapper les noms de colonnes avec `column_map` {#mapping-column-names-with-column_map}
...
### Mapper les noms de colonnes avec `columnMapping` {#mapping-column-names-with-columnmapping}
...
### Accéder aux variables de vos ops dans les prompts de scoring {#access-variables-from-your-ops-in-scoring-prompts}
...
### Résumé final du scorer {#final-summarization-of-the-scorer}
...
### Appliquer des évaluateurs à un appel {#apply-scorers-to-a-call}
...
Pour évaluer les appels de production avec un évaluateur qui s’exécute sur votre propre infrastructure, associez un [évaluateur distant](/fr/weave/guides/evaluation/remote-scorers) à un moniteur.
...
### Utiliser `preprocess_model_input` {#use-preprocess_model_input}
...
## Analyse des scores {#score-analysis}
...
### Analyser les scores d’un appel unique {#analyze-a-single-calls-scores}

#### API pour un appel unique {#single-call-api}
...
#### UI d’un appel unique {#single-call-ui}
...
### Analyser les scores de plusieurs appels {#analyze-multiple-calls-scores}

#### API des appels multiples {#multiple-calls-api}
...
#### Interface des appels multiples {#multiple-calls-ui}
...
### Analyser tous les appels évalués par un scorer spécifique {#analyze-all-calls-scored-by-a-specific-scorer}

#### API de tous les appels par scorer {#all-calls-by-scorer-api}
...
#### Tous les appels d’un Scorer dans l’UI {#all-calls-by-scorer-ui}
