...
<Tabs>
  <Tab title="Python">
    Les exemples de ce guide utilisent des annotations. Nous vous recommandons d’utiliser des annotations, car c’est le moyen le plus simple de commencer à journaliser vos médias. Pour des configurations plus avancées, voir la [section Content API](#using-the-contents-api).

    Pour journaliser des médias dans Weave, ajoutez des annotations de type comme `Annotated[bytes, Content]` ou `Annotated[str, Content]` comme types d’entrée ou de retour dans vos ops. Si vous annotez des arguments de chemin d’accès avec `Annotated[str, Content]`, Weave ouvre, détecte et affiche automatiquement le média dans votre trace.
  </Tab>

  <Tab title="TypeScript">
    Le SDK TypeScript fournit des fonctions dédiées à la journalisation des médias :
...
Les sections suivantes fournissent des exemples pratiques de journalisation de chaque type de média.
...
<Tabs>
  <Tab title="Python">
    Ajoutez des images au journal en annotant des fonctions avec les types `Annotated[bytes, Content]` ou des chemins de fichier avec `Annotated[str, Content]`.

    L&#39;exemple suivant dessine une image de base, puis l&#39;ajoute au journal dans Weave à l&#39;aide de l&#39;annotation `Content` :
    ...
    Weave journalise l&#39;image et renvoie un lien vers la trace où vous pouvez afficher l&#39;image.
    ...
    ### Exemple avancé : redimensionner les grandes images avant leur journalisation

    Il peut être utile de redimensionner les images avant leur journalisation afin de réduire le coût de rendu de l’interface utilisateur et l’impact sur le stockage. Vous pouvez utiliser `postprocess_output` dans votre `@weave.op` pour redimensionner une image.
    ...
    Weave consigne l’image redimensionnée dans le journal et renvoie un lien vers la trace, où vous pouvez voir l’image.
  </Tab>

  <Tab title="TypeScript">
    L’exemple suivant ajoute une image au journal à l’aide de la fonction `weaveImage` :
    ...
    Weave journalise l’image et renvoie un lien vers la trace dans laquelle vous pouvez voir l’image.

    ### Exemple avancé : Journaliser une image générée via l’API OpenAI DALL-E

    L’exemple suivant journalise une image générée via l’API OpenAI DALL-E à l’aide de la fonction `weaveImage` :
...
Les exemples suivants génèrent des documents et les journalisent dans l&#39;interface de Weave.
...
<Tabs>
  <Tab title="Python">
    ...
    Weave prend automatiquement en charge les types de fichiers `pdf`, `csv`, `md`, `text`, `json`, `xml`. Vous pouvez également journaliser des documents à l&#39;aide de chemins de fichier avec `Annotated[str, Content]`.

    L&#39;exemple suivant montre comment stocker des copies des fichiers PDF et CSV d&#39;entrée, puis enregistrer le contenu des fichiers renvoyé par la fonction :
    ...
    Cet exemple montre comment journaliser des documents dans un système de Retrieval-Augmented Generation (RAG) :
...
<Tabs>
  <Tab title="Python">
    ...
    ### Exemple avancé : générer et journaliser un audio créé par l’IA

    Cet exemple génère et journalise un audio créé par l’IA à l’aide de l’annotation `Content` :
    ...
    Cet audio est enregistré dans Weave et s’affiche automatiquement dans l’interface utilisateur, avec un lecteur audio. Dans ce lecteur, vous pouvez consulter et télécharger la forme d’onde audio brute.

    <Frame>
      ![Enregistrement audio dans Weave](/images/weave/audio.png)
    </Frame>

    L’exemple suivant montre comment enregistrer de l’audio à l’aide d’une réponse en flux continu de l’API OpenAI :
  ...
  <Tab title="TypeScript">
    L’exemple suivant charge un fichier audio existant et l’enregistre dans le journal de Weave :
...
<Tip>
  Essayez notre cookbook sur la [journalisation audio](/fr/weave/cookbooks/audio_with_weave). Le cookbook comprend également un exemple avancé d’assistant basé sur une API audio en temps réel et intégré à Weave.
</Tip>
...
Les exemples suivants montrent comment générer du HTML et le journaliser dans l’interface de Weave.
...
Ce HTML est journalisé dans Weave et affiché automatiquement dans l’interface. Cliquez sur la cellule `file_name.html` du tableau pour l’ouvrir en plein écran. Vous pouvez également télécharger le fichier `.html` brut.
...
* Ouvrir un fichier avec une application par défaut (comme un lecteur PDF)
* Exporter le modèle au format JSON pour le téléverser vers votre propre stockage blob (comme S3)
...
<div id="custom-mimetypes-with-type-annotations">
  #### Types MIME personnalisés avec des annotations de type
</div>
...
<div id="custom-mimetypes-with-direct-initialization">
  #### Initialisation directe de types MIME personnalisés
</div>
