...
<Tabs>
  <Tab title="Python">
    Les exemples de ce guide utilisent des annotations. Nous recommandons d’utiliser des annotations, car c’est la manière la plus simple de commencer à enregistrer vos médias. Pour des configurations plus avancées, voir la [section Content API](#using-the-contents-api).

    Pour enregistrer des médias dans Weave, ajoutez des annotations de type comme `Annotated[bytes, Content]` ou `Annotated[str, Content]` comme types d’entrée ou de retour dans vos ops. Si vous annotez des arguments de chemin avec `Annotated[str, Content]`, Weave ouvre, détecte et affiche automatiquement le média dans votre trace.
  </Tab>

  <Tab title="TypeScript">
    Le SDK TypeScript fournit des fonctions dédiées pour enregistrer des médias :
...
<Frame>
  ![Capture d’écran de la vue de trace du chat citrouille](/weave/guides/core-types/imgs/cat-pumpkin-trace.png)
</Frame>
...
<Tabs>
  <Tab title="Python">
    ...
    L&#39;exemple suivant crée une image simple, puis l&#39;ajoute au journal dans Weave à l&#39;aide de l&#39;annotation `Content` :
  ...
  <Tab title="TypeScript">
    L&#39;exemple suivant enregistre une image dans le journal à l&#39;aide de la fonction `weaveImage` :
    ...
    ### Exemple avancé : journaliser une image générée via l’API DALL-E d’OpenAI

    L’exemple suivant journalise une image générée via l’API DALL-E d’OpenAI à l’aide de la fonction `weaveImage` :
...
<Frame>
  ![Journalisation de vidéos dans Weave](/images/weave/video.png)
</Frame>
...
<Tip>
  Consultez notre cookbook sur la [journalisation audio](/fr/weave/cookbooks/audio_with_weave). Il inclut également un exemple avancé d’assistant basé sur une API audio en temps réel et intégré à Weave.
</Tip>
