...
## Démarrage rapide de Dataset {#dataset-quickstart}
...
## Créer un dataset à partir d&#39;autres objets {#create-a-dataset-from-other-objects}
...
## Créer, modifier et supprimer un dataset dans l’interface utilisateur {#create-edit-and-delete-a-dataset-in-the-ui}
...
### Créer un nouveau dataset {#create-a-new-dataset}
...
### Modifier un dataset {#edit-a-dataset}
...
### Supprimer un dataset {#delete-a-dataset}
...
### Ajouter une nouvelle trace d’agent à un dataset {#add-a-new-agent-trace-to-a-dataset}

Pour ajouter des tours de conversation d’agent et des Appels d’outil à un `Dataset`, voir [Ajouter des messages d’agent à un dataset](/fr/weave/guides/tracking/view-agent-activity#add-agent-messages-to-a-dataset).

### Ajouter une nouvelle trace à un dataset {#add-a-new-trace-to-a-dataset}

Pour ajouter à un `Dataset` des traces générées à partir d’opérations et d’Appels (à l’aide du décorateur `@weave.op`) :
