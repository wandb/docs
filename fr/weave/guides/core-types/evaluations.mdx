...
keywords: ["évaluation", "évaluateurs", "EvaluationLogger", "preprocess_model_input", "trials"]
---
...
Avec votre objet `Evaluation`, le dataset, les évaluateurs et le modèle définis, vous êtes prêt à exécuter l’évaluation et à capturer les résultats dans Weave. Pour lancer une évaluation, appelez `.evaluate()` sur l’objet `Evaluation`.
...
<Tabs>
  <Tab title="Python">
    <Warning>
      Weave applique la fonction `preprocess_model_input` uniquement aux entrées avant leur transmission à la fonction de prédiction du modèle. Les fonctions de scorer reçoivent toujours l’exemple original du jeu de données, sans prétraitement.
    </Warning>
