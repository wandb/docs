...
La génération augmentée par récupération (RAG) est une approche courante pour créer des applications d’IA générative ayant accès à des bases de connaissances personnalisées.
...
* Créer une application RAG avec une étape de récupération qui trouve des documents pertinents
...
<div id="build-a-knowledge-base">
  ## build une base de connaissances
</div>
...
Ensuite, appliquez le décorateur `weave.op()` à la fonction de récupération `get_most_relevant_document` et créez une classe `Model`. Appelez `weave.init('<team-name>/rag-quickstart')` pour commencer à suivre toutes les entrées et sorties de vos fonctions afin de pouvoir les inspecter plus tard. Si vous ne spécifiez pas de nom d&#39;équipe, les données de sortie sont enregistrées dans votre [équipe ou entité W&amp;B par défaut](/fr/platform/app/settings-page/user-settings#default-team).
...
Dans certaines applications, vous pouvez vouloir créer des classes d’évaluation personnalisées. Par exemple, une classe `LLMJudge` standardisée peut être créée avec des paramètres spécifiques (p. ex. modèle de chat, prompt), une logique de score spécifique pour chaque ligne et un calcul spécifique du score agrégé. Weave définit une liste de classes `Scorer` prêtes à l’emploi et facilite également la création d’un `Scorer` personnalisé. L’exemple suivant montre comment créer une `class CorrectnessLLMJudge(Scorer)` personnalisée.
  ...
  <Tab title="Python">
    ...
    class CorrectnessLLMJudge(Scorer):
        ...
        async def score(self, output: Optional[dict], query: str, answer: str) -> Any:
            """Évalue la correction des prédictions en comparant pred, query et target.
            Arguments:
...
* Encapsulez les appels au LLM et les fonctions de l&#39;étape de récupération avec `weave.op()`
* (facultatif) Créez une sous-classe `Model` avec une fonction `predict` et les détails de l&#39;application
* Collectez des exemples pour l&#39;évaluation
* Créez des fonctions de score qui attribuent un score à un exemple
* Utilisez la classe `Evaluation` pour exécuter des évaluations sur vos exemples

**REMARQUE :** Il arrive que l&#39;exécution asynchrone des évaluations déclenche une limite de débit sur les modèles d&#39;OpenAI, Anthropic, etc. Pour éviter cela, vous pouvez définir une variable d&#39;environnement afin de limiter le nombre de workers parallèles, par exemple `WEAVE_PARALLELISM=3`.
...
<Tabs>
  <Tab title="Python">
    ...
    # Définir un objet Evaluation et lui transmettre des exemples de questions ainsi que des fonctions de scoring
    evaluation = weave.Evaluation(dataset=questions, scorers=[context_precision_score])
...
Ce tutoriel a montré comment intégrer l’observabilité à différentes étapes de vos applications, comme à l’étape de récupération dans cet exemple. Vous avez également appris à créer des fonctions de score plus complexes, comme un juge LLM, pour l’évaluation automatique des réponses de l’application.
