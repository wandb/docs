---
title: "weave"
description: "Consultez la documentation de référence complète du SDK Python de W&B Weave, avec tous les modules et toutes les classes."
---
...
* <b>`CallsIter`</b>:  Un itérateur sur des objets Appel représentant des exécutions d’évaluation.
...
* <b>`dest`</b>: Chemin de destination vers lequel le fichier sera copié (string or pathlib.Path)  Le chemin de destination peut être un fichier ou un répertoire.
  **Arguments :**
...
Les fonctions décorées avec @weave.op sont déjà des instances d’Op. Cette fonction ne devrait donc avoir aucun effet à l’exécution. Vous pouvez toutefois l’utiliser pour satisfaire les vérificateurs de type si vous devez accéder aux attributs d’OpDef de manière sûre au niveau des types.
...
**Retourne :**
L’objet Appel de l’Op en cours d’exécution, ou None si le tracking n’a pas été initialisé ou si cette méthode est appelée en dehors d’un Op.
...
Enregistrez et créez une version d’un objet Python.

Weave crée une nouvelle version de l’objet si un objet portant ce nom existe déjà et que son empreinte de contenu ne correspond pas à celle de sa dernière version.
...
Si vous avez l’ID de l’Appel, par exemple via l’UI, vous pouvez utiliser la méthode `get_call` sur le `WeaveClient` renvoyé par `weave.init` pour récupérer l’objet Appel correspondant.
...
* <b>`NoCurrentCallError`</b>:  Si tracking n’a pas été initialisé ou si cette méthode est appelée en dehors d’un Op.
