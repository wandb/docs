...
Méthode personnalisée non définie dans l&#39;interface formelle TraceServerInterface pour exposer le processeur de retour sous-jacent. Elle doit être formalisée dans une interface côté client.
...
Créez un tableau en spécifiant des empreintes de lignes au lieu des lignes elles-mêmes.
...
Semblable à `calls/batch_upsert`, nous pouvons ajuster dynamiquement la taille de la charge utile en raison de la propriété selon laquelle les mises à jour de tableau peuvent être décomposées en une série de mises à jour.
