...
Méthode personnalisée non définie dans l’interface formelle TraceServerInterface pour exposer le processeur d’appel sous-jacent. Devrait être formalisée dans une interface côté client.
...
Méthode personnalisée non définie dans l’interface formelle TraceServerInterface pour exposer le processeur de feedback sous-jacent. Elle doit être formalisée dans une interface côté client.
...
Créez un tableau en spécifiant les empreintes des lignes plutôt que les lignes elles-mêmes.
...
Comme pour `calls/batch_upsert`, nous pouvons ajuster dynamiquement la taille de la charge utile, puisque les mises à jour de tableau peuvent être décomposées en une série de mises à jour.
