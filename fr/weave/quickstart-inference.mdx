...
description: "Découvrez les bases de Weave à l’aide de W&B Inference pour tracer les appels au modèle, comparer les résultats et effectuer des évaluations."
---
...
* build une application LLM de base avec Tracing automatique
* comparer plusieurs modèles
* Évaluez la performance du modèle sur un jeu de données
...
Pour commencer, faites un copier-coller de l&#39;exemple de code suivant. Cet exemple de code utilise Llama 3.1-8B de W&amp;B Inference.
...
<div id="build-a-text-summarization-application">
  ## Build une application de synthèse de texte
</div>

Ensuite, essayez d’exécuter ce code : il s’agit d’une application de synthèse de texte de base qui montre comment Weave génère des traces pour des opérations imbriquées :
...
<Tabs>
  <Tab title="Python">
    ...
    def summarize_text(text: str) -> dict:
        """Pipeline principal de synthèse."""
...
Évaluez la performance d’un modèle sur une tâche de questions-réponses à l’aide de `EvaluationLogger` intégré de Weave. Cela permet un suivi structuré de l’évaluation avec agrégation automatique, capture de l’utilisation des jetons et fonctionnalités de comparaison avancées dans l’interface utilisateur.
...
<Tabs>
  <Tab title="Python">
    ...
    def evaluate_model(model: InferenceModel, dataset: list[dict]):
        ...
        # Convertir le nom du modèle en un format valide (remplacer les caractères non alphanumériques par des traits de soulignement)
...
* Accédez à l’onglet **Evals** pour voir les résultats agrégés des évaluations
...
Pour consulter la liste complète des modèles disponibles, voir la [section Modèles disponibles](https://docs.wandb.ai/inference/models/) dans la documentation de W&amp;B Inference.

<div id="next-steps">
  ## Prochaines étapes
</div>

* **Utiliser le Playground** : [Essayez des modèles de manière interactive](/fr/weave/guides/tools/playground#access-the-playground) dans le Playground Weave
