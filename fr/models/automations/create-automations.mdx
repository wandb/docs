...
Cette page explique comment créer et gérer des [automatisations](/fr/models/automations) W&amp;B, qui se déclenchent en réponse à des événements spécifiques dans W&amp;B. Lorsqu’un événement spécifique se produit au sein d’une organisation, d’une équipe, d’un projet, d’un registre ou d’une collection, l’automatisation envoie une notification à un canal Slack, déclenche un webhook ou envoie un prompt à ARIA. Utilisez cet aperçu pour comprendre les prérequis et le flux de travail général, puis suivez les instructions détaillées dans [Créer une automatisation Slack](/fr/models/automations/create-automations/slack), [Créer une automatisation webhook](/fr/models/automations/create-automations/webhook) ou [Créer une automatisation ARIA](/fr/models/automations/create-automations/aria).
...
Vous pouvez créer une automatisation depuis le hub global **Automatisations** dans la navigation de gauche, depuis l’onglet **Automatisations** du projet ou du registre, ou directement depuis un panneau de graphique linéaire dans le Workspace. Lorsque vous créez une automatisation depuis le hub, vous choisissez sa portée (organisation, équipe, registre, collection de registre ou projet). Voir [hub Automatisations](/fr/models/automations#automations-hub).

### Créer depuis l’onglet Automatisations {#create-from-the-automations-tab}
...
### Créer une automatisation sur une métrique de run à partir d’un panneau du Workspace {#create-a-run-metric-automation-from-a-workspace-panel}
...
* [Créer une automatisation ARIA](/fr/models/automations/create-automations/aria)
...
{/* Rétablir une fois la régression du SDK Python `create_automation` corrigée (en interne, WB-34263) :
