...
## Code de sortie et signaux {#exit-status-and-signals}
...
## Runs inactifs et délais d’expiration côté serveur {#stale-runs-and-server-side-timeouts}
...
## Signaux interceptables et préemption {#catchable-signals-and-preemption}
...
## `SIGKILL` (impossible à intercepter) {#sigkill-uncatchable}
...
## Transmission des signaux de l&#39;agent au processus enfant {#signal-forwarding-from-agent-to-child}
...
## Clusters préemptibles comme SLURM {#preemptible-clusters-like-slurm}
...
## `wandb sweep --cancel` {#wandb-sweep-cancel}
...
## Signaux envoyés à l’agent ou au run {#signals-to-the-agent-versus-signals-to-the-run}
...
## Référence : `mark_preempting()` et l’état final du run {#reference-mark_preempting-and-final-run-state}
