...
## Reprendre un run en utilisant obligatoirement le même ID du run {#resume-a-run-that-must-use-the-same-run-id}
...
## Reprendre un run sans écraser le run existant {#resume-a-run-without-overriding-the-existing-run}
...
## Activer la reprise automatique des runs {#enable-runs-to-automatically-resume}
...
## Reprendre les runs Sweeps préemptibles {#resume-preemptible-sweeps-runs}
...
### Gérer un signal de préemption {#handle-a-preemption-signal}
...
1. Appelez [`mark_preempting()`](/fr/models/ref/python/experiments/run#mark_preempting) lorsqu’un run est actif.
