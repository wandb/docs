...
<Note>
  Le fork d’un run est en préversion pour le Cloud mutualisé et le Cloud dédié. Contactez l’assistance pour obtenir des conseils concernant le Cloud dédié. Il n’est pas actuellement disponible pour les déploiements autogérés.

  Les fonctionnalités en préversion sont disponibles pour une évaluation en production, mais les fonctionnalités, les API et le comportement sont en cours de développement actif et peuvent changer avant la disponibilité générale.
</Note>
...
Lorsque vous forkez à partir d’un run, W&amp;B crée un nouveau run en utilisant l’[ID unique](/fr/models/runs/run-identifiers#unique-run-identifiers) du run source et une étape spécifiée. Les métriques récapitulatives du run source sont copiées dans le run forké. Le run forké partage l’ensemble de l’historique et des fichiers du run source jusqu’à l’étape spécifiée.
...
## Forker à partir d’un run précédemment enregistré {#fork-from-a-previously-logged-run}
...
## Forker à partir d’un run dans le même script {#fork-from-a-run-in-the-same-script}
