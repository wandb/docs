...
## Un seul run par processus {#single-run-per-process}
...
## Plusieurs runs dans un même processus {#multiple-runs-in-one-process}
...
### Options de `reinit` {#reinit-options}
...
### Configurer `reinit` {#configure-reinit}
...
### Exemple : Processus concurrents {#example-concurrent-processes}
