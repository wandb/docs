...
description: Supprimez des runs d’un projet W&B à l’aide de W&B App ou de l’API publique, et découvrez comment les données des runs supprimés sont retirées du stockage.
---

## Supprimer des runs {#delete-runs}
...
<Tabs>
  <Tab title="W&B App" value="ui">
    ...
    5. Dans le volet latéral qui s’affiche, choisissez **Supprimer**.
  ...
  <Tab title="Python" value="python">
    Utilisez l’API publique de W&amp;B pour supprimer des runs par programmation. Les opérations de suppression peuvent porter sur un seul run ou sur plusieurs runs à la fois.

    Utilisez [`wandb.Api.Run.delete()`](/fr/models/ref/python/public-api/run#method-run-delete) pour supprimer un seul run. Indiquez le chemin du run au format `entity/project/runID`.
    ...
    Utilisez `wandb.Api.runs()` pour récupérer plusieurs runs d’un projet, puis supprimez chaque run. L’exemple suivant supprime tous les runs d’un projet :
    ...
    Vous pouvez utiliser des filtres pour ne supprimer que les runs correspondant à des critères précis, comme leur date de création, leurs tags ou d’autres métadonnées. L’exemple suivant prévisualise puis supprime les runs créés avant un horodatage UTC donné :
    ...
    1. Obtenez les références de fichier pertinentes avec [`wandb.Api.Run.files()`](/fr/models/ref/python/public-api/run#method-run-files).
...
## Organigramme de suppression d’un run {#run-deletion-flowchart}
...
## Quand les données de run supprimées sont effacées du stockage {#when-deleted-run-data-is-removed-from-storage}
