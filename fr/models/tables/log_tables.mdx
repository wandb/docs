...
Le paramètre `log_mode` de [`wandb.Table`](/fr/models/ref/python/data-types/table) détermine comment un tableau est journalisé et mis à jour pendant vos expériences de ML. Le paramètre `log_mode` accepte l&#39;un des trois arguments suivants : `IMMUTABLE`, `MUTABLE` et `INCREMENTAL`. Chaque mode a des implications différentes sur la manière dont un tableau est journalisé, sur les modifications qui peuvent lui être apportées et sur son rendu dans l&#39;application W&amp;B.
...
| ------------- | ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `IMMUTABLE`   | Une fois qu&#39;un tableau est journalisé dans W&amp;B, vous ne pouvez plus le modifier.                  | - Stockage de données tabulaires générées à la fin d&#39;un run pour une analyse ultérieure                                                                                               | - Surcharge minimale lorsqu&#39;il est journalisé à la fin d&#39;un run<br />- Toutes les lignes sont affichées dans l&#39;interface utilisateur |
...
Si vous souhaitez uniquement ajouter de nouveaux lots de lignes (sans colonnes) de façon incrémentielle, comme dans une boucle d’entraînement, envisagez plutôt d’utiliser le [mode `INCREMENTAL`](#INCREMENTAL-mode).

<div id="incremental-mode">
  ### mode `INCREMENTAL`
</div>

En mode incrémentiel, vous journalisez des lots de lignes dans un tableau pendant l&#39;expérience de machine learning. Ce mode est idéal pour la surveillance des jobs de longue durée ou lorsque vous travaillez avec de grands tableaux dont la journalisation à chaque mise à jour pendant le run serait inefficace. Dans l&#39;interface utilisateur, le tableau est mis à jour avec les nouvelles lignes à mesure qu&#39;elles sont journalisées, ce qui vous permet de consulter les données les plus récentes sans avoir à attendre la fin complète du run. Vous pouvez également parcourir les incréments pour afficher le tableau à différents moments.

<Note>
  Les espaces de travail de run dans l&#39;application W&amp;B sont limités à 100 incréments. Si vous journalisez plus de 100 incréments, seuls les 100 plus récents s&#39;affichent dans l&#39;espace de travail du run.
</Note>
...
La journalisation incrémentielle est généralement plus efficace d’un point de vue computationnel que la journalisation d’un nouveau tableau à chaque fois (`log_mode=MUTABLE`). Cependant, l’application W&amp;B peut ne pas afficher toutes les lignes du tableau si vous journalisez un grand nombre d’incréments. Si votre objectif est de mettre à jour et de consulter les données de votre tableau pendant que votre run est en cours, tout en conservant toutes les données disponibles pour l’analyse, envisagez d’utiliser deux tableaux : l’un avec le mode de journalisation `INCREMENTAL` et l’autre avec le mode de journalisation `IMMUTABLE`.
...
<Note>
  Les incréments sont journalisés dans un nouveau tableau si vous désactivez les résumés pour une clé utilisée par le tableau incrémentiel à l’aide de `wandb.Run.define_metric("<table_key>", summary="none")` ou de `wandb.Run.define_metric("*", summary="none")`.
</Note>

<div id="training-with-incremental-batch-training">
  ### Entraînement avec un entraînement incrémentiel par lots
</div>
...
with wandb.init(project="batch-training-incremental") as run:
    ...
    for step in range(get_num_batches()):
        ...
        # Journaliser l'état actuel du tableau de façon incrémentielle
        run.log({"training_table": table}, step=step)
```
