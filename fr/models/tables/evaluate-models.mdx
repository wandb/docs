...
## Évaluer des modèles avec Weave {#evaluate-models-with-weave}
...
### Fonctionnalités clés pour l’évaluation des modèles {#key-features-for-model-evaluation}
...
### Premiers pas : évaluer un modèle à partir du registre W&amp;B {#getting-started-evaluate-a-model-from-wb-registry}

Téléchargez un modèle depuis le registre de W&amp;B Models et évaluez-le avec Weave :
...
# Définir un ChatModel qui se charge depuis le registre W&B
class ChatModel(weave.Model):
    model_name: str
    
    def model_post_init(self, __context):
        # Télécharger le modèle depuis le registre de W&B Models
...
### Intégrer les évaluations Weave à W&amp;B Models {#integrate-weave-evaluations-with-wb-models}
...
### Fonctionnalités avancées de Weave {#advanced-weave-features}

#### Évaluateurs et juges personnalisés {#custom-scorers-and-judges}
...
#### Évaluations par lot {#batch-evaluations}
...
## Évaluer des modèles avec W&amp;B Tables {#evaluate-models-with-tables}
...
### Exemple de base : Enregistrer les résultats d’évaluation {#basic-example-log-evaluation-results}
...
### Flux de travail avancés avec les tableaux {#advanced-table-workflows}
...
<Frame>
  ![Comparaison côte à côte des prédictions du modèle sur différentes époques d’entraînement](/images/data_vis/table_comparison.png)
</Frame>

#### Suivre les prédictions au fil du temps {#track-predictions-over-time}
...
for epoch in range(num_epochs):
    ...
    # Évaluez et journalisez les prédictions pour cette époque
...
### Analyse interactive dans l’interface W&amp;B {#interactive-analysis-in-the-wb-ui}
...
### Exemple : Analyse d’erreurs avec des tableaux enrichis {#example-error-analysis-with-enriched-tables}
