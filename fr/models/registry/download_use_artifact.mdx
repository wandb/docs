...
Une fois que vous disposez du registre, de la collection et de l&#39;alias ou de l&#39;index de la version de l&#39;artifact, vous pouvez construire le chemin vers l&#39;artifact lié à l&#39;aide du modèle de chaîne ci-dessous :
...
<details>
  ...
  # Initialiser un run dans l’équipe et le projet spécifiés
  with wandb.init(entity=TEAM_ENTITY, project = PROJECT_NAME) as run:

      artifact_name = f"wandb-registry-{REGISTRY}/{COLLECTION}:{ALIAS}"

      # Accéder à un artifact et le marquer comme entrée de votre run pour assurer le suivi de la traçabilité
      fetched_artifact = run.use_artifact(artifact_or_name = artifact_name)  
