...
Les *grilles de panneaux* sont un type spécifique de bloc qui contient des panneaux et des *run sets*. Les run sets constituent une collection de runs enregistrés dans un projet sur W&amp;B. Les panneaux sont des visualisations des données de run set.
...
Les graphiques vous permettent de visualiser les données de run dans un rapport. Chaque grille de panneaux comporte un ensemble d’ensembles de runs et un ensemble de panneaux. Les ensembles de runs au bas de la section contrôlent les données affichées dans les panneaux de la grille. Créez une nouvelle grille de panneaux si vous souhaitez ajouter des graphiques qui récupèrent des données à partir d’un autre ensemble de runs.
...
<Tabs>
  <Tab title="W&B App">
    ...
    2. Dans l&#39;ensemble de runs, recherchez le run à renommer. Survolez le nom du rapport et cliquez sur le menu **action (<Icon icon="ellipsis-vertical" iconType="solid" />)**. Sélectionnez l&#39;une des options suivantes, puis soumettez le formulaire.
  ...
  <Tab title="Report and Workspace API">
    Ajoutez des ensembles de runs depuis des projets avec les classes `wr.Runset()` et `wr.PanelGrid`. Pour ajouter un runset, suivez ces étapes :
...
Un rapport met automatiquement à jour les ensembles de runs pour afficher les données les plus récentes du projet. Figez un ensemble de runs lorsque vous souhaitez en conserver l’état dans un rapport à un instant donné, afin que les runs ultérieurs ne modifient pas ce que le rapport affiche.
...
Filtrez des ensembles de runs par code et ajoutez-les à un rapport à l’aide de l’[API Workspace and Reports](/fr/models/ref/wandb_workspaces/reports). Le filtrage permet de restreindre un ensemble de runs aux exécutions spécifiques que vous souhaitez afficher, en fonction des valeurs de configuration, des métriques, des tags ou des propriétés d’exécution.
...
L’extrait de code suivant montre comment filtrer un ensemble de runs en fonction de ses tags. Les tags sont des valeurs que vous ajoutez à un run (par programmation ou avec la W&amp;B App).
...
<Tabs>
  <Tab title="Interface de l’application">
    ...
    Sélectionnez le nom du langage de programmation à droite du bloc de code pour ouvrir un menu déroulant. Dans ce menu, sélectionnez la syntaxe de votre langage de programmation. Vous pouvez choisir parmi JavaScript, Python, CSS, JSON, HTML, Markdown et YAML.
  </Tab>
...
Repliez les en-têtes dans un rapport pour masquer le contenu d’un bloc de texte. Lorsque le rapport se charge, seul le contenu des en-têtes dépliés s’affiche. Replier les en-têtes dans les rapports aide à organiser votre contenu et à éviter de charger trop de données. Le GIF suivant illustre la procédure.
...
Pour comparer plus de variables qu’un graphique 2D ne peut en afficher, vous pouvez utiliser un dégradé de couleurs comme dimension supplémentaire. Utiliser un dégradé de couleurs pour représenter l’une des variables peut faciliter l’interprétation des tendances.

1. Choisissez une variable à représenter à l’aide d’un dégradé de couleurs, comme les scores de pénalité ou les taux d’apprentissage. Cela permet de mieux comprendre comment la pénalité (couleur) interagit avec la récompense/les effets secondaires (axe des y) au fil de l’entraînement (axe des x).
