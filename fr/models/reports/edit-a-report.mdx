...
Les *grilles de panneaux* sont un type spécifique de bloc qui contient des panneaux et des *ensembles de runs*. Les ensembles de runs constituent une collection de runs enregistrés dans un projet sur W&amp;B. Les panneaux sont des visualisations des données des ensembles de runs.

<Tip>
  Pour faire réviser par [ARIA](/fr/aria/overview) un rapport dont vous êtes propriétaire, ouvrez le rapport, cliquez sur **Edit with ARIA**, puis décrivez les modifications souhaitées. ARIA enregistre ses modifications directement dans le rapport existant.
</Tip>
...
## Ajouter des graphiques {#add-plots}
...
## Ajouter des ensembles de runs {#add-run-sets}
...
## Figer un ensemble de runs {#freeze-a-run-set}
...
## Regrouper un ensemble de runs par programmation {#group-a-run-set-programmatically}
...
### Regrouper les runs par valeurs de configuration {#group-runs-by-config-values}
...
### Regrouper les runs par métadonnées de run {#group-runs-by-run-metadata}
...
### Regrouper les runs par métriques de synthèse {#group-runs-by-summary-metrics}
...
## Filtrer un ensemble de runs par code {#filter-a-run-set-programmatically}
...
### Filtres de configuration {#config-filters}
...
### Filtres de métriques {#metric-filters}
...
### Filtres de SummaryMetric {#summarymetric-filters}
...
## Ajouter des blocs de code {#add-code-blocks}
...
## Ajouter du Markdown {#add-markdown}
...
## Ajouter des éléments HTML {#add-html-elements}
...
## Intégrer des liens de médias enrichis {#embed-rich-media-links}
...
## Dupliquer des grilles de panneaux {#duplicate-panel-grids}
...
## Supprimer des grilles de panneaux {#delete-panel-grids}
...
## Replier les en-têtes pour organiser les rapports {#collapse-headers-to-organize-reports}
...
## Visualiser les relations sur plusieurs dimensions {#visualize-relationships-across-multiple-dimensions}
