...
* Explorer vos données sans perte de fidélité des données : W&amp;B recalcule la taille des buckets du axe X lorsque vous zoomez sur des points de données spécifiques. Cela vous permet d&#39;explorer vos données sans perdre en précision. W&amp;B met en cache les agrégations précédemment calculées afin de réduire les temps de chargement, ce qui est utile lorsque vous parcourez de grands jeux de données.

Il existe deux façons d’afficher des graphiques en courbes en fidélité totale :

* **Bande** (par défaut) : affiche les valeurs minimale et maximale dans chaque bucket sous forme de bande ombrée, tandis que la ligne montre la tendance générale.
* **Ligne** : relie directement les points, ce qui facilite l’identification des pics et des creux.

### Activer la fidélité totale {#turn-on-full-fidelity}
...
<Tabs>
  <Tab title="Tous les graphiques dans un Workspace">
    ...
    3. Dans le volet qui s’affiche, sélectionnez **graphique en courbes**.
    4. Choisissez **fidélité totale** dans la section **agrégation des points**.
    5. Dans la liste déroulante **Affichage de fidélité totale**, sélectionnez **Bande** ou **Ligne**.
    6. Configurez l’algorithme et les paramètres de **lissage**.
    7. Réglez **Aggregation** sur **Mean**, **Min** ou **Max**.
    8. Cliquez sur **Apply**.
  </Tab>
...
### Configurer l’ombrage {#configure-shading}
...
### Explorez vos données sans perdre en fidélité des données {#explore-your-data-without-losing-data-fidelity}
...
### Activer l’échantillonnage aléatoire {#enable-random-sampling}
