...
description: "Référence des paramètres du panneau de graphique en courbes, notamment l’axe X, l’axe Y, le lissage, l’agrégation et les options de regroupement."
keywords: ["lissage EMA", "lissage gaussien", "fidélité totale", "modèle de légende", "expressions"]
---
...
### Méthode d’agrégation des points {#point-aggregation-method}

Choisissez le mode d’échantillonnage pour afficher les points de données :

* **Fidélité totale** (par défaut) : Voir [Fidélité totale](/fr/models/app/features/panels/line-plot/sampling/#full-fidelity).
* **Échantillonnage aléatoire** : Voir [Échantillonnage aléatoire](/fr/models/app/features/panels/line-plot/sampling/#random-sampling).
...
### Ignorer les valeurs aberrantes {#ignore-outliers}
...
### Nombre maximal de runs ou de groupes {#max-runs-or-groups}
...
### Regroupement des runs {#run-grouping}

Lorsque la Runs table est regroupée, chaque graphique en courbes fusionne les runs d’un même groupe en une seule courbe. Configurez l’application de ce regroupement aux graphiques :

* **Utiliser le regroupement de la Runs table dans les graphiques** : Applique le regroupement de la Runs table aux graphiques en courbes. Activé par défaut. Désactivez cette option pour représenter chaque run individuellement, que la Runs table soit regroupée ou non.
* **Utiliser uniquement le dernier run par groupe** : Représente un run par groupe au lieu de combiner les runs du groupe. Désactivé par défaut. Disponible uniquement au niveau du Workspace.
* **Agrégation** : Définit la valeur de la courbe combinée pour chaque groupe. Les options sont **moyenne**, **minimum** ou **maximum**. Indisponible lorsque **Utiliser uniquement le dernier run par groupe** est activé.

Si vous exécutez une expérience plusieurs fois dans le même groupe, activez **Utiliser uniquement le dernier run par groupe**. Chaque graphique affiche alors uniquement la tentative la plus récente, tandis que les runs précédents restent dans le projet.

Le dernier run d’un groupe est le premier run de ce groupe dans le sélecteur de runs. Modifiez l’ordre de tri de la Runs table pour représenter un run différent pour chaque groupe.

<Note>
  **Utiliser uniquement le dernier run par groupe** ne prend effet que lorsque la **Méthode d’agrégation des points** est définie sur **fidélité totale**, la valeur par défaut. Cette option n’a aucun effet en mode **échantillonnage aléatoire**.
</Note>
...
## Paramètres de regroupement {#grouping-settings}

Regroupez tous les runs en activant le regroupement, ou regroupez-les selon une variable donnée. Lorsque vous activez le regroupement dans le Runs table, les groupes s’affichent automatiquement dans le graphique.

* **Regrouper les runs** : Active le regroupement des runs dans le graphique. Requis pour configurer la zone ombrée du graphique. Pour cesser d’agréger les runs en une seule courbe, désactivez cette option : il n’existe pas de commande distincte pour désactiver l’agrégation tout en conservant le regroupement activé.
...
* **Agrégation** : La valeur de la courbe sur le graphique. Les options sont la moyenne, la médiane, le minimum et le maximum du groupe. Ce contrôle s’applique uniquement lorsque **Regrouper les runs** est activé.
...
## Paramètres du graphique {#chart-settings}
...
## Paramètres de la légende {#legend-settings}
...
### Modèle de légende {#legend-template}
...
### Valeurs spécifiques à un point {#point-specific-values}
...
### Créer de nouveaux graphiques en courbes avec des expressions {#create-new-line-plots-with-expressions}
...
### Transformez des graphiques en courbes existants à l’aide d’expressions {#transform-existing-line-plots-with-expressions}
...
### Référencer des valeurs {#referencing-values}
...
#### Configurations imbriquées {#nested-configs}
...
### Constantes et fonctions mathématiques {#math-constants-and-functions}
...
#### Constantes mathématiques {#math-constants}
...
#### Fonctions arithmétiques et statistiques {#arithmetic-and-statistical-functions}
...
#### Fonctions logarithmiques et exponentielles {#logarithmic-and-exponential-functions}
...
#### Fonctions hyperboliques {#hyperbolic-functions}
...
### Expressions de panneau à plusieurs métriques {#multi-metric-panel-expressions}
...
#### Groupes de capture {#capture-groups}
