...
## Ajouter un graphique en courbes {#add-a-line-plot}
...
<Tabs>
  ...
  <Tab title="Graphique en courbes avec plusieurs métriques">
    Dans un [espace de travail automatique](/fr/models/app/features/panels#workspace-modes), W&amp;B crée automatiquement un graphique en courbes à métrique unique pour chaque métrique enregistrée.

    Cette section explique comment créer un graphique en courbes affichant plusieurs métriques sélectionnées à l’aide d’une expression régulière JavaScript. Un graphique en courbes multi-métrique vous permet de comparer des métriques associées dans un même panneau au lieu de les examiner dans plusieurs panneaux.
    ...
    4. Dans **Regex**, saisissez une expression au format [RE2 de Google](https://github.com/google/re2/wiki/Syntax). Au fur et à mesure de la saisie, l’interface utilisateur se met à jour pour indiquer quelles métriques correspondent à l’expression. Par défaut, le nom du graphique affiche l’expression régulière utilisée. Le graphique inclut des courbes pour toutes les métriques qui correspondent à l’expression, y compris les métriques enregistrées ultérieurement.
...
### Expressions régulières multi-métriques {#multi-metric-regular-expressions}

    Les graphiques en courbes multi-métriques utilisent la [syntaxe RE2 de Google](https://github.com/google/re2/wiki/Syntax) pour faire correspondre les noms de métriques. Les sections suivantes décrivent des cas d’usage courants et donnent plus de détails sur le fonctionnement des expressions régulières, notamment sur la manière dont les groupes de capture affectent les panneaux créés par W&amp;B.
...
## Modifier les paramètres du graphique en courbes {#edit-line-plot-settings}
...
### Graphique en courbes individuel {#individual-line-plot}
...
### Tous les graphiques en courbes d’une section {#all-line-plots-in-a-section}
...
### Tous les graphiques en courbes d’un Workspace {#all-line-plots-in-a-workspace}
...
## Visualiser des valeurs moyennes sur un graphique {#visualize-average-values-on-a-plot}
...
## Visualiser une valeur NaN dans un graphique {#visualize-nan-value-on-a-plot}
...
## Comparer plusieurs métriques sur un même graphique {#compare-multiple-metrics-on-one-chart}
...
## Modifier les couleurs des lignes {#change-the-colors-of-the-lines}
...
## Visualiser sur différents axes X {#visualize-on-different-x-axes}
...
## Masquer la légende du graphique {#hide-chart-legend}
...
## Créer une notification de métriques de run {#create-a-run-metrics-notification}
