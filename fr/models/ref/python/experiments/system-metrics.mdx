---
description: Référence des métriques système du CPU, GPU, de la mémoire, du disque et du réseau que le SDK W&B enregistre automatiquement pendant les exécutions.
...
## Afficher les métriques système {#view-system-metrics}

Vous pouvez afficher et surveiller les métriques système à l’aide de la W&amp;B App ou de l’interface terminal `wandb leet`.
...
<Tabs>
  ...
  <Tab title="LEET">
    Pour afficher les métriques système d’un run local dans votre terminal à l’aide de l’interface terminal `wandb leet` :
    ...
    2. Démarrez `wandb leet` à l’aide de l’une des commandes suivantes :
    ...
    Sans chemin du run, LEET ouvre par défaut la vue workspace. Appuyez sur `2` pour afficher les métriques système du run mis en évidence. Un répertoire de run ou un fichier `.wandb` ouvre la vue run unique, où les métriques système apparaissent par défaut dans la barre latérale droite.
    ...
    * `y` : Faire basculer le graphique ciblé entre les modes linéaire, logarithmique et, pour les métriques en pourcentage, heatmap par buckets.
    * `n` et `N` : Naviguer entre les pages du volet ciblé.
    * `q` ou `ctrl+c` : Quitter.

    Pour surveiller la machine actuelle sans run, lancez le system monitor autonome (SYMON) :

    ```bash
    wandb leet symon --interval 2s
    ```

    Voir [LEET terminal UI](/fr/models/app/leet-tui) pour le guide complet, ou la référence CLI [`wandb leet`](/fr/models/ref/cli/wandb-leet) pour la syntaxe des commandes.
  </Tab>
</Tabs>
...
<div id="process-cpu-percent-cpu">
  ### Pourcentage CPU du processus (CPU) {#process-cpu-percent-cpu}
</div>
...
<div id="process-cpu-threads">
  ### Threads CPU du processus {#process-cpu-threads}
</div>
...
<div id="disk-usage-percent">
  ### Pourcentage d’utilisation du disque {#disk-usage-percent}
</div>
...
<div id="disk-out">
  ### Écriture sur disque {#disk-out}
</div>

Représente le volume total des écritures système sur disque en mégaoctets (Mo).
Comme pour [Disque en lecture](#disk-in), les octets d&#39;écriture initiaux sur disque sont enregistrés lors du premier échantillonnage. Les échantillons suivants calculent la différence entre le nombre actuel d&#39;octets écrits et la valeur initiale.
...
<div id="process-memory-rss">
  ### Mémoire RSS du processus {#process-memory-rss}
</div>
...
<div id="process-memory-percent">
  ### Pourcentage de mémoire du processus {#process-memory-percent}
</div>
...
<div id="memory-available">
  ### Mémoire disponible {#memory-available}
</div>
...
<div id="network-sent">
  ### Octets réseau envoyés {#network-sent}
</div>
...
<div id="network-received">
  ### Données reçues sur le réseau {#network-received}
</div>

Indique le nombre total d’octets reçus sur le réseau.
Comme pour [octets réseau envoyés](#network-sent), le nombre initial d’octets reçus est enregistré lors de la première initialisation de la métrique. Les échantillons suivants calculent la différence entre le nombre actuel d’octets reçus et la valeur initiale.
...
<div id="gpu-memory-utilization">
  ### Utilisation de la mémoire GPU {#gpu-memory-utilization}
</div>
...
<div id="gpu-memory-allocated">
  ### Mémoire GPU allouée {#gpu-memory-allocated}
</div>
...
<div id="gpu-memory-allocated-bytes">
  ### Mémoire GPU allouée en octets {#gpu-memory-allocated-bytes}
</div>
...
<div id="gpu-utilization">
  ### Utilisation du GPU {#gpu-utilization}
</div>
...
<div id="gpu-temperature">
  ### Température du GPU {#gpu-temperature}
</div>
...
<div id="gpu-power-usage-watts">
  ### Consommation électrique du GPU en watts {#gpu-power-usage-watts}
</div>
...
<div id="gpu-power-usage-percent">
  ### Pourcentage de consommation électrique du GPU {#gpu-power-usage-percent}
</div>
...
<div id="gpu-sm-clock-speed">
  ### Vitesse d’horloge du SM du GPU {#gpu-sm-clock-speed}
</div>
...
<div id="gpu-memory-clock-speed">
  ### Fréquence d’horloge de la mémoire du GPU {#gpu-memory-clock-speed}
</div>
...
<div id="gpu-graphics-clock-speed">
  ### Fréquence d’horloge graphique du GPU {#gpu-graphics-clock-speed}
</div>
...
<div id="gpu-corrected-memory-errors">
  ### Erreurs de mémoire GPU corrigées {#gpu-corrected-memory-errors}
</div>
...
<div id="gpu-uncorrected-memory-errors">
  ### Erreurs de mémoire non corrigées du GPU {#gpu-uncorrected-memory-errors}
</div>
...
<div id="gpu-encoder-utilization">
  ### Utilisation de l’encodeur GPU {#gpu-encoder-utilization}
</div>
...
<div id="amd-gpu-utilization">
  ### Utilisation des GPU AMD {#amd-gpu-utilization}
</div>
...
<div id="amd-gpu-memory-allocated">
  ### Mémoire GPU AMD allouée {#amd-gpu-memory-allocated}
</div>
...
<div id="amd-gpu-temperature">
  ### Température du GPU AMD {#amd-gpu-temperature}
</div>
...
<div id="amd-gpu-power-usage-watts">
  ### Consommation électrique du GPU AMD en watts {#amd-gpu-power-usage-watts}
</div>
...
<div id="amd-gpu-power-usage-percent">
  ### Pourcentage de consommation électrique du GPU AMD {#amd-gpu-power-usage-percent}
</div>
...
<div id="apple-arm-mac-gpu">
  ## GPU sur Mac Apple ARM {#apple-arm-mac-gpu}
</div>

<div id="apple-gpu-utilization">
  ### Utilisation du GPU Apple {#apple-gpu-utilization}
</div>
...
<div id="apple-gpu-memory-allocated">
  ### Mémoire allouée du GPU Apple {#apple-gpu-memory-allocated}
</div>
...
<div id="apple-gpu-temperature">
  ### Température du GPU Apple {#apple-gpu-temperature}
</div>
...
<div id="apple-gpu-power-usage-watts">
  ### Consommation du GPU Apple en watts {#apple-gpu-power-usage-watts}
</div>
...
<div id="apple-gpu-power-usage-percent">
  ### Pourcentage d’utilisation de la puissance du GPU Apple {#apple-gpu-power-usage-percent}
</div>
...
<div id="ipu-device-metrics">
  ### Métriques de l’appareil IPU {#ipu-device-metrics}
</div>
...
<div id="tpu-compute-metrics">
  ### Métriques de calcul TPU {#tpu-compute-metrics}
</div>
...
<div id="tpu-memory-metrics">
  ### Métriques de mémoire des TPU {#tpu-memory-metrics}
</div>
...
<div id="tpu-interconnect-health">
  ### État de santé de l’interconnexion TPU {#tpu-interconnect-health}
</div>
...
<div id="tpu-transfer-latency">
  ### Latence des transferts TPU {#tpu-transfer-latency}
</div>
...
<div id="tpu-collective-communication">
  ### Communications collectives TPU {#tpu-collective-communication}
</div>
...
<div id="tpu-network-metrics">
  ### Métriques réseau des TPU {#tpu-network-metrics}
</div>
...
<div id="tpu-hlo-execution-metrics">
  ### Métriques d&#39;exécution HLO sur TPU {#tpu-hlo-execution-metrics}
</div>
...
<div id="trainium-neuron-core-utilization">
  ### Utilisation des NeuronCore de Trainium {#trainium-neuron-core-utilization}
</div>
...
<div id="trainium-host-memory-usage-total">
  ### Utilisation totale de la mémoire de l’hôte Trainium {#trainium-host-memory-usage-total}
</div>
...
<div id="trainium-neuron-device-total-memory-usage">
  ### Utilisation totale de la mémoire du périphérique Neuron Trainium {#trainium-neuron-device-total-memory-usage}
</div>
...
<div id="trainium-host-memory-usage-breakdown">
  ### Répartition de l’utilisation de la mémoire hôte de Trainium : {#trainium-host-memory-usage-breakdown}
</div>
...
<div id="trainium-neuron-core-memory-usage-breakdown">
  ### Répartition de l’utilisation de la mémoire des NeuronCore Trainium {#trainium-neuron-core-memory-usage-breakdown}
</div>
