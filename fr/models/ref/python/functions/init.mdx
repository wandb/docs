...
* `mode` : indique comment les données du run sont gérées, avec les options suivantes :
  ...
  * `"shared"` : (Il s’agit d’une fonctionnalité expérimentale.) Permet à plusieurs processus,
    ...
    à l’aide du run ID utilisé par le nœud principal.
...
* `reinit` :  Forme abrégée du paramètre &quot;reinit&quot;. Détermine le comportement de
...
* `resume` : contrôle le comportement lors de la reprise d’un run avec l’`id` spécifié.
