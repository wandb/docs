...
La courbe précision-rappel est particulièrement utile pour évaluer des classificateurs sur des jeux de données déséquilibrés. Une aire élevée sous la courbe PR indique à la fois une précision élevée (un faible taux de faux positifs) et un rappel élevé (un faible taux de faux négatifs). La courbe fournit des indications sur l’équilibre entre faux positifs et faux négatifs à différents niveaux de seuil, ce qui aide à évaluer les performances d’un modèle.

**Arguments :**

* `y_true`:  Étiquettes binaires réelles. La forme doit être (`num_samples`,).
* `y_probas`:  Scores ou probabilités prédits pour chaque classe.  Il peut s’agir d’estimations de probabilité, de scores de confiance ou de valeurs de décision non seuillées. La forme doit être (`num_samples`, `num_classes`).
...
* `split_table`:  Indique si le tableau doit être scindé dans une section séparée  de l’interface W&amp;B. Si `True`, le tableau sera affiché dans une section intitulée  &quot;Tableaux de graphiques personnalisés&quot;. La valeur par défaut est `False`.
