...
* `integration_id` (str): ID de l’intégration webhook utilisée pour envoyer la requête.
* `request_payload` (Optional[Annotated]): La charge utile, éventuellement avec des variables de modèle, à envoyer dans la requête webhook.
...
Définit une action webhook qui envoie à l’intégration (webhook) spécifiée.
