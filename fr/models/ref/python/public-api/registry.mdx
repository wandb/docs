...
Indique si tous les types d&#39;artefact sont autorisés dans le registre.

Si `True`, des artefacts de tout type peuvent être ajoutés. Si `False`, les artefacts sont limités aux types indiqués dans `artifact_types`.

**Retourne :**

* `bool` : La valeur de propriété `allow_all_artifact_types`.

***

### <kbd>propriété</kbd> Registry.artifact_types

Renvoie les types d&#39;artefact autorisés dans le registre.

Si `allow_all_artifact_types` vaut `True`, `artifact_types` reflète les types précédemment enregistrés ou actuellement utilisés dans le registre. Si `allow_all_artifact_types` vaut `False`, les artefacts sont limités aux types présents dans `artifact_types`.

**Remarque :**

> Les types d&#39;artefact précédemment enregistrés ne peuvent pas être supprimés.
...
registry.save()  # une fois enregistré, le type d'artefact `model` ne peut plus être supprimé
registry.artifact_types.append("accidentally_added")
registry.artifact_types.remove(
     "accidentally_added"
)  # Les types ne peuvent être supprimés que s'ils n'ont pas encore été enregistrés
```

**Retourne :**

* `AddOnlyArtifactTypesList` : la valeur de propriété artifact&#95;types.
...
Horodatage indiquant quand le registre a été créé.
...
### <kbd>propriété</kbd> Registry.description
...
Nom complet du registre, préfixe `wandb-registry-` compris.

**Retourne :**

* `str` : La valeur de propriété `full_name`.
...
* `str` : La valeur de propriété `name`.
...
* `str`: La valeur de propriété `organization`.
...
* `Literal["organization", "restricted"]`:  Le niveau de visibilité.
  * &quot;organization&quot;: Tous les membres de l’organisation peuvent consulter ce registre. Vous pouvez modifier leurs rôles plus tard dans les paramètres de l’interface utilisateur.
...
* `Literal['organization', 'restricted']`: La valeur de propriété de visibilité.
...
Ajoute des utilisateurs ou des équipes à ce registre.

**Arguments :**

* `members`:  Les utilisateurs ou équipes à ajouter au registre. Accepte des objets `User`, des objets `Team` ou leurs ID sous forme de chaîne de caractères.

**Retourne :**
Ce registre, afin de pouvoir enchaîner d’autres méthodes si nécessaire.
...
Le nom du registre doit être unique au sein de l’organisation. Cette fonction doit être appelée avec `api.create_registry()`
...
* `artifact_types` :  Une liste facultative des types d’artefact autorisés.
...
Renvoie les membres actuels de ce registre (utilisateurs et équipes).
...
Supprime des utilisateurs ou des équipes de ce registre.

**Arguments :**

* `members` :  Les utilisateurs ou les équipes à retirer du registre. Accepte les objets `User`, les objets `Team` ou leurs ID sous forme de chaîne.
...
* `TypeError` :  Si aucun membre n’est fourni en argument.
* `ValueError` :  S’il est impossible de déduire ou d’analyser des ID d’utilisateur ou d’équipe.
...
Enregistrez les attributs du registre sur le backend.
