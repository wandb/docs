...
registry.save()  # une fois sauvegardé, le type d'artefact `model` ne peut plus être supprimé
...
* `TypeError` :  Si aucun membre n’est passé en argument.
* `ValueError` :  S’il est impossible de déduire ou d’analyser les ID de l’utilisateur ou de l’équipe.
