---
title: TensorBoard
description: "Synchronisez les journaux TensorBoard avec W&B pour une visualisation dans le cloud, le partage et une analyse centralisée aux côtés des métriques système."
---
...
<Note>
  W&amp;B prend en charge l’intégration de TensorBoard dans le SaaS multilocataire de W&amp;B.
</Note>
...
L’argument `step` de `run.log()` est désactivé lors de la synchronisation avec TensorBoard. Si vous souhaitez définir un nombre d&#39;étapes différent, vous pouvez consigner les métriques avec une métrique d&#39;étape, comme suit :
...
Si vous souhaitez avoir davantage de contrôle sur la façon dont TensorBoard est patché, vous pouvez appeler `wandb.tensorboard.patch` au lieu de passer `sync_tensorboard=True` à `wandb.init`.
...
<Warning>
  Vous devez appeler soit `wandb.init()`, soit `wandb.tensorboard.patch` **avant** d’appeler `tf.summary.create_file_writer` ou de construire un `SummaryWriter` via `torch.utils.tensorboard`.
</Warning>
...
Si vous utilisez l’intégration TensorBoard de PyTorch, vous devrez peut-être téléverser manuellement le fichier JSON du Profiler PyTorch.
...
`wandb` 0.20.0 et versions ultérieures prend en charge la synchronisation des fichiers `tfevents` stockés dans S3, GCS ou Azure. `wandb` utilise les identifiants par défaut de chaque fournisseur de cloud, correspondant aux commandes du tableau suivant :

| Fournisseur de cloud | Identifiants                            | Format du répertoire de journalisation |
| -------------------- | --------------------------------------- | -------------------------------------- |
