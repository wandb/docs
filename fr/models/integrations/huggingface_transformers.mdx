...
<Tabs>
  <Tab title="Ligne de commande">
    ````bash python run_glue.py \ # exécutez votre script Python --report_to wandb \ # activez le logging vers W&B --run_name bert-base-high-lr \ # nom du run W&B (facultatif) # autres arguments de ligne de commande
    ici ```
    </Tab>
    <Tab title="Python"></Tab>
    </Tabs>

    <Note>Vous utilisez TensorFlow ? Remplacez simplement le `Trainer` de PyTorch par le `TFTrainer` de TensorFlow.</Note>


    ### Activez l&#39;enregistrement des points de contrôle du modèle

    Avec [Artifacts](/models/artifacts/), vous pouvez stocker gratuitement jusqu&#39;à 100 Go de modèles et de jeux de données, puis utiliser le [registre W&amp;B](/models/registry/). Avec le registre, vous pouvez enregistrer des modèles pour les explorer et les évaluer, les préparer pour la préproduction ou les déployer dans votre environnement de production.
