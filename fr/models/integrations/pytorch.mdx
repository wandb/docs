---
title: PyTorch
description: "Intégrer W&B à PyTorch pour le suivi des expériences, la gestion des versions des jeux de données et la journalisation des métriques, des gradients et des modèles."
---
...
Utilisez [W&amp;B](https://wandb.ai) pour le suivi des expériences en machine learning, la gestion des versions des jeux de données et la collaboration au sein des projets.
...
<div id="what-this-notebook-covers">
  ## Ce que présente ce notebook
</div>

Nous vous montrons comment intégrer W&amp;B à votre code PyTorch afin d’ajouter le suivi des expériences à votre pipeline.

<Frame>
  <img src="/images/tutorials/pytorch.png" alt="Schéma d’intégration entre PyTorch et W&B" />
</Frame>
...
with wandb.init(project="new-sota-model", config=config) as run:
    ...
    # journaliser des métriques dans votre boucle d'entraînement pour visualiser les performances du modèle
    run.log(metrics)

    # optionnel : enregistrer le modèle à la fin
    model.to_onnx()
    run.save("model.onnx")
```
...
**Remarque** : les sections commençant par *Étape* suffisent pour intégrer W&amp;B à un pipeline existant. Le reste se contente de charger des données et de définir un modèle.
...
Dans le code, la première chose à faire consiste à définir notre expérience :
...
puis d&#39;y accéder selon les besoins.

Dans cet exemple, nous ne faisons varier que quelques hyperparamètres
et codons le reste à la main.
Mais n&#39;importe quelle partie de votre modèle peut figurer dans la `config`.

Nous incluons également quelques métadonnées : nous utilisons le jeu de données MNIST et une architecture
convolutionnelle. Si, plus tard, nous travaillons par exemple
avec des architectures entièrement connectées sur CIFAR dans le même projet,
cela nous aidera à distinguer nos runs.
...
qui est assez typique pour l’entraînement d’un modèle :
...
def model_pipeline(hyperparameters):
    ...
    with wandb.init(project="pytorch-demo", config=hyperparameters) as run:
        # accéder à tous les hyperparamètres via run.config, pour que la journalisation corresponde à l'exécution.
        config = run.config
...
L&#39;appel de cette fonction établit une ligne de communication
entre votre code et nos serveurs.

Transmettre le dictionnaire `config` à `wandb.init`
journalise immédiatement toutes ces informations,
de sorte que vous saurez toujours quelles valeurs d&#39;hyperparamètre
vous avez définies pour votre expérience.

Pour vous assurer que les valeurs que vous avez choisies et journalisées sont bien celles utilisées
...
> (par exemple, si un gigantesque monstre marin attaque nos centres de données)
> ne fasse pas planter votre code.
> Une fois le problème résolu, par exemple lorsque le Kraken retourne dans les profondeurs,
> vous pouvez journaliser les données avec `wandb sync`.
...
def make(config):
    ...
    # Créer la perte et l'optimiseur
...
N’hésitez pas à expérimenter et à essayer quelques expériences --
...
Dans notre `model_pipeline`, passons à l’étape suivante : il est temps de définir comment `entraîner` le modèle.
...
toutes les `log_freq` étapes d’entraînement.

Il vous suffit de l’appeler avant de démarrer l’entraînement.

Le reste du code d’entraînement reste inchangé :
nous itérons sur les époques et les lots,
en exécutant les passes avant et arrière
et en appliquant notre `optimizer`.

```python
def train(model, loader, criterion, optimizer, config):
    # Indiquer à wandb de surveiller le modèle : gradients, poids, et plus encore.
...
La seule différence se situe dans le code de journalisation :
alors qu’auparavant vous pouviez signaler des métriques en les affichant dans le terminal,
...
Ces chaînes identifient les objets journalisés, qui constituent les valeurs.
Vous pouvez aussi indiquer facultativement à quelle `step` d’entraînement vous en êtes.

> *Remarque* : j’aime utiliser le nombre d’exemples que le modèle a vus,
> car cela facilite les comparaisons entre différentes tailles de lot,
> mais vous pouvez aussi utiliser les étapes brutes ou le nombre de lots. Pour les runs d’entraînement plus longs, il peut également être pertinent de journaliser par `epoch`.

```python
def train_log(loss, example_ct, epoch):
    with wandb.init(project="pytorch-demo") as run:
        # Journaliser la perte et le numéro d'époque
        # C'est ici que nous journalisons les métriques dans W&B
...
l’exécuter sur de nouvelles données provenant de la production, par exemple,
ou l’appliquer à des exemples sélectionnés à la main.

<div id="optional-call-runsave">
  ## (Facultatif) Appelez `run.save()`
</div>

C&#39;est aussi le moment idéal pour enregistrer l&#39;architecture du modèle
et les paramètres finaux sur disque.
Pour une compatibilité maximale, nous allons `export` notre modèle au
...
sont enregistrés sur les serveurs de W&amp;B : plus besoin de chercher quel `.h5` ou `.pb`
correspond à quels runs d&#39;entraînement.

Pour des fonctionnalités `wandb` plus avancées de stockage, de
gestion des versions et de distribution de
...
<div id="run-training-and-watch-your-metrics-live-on-wandbai">
  ### Lancez l’entraînement et suivez vos métriques en direct sur wandb.ai
</div>

Maintenant que nous avons défini l’ensemble du pipeline et ajouté
ces quelques lignes de code W&amp;B,
nous sommes prêts à lancer notre expérience avec suivi complet.
...
<div id="test-hyperparameters-with-sweeps">
  ### Tester les hyperparamètres avec Sweeps
</div>

Dans cet exemple, nous n’avons examiné qu’un seul ensemble d’hyperparamètres.
Mais une part importante de la plupart des flux de travail ML consiste à itérer sur
...
Consultez un [notebook Colab qui montre l’optimisation des hyperparamètres avec W&amp;B Sweeps](https://wandb.me/sweeps-colab).

Exécuter un balayage d’hyperparamètres avec W&amp;B est très simple. Il n’y a que 3 étapes simples :
...
Explorez des exemples de projets suivis et visualisés avec W&amp;B dans notre [Gallery →](https://app.wandb.ai/gallery).

<div id="advanced-setup">
  ## Configuration avancée
</div>

1. [Variables d&#39;environnement](/fr/platform/hosting/env-vars/): Définissez des clés API dans des variables d&#39;environnement afin de pouvoir exécuter l&#39;entraînement sur un cluster géré.
2. [Mode hors ligne](/fr/support/models/articles/can-i-run-wandb-offline): Utilisez le mode `dryrun` pour effectuer l&#39;entraînement hors ligne et synchroniser les résultats plus tard.
3. [Sur site](/fr/platform/hosting/hosting-options/self-managed): Installez W&amp;B dans un cloud privé ou sur des serveurs isolés du réseau au sein de votre propre infrastructure. Nous proposons des installations locales pour tous les profils, du monde universitaire aux équipes d&#39;entreprise.
