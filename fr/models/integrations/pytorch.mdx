...
# capturer un dictionnaire d'hyperparamètres avec la configuration
config = {
    "learning_rate": 0.001,
    "epochs": 100,
    "batch_size": 128
...
Suivez le [tutoriel vidéo](https://wandb.me/pytorch-video).

**Remarque** : les sections qui commencent par *Étape* suffisent pour intégrer W&amp;B à un pipeline existant. Le reste se contente de charger des données et de définir un modèle.
...
vous devrez vous inscrire gratuitement via le lien qui s&#39;affiche.
...
Dans le code, la première chose que nous faisons est de définir notre expérience :
...
1. nous commençons par appeler `make` pour créer un modèle, ainsi que les données et l’optimiseur associés, puis
2. nous appelons `train` pour entraîner le modèle, et enfin
3. nous utilisons `test` pour voir comment l’entraînement s’est déroulé.
...
Le passage du dictionnaire `config` à `wandb.init`
...
Pour vous assurer que les valeurs que vous avez choisies et enregistrées sont bien celles utilisées
...
> vous pouvez enregistrer les données avec `wandb sync`.
...
N’hésitez pas à modifier cela et à essayer quelques expériences --
...
en exécutant des passes avant et arrière
...
alors qu’auparavant vous pouviez rapporter des métriques en les affichant dans le terminal,
...
> car cela facilite la comparaison entre différentes tailles de lot,
> mais vous pouvez aussi utiliser le nombre brut d’étapes ou de lots. Pour les runs d’entraînement plus longs, il peut également être pertinent de journaliser par `epoch`.
...
et ses paramètres finaux sur disque.
...
sont enregistrés sur les serveurs de W&amp;B : plus besoin de perdre le fil entre les fichiers `.h5` ou `.pb`
et les runs d&#39;entraînement auxquels ils correspondent.
...
2. [Mode hors ligne](/fr/models/support/run_wandb_offline/) : Utilisez le mode `dryrun` pour entraîner hors ligne et synchroniser les résultats plus tard.
3. [Sur site](/fr/platform/hosting/hosting-options/self-managed) : Installez W&amp;B dans un cloud privé ou sur des serveurs isolés du réseau au sein de votre propre infrastructure. Nous proposons des installations locales pour tous les profils, du milieu académique aux équipes d&#39;entreprise.
4. [Sweeps](/fr/models/sweeps/) : Configurez rapidement une recherche d&#39;hyperparamètres avec notre outil léger d&#39;ajustement.