...
Utilisez l’interface de l’application W&amp;B ou le SDK Python W&amp;B pour définir une politique TTL pour un artifact existant.

<Note>Lorsque vous modifiez le TTL d’un artifact, le délai avant expiration est toujours calculé à partir du timestamp `createdAt` de l’artifact.</Note>
