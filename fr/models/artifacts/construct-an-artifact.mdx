...
## Construire un artifact {#construct-an-artifact}
...
### Créez un objet artifact Python avec `wandb.Artifact()` {#create-an-artifact-python-object-with-wandbartifact}
...
<Info>
  Les Artifacts ne peuvent pas porter le même nom, même s&#39;ils ont des types différents. Par exemple, vous ne pouvez pas créer un artifact nommé `cats` de type `dataset` et un autre artifact nommé `cats` de type `model`.
</Info>

Vous pouvez aussi fournir une description, des métadonnées, un algorithme de digest et d&#39;autres paramètres lorsque vous initialisez un objet artifact.
...
<Note>
  MD5 reste l&#39;algorithme de digest par défaut. Avec le SDK Python W&amp;B 0.29.0 ou une version ultérieure, définissez `digest_algorithm="XXH128"` pour activer un hachage plus rapide pour les nouveaux artifacts. Les artifact collections existantes qui utilisent MD5 continuent d&#39;utiliser MD5. Les versions du SDK antérieures à 0.29.0 ne peuvent pas vérifier les artifacts contenant des entrées XXH128 avec `Artifact.verify()`.
</Note>

Pour plus d&#39;informations sur les attributs et paramètres disponibles, consultez la définition de la classe [`wandb.Artifact`](/fr/models/ref/python/experiments/artifact) dans le guide de Référence du SDK Python.

### Ajoutez un ou plusieurs fichiers à l&#39;artifact {#add-one-or-more-files-to-the-artifact}
...
### Enregistrez votre artifact sur le serveur W&amp;B {#save-your-artifact-to-the-wb-server}
...
## Ajouter des fichiers à un artifact {#add-files-to-an-artifact}
...
### Ajouter un seul fichier {#add-a-single-file}
...
### Ajouter plusieurs fichiers {#add-multiple-files}
...
### Ajouter une référence URI {#add-a-uri-reference}
...
### Ajouter des fichiers à des artefacts depuis des exécutions parallèles {#add-files-to-artifacts-from-parallel-runs}
...
## Trouver le chemin d’accès des Artifacts enregistrés et d’autres métadonnées {#find-path-for-logged-artifacts-and-other-metadata}
