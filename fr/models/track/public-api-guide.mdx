...
### Exporter les données d’un run {#export-run-data}
...
### Interroger plusieurs runs {#querying-multiple-runs}
...
### Obtenir le nom et l’ID d’un run pendant un run actif {#get-a-runs-name-and-id-during-an-active-run}
...
## Exemples d’utilisation de l’API publique {#public-api-examples}

### Lire les métriques d’un run {#read-metrics-from-a-run}
...
### Lire des métriques spécifiques d’un run {#read-specific-metrics-from-a-run}
...
### Comparer deux runs {#compare-two-runs}
...
### Mettre à jour les métriques d’un run après la fin du run {#update-metrics-for-a-run-after-the-run-has-finished}
...
### Renommer une métrique dans un run terminé {#rename-a-metric-in-a-completed-run}
...
### Mettre à jour la configuration d’un run existant {#update-config-for-an-existing-run}
...
### Exporter l’utilisation des ressources système vers un fichier CSV {#export-system-resource-consumptions-to-a-csv-file}
...
### Obtenir des données de métriques non échantillonnées {#get-unsampled-metric-data}
...
### Obtenir des données paginées dans l’historique {#get-paginated-data-from-history}
...
### Exporter les métriques de tous les runs d’un projet dans un fichier CSV {#export-metrics-from-all-runs-in-a-project-to-a-csv-file}
...
### Obtenir l’heure de début d’un run {#get-the-starting-time-for-a-run}
...
### Importer des fichiers dans un run terminé {#upload-files-to-a-finished-run}
...
### Télécharger un fichier d’un run {#download-a-file-from-a-run}
...
### Télécharger tous les fichiers d’un run {#download-all-files-from-a-run}
...
### Obtenir les runs d’un balayage spécifique {#get-runs-from-a-specific-sweep}
...
### Obtenir le meilleur run d’un balayage {#get-the-best-run-from-a-sweep}
...
### Télécharger le fichier du meilleur modèle depuis un balayage {#download-the-best-model-file-from-a-sweep}
...
### Supprimer d’un run tous les fichiers ayant une extension donnée {#delete-all-files-with-a-given-extension-from-a-run}
...
### Télécharger les données des métriques système {#download-system-metrics-data}
...
### Mettre à jour les métriques du summary {#update-summary-metrics}
...
### Obtenir la commande qui a lancé le run {#get-the-command-that-ran-the-run}
...
{/* ### Échantillonnage
  ...
  La méthode `history` par défaut échantillonne les métriques selon un nombre fixe d’échantillons (500 par défaut, modifiable à l’aide de l’argument `samples`). Pour exporter toutes les données d’un run volumineux, utilisez la méthode `run.scan_history()`. Pour plus de détails, voir la [Référence de l’API](/models/ref/python/public-api).  */}