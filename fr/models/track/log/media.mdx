...
<Tabs>
  ...
  <Tab title="Journaliser des images PIL">
    ...
    with wandb.init(project="") as run:
        ...
        # Si nécessaire, convertissez-la en RGB
        if image.mode != "RGB":
...
<Tabs>
  <Tab title="Masques de segmentation">
    ...
    * `"class_labels"`: (facultatif) un dictionnaire associant les étiquettes de classe entières du masque de l’image à des noms de classe lisibles
    ...
    Pour consigner plusieurs masques, consignez un dictionnaire de masques avec plusieurs clés, comme dans l’extrait de code ci-dessous.
  ...
  <Tab title="Boîtes englobantes">
    Consignez des boîtes englobantes avec des images, et utilisez des filtres et des bascules pour visualiser dynamiquement différents ensembles de boîtes dans l’UI.
    ...
    Pour consigner une boîte englobante, vous devez fournir à l’argument mot-clé `boxes` de `wandb.Image` un dictionnaire contenant les clés et valeurs suivantes :
                ...
                "box_data": [
                    {
                        ...
                        # (à titre illustratif uniquement, toutes les boîtes sont susceptibles
                        # d'être dans le même domaine/format)
