...
## Suivre un seul processus {#track-a-single-process}
...
## Suivre plusieurs processus {#track-multiple-processes}
...
### Suivre chaque processus séparément {#track-each-process-separately}
...
### Organiser les runs distribués {#organize-distributed-runs}
...
### Suivre tous les processus dans un seul run {#track-all-processes-to-a-single-run}
...
Dans cette approche, vous utilisez un nœud principal et un ou plusieurs nœuds workers. Avant de démarrer le job distribué, générez un ID de run unique et configurez chaque processus pour utiliser cet ID de run. Pendant l’entraînement, chaque nœud worker consigne ses données dans le même ID de run que le nœud principal. W&amp;B agrège les métriques de tous les nœuds et les affiche dans la W&amp;B App UI.
...
2. Définissez `WANDB_RUN_ID` sur le même ID de run généré que celui utilisé par le nœud principal avant le démarrage du processus worker.
...
<Tip>
  ...
  * Générez l’ID de run avant de démarrer le job distribué. Utilisez votre lanceur, planificateur ou framework d’orchestration pour définir `WANDB_RUN_ID` pour chaque processus.
  * Configurez les variables d’environnement W&amp;B avant le démarrage de chaque processus. Évitez de modifier `os.environ` au moment de l’exécution, car le SDK W&amp;B pourrait ne pas détecter la modification.
</Tip>

La manière dont vous générez et distribuez l’ID de run dépend du framework que vous utilisez pour orchestrer les nœuds. Ce guide ne fournit pas d’exemple de lanceur générique. Les extraits suivants présentent la configuration W&amp;B au sein de chaque processus après que votre couche d’orchestration a défini `WANDB_RUN_ID`.
...
with wandb.init(
    ...
    # Placer ici la logique du nœud principal
```

L’exemple suivant montre la configuration correspondante pour un nœud worker. Définissez `rank` sur un identifiant unique pour le worker, en fonction de votre framework d’orchestration.
...
# Initialiser un run dans un nœud worker.
# W&B lit automatiquement WANDB_RUN_ID depuis l'environnement du processus worker.
with wandb.init(
    ...
    # La logique du nœud worker se place ici
```

Dans un déploiement distribué, les nœuds worker peuvent s’exécuter sur des machines distinctes.
...
### Processus enfant {#spawn-process}
