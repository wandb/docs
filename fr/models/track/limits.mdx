...
Les noms de métriques doivent respecter certaines contraintes de nommage imposées par GraphQL. Voir [les contraintes de nommage des métriques](/fr/models/track/log/#metric-naming-constraints) pour en savoir plus.

{/* ### Journaliser des médias sous le même nom de métrique
  Journalisez les médias liés sous le même nom de métrique :
...
<Note>Data is saved and tracked even if you log values wider than the recommended amount. However, your plots may load more slowly.</Note>
...
with wandb.init(project="metric-frequency") as run:
    ...
    run.log(
        ...
        commit=True,
    )  # Valider ensemble les métriques groupées par étape
...
{/* Activez le traitement par lots des appels à `wandb.Run.log()` en passant `commit=False` afin de minimiser le nombre total d'appels à l'API pour une étape donnée. Voir [la documentation](/models/ref/python/experiments/run/#method-runlog) de `run.log()` pour plus de détails. */}

<Note>W&amp;B continues to accept your logged data but pages may load more slowly if you exceed guidelines.</Note>
...
with wandb.init(
    project="config-size",
    config={
        "large_list": list(range(10000000)),  # Liste volumineuse
        "large_string": "a" * 10000000,  # Chaîne volumineuse
    }
) as run:
    # Votre code d'entraînement ici
    pass
