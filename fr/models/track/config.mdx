...
<div id="nested-dictionaries-in-config">
  ### Dictionnaires imbriqués dans la configuration
</div>

`config-defaults.yaml` prend en charge les dictionnaires imbriqués lorsqu&#39;un mappage imbriqué constitue la `value` d&#39;une clé de premier niveau. Vous pouvez également transmettre un dictionnaire imbriqué directement à `wandb.init()` :
...
Certaines vues de l&#39;W&amp;B App affichent les clés de configuration imbriquées sous forme pointée (par exemple, `EPISODE_BUDGET.cramped_room`). Les filtres de l&#39;API publique préfixent ces chemins par `config.` (par exemple, `config.EPISODE_BUDGET.cramped_room`). En Python, accédez aux valeurs imbriquées par indexation classique de dictionnaire : `run.config["EPISODE_BUDGET"]["cramped_room"]`.
