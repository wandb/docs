...
Par défaut, chaque appel à `wandb.Run.log()` correspond à une nouvelle `step`. W&amp;B utilise les étapes comme axe X par défaut lorsqu&#39;il crée des graphiques et des panneaux. Vous pouvez aussi créer et utiliser un axe X personnalisé, ou capturer une métrique récapitulative personnalisée. Pour plus d&#39;informations, voir [Personnaliser les axes des logs](/fr/models/track/log/customize-logging-axes/).

{/* [INSÉRER UN EXEMPLE PLUS PERTINENT] */}

{/* Si vous souhaitez journaliser des données dans une seule étape de l’historique depuis de nombreux endroits de votre code, vous pouvez passer un index d’étape à `run.log()` comme suit :
