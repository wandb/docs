...
# 레퍼런스 문서 생성을 위한 라이선스 고지 {#license-notice-for-reference-documentation-generation}

## Overview {#overview}

이 디렉터리의 스크립트는 개발/CI 과정에서 레퍼런스 문서를 생성하는 용도로만 사용됩니다. Weave 라이브러리와 함께 배포되지 않으며, 프로덕션 코드에도 포함되지 않습니다.

## 의존성과 해당 라이선스 {#dependencies-and-their-licenses}
...
### 전이적 의존성 (via lazydocs) {#transitive-dependencies-via-lazydocs}
...
## 엄격한 라이선스 정책이 있는 조직의 경우 {#for-organizations-with-strict-license-policies}
...
### 알려진 Socket Security 경고 {#known-socket-security-warnings}
