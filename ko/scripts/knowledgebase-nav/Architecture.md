...
생성기는 지원팀(지식 베이스) 내비게이션을 아티클 콘텐츠와 일관되게 유지합니다. 이 도구는 설정된 제품(예: Models, Weave, Inference)을 대상으로 실행되며, `support/<product>/articles/` 아래의 MDX 아티클을 조회하고, 생성된 MDX 페이지와 루트 `support.mdx`의 개수를 업데이트합니다. 생성기는 `docs.json`을 읽거나 쓰지 않습니다. 사용자가 워크플로의 PR 댓글을 바탕으로 해당 파일을 직접 편집합니다.
...
커밋되는 경로 패턴에는 `support.mdx`, `support/*/articles/*.mdx`, `support/*/tags/*.mdx`, 그리고 `support/*.mdx`(제품 인덱스)가 포함됩니다. `docs.json`은 의도적으로 제외되며, 사람이 수동으로 업데이트합니다.

<div id="pipeline-orchestration">
  ## 파이프라인 오케스트레이션
</div>

`run_pipeline(repo_root, config_path)`는 CLI와 테스트에서 사용하는 단일 진입점입니다. `config.yaml`을 로드하고, 모든 제품에 대해 Jinja2 환경 하나를 빌드한 다음 각 제품을 순회합니다. 루프가 끝나면 `support.mdx`를 한 번 업데이트합니다. `docs.json`은 건드리지 않습니다.
...
`render_tag_pages`는 정렬된 페이지 ID 문자열(예: `support/models/tags/security`)을 반환합니다. `pr_report.py`는 워크플로의 PR 댓글에서 &quot;docs.json update required&quot; 섹션을 구축할 때 동일한 ID를 사용하므로, 사람이 `docs.json`에서 해당 `Support: <display_name>` 탭을 업데이트할 수 있습니다.
...
* **`cleanup_stale_tag_pages`**는 방금 생성되지 않은 tags 디렉터리의 `.mdx` 파일을 삭제해 tags 디렉터리에 오래된 항목이 남지 않도록 합니다.
...
* **`update_support_index`**는 루트 `support.mdx`의 product Card에 있는 개수 줄을 업데이트합니다. `_COUNTS_START_RE` / `_COUNTS_END_RE`를 통해 마커를 찾고, 마이그레이션 시에는 단순 개수 줄 패턴을 대체 수단으로 사용합니다.
* **`update_support_featured`**는 루트 `support.mdx`의 추천 아티클 섹션을 다시 생성하며, `_FEATURED_START_RE` / `_FEATURED_END_RE`를 통해 블록을 찾습니다.

이 파이프라인은 `docs.json`을 수정하지 않습니다. 태그 페이지의 추가 및 제거는 `pr_report.py`를 통해 사람이 검토할 수 있도록 표시되며, 이 스크립트는 워크플로의 PR 댓글에 영향을 받는 페이지 ids를 나열합니다.
...
* **`_FEATURED_START`** / **`_FEATURED_END`** — `support.mdx`에 기록되는 정규 `{/* AUTO-GENERATED: 추천 아티클 */}` 문자열입니다. **`_FEATURED_START_RE`** / **`_FEATURED_END_RE`** — 추천 아티클 블록을 찾는 데 사용되는 패턴입니다.
...
* **오래된 태그 정리**: 더 이상 어떤 아티클 키워드와도 대응하지 않는 태그 페이지는 생성 후 삭제됩니다. 이렇게 하면 태그 디렉터리에 고아 항목이 남지 않으며, 이후 워크플로의 PR 댓글에서 사람이 `docs.json`에서 일치하는 항목을 제거하라고 안내합니다.
...
* **`docs.json`은 사람이 편집합니다**: 생성기는 `docs.json`을 읽거나 쓰지 않습니다. 태그 페이지 추가 및 제거는 `pr_report.py`를 통해 표시되며, 이 스크립트는 페이지 ID를 `Support: <display_name>`별로 묶어 나열하므로 사람이 해당 탭을 수동으로 업데이트할 수 있습니다.
* **골든 테스트**: 생성된 태그 페이지, 제품 인덱스 페이지, 아티클 파일(푸터 마커 포함), 루트 `support.mdx`를 커밋된 트리와 비교해 출력 드리프트가 통합 diff로 드러나도록 합니다. 골든 스위트는 또한 임시 트리에서 `docs.json`이 절대 생성되지 않음을 검증합니다.
