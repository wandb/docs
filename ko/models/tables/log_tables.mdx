...
2. [`wandb.Table`](/models/ref/python/data-types/table) 클래스로 Table 오브젝트를 생성합니다. `columns` 및 `data` 파라미터에 각각 테이블의 컬럼과 데이터를 지정합니다. 선택 사항인 `log_mode` 파라미터를 `IMMUTABLE` (기본값), `MUTABLE`, 또는 `INCREMENTAL` 중 하나로 설정하는 것이 좋습니다. 자세한 내용은 다음 섹션의 [테이블 로깅 모드](#로깅-모드)를 참조하세요.
