...
## Weave로 모델 평가하기 {#evaluate-models-with-weave}
...
### 모델 평가의 주요 기능 {#key-features-for-model-evaluation}
...
### 시작하기: W&amp;B 레지스트리의 모델 평가하기 {#getting-started-evaluate-a-model-from-wb-registry}

W&amp;B Models 레지스트리에서 모델을 다운로드한 다음 Weave를 사용해 평가하세요:
...
### W&amp;B Models와 Weave 평가 통합하기 {#integrate-weave-evaluations-with-wb-models}
...
### Weave의 고급 특성 {#advanced-weave-features}

#### 맞춤형 Scorer와 평가자 {#custom-scorers-and-judges}
...
## 테이블을 사용해 모델 평가하기 {#evaluate-models-with-tables}
...
### 기본 예시: 평가 결과 로깅하기 {#basic-example-log-evaluation-results}
...
for idx, (input_data, label) in enumerate(test_dataset):
    ...
    eval_table.add_data(
        idx,
        wandb.Image(input_data),  # 이미지 또는 기타 미디어 로깅하기
        label,
        predicted_class,
        confidence,
...
### 고급 테이블 워크플로 {#advanced-table-workflows}

#### 여러 모델 비교 {#compare-multiple-models}

서로 다른 모델의 eval 테이블을 같은 키에 로깅해 직접 비교합니다:
...
#### 시간 경과에 따른 예측 추적 {#track-predictions-over-time}
...
### W&amp;B UI에서 대화형 분석 {#interactive-analysis-in-the-wb-ui}
...
### 예시: 보강된 테이블을 활용한 오류 분석 {#example-error-analysis-with-enriched-tables}
