...
<Note>
  바로 실행해 볼 수 있는 코드부터 보고 싶다면, 이 [Google Colab](https://wandb.me/hf)을 확인하세요.
</Note>
...
코드에서든 명령줄에서든 `Trainer`의 트레이닝 매개변수를 정의할 때 **가장 중요한 단계는** W&amp;B 로깅을 활성화할 수 있도록 `report_to`를 `"wandb"`로 설정하는 것입니다.

`TrainingArguments`의 `logging_steps` 매개변수는 트레이닝 중 트레이닝 메트릭이 W&amp;B에 얼마나 자주 전송되는지를 제어합니다. `run_name` 매개변수를 사용해 W&amp;B에서 트레이닝 run의 이름을 지정할 수도 있습니다.
...
with wandb.init(
    ...
    # checkpoint를 폴더에 다운로드하고 경로를 반환합니다
    ...
    # 트레이닝 매개변수를 여기에 입력하세요.
    training_args = TrainingArguments()

    trainer = Trainer(model=model, args=training_args)

    # checkpoint 디렉터리를 사용하여 checkpoint에서 트레이닝을 재개해야 합니다
    trainer.train(resume_from_checkpoint=checkpoint_dir)
```

<div id="how-do-i-log-and-view-evaluation-samples-during-training">
  ### 트레이닝 중 평가 샘플을 로그하고 확인하려면 어떻게 해야 하나요
</div>
...
아래에는 이 새 콜백을 HF Trainer에 추가하는 일반적인 패턴이 나와 있으며, 더 아래에는 평가 출력을 W&amp;B Table에 로그하는 전체 코드 예제가 나와 있습니다:

```python
# 평소와 같이 Trainer를 인스턴스화합니다
trainer = Trainer()

# 새 logging 콜백을 인스턴스화하고 Trainer 객체를 전달합니다
...
다음 섹션에서는 트레이닝 중에 모델 예측을 실행하고 평가 샘플을 W&amp;B Table에 log하도록 `WandbCallback`을 사용자 지정하는 방법을 보여줍니다. `eval_steps`마다 Trainer 콜백의 `on_evaluate` 방법을 사용합니다.
...
마지막으로 데이터프레임으로 `wandb.Table`을 생성하고 이를 wandb에 log합니다.
또한 `freq` 에포크마다 예측을 log하여 로깅 빈도를 제어할 수 있습니다.
...
class WandbPredictionProgressCallback(WandbCallback):
    """트레이닝 중 모델 예측을 기록하는 맞춤형 WandbCallback.

    이 콜백은 트레이닝 중 각 logging step마다 모델 예측과 레이블을 wandb.Table에 기록합니다.
    ...
    Attributes:
        ...
        num_samples (int, 선택): 예측 생성을 위해 검증 데이터셋에서
          선택할 샘플 수. 기본값은 100.
        freq (int, 선택): 로깅 빈도. 기본값은 2.
    """
