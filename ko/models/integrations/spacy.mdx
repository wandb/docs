...
| `model_log_interval` | `Optional int`. 기본값은 `None` 입니다. 이 옵션을 설정하면 [Artifacts](/ko/models/artifacts/) 를 통한 [모델 버전 관리](/ko/models/registry/) 가 활성화됩니다. 모델 체크포인트를 로그할 스텝 간격을 입력하세요. |
...
트레이닝이 시작되면 해당 트레이닝 [W&B 페이지](/ko/models/runs/) 로 연결되는 링크가 출력되며, 이 링크를 통해 W&B 웹 UI의 실험 추적 [대시보드](/ko/models/track/workspaces/) 로 이동할 수 있습니다.