...
description: "Hugging Face Diffusers에서 W&B autolog를 사용해 프롬프트, 생성된 미디어, 설정, 파이프라인 아키텍처를 추적하세요."
---
...
[Hugging Face Diffusers](https://huggingface.co/docs/diffusers/index)는 이미지, 오디오, 심지어 분자의 3D 구조까지 생성할 수 있는 최첨단 사전 학습 확산 모델을 위한 대표적인 라이브러리입니다. W&amp;B 인테그레이션은 이러한 사용 편의성은 그대로 유지하면서, 대화형 중앙 대시보드에서 풍부하고 유연한 실험 추적, 미디어 시각화, 파이프라인 아키텍처, 설정 관리를 제공합니다.
&quot;
...
코드 두 줄만 추가하면 실험에 연결된 모든 프롬프트, 네거티브 프롬프트, 생성된 미디어, 설정을 로깅할 수 있습니다. 로깅을 시작하려면 다음 두 줄의 코드를 추가하세요:
...
<Frame caption="실험 결과가 어떻게 로깅되는지에 대한 예시입니다.">
...
2. `autolog`를 사용해 W&amp;B Run을 초기화하고 [지원되는 모든 파이프라인 Call](https://github.com/wandb/wandb/blob/main/wandb/integration/diffusers/autologger.py#L12-L72)의 inputs과 출력을 자동으로 추적하세요.

   `autolog()` 함수를 `init` 파라미터와 함께 호출할 수 있으며, 이 파라미터는 [`wandb.init()`](/ko/models/ref/python/functions/init)에 필요한 파라미터의 딕셔너리를 받습니다.

   `autolog()`를 호출하면 W&amp;B Run이 초기화되고 [지원되는 모든 파이프라인 Call](https://github.com/wandb/wandb/blob/main/wandb/integration/diffusers/autologger.py#L12-L72)의 inputs과 출력이 자동으로 추적됩니다.

   * 각 파이프라인 Call은 Workspace의 자체 [테이블](/ko/models/tables/)로 추적되며, 해당 파이프라인 Call에 연결된 설정은 해당 run의 설정에 있는 워크플로 목록에 추가됩니다.
   * 프롬프트, 네거티브 프롬프트, 생성된 미디어는 [`wandb.Table`](/ko/models/tables/)에 로깅됩니다.
   * 시드와 파이프라인 아키텍처를 포함해 실험에 연결된 다른 모든 설정은 run의 설정 섹션에 저장됩니다.
   * 각 파이프라인 Call에 대해 생성된 미디어도 run의 [미디어 패널](/ko/models/track/log/media/)에 로깅됩니다.

   <Note>
     [지원되는 파이프라인 Call 목록](https://github.com/wandb/wandb/blob/main/wandb/integration/diffusers/autologger.py#L12-L72)을 찾을 수 있습니다. 이 인테그레이션의 새로운 기능을 요청하거나 이와 연결된 버그를 보고하려면 [W&amp;B GitHub issues page](https://github.com/wandb/wandb/issues)에서 이슈를 여세요.
   </Note>
...
다음은 autolog가 실제로 어떻게 동작하는지 보여 주는 간단한 엔드 투 엔드 예시입니다:
...
<Note>
  파이프라인을 호출한 뒤 IPython 노트북 환경에서 코드를 실행할 때는 [`wandb.Run.finish()`](/ko/models/ref/python/functions/finish)를 명시적으로 호출해야 합니다. Python 스크립트를 실행할 때는 이 호출이 필요하지 않습니다.
</Note>
