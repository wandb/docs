...
[Hugging Face Diffusers](https://huggingface.co/docs/diffusers)는 최첨단 사전 학습 디퓨전 모델을 사용해 이미지, 오디오, 심지어 분자의 3D 구조까지 생성할 수 있는 대표적인 라이브러리입니다. W&amp;B 인테그레이션은 이러한 사용 편의성을 해치지 않으면서, 풍부하고 유연한 실험 추적, 미디어 시각화, 파이프라인 아키텍처, 설정 관리를 인터랙티브한 중앙 집중식 대시보드에 더해 줍니다.
