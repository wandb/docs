---
description: "W&B를 Metaflow와 통합하여 메트릭과 아티팩트를 자동으로 로깅하고 실험을 추적하며 ML 워크플로를 관리하세요."
title: Metaflow
---
...
이 인테그레이션을 사용하면 사용자가 Metaflow [steps and 플로우](https://docs.metaflow.org/metaflow/basics)에 데코레이터를 적용하여 파라미터와 아티팩트를 W&amp;B에 자동으로 로깅할 수 있습니다.
...
<Note>
  `wandb` 버전 0.19.8 이하의 경우 `plum-dispatch` 대신 `fastcore` 버전 1.8.0 이하(`fastcore<1.8.0`)를 설치하세요.
</Note>
...
<Tabs>
  <Tab title="Step">
    step에 데코레이터를 적용하면 해당 step에서 특정 유형에 대한 로깅을 켜거나 끌 수 있습니다.
  ...
  <Tab title="Flow">
    플로우에 데코레이터를 적용하는 것은 기본값으로 모든 구성 step에 데코레이터를 적용하는 것과 같습니다.

    이 경우 `WandbExampleFlow`의 모든 step은 기본적으로 데이터셋과 모델을 로깅하며, 이는 각 step에 `@wandb_log(datasets=True, models=True)`를 적용한 것과 같습니다.
  ...
  <Tab title="Flow and Steps">
    플로우에 데코레이터를 적용하는 것은 기본값으로 모든 step에 데코레이터를 적용하는 것과 같습니다. 즉, 이후에 다른 `@wandb_log`로 step을 데코레이트하면 플로우 수준의 데코레이터 설정이 덮어써집니다.
    ...
    * `end`는 데이터셋과 모델을 모두 로깅하지 않습니다.
    ...
    class WandbExampleFlow(FlowSpec):
      ...
      # 이 step은 설정이 덮어써지므로 데이터셋과 모델을 로깅하지 않습니다
...
| kwarg      | Options                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `datasets` | <ul><li><code>True</code>: 데이터셋인 인스턴스 변수를 로깅합니다</li><li><code>False</code></li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `models`   | <ul><li><code>True</code>: 모델인 인스턴스 변수를 로깅합니다</li><li><code>False</code></li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `others`   | <ul><li><code>True</code>: 피클로 직렬화할 수 있는 기타 모든 항목을 로깅합니다</li><li><code>False</code></li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `settings` | <ul><li><code>wandb.Settings(...)</code>: 이 step 또는 플로우에 대해 사용할 사용자 정의 <code>wandb</code> 설정을 지정합니다</li><li><code>None</code>: <code>wandb.Settings()</code>를 전달하는 것과 동일합니다</li></ul><p>기본적으로 다음과 같이 동작합니다.</p><ul><li><code>settings.run&#95;group</code>가 <code>None</code>이면 <code>&#123;flow&#95;name&#125;/&#123;run&#95;id&#125;</code>로 설정됩니다</li><li><code>settings.run&#95;job&#95;type</code>이 <code>None</code>이면 <code>&#123;run&#95;job&#95;type&#125;/&#123;step&#95;name&#125;</code>으로 설정됩니다</li></ul> |
...
<div id="what-exactly-do-you-log-do-you-log-all-instance-and-local-variables">
  ### 정확히 무엇을 로깅하나요? 인스턴스 변수와 로컬 변수를 모두 로깅하나요?
</div>

`wandb_log`는 인스턴스 변수만 로깅합니다. 로컬 변수는 절대 로깅하지 않습니다. 이는 불필요한 데이터가 로깅되는 것을 방지하는 데 유용합니다.
...
네. 어떤 아티팩트가 step A의 출력이면서 step B의 입력인 경우, 이를 기반으로 리니지 DAG를 자동으로 구성합니다.
