...
W&B와 DSPy를 함께 사용하여 언어 모델 프로그램을 트래킹하고 최적화하세요. W&B는 다음과 같은 기능을 제공함으로써 [Weave DSPy integration](/ko/weave/guides/integrations/dspy)을 보완합니다:
...
1. `WANDB_API_KEY` [환경 변수](/ko/models/track/environment-variables/)를 설정하고 로그인합니다:
...
W&B가 처음이신가요? [퀵스타트 가이드](/ko/models/quickstart/)를 확인해 보세요.
...
DSPy를 사용한 Weave 트레이싱, 평가 및 최적화에 대한 자세한 내용은 [Weave DSPy integration 가이드](/ko/weave/guides/integrations/dspy)를 참조하세요.
...
자세한 내용은 [W&B Tables 가이드](/ko/models/tables/visualize-tables/) 및 [Tables 튜토리얼](/ko/models/tutorials/tables)에서 확인할 수 있습니다.
