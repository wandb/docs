...
description: "단일 CLI 파라미터로 Hugging Face AutoTrain에서 W&B 실험 추적을 사용해 코드 없이 모델을 트레이닝하세요."
---

[Hugging Face AutoTrain](https://huggingface.co/docs/autotrain/index)은 자연어 처리(NLP), 컴퓨터 비전(CV), 음성 작업은 물론 테이블형(Tabular) 작업을 위한 최첨단 모델을 코드 없이 트레이닝할 수 있는 노코드 도구입니다.

[W&amp;B](https://wandb.com/)는 Hugging Face AutoTrain에 직접 통합되어 실험 추적과 설정 관리 기능을 제공합니다. 실험에 사용할 CLI 명령에 파라미터 하나만 추가하면 될 정도로 간단합니다.
...
Hugging Face AutoTrain은 CSV 맞춤형 데이터셋이 제대로 동작하려면 특정 형식을 갖추고 있어야 합니다.

* 트레이닝 파일에는 반드시 트레이닝에 사용되는 `text` 열이 포함되어야 합니다. 가장 좋은 결과를 얻으려면 `text` 열의 데이터는 `### Human: Question?### Assistant: Answer.` 형식을 따라야 합니다. 훌륭한 예시는 [`timdettmers/openassistant-guanaco`](https://huggingface.co/datasets/timdettmers/openassistant-guanaco)에서 확인할 수 있습니다.
...
명령줄 또는 노트북에서 `autotrain` 고급 기능을 사용해 트레이닝을 시작할 수 있습니다. `--log` 인수를 사용하거나, 결과를 [W&amp;B Run](/ko/models/runs/)에 로깅하려면 `--log wandb`를 사용하세요.
