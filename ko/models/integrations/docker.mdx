...
W&B는 코드가 실행된 도커 이미지에 대한 포인터를 저장할 수 있으므로, 이전 실험이 실행되었던 정확한 환경으로 복원할 수 있습니다. wandb 라이브러리는 이 상태를 유지하기 위해 **WANDB_DOCKER** 환경 변수를 확인합니다. 이 상태를 자동으로 설정해 주는 몇 가지 헬퍼를 제공합니다.
