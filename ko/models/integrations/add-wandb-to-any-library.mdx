...
## 인테그레이션 설계 {#design-the-integration}
...
## W&amp;B 설치 방식 결정 {#decide-how-to-install-wb}

라이브러리와 함께 W&amp;B를 자동으로 설치할지, 선택 기능으로 제공할지 결정하세요.

### W&amp;B를 의존성으로 추가 {#require-wb-as-a-dependency}
...
### W&amp;B를 선택 종속성으로 지정 {#make-wb-an-optional-dependency}
...
### 명령줄에서 인증하기 {#authenticate-from-the-command-line}
...
### Python에서 인증 {#authenticate-from-python}
...
## W&amp;B 로깅을 선택적으로 사용 {#make-wb-logging-optional}
...
### `disabled` 모드 사용 {#use-disabled-mode}
...
### `offline` 모드 사용 {#use-offline-mode}
...
## 설정 및 메트릭 로깅 {#log-configuration-and-metrics}
...
### 설정 값 기록 {#log-configuration-values}
...
### 맞춤형 로깅 축 정의 {#define-a-custom-log-axis}
...
## 아티팩트를 사용해 모델과 데이터셋 추적하기 {#track-models-and-datasets-with-artifacts}
...
### 모델 체크포인트 로깅 {#log-model-checkpoints}
...
## 아티팩트를 W&amp;B 레지스트리에 연결하기 {#link-artifacts-to-the-wb-registry}
...
## 하이퍼파라미터 튜닝 {#tune-hyperparameters}
...
## 분산 트레이닝 지원 {#support-distributed-training}
