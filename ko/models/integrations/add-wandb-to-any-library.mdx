...
<Note>
W&B 사용법을 익히는 중이라면, 더 읽기 전에 [Experiment Tracking](/ko/models/track/)과 같은 이 문서의 다른 W&B 가이드를 먼저 살펴보는 것을 권장합니다.
</Note>
...
설정 파일 정의에 대한 자세한 정보는 [실험 설정하기](/ko/models/track/config/)를 참조하세요.

### W&B에 로그 남기기

#### 메트릭 로그

키 값이 메트릭의 이름인 사전(dictionary)을 만듭니다. 이 사전 오브젝트를 [`run.log`](/ko/models/)에 전달합니다:
...
[`wandb.Run.log()` 레퍼런스](/ko/models/)를 참조하세요.
...
미디어, 오브젝트, 플롯 등에 대해서는 [로그 가이드](/ko/models/)를 참조하세요.
...
자세한 내용은 [분산 트레이닝 실험 로그하기](/ko/models/track/log/distributed-training/)를 참조하세요.
...
커스텀 에일리어스를 생성하는 방법은 [커스텀 에일리어스 생성하기](/ko/models/artifacts/create-a-custom-alias/)를 참조하세요.
...
분산 환경이나 단순 추론 등을 위해 `wandb` run을 생성하지 않고(`wandb.init` 없이) 아티팩트를 다운로드하려면, [wandb API](/ko/models/ref/python/public-api/)를 사용하여 아티팩트를 참조할 수 있습니다:
...
라이브러리에서 W&B 하이퍼파라미터 튜닝을 활용하고 싶다면, [W&B Sweeps](/ko/models/sweeps/)를 라이브러리에 추가할 수 있습니다.
