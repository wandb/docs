...
그레이디언트를 자동으로 로그에 기록하려면, [`wandb.Run.watch()`](/ko/models/ref/python/experiments/run.md/#method-runwatch)를 호출하고 PyTorch 모델을 전달하면 됩니다.
...
동일한 스크립트에서 여러 모델을 추적해야 하는 경우, 각 모델에 대해 [`wandb.Run.watch()`](/ko/models/ref/python/experiments/run/#method-runwatch)를 별도로 호출할 수 있습니다.
...
이미지 데이터가 포함된 PyTorch `Tensors`를 [`wandb.Image`](/ko/models/ref/python/data-types/image)에 전달할 수 있으며, [`torchvision`](https://pytorch.org/vision/stable/index.html) 유틸리티가 이를 자동으로 이미지로 변환하는 데 사용됩니다.
...
PyTorch 및 기타 프레임워크에서 W&B 에 풍부한 미디어를 로그 기록하는 방법에 대한 자세한 내용은 [미디어 로그 기록 가이드](/ko/models/track/log/media/)를 확인하세요.
...
데이터셋과 모델을 로그 기록하고 시각화하는 방법에 대한 자세한 내용은 [W&B Tables 가이드](/ko/models/tables/)를 확인하세요.
...
W&B 는 [PyTorch Kineto](https://github.com/pytorch/kineto)의 [Tensorboard 플러그인](https://github.com/pytorch/kineto/blob/main/tb_plugin/README.md)과 직접 연동되어 PyTorch 코드를 프로파일링하고, CPU 와 GPU 통신의 세부 사항을 검사하며, 병목 현상 및 최적화 지점을 식별하는 툴을 제공합니다.
