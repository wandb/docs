...
이 페이지에서는 TensorBoard 로그를 W&amp;B와 동기화하여 TensorBoard 로그를 클라우드에 업로드하고, 동료 및 반 친구들과 결과를 공유하며, 분석을 한곳에 모아 관리하는 방법을 설명합니다. 이미 TensorBoard에 로그를 기록하고 있고 클라우드 호스팅 시각화, 공유, 그리고 W&amp;B 시스템 메트릭과의 나란히 비교가 필요하다면 이 인테그레이션을 사용하세요.
...
TensorBoard 동기화를 활성화하려면 W&amp;B run을 초기화할 때 `sync_tensorboard=True`를 설정하세요. W&amp;B는 트레이닝 코드에서 생성되는 모든 TensorBoard 이벤트를 자동으로 업로드합니다.
...
run이 완료되면 W&amp;B에서 TensorBoard 이벤트 파일에 액세스할 수 있으며 W&amp;B 기본 차트에서 메트릭을 시각화할 수 있습니다. 또한 W&amp;B는 시스템의 CPU 또는 GPU 사용량, `git` 상태, run에 사용된 터미널 명령 등 추가 정보도 캡처합니다.
...
다음 섹션에서는 추가 메트릭 로깅, 패치 구성, 과거 Runs 동기화, 노트북 환경 사용 등 TensorBoard 인테그레이션을 맞춤형으로 설정할 때 자주 묻는 질문에 답합니다.
...
W&amp;B의 TensorBoard 패치 방식을 더 세밀하게 제어하려면 `wandb.init()`에 `sync_tensorboard=True`를 전달하는 대신 `wandb.tensorboard.patch()`를 호출하세요.
...
# wandb run을 완료하여 tensorboard 로그를 W&B에 업로드합니다 (노트북에서 실행 중인 경우)
run.finish()
```

기본 TensorBoard를 패치하려면 이 방법에 `tensorboard_x=False`를 전달하세요. PyTorch와 함께 TensorBoard 1.14보다 높은 버전을 사용하는 경우에는 `pytorch=True`를 전달해 패치되도록 할 수 있습니다. 두 옵션 모두 임포트된 라이브러리 버전에 따라 적절한 기본값이 자동으로 적용됩니다.

기본적으로 W&amp;B는 `tfevents` 파일과 모든 `.pbtxt` 파일도 동기화합니다. 이를 통해 W&amp;B가 사용자를 대신해 TensorBoard 인스턴스를 실행할 수 있습니다. run 페이지에 [TensorBoard 탭](https://www.wandb.com/articles/hosted-tensorboard)이 표시됩니다. 이 동작은 `wandb.tensorboard.patch`에 `save=False`를 전달해 비활성화할 수 있습니다.
...
<Warning>
  `tf.summary.create_file_writer()`를 호출하거나 `torch.utils.tensorboard`에서 `SummaryWriter`를 생성하기 전에 반드시 `wandb.init()` 또는 `wandb.tensorboard.patch()` 중 하나를 호출해야 합니다.
</Warning>
...
`wandb` 0.20.0 이상에서는 S3, GCS 또는 Azure에 저장된 `tfevents` 파일의 동기화를 지원합니다. `wandb`는 각 클라우드 제공업체의 기본 자격 증명을 사용합니다. 아래 표에는 각 공급자의 자격 증명을 설정하는 명령과 예상되는 로깅 디렉터리 형식이 나와 있습니다.
