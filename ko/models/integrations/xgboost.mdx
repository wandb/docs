---
description: "W&B를 XGBoost와 통합해 그래디언트 부스팅 메트릭, feature importance, 모델 성능을 자동으로 로깅하세요."
title: XGBoost
---
...
`wandb` 라이브러리는 XGBoost로 트레이닝하는 동안 메트릭, 설정, 저장된 부스터를 로깅할 수 있는 `WandbCallback` 콜백을 제공합니다. 여기에서 XGBoost `WandbCallback`의 출력이 표시된 [실시간 W&amp;B Dashboard](https://wandb.ai/morg/credit_scorecard)를 확인할 수 있습니다.
...
XGBoost 메트릭, 설정 및 부스터 모델을 W&amp;B에 로깅하려면 XGBoost에 `WandbCallback`만 전달하면 됩니다:
...
XGBoost와 W&amp;B의 로깅을 자세히 살펴보려면 [이 노트북](https://wandb.me/xgboost)을 열어보세요
...
* `define_metric`: (boolean) True(기본값)이면 트레이닝의 마지막 step이 아니라 최적 step에서의 모델 성능을 `run.summary`에 기록합니다.

[WandbCallback의 소스 코드](https://github.com/wandb/wandb/blob/main/wandb/integration/xgboost/xgboost.py)를 확인할 수 있습니다.

추가 예시는 [GitHub 예제 저장소](https://github.com/wandb/examples/tree/master/examples/boosting-algorithms)를 참고하세요.
