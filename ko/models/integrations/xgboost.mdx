...
`wandb` 라이브러리는 XGBoost로 학습하는 동안 메트릭, 설정 및 저장된 부스터를 로깅하기 위한 `WandbCallback` 콜백을 제공합니다. 여기에서 XGBoost `WandbCallback`의 출력 결과를 보여주는 [실시간 W&amp;B Dashboard](https://wandb.ai/morg/credit_scorecard)를 확인할 수 있습니다.
