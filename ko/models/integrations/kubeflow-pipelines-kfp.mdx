---
description: "Kubeflow Pipelines와 W&B를 통합하여 ML 파이프라인 컴포넌트 전반에서 실험을 추적하고 메트릭을 시각화합니다."
title: Kubeflow Pipelines (kfp)
---

import ApiKeyCreateStreamlined from "/snippets/ko/_includes/api-key-create-streamlined.mdx";

[Kubeflow Pipelines (kfp) ](https://www.kubeflow.org/docs/components/pipelines/overview/)는 도커 컨테이너를 기준으로 이식성과 확장성이 뛰어난 머신 러닝(ML) 워크플로를 구축하고 배포하기 위한 플랫폼입니다.
...
`@wandb_log` 데코레이터를 추가한 뒤 평소처럼 컴포넌트를 생성하세요. 그러면 파이프라인을 실행할 때마다 입력/출력 파라미터와 아티팩트가 자동으로 W&amp;B에 로깅됩니다.
...
| ------------------ | --------------------- | -------------------------------------------- |
| Input Scalar       | [`config`](/ko/models/)  | [Overview tab](/ko/models/runs/#overview-tab)   |
| Output Scalar      | [`summary`](/ko/models/) | [Overview tab](/ko/models/runs/#overview-tab)   |
| Input Artifact     | 입력 아티팩트               | [Artifacts tab](/ko/models/runs/#artifacts-tab) |
| Output Artifact    | 출력 아티팩트               | [Artifacts tab](/ko/models/runs/#artifacts-tab) |
...
[지원하는 프레임워크 인테그레이션](/ko/models/integrations)을 사용하는 경우, 콜백을 직접 전달할 수도 있습니다:
