...
컨테이너에 [환경 변수](/ko/models/track/environment-variables/) 를 명시적으로 전달해야 할 수도 있습니다. 양방향 연결을 위해 `WANDB_KUBEFLOW_URL` 환경 변수를 Kubeflow Pipelines 인스턴스의 베이스 URL로 설정해야 합니다. 예를 들어, `https://kubeflow.mysite.com` 과 같습니다.
...
웹 앱 UI는 Kubeflow Pipelines의 `Visualizations` 탭과 동일한 콘텐츠를 제공하지만 더 넓은 공간을 활용할 수 있습니다. [여기에서 웹 앱 UI에 대해 자세히 알아보세요](/ko/models/app).
...
* 프로그래밍 방식의 엑세스는 [Public API 문서](/ko/models/ref/python/public-api/) 를 참조하세요.
...
[지원되는 프레임워크 인테그레이션](/ko/models/integrations) 을 사용하는 경우, 콜백을 직접 전달할 수도 있습니다.
