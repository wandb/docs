...
[Kubeflow Pipelines (kfp) ](https://www.kubeflow.org/docs/components/pipelines/overview/)는 Docker 컨테이너 기반으로 이식 가능하고 확장 가능한 머신러닝(ML) 워크플로를 구축하고 배포하기 위한 플랫폼입니다.

이 통합을 사용하면 kfp Python 함수형 컴포넌트에 데코레이터를 적용해 파라미터와 아티팩트를 W&amp;B에 자동으로 로그로 기록할 수 있습니다.
...
`@wandb_log` 데코레이터를 추가한 뒤 평소처럼 컴포넌트를 정의하세요. 이렇게 하면 파이프라인을 실행할 때마다 입력/출력 매개변수와 아티팩트가 자동으로 W&amp;B에 기록됩니다.
...
W&amp;B로 로깅된 Kubeflow Pipelines UI의 실행을 아무거나 클릭합니다.
...
웹 앱 UI에는 Kubeflow Pipelines의 `Visualizations` 탭과 동일한 내용이 더 넓은 화면 공간에 표시됩니다. [웹 앱 UI에 대한 자세한 내용은 여기에서 확인하세요](/ko/models/app).

<Frame>
  <img src="/images/integrations/kubeflow_pipelines.png" alt="실행 세부 정보" />
</Frame>
...
<div id="via-the-public-api-for-programmatic-access">
  ### Public API를 통한 액세스(프로그래밍 방식 액세스)
</div>

* 프로그래밍 방식으로 액세스하려면 [Public API 문서](/ko/models/ref/python/public-api/)를 참고하세요.

<div id="concept-mapping-from-kubeflow-pipelines-to-wb">
  ### Kubeflow Pipelines에서 W&amp;B로의 개념 매핑
</div>

다음은 Kubeflow Pipelines의 개념을 W&amp;B에 대응한 표입니다.
...
| Input Artifact | Input Artifact | [Artifacts 탭](/ko/models/runs/#artifacts-tab) |
| Output Artifact | Output Artifact | [Artifacts 탭](/ko/models/runs/#artifacts-tab) |

<div id="fine-grain-logging">
  ## 세밀한 로깅
</div>

로깅을 더 세밀하게 제어하려면 컴포넌트 내부에 `wandb.log` 및 `wandb.log_artifact` 호출을 적절한 위치에 넣어 주면 됩니다.

<div id="with-explicit-wandblog_artifacts-calls">
  ### 명시적인 `wandb.log_artifacts` 호출 사용
</div>

아래 예제에서는 모델을 학습합니다. `@wandb_log` 데코레이터는 관련된 입력과 출력을 자동으로 추적합니다. 학습 과정을 로깅하고 싶다면 다음과 같이 명시적으로 로깅을 추가할 수 있습니다:
...
[지원되는 프레임워크 통합](/ko/models/integrations)을 사용하고 있다면 콜백을 직접 전달할 수도 있습니다.
