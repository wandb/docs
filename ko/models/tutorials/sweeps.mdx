...
1. **스윕 정의:** 탐색할 파라미터, 탐색 전략, 최적화 메트릭 등을 명시하는 사전(dictionary) 또는 [YAML 파일](/ko/models/sweeps/define-sweep-configuration/)을 생성합니다.
...
커맨드라인에서 스윕을 실행하는 경우, [YAML 파일](/ko/models/sweeps/define-sweep-configuration/)로 스윕 구성을 지정해야 합니다.
</Note>

### 탐색 메소드 선택

먼저, 설정 사전 내에 하이퍼파라미터 탐색 메소드를 지정합니다. [그리드, 랜덤 검색, 베이지안 탐색의 세 가지 하이퍼파라미터 탐색 전략 중에서 선택할 수 있습니다.](/ko/models/sweeps/sweep-config-keys/#method)
...
전체 설정 옵션 목록은 [스윕 구성 옵션](/ko/models/sweeps/sweep-config-keys/)을 참조하세요.
...
터미널에서 W&B Sweeps를 생성하는 방법에 대한 자세한 내용은 [W&B Sweep 연습](/ko/models/sweeps/walkthrough/)을 참조하세요.
...
* [`wandb.init()`](/ko/models/ref/python/functions/init/): 새로운 W&B Run을 초기화합니다. 각 run은 트레이닝 함수의 단일 실행입니다.
* [`run.config`](/ko/models/track/config/): 실험하려는 하이퍼파라미터가 포함된 스윕 구성을 전달합니다.
* [`run.log()`](/ko/models/ref/python/experiments/run/#method-runlog): 각 에포크에 대한 트레이닝 손실(loss)을 로그로 기록합니다.
