...
W&B는 이미지, 비디오, 오디오 등을 지원합니다. 풍부한 미디어를 로그하여 결과를 탐색하고 run, 모델 및 데이터셋을 시각적으로 비교할 수 있습니다. 데이터 볼륨을 시각화하려면 [세그멘테이션 마스크 오버레이 시스템](/ko/models/track/log/media/#image-overlays-in-tables)을 사용하세요. [tables](/ko/models/tables/)에 세그멘테이션 마스크를 로그하려면 테이블의 각 행에 대해 `wandb.Image` 오브젝트를 제공해야 합니다.
