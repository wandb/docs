...
1. [run](/ko/models/runs/)을 초기화하고 트래킹하려는 하이퍼파라미터를 전달합니다.
...
W&B runs는 자동으로 [메트릭](/ko/models/runs/#workspace-tab), 시스템 정보, [하이퍼파라미터](/ko/models/runs/#overview-tab), [터미널 출력](/ko/models/runs/#logs-tab)을 로그하며, 모델 입력 및 출력값이 포함된 [대화형 테이블](/ko/models/tables/)을 볼 수 있습니다. 
...
W&B를 사용하면 저장된 모델과 모델 체크포인트를 팀 또는 조직의 다른 구성원과 쉽게 공유할 수 있습니다. 팀 외부 구성원과 모델 및 모델 체크포인트를 공유하는 방법은 [W&B Registry](/ko/models/registry/)를 참조하세요.
...
[W&B Alerts](/ko/models/runs/alert/)를 생성하여 Python 코드에서 Slack이나 이메일로 알림을 보낼 수 있습니다. 
