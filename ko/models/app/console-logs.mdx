---
title: 콘솔 로그
description: "W&B 실험 run의 정보, 경고, 오류를 포함한 콘솔 로그 메시지를 확인하고 디버깅합니다."
---

실험을 실행하면 콘솔에 다양한 메시지가 출력되는 것을 확인할 수 있습니다. W&amp;B는 콘솔 로그를 캡처하여 W&amp;B App에 표시합니다. 이러한 메시지를 사용하여 실험의 동작을 디버깅하고 모니터링하십시오.
...
W&amp;B는 여러 유형의 콘솔 로그를 캡처합니다. 정보 메시지, 경고, 오류가 있으며, 각 로그의 심각도를 나타내는 접두사가 함께 표시됩니다.
...
코드에서 `wandb.Settings` 객체를 `wandb.init()`에 전달하여 W&amp;B가 콘솔 로그를 처리하는 방식을 설정할 수 있습니다. `wandb.Settings`에서는 다음 파라미터를 설정하여 콘솔 로그 동작을 제어할 수 있습니다:
...
W&amp;B는 애플리케이션의 콘솔 로그를 캡처하지만, 자체 로깅 설정에는 영향을 주지 않습니다. Python에 내장된 `print()` 함수나 `logging` 모듈을 사용해 메시지를 기록할 수 있습니다.
...
콘솔 로그에서 타임스탬프를 표시/숨기기할 수 있습니다. 콘솔 페이지에서 왼쪽 상단에 있는 **Timestamp visible** 드롭다운을 선택합니다. 여기에서 타임스탬프를 표시하거나 숨길지를 선택할 수 있습니다.
...
<Warning>
  `x_label`과 같은 `x_` 접두사가 붙은 파라미터는 퍼블릭 프리뷰 기능입니다. 피드백을 제공하려면 [W&amp;B 저장소에 GitHub issue를 생성](https://github.com/wandb/wandb)하세요.
</Warning>

콘솔 로그 페이지 상단의 UI 검색창에서 `wandb.Settings`의 `x_label` 인수로 전달한 레이블을 기준으로 콘솔 로그를 필터링할 수 있습니다.
...
with wandb.init(
    ...
    project="project",
    settings=wandb.Settings(
        x_label="custom_label"  # (선택 사항) 로그 필터링을 위한 맞춤형 레이블
    )
) as run:
    # 여기에 코드 작성
```
...
W&amp;B App에서 run의 콘솔 로그를 복사하려면 다음 단계를 따르세요:
