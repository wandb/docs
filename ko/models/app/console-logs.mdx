...
## 콘솔 로그 보기 {#view-console-logs}

run 중에 생성된 메시지를 확인하려면 W&amp;B App에서 run의 콘솔 로그를 봅니다.

1. W&amp;B App에서 프로젝트로 이동합니다.
2. **Runs** 테이블에서 run을 선택합니다.
3. run 페이지에서 **Logs** 탭을 선택합니다.
4. 선택 사항: 현재 스트리밍 중인 run에서 위로 스크롤한 후 최신 로그로 돌아가려면 **Jump to bottom**을 선택합니다.

로그가 스트리밍되는 동안 오른쪽 상단에 줄임표 아이콘 (<Icon icon="ellipsis" iconType="solid" />)이 표시됩니다. 로그 폴링이 끝나면 W&amp;B는 이 아이콘을 다운로드 버튼 (<Icon icon="arrow-down-to-bracket" iconType="solid" />)으로 바꿉니다.

{/* ## 콘솔 로그 필드

  W&B App에서 콘솔 로그 테이블에 표시되는 필드를 수정할 수 있습니다. */}

## 콘솔 로그 유형 {#types-of-console-logs}
...
### 멀티파트 콘솔 로깅 {#multipart-console-logging}
...
## 콘솔 로그 검색 {#search-console-logs}

콘솔 로그 페이지의 검색창을 사용해 라벨, 키워드 또는 오류 메시지와 같은 특정 텍스트를 찾으세요.

새 로그가 스트리밍되면 W&amp;B가 일치 항목 수를 업데이트합니다.

## 맞춤형 라벨로 필터링 {#filter-with-custom-labels}
...
## 콘솔 로그 다운로드 {#download-console-logs}
...
4. 콘솔 로그 페이지에서 로그 파일 다운로드 버튼(<Icon icon="arrow-down-to-bracket" iconType="solid" />)을 클릭합니다.

run이 콘솔 로그를 활발히 스트리밍하는 동안에는 일시적으로 로그를 다운로드하지 못할 수 있습니다. 스트리밍 중에는 W&amp;B가 다운로드 버튼을 줄임표 아이콘(<Icon icon="ellipsis" iconType="solid" />)으로 대체합니다. run이 종료되거나 스트리밍을 중지할 때까지 기다린 후 다시 시도합니다.

## 콘솔 로그 복사 {#copy-console-logs}
