...
먼저, 스크립트에서 데이터를 로그합니다. 하이퍼파라미터와 같이 트레이닝 시작 시점에 설정되는 단일 포인트의 경우 [wandb.Run.config](/ko/models/track/config/)를 사용하세요. 시간에 따른 여러 포인트의 경우 [wandb.Run.log()](/ko/models/track/log/)를 사용하고, 커스텀 2D 어레이는 `wandb.Table()`로 로그합니다. 로그되는 키 하나당 최대 10,000개의 데이터 포인트를 로그하는 것을 권장합니다.
