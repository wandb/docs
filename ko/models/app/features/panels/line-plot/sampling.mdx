---
title: 포인트 집계
description: "W&B 라인 플롯의 두 가지 포인트 집계 모드인 full fidelity 버킷 샘플링과 랜덤 샘플링을 이해하세요."
---

라인 플롯에서 포인트 집계 방법을 사용하여 데이터 시각화의 정확도와 성능을 향상하세요. 포인트 집계 모드에는 [full fidelity](#full-fidelity)와 [random sampling](#random-sampling)의 두 가지 유형이 있습니다. 기본적으로 W&amp;B는 full fidelity 모드를 사용합니다.

<div id="full-fidelity">
  ## Full fidelity
</div>

full fidelity 모드를 사용하면 W&amp;B는 x축을 동적 버킷으로 나눕니다. 각 선의 포인트 수는 차트 크기와 run 수에 따라 조정됩니다. 각 버킷 내의 최소값과 최대값을 계산하고(선택적 음영 처리에 사용됨), 각 버킷의 마지막 값(평균이 아님)을 사용해 기본 선을 그립니다.

포인트 집계를 위해 full fidelity 모드를 사용할 때의 주요 장점은 다음 세 가지입니다:

* 극단값과 스파이크 보존: 데이터의 극단값과 스파이크를 유지합니다.
* 최소값 및 최대값 포인트가 렌더링되는 방식 설정: W&amp;B App을 사용해 극단값(최소/최대값)을 음영 영역으로 표시할지 여부를 대화형으로 설정할 수 있습니다.
* 데이터 충실도를 잃지 않고 데이터 탐색: 특정 데이터 포인트로 확대하면 W&amp;B가 x축 버킷 크기를 재계산합니다. 이를 통해 정확도를 잃지 않고 데이터를 탐색할 수 있습니다. 이전에 계산된 집계를 저장하기 위해 캐싱을 사용하여 로딩 시간을 줄이며, 특히 대규모 데이터셋을 탐색할 때 유용합니다.
...
극단값이나 스파이크와 같은 중요한 지점을 놓치지 않고 데이터셋의 특정 영역을 분석할 수 있습니다. 선형 플롯에서 확대(zoom-in)하면, W&amp;B는 각 버킷 내 최소값, 최대값, 마지막 값을 계산하는 데 사용되는 버킷 크기를 자동으로 조정합니다.

<Frame>
  <img src="/images/app_ui/zoom_in.gif" alt="Plot zoom 기능" />
</Frame>

W&amp;B는 동적 비닝을 사용해 x축을 버킷으로 나눕니다. 각 선에 표시되는 포인트 수는 차트 크기와 run 수에 따라 조정됩니다. 차트 크기가 더 작거나 run 수가 더 많으면 차트의 응답성을 유지하고 더 많은 선을 표시할 수 있도록 선당 포인트 수가 줄어들 수 있습니다. 각 버킷에 대해 W&amp;B는 다음 값을 계산합니다:

* **Minimum**: 해당 버킷에서 가장 낮은 값(음영 처리에 사용됨)
* **Maximum**: 해당 버킷에서 가장 높은 값(음영 처리에 사용됨)
* **Line value**: 해당 버킷의 마지막 값으로, 선을 그리는 데 사용됨

W&amp;B는 값을 버킷 단위로 플로팅하여 데이터를 온전히 표현하면서, 모든 플롯에 극단값이 포함되도록 합니다. 선은 각 버킷의 마지막 값부터 그려집니다. 충분히 확대하면, Full fidelity 모드에서 추가 집계 없이 모든 데이터 포인트를 그대로 렌더링할 수 있으며, 정확한 기준값은 현재 차트 크기와 run 수에 따라 달라집니다.
...
랜덤 샘플링은 라인 플롯을 렌더링하기 위해 무작위로 선택된 1,500개의 데이터 포인트를 사용합니다. 샘플링 과정에서 일부 데이터 포인트가 제외되므로 이상치나 스파이크를 발견하기가 더 어려워집니다.
