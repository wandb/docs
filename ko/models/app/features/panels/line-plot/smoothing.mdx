...
[대화형 W&amp;B 리포트](https://wandb.ai/carey/smoothing-example/reports/W-B-Smoothing-Features--Vmlldzo1MzY3OTc)에서 이 스무딩 기능들을 실제로 확인해 보세요.
...
시간 가중 지수 이동 평균(Time Weighted Exponential Moving Average, TWEMA) 스무딩 알고리즘은 이전 지점의 가중치를 지수적으로 감소시키는 방식으로 시계열 데이터를 스무딩하는 기법입니다. 이 기법에 대한 자세한 내용은 [Exponential Smoothing](https://www.wikiwand.com/en/Exponential_smoothing)을 참조하세요. 범위는 0에서 1 사이입니다. 시계열의 초기 값들이 0으로 편향되지 않도록 de-bias 항이 추가됩니다.
...
가우시안 스무딩(또는 가우시안 커널 스무딩)은 각 포인트에, 스무딩 파라미터로 지정한 표준편차를 갖는 가우시안 분포에 해당하는 가중치를 적용해 가중 평균을 계산합니다. 스무딩된 값은 각 입력 x 값마다 해당 값의 이전과 이후에 있는 포인트들을 기준으로 계산됩니다.

다음은 [앱에서 표시되는 모습](https://wandb.ai/carey/smoothing-example/reports/W-B-Smoothing-Features--Vmlldzo1MzY3OTc#3.-gaussian-smoothing)입니다:
...
러닝 평균은 주어진 x 값의 앞뒤 구간에 있는 데이터 포인트들의 평균값으로 각 포인트를 대체하는 스무딩 알고리즘입니다. 자세한 내용은 [위키백과의 &quot;Boxcar Filter&quot;](https://en.wikipedia.org/wiki/Moving_average)를 참조하세요. 러닝 평균에 대해 선택한 파라미터는 이동 평균을 계산할 때 Weights and Biases가 고려할 포인트 개수를 지정합니다.

포인트들이 x-axis 상에서 고르게 간격을 두고 배치되어 있지 않다면, 대신 가우시안 스무딩(Gaussian Smoothing)을 사용하는 것이 좋습니다.
...
지수 이동 평균 (EMA) 스무딩 알고리즘은 지수 창 함수를 사용하여 시계열 데이터를 스무딩하는 경험칙적 기법입니다. 기법에 대한 자세한 내용은 [Exponential Smoothing](https://www.wikiwand.com/en/Exponential_smoothing)을 참조하세요. 범위는 0에서 1까지입니다. 시계열의 초기 값들이 0으로 편향되지 않도록 편향 보정 항(debias term)이 추가됩니다.

많은 경우 EMA 스무딩는 버킷팅을 먼저 한 후 스무딩하는 대신 이력 전체를 스캔하여 적용됩니다. 이렇게 하면 더 정확한 스무딩이 이루어지는 경우가 많습니다.

다음과 같은 상황에서는 EMA 스무딩가 버킷팅 이후에 적용됩니다.
...
다음은 내부적으로 어떻게 작동하는지 보여주는 예시 코드입니다.
