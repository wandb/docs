...
<Tabs>
  ...
  <Tab title="여러 메트릭 선형 플롯">
    ...
    4. **Regex**에 [Google&#39;s RE2](https://github.com/google/re2/wiki/Syntax) 형식의 표현식을 입력합니다. 입력하는 동안 UI가 업데이트되어 어떤 메트릭이 표현식과 일치하는지 보여줍니다. 기본적으로 플롯 이름에는 해당 플롯에서 사용하는 정규 표현식이 표시됩니다. 이 플롯에는 앞으로 로깅될 메트릭을 포함해, 표현식과 일치하는 모든 메트릭의 선이 포함됩니다.
...
### 다중 메트릭 정규 표현식 {#multi-metric-regular-expressions}

    다중 메트릭 선형 플롯은 메트릭 이름과 일치시키기 위해 [Google&#39;s RE2](https://github.com/google/re2/wiki/Syntax) 구문을 사용합니다. 다음 섹션에서는 일반적인 사용 사례를 설명하고, 캡처 그룹이 W&amp;B가 생성하는 패널에 어떤 영향을 미치는지 등 정규 표현식이 작동하는 방식에 대한 자세한 내용을 제공합니다.
...
## 선형 플롯 설정 편집 {#edit-line-plot-settings}
...
### 섹션의 모든 선형 플롯 {#all-line-plots-in-a-section}
...
### 워크스페이스의 모든 선형 플롯 {#all-line-plots-in-a-workspace}
...
## 플롯에서 평균값 시각화하기 {#visualize-average-values-on-a-plot}
...
## 플롯에서 NaN 값 시각화하기 {#visualize-nan-value-on-a-plot}
...
## 하나의 차트에서 여러 메트릭 비교하기 {#compare-multiple-metrics-on-one-chart}
...
## 선 색상 변경 {#change-the-colors-of-the-lines}
...
## 서로 다른 x축으로 시각화하기 {#visualize-on-different-x-axes}
...
## run 메트릭 알림 만들기 {#create-a-run-metrics-notification}
