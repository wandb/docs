...
## 코드 저장 활성화 {#enable-code-saving}

팀 또는 조직의 코드 저장을 설정하세요. 팀 및 조직 제어에 대해서는 [개인정보 보호 설정 구성](/ko/platform/hosting/privacy-settings)에 설명되어 있습니다.

<Info>
  기본적으로 W&amp;B는 모든 팀에서 코드 저장을 비활성화합니다. 팀에서 이 기능을 켜려면 먼저 조직 관리자가 조직의 **기본 코드 저장 비활성화** 설정을 꺼진 상태로 두어야 합니다. [조직](#organization) 섹션을 참조하세요.
</Info>

### 팀 {#team}

팀 관리자로서 팀의 코드 저장을 활성화하려면 팀 **Settings** 페이지를 열고 **Privacy** 섹션으로 이동한 다음, 해당 팀의 run에 대해 **Enable code saving by default**를 설정하세요. 이 옵션은 조직 관리자가 조직 전체에 대해 **기본 코드 저장 비활성화**를 켜지 않은 경우에만 사용 가능합니다. 내비게이션 단계는 [팀의 개인정보 보호 설정 구성](/ko/platform/hosting/privacy-settings#configure-privacy-settings-for-a-team)을 참조하세요.

### 조직 {#organization}

조직 관리자는 **Settings**로 이동하여 **Privacy**를 선택한 다음, **Project and content security** 섹션의 **기본 코드 저장 비활성화**을 사용해 모든 팀에서 코드 저장을 기본적으로 끌지 여부를 제어하세요. 이 설정이 켜져 있는 동안에는 팀 관리자가 자신의 팀에 대해 **Enable code saving by default**를 켤 수 없습니다. 조직 제어 기능의 전체 목록은 [모든 팀에 개인정보 보호 설정 강제 적용](/ko/platform/hosting/privacy-settings#enforce-privacy-settings-for-all-teams)을 참조하세요.
...
### 코드 자동 캡처 {#automatically-capture-code}
...
## 로컬 변경 사항 보기 {#view-local-changes}
...
## Jupyter 세션 이력 {#jupyter-session-history}
...
## run 간 코드 비교 {#compare-code-across-runs}
