...
with wandb.init(project=PROJECT, entity=TEAM_ENTITY, config=config) as run:    
    ...
    artifact = wandb.Artifact(name=model_artifact_name, type="model", description="내 트레이닝된 모델")
...
* 아티팩트를 run의 입력으로 표시해 재현성과 추적 가능성을 보장합니다. 자세한 내용은 [Create and view lineage map](/ko/models/registry/lineage)을 참조하세요.
...
```python
# Artifact 이름은 팀 프로젝트 내의 특정 아티팩트 버전을 지정합니다
...
experiment_summary = """W&B를 사용하여 단순한 모델을 트레이닝하기 위해 수행된 실험의 요약입니다."""
dataset_info = """트레이닝에 사용된 데이터셋은 단순한 모델이 생성한 합성 데이터로 구성됩니다."""
model_info = """이 모델은 약간의 노이즈가 포함된 입력 데이터를 기반으로 출력을 예측하는 단순한 선형 회귀 모델입니다."""

report = wr.Report(
    project=PROJECT,
    entity=TEAM_ENTITY,
    title="나의 멋진 모델 트레이닝 리포트",
    ...
    blocks= [
        ...
        wr.PanelGrid(
            panels=[
                wr.LinePlot(title="트레이닝 Loss", x="Step", y=["loss"], title_x="Step", title_y="Loss")
                ],
            ),  
    ]
...
# 찾은 각 아티팩트의 이름, 컬렉션, 별칭, 태그, 생성일 출력
