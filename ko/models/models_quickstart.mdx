...
## 패키지 설치 및 임포트 {#install-and-import-packages}
...
이제 모델을 트레이닝하고 아티팩트로 저장했으므로, W&amp;B의 레지스트리에 게시할 수 있습니다. 프로젝트에서 아티팩트를 조회하고 모델 레지스트리에 게시할 준비를 하려면 [`wandb.Run.use_artifact()`](/ko/models/ref/python/experiments/run/#method-runuse_artifact)를 사용합니다. `wandb.Run.use_artifact()`는 두 가지 핵심 용도로 사용됩니다.
...
## 대시보드에서 트레이닝 데이터 확인하기 {#view-the-training-data-in-the-dashboard}
...
## 모델을 W&amp;B 레지스트리에 게시하기 {#publish-the-model-to-the-wb-registry}
...
## Inference용 모델 아티팩트를 레지스트리에서 조회 {#retrieve-model-artifact-from-registry-for-inference}
...
## 발견한 내용을 리포트로 공유하기 {#share-your-findings-with-a-report}
