...
# 현재 스크립트 또는 노트북에서 run이 시작된 경우 Run 객체에서 다음 값을 확인하거나,
# W&B App UI에서 복사할 수 있습니다.
username = run.entity
project = run.project
run_id = run.id

# 참고: api.run()은 wandb.init()과 다른 유형의 객체를 반환합니다.
api = wandb.Api()
...
with wandb.init() as run:
    ...
    run.config.update(flags.FLAGS)  # tensorflow 플래그를 설정으로 추가
```
