...
2. **코드 저장**: 모델을 재현할 때 노트북에서 어떤 셀이 어떤 순서로 실행되었는지 알기 어렵습니다. [설정 페이지](/ko/platform/app/settings-page/)에서 코드 저장을 켜면 각 experiment에 대한 셀 실행 기록이 저장됩니다.
...
<Note>
W&B로 무엇을 할 수 있는지 더 자세히 알고 싶으신가요? [데이터 및 미디어 로깅 가이드](/ko/models/track/log/)를 확인하거나, [자주 사용하는 ML 툴킷과 통합하는 방법](/ko/guides/integrations/)을 배우거나, [참조 문서](/ko/models/ref/python/) 또는 [예제 리포지토리](https://github.com/wandb/examples)로 바로 이동하세요.
</Note>
