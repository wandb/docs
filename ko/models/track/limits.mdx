...
{/* 각 step에 대해 수행되는 전체 API 호출 수를 최소화하려면 `commit=False`를 전달하여 `run.log` 호출에서 배치 처리를 활성화하세요. 자세한 내용은 `run.log`에 대한 [문서](/ko/models/ref/python/experiments/run/#method-runlog)를 참조하세요. */}
