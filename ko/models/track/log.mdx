---
description: 메트릭, 비디오, 맞춤형 플롯 등을 확인하세요
title: 개요
---
...
기본적으로 `wandb.Run.log()`를 호출할 때마다 새로운 `step`이 생성됩니다. W&amp;B는 차트와 패널을 만들 때 step을 기본 x-axis로 사용합니다. 필요에 따라 맞춤형 x-axis를 만들어 사용할 수도 있고, 맞춤형 summary 메트릭을 캡처할 수도 있습니다. 자세한 내용은 [로그 축 사용자 지정](/ko/models/track/log/customize-logging-axes/)을 참조하세요.
...
{/* `wandb.Run.log()`에서 `commit=False`를 설정하면 메트릭을 누적할 수 있습니다. 단, 메트릭이 저장되도록 하려면 마지막에는 `commit=True`(기본값)로 `run.log()`를 호출해야 합니다.
...
## 자동으로 로깅되는 데이터 {#automatically-logged-data}
...
## 특정 W&amp;B API calls에서 어떤 데이터가 로깅되나요? {#what-data-is-logged-with-specific-wb-api-calls}
...
## 메트릭 이름 지정 제약 사항 {#metric-naming-constraints}
