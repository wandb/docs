---
description: 메트릭, 비디오, 맞춤형 플롯 등을 추적
title: Overview
---
...
W&amp;B Python SDK를 사용해 메트릭, 미디어 또는 맞춤형 객체로 이루어진 딕셔너리를 step에 로깅합니다. W&amp;B는 각 step에서 키-값 쌍을 수집하고, `wandb.Run.log()`로 데이터를 로깅할 때마다 이를 하나의 통합된 딕셔너리에 저장합니다. 스크립트에서 로깅한 데이터는 먼저 로컬 머신의 `wandb` 디렉터리에 저장된 다음, W&amp;B 클라우드 또는 [프라이빗 서버](/ko/platform/hosting/)에 동기화됩니다.

<Note>
  키-값 쌍은 각 step에 동일한 값을 전달한 경우에만 하나의 통합된 딕셔너리에 저장됩니다. `step`에 서로 다른 값을 로깅하면 W&amp;B는 수집된 모든 키와 값을 메모리에 저장합니다.
</Note>

기본적으로 `wandb.Run.log()`를 call할 때마다 새로운 `step`이 생성됩니다. W&amp;B는 차트와 패널을 만들 때 step을 기본 x축으로 사용합니다. 필요에 따라 맞춤형 x축을 만들어 사용할 수도 있고, 맞춤형 summary 메트릭을 캡처할 수도 있습니다. 자세한 내용은 [로그 축 사용자 지정](/ko/models/track/log/customize-logging-axes/)을 참조하세요.

{/* [더 나은 예시 추가] */}

{/* 코드의 여러 곳에서 하나의 이력 step에 기록하려면, 다음과 같이 `run.log()`에 step 인덱스를 전달할 수 있습니다:
...
<Warning>
  각 `step`(0, 1, 2 등)에 대한 연속된 값을 기록하려면 `wandb.Run.log()`를 사용하세요. 특정 이력 step에 기록할 수는 없습니다. W&amp;B는 &quot;현재&quot; step과 &quot;다음&quot; step에만 기록합니다.
</Warning>

{/* `run.log`에서 `commit=False`를 설정하면 메트릭을 누적할 수 있습니다. 다만 메트릭을 저장하려면 마지막에는 `commit=True`(기본값)로 `run.log`를 호출해야 합니다.
...
3. **x-axis 사용자 지정**: 동일한 log call에 맞춤형 x-axis를 추가해 W&amp;B 대시보드에서 다른 축을 기준으로 메트릭을 시각화하세요. 예:
   ...
   ```
   특정 메트릭의 기본 x-axis를 설정하려면 [Run.define&#95;metric()](/ko/models/ref/python/experiments/run#define_metric)을 사용하세요.
