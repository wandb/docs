---
description: W&B에 저장한 데이터를 내보내거나 업데이트합니다
title: 데이터 내보내기 및 업데이트
---
...
### 여러 run 쿼리하기 {#querying-multiple-runs}
...
### 활성 run 중에 run 이름과 ID 조회하기 {#get-a-runs-name-and-id-during-an-active-run}
...
## Public API 예시 {#public-api-examples}

### run에서 메트릭 조회 {#read-metrics-from-a-run}
...
### run에서 특정 메트릭 조회 {#read-specific-metrics-from-a-run}
...
### run이 완료된 후 해당 run의 메트릭 업데이트 {#update-metrics-for-a-run-after-the-run-has-finished}

이 예제에서는 이전 run의 정확도를 `0.9`로 설정합니다:
...
### 완료된 run의 메트릭 이름 바꾸기 {#rename-a-metric-in-a-completed-run}
...
### 기존 run의 설정 업데이트 {#update-config-for-an-existing-run}
...
자세한 내용은 [실험 구성](/ko/models/track/config#existing-run)을 참조하세요.

### 시스템 리소스 사용량을 CSV 파일로 내보내기 {#export-system-resource-consumptions-to-a-csv-file}
...
### 샘플링되지 않은 메트릭 데이터 조회 {#get-unsampled-metric-data}
...
### 이력에서 페이지 단위로 데이터 조회 {#get-paginated-data-from-history}

run의 전체 샘플링되지 않은 이력을 조회하려면 `run.scan_history()`를 사용하세요. 샘플링되지 않은 이력에는 해당 run의 모든 이력 레코드가 포함됩니다. 반면 `run.history()`의 샘플링된 뷰에서는 일부 개별 레코드가 누락될 수 있습니다.

`page_size` 매개변수는 API 요청당 가져오는 이력 레코드의 최대 개수를 제어하며, 기본값은 `1000`입니다. 요청이 느리거나 시간 초과가 발생하면 더 작은 페이지 크기를 사용해 보세요. 페이지 크기를 줄이면 시간 초과 위험을 낮출 수 있지만, 더 많은 API 요청이 필요합니다.
...
### 프로젝트의 모든 run에서 메트릭을 CSV 파일로 내보내기 {#export-metrics-from-all-runs-in-a-project-to-a-csv-file}
...
### run의 시작 시간 조회 {#get-the-starting-time-for-a-run}
...
### 완료된 run에 파일 업로드 {#upload-files-to-a-finished-run}
...
### run에서 파일 다운로드 {#download-a-file-from-a-run}
...
### run의 모든 파일 다운로드 {#download-all-files-from-a-run}
...
### 특정 sweep의 run 조회 {#get-runs-from-a-specific-sweep}
...
### sweep에서 최적의 run 조회 {#get-the-best-run-from-a-sweep}
...
### sweep에서 가장 성능이 좋은 모델 파일 다운로드 {#download-the-best-model-file-from-a-sweep}
...
### run에서 지정한 확장자의 모든 파일 삭제 {#delete-all-files-with-a-given-extension-from-a-run}
...
### 시스템 메트릭 데이터 다운로드 {#download-system-metrics-data}
...
### summary 메트릭 업데이트 {#update-summary-metrics}
...
### run을 실행한 명령어 조회 {#get-the-command-that-ran-the-run}
...
{/* ### 샘플링
  ...
  기본 이력 방법은 메트릭을 고정된 수의 샘플로 샘플링합니다(기본값은 500이며, `samples` 인수로 변경할 수 있습니다). 대규모 run의 모든 데이터를 내보내려면 `run.scan_history()` 방법을 사용할 수 있습니다. 자세한 내용은 [API Reference](/models/ref/python/public-api)를 참조하세요.  */}