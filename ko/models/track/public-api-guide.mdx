...
Public API를 사용하여 W&B에 저장한 데이터를 내보내거나 업데이트합니다. 이 API를 사용하기 전에 스크립트에서 데이터를 기록하십시오. 자세한 내용은 [퀵스타트](/ko/models/quickstart/)를 확인하세요.
...
사용 가능한 함수에 대한 자세한 내용은 [생성된 참조 문서](/ko/models/ref/python/public-api/)를 참조하십시오.
...
Public API를 사용하려면 `<entity>/<project>/<run_id>`인 Run 경로가 필요한 경우가 많습니다. 앱 UI에서 Run 페이지를 열고 [Overview 탭](/ko/models/runs/#overview-tab)을 클릭하여 Run 경로를 가져옵니다.
...
기본 히스토리 메서드는 메트릭을 고정된 수의 샘플로 샘플링합니다(기본값은 500이며, `samples` __ 인수로 변경할 수 있음). 대규모 Run에서 모든 데이터를 내보내려면 `run.scan_history()` 메서드를 사용하면 됩니다. 자세한 내용은 [API 참조](/ko/models/ref/python/public-api)를 참조하십시오.
...
- **Run 태그**: Run 태그에서 동적으로 추적하고 UI에서 필터를 사용하여 테이블을 원하는 Runs로 필터링합니다. 스크립트에서 태그를 설정한 다음 Runs 테이블과 Run 페이지의 Overview 탭 모두에서 UI에서 편집할 수 있습니다. 자세한 내용은 [여기](/ko/models/runs/tags/)의 자세한 지침을 참조하십시오.

## Public API 예제

### matplotlib 또는 seaborn에서 시각화하기 위해 데이터 내보내기

몇 가지 일반적인 내보내기 패턴은 [API 예제](/ko/models/ref/python/public-api/)를 확인하십시오. 사용자 정의 플롯 또는 확장된 Runs 테이블에서 다운로드 버튼을 클릭하여 브라우저에서 CSV를 다운로드할 수도 있습니다.
