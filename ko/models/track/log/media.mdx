...
각 단계에서 이미지를 로깅할 때마다 UI에서 사용할 수 있습니다. 이미지 패널을 펼친 다음 단계 슬라이더를 사용해 서로 다른 단계의 이미지를 확인하세요. 이를 통해 트레이닝 동안 모델 출력이 어떻게 변하는지 쉽게 비교할 수 있습니다.

서로 다른 Runs, 단계 또는 인덱스의 이미지나 비디오를 하나의 뷰에서 비교하려면, 미디어 패널의 [Compare mode](/ko/models/app/features/panels/media#compare-mode)를 사용하세요.
