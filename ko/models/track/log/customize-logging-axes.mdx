...
특정 상황에서는 로그 스케일 x-축과 같이 다른 x-축을 기준으로 메트릭을 로그하는 것이 더 합리적일 수 있습니다. [`define_metric()`](/ko/models/ref/python/experiments/run/#define_metric) 메소드를 사용하여 로그하는 모든 메트릭을 커스텀 x-축으로 사용할 수 있습니다.
