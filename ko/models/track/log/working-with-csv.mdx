...
W&B Python 라이브러리를 사용하여 CSV 파일을 로그하고 [W&B Dashboard](/models/track/workspaces/)에서 시각화하세요. W&B Dashboard는 기계학습 모델의 결과를 정리하고 시각화하는 중심 공간입니다. 이는 W&B에 로그되지 않은 [이전 기계학습 실험 정보가 포함된 CSV 파일](#실험-csv-파일-가져오기-및-로그)이 있거나 [데이터셋이 포함된 CSV 파일](#데이터셋-csv-파일-가져오기-및-로그)을 가지고 있는 경우 특히 유용합니다.
