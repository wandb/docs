...
W&B Python 라이브러리를 사용하여 CSV 파일을 로그하고 [W&B Dashboard](/ko/models/track/workspaces/)에서 시각화하세요. W&B Dashboard는 기계학습 모델의 결과를 정리하고 시각화하는 중심 공간입니다. 이는 W&B에 로그되지 않은 [이전 기계학습 실험 정보가 포함된 CSV 파일](#실험-csv-파일-가져오기-및-로그)이 있거나 [데이터셋이 포함된 CSV 파일](#데이터셋-csv-파일-가져오기-및-로그)을 가지고 있는 경우 특히 유용합니다.
...
2. [W&B Dashboards](/ko/models/track/workspaces/)를 활용하기 위해 CSV 파일을 W&B Table로 변환합니다. 
...
```
W&B Artifacts에 대한 자세한 정보는 [Artifacts 챕터](/ko/models/artifacts/)를 참조하세요.  
...
* 실험에 필요한 설정 (당사의 [Sweeps 하이퍼파라미터 튜닝](/ko/models/sweeps/)을 활용할 수 있는 추가적인 이점 포함)
...
2. 다음으로, [`wandb.init()`](/ko/models/ref/python/functions/init)을 사용하여 W&B에 트래킹 및 로그를 위한 새로운 W&B Run을 시작합니다.
...
실험이 진행됨에 따라, W&B에서 확인, 쿼리 및 분석할 수 있도록 메트릭의 모든 인스턴스를 로그하고 싶을 수 있습니다. 이를 위해 [`run.log()`](/ko/models/ref/python/experiments/run/#method-runlog) 코맨드를 사용하세요.

```python
run.log({key: val})
```

선택적으로 [`define_metric`](/ko/models/ref/python/experiments/run#define_metric) API를 사용하여 실행 결과를 정의하는 최종 요약 메트릭을 로그할 수 있습니다. 이 예시에서는 `run.summary.update()`를 사용하여 요약 메트릭을 run에 추가합니다.
