...
rank 0 프로세스 내에서 [`wandb.init()`](/ko/models/ref/python/functions/init)으로 W&B run을 초기화하고 해당 run에 실험 내용을 로그([`wandb.log`](/ko/models/ref/python/experiments/run/#method-runlog))합니다.
...
* 각 프로세스에 대한 run을 생성하여 [각 프로세스를 개별적으로 추적](/ko/models/track/log/distributed-training/#track-each-process-separately).
* [모든 프로세스를 단일 run으로 추적](/ko/models/track/log/distributed-training/#track-all-processes-to-a-single-run).
...
여러 실험에 걸쳐 있는 run들을 관리하기 어려울 수 있습니다. 이를 완화하기 위해 W&B를 초기화할 때 `group` 파라미터에 값(`wandb.init(group='group-name')`)을 제공하여 해당 run이 어떤 실험에 속하는지 추적하세요. 실험에서 트레이닝 및 평가 W&B Runs를 추적하는 방법에 대한 자세한 내용은 [Group Runs](/ko/models/runs/grouping/)를 참조하세요.
...
노드에 `job_type`을 설정한 후에는 Workspace에서 [저장된 뷰(saved views)](/ko/models/track/workspaces/#create-a-new-saved-workspace-view)를 생성하여 run을 정리할 수 있습니다. 우측 상단의 **...** 액션 메뉴를 클릭하고 **Save as new view**를 클릭합니다.
...
프라이머리 노드 내에서 [`wandb.init()`](/ko/models/ref/python/functions/init)으로 W&B run을 초기화합니다. `settings` 파라미터에 다음과 같은 내용이 포함된 `wandb.Settings` 오브젝트를 전달합니다(`wandb.init(settings=wandb.Settings())`):
...
각 워커 노드에서 [`wandb.init()`](/ko/models/ref/python/functions/init)으로 W&B run을 초기화하고 다음을 제공합니다:
...
3. 선택적으로 [`x_update_finish_state`](https://github.com/wandb/wandb/blob/main/wandb/sdk/wandb_settings.py#L772)를 `False`로 설정합니다. 이는 프라이머리 노드가 아닌 노드가 [run의 상태](/ko/models/evaluate-models/#run-states)를 조기에 `finished`로 업데이트하는 것을 방지하여, run 상태가 일관되게 유지되고 프라이머리 노드에 의해 관리되도록 보장합니다.
...
라인 플롯 패널을 커스텀하는 방법에 대한 정보는 [Line plots](/ko/models/app/features/panels/line-plot/)를 참조하세요.
