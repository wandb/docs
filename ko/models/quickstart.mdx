...
Python 스크립트 또는 노트북에서 [`wandb.init()`](/ko/models/ref/python/run)으로 W&B run 오브젝트를 초기화합니다. `config` 파라미터에 사전을 사용하여 하이퍼파라미터 이름과 값을 지정합니다.
...
[Run](/ko/models/runs/)은 W&B의 핵심 요소로, [메트릭 추적](/ko/models/track/), [로그 생성](/ko/models/track/log/) 등에 사용됩니다.
...
2. [W&B Reports](/ko/models/reports/)를 사용하여 runs을 구성하고, 시각화를 자동화하고, 발견한 내용을 요약하고, 협업자와 업데이트를 공유합니다.
3. [W&B Artifacts](/ko/models/artifacts/)를 생성하여 기계 학습 파이프라인 전체에서 데이터셋, Models, 종속성 및 결과를 추적합니다.
4. [W&B Sweeps](/ko/models/sweeps/)로 하이퍼파라미터 검색을 자동화하고 Models을 최적화합니다.
5. [중앙 대시보드](/ko/models/tables/)에서 runs을 분석하고, 모델 예측값을 시각화하고, 인사이트를 공유합니다.
