...
Artifact 버전을 조직에서 사용할 수 있도록 하려면, W&B Registry의 [collection](/ko/models/registry/create_collection)에 해당 버전을 *link*하세요. Linking을 통해 해당 버전은 [비공개 프로젝트 레벨 스코어에서 공유 가능한 조직 레벨 스코프](/ko/models/registry/create_registry#visibility-types)로 이동합니다. Artifact 버전은 [W&B Python SDK를 사용하여 프로그래밍 방식으로 또는 W&B App에서 대화형으로](/ko/models/registry/link_version#link-an-artifact-to-a-collection) link할 수 있습니다.
...
* collection이 속한 registry가 이미 존재하는지 확인하세요. registry 존재 여부를 확인하려면 [Registry App으로 이동하여](/ko/models/registry/search_registry) registry 이름을 검색하세요.
</Note>

{/* <Note>
If an artifact version logs metrics (such as by using `wandb.Run.log_artifact()`), you can view metrics for that version from its details page, and you can compare metrics across artifact versions from the artifact's page. Refer to [View linked artifacts in a registry](#registry에서-link된-artifact-확인하기).
</Note> */}
...
[`wandb.Run.link_artifact()`](/ko/ref/python/experiments/run#link_artifact") 또는 [`wandb.Artifact.link()`](/ko/ref/python/experiments/artifact#method-artifactlink")를 사용하여 프로그래밍 방식으로 Artifact 버전을 collection에 link합니다.

<Note>
[run의 컨텍스트 내에서](#run의-컨텍스트-내에서-artifact-버전-link하기) Artifact 버전을 link하려면 `wandb.Run.link_artifact()`를 사용하세요. [run의 컨텍스트 외부에서](#run의-컨텍스트-외부에서-artifact-버전-link하기) *기존 Artifact 버전*을 link하려면 `wandb.Artifact.link()`를 사용하세요.
</Note>
...
* Source version: the artifact version inside a team's project that is logged to a [run](/ko/models/track/runs/).
...
Registry App에서 [link된 Artifact의 메타데이터, 버전 데이터, 사용량, 계보(lineage) 정보](/ko/models/registry/link_version#view-linked-artifacts-in-a-registry) 등을 확인할 수 있습니다.
...
#### 팀 entity에서 로그하기
1. [`wandb.init()`](/ko/models/ref/python/functions/init)으로 run을 초기화할 때 팀을 entity로 지정하세요. run을 초기화할 때 `entity`를 지정하지 않으면 run은 기본 entity를 사용하며, 이는 팀 entity일 수도 있고 아닐 수도 있습니다.
...
2. `wandb.Run.log_artifact()`를 사용하거나 Artifact 오브젝트를 생성하고 다음을 사용하여 파일을 추가하여 run에 Artifact를 로그합니다:
    ...
    ```
    Artifact 로그에 대해서는 [Construct artifacts](/ko/models/artifacts/construct-an-artifact/)를 참조하세요.
