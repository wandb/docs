...
Artifacts 의 속성을 파악한 후에는 [연결된 Artifacts의 경로 구성](#연결된-artifacts의-경로-구성)을 통해 Artifacts 를 다운로드할 수 있습니다. 또는 W&B App UI에서 [사전 생성된 코드조각을 복사하여 붙여넣기](#사전-생성된-코드조각-복사-및-붙여넣기) 함으로써 Registry에 연결된 Artifacts 를 다운로드할 수도 있습니다.
...
`wandb.Run.use_artifact()` 메소드는 [run](/ko/models/runs/) 을 생성함과 동시에 다운로드한 Artifacts 를 해당 run 의 입력으로 마킹합니다.
...
파라미터 및 반환 타입에 대한 자세한 내용은 API 레퍼런스의 [`wandb.Run.use_artifact()`](/ko/models/ref/python/experiments/run#use_artifact) 및 [`Artifact.download()`](/ko/models/ref/python/experiments/artifact#download) 를 참조하세요.
