...
[W&B Registry App의 글로벌 검색 창](/ko/models/core/registry/search_registry/#search-for-registry-items)을 사용하여 registry, collection, artifact version tag, collection tag 또는 에일리어스를 찾으세요. W&B Python SDK를 사용하여 특정 기준에 따라 [MongoDB 스타일 쿼리로 registries, collections 및 artifact versions을 필터링](/ko/models/core/registry/search_registry/#query-registry-items-with-mongodb-style-queries)할 수 있습니다.
...
`wandb.Api().registries()` 메소드를 사용하려면 먼저 W&B Python SDK([`wandb`](/ko/models/ref/python)) 라이브러리를 가져옵니다.
```python
import wandb
...
```
아티팩트 오브젝트의 속성 전체 목록은 API Reference 문서의 [Artifacts Class](/ko/models/ref/python/artifact/)를 참조하세요.
