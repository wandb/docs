...
W&B에서는 모델을 메모리에 로드하는 방법에 대한 정보를 모델 카드를 통해 문서화할 것을 권장합니다. 자세한 내용은 [기계 학습 모델 문서화](/ko/models/core/registry/model_registry/create-model-cards/) 페이지를 참조하세요.
</Note>
...
가능한 파라미터 및 반환 유형에 대한 자세한 내용은 API Reference 가이드의 [`use_model`](/ko/models/ref/python/run#use_model)을 참조하세요.
...
W&B Registry를 사용하여 모델 아티팩트를 추적, 구성 및 사용합니다. 자세한 내용은 [Registry 문서](/ko/models/core/registry/)를 참조하세요.
</Note>
