...
* [`wandb.init`은 제 트레이닝 프로세스에 어떤 영향을 미치나요?](/ko/models/support/wandbinit_training_process/)
* [Sweeps에서 사용자 정의 CLI 코맨드를 어떻게 사용하나요?](/ko/models/support/custom_cli_commands_sweeps/)
* [오프라인으로 메트릭을 저장하고 나중에 W&B에 동기화할 수 있나요?](/ko/models/support/same_metric_appearing_more/)
* [제 트레이닝 코드에서 run의 이름을 어떻게 구성할 수 있나요?](/ko/models/support/configure_name_run_training_code/)
...
이 섹션에서는 Artifacts 관리 및 편집을 위한 워크플로우를 설명합니다. 많은 워크플로우가 W&B에 저장된 데이터에 대한 엑세스를 제공하는 [클라이언트 라이브러리](/ko/models/ref/python/)의 구성 요소인 [W&B API](/ko/models/track/public-api-guide/)를 활용합니다.
</Accordion>
</Update>
...
[스윕](/ko/models/sweeps/)에서 모델을 로깅하는 효과적인 방법 중 하나는 스윕을 위한 모델 아티팩트를 생성하는 것입니다. 각 버전은 스윕의 서로 다른 run을 나타냅니다. 다음과 같이 구현합니다.
...
2. 네트워크 연결이 불안정한 경우, [선택적 환경 변수](/ko/models/track/environment-variables/#optional-environment-variables) `WANDB_MODE`를 `offline`으로 설정하여 오프라인 모드로 작동하고, 인터넷 엑세스가 가능한 장치에서 나중에 파일을 동기화합니다.
3. 로컬에서 실행되고 클라우드 서버와 동기화를 피하는 [W&B Private Hosting](/ko/platform/hosting/) 사용을 고려하세요.
...
* `offline`: 클라이언트가 데이터를 wandb 서버로 보내는 대신 로컬 머신에 데이터를 저장합니다. 나중에 데이터를 동기화하려면 [`wandb sync`](/ko/models/ref/cli/wandb-sync) 코맨드를 사용하세요.
...
하나의 run에 여러 개의 태그가 있을 수 있으므로 태그별 그룹화는 지원되지 않습니다. 이러한 run에 대한 [`config`](/ko/models/track/config/) 오브젝트에 값을 추가하고 대신 이 config 값으로 그룹화하세요. 이는 [API](/ko/models/track/config/#set-the-configuration-after-your-run-has-finished)를 사용하여 수행할 수 있습니다.
</Accordion>
</Update>
...
<Tabs>
<Tab title="NOT_FOUND_6">
```python
wandb.init(settings=wandb.Settings(start_method="fork"))
```
</Tab>
<Tab title="NOT_FOUND_7">
...
복잡한 워크플로우의 경우, 여러 개의 run 을 사용하고 [`wandb.init`](/ko/models/track/create-an-experiment/) 에서 group 파라미터를 단일 experiment 내의 모든 프로세스에 대해 고유한 값으로 설정합니다. [**Runs** tab](/ko/models/track/project-page/#runs-tab) 은 테이블을 그룹 ID별로 그룹화하여 시각화가 올바르게 작동하도록 보장합니다. 이 접근 방식을 사용하면 결과를 한 곳에 기록하면서 동시 Experiments 와 트레이닝 run 을 수행할 수 있습니다.

더 간단한 워크플로우의 경우, `resume=True` 및 `id=UNIQUE_ID` 와 함께 `wandb.init` 을 호출한 다음, 동일한 `id=UNIQUE_ID` 와 함께 `wandb.init` 을 다시 호출합니다. [`wandb.log`](/ko/models/track/log/) 또는 `wandb.summary` 로 정상적으로 로그하면 run 값이 그에 따라 업데이트됩니다.
</Accordion>
</Update>
...
다음 예제는 [`wandb.log()`](/ko/models/ref/python/log/)를 사용하여 여러 가지 방법으로 손실을 기록하는 방법을 보여줍니다.

<Tabs>
<Tab title="NOT_FOUND_8">
```python
wandb.log({f"losses/loss-{ii}": loss for ii, 
  loss in enumerate(losses)})
```
</Tab>
<Tab title="NOT_FOUND_9">
```python
# 손실을 히스토그램으로 변환합니다.
...
예, 메트릭 과 함께 `batch` 및 `epoch`와 같은 인덱스를 로그합니다. 한 단계에서 `wandb.log({'train_accuracy': 0.9, 'batch': 200})`을 사용하고 다른 단계에서 `wandb.log({'val_accuracy': 0.8, 'epoch': 4})`을 사용합니다. UI에서 원하는 값을 각 차트의 x축으로 설정합니다. 특정 인덱스에 대한 기본 x축을 설정하려면 [Run.define_metric()](/ko/models/ref/python/run#define_metric)을 사용하세요. 제공된 예제의 경우 다음 코드를 사용하세요.
...
W&B는 UI를 통한 run 이동을 지원하지만, run 복사는 지원하지 않습니다. run과 함께 로그된 Artifacts는 새 프로젝트로 전송되지 않습니다. Artifacts를 run의 새 위치로 수동으로 이동하려면 [`wandb artifact get`](/ko/models/ref/cli/wandb-artifact/wandb-artifact-get/) SDK 코맨드 또는 [`Api.artifact` API](/ko/models/ref/python/public-api/api/#artifact)를 사용하여 아티팩트를 다운로드한 다음 [wandb artifact put](/ko/models/ref/cli/wandb-artifact/wandb-artifact-put/) 또는 `Api.artifact` API를 사용하여 run의 새 위치로 업로드할 수 있습니다.
</Accordion>
</Update>
...
1. 모든 프로세스에서 `wandb.init`을 호출하고 [group](/ko/models/runs/grouping/) 키워드 인수를 사용하여 공유 그룹을 생성합니다. 각 프로세스는 자체 wandb run을 가지며, UI는 트레이닝 프로세스를 함께 그룹화합니다.
...
<Note>
Torch DDP를 사용한 코드 예제를 포함하여 이러한 접근 방식에 대한 자세한 설명은 [Distributed Training Guide](/ko/models/track/log/distributed-training/) 를 참조하십시오.
</Note>
</Accordion>
</Update>
...
[Workspace](/ko/models/track/project-page/#workspace-tab) 설정에서 `/`로 구분된 첫 번째 세그먼트 또는 모든 세그먼트를 기반으로 패널 그룹화를 조정합니다.
</Accordion>
</Update>
...
`wandb.plot.line_series()`를 사용하여 여러 줄로 된 사용자 정의 차트를 만드세요. 라인 차트를 보려면 [프로젝트 페이지](/ko/models/track/project-page/)로 이동하세요. 범례를 추가하려면 `wandb.plot.line_series()`에 `keys` 인수를 포함하세요. 예:
...
**여러 줄** 탭 아래의 [여기](/ko/models/track/log/plots/#basic-charts)에서 여러 줄 플롯에 대한 추가 세부 정보를 참조하세요.
</Accordion>
</Update>
...
[`wandb.Run`](/ko/models/ref/python/run) 의 `.name` 속성은 다음과 같이 액세스할 수 있습니다:
...
<Tabs>
<Tab title="NOT_FOUND_12">
```python
%env "WANDB_NOTEBOOK_NAME" "여기에 노트북 이름"
```
</Tab>
<Tab title="NOT_FOUND_13">
```python
import os
...
그리드 검색이 완료되었지만 충돌로 인해 일부 W&B Runs을 다시 실행해야 하는 경우, 다시 실행할 특정 W&B Runs을 삭제하세요. 그런 다음 [스윕 제어 페이지](/ko/models/sweeps/sweeps-ui/)에서 **다시 시작** 버튼을 선택합니다. 새 Sweep ID를 사용하여 새로운 W&B Sweep 에이전트를 시작하세요.
...
`wandb.init` 가 호출되면 시스템은 원격 저장소 링크와 최신 커밋의 SHA를 포함한 git 정보를 자동으로 수집합니다. 이 정보는 [run 페이지](/ko/models/runs/#view-logged-runs)에 나타납니다. 이 정보를 보려면 스크립트를 실행할 때 현재 작업 디렉토리가 git으로 관리되는 폴더 내에 있는지 확인하세요.
...
무엇보다도 서비스 계정은 주기적인 재 트레이닝, 야간 빌드 등과 같이 wandb 에 기록된 자동화된 작업을 추적하는 데 유용합니다. 원하는 경우 [환경 변수](/ko/models/track/environment-variables/) `WANDB_USERNAME` 또는 `WANDB_USER_EMAIL`을 사용하여 이러한 머신에서 실행된 run 과 사용자 이름을 연결할 수 있습니다.

자세한 내용은 [팀 서비스 계정 행동](/ko/platform/app/settings-page/teams/#team-service-account-behavior)을 참조하십시오.
...
<Note>
**Built-in** 서비스 계정 외에도 W&B 는 [SDK 및 CLI 에 대한 ID 페더레이션](/ko/platform/hosting/iam/identity_federation/#external-service-accounts)을 사용하는 **External service accounts** 도 지원합니다. JSON Web Tokens (JWT)를 발행할 수 있는 ID 공급자에서 관리되는 서비스 ID 를 사용하여 W&B 작업을 자동화하려는 경우 외부 서비스 계정을 사용하십시오.
</Note>
</Accordion>
</Update>
...
<Tabs>
<Tab title="NOT_FOUND_14">
```python
%env WANDB_SILENT=True
```
</Tab>
<Tab title="NOT_FOUND_15">
```python
import os
...
<Tab title="NOT_FOUND_16">
```python
%env WANDB_SILENT=true
```
</Tab>
<Tab title="NOT_FOUND_17">
```shell
WANDB_SILENT=true
```
</Tab>
...
- [`wandb.Artifact().download`](/ko/models/ref/python/public-api/api) 에 전달합니다.
