...
*자동화*는 W&amp;B의 특정 이벤트에 대응하여 Slack 알림을 보내거나 웹훅을 실행합니다. 자동화의 범위는 조직, 팀, 프로젝트, 레지스트리 또는 컬렉션으로 지정할 수 있습니다. 자동화는 나중에 생성되는 프로젝트, 레지스트리 및 컬렉션을 포함해 범위 내의 모든 항목에서 이벤트를 감시합니다. 자동화를 만드는 step과 사용 가능한 이벤트는 범위에 따라 다릅니다. 범위별 이벤트 유형은 [자동화 이벤트 및 범위](/ko/models/automations/automation-events)를 참조하세요.
