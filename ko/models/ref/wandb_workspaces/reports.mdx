...
* `xaxis_format` (Optional[str]):  x축 형식입니다. 이 옵션은 사용자 지정 메트릭을 정의한 경우 표시됩니다. 예를 들어, x축을 날짜 및 시간 형식으로 표시하려면 &#39;datetime&#39;을 지정할 수 있습니다.
...
* `point_visualization_method` (Optional[PointVizMethod]):  표시할 수 있는 양보다 포인트가 너무 많을 때 포인트를 집계하는 데 사용하는 방법입니다. 가능한 값은 &quot;bucketing-gorilla&quot;(데이터 포인트를 버킷으로 나누고 각 버킷의 최소값, 최대값, 평균값을 표시해 이상치와 급격한 변화를 보존) 또는 &quot;sampling&quot;(더 빠른 렌더링을 위해 포인트를 무작위로 샘플링하지만 이상치를 놓칠 수 있음)입니다.
...
리포트에 대해 현재 활성화된 모든 공개 액세스 토큰을 폐기합니다.

**戻り値**

* `bool`:  링크가 폐기된 경우 True, 활성 링크가 없으면 False입니다.
...
이 리포트의 매직 링크 URL을 가져옵니다. 공유가 활성화되어 있지 않으면 `None`을 반환합니다.

활성 상태의 액세스 토큰을 확인하기 위해 네트워크 요청을 보냅니다. 액세스 토큰이 추가된 리포트 URL을 반환하므로, 프로젝트가 비공개여도 링크를 가진 사람은 누구나 리포트를 볼 수 있습니다.
...
리포트 패널 그리드에서 개별 run의 표시 모양과 가시성을 구성합니다.

`RunSettings`를 사용하면 `Runset` 내에서 특정 run이 렌더링되는 방식을 사용자 지정할 수 있으며, 여기에는 선 색상과 차트에서의 가시성이 포함됩니다.

**속성:**

- `color`:  run의 표시 색상입니다. 16진수 형식(`#ff0000`), CSS 색상 이름(red), 또는 RGB 문자열("rgb(255,0,0)")을 허용합니다.
 - `disabled`:  True이면 차트에서 run을 숨깁니다. 이는 UI에서 눈 감김 아이콘을 클릭하는 것과 동일합니다.
...
* `custom_run_colors` (dict):  run ID(또는 그룹화된 run의 경우 `RunsetGroup` 키)를 색상에 매핑하는 사전입니다. 단순한 run별 색상 지정에는 `run_settings` 사용을 권장합니다.
...
* `table_name` (str):  테이블, DataFrame, 플롯 또는 값의 이름입니다.
* `layout` (Layout):  위치와 크기를 포함한 패널의 레이아웃 설정입니다. Panel 기본 클래스에서 상속됩니다. 너비(`w`)와 높이(`h`)를 조정하는 데 사용합니다.

**예시:**
사용자 지정 크기의 테이블을 표시하는 요약 테이블 패널이 포함된 리포트를 생성합니다:
...
* w (너비): 그리드 단위의 너비 (기본값: 8, 최대: 24)
  ...
  * x (x 위치): 그리드에서의 가로 위치 (기본값: 0)
  * y (y 위치): 그리드에서의 세로 위치 (기본값: 0)
