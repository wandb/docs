...
## <kbd>class</kbd> `BarPlot` {#class-barplot}
...
## <kbd>class</kbd> `BlockQuote` {#class-blockquote}
...
## <kbd>class</kbd> `CalloutBlock` {#class-calloutblock}
...
## <kbd>class</kbd> `CheckedList` {#class-checkedlist}
...
## <kbd>class</kbd> `CheckedListItem` {#class-checkedlistitem}
...
## <kbd>class</kbd> `CodeBlock` {#class-codeblock}
...
## <kbd>class</kbd> `CodeComparer` {#class-codecomparer}
...
## <kbd>class</kbd> `Config` {#class-config}
...
## <kbd>class</kbd> `CustomChart` {#class-customchart}
...
### <kbd>classmethod</kbd> `from_table` {#classmethod-from_table}
...
## <kbd>class</kbd> `Gallery` {#class-gallery}
...
## <kbd>class</kbd> `GalleryReport` {#class-galleryreport}
...
## <kbd>class</kbd> `GalleryURL` {#class-galleryurl}
...
## <kbd>class</kbd> `GradientPoint` {#class-gradientpoint}
...
## <kbd>class</kbd> `Heading` {#class-heading}

***

## <kbd>class</kbd> `HorizontalRule` {#class-horizontalrule}

HTML 수평선.

***

## <kbd>class</kbd> `Image` {#class-image}
...
## <kbd>class</kbd> `InlineCode` {#class-inlinecode}
...
## <kbd>class</kbd> `InlineLatex` {#class-inlinelatex}
...
## <kbd>class</kbd> `LatexBlock` {#class-latexblock}
...
## <kbd>class</kbd> `Layout` {#class-layout}
...
## <kbd>class</kbd> `LinePlot` {#class-lineplot}
...
* `line_titles` (Optional[Dict[str, str]]):  각 시리즈의 표시 제목입니다. 키는 &quot;{runId}:{metricName}&quot; 형식입니다.
* `line_colors` (Optional[Dict[str, Any]]):  각 시리즈의 색상입니다. 키는 &quot;{runId}:{metricName}&quot; 형식을 사용합니다.
* `line_widths` (Optional[Dict[str, float]]):  시리즈별 선 두께입니다. 키는 &quot;{runId}:{metricName}&quot; 형식을 사용합니다.
* `line_marks` (Optional[Dict[str, Mark]]):  시리즈별 대시 스타일입니다. 키는 &quot;{runId}:{metricName}&quot; 형식입니다.
* `유효한 값`:  &quot;solid&quot;, &quot;dashed&quot;, &quot;dotted&quot;, &quot;dotdash&quot;, &quot;dotdotdash&quot;, &quot;points&quot;.

---

## <kbd>class</kbd> `Link` {#class-link}
...
## <kbd>class</kbd> `MarkdownBlock` {#class-markdownblock}
...
## <kbd>class</kbd> `MarkdownPanel` {#class-markdownpanel}
...
## <kbd>class</kbd> `MediaBrowser` {#class-mediabrowser}
...
## <kbd>class</kbd> `Metric` {#class-metric}
...
## <kbd>class</kbd> `OrderBy` {#class-orderby}
...
## <kbd>class</kbd> `OrderedList` {#class-orderedlist}
...
## <kbd>class</kbd> `OrderedListItem` {#class-orderedlistitem}
...
## <kbd>class</kbd> `Panel` {#class-panel}
...
## <kbd>class</kbd> `PanelGrid` {#class-panelgrid}
...
## <kbd>class</kbd> `ParallelCoordinatesPlot` {#class-parallelcoordinatesplot}
...
## <kbd>class</kbd> `ParallelCoordinatesPlotColumn` {#class-parallelcoordinatesplotcolumn}
...
## <kbd>class</kbd> `ParameterImportancePlot` {#class-parameterimportanceplot}
...
## <kbd>class</kbd> `Report` {#class-report}
...
### <kbd>방법</kbd> `disable_share_link` {#method-disable_share_link}
...
### <kbd>방법</kbd> `enable_share_link` {#method-enable_share_link}
...
### <kbd>방법</kbd> `get_share_url` {#method-get_share_url}
...
### <kbd>방법</kbd> `to_html` {#method-to_html}
...
## <kbd>class</kbd> `RunComparer` {#class-runcomparer}
...
## <kbd>class</kbd> `RunSettings` {#class-runsettings}
...
## <kbd>클래스</kbd> `Runset` {#class-runset}
...
* `pinned_columns` (LList[str]): runs table의 왼쪽에 고정할 열 접근자입니다
* `the runs table (e.g. `[&quot;summary`: accuracy", "config:lr.value"]`). `hidden_columns`에도 있는 열은 저장 시 제거됩니다.
* `visible_columns` (LList[str]): 강제로 표시할 열 접근자입니다. 표시할 집합을 선별하려면 `lock_columns=True`와 함께 사용하세요. 다시 로드할 때는 `column_order`를 통해서만 설정된 열도 여기에 포함됩니다.
* `hidden_columns` (LList[str]): 숨길 열 접근자입니다. `pinned_columns`, `visible_columns`, `column_order`보다 우선합니다. 다시 로드할 때는 서버에서 미리 채워 넣은 항목이 포함될 수 있습니다
* `like `run`: name`.
* `column_order` (LList[str]): 왼쪽에서 오른쪽으로의 열 순서입니다. `lock_columns=True`에서는 `pinned_columns` 또는 `visible_columns`에 없는 항목은 제외됩니다.
* `column_widths` (Dict[str, int]): 열별 픽셀 너비입니다.
* `lock_columns` (Optional[bool]): Runs table 열 집합에 대한 선택적 잠금입니다. `True`이면 `pinned_columns`와 `visible_columns`만 표시합니다. `False` 또는 `None`(기본값)이면 제한 없이 허용됩니다. Runs table 툴바의 잠금 아이콘과 일치합니다.

열 형식:

* 열은 `"<section>: <name>"` 형식으로 지정합니다:
  * `run:<attr>`: 기본 제공 run 속성(예: `run:state`)
  * `config:<key>.value`: config 키(`.value` 접미사에 유의하세요)
  * `summary:<key>`: summary 메트릭
  * `tags:__ALL__`: run 태그

열 제한: FE Runs table은 최대 500개 열까지만 표시합니다. 표시 집합(고정 + 표시 + column&#95;order, hidden 제외)이 500개에 도달하거나 이를 초과하면 SDK는 저장 시 경고를 출력합니다. 숨겨진 열은 개수에 포함되지 않습니다. SDK는 잘라내지 않고 전체 집합을 전송하며, 프런트엔드가 렌더링 시 제한을 강제 적용합니다.
...
### <kbd>방법</kbd> `convert_filterexpr_list_to_string` {#method-convert_filterexpr_list_to_string}

```python
convert_filterexpr_list_to_string()
```

FilterExpr 목록을 string 표현식으로 변환합니다.

***

### <kbd>방법</kbd> `merge_custom_run_colors_into_run_settings` {#method-merge_custom_run_colors_into_run_settings}
...
## <kbd>class</kbd> `RunsetGroup` {#class-runsetgroup}
...
## <kbd>class</kbd> `RunsetGroupKey` {#class-runsetgroupkey}
...
## <kbd>class</kbd> `ScalarChart` {#class-scalarchart}
...
## <kbd>클래스</kbd> `ScatterPlot` {#class-scatterplot}
...
## <kbd>class</kbd> `SoundCloud` {#class-soundcloud}
...
## <kbd>class</kbd> `Spotify` {#class-spotify}
...
## <kbd>class</kbd> `SummaryMetric` {#class-summarymetric}
...
## <kbd>class</kbd> `TableOfContents` {#class-tableofcontents}

리포트에 지정된 H1, H2, H3 HTML 블록을 사용해 섹션과 하위 섹션 목록을 표시하는 블록입니다.

***

## <kbd>class</kbd> `TextWithInlineComments` {#class-textwithinlinecomments}
...
## <kbd>class</kbd> `Twitter` {#class-twitter}
...
## <kbd>class</kbd> `UnorderedList` {#class-unorderedlist}
...
## <kbd>class</kbd> `UnorderedListItem` {#class-unorderedlistitem}
...
## <kbd>class</kbd> `Video` {#class-video}
...
## <kbd>class</kbd> `WeaveBlockArtifact` {#class-weaveblockartifact}
...
## <kbd>class</kbd> `WeaveBlockArtifactVersionedFile` {#class-weaveblockartifactversionedfile}
...
## <kbd>class</kbd> `WeaveBlockSummaryTable` {#class-weaveblocksummarytable}
...
## <kbd>class</kbd> `WeavePanel` {#class-weavepanel}
...
## <kbd>class</kbd> `WeavePanelArtifact` {#class-weavepanelartifact}
...
## <kbd>class</kbd> `WeavePanelArtifactVersionedFile` {#class-weavepanelartifactversionedfile}
...
## <kbd>class</kbd> `WeavePanelSummaryTable` {#class-weavepanelsummarytable}
