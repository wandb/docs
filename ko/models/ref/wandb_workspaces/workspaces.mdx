...
ws.Workspace(
     ...
     entity="entity", # workspace를 소유한 entity
...
Workspace의 runset(Runs가 포함된 왼쪽 바)에 대한 설정입니다.

**속성:**

* `query` (str):  runset을 필터링하기 위한 쿼리입니다(정규식 expr일 수 있으며, 다음 파라미터를 참조하세요).
* `regex_query` (bool):  쿼리(위)가 정규식 expr인지 여부를 제어합니다. 기본값은 `False`입니다.
* `filters` (Union[str, LList[expr.FilterExpr]]):  runset에 적용할 필터 목록 또는 문자열 표현식입니다.
  * 목록인 경우: 필터는 함께 AND로 결합됩니다. 필터 생성에 대한 자세한 내용은 FilterExpr를 참조하세요.
...
* `order` (LList[expr.Ordering]):  runset에 적용할 메트릭 및 정렬 순서 목록입니다.
* `run_settings` (Dict[str, RunSettings]):  run 설정의 사전으로, 키는 run의 ID이고 값은 RunSettings 객체입니다.
* `pinned_columns` (LList[str]):  고정할 column 이름 목록입니다.
...
run:displayName이 존재하는지 확인하고 내부 column 필드를 설정하세요.
...
섹션의 패널 레이아웃 설정으로, 일반적으로 W&amp;B App Workspace UI의 해당 섹션 오른쪽 상단에 표시됩니다.
...
섹션용 패널 설정으로, 섹션에 대한 `WorkspaceSettings`와 유사합니다.

여기에 적용된 설정은 더 세밀한 패널 설정에 의해 다음 우선순위에 따라 재정의될 수 있습니다: Section &lt; Panel.
...
섹션, 설정, 그리고 run 세트 설정을 포함한 W&amp;B Workspace를 나타냅니다.
...
* `sections` (LList[Section]):  Workspace에 있는 섹션의 순서가 있는 목록입니다. 첫 번째 섹션은 Workspace의 맨 위에 있습니다.
* `settings` (WorkspaceSettings):  Workspace의 설정으로, 일반적으로 UI에서 Workspace 상단에 표시됩니다.
* `runset_settings` (RunsetSettings):  Workspace의 runset 설정입니다(Runs이 포함된 왼쪽 사이드바).
* `auto_generate_panels` (bool):  이 프로젝트에 로깅된 모든 키에 대해 패널을 자동으로 생성할지 여부입니다. 사용 가능한 모든 데이터를 기본적으로 시각화하려는 경우 권장됩니다. 이 값은 Workspace를 생성할 때만 설정할 수 있으며 이후에는 수정할 수 없습니다.
...
현재 Workspace를 W&amp;B에 저장하세요.

**반환값:**

* `Workspace`:  저장된 내부 name과 ID가 반영된 업데이트된 Workspace입니다.
...
* `Workspace`: 저장된 내부 이름과 ID가 반영된 업데이트된 Workspace입니다.
...
Workspace에 대한 설정입니다. 일반적으로 UI에서 Workspace 상단에 표시됩니다.

이 객체에는 X축, 스무딩, 이상치, 패널, 툴팁, Runs, 패널 쿼리 바에 대한 설정이 포함됩니다.

여기에 적용된 설정은 더 세부적인 Section 및 Panel 설정에 의해 다음 우선순위에 따라 재정의될 수 있습니다: Workspace &lt; Section &lt; Panel

**Attributes:**

* `x_axis` (str):  X축 메트릭 이름 설정.
* `x_min` (Optional[float]):  X축의 최솟값.
* `x_max` (Optional[float]):  X축의 최댓값.
* `smoothing_type` (Literal[&#39;exponentialTimeWeighted&#39;, &#39;exponential&#39;, &#39;gaussian&#39;, &#39;average&#39;, &#39;none&#39;]):  모든 패널에 적용되는 스무딩 유형.
* `smoothing_weight` (int):  모든 패널에 적용되는 스무딩 가중치.
* `ignore_outliers` (bool):  모든 패널에서 이상치를 무시할지 여부.
* `sort_panels_alphabetically` (bool):  모든 섹션의 패널을 알파벳순으로 정렬할지 여부.
* `group_by_prefix` (Literal[&quot;first&quot;, &quot;last&quot;]):  첫 번째 또는 마지막 prefix를 기준으로 패널을 그룹화합니다. 기본값은 `last`입니다.
* `remove_legends_from_panels` (bool):  모든 패널에서 범례를 제거할지 여부.
* `tooltip_number_of_runs` (Literal[&quot;default&quot;, &quot;all&quot;, &quot;none&quot;]):  툴팁에 표시할 Runs 수.
* `tooltip_color_run_names` (bool):  툴팁에서 run 이름의 색상을 runset과 일치시키려면 True, 그렇지 않으면 False입니다. 기본값은 `True`입니다.
* `max_runs` (int):  패널당 표시할 최대 Runs 수(이 값은 runset의 처음 10개 Runs가 됩니다).
* `point_visualization_method` (Literal[&quot;line&quot;, &quot;point&quot;, &quot;line&#95;point&quot;]):  포인트 시각화 방법.
* `panel_search_query` (str):  패널 검색 바의 쿼리(정규식 표현식일 수 있음).
