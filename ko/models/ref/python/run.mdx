...
[`wandb.init` 가이드](/ko/models/track/launch)를 확인하세요.
...
[가이드](/ko/models/track/log/distributed-training)를 확인하세요.
...
[로깅 가이드](/ko/models/track/log)를 참조하세요.
...
W&B 앱의 [자체 호스팅 인스턴스](/ko/guides/hosting)에서 기록된 데이터를 시각화하거나,
[API](/ko/models/track/public-api-guide)를 사용하여 로컬에서 시각화하고 탐색할 데이터를 내보냅니다(예: Jupyter 노트북).
...
지원되는 모든 유형에 대한
[참조 문서](/ko/models/ref/python/data-types/)를 참조하거나
3D 분자 구조 및 분할 마스크에서 PR 곡선 및 히스토그램에 이르기까지 예제에 대한
[로깅 가이드](/ko/models/track/log)를 확인하세요.
`wandb.Table`을 사용하여 구조화된 데이터를 기록할 수 있습니다. 자세한 내용은
[테이블 로깅 가이드](/ko/models/tables/tables-walkthrough)를 참조하세요.
