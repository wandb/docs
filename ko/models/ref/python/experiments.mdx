...
이 클래스들은 머신러닝 실험을 추적하고, 아티팩트를 관리하며, SDK 동작을 설정하는 데 필요한 핵심 구성 요소입니다. 이러한 기본 클래스를 사용하면 메트릭을 로깅하고, 모델 체크포인트를 저장하며, 데이터셋의 버전을 관리하고, 재현성과 협업 기능을 완벽하게 지원하는 실험 설정을 관리할 수 있습니다.
...
| Class                                                  | 설명                                                            |
| ------------------------------------------------------ | ------------------------------------------------------------- |
| [`Run`](/ko/models/ref/python/experiments/run/)           | 메트릭, 설정, 출력이 포함된 단일 ML 실험을 나타내는, W&amp;B에 로깅되는 기본 계산 단위입니다.   |
| [`Artifact`](/ko/models/ref/python/experiments/artifact/) | 자동 중복 제거와 리니지 추적을 지원하는 데이터셋 및 모델 버전 관리를 위한 유연하고 가벼운 구성 요소입니다. |
| [`Settings`](/ko/models/ref/python/experiments/settings/) | 로깅부터 API 상호작용까지의 동작을 제어하는 W&amp;B SDK용 설정 관리입니다.              |
