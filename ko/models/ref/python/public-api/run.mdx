...
run이 실행되는 동안 W&amp;B는 해당 process가 stdout 및 stderr에 기록하는 내용을
...
* `per_page`: 로그를 처음부터 읽을 때 요청당 가져올 줄 수입니다. `last`가 지정되면 무시됩니다.
* `last`: 설정하면 단일 요청으로 로그의 마지막 N줄만 가져옵니다. 실행 중인 run을 확인하거나 충돌을 진단할 때 유용합니다. 백엔드는 하나의 요청으로 반환할 수 있는 줄 수에 한도를 두므로, 더 많은 줄을 요청하면 최신 줄만 반환됩니다.
...
* `keys`: run의 이력에서 읽을 메트릭 목록입니다. 스캔은 요청한 모든 키를 포함하는 행만 반환합니다. 키가 서로 다른 step에 로깅되었다면 각 키에 대해 별도의 Call을 사용하거나 이 인수를 생략하세요. keys를 지정하지 않으면 모든 메트릭이 반환됩니다.
