...
Reports는 `BetaReport` 객체의 지연 이터레이터입니다.
...
* `name` (str, 선택):  필터링할 리포트의 이름입니다. `None`이면 모든 리포트를 가져옵니다.
* `entity` (str, 선택):  프로젝트의 entity 이름입니다. 기본값은 프로젝트 entity입니다.
...
페이지 매김을 위한 GraphQL 쿼리 변수를 업데이트합니다.
