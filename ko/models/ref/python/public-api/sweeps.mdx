...
# 스윕을 반복하고 세부 정보를 출력합니다
for sweep in sweeps:
     print(f"스윕 이름: {sweep.name}")
     print(f"스윕 ID: {sweep.id}")
     print(f"스윕 URL: {sweep.url}")
     print("----------")
```
...
* `client`:  W&amp;B를 쿼리하는 데 사용되는 API client입니다.
* `entity`:  스윕을 보유한 entity입니다.
