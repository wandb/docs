...
### <kbd>속성</kbd> internal_id

가져온 경우 이 레지스트리의 기반 프로젝트에 대한 GraphQL `internalId`입니다.

이는 base64로 인코딩된 전역 ID입니다. 디코딩하면
`Project:123` 또는 `ProjectInternalId:123` 형식입니다.
...
* `order`: 결과 정렬 순서를 지정하는 선택 사항인 string입니다. `created_at`, `artifact_size`, 또는 `linked_at`을 지정할 수 있습니다. &#39;+&#39;를 접두사로 사용하면 오름차순(기본값)으로 정렬하고, &#39;-&#39;를 접두사로 사용하면 내림차순으로 정렬합니다.
