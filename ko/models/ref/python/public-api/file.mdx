...
* path&#95;uri (str): 버킷 내 파일 경로로, 현재는 S3 객체 및 레퍼런스 파일에만 사용 가능
...
저장소 버킷에 있는 file의 URI 경로를 반환합니다.

**반환값:**

* `str`:  file이 S3에 저장된 경우 S3 URI(예: &#39;s3://bucket/path/to/file&#39;), 레퍼런스 file인 경우 직접 URL, 또는 사용 가능하지 않은 경우 빈 문자열입니다.

**반환값:**

* `str`: The path&#95;uri 속성 값.

***

### <kbd>속성</kbd> File.size

ファイル의 크기를 바이트 단위로 반환합니다.
...
run이 이전에 wandb 서버에 저장해 둔 파일을 다운로드합니다.

**매개변수:**

* `root`:  파일을 저장할 로컬 디렉터리입니다. 기본값은 현재 작업 디렉터리(&quot;.&quot;)입니다.
* `replace`:  `True`이면 로컬에 파일이 이미 있을 경우 덮어씁니다. 기본값은 `False`입니다.
* `exist_ok`:  `True`이면 파일이 이미 존재해도 `ValueError`를 발생시키지 않으며, `replace=True`가 아닌 한 다시 다운로드하지 않습니다. 기본값은 `False`입니다.
* `api`:  지정한 경우 파일 다운로드에 사용할 `Api` 인스턴스입니다.
