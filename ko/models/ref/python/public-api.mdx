...
| 컴포넌트 | 설명 |
|-----------|-------------|
| [`Api`](/ko/models/ref/python/public-api/api/) | Public API 의 메인 엔트리 포인트입니다. 조직 전체의 Runs, Projects 및 Artifacts 를 쿼리합니다. |
| [`Runs`](/ko/models/ref/python/public-api/runs/) | 기록(history), 로그, 메트릭을 포함하여 개별 트레이닝 Runs 를 엑세스하고 관리합니다. |
| [`Artifacts`](/ko/models/artifacts/) | 모델 Artifacts, Datasets 및 기타 버전 관리된 파일을 쿼리하고 다운로드합니다. |
| [`Sweeps`](/ko/models/sweeps/) | 하이퍼파라미터 스윕 데이터에 엑세스하고 최적화 결과를 분석합니다. |
| [`Projects`](/ko/models/ref/python/public-api/projects/) | Projects 를 관리하고 프로젝트 수준의 메타데이터 및 설정에 엑세스합니다. |
| [`Reports`](/ko/models/reports/create-a-report/) | W&B Reports 에 프로그래밍 방식으로 엑세스하고 관리합니다. |
| [`Team`](/ko/models/ref/python/public-api/team) | 팀 정보를 쿼리하고 팀 수준의 리소스를 관리합니다. |
| [`User`](/ko/models/ref/python/public-api/user) | 사용자 프로필 및 사용자별 데이터에 엑세스합니다. |
| [`Files`](/ko/models/ref/python/public-api/files/) | Runs 와 관련된 파일을 다운로드하고 관리합니다. |
| `History` | 트레이닝 중 로깅된 상세 시계열 메트릭에 엑세스합니다 (Run.history 참고). |
| [`Automations`](./automations/) | 자동화된 워크플로우 및 액션을 관리합니다. |
| [`Integrations`](/ko/models/integrations) | 서드파티 인테그레이션을 구성하고 관리합니다. |
...
레지스트리, 컬렉션 또는 Artifact 를 쿼리하는 방법에 대한 자세한 내용은 [Find registry items](/ko/models/registry/search_registry) 를 참조하세요.