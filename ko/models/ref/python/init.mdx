...
자세한 예제를 포함하여 `wandb.init()` 사용에 대한 자세한 내용은 [가이드 및 FAQ](/ko/models/track/launch)를 확인하십시오.
...
| 반환 | Description |
| :--- | :--- |
|  현재 run에 대한 핸들인 `Run` 오브젝트입니다. 이 오브젝트를 사용하여 데이터 로깅, 파일 저장 및 run 완료와 같은 작업을 수행합니다. 자세한 내용은 [Run API](/ko/models/ref/python/experiments/run/)를 참조하십시오. |
