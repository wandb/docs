...
웹훅 Request를 전송하는 자동화 액션을 정의합니다.
...
* `integration_id` (str): Request를 보내는 데 사용할 웹훅 인테그레이션의 ID입니다.
* `request_payload` (Optional[Annotated]): 템플릿 변수를 포함할 수 있는, 웹훅 Request에 포함해 전송할 페이로드입니다.
...
지정된 웹훅 인테그레이션으로 보내는 웹훅 액션을 정의합니다.
