...
편집 가능한 저장된 W&B automation의 로컬 인스턴스입니다.
...
* `event` (SavedEvent): 이 Automation을 트리거할 이벤트입니다.
* `action` (Union[SavedLaunchJobAction, SavedNotificationAction, SavedWebhookAction, SavedNoOpAction]): 이 Automation이 트리거될 때 실행될 액션입니다.

**반환값:**
`Automation` 객체를 반환합니다.
