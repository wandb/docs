...
`wandb.init()`는 run에 데이터를 로깅하기 위한 새 백그라운드 프로세스를 생성하고, 기본적으로 데이터를 https://wandb.ai 에 동기화하므로 결과를 실시간으로 확인할 수 있습니다. 데이터 로깅이 끝나면 `wandb.Run.finish()`를 호출해 run을 종료하세요. `run.finish()`를 호출하지 않으면 스크립트가 종료될 때 run도 함께 종료됩니다.
...
with wandb.init(config=config) as run:
    # run에 accuracy와 loss를 로깅합니다
    acc = 0.95  # 예시 accuracy
    loss = 0.05  # 예시 loss
