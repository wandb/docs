...
init(
    ...
    anonymous: 'DoNotSet' = <object object at 0x10482d250>
) → Run
```
...
`wandb.init()`는 run에 데이터를 로깅하기 위한 새 백그라운드 프로세스를 생성하고, 기본적으로 데이터를 [https://wandb.ai](https://wandb.ai)에 동기화하므로 결과를 실시간으로 확인할 수 있습니다. 데이터 로깅이 끝나면 `run.finish()`를 호출해 run을 종료하거나, run을 컨텍스트 매니저로 사용해 자동으로 호출되도록 하세요:

with wandb.init() as run:  ...  # 블록 끝에서 run.finish()가 실행됩니다
