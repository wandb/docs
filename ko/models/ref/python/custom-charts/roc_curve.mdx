...
## <kbd>함수</kbd> wandb.plot.roc_curve
...
* `labels`: `y_true`의 클래스 인덱스에 대응하는 사람이 읽기 쉬운 레이블입니다. 예를 들어 `labels=['dog', 'cat']`이면 플롯에서 클래스 0은 &#39;dog&#39;, 클래스 1은 &#39;cat&#39;으로 표시됩니다. None이면 `y_true`의 원래 클래스 인덱스를 사용합니다. 기본값은 None입니다.
* `classes_to_plot`: ROC 곡선에 포함할 고유 클래스 레이블의 하위 집합입니다. None이면 `y_true`의 모든 클래스가 플롯됩니다. 기본값은 None입니다.
...
# 실제 레이블: 각 샘플에 "Diabetes", "Hypertension", 또는 "Heart Disease" 부여
...
# W&B run을 초기화하고 질병 분류에 대한 ROC 곡선 플롯 기록
