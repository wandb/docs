...
<div id="function-wandbplotconfusion_matrix">
  ## <Badge color="blue" size="lg" shape="rounded">함수</Badge> wandb.plot.confusion_matrix()
</div>

확률 또는 예측 시퀀스로 혼동 행렬을 생성합니다.
...
<ResponseField name="probs" type="Sequence[Sequence[float]] | None">
  각 클래스의 예측 확률 시퀀스입니다. 시퀀스의 형태는 (N, K)여야 합니다. 여기서 N은 샘플 수이고 K는 클래스 수입니다. 이 값을 제공하는 경우 `preds`를 제공해서는 안 됩니다.
</ResponseField>
...
<ResponseField name="preds" type="Sequence[T] | None">
  예측 클래스 레이블의 시퀀스입니다. 이 값을 제공하는 경우 `probs`를 제공해서는 안 됩니다.
</ResponseField>

<ResponseField name="class_names" type="Sequence[str] | None">
  클래스 이름의 시퀀스입니다. 제공하지 않으면 클래스 이름은 &quot;Class&#95;1&quot;, &quot;Class&#95;2&quot; 등으로 정의됩니다.
</ResponseField>
...
<ResponseField name="split_table" type="bool">
  테이블을 W&amp;B UI의 별도 섹션으로 분리할지 여부입니다. `True`이면 테이블이 &quot;Custom Chart Tables&quot; 섹션에 표시됩니다. 기본값은 `False`입니다.
</ResponseField>
...
야생동물 분류에 대한 무작위 확률로 혼동 행렬을 로깅하는 예시:
...
이 예제에서는 무작위 확률을 사용해 혼동
행렬을 생성합니다.

시뮬레이션된 모델 예측과 정확도 85%로
혼동 행렬을 로깅하는 예시:
...
이 예제에서는 정확도 85%의 예측을 시뮬레이션하여 혼동 행렬을 생성합니다.