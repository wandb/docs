...
* `preds`:  예측된 클래스 레이블 시퀀스입니다. 이 인자를 제공하는 경우 `probs`는 제공하지 않아야 합니다.
* `class_names`:  클래스 이름 시퀀스입니다. 제공되지 않으면 클래스 이름은 &quot;Class&#95;1&quot;, &quot;Class&#95;2&quot; 등으로 정의됩니다.
* `title`:  혼동 행렬 차트의 제목입니다.
* `split_table`:  테이블을 W&amp;B UI에서 별도 섹션으로 분리할지 여부입니다. `True`인 경우 테이블은 &quot;Custom Chart Tables&quot;라는 섹션에 표시됩니다. 기본값은 `False`입니다.
...
* `ValueError`:  `probs`와 `preds`가 모두 제공되었거나, 예측값과 실제 레이블의 개수가 서로 다른 경우 발생합니다. 고유한 예측 클래스의 수가 클래스 이름의 수를 초과하거나, 고유한 실제 레이블의 수가 클래스 이름의 수를 초과하는 경우에도 발생합니다.
* `wandb.Error`:  NumPy가 설치되어 있지 않은 경우 발생합니다.

**Examples:**
야생 동물 분류를 위해 랜덤 확률을 사용해 혼동 행렬을 로그하는 예시:
...
시뮬레이션된 모델 예측과 85% 정확도를 사용해 혼동 행렬을 로깅하기:
...
이 예제에서는 85% 정확도의 예측값을 시뮬레이션하여 혼동 행렬을 생성합니다.
