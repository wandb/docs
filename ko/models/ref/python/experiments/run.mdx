...
```python
with wandb.init() as run:
     # 예시: X축으로 사용하기 위해 "epoch" 메트릭을 로그합니다.
...
* `Python objects including`:  `int`, `float`, `string`, 모든 `wandb.data_types`, 직렬화 가능한 Python 객체의 목록, 튜플, NumPy 배열, 그리고 이 구조를 따르는 다른 `dict`
* `step`:  log할 step 번호입니다. `None`이면 암시적으로 자동 증가하는 step이 사용됩니다. 설명의 notes를 참조하세요.
* `commit`:  true이면 step을 완료하고 업로드합니다. false이면 step의 데이터를 누적합니다. 설명의 notes를 참조하세요. `step`이 `None`이면 기본값은 `commit=True`이고, 그렇지 않으면 기본값은 `commit=False`입니다.

**예시:**
더 많고 자세한 예시는 [logging 가이드](https://docs.wandb.com/guides/track/log)를 참조하세요.
