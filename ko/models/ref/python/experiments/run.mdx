...
여기에는 `run.save()`로 저장된 파일과 `run.log()`에 전달된 특정 데이터 유형에 대해 자동으로
생성된 파일이 포함됩니다. run의 모든 데이터가 저장된
디렉터리는 `run.sync_dir`를 참조하세요.
...
run 경로에는 entity, 프로젝트, run ID가 포함되며, 형식은
...
### <kbd>속성</kbd> start_time

run이 시작된 시점의 Unix 타임스탬프(초 단위)입니다.

### <kbd>속성</kbd> step

다음 `log()` Call에 사용될 W&amp;B step입니다.

`mode="shared"` run에서는 예외가 발생합니다.
...
run의 모든 데이터가 저장된 디렉터리입니다.

이 디렉터리를 `wandb sync`에 전달하여 run을 업로드하거나 다시 업로드할 수 있습니다.
...
* `summary`: summary에 추가할 집계 메트릭을 지정합니다. 지원되는 집계에는 &quot;min&quot;, &quot;max&quot;, &quot;mean&quot;, &quot;last&quot;, &quot;first&quot;, &quot;copy&quot;, &quot;none&quot;이 포함됩니다. &quot;none&quot;을 지정하면 summary가 생성되지 않습니다. &quot;copy&quot;는 사용 중단되었으므로 사용하지 않는 것이 좋습니다.
...
* `artifact_or_path`: 이 artifact의 콘텐츠 경로입니다.
  다음 형식을 사용할 수 있습니다:
  * `/local/directory`
  * `/local/directory/file.txt`
  * `s3://bucket/path`
    또는 `wandb.Artifact`를 호출해 생성한 Artifact 객체를 전달할 수도 있습니다.
* `name`: artifact 이름입니다. entity/프로젝트 접두사를 붙일 수 있습니다.
...
이력에 저장하고, 이러한 메트릭의 summary 값을 업데이트합니다.
...
차트의 X축에는 어떤 metric이든 사용할 수 있습니다.
자세한 내용은 [맞춤형 log 축](https://docs.wandb.ai/models/track/log/customize-logging-axes)을 참조하세요.
...
`step` 및 `commit` 매개변수를 사용하면 여러 번의 `wandb.Run.log()` 호출로 동일한 step에 log할 수 있습니다.
다음은 모두 동일합니다:
...
* `data`: `str` 키와 직렬화할 수 있는 Python 객체 값을 갖는 `dict`입니다. 여기에는 `int`, `float`, `string`, `wandb.data_types`의 모든 유형, 직렬화할 수 있는 Python 객체의 목록, 튜플 및 NumPy 배열, 그리고 이 구조의 다른 `dict`가 포함됩니다.
...
* `artifact_or_path`: (str 또는 Artifact) 이 artifact의 콘텐츠 경로입니다.
  다음 형식 중 하나일 수 있습니다:
  * `/local/directory`
  * `/local/directory/file.txt`
  * `s3://bucket/path`
...
* `include_fn`: 파일 경로와 (선택적으로) 루트 경로를 받아, 포함해야 하면 True를 반환하고 그렇지 않으면 False를 반환하는 callable입니다. 기본값은 `lambda path, root: path.endswith(".py")`입니다.
...
* `glob`: `glob_str`를 glob 패턴으로 처리할지 여부입니다. 이전 버전과의 호환성을 위해 기본값은 `True`입니다. 이름에 `[`, `]`, `*`, `?`와 같은 glob 메타문자가 포함되어 있고 이를 패턴으로 해석하지 않으려는 경우 `False`로 설정하면 `glob_str`를 리터럴 경로로 처리합니다.
...
* `artifact_or_path`: 이 artifact의 콘텐츠 경로입니다.
  ...
  * `/local/directory/file.txt`
  * `s3://bucket/path`
* `name`: artifact 이름입니다. &quot;entity/프로젝트&quot; 접두사를 붙일 수 있습니다. 지정하지 않으면
...
* `artifact_or_name`: 사용할 artifact의 이름입니다. artifact가 로깅된 프로젝트 이름
...
* `log_freq`: 그라디언트와 파라미터를 로깅하는 빈도(배치 단위)입니다. (기본값=1000)
...
* `text`: 작성할 텍스트입니다. 끝에 개행 문자가 없으면 자동으로 추가됩니다.