---
description: W&B SDK가 run 중에 자동으로 로깅하는 CPU, GPU, 메모리, 디스크, 네트워크 시스템 메트릭에 대한 레퍼런스입니다.
title: "시스템 메트릭 레퍼런스"
aliases:
...
<div id="view-system-metrics">
  ## 시스템 메트릭 보기 {#view-system-metrics}
</div>

W&amp;B App 또는 `wandb leet` 터미널 UI를 사용해 시스템 메트릭을 확인하고 모니터링할 수 있습니다.
...
<Tabs>
  ...
  <Tab title="LEET">
    `wandb leet` 터미널 UI를 사용해 터미널에서 로컬 run의 시스템 메트릭을 보려면 다음 단계를 따르세요.
    ...
    run 경로를 지정하지 않으면 LEET는 기본적으로 워크스페이스 뷰를 엽니다. `2`를 누르면 강조 표시된 run의 시스템 메트릭이 표시됩니다. run 디렉터리나 `.wandb` 파일을 지정하면 단일 run 뷰가 열리며, 기본적으로 오른쪽 사이드바에 시스템 메트릭이 표시됩니다.

    시스템 메트릭 패널에는 다음 정보가 표시됩니다.
    ...
    * `y`: 포커스된 차트를 선형, 로그, 그리고 백분율 메트릭의 경우 버킷 heatmap 모드 간에 전환합니다.
    * `n` 및 `N`: 포커스된 패널의 페이지 간을 이동합니다.
    * `q` 또는 `ctrl+c`: 종료합니다.

    run 없이 현재 머신을 모니터링하려면 독립 실행형 system monitor(SYMON)를 실행하세요.

    ```bash
    wandb leet symon --interval 2s
    ```

    전체 가이드는 [LEET 터미널 UI](/ko/models/app/leet-tui)를, 명령 구문은 [`wandb leet`](/ko/models/ref/cli/wandb-leet) CLI 레퍼런스를 참조하세요.
  </Tab>
</Tabs>
...
<div id="process-cpu-percent-cpu">
  ### 프로세스 CPU 비율(CPU) {#process-cpu-percent-cpu}
</div>
...
<div id="process-cpu-threads">
  ### 프로세스 CPU 스레드 {#process-cpu-threads}
</div>
...
[디스크 In](#disk-in)과 마찬가지로, 첫 번째 샘플을 수집할 때 초기 디스크 쓰기 바이트 수를 기록합니다. 이후 샘플에서는 현재 쓰기 바이트 수와 초기값의 차이를 계산합니다.
...
<div id="process-memory-rss">
  ### 프로세스 메모리 RSS {#process-memory-rss}
</div>
...
<div id="process-memory-percent">
  ### 프로세스 메모리 백분율 {#process-memory-percent}
</div>
...
### 네트워크 수신량 {#network-received}

네트워크를 통해 수신된 총 바이트 수를 나타냅니다.
[네트워크 전송](#network-sent)와 마찬가지로, 메트릭이 처음 초기화될 때 초기 수신 바이트 수가 기록됩니다. 이후 샘플에서는 현재 수신된 바이트 수와 초기값의 차이를 계산합니다.
...
<div id="gpu-memory-utilization">
  ### GPU 메모리 사용량 {#gpu-memory-utilization}
</div>
...
<div id="gpu-memory-allocated">
  ### GPU 메모리 할당 {#gpu-memory-allocated}
</div>
...
<div id="gpu-memory-allocated-bytes">
  ### GPU 메모리 할당 바이트 수 {#gpu-memory-allocated-bytes}
</div>
...
<div id="gpu-power-usage-watts">
  ### GPU 전력 사용량(와트) {#gpu-power-usage-watts}
</div>
...
<div id="gpu-power-usage-percent">
  ### GPU 전력 사용 비율 {#gpu-power-usage-percent}
</div>
...
<div id="gpu-sm-clock-speed">
  ### GPU SM 클록 속도 {#gpu-sm-clock-speed}
</div>
...
<div id="gpu-memory-clock-speed">
  ### GPU 메모리 클록 속도 {#gpu-memory-clock-speed}
</div>
...
<div id="gpu-graphics-clock-speed">
  ### GPU 그래픽 클록 속도 {#gpu-graphics-clock-speed}
</div>
...
<div id="gpu-corrected-memory-errors">
  ### GPU 정정된 메모리 오류 {#gpu-corrected-memory-errors}
</div>
...
<div id="gpu-uncorrected-memory-errors">
  ### GPU 정정되지 않은 메모리 오류 {#gpu-uncorrected-memory-errors}
</div>
...
<div id="gpu-encoder-utilization">
  ### GPU 인코더 사용량 {#gpu-encoder-utilization}
</div>
...
<div id="amd-gpu-utilization">
  ### AMD GPU 사용량 {#amd-gpu-utilization}
</div>
...
<div id="amd-gpu-memory-allocated">
  ### AMD GPU 메모리 할당 {#amd-gpu-memory-allocated}
</div>
...
<div id="amd-gpu-temperature">
  ### AMD GPU 온도 {#amd-gpu-temperature}
</div>
...
<div id="amd-gpu-power-usage-watts">
  ### AMD GPU 전력 사용량(와트) {#amd-gpu-power-usage-watts}
</div>
...
<div id="amd-gpu-power-usage-percent">
  ### AMD GPU 전력 사용 비율 {#amd-gpu-power-usage-percent}
</div>
...
<div id="apple-arm-mac-gpu">
  ## Apple ARM Mac GPU {#apple-arm-mac-gpu}
</div>

<div id="apple-gpu-utilization">
  ### Apple GPU 사용량 {#apple-gpu-utilization}
</div>
...
<div id="apple-gpu-memory-allocated">
  ### Apple GPU 메모리 할당 {#apple-gpu-memory-allocated}
</div>
...
<div id="apple-gpu-temperature">
  ### Apple GPU 온도 {#apple-gpu-temperature}
</div>
...
<div id="apple-gpu-power-usage-watts">
  ### Apple GPU 전력 사용량(와트) {#apple-gpu-power-usage-watts}
</div>
...
<div id="apple-gpu-power-usage-percent">
  ### Apple GPU 전력 사용 비율 {#apple-gpu-power-usage-percent}
</div>
...
<div id="ipu-device-metrics">
  ### IPU 디바이스 메트릭 {#ipu-device-metrics}
</div>
...
<div id="tpu-compute-metrics">
  ### TPU 컴퓨팅 메트릭 {#tpu-compute-metrics}
</div>
...
<div id="tpu-memory-metrics">
  ### TPU 메모리 메트릭 {#tpu-memory-metrics}
</div>
...
<div id="tpu-interconnect-health">
  ### TPU 인터커넥트 상태 {#tpu-interconnect-health}
</div>
...
<div id="tpu-transfer-latency">
  ### TPU 전송 지연 시간 {#tpu-transfer-latency}
</div>
...
<div id="tpu-collective-communication">
  ### TPU 집단 통신 {#tpu-collective-communication}
</div>
...
<div id="tpu-network-metrics">
  ### TPU 네트워크 메트릭 {#tpu-network-metrics}
</div>
...
<div id="tpu-hlo-execution-metrics">
  ### TPU HLO 실행 메트릭 {#tpu-hlo-execution-metrics}
</div>
...
<div id="trainium-neuron-core-utilization">
  ### Trainium Neuron Core 사용량 {#trainium-neuron-core-utilization}
</div>
...
<div id="trainium-host-memory-usage-total">
  ### Trainium 호스트 총 메모리 사용량 {#trainium-host-memory-usage-total}
</div>
...
<div id="trainium-neuron-device-total-memory-usage">
  ### Trainium Neuron 디바이스 총 메모리 사용량 {#trainium-neuron-device-total-memory-usage}
</div>
...
<div id="trainium-host-memory-usage-breakdown">
  ### Trainium 호스트 메모리 사용량 분석: {#trainium-host-memory-usage-breakdown}
</div>
...
<div id="trainium-neuron-core-memory-usage-breakdown">
  ### Trainium Neuron Core 메모리 사용량 세부 내역 {#trainium-neuron-core-memory-usage-breakdown}
</div>
