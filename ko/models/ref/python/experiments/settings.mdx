...
* `capture_loggers` (Optional[Dict[str, str]]): run의 Logs 탭에 캡처할 Python 로거 이름입니다.
  로거 이름을 최소 로그 수준에 매핑한 값입니다. 이 값을 설정하면 wandb는
  이름이 지정된 각 로거에 `logging.Handler`를 설치하고 run이 종료되면 이를 제거합니다.
  이들 로거에서 발생한 로그 레코드는
...
* `stop_fn` (Optional[Callable]): run을 중지할 때 실행할 콜백입니다.
