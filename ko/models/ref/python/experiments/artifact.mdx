...
에일리어스는 프로그래밍 방식으로 참조할 수 있는 가변 참조입니다. W&B 앱 UI 또는 프로그래밍 방식으로 아티팩트의 에일리어스를 변경할 수 있습니다. 자세한 내용은 [새 아티팩트 버전 생성](/ko/guides/artifacts/create-a-new-artifact-version)을 참조하세요. 
...
아티팩트에 추가하는 파일이나 디렉토리와 달리, 참조는 W&B로 업로드되지 않습니다. 자세한 내용은 [외부 파일 추적](/ko/guides/artifacts/track-external-files)을 참조하세요.
