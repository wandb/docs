---
title: 웹훅 자동화 생성하기
description: "특정 이벤트가 발생할 때 외부 서비스로 HTTP 요청을 보내도록 W&B에서 웹훅 자동화를 생성합니다."
---
...
이 페이지에서는 웹훅 [자동화](/ko/models/automations/)를 생성하는 방법을 설명합니다. Slack 자동화를 생성하려면 [Create a Slack automation](/ko/models/automations/create-automations/slack/)을 대신 참조하세요.

웹훅 자동화를 생성하는 과정은 크게 다음 단계로 이루어집니다:

1. 필요한 경우 액세스 토큰, 비밀번호 또는 SSH 키처럼 자동화에 필요한 각 민감한 문자열에 대해 [W&amp;B 시크릿을 생성](/ko/platform/secrets/)합니다. 시크릿은 **Team Settings**에서 정의합니다.
2. 엔드포인트와 인증 세부 정보를 정의하고 통합에 필요한 시크릿에 대한 액세스 권한을 부여하기 위해 [웹훅을 생성](#create-a-webhook)합니다.
3. 감시할 [이벤트](/ko/models/automations/automation-events/)와 W&amp;B가 전송할 페이로드를 정의하기 위해 [자동화를 생성](#create-an-automation)합니다. 페이로드에 필요한 시크릿에 대한 액세스 권한도 자동화에 부여합니다.
...
W&amp;B App UI를 사용해 대화형으로, 또는 Bash 스크립트를 사용해 프로그래밍 방식으로 웹훅를 문제 해결할 수 있습니다. 새 웹훅를 생성할 때나 기존 웹훅를 수정할 때 웹훅를 점검할 수 있습니다.

W&amp;B에서 `POST` 요청에 사용하는 포맷에 대한 자세한 내용은 **Bash script** 탭을 참조하세요.

<Tabs>
  <Tab title="W&B App UI">
    팀 관리자는 W&amp;B App UI를 사용해 웹훅를 대화형으로 테스트할 수 있습니다.
    ...
    6. **Test webhook**을 클릭합니다. 그러면 W&amp;B App UI 내에서 엔드포인트로부터의 응답을 W&amp;B가 표시합니다.
       <Frame>
         <img src="/images/models/webhook_ui_testing.gif" alt="웹훅를 테스트하는 데모" />
       </Frame>
  ...
  <Tab title="Bash script">
    이 셸 스크립트는 웹훅가 트리거될 때 W&amp;B가 웹훅 자동화로 보내는 요청과 유사한 `POST` 요청을 생성하는 한 가지 방법을 보여줍니다.

    아래 코드를 셸 스크립트에 복사해 붙여넣어 웹훅를 문제 해결하세요. 다음 값은 직접 지정해야 합니다:
    ...
    * `API_ENDPOINT`
    ...
    # HMAC 서명 생성
    # 보안을 위해 W&B는 페이로드와 webhook에 연결된 공유 시크릿으로
    # HMAC SHA-256을 계산한 X-Wandb-Signature를 헤더에 포함합니다.
