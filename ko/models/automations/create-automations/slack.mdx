...
keywords: ["Slack 인테그레이션", "알림", "자동화 이벤트", "레지스트리 자동화", "프로젝트 자동화"]
---
...
## Slack 인테그레이션 추가 {#add-a-slack-integration}
...
2. **Slack channel integrations** 섹션에서 새 Slack 인스턴스를 추가하려면 **Connect Slack**을 클릭합니다. 기존 Slack 인스턴스에 채널을 추가하려면 **New integration**을 클릭합니다.
   ...
   ![Team의 두 Slack 인테그레이션](/images/automations/slack_integrations.png)
...
## Slack 인테그레이션 보기 및 관리 {#view-and-manage-slack-integrations}
...
<Note>
  더 광범위하게 자동화를 적용하려면 전역 **Automations** 허브에서 자동화를 만들고 **Team** 또는 **Organization** 범위를 선택하세요. 단계는 아래에 설명된 단계와 같습니다. 이러한 범위에서 W&amp;B App은 아티팩트 및 collection 이벤트만 지원합니다. [Automations hub](/ko/models/automations#automations-hub)를 참조하세요.
</Note>
...
<Tabs>
  <Tab title="레지스트리">
    ...
    6. **액션 type**을 **Slack notification**으로 설정합니다. Slack 채널을 선택한 다음 **Next step**을 클릭합니다.
  ...
  <Tab title="프로젝트">
    ...
    2. 프로젝트 페이지로 이동해 **Automations** 탭을 클릭한 다음 **Create automation**을 클릭합니다.
       ...
       또는 Workspace의 선형 플롯에서 해당 메트릭에 대한 [run 메트릭 automation](/ko/models/automations/automation-events/#run-events)을 직접 만들 수 있습니다. 패널 위로 마우스를 가져간 다음 패널 상단의 벨 아이콘을 클릭합니다.
    ...
    5. **액션 type**을 **Slack notification**으로 설정합니다. Slack 채널을 선택한 다음 **Next step**을 클릭합니다.
