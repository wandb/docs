...
자동 생성된 구성은 완료한 Runs를 바탕으로 스윕할 값을 추정합니다. 어떤 범위의 하이퍼파라미터를 시도할지 지정하도록 구성을 수정하세요. 스윕을 시작하면 호스팅된 W&B 스윕 서버에서 새 프로세스가 시작됩니다. 이 중앙 집중식 서비스는 에이전트, 즉 트레이닝 작업을 실행하는 머신을 조정합니다.
