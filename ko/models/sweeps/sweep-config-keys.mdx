...
스윕 구성은 중첩된 키-값 쌍으로 구성됩니다. 스윕 구성 내에서 최상위 키를 사용하여 검색할 파라미터 ( [`parameter`](/ko/models/sweeps/sweep-config-keys/#parameters) 키), 파라미터 공간을 검색하는 방법 ( [`method`](/ko/models/sweeps/sweep-config-keys/#method) 키) 등과 같은 스윕 검색의 특성을 정의합니다.
...
스윕 구성을 구성하는 방법에 대한 자세한 내용은 [스윕 구성](/ko/models/sweeps/sweep-config-keys/) 구조를 참조하십시오.
...
#### 랜덤 검색
각 반복에서 분포에 따라 임의의, 정보에 입각하지 않은 하이퍼파라미터 값 집합을 선택합니다. 랜덤 검색은 커맨드라인, Python 스크립트 또는 [W&B 앱 UI](/ko/models/sweeps/sweeps-ui/) 내에서 프로세스를 중지하지 않는 한 영원히 실행됩니다.
...
베이지안 탐색은 커맨드라인, Python 스크립트 또는 [W&B 앱 UI](/ko/models/sweeps/sweeps-ui/) 내에서 프로세스를 중지하지 않는 한 영원히 실행됩니다.
...
<Note>
Hyperband는 몇 분마다 종료할 [W&B run](/ko/models/ref/python/run)을 확인합니다. run 또는 반복이 짧으면 종료 run 타임스탬프가 지정된 brackets와 다를 수 있습니다.
</Note>
