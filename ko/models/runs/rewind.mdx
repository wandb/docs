...
* You must use monotonically increasing steps. This does not work with non-monotonic steps defined with [`define_metric()`]({{< relref "/ref/python/experiments/run#define_metric" >}}) because it disrupts the required chronological order of run history and system metrics.
{{% /alert %}}
...
Use `resume_from` with [`wandb.init()`]({{< relref "/ref/python/functions/init" >}}) to "rewind" a run’s history to a specific step. Specify the name of the run and the step you want to rewind from:
