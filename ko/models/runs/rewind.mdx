...
* 단조롭게 증가하는 단계를 사용해야 합니다. [`define_metric()`](/ko/models/ref/python/run#define_metric)으로 정의된 비단조 단계를 사용하면 run 기록 및 시스템 메트릭의 필수 시간순서가 방해되므로 사용할 수 없습니다.
</Note>
...
`resume_from`과 함께 [`wandb.init()`](/ko/models/ref/python/init)을 사용하여 run 기록을 특정 단계로 "되감습니다". 되감을 run의 이름과 되감을 단계를 지정합니다.
...
되감은 run에서 포크하려면 `wandb.init()`에서 [**`fork_from`**](/ko/models/runs/forking/) 인수를 사용하고 포크할 소스 run ID와 소스 run의 단계를 지정합니다.
