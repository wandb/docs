...
run의 이력을 수정하려면 run을 되감으십시오. run을 되감기하면, W&amp;B는 동일한 run ID를 유지하면서 run의 상태를 지정한 스텝으로 재설정합니다.

<Warning>
  run 되감기 옵션은 현재 비공개 프리뷰 단계이며 활발히 개발 중입니다. Rewind에는 알려진 성능 제한이 있으므로, W&amp;B는 일반적으로 대안으로 [Forking](./forking)을 권장합니다.
...
W&amp;B는 새로 로깅된 이력을 기반으로 되감은 run의 요약 메트릭을 다시 계산합니다. 이는 다음과 같은 동작을 의미합니다:
...
특정 스텝부터 run을 되감고 그 시점부터 새 데이터를 기록할 수 있습니다. 되감기를 시작할 run ID와 스텝을 모두 [`wandb.init()`](/ko/models/ref/python/functions/init)의 `resume_from` 파라미터에 인수로 전달하세요. `resume_from` 파라미터는 `<run ID>?_step=<step>` 형식의 문자열을 받습니다. 여기서 `<run ID>`는 되감으려는 run의 run ID이고 `<step>`은 되감기를 시작할 스텝입니다.

예를 들어 300 스텝 동안 직선 형태의 데이터를 기록한다고 가정해 보겠습니다:
...
프로젝트의 워크스페이스에서는 step 0부터 step 300까지의 선 그래프를 볼 수 있습니다:

<img src="/images/runs/rewind_run_original.png" alt="원래 run의 선 그래프" />
...
* 선 플롯은 step 0부터 step 200까지의 원래 선형 추세를 보여주며, 새로운 미세한 물결 패턴은 step 250부터 시작합니다(왼쪽 이미지).
