...
[`wandb.init()`](/ko/models/ref/python/functions/init)을 사용하여 W&B Run 을 초기화합니다. 
...
동일한 프로세스에서 여러 개의 활성 Runs 를 관리하는 방법은 [한 프로세스 내의 여러 runs](/ko/models/runs/initialize-run#multiple-runs-in-one-process)를 참조하세요.
...
|----------------|----------------|----------------| -----------------|
| `create_new` | 기존의 활성 runs 를 종료하지 않고 `wandb.init()`으로 새 run 을 생성합니다. W&B는 전역 `wandb.Run`을 새 runs 로 자동으로 전환하지 않습니다. 각 run 오브젝트를 직접 유지해야 합니다. 자세한 내용은 아래의 [한 프로세스 내의 여러 runs 예시](/ko/models/runs/initialize-run/#example-multiple-runs-in-one-process)를 참조하세요. | 예 | 동시 프로세스를 생성하고 관리하는 데 이상적입니다. 예를 들어, "보조" runs 를 시작하거나 종료하는 동안 활성 상태를 유지하는 "기본" run 이 있는 경우입니다. |
...
<Note>
W&B는 Hugging Face Trainer, Keras 콜백, PyTorch Lightning과 같이 단일 전역 run 을 가정하는 [W&B 인테그레이션](/ko/models/integrations)에 대해 `create_new` 모드를 지원하지 않습니다. 이러한 인테그레이션을 사용하는 경우 각 하위 실험을 별도의 프로세스에서 실행해야 합니다.
</Note>
