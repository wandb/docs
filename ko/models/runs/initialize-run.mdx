---
title: run 초기화
description: "실험 추적을 시작하기 위해 wandb.init()으로 W&B run을 초기화하고, 하나의 프로세스에서 동시 run을 처리하는 방법도 설명합니다."
---
...
다음 예제 코드 스니펫은 W&amp;B Python SDK를 import하고 run을 초기화하는 방법을 보여줍니다.
...
출력 결과를 보면 W&amp;B가 엔티티 `nico` 아래의 프로젝트 `awesome-project`에 run `exalted-darkness-6`을 로깅한다는 것을 알 수 있습니다. `pgbn9y21`은 이 run에 대해 W&amp;B가 생성한 고유한 run ID입니다.
