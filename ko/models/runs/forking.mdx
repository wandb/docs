...
run을 포크하면 W&amp;B는 소스 run의 [고유 ID](/ko/models/runs/run-identifiers#unique-run-identifiers)와 지정한 step을 사용해 새 run을 생성합니다. 소스 run의 요약 메트릭은 포크된 run으로 복사됩니다. 포크된 run은 지정한 step까지의 소스 run의 모든 기록과 파일을 공유합니다.
