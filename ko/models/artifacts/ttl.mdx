...
## 자동 생성된 Artifacts {#auto-generated-artifacts}
...
## TTL 정책을 설정하고 편집할 수 있는 사용자 지정 {#define-who-can-edit-and-set-ttl-policies}
...
### 아티팩트를 생성한 후 TTL 정책 설정 또는 편집 {#set-or-edit-a-ttl-policy-after-you-create-an-artifact}
...
### 팀의 기본 TTL 정책 설정 {#set-default-ttl-policies-for-a-team}
...
### run 외부에서 TTL 정책 설정 {#set-a-ttl-policy-outside-of-a-run}
...
## TTL 정책 비활성화 {#deactivate-a-ttl-policy}
...
## 연결된 아티팩트 버전의 TTL 정책 {#ttl-policies-for-linked-artifact-versions}

W&amp;B는 프로덕션 워크플로에서 사용되는 아티팩트가 만료되지 않도록 연결된 아티팩트 버전의 TTL 정책을 비활성화합니다.

아티팩트 버전을 레지스트리 컬렉션에 연결하면 W&amp;B는 해당 TTL 정책을 비활성화하고 TTL을 `None`으로 설정합니다. 이후 아티팩트 버전의 연결을 해제해도 W&amp;B는 이전 TTL 정책이나 기간을 복원하지 않습니다. 연결을 해제한 후에도 아티팩트 버전에 TTL 정책이 필요하면 새 TTL 정책을 설정하세요.