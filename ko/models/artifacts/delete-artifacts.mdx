...
<Note>
  entity, 프로젝트 또는 artifact collection을 삭제하면 이 페이지에 설명된 artifact 삭제 프로세스가 트리거됩니다. run을 삭제하면서 연결된 artifact도 삭제하도록 선택하면, 해당 artifact는 동일한 소프트 삭제 및 가비지 컬렉션 워크플로를 따릅니다.
</Note>
...
<Tabs>
  <Tab title="W&B App" value="ui">
    ...
    4. 삭제하려는 artifact 버전 옆의 가로 점 세 개(`...`)를 클릭합니다.
    5. 드롭다운에서 **버전 삭제**를 선택합니다.
  </Tab>

  <Tab title="W&B Python SDK" value="sdk">
    [wandb.Artifact.delete()](/ko/models/ref/python/experiments/artifact#delete)를 사용해 프로그래밍 방식으로 artifact 버전을 삭제합니다. artifact의 전체 이름을 지정합니다. 전체 이름은 `<entity>/<project>/<artifact_name>:<version>` 형식입니다. 하나 이상의 alias가 연결되어 있어도 artifact를 삭제하려면 `delete_aliases` 매개변수를 `True`로 설정합니다.
...
<div id="delete-an-artifact-collection">
  ## artifact collection 삭제
</div>

<Tabs>
  <Tab title="W&B App" value="ui">
    artifact collection을 삭제하려면 다음 단계를 따르세요.

    1. 삭제하려는 artifact collection으로 이동합니다.
    2. artifact collection 이름 옆의 가로 점 3개(`...`)를 선택합니다.
    3. 드롭다운 메뉴에서 **Delete**를 선택합니다.
  </Tab>

  <Tab title="W&B Python SDK" value="sdk">
    [wandb.Artifact.delete()](/ko/models/ref/python/experiments/artifact#delete) 방법을 사용해 프로그래밍 방식으로 artifact collection을 삭제합니다.

    `wandb.Api.artifact_collection(name="")`에 artifact collection의 전체 경로를 지정합니다. 전체 경로는 `<entity>/<project>/<artifact_collection_name>`로 구성됩니다.
    ...
    # 경로를 사용해 artifact collection을 조회합니다. 경로는
...
W&amp;B의 공유 클라우드를 사용하는 경우 가비지 컬렉션은 기본적으로 활성화됩니다. W&amp;B를 어떻게 호스팅하는지에 따라 가비지 컬렉션을 활성화하려면 추가 단계가 필요할 수 있습니다. 여기에는 다음이 포함됩니다:
