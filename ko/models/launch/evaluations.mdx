...
이 페이지는 [LLM Evaluation Jobs](/ko/models/launch)에서 카테고리별로 제공하는 평가 벤치마크 목록을 안내합니다.

특정 벤치마크를 실행하려면 팀 관리자가 필요한 API 키를 [팀 범위 secret](/ko/platform/secrets#add-a-secret)으로 추가해야 합니다. 모든 팀 멤버는 평가 job을 구성할 때 이 secret을 지정할 수 있습니다.
