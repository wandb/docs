...
이 페이지에서는 CoreWeave가 관리하는 인프라를 사용하여 W&B Models에 있는 모델 체크포인트에 대해 일련의 평가 벤치마크를 실행하는 [LLM Evaluation Jobs](/ko/models/launch) 사용 방법을 설명합니다. 공개적으로 액세스 가능한 URL에서 서빙되는 호스팅된 API 모델을 평가하려면 [Evaluate an API-hosted model](/ko/models/launch/evaluate-hosted-model)을 대신 참조하세요.

## 사전 요구 사항
1. LLM Evaluation Jobs에 대한 [요구 사항 및 제한 사항](/ko/models/launch#more-details)을 검토하세요.
1. 특정 벤치마크를 실행하려면 Teams 관리자가 필요한 API 키를 [team-scoped secrets](/ko/platform/secrets#add-a-secret)로 추가해야 합니다. 모든 Teams 멤버는 평가 작업을 구성할 때 해당 시크릿을 지정할 수 있습니다. 요구 사항은 [Evaluation model catalog](/ko/models/launch/evaluations)를 참조하세요.
...
1. 평가 결과를 위한 새로운 [W&B Projects](/ko/models/track/project-page)를 생성합니다. 왼쪽 탐색 창에서 **Create new project**를 클릭합니다.
...
1. 특정 벤치마크의 작동 방식과 구체적인 요구 사항을 이해하기 위해 해당 문서를 검토하세요. 편의를 위해 [Available evaluation benchmarks](/ko/models/launch/evaluations) 레퍼런스에 관련 링크가 포함되어 있습니다.
