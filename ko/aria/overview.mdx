...
1. 아무 프로젝트에서나 페이지 오른쪽 상단의 **Ask ARIA**를 클릭합니다.
   <Frame>
...
실제 예시는 [예시 채팅](/ko/aria/overview#example-chats)를 참조하세요. 채팅 관리에 대한 자세한 내용은 [Chat with ARIA](/ko/aria/chat)를 참조하세요.
...
{/*
  ...
  1. ARIA 채팅 창에서 **<Icon icon="bars" iconType="solid" />**를 클릭합니다.
  1. **Customization** 섹션에서 **Project memory**를 클릭합니다. ARIA가 프로젝트에 대해 보유한 모든 메모리가 표시됩니다.
  1. 메모리를 수정하려면 **<Icon icon="pencil" iconType="solid" />**를 클릭합니다. 메모리의 **Name**, **Description** 또는 **Memory**를 업데이트한 후 **Save**를 클릭합니다.
  1. 메모리를 삭제하려면 **<Icon icon="trash" iconType="solid" />**를 클릭합니다.

  ARIA가 새 메모리를 학습할 때까지 기다리지 않고 대화형으로 프로젝트 메모리를 만들 수 있습니다. 예시:

  <AriaChatBubbles
  prompt="차트에 의미 기반 색상을 사용하는 것을 선호한다고 기억해 주세요."
  response="먼저 기존 프로젝트 메모리를 확인하여 이 기본 설정이 중복되지 않고 올바른 주제 아래에 기록되도록 하겠습니다 ... 새 프로젝트 메모리를 저장했습니다: chart-coloring. 이 메모리에는 차트가 임의의 팔레트 순서가 아니라 시리즈/범주의 의미 또는 상태를 기반으로 일관된 의미 기반 색상을 사용해야 한다고 기록되어 있습니다. "
  />
