...
ARIA는 학습한 내용을 기억합니다. 작업하는 동안 ARIA는 프로젝트 범위의 &#95;메모리&#95;를 작성하므로 새 세션을 시작할 때마다 설정, 결론 또는 기본 설정을 처음부터 다시 파악할 필요가 없습니다. 팀에서 로그 스케일 차트를 선호한다고 한 번 알려주면 ARIA가 기억합니다.

각 메모리에는 ARIA가 언제 이를 불러와야 하는지 알 수 있도록 이름과 설명이 있으며, 메모리 텍스트도 포함됩니다.

프로젝트에 액세스할 수 있는 모든 사용자는 메모리를 보고, 편집하거나 삭제할 수 있습니다.

1. ARIA 채팅 창에서 **<Icon icon="bars" iconType="solid" />**를 클릭합니다.
2. **Customization** 섹션에서 **Project memory**를 클릭합니다. ARIA가 프로젝트에 대해 저장한 각 메모리가 표시됩니다.
3. 메모리를 편집하려면 **<Icon icon="pencil" iconType="solid" />**를 클릭합니다. 메모리의 **Name**, **Description** 또는 **Memory**를 업데이트한 다음 **Save**를 클릭합니다.
4. 메모리를 삭제하려면 **<Icon icon="trash" iconType="solid" />**를 클릭합니다.

ARIA가 새 메모리를 학습할 때까지 기다리지 않고 대화형으로 프로젝트 메모리를 만들 수 있습니다. 예:

<AriaChatBubbles prompt="차트에는 의미론적 색상 지정을 선호한다는 점을 기억해 주세요." response="먼저 기존 프로젝트 메모리를 확인하여 이 기본 설정이 중복되지 않고 적절한 주제 아래에 기록되도록 하겠습니다 ... 새 프로젝트 메모리를 저장했습니다: chart-coloring. 이 메모리에는 차트가 임의의 팔레트 순서 대신 계열/범주의 의미 또는 상태에 따라 일관된 의미론적 색상을 사용해야 한다고 기록되어 있습니다. " />
