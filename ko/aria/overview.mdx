...
<Note>
  ...
  ARIA는 개인 entity가 아닌 팀 프로젝트에서 작동합니다. 조직 관리자도 **Smart features**를 활성화해야 합니다. [Governance and security](/ko/aria/governance)를 참조하세요.
</Note>
...
* 실패한 run을 진단하거나 새로운 모델 버전을 검토하는 등 [W&amp;B의 이벤트에 자동으로 대응합니다](/ko/models/automations/create-automations/aria).
...
## 작업 중인 위치에서 ARIA 시작하기 {#start-aria-from-where-you-work}

페이지 상단의 **Ask ARIA** 외에도 W&amp;B 곳곳에 있는 버튼으로 ARIA를 시작할 수 있습니다. 이러한 버튼은 대부분 메시지 상자에 힌트가 입력된 새 채팅을 엽니다. 원하는 내용을 설명한 다음 메시지를 보내면 됩니다.

| 위치                                       | 버튼                                                      | ARIA가 도와주는 작업                                                                                                                    |
| ---------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| 크래시되거나 실패한 run의 페이지                      | **Debug with ARIA**                                     | run이 크래시되거나 실패한 이유를 설명합니다. 이 버튼을 누르면 새 채팅이 시작되고 바로 ARIA에 질문합니다.                                                                  |
| Workspace **Runs** table 툴바              | **Show most useful columns with ARIA**                  | **Runs** table의 열을 표시, 숨김, 고정, 순서 변경, 크기 조정합니다.                                                                                  |
| **Runs** table의 **Manage columns** 대화 상자 | **Manage columns with ARIA**                            | **Runs** table의 열을 표시, 숨김, 고정, 순서 변경, 크기 조정합니다.                                                                                  |
| **Runs** table의 **Filter** 메뉴            | **Describe the filters you want ARIA to create** 텍스트 필드 | **Runs** table을 필터링합니다. 원하는 run에 대한 설명을 입력하고 **Enter** 키를 누르세요. ARIA가 **Run Filter**를 제안합니다. 채팅에서 제안된 필터를 클릭하면 workspace에 적용됩니다. |
| 저장된 뷰 메뉴                                 | **Ask ARIA**                                            | workspace의 저장된 뷰를 만듭니다.                                                                                                          |
| 프로젝트의 **Reports** 탭                      | **Draft report with ARIA**                              | 새 report를 만듭니다.                                                                                                                  |
| 본인이 소유한 report                           | **Edit with ARIA**                                      | report를 수정합니다. ARIA는 변경 사항을 기존 report에 저장합니다.                                                                                    |

특정 run, workspace 패널 또는 W&amp;B Weave 트레이스나 평가에 대한 컨텍스트를 ARIA에 제공하려면 [대화에 컨텍스트 추가하기](/ko/aria/chat#add-context-to-a-conversation)를 참조하세요.

## ARIA의 진행 상황 확인 {#follow-arias-progress}

ARIA는 작업 중에 응답을 스트림으로 제공하므로 완료된 답변을 기다리지 않고 진행 상황을 step별로 확인할 수 있습니다. 질문의 복잡도에 따라 ARIA가 완료하는 데 잠시 시간이 걸릴 수 있습니다. ARIA가 작업하는 동안 채팅 창을 닫고 나중에 다시 와서 응답을 확인하거나, 기다리지 않고 [다음 메시지를 큐에 추가](/ko/aria/chat#queue-messages)할 수 있습니다. ARIA에 더 많은 정보가 필요하면 추가 설명을 요청합니다.

컨텍스트를 유지할 수 있도록 ARIA는 선형 플롯과 막대 차트를 채팅에서 바로 렌더링하며, 전체 화면 뷰도 지원합니다. Workspace의 **Runs** 탭에서 run에 마우스를 올리면 ARIA에서 해당 run의 플롯이 강조 표시되고, ARIA에서 패널에 마우스를 올리면 **Runs** 탭에서 해당 run이 강조 표시됩니다.

<Frame>![ARIA 선형 플롯 상호작용을 보여주는 애니메이션](/images/agent/aria-line-plot-interactions.gif)</Frame>
...
## 프로젝트 메모리 보기 및 관리 {#view-and-manage-project-memories}

ARIA는 학습한 내용을 기억합니다. 작업하는 동안 ARIA는 프로젝트 범위의 &#95;메모리&#95;를 작성하므로, 새 세션마다 설정, 결론 또는 기본 설정을 처음부터 다시 파악할 필요가 없습니다. 팀에서 로그 스케일 차트를 선호한다고 한 번만 알려주면 ARIA가 이를 기억합니다.

각 메모리에는 ARIA가 언제 불러와야 하는지 알 수 있도록 이름과 설명, 그리고 메모리 텍스트 자체가 포함됩니다.

프로젝트를 소유한 팀의 모든 구성원은 메모리를 보고, 수정하거나 삭제할 수 있습니다:

1. ARIA 채팅 창에서 **<Icon icon="sidebar-reveal" iconType="solid" />**(**Reveal chat history**)를 클릭하여 채팅 이력 사이드바를 엽니다.
2. **Customize** 섹션에서 **Project memory**를 클릭합니다. ARIA가 프로젝트에 대해 보유한 모든 메모리가 표시됩니다.
3. 메모리를 수정하려면 **<Icon icon="pencil" iconType="solid" />**를 클릭합니다. 메모리의 **Name**, **설명** 또는 **Memory**를 업데이트한 후 **Save**를 클릭합니다.
4. 메모리를 삭제하려면 **<Icon icon="trash" iconType="solid" />**를 클릭합니다.

ARIA가 새 메모리를 학습할 때까지 기다리지 않고 대화형으로 프로젝트 메모리를 만들 수 있습니다. 예시:

<AriaChatBubbles prompt="차트에 의미 기반 색상을 사용하는 것을 선호한다고 기억해 주세요." response="먼저 기존 프로젝트 메모리를 확인하여 이 기본 설정이 중복되지 않고 올바른 주제 아래에 기록되도록 하겠습니다 ... 새 프로젝트 메모리를 저장했습니다: chart-coloring. 이 메모리에는 차트가 임의의 팔레트 순서가 아니라 시리즈/범주의 의미 또는 상태를 기반으로 일관된 의미 기반 색상을 사용해야 한다고 기록되어 있습니다. " />
...
<Accordion title="ARIA의 권장 사항을 바탕으로 실험 실행">
  ...
  ARIA는 W&amp;B Launch를 사용해 샌드박스 환경에서 실험을 실행합니다. 자세한 내용은 [ARIA가 실험을 실행하도록 설정](/ko/aria/autoresearch)을 참조하세요.
</Accordion>
...
<Accordion title="하이퍼파라미터 튜닝 추천 받기">
  ...
  ARIA는 또한 모든 Runs가 SGD optimizer를 사용한다는 점을 파악합니다. 이러한 인사이트를 바탕으로, 모델 성능 개선에 도움이 되는지 확인하기 위해 향후 Runs에서 Adam(Adaptive Moment Estimation)과 같은 다른 optimizer를 시도해 볼 것을 권장합니다.
