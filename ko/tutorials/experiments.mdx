...
1. [W&B run](/ko/models/runs/)을 초기화하고 추적하려는 하이퍼파라미터를 전달합니다.
...
W&B run은 자동으로 [메트릭](/ko/models/runs/#workspace-tab),
시스템 정보,
[하이퍼파라미터](/ko/models/runs/#overview-tab),
[터미널 출력](/ko/models/runs/#logs-tab)을 기록하고
모델 입력 및 출력이 있는 [대화형 테이블](/ko/models/tables/)이 표시됩니다.
...
W&B를 사용하면 저장된 모델 및 모델 체크포인트를 팀 또는 조직의 다른 구성원과 쉽게 공유할 수 있습니다. 팀 외부의 구성원과 모델 및 모델 체크포인트를 공유하는 방법에 대한 자세한 내용은 [W&B Registry](/ko/models/core/registry/)를 참조하세요.
...
Python 코드에서 Slack 또는 이메일로 알림을 보내도록 [W&B Alerts](/ko/models/runs/alert/)를 만듭니다.
...
[여기에서 W&B Alerts에 대한 전체 문서를 찾을 수 있습니다](/ko/models/runs/alert/).
