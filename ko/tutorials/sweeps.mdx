...
1. **스윕 정의:** 검색할 파라미터, 검색 전략, 최적화 메트릭 등을 지정하는 사전 또는 [YAML 파일](/ko/models/sweeps/define-sweep-configuration/)을 만들어 이를 수행합니다.
...
커맨드라인 내에서 스윕을 실행하는 경우 [YAML 파일](/ko/models/sweeps/define-sweep-configuration/)로 스윕 구성을 지정해야 합니다.
</Note>

### 검색 방법 선택

먼저 구성 사전 내에서 하이퍼파라미터 검색 방법을 지정합니다. [선택할 수 있는 세 가지 하이퍼파라미터 검색 전략(Grid, Random, 베이지안 탐색)이 있습니다](/ko/models/sweeps/sweep-config-keys/#method).
...
전체 구성 옵션 목록은 [스윕 구성 옵션](/ko/models/sweeps/sweep-config-keys/)을 참조하십시오.
...
터미널에서 W&B Sweeps를 만드는 방법에 대한 자세한 내용은 [W&B Sweep 연습](/ko/models/sweeps/walkthrough/)을 참조하십시오.
...
* [`wandb.init()`](/ko/models/ref/python/init): 새 W&B run을 초기화합니다. 각 run은 트레이닝 함수를 한 번 실행하는 것입니다.
* [`wandb.config`](/ko/models/track/config/): 실험하려는 하이퍼파라미터로 스윕 구성을 전달합니다.
* [`wandb.log()`](/ko/models/ref/python/log): 각 에포크에 대한 트레이닝 손실을 기록합니다.
