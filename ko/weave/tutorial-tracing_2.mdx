...
LLM 기반 애플리케이션에는 모니터링이 중요한 여러 LLM call과 추가적인 데이터 처리 및 검증 로직이 포함될 수 있습니다. 많은 애플리케이션에서 흔히 볼 수 있는 깊이 중첩된 call 구조에서도, 트레이스하려는 각 함수에 `weave.op()`만 추가하면 Weave가 중첩된 함수 전반에 걸친 부모-자식 관계를 추적합니다.
