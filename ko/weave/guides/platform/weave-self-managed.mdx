...
* **Storage Class**: 영구 볼륨용으로 정상 작동하는 StorageClass(예를 들어 `gp3`, `standard` 또는 `nfs-csi`)
* **S3 버킷**: 적절한 액세스 권한이 설정된, 미리 구성된 S3 또는 S3 호환 버킷
...
```bash
# 실제 이름을 확인하려면 Keeper 서비스 목록 조회
kubectl get svc -n clickhouse | grep keeper

# 이름 패턴을 확인하려면 Keeper 파드 목록 조회
kubectl get pods -n clickhouse -l app=clickhouse-keeper
```
...
Weave가 제대로 작동하는지 확인하려면 간단한 Python 테스트를 만드세요:

```python
import os
import weave

# Self-Managed W&B 인스턴스를 가리키도록 Weave를 설정합니다
...
이를 실행한 후 조직의 traces 페이지에서 W&amp;B UI를 열어 트레이스를 확인하세요.
...
연결에 실패하면 ch-server.yaml의 Keeper 호스트 이름이 실제 Keeper 배포 환경과 일치하지 않을 가능성이 높습니다. 명명 패턴은 4단계의 &quot;Understanding Keeper Naming&quot;을 참조하세요.
