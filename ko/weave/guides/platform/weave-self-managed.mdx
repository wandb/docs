...
## 중요한 설정 참고 사항 {#important-setup-notes}
...
## 자체 관리형 Weave 인스턴스 배포 {#deploy-your-self-managed-weave-instance}
...
### Altinity ClickHouse Operator 배포 {#deploy-the-altinity-clickhouse-operator}
...
#### Altinity Helm 저장소 추가하기 {#add-the-altinity-helm-repository}
...
#### 오퍼레이터 설정 생성 {#create-the-operator-configuration}
...
#### Operator 설치하기 {#install-the-operator}
...
#### 오퍼레이터 설치 확인하기 {#verify-the-operator-installation}
...
#### S3 자격 증명 설정 {#configure-s3-credentials}
...
##### 옵션 A: AWS IAM 역할 사용(IRSA, AWS 환경에서 권장) {#option-a-use-aws-iam-roles-irsa-recommended-for-aws}
...
##### 옵션 B: 액세스 키 사용 {#option-b-use-access-keys}
...
#### Keeper 설정 만들기 {#create-the-keeper-configuration}
...
#### ClickHouse Keeper 리소스 배포 {#deploy-clickhouse-keeper-resources}

```bash
kubectl apply -f ch-keeper.yaml
```

#### Keeper 배포 확인 {#verify-the-keeper-deployment}
...
#### S3 설정 업데이트 {#update-the-s3-configuration}
...
#### 자격 증명 구성(옵션 B만 해당) {#configure-credentials-option-b-only}
...
#### ClickHouse 클러스터 리소스 배포 {#deploy-the-clickhouse-cluster-resources}

```bash
kubectl apply -f ch-server.yaml
```

#### ClickHouse 배포 확인 {#verify-the-clickhouse-deployment}
...
### W&amp;B Platform에서 Weave 활성화 {#enable-weave-in-the-wb-platform}
...
#### ClickHouse 연결 정보 확인 {#gather-clickhouse-connection-information}
...
#### W&amp;B Custom Resource 업데이트 {#update-the-wb-custom-resource}
...
#### 업데이트된 설정 적용하기 {#apply-the-updated-configuration}

```bash
kubectl apply -f wandb-cr.yaml
```

#### Weave Trace 배포 확인 {#verify-the-weave-trace-deployment}
...
#### 데이터베이스 생성 여부 확인 {#verify-database-creation}
...
### Weave가 활성화되어 있는지 확인 {#verify-that-weave-is-enabled}

이 마지막 단계에서는 Weave 라이선스가 적용되어 있고, W&amp;B Console에서 액세스할 수 있으며, 클라이언트 SDK에서 트레이스를 기록할 수 있는지 확인합니다.

#### W&amp;B Console에 접속 {#access-the-wb-console}

웹 브라우저에서 W&amp;B 인스턴스 URL로 이동합니다.

#### Weave 라이선스 상태 확인 {#check-the-weave-license-status}
...
#### Weave 기능 테스트 {#test-weave-functionality}
...
### ClickHouse Keeper 문제 {#clickhouse-keeper-issues}
...
### ClickHouse 서버 문제 {#clickhouse-server-issues}
...
### Weave Trace 문제 {#weave-trace-issues}
...
### 최소 프로덕션 설정 {#minimum-production-setup}
...
### 권장 프로덕션 설정 {#recommended-production-setup}
...
### ClickHouse 스케일링 {#scale-clickhouse}
...
### 다른 ClickHouse 버전 사용하기 {#use-a-different-clickhouse-version}
...
<Warning>
  ClickHouse Server를 업그레이드할 때는 ClickHouse Keeper도 호환되는 버전으로 업그레이드하세요. W&amp;B Self-Managed deployment에서 ClickHouse 버전을 변경하기 전에 [업그레이드를 위한 ClickHouse 호환성](/ko/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) 및 [지원되는 W&amp;B Server 릴리스](/ko/release-notes/server-releases) 페이지를 검토하세요.
</Warning>

### ClickHouse 모니터링 {#monitor-clickhouse}
...
<Note>
  Weave는 지원되는 ClickHouse 버전이 필요합니다. ClickHouse 또는 W&amp;B Server를 업그레이드하기 전에 [업그레이드를 위한 ClickHouse 호환성](/ko/platform/hosting/self-managed/operator#clickhouse-compatibility-for-upgrades) 및 [지원되는 W&amp;B Server 릴리스](/ko/release-notes/server-releases)를 참조하세요. ClickHouse Server와 ClickHouse Keeper는 함께 업그레이드하세요.
</Note>

### ClickHouse Operator 업그레이드 {#upgrade-the-clickhouse-operator}
...
### ClickHouse Server 업그레이드 {#upgrade-clickhouse-server}
...
### Weave Trace 업그레이드 {#upgrade-weave-trace}
