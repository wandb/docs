...
&#95;Evaluation 기반 LLM 애플리케이션 개발&#95;은 일관되고 엄선된 예시를 사용해 동작을 측정함으로써 LLM 애플리케이션을 지속적으로 개선할 수 있도록 도와줍니다.
...
## `Evaluation` 객체 만들기 {#create-an-evaluation-object}
...
### 선택 사항: 맞춤형 이름 지정 {#optional-custom-naming}
...
#### `Evaluation` 객체 이름 지정 {#name-the-evaluation-object}
...
#### 개별 평가 run 이름 지정 {#name-individual-evaluation-runs}
...
## 테스트 예제용 데이터셋 정의 {#define-a-dataset-of-test-examples}
...
## 채점 함수를 정의합니다 {#define-scoring-functions}
...
### 선택: 맞춤형 `Scorer` 클래스를 정의합니다 {#optional-define-a-custom-scorer-class}
...
## 평가할 모델 또는 함수 정의하기 {#define-a-model-or-function-to-evaluate}
...
### 선택 사항: 평가할 함수 정의하기 {#optional-define-a-function-to-evaluate}
...
### 선택 사항: 여러 번 실행 {#optional-run-multiple-trials}
...
### 선택 사항: 여러 번 실행 {#optional-run-multiple-trials-2}
...
## 전체 평가 코드 예시 {#full-evaluation-code-example}
...
## 고급 평가 사용법 {#advanced-evaluation-usage}

다음 섹션에서는 데이터셋 전처리, 서드파티 데이터셋 인테그레이션, 저장된 UI 뷰, 명령형 로깅 API 등 더 복잡한 평가 워크플로를 위한 선택 기능을 다룹니다.

### 평가 전에 데이터셋 행 형식 맞추기 {#format-dataset-rows-before-evaluating}
...
### 평가에서 HuggingFace 데이터셋 사용하기 {#use-huggingface-datasets-with-evaluations}
...
### 명령형 평가 (`EvaluationLogger`) {#imperative-evaluations-evaluationlogger}

더 유연한 평가 프레임워크를 선호한다면 Weave의 [`EvaluationLogger`](../evaluation/evaluation_logger)를 참조하세요. `EvaluationLogger`는 Python과 TypeScript 모두에서 사용할 수 있으며, 복잡한 워크플로에 더 유연하게 대응할 수 있습니다. 반면 표준 평가 프레임워크는 더 체계적인 구조와 가이드를 제공합니다.