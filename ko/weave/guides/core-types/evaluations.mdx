...
<Tabs>
  <Tab title="Python">
    `Model`을 평가하려면 `Evaluation`으로 `.evaluate()`를 호출하세요. `Models`는 실험해 보고 Weave에서 캡처하려는 매개변수가 있을 때 사용합니다.
    ...
    이 코드는 각 예제에 대해 `predict`를 실행하고, 각 채점 함수로 출력에 점수를 매깁니다.
  ...
  <Tab title="TypeScript">
    ...
    이 코드는 각 예제에 대해 `myModel`을 실행하고, 각 채점 함수로 출력에 점수를 매깁니다.
  </Tab>
</Tabs>
