...
description: "평가 메트릭을 기준으로 서로 다른 모델 버전을 비교하고 순위를 매깁니다"
---

Weave &#95;Leaderboards&#95;를 사용하면 여러 메트릭 전반에서 여러 모델을 평가하고 비교하여 정확도, 생성 품질, 지연 시간 또는 맞춤형 평가 로직을 측정할 수 있습니다. 리더보드는 중앙 위치에서 모델 성능을 시각화하고, 시간에 따른 변화를 추적하며, 팀 전체 벤치마크를 맞추는 데 도움을 줍니다.
...
<Note>
  리더보드 생성은 Weave UI와 Weave Python SDK에서만 가능합니다. TypeScript 사용자는 [Weave UI](#using-the-ui)를 사용하여 리더보드를 생성하고 관리할 수 있습니다.
</Note>
...
[Weave UI](#ui) 또는 [프로그래밍 방식](#python)을 통해 리더보드를 생성할 수 있습니다.
...
다음 예제는 Weave 평가를 사용해 맞춤형 메트릭으로 공유 데이터셋에서 세 개의 요약 모델을 비교하는 리더보드를 생성합니다. 이 예제에서는 작은 벤치마크를 만들고, 각 모델을 평가한 뒤 [Jaccard similarity](https://www.learndatasci.com/glossary/jaccard-similarity/)로 각 모델에 점수를 매기고, 결과를 Weave 리더보드에 게시합니다.
