---
title: "미디어 로깅"
description: "트레이스에서 반환되는 이미지, 비디오 등의 미디어를 로깅합니다."
---

W&amp;B Weave는 비디오, 이미지, 오디오 클립, PDF, CSV 데이터, HTML을 표시하는 전용 함수를 포함해 다양한 콘텐츠 유형의 로깅 및 표시를 지원합니다. 이 가이드에서는 각 미디어 유형을 로깅하고 표시하는 기본 및 고급 예시를 제공합니다.
...
<Tabs>
  <Tab title="Python">
    이 가이드의 예시에서는 어노테이션을 사용합니다. 미디어 로깅을 시작하는 가장 간단한 방법이기 때문에 어노테이션 사용을 권장합니다. 더 고급 설정은 [Content API 섹션](#using-the-contents-api)을 참조하세요.

    Weave에 미디어를 로깅하려면 ops의 입력 또는 반환 타입으로 `Annotated[bytes, Content]` 또는 `Annotated[str, Content]`와 같은 유형 어노테이션을 추가하세요. 경로 인수를 `Annotated[str, Content]`로 어노테이션하면 Weave가 해당 미디어를 트레이스 내에서 자동으로 열어 감지하고 표시합니다.
  </Tab>
  ...
  <Tab title="TypeScript">
    ...
    * `weave.weaveAudio({ data: Buffer })` - 오디오용(WAV 형식)
    ...
    <Note>
      TypeScript SDK는 현재 이미지와 오디오 로깅만 지원합니다. 비디오, 문서 또는 HTML을 로깅하려면 대신 Weave Python SDK를 사용하세요.
    </Note>
  </Tab>
</Tabs>
...
다음 예시는 이미지를 생성하고 Weave UI에 로깅하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    `Annotated[bytes, Content]` 유형으로 함수를, `Annotated[str, Content]`로 파일 경로를 어노테이션해 이미지를 로깅합니다.

    다음 예시에서는 기본 이미지를 그린 다음 `Content` 어노테이션을 사용해 Weave에 로깅합니다:
    ...
    Weave는 이미지를 로깅하고 이미지를 볼 수 있는 트레이스로 연결되는 링크를 반환합니다.

    ### 고급 예시: DALL-E로 이미지를 생성하고 Weave에 로깅하기

    다음 예시는 고양이 그림을 생성하고 이를 Weave에 로깅합니다:
    ...
    ### 고급 예시: 로깅하기 전에 큰 이미지 크기 조정

    UI 렌더링 비용과 저장소 영향을 줄이기 위해 로깅하기 전에 이미지 크기를 조정하면 유용할 수 있습니다. `@weave.op`에서 `postprocess_output`을 사용하여 이미지 크기를 조정할 수 있습니다.
    ...
    Weave는 크기 조정된 이미지를 로깅하고, 이미지를 볼 수 있는 트레이스 링크를 반환합니다.
  </Tab>

  <Tab title="TypeScript">
    다음 예시에서는 `weaveImage` 함수를 사용해 이미지를 로깅합니다:
    ...
    Weave가 이미지를 로깅하고, 이미지를 볼 수 있는 트레이스에 대한 링크를 반환합니다.

    ### 고급 예시: OpenAI DALL-E API를 통해 생성된 이미지 로깅하기

    다음 예시는 `weaveImage` 함수를 사용하여 OpenAI DALL-E API를 통해 생성된 이미지를 로깅합니다:
...
다음 예시는 Weave UI에 비디오를 생성하고 로깅하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    함수에 `Annotated[bytes, Content]` 타입 어노테이션을 지정해 비디오를 로깅할 수 있습니다. Weave는 `mp4` 비디오를 자동으로 처리합니다. 간단한 예시는 다음과 같습니다:
    ...
    Weave는 비디오를 로깅하고, 비디오를 볼 수 있는 트레이스 링크를 반환합니다.

    ### 고급 예시: 비디오 분석 프로젝트 내에서 비디오 로깅

    다음 예시는 비디오 이해(video-understanding) 프로젝트에서 비디오를 로깅하는 방법을 보여줍니다:
    ...
    # 참고: https://aistudio.google.com/app/apikey 에서 API 키를 발급받으세요.
    client = genai.Client()
    weave.init('your-team-name/your-project-name')
...
다음 예제에서는 문서를 생성해 Weave UI에 로깅하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    함수에 `Annotated[bytes, Content]` 타입 어노테이션을 추가하거나, `Annotated[str, Content[Literal['text']]]`처럼 문서 타입을 지정해 문서를 로깅할 수 있습니다.

    Weave는 `pdf`, `csv`, `md`, `text`, `json`, `xml` 파일 유형을 자동으로 처리합니다. `Annotated[str, Content]`와 함께 파일 경로를 사용해 로깅할 수도 있습니다.

    다음 예제는 입력 PDF 및 CSV 파일의 사본을 저장하고, 함수가 반환한 파일 내용도 저장하는 방법을 보여줍니다.
    ...
    ### 고급 예제: RAG 시스템에서 문서 로깅

    이 예제는 Retrieval-Augmented Generation(RAG) 시스템에서 문서를 로깅하는 방법을 보여줍니다.
    ...
    def create_and_query_document(pdf_path: Annotated[str, Content], question: str) -> str:
        """PDF에서 텍스트를 추출하고 RAG를 사용해 질문에 답변"""
...
다음 예시들은 오디오를 Weave에 로깅하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    함수에 `Annotated[bytes, Content]` 유형 어노테이션을 추가하거나 `Annotated[str, Content[Literal['mp3']]]`로 오디오 유형을 지정하여 오디오를 Weave에 로깅하세요.

    Weave는 `mp3`, `wav`, `flac`, `ogg`, `m4a` ファイル 유형을 자동으로 처리합니다. `Annotated[str, Content]`를 사용해 ファイル 경로로도 로깅할 수 있습니다.

    다음 코드 스니펫은 사인파를 생성하고 녹음한 다음 오디오를 Weave에 로깅합니다:
    ...
    ### 고급 예시: AI가 생성한 오디오 생성 및 로깅

    이 예제에서는 `Content` annotation을 사용하여 AI가 생성한 오디오를 생성하고 로깅합니다:
    ...
    이 오디오는 Weave에 로깅되며 오디오 플레이어와 함께 UI에 자동으로 표시됩니다. 오디오 플레이어에서 원시 오디오 파형을 보고 다운로드할 수 있습니다.

    <Frame>
      ![Weave에서의 오디오 로깅](/images/weave/audio.png)
    </Frame>

    다음 예시는 OpenAI API의 streaming Response를 사용해 오디오를 로깅하는 방법을 보여줍니다:
  ...
  <Tab title="TypeScript">
    다음 예시는 기존 오디오 파일을 로드하여 Weave에 로깅합니다:
    ...
    ### 고급 예시: OpenAI의 TTS API를 사용해 오디오를 생성하고 Weave에 로깅하기

    다음 예시에서는 OpenAI의 TTS API를 사용해 오디오를 생성하고 이를 Weave에 로깅합니다:
...
<Tip>
  [오디오 로깅](/ko/weave/cookbooks/audio_with_weave)용 쿡북을 사용해 보세요. 이 쿡북에는 Weave로 통합된 실시간 오디오 API 기반 어시스턴트의 심화 예시도 포함되어 있습니다.
</Tip>
...
다음 예시는 HTML을 생성하고 Weave UI에 로깅하는 방법을 보여줍니다.

<Frame>
  ![Weave의 HTML 로깅](/images/weave/html.png)
</Frame>

<Tabs>
  <Tab title="Python">
    함수를 `Annotated[bytes, Content[Literal['html']]]`로 어노테이션하여 대화형 HTML을 로깅합니다.

    다음 예시는 간단한 HTML 페이지를 생성하고 Weave에 로깅합니다:
    ...
    ### 고급 예시: W&amp;B Inference를 사용해 독립형 HTML 페이지를 생성하고 Weave에 로깅하기

    이 예시는 [W&amp;B Inference](https://docs.wandb.ai/inference/)를 사용해 독립형 HTML 페이지를 생성하고, 해당 페이지를 Weave에 로깅합니다:
...
이 HTML은 Weave에 로깅되고 UI에 자동으로 표시됩니다. 테이블에서 `file_name.html` 셀을 클릭하면 전체 화면으로 열립니다. 원본 `.html` 파일을 다운로드할 수도 있습니다.

<div id="using-the-contents-api">
  ## Contents API 사용하기
</div>

Content API는 Weave의 미디어 객체를 처리합니다. base64 데이터, 파일 경로, 원시 바이트 또는 텍스트를 Weave로 임포트할 수 있습니다.

<Note>
  Content API는 Python에서만 사용 가능합니다.
</Note>
...
Content API를 사용하는 주요 방법은 유형 어노테이션과 직접 초기화 두 가지입니다.

유형 어노테이션은 사용할 적절한 생성자를 자동으로 감지하는 반면, 직접 초기화는 더 세밀한 제어를 제공하며 코드에서 Content API의 런타임 특성을 활용할 수 있게 합니다.

<div id="type-annotations">
  ### 유형 어노테이션
</div>

Weave Content API는 주로 유형 어노테이션을 통해 사용하도록 설계되었습니다. 유형 어노테이션은 트레이스된 입력과 출력이 콘텐츠 blob으로 처리되고 저장되어야 한다는 것을 Weave에 알려 줍니다.
...
* 자체 blob storage(예: S3)에 업로드할 수 있도록 모델을 JSON으로 직렬화하기
* `Content` blob과 연결할 맞춤형 메타데이터 전달하기(예: 이를 생성하는 데 사용한 모델)

다음 방법 중 하나를 사용해 대상 유형에서 `Content`를 직접 초기화할 수 있습니다:

* `Content.from_path` - 파일 경로에서 생성
* `Content.from_bytes` - 원시 바이트에서 생성
* `Content.from_text` - 텍스트 string에서 생성
...
# 입력은 경로 string으로, 출력은 Weave의 PDF 파일로 표시됩니다
...
Weave는 대부분의 바이너리 MIME 타입을 감지할 수 있지만, 맞춤형 MIME 타입이나 마크다운 같은 텍스트 문서는 자동으로 감지되지 않을 수 있으므로 ファイル의 MIME 타입이나 확장자를 직접 지정해야 할 수 있습니다.

<div id="custom-mimetypes-with-type-annotations">
  #### 유형 어노테이션을 사용한 맞춤형 MIME 타입
</div>
...
클래스 속성과 메서드의 전체 목록을 보려면 [Content 레퍼런스 문서](/ko/weave/reference/python-sdk#class-content)를 참조하세요.
...
* `save(dest: str | Path) -> None`: 콘텐츠를 ファイル에 저장합니다
* `open() -> bool`: 시스템 기본 애플리케이션으로 ファイル을 엽니다(콘텐츠가 이미 저장되었거나 경로에서 로드된 상태여야 합니다)
* `as_string() -> str`: 데이터를 문자열로 표시합니다(바이트는 encoding 속성을 사용해 디코딩됩니다)

<div id="initialization-methods">
  #### 초기화 메서드
</div>

파일 경로에서 `content` 객체를 생성합니다:
...
base64로 인코딩된 데이터로 `content` 객체를 생성합니다:
...
모든 Content 객체에 맞춤형 메타데이터를 연결할 수 있습니다:
