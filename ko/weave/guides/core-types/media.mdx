---
title: "미디어 로깅"
description: "트레이스에서 반환된 이미지, 비디오 등의 미디어를 로깅합니다."
---

W&amp;B Weave는 비디오, 이미지, 오디오 클립, PDF, CSV 데이터, HTML을 표시하기 위한 전용 함수를 포함해 다양한 콘텐츠 유형의 로깅 및 표시를 지원합니다. 이 가이드에서는 각 미디어 유형을 로깅하고 표시하는 기본 및 고급 예제를 제공합니다.
...
<Tabs>
  <Tab title="Python">
    이 가이드의 예제에서는 애너테이션을 사용합니다. 미디어 로깅을 시작하는 가장 간단한 방법이기 때문에 애너테이션 사용을 권장합니다. 더 고급 설정은 [Content API 섹션](#using-the-contents-api)을 참조하세요.

    Weave에 미디어를 로깅하려면 op의 입력 또는 반환 타입에 `Annotated[bytes, Content]` 또는 `Annotated[str, Content]`와 같은 타입 애너테이션을 추가하세요. 경로 인자를 `Annotated[str, Content]`로 애너테이션하면 Weave가 해당 미디어를 트레이스 내에서 자동으로 열고, 감지하고, 표시합니다.
  </Tab>
...
다음 섹션에서는 각 미디어 타입을 로깅하는 실용적인 예제를 제공합니다.
...
<Tabs>
  <Tab title="Python">
    `Annotated[bytes, Content]` 타입 애너테이션으로 함수의 인자를 지정하거나, `Annotated[str, Content]` 타입 애너테이션으로 파일 경로를 지정해 이미지를 로깅합니다.

    다음 예시는 기본 이미지를 그린 뒤 `Content` 애너테이션을 사용해 Weave에 로깅하는 방법을 보여줍니다.
    ...
    Weave는 이미지를 로그로 남기고, 해당 이미지를 확인할 수 있는 트레이스 링크를 반환합니다.

    ### 고급 예시: DALL-E로 이미지 생성 후 Weave에 로깅하기

    다음 예시에서는 고양이 이미지를 생성한 뒤 Weave에 로깅합니다:
    ...
    ### 고급 예시: 로깅 전에 큰 이미지 크기 조정하기

    UI 렌더링 비용과 스토리지 사용량을 줄이기 위해 로깅 전에 이미지 크기를 조정하는 것이 도움이 될 수 있습니다. `@weave.op`에서 `postprocess_output`을 사용해 이미지 크기를 조정할 수 있습니다.
    ...
    Weave는 크기가 조정된 이미지를 기록하고, 이미지를 확인할 수 있는 trace 링크를 반환합니다.
  </Tab>
  ...
  <Tab title="TypeScript">
    ...
    Weave는 이미지를 기록하고, 그 이미지를 확인할 수 있는 트레이스 링크를 반환합니다.

    ### 고급 예제: OpenAI DALL-E API로 생성된 이미지 로깅하기

    다음 예제에서는 `weaveImage` 함수를 사용하여 OpenAI DALL-E API로 생성된 이미지를 기록합니다:
...
<Tabs>
  <Tab title="Python">
    함수에 `Annotated[bytes, Content]` 타입을 어노테이션하여 비디오를 로깅할 수 있습니다. Weave는 `mp4` 비디오를 자동으로 처리합니다. 간단한 예시는 다음과 같습니다:
    ...
    Weave는 비디오를 로깅하고, 비디오를 확인할 수 있는 trace 링크를 반환합니다.

    ### 고급 예시: 비디오 분석 프로젝트 내에서 비디오 로깅

    다음 예시는 비디오 이해(video-understanding) 프로젝트 내에서 비디오를 로깅하는 방법을 보여줍니다:
...
다음 예제들은 Weave UI에 문서를 생성해 로깅하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    함수에 `Annotated[bytes, Content]` 타입으로 애노테이션을 달거나, `Annotated[str, Content[Literal['text']]`로 문서 타입을 지정하여 문서를 로깅할 수 있습니다.

    Weave는 `pdf`, `csv`, `md`, `text`, `json`, `xml` 파일 타입을 자동으로 처리합니다. 또한 `Annotated[str, Content]`와 함께 파일 경로를 사용해 로깅할 수도 있습니다.

    다음 예제는 입력 PDF 및 CSV 파일의 사본을 저장한 다음, 함수가 반환하는 파일 내용을 다시 저장하는 방법을 보여줍니다:
  ...
  <Tab title="TypeScript">
    ```plaintext
    이 기능은 아직 Weave TypeScript SDK에서는 사용할 수 없습니다.
    ```
  </Tab>
</Tabs>
...
다음 예제들은 오디오를 Weave에 로깅하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    `Annotated[bytes, Content]` 타입으로 함수에 애너테이션을 추가하거나, `Annotated[str, Content[Literal['mp3']]`를 사용해 오디오 유형을 지정하여 Weave에 오디오를 로깅할 수 있습니다.

    Weave는 `mp3`, `wav`, `flac`, `ogg`, `m4a` 파일 형식을 자동으로 처리합니다. 또한 `Annotated[str, Content]`와 함께 파일 경로를 사용해 로깅할 수도 있습니다.

    다음 코드 예제는 사인파를 생성해 녹음한 뒤, 그 오디오를 Weave에 로깅합니다:
    ...
    ### 고급 예시: AI 생성 오디오 생성 및 기록

    이 예시에서는 `Content` 애노테이션을 사용하여 AI가 생성한 오디오를 만들고 로그로 기록합니다.
    ...
    이 오디오는 Weave에 로깅되어 UI에서 오디오 플레이어와 함께 자동으로 표시됩니다. 오디오 플레이어에서 원본 오디오 파형을 확인하고 다운로드할 수 있습니다.

    <Frame>
      ![Weave에서 오디오 로깅](/images/weave/audio.png)
    </Frame>
  ...
  <Tab title="TypeScript">
    다음 예제는 기존 오디오 파일을 로드한 뒤 Weave에 로깅합니다:
    ...
    ### 고급 예제: OpenAI의 TTS API로 오디오를 생성하고 Weave에 기록하기

    다음 예제에서는 OpenAI의 TTS API를 사용해 오디오를 생성하고 이를 Weave에 기록합니다.
...
<Tip>
  [Audio Logging](/ko/weave/cookbooks/audio_with_weave)용 쿡북을 사용해 보세요. 이 쿡북에는 Weave와 통합된 실시간 오디오 API 기반 어시스턴트의 심화 예제도 포함되어 있습니다.
</Tip>
...
다음 예제는 HTML을 생성하고 Weave UI에 기록하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    함수를 `Annotated[bytes, Content[Literal['html']]]`로 어노테이션해서 인터랙티브 HTML을 기록합니다.

    다음 예제는 간단한 HTML 페이지를 생성하고 Weave에 기록합니다:
    ...
    ### 고급 예제: W&amp;B Inference를 사용해 독립형 HTML 페이지를 생성하고 Weave에 기록하기

    이 예제는 [W&amp;B Inference](https://docs.wandb.ai/inference/)를 사용해 독립형 HTML 페이지를 생성하고, 해당 페이지를 Weave에 기록합니다:
...
이 HTML은 Weave에 기록되고 UI에 자동으로 표시됩니다. 테이블에서 `file_name.html` 셀을 클릭하면 전체 화면으로 열립니다. 원본 `.html` 파일을 다운로드할 수도 있습니다.

<div id="using-the-contents-api">
  ## Contents API 사용하기
</div>

Content API는 Weave에서 미디어 객체를 다룹니다. Base64 데이터, 파일 경로, 원시 바이트, 텍스트 형태로 된 콘텐츠를 Weave에 가져올 수 있습니다.
...
타입 애노테이션은 사용할 적절한 생성자를 자동으로 판별해 주는 반면, 직접 초기화는 더 세밀하게 제어할 수 있으며 코드에서 Content API의 런타임 기능을 적극적으로 활용할 수 있게 해줍니다.

<div id="type-annotations">
  ### 타입 애너테이션
</div>

Weave Content API는 주로 타입 애너테이션을 통해 사용하는 것을 기본으로 설계되었습니다. 타입 애너테이션은 추적된 입력과 출력이 콘텐츠 블롭으로 처리되고 저장되어야 함을 Weave에 알려 주는 역할을 합니다.
...
* 기본 애플리케이션으로 파일 열기 (예: PDF 뷰어)
* 모델을 JSON으로 내보내서 자체 blob 스토리지(예: S3)에 업로드하기
* `Content` blob에 연결할 사용자 정의 메타데이터 전달하기 (예: 이를 생성하는 데 사용된 모델)

다음 메서드 중 하나를 사용해 대상 타입에서 바로 `Content`를 초기화할 수 있습니다:
...
# 입력은 경로 문자열로, 출력은 Weave의 PDF 파일로 표시됩니다
...
Weave는 대부분의 바이너리 MIME 타입을 감지할 수 있지만, 커스텀 MIME 타입이나 마크다운 같은 텍스트 문서는 자동으로 감지되지 않을 수 있어 파일의 MIME 타입 또는 확장자를 직접 지정해야 할 수 있습니다.

<div id="custom-mimetypes-with-type-annotations">
  #### 타입 애너테이션을 사용한 커스텀 MIME 타입
</div>
...
클래스 속성과 메서드의 전체 목록은 [Content 레퍼런스 문서](/ko/weave/reference/python-sdk#class-content)를 참조하세요.
...
* `save(dest: str | Path) -> None`: 콘텐츠를 파일에 저장합니다
* `open() -> bool`: 시스템 기본 애플리케이션으로 파일을 엽니다(콘텐츠가 이미 저장되었거나 경로에서 로드된 상태여야 합니다)
* `as_string() -> str`: 데이터를 문자열로 반환합니다(바이트는 encoding 속성을 사용해 디코딩됩니다)

<div id="initialization-methods">
  #### 초기화 메서드
</div>

파일 경로로부터 `content` 객체를 생성합니다:
...
base64로 인코딩된 데이터를 사용해 `content` 객체를 생성합니다:
