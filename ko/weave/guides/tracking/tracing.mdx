...
<Tabs>
  <Tab title="Python">
    Weave 는 `openai`, `anthropic`, `cohere`, `mistral`과 같은 [일반적인 LLM 라이브러리에 대한 호출](/ko/weave/guides/integrations)을 자동으로 추적합니다. 프로그램 시작 시 [`weave.init('project_name')`](/ko/weave/reference/python-sdk#function-init)을 호출하기만 하면 됩니다:

    <Tip>
    [`weave.init`의 `autopatch_settings` 인수를 사용하여](#특정-인테그레이션-활성화) Weave 의 기본 추적 동작을 제어할 수 있습니다.
    </Tip>
  ...
  <Tab title="TypeScript">
    Weave 는 `openai`와 같은 [일반적인 LLM 라이브러리에 대한 호출](/ko/weave/guides/integrations)을 자동으로 추적합니다.
    ...
    JS / TS 프로젝트를 위한 전체 설정 가이드는 [TypeScript SDK: Third-Party Integration Guide](/ko/weave/guides/integrations/js)를 참조하세요.
...
<Tabs>
  <Tab title="Python">
    Weave 를 사용하면 [`@weave.op`](/ko/weave/reference/python-sdk/#function-op) 데코레이터를 사용하여 이러한 호출을 수동으로 추적할 수 있습니다. 예:
    ...
    [클래스의 메소드](#weavemarkdown으로-렌더링된-traces-커스터마이징)도 추적할 수 있습니다.
  ...
  <Tab title="TypeScript">
    Weave 를 사용하면 [`weave.op`](/ko/weave/reference/typescript-sdk/functions/op)으로 함수를 래핑하여 이러한 호출을 수동으로 추적할 수 있습니다. 예:
...
<Tabs>
  <Tab title="Python">
    ...
    2. 호출별로 표시 이름을 변경합니다. 이는 `Call` 오브젝트를 반환하는 [`Op.call`](/ko/weave/reference/python-sdk/trace/op#function-call) 메소드를 사용하며, 그런 다음 [`Call.set_display_name`](/ko/weave/reference/python-sdk/trace/weave_client#method-set_display_name)을 사용하여 표시 이름을 설정할 수 있습니다.
    ```python lines lines
    result, call = my_function.call("World")
  ...
  호출의 기본 이름을 오버라이드하려면 `weave.op()`을 호출할 때 `callDisplayName` 옵션을 사용하세요.
    ...
    실행 후에도 [호출의 표시 이름을 업데이트](/ko/weave/guides/tracking/tracing#set-display-name)할 수 있습니다.
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Python">
    기본적으로 병렬 호출은 Weave 에서 모두 별도의 루트 호출로 나타납니다. 동일한 부모 `op` 아래에 올바르게 중첩되게 하려면 [`ThreadPoolExecutor`](/ko/weave/reference/python-sdk/trace/util#class-contextawarethreadpoolexecutor)를 사용하세요.
...
<Tabs>
    ...
    <Tab title="HTTP API">
    * 호출 시작: [POST `/call/start`](/ko/weave/reference/service-api/calls/call-start)
    * 호출 종료: [POST `/call/end`](/ko/weave/reference/service-api/calls/call-end)
    ```bash
...
<Tab title="Python">
Weave Python SDK를 사용하여 호출을 보려면 [`get_call`](/ko/weave/reference/python-sdk/trace/weave_client#method-get_call) 메소드를 사용할 수 있습니다:
...
<Tab title="HTTP API">
Service API를 사용하여 호출을 보려면 [`/call/read`](/ko/weave/reference/service-api/calls/call-read) 엔드포인트에 요청을 보낼 수 있습니다.
...
* [표시 이름 설정](#여러-calls-삭제)
* [피드백 추가](#add-feedback)
* [Call 삭제](#querying-and-exporting-calls)
...
<Tabs>
<Tab title="Python">
호출의 표시 이름을 설정하려면 [`Call.set_display_name()`](/ko/weave/reference/python-sdk/trace/weave_client#method-set-display-name) 메소드를 사용할 수 있습니다.
...
<Tab title="TypeScript">
호출의 표시 이름을 설정하려면 [`client.updateCall`](/ko/weave/reference/typescript-sdk/classes/weaveclient#updatecall)을 사용하여 호출 ID로 직접 업데이트하세요:
...
<Tab title="HTTP API">
Service API를 사용하여 호출의 표시 이름을 설정하려면 [`/call/update`](/ko/weave/reference/service-api/calls/call-update) 엔드포인트에 요청을 보낼 수 있습니다.
...
자세한 내용은 [Feedback Documentation](/ko/weave/guides/tracking/feedback)을 참조하세요.

### Call 삭제

<Tabs>
<Tab title="Python">
Python API를 사용하여 Call을 삭제하려면 [`Call.delete`](/ko/weave/reference/python-sdk/trace/weave_client#method-delete) 메소드를 사용할 수 있습니다.
...
<Tab title="HTTP API">
Service API를 사용하여 호출을 삭제하려면 [`/calls/delete`](/ko/weave/reference/service-api/calls/calls-delete) 엔드포인트에 요청을 보낼 수 있습니다.
...
<Tabs>
    <Tab title="Python">
    Python API를 사용하여 호출을 가져오려면 [`client.get_calls`](/ko/weave/reference/python-sdk/trace/weave_client#method-get_calls) 메소드를 사용할 수 있습니다:
    ...
    <Tab title="TypeScript">
    TypeScript API를 사용하여 호출을 가져오려면 [`client.getCalls`](/ko/weave/reference/typescript-sdk/classes/weaveclient#getcalls) 메소드를 사용할 수 있습니다.
    ```typescript
    import * as weave from 'weave'
    ...
    <Tab title="HTTP API">
    가장 강력한 쿼리 레이어는 Service API에 있습니다. Service API를 사용하여 호출을 가져오려면 [`/calls/stream_query`](/ko/weave/reference/service-api/calls/calls-query-stream) 엔드포인트에 요청을 보낼 수 있습니다.
...
필드 전체 목록은 [스키마](/ko/weave/reference/python-sdk/trace_server/trace_server_interface#class-callschema)를 참조하세요.
...
Trace 테이블 구성, 필터 및 정렬을 _저장된 뷰_로 저장하여 선호하는 설정에 빠르게 엑세스할 수 있습니다. UI 및 Python SDK를 통해 저장된 뷰를 구성하고 엑세스할 수 있습니다. 자세한 내용은 [Saved Views](/ko/weave/guides/tools/saved-views)를 참조하세요.

## Traces 테이블에서 W&B run 보기

Weave 를 사용하면 코드의 함수 호출을 추적하고 이들이 실행된 [W&B runs](/ko/models/runs/)에 직접 링크할 수 있습니다. 
...
자세한 내용은 [How to use Weave with PII data](/ko/weave/cookbooks/pii)를 참조하세요.

## FAQs

### 대규모 trace가 잘리는 현상을 어떻게 방지하나요?

자세한 내용은 [문제 해결 가이드](/ko/weave/guides/troubleshooting)의 [Trace 데이터가 잘림](/ko/weave/guides/troubleshooting#trace-data-is-truncated) 섹션을 참조하세요.
