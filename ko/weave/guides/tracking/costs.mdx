...
W&amp;B Weave는 토큰 사용량을 추적하고 사용된 모델의 가격을 적용해 각 LLM 호출의 비용을 자동으로 계산하므로, 트레이스와 평가에서 애플리케이션 지출을 직접 모니터링하고 분석할 수 있습니다.

또한 다른 가격 책정이 필요하거나, 내부 비용 모델을 사용하거나, Weave가 자동으로 가격을 책정하지 않는 작업의 비용이 필요한 경우 사용자 정의 비용을 추적할 수도 있습니다.
...
<Tabs>
  <Tab title="Python">
    [`add_cost`](/ko/weave/reference/python-sdk/trace/weave_client#method-add-cost) 메서드를 사용하여 사용자 정의 비용을 추가할 수 있습니다.
...
<Tabs>
  <Tab title="Python">
    [`query_costs`](/ko/weave/reference/python-sdk/trace/weave_client#method-query-costs) 메서드를 사용해서 비용을 쿼리할 수 있습니다.
    비용을 쿼리하는 방법은 여러 가지가 있으며, 단일 비용 ID를 전달하거나 LLM 모델 이름 목록을 전달할 수 있습니다.
...
<Tabs>
  <Tab title="Python">
    약간의 설정만 하면 `calls_query`를 사용하고 `include_costs=True`를 추가해 프로젝트의 비용을 계산할 수 있습니다.
...
커스텀 모델에 비용을 설정하는 방법은 [커스텀 모델로 비용 설정하기](/ko/weave/cookbooks/custom_model_cost) 쿡북을 참고하세요.
