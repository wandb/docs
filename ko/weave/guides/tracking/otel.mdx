...
`<your-subdomain>`을 조직의 고유한 W&amp;B 도메인으로 바꾸세요. 예를 들면 `acme.wandb.io`입니다.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    다음 코드를 `openllmetry_example.ts` 같은 TypeScript 파일에 붙여넣으세요. 이 예제에서는 Traceloop OpenAI instrumentation 패키지를 사용합니다:
...
* `wandb.is_turn`: 스팬을 대화 턴으로 표시합니다(스레드 view에서 행으로 표시됨)
