...
description: "전용 엔드포인트를 통해 OpenTelemetry 호환 트레이스 데이터를 임포트합니다"
keywords: ["OTLP", "OpenTelemetry Collector", "OpenInference", "OpenLLMetry", "스레드"]
---

<Warning>
  Weave의 새 Agents 뷰로 OTel span을 전송하려면 [Weave의 새 OTel 네이티브 워크플로](/ko/weave/guides/tracking/trace-agents-otel)를 사용하는 것이 좋습니다. 원시 OTLP 트레이스 수집 데이터를 Weave의 표준 Traces 뷰로 전송하려는 경우에만 이 가이드를 사용하세요.
</Warning>
...
<Note>
  이 엔드포인트는 **Traces** 및 **Threads** 뷰에 데이터를 제공합니다. 멀티턴 에이전트에서 **Agents** 뷰로 OTel span을 전송하려면 대신 agents 엔드포인트를 사용하세요. [OpenTelemetry spans를 Agents view로 전송](/ko/weave/guides/tracking/trace-agents-otel)을 참조하세요.
</Note>

## 엔드포인트 세부 정보 {#endpoint-details}
...
## 인증 및 라우팅 {#authentication-and-routing}

Weave는 요청 인증에 `wandb-api-key` 헤더를 사용하고, span을 올바른 entity 및 프로젝트로 라우팅하기 위해 `TracerProvider`의 리소스 속성을 사용합니다. `wandb-api-key` 헤더에 W&amp;B API 키를 전달한 다음, `TracerProvider` 클래스에서 다음 키를 OpenTelemetry 리소스 속성으로 지정하세요.

* `wandb.entity`: W&amp;B 팀 또는 사용자 이름입니다.
* `wandb.project`: 트레이스를 보낼 프로젝트 이름입니다.

다음 예시는 인증 및 프로젝트 라우팅을 구성하는 방법을 보여줍니다.

<Warning>
  아래의 `WEAVE_OTLP_ENDPOINT`는 OTel SDK의 `OTEL_EXPORTER_OTLP_ENDPOINT` 환경 변수가 아니라 로컬 변수이며, 전체 트레이스 경로(`.../otel/v1/traces`)로 설정됩니다. 이 값은 익스포터의 `endpoint=`/`url:`에 명시적으로 전달되므로 SDK는 요청을 해당 URL로 그대로 전송합니다. 반대로 실제 `OTEL_EXPORTER_OTLP_ENDPOINT`에 값을 `export`하고 인수 없이 익스포터를 생성하면(예: `OTLPSpanExporter()`), SDK가 `/v1/traces`를 자동으로 덧붙입니다. 따라서 이 변수에는 base URL(`.../otel`)이 들어 있어야 합니다. 그렇지 않으면 경로가 중복되어(`.../otel/v1/traces/v1/traces`) `404`가 발생하고 span이 조용히 누락됩니다. `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`도 마찬가지입니다. 이 경우에도 SDK가 `/v1/traces`를 추가하지 않으므로, 해당 변수에는 전체 트레이스 경로가 들어 있어야 합니다.
</Warning>
...
### OpenInference 계측 {#openinference-instrumentation}
...
### OpenLLMetry 계측 {#openllmetry-instrumentation}
...
## OpenTelemetry Collector 사용하기 {#use-an-opentelemetry-collector}
...
### collector 설정하기 {#set-up-a-collector}
...
## OTel 트레이스를 스레드로 정리하기 {#organize-otel-traces-into-threads}
...
| `gen_ai.input.messages`           | `inputs`                      | Logfire에서 생성된 입력 메시지입니다.         | 목록, dict, 문자열           | `[{"role":"user","parts":[...]}]`              |
| `pydantic_ai.all_messages`        | `inputs`                      | Pydantic AI 에이전트 run의 모든 메시지입니다. | 목록                      | `[{"role":"user","content":"abc"}]`            |
...
| `gen_ai.output.messages`          | `outputs`                     | Logfire에서 생성된 출력 메시지입니다.         | 목록, dict, 문자열           | `[{"role":"assistant","parts":[...]}]`         |
...
| `gen_ai.system_instructions`      | `attributes.system`           | 시스템 프롬프트 또는 지침.                  | 문자열                     | `"당신은 유용한 도우미입니다."`                            |
