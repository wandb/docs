---
title: 하위 에이전트 트레이스
description: Weave sub-agent span을 사용해 중첩된 에이전트와 작업을 위임받은 에이전트를 트레이스하세요.
---
...
하위 에이전트는 turn 내에서 실행되는 위임된 에이전트 호출입니다. 한 에이전트가 다른 에이전트에 작업을 넘길 때, 예를 들어 supervisor 에이전트가 specialist 에이전트에 작업을 할당하는 경우 하위 에이전트를 사용하세요.

Weave로 계측하면 하위 에이전트는 부모 turn과 동일한 트레이스에 중첩된 `invoke_agent` OTel span을 생성합니다. **Agents** 뷰에서는 이 중첩이 이를 트리거한 turn 아래의 하위 에이전트 호출로 표시되며, 그 아래에 자체 LLM Calls와 tool calls가 그룹화되어 나타납니다.

<div id="sub-agent-data-model">
  ## 하위 에이전트 데이터 모델
</div>

`weave.start_subagent` span은 OTel `invoke_agent` span에 매핑되며, 상위 turn과 동일한 오퍼레이션 이름을 생성합니다. Weave는 트레이스의 부모-자식 관계를 통해 이 둘을 구분합니다:
...
하위 에이전트는 현재 활성 세션의 `conversation_id`를 상속하므로 Agents 뷰에서 나머지 대화와 함께 그룹화됩니다.
...
Weave는 대화를 `weave.start_session`으로 감싸고 이어서 `session.start_turn`을 사용해 전체 계층 구조를 캡처합니다. 그런 다음 Weave는 specialist의 `weave.start_subagent` 블록을 사용해 하위 에이전트 트레이스를 캡처하고, 각 LLM Call과 도구 실행을 하위 span으로 기록합니다.
...
Agents 뷰에서 하위 에이전트는 turn 내부에 중첩된 `invoke_agent` 블록으로 표시되며, 그 아래에 자체 LLM Calls와 tool calls가 함께 그룹화되어 나타납니다. supervisor의 직접 LLM Calls는 하위 에이전트와 같은 계층에 그대로 표시됩니다.
    ...
    with session.start_turn(user_message="Compare Anthropic and OpenAI.") as turn:
        ...
        with weave.start_subagent(name="research-coordinator") as coordinator:
            with weave.start_subagent(name="anthropic-researcher") as r1:
                ...
                # 중첩: 연구자가 자체 요약기 하위 에이전트에 작업을 위임합니다.
