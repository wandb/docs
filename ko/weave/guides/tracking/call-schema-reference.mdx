...
이 레퍼런스에서는 W&amp;B Weave의 Call 객체 스키마를 설명합니다. 여기에는 속성, 실행 중 `summary` 딕셔너리에 맞춤형 데이터를 기록하는 방법, 그리고 Call이 완료된 후 summary 데이터를 조회하는 방법이 포함됩니다. 각 Call에 대해 Weave가 기록하는 데이터를 검사, 필터링하거나 확장해야 할 때 이 페이지를 사용하세요. Call을 쿼리하는 방법에 대한 자세한 내용은 [Query and export calls](/ko/weave/guides/tracking/querying-calls)를 참조하세요.

## Call 속성 {#call-properties}

다음 표는 Weave에서 Call의 주요 속성을 보여줍니다. 전체 구현은 다음을 참조하세요:
...
## Call summary 사용 {#use-call-summary}

`summary` 속성을 사용하면 Call에 맞춤형 실행 후 데이터를 추가하여 나중에 Weave의 기본 제공 메트릭과 함께 분석할 수 있습니다. 이 속성은 Call 실행 중에 값을 쓸 수 있는 딕셔너리입니다. Call이 완료되면 Weave는 사용자가 쓴 값과 자체적으로 계산한 데이터를 깊게 병합(deep merge)하여 결과를 저장합니다.
...
### Call 실행 중에 작성하기 {#write-during-a-call}
...
### summary 데이터 조회 {#read-summary-data}
...
<Tabs>
  <Tab title="Python">
    ...
    print(weave_summary.get("costs"))        # 모델별 비용 세부 내역입니다.
  ...
  <Tab title="TypeScript">
    ...
    console.log(weaveSummary?.costs);        // 모델별 비용 세부 내역입니다.
