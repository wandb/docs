...
이 페이지에서는 W&amp;B Weave의 Call 객체 스키마에 대한 레퍼런스를 제공합니다. Call를 쿼리하는 방법은 [Call 쿼리 및 내보내기](/ko/weave/guides/tracking/querying-calls)를 참조하세요.
...
* 시간 정보(`started_at`, `ended_at`)는 성능 분석을 지원합니다.

* `attributes` 및 `inputs` 속성은 call의 컨텍스트를 제공합니다. `attributes`는 call이 시작되면 고정되므로 호출 전에 `weave.attributes`로 설정하세요. `output` 및 `summary`는 결과를 캡처합니다.
  -`summary` 속성에는 메트릭이나 기타 호출 후 값을 저장할 수 있습니다. 실행 중에 `call.summary`를 수정하세요. 추가한 값은 Call이 완료될 때 Weave가 계산한 summary 데이터와 병합됩니다.
  * Weave가 계산한 summary 데이터:
    * `costs`: LLM 모델 사용 데이터와 token 가격 데이터를 기반으로 계산된 call의 총비용입니다. 비용 계산에 대한 자세한 내용은 [Track costs](/ko/weave/guides/tracking/costs)를 참조하세요.
    * `latency_ms`: `started_at`와 `ended_at` 사이의 경과 시간을 밀리초 단위로 나타낸 값입니다. `status`가 `RUNNING`이면 `null`입니다.
    * `status`: 실행 상태입니다: `SUCCESS`, `ERROR`, `RUNNING`, `DESCENDANT_ERROR`(즉, call 자체는 성공했지만 하위 call에서 오류가 발생했음을 의미). {/* [empty ref](/weave/reference/python-sdk/trace_server/trace_server_interface#class-tracestatus)*/}

* W&amp;B와의 인테그레이션은 `wb_user_id` 및 `wb_run_id`를 통해 이루어집니다.

이 포괄적인 속성 집합을 사용하면 프로젝트 전반에서 함수 call을 상세하게 추적하고 분석할 수 있습니다.