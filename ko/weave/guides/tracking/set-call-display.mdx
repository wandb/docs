---
title: "Call 표시 이름 설정"
description: "W&B Weave tracing에서 Call의 표시 이름을 설정하거나 재정의합니다"
---

Ops는 Calls를 생성합니다. Op는 `@weave.op`으로 데코레이트한 함수 또는 메서드입니다. 기본적으로 Op의 이름은 함수 이름이며, 연결된 Calls도 같은 표시 이름을 사용합니다.
...
<Tabs>
  <Tab title="Python">
    1. Op를 호출할 때 표시 이름을 변경합니다.
       다음 예시에서는 `__weave` 딕셔너리를 사용해 Op 표시 이름보다 우선 적용되는 Call 표시 이름을 설정합니다:

    ```python lines
    result = my_function("World", __weave={"display_name": "My Custom Display Name"})
    ```

    2. 각 Call별로 표시 이름을 변경합니다.
       다음 예시에서는 [`Op.call`](/ko/weave/reference/python-sdk/trace/op#function-call) 메서드를 사용해 `call` 객체를 반환한 다음, [`call.set_display_name`](/ko/weave/reference/python-sdk/trace/weave_client#method-set_display_name)를 사용해 표시 이름을 설정합니다:
    ...
    3. 특정 Op의 모든 Calls에 대해 표시 이름을 변경합니다.
       다음 예시에서는 `@weave.op` 함수 데코레이터 자체에 새 표시 이름을 설정하여 해당 Op의 모든 Calls에 적용합니다:
    ...
    `call_display_name`은 `call` 객체를 입력받아 문자열을 반환하는 함수일 수도 있습니다. 함수가 실행될 때 Weave가 `call` 객체를 자동으로 전달하므로, 이를 사용해 함수 이름, Call 입력값, 필드 등을 기준으로 이름을 동적으로 생성할 수 있습니다.
    ...
    4. Op 자체의 표시 이름을 변경합니다.
       Op에 연결된 Calls는 동일한 표시 이름을 사용합니다. Op 자체의 이름을 재정의하면 Call의 표시 이름도 함께 바뀝니다. 이 작업은 두 가지 방법으로 할 수 있습니다:
