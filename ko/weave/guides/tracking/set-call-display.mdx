...
<Tabs>
  <Tab title="Python">
    ...
    `call_display_name`은 `call` 객체를 받아 문자열을 반환하는 함수일 수도 있습니다. 함수가 실행될 때 Weave가 `call` 객체를 자동으로 전달하므로, 이를 사용해 함수 이름, Call 입력값, 필드 등을 기반으로 이름을 동적으로 생성할 수 있습니다. 이는 각 Call의 이름에 런타임 컨텍스트가 반영되도록 하려는 경우에 유용합니다.
