---
title: "스레드 트레이스"
description: "LLM 애플리케이션에서 스레드를 사용해 멀티턴 대화를 트레이스하고 분석합니다."
---

W&amp;B Weave &#95;스레드&#95;를 사용하면 LLM 애플리케이션에서 멀티턴 대화를 추적하고 분석할 수 있습니다. 스레드는 관련된 Call을 공통 `thread_id`로 묶어 전체 세션을 시각화하고, 여러 턴에 걸친 대화 수준 메트릭을 추적할 수 있게 해줍니다. 스레드는 프로그래밍 방식으로 생성할 수 있으며 Weave UI에서 시각화할 수 있습니다.

스레드를 시작하려면 다음을 수행하세요:
...
2. 일반적인 사용 패턴과 실제 활용 사례를 보여주는 코드 샘플을 사용해 보세요.
...
* 멀티턴 대화
* 세션 기반 워크플로
* 서로 연관된 오퍼레이션의 일련의 흐름

스레드를 사용하면 Call을 컨텍스트별로 그룹화할 수 있어, 여러 step에 걸쳐 시스템이 어떻게 응답하는지 더 쉽게 파악할 수 있습니다. 예를 들어, 단일 사용자 세션, 에이전트의 의사 결정 체인, 인프라와 비즈니스 로직 계층 전반에 걸친 복잡한 Request를 추적할 수 있습니다.

애플리케이션을 스레드와 턴으로 구조화하면 더 깔끔한 메트릭과 Weave UI에서의 더 나은 가시성을 확보할 수 있습니다. 모든 저수준 Op를 일일이 보는 대신, 중요한 고수준 step에 집중할 수 있습니다.
...
&#95;스레드&#95;는 공통된 대화 컨텍스트를 공유하는 관련 Call들을 논리적으로 묶은 그룹입니다. &#95;스레드&#95;는 다음과 같은 특징이 있습니다:

* 고유한 `thread_id`를 가집니다
* 하나 이상의 &#95;턴&#95;을 포함합니다
* Call 전반에 걸쳐 컨텍스트를 유지합니다
* 완전한 사용자 세션 또는 상호작용 흐름을 나타냅니다

<div id="turn">
  ### 턴(Turn)
</div>

&#95;턴(Turn)&#95;은 스레드 내에서의 상위 수준 오퍼레이션으로, UI의 스레드 뷰에서 개별 행으로 표시됩니다. 각 턴은 다음을 의미합니다:

* 대화나 워크플로에서 하나의 논리적 step을 나타냅니다.
* 턴은 스레드 컨텍스트의 직계 하위 요소이며, 중첩된 더 하위 수준의 Call을 포함할 수 있지만 이러한 Call은 스레드 수준 통계에는 표시되지 않습니다.

<div id="call">
  ### Call
</div>

&#95;Call&#95;은(는) 애플리케이션에서 `@weave.op` 데코레이터가 적용된 모든 함수 실행(Call)을 의미합니다.

* &#95;Turn Calls&#95;은(는) 새 턴을 시작하는 최상위 오퍼레이션입니다
* &#95;Nested Calls&#95;은(는) 하나의 턴 안에 포함된 하위 수준 오퍼레이션입니다

<div id="trace">
  ### 트레이스
</div>

&#95;Trace&#95;는 단일 오퍼레이션에 대한 전체 Call 스택을 캡처합니다. 스레드는 동일한 논리적 대화 또는 세션의 일부인 트레이스를 함께 그룹화합니다. 즉, 하나의 스레드는 여러 턴으로 구성되며, 각 턴은 대화의 한 부분을 나타냅니다. 트레이스에 대한 자세한 내용은 [Tracing Overview](/ko/weave/guides/tracking/tracing)을 참조하세요.

<div id="ui-overview">
  ## UI Overview
</div>

Weave 사이드바에서 **스레드**를 선택하면 [스레드 list view](#threads-list-view)가 열립니다.

<Frame>
  ![Weave 사이드바의 스레드 아이콘](/weave/guides/tracking/imgs/threads-sidebar.png)
</Frame>
...
* 프로젝트의 최근 스레드(스레드)를 목록으로 표시합니다
* 열에는 턴 수, 시작 시간, 마지막 업데이트 시간이 표시됩니다
* 행을 클릭하면 [스레드 상세 드로어](#threads-detail-drawer)가 열립니다

<Frame>
  ![스레드 목록 보기](/weave/guides/tracking/imgs/threads-list.png)
</Frame>
...
* Call 레벨 메타데이터(레이턴시, 입력, 출력)를 포함한다.
* 로깅된 경우 메시지 내용이나 구조화된 데이터를 선택적으로 표시한다.
* 턴의 전체 실행을 보려면 스레드 상세 드로어에서 해당 턴을 열 수 있다. 이렇게 하면 해당 턴 동안 발생한 모든 중첩 오퍼레이션을 자세히 살펴볼 수 있다.
* 턴에 LLM calls에서 추출된 메시지가 포함되어 있으면 오른쪽 채팅 패널에 나타난다. 이러한 메시지는 일반적으로 지원되는 인테그레이션(예: `openai.ChatCompletion.create`)에서 수행된 Call에서 나오며, 표시되기 위해서는 특정 기준을 충족해야 한다. 자세한 내용은 [Chat view behavior](#chat-view-behavior)를 참조하라.

<div id="chat-view-behavior">
  ### 채팅 뷰 동작
</div>

채팅 패널에는 각 턴에서 수행된 LLM Call에서 추출한 구조화된 메시지 데이터가 표시됩니다. 이 뷰에서는 상호작용을 대화형 형식으로 볼 수 있습니다.
...
Weave는 한 턴 내에서 LLM 제공자와 직접 상호작용하는(예: 프롬프트를 보내고 Response를 받는) Call에서 메시지를 추출합니다. 다른 Call 안에 더 이상 중첩되지 않은 Call만 메시지로 표시됩니다. 이렇게 하면 중간 step이나 집계된 내부 로직이 중복되는 것을 방지할 수 있습니다.

일반적으로 자동 패치된 서드파티 SDK가 메시지를 생성합니다. 예를 들면 다음과 같습니다:
...
어떤 턴에서 메시지가 생성되지 않으면 채팅 패널에는 해당 턴에 대해 비어 있는 메시지 섹션이 표시됩니다. 채팅 패널에는 여전히 같은 스레드의 다른 턴에서 온 메시지가 포함될 수 있습니다.
...
스레드 상세 뷰로 돌아가는 뒤로 가기 버튼이 왼쪽 상단에 나타납니다. Weave는 전환 시 UI 상태(예: 스크롤 위치)를 유지하지 않습니다.
...
이 section의 각 예시는 application에서 턴과 스레드를 구성하는 서로 다른 전략을 보여줍니다. 대부분의 예시에서는 stub 함수 내부의 LLM Call 또는 system behavior를 직접 provide해야 합니다.

* 세션이나 대화를 추적하려면 `weave.thread()` 컨텍스트 관리자를 사용하세요.
* 논리적 오퍼레이션을 `@weave.op`로 데코레이션하여 턴 또는 중첩된 Call로 추적하세요.
* `thread_id`를 전달하면 Weave는 해당 블록의 모든 오퍼레이션을 같은 스레드로 그룹화하는 데 이를 사용합니다. `thread_id`를 생략하면 Weave가 고유한 값을 자동 생성해 줍니다.

`weave.thread()`의 Return Value는 `thread_id` 속성을 가진 `ThreadContext` 객체이며, 이를 log하거나, 재사용하거나, 다른 시스템에 전달할 수 있습니다.

중첩된 `weave.thread()` 컨텍스트는 같은 `thread_id`를 재사용하지 않는 한 항상 새 스레드를 시작합니다. 하위 컨텍스트를 종료해도 상위 컨텍스트가 중단되거나 덮어써지지 않습니다. 따라서 app 로직에 따라 포크된 스레드 구조나 계층화된 스레드 오케스트레이션을 사용할 수 있습니다.

<div id="basic-thread-creation">
  ### 기본 스레드 생성
</div>

다음 코드 예시는 `weave.thread()`를 사용해 하나 이상의 오퍼레이션을 공통 `thread_id`로 묶는 방법을 보여줍니다. 이는 애플리케이션에서 스레드를 사용하기 시작하는 가장 단순한 방법입니다.
...
이 예제에서는 `@weave.op` 데코레이터와 `weave.thread()` 컨텍스트 관리를 사용해 대화형 에이전트를 수동으로 정의하는 방법을 보여줍니다. `process_user_message`를 호출할 때마다 스레드에 새 턴이 생성됩니다. 자체 에이전트 루프를 구축하면서 컨텍스트와 중첩 처리 방식을 완전히 제어하려는 경우 이 패턴을 사용할 수 있습니다.

짧게 유지되는 상호작용에는 AUTO-GENERATED 스레드 ID를 사용하고, 세션 간에 스레드 컨텍스트를 유지하려면 `user_session_123`와 같은 맞춤형 세션 ID를 전달하세요.
...
이 예제는 스레드 컨텍스트가 어떻게 적용되느냐에 따라 호출 스택의 서로 다른 깊이에서 턴을 정의할 수 있음을 보여줍니다. 예제에서는 두 개의 제공자(OpenAI와 Anthropic)를 사용하며, 각각 턴 경계에 도달하기 전까지의 호출 깊이가 다릅니다.

모든 턴은 동일한 `thread_id`를 공유하지만, 제공자 로직에 따라 턴 경계가 스택의 서로 다른 레벨에 나타납니다. 이는 서로 다른 백엔드에 대해 Call을 다르게 트레이스해야 하면서도, 여전히 동일한 스레드로 묶어야 할 때 유용합니다.
...
class MultiProviderAgent:
    """서로 다른 호출 체인 깊이를 가진 제공자 간에 라우팅하는 메인 에이전트"""
    ...
    def handle_conversation_turn(self, user_input: str, thread_id: str) -> str:
        """
        서로 다른 호출 체인 깊이를 가진 제공자로 라우팅합니다.
...
# - 호출 체인의 보조 오퍼레이션은 턴이 아닌 중첩 호출로 추적
```
...
이전에 시작한 세션을 재개해 동일한 스레드에 Call을 계속 추가해야 하는 경우가 있습니다. 반대로 기존 세션을 재개할 수 없어서 새 스레드를 시작해야 하는 경우도 있습니다.

선택적 스레드 재개를 구현할 때는 **절대** `thread_id` 파라미터를 `None`으로 두지 마세요. 이렇게 하면 스레드 그룹화가 완전히 비활성화됩니다. 대신 항상 유효한 스레드 ID를 제공하세요. 새 스레드를 만들어야 한다면 `generate_id()` 같은 함수를 사용해 고유 식별자를 생성하세요.

`thread_id`를 지정하지 않으면 Weave의 내부 구현이 자동으로 임의의 UUID v7을 생성합니다. 이 동작을 직접 구현한 `generate_id()` 함수에서 그대로 재현해도 되고, 원하는 고유 string 값을 사용해도 됩니다.
...
이 예제는 여러 스레드가 서로 조율되어 동작하는 방식으로 복잡한 애플리케이션을 구성하는 방법을 보여줍니다.

각 계층은 자체 스레드 컨텍스트에서 실행되므로 관심사를 깔끔하게 분리할 수 있습니다. 상위 애플리케이션 스레드는 공유 `ThreadContext`를 사용해 스레드 ID를 설정함으로써 이러한 계층을 조율합니다. 시스템의 서로 다른 부분을 독립적으로 분석하거나 모니터링하면서도 공통 세션에 연결해 두어야 할 때 이 패턴을 사용하십시오.
...
class ThreadSchema:
    ...
    turn_count: int          # 이 스레드의 턴 Call 횟수
    start_time: datetime     # 이 스레드에서 턴 Call의 가장 이른 시작 시간
    last_updated: datetime   # 이 스레드에서 턴 Call의 가장 늦은 종료 시간
