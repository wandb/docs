...
**객체**는 버전이 지정된 직렬화 가능한 데이터입니다. Weave는 객체가 변경되면 자동으로 버전을 매기고 변경 불가능한 이력을 생성합니다. 객체에는 다음이 포함됩니다:

* **Datasets**: 평가를 위한 예제 모음
* **Models**: LLM 로직을 위한 설정과 파라미터
* **Prompts**: 버전이 지정된 프롬프트 템플릿
...
<Tabs>
  <Tab title="Python">
    `weave.publish`는 Ref를 반환합니다. 어떤 Ref에서든 `.get()`을 호출하여 객체를 다시 조회할 수 있습니다.
...
<Tabs>
  <Tab title="Python">
    ...
    삭제된 객체에 액세스하면 오류가 발생합니다. 삭제된 객체를 참조하는 객체를 resolve하면, 삭제된 객체 대신 `DeletedRef`가 반환됩니다.
  </Tab>
...
Weave에서 완전히 지정된 object ref URI는 다음과 같습니다:
