...
* PII 마스킹을 활성화하기 위해 `weave.init` 호출에서 켜거나 끌 수 있는 `redact_pii` 설정
* `redact_pii = True`일 때 [일반적인 entity](#entities-redacted-by-default)를 자동으로 마스킹
* 구성 가능한 `redact_pii_fields` 설정을 사용한 맞춤형 마스킹 필드
* `redact_pii_exclude_fields` 설정을 사용하여 특정 entity를 마스킹에서 제외
