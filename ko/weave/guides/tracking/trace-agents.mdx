...
| ------------ | ------------- | ------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 에이전트         | *(클래스 없음)*    | *(span 없음, `agent_name` 속성으로 그룹화됨)*         | Agents 탭의 에이전트형 애플리케이션으로, 하나 이상의 세션을 포함합니다            |                                                                                                                                |
| Session      | `Session`     | *(span 없음, 턴은 `conversation_id` 속성으로 그룹화됨)* | 하나 이상의 턴을 포함하는 대화 또는 run입니다.                          | [Python](/ko/weave/reference/typescript-sdk/classes/session) <br /> [TypeScript](/ko/weave/reference/typescript-sdk/classes/session) |
...
flowchart TB
    ...
    S1["세션 1<br/>conversation_id<br/>(OTel span 없음)"]
    S2["세션 2<br/>conversation_id<br/>(OTel span 없음)"]
...
세션은 상위 span이 아니라 공통 `conversation_id` 속성을 기준으로 턴을 그룹화하므로, 각 턴이 자체 OTel 트레이스를 시작합니다. 이 설계는 분산 트레이싱과 병렬 실행을 지원합니다. 클라이언트는 서버 측 집계 없이 span을 OTel collector로 직접 전송합니다.
