...
## 나만의 Scorer 만들기 {#create-your-own-scorers}
...
### 함수 기반 Scorer {#function-based-scorers}
...
### 클래스 기반 Scorers {#class-based-scorers}
...
## Scorer의 작동 방식 {#how-scorers-work}
...
### Scorer 키워드 인수 {#scorer-keyword-arguments}
...
### `column_map`으로 열 이름 매핑하기 {#mapping-column-names-with-column_map}
...
### `columnMapping`으로 열 이름 매핑하기 {#mapping-column-names-with-columnmapping}
...
### 스코어링 프롬프트에서 ops의 변수에 액세스하기 {#access-variables-from-your-ops-in-scoring-prompts}
...
### scorer의 최종 요약 {#final-summarization-of-the-scorer}
...
### Call에 Scorer 적용하기 {#apply-scorers-to-a-call}

`weave.Evaluation`의 일부로 Scorer를 실행하는 것 외에도, 개별 Call에 직접 적용할 수 있습니다. 이는 프로덕션 트래픽을 score하거나 특정 op 호출에 평가 메트릭을 연결하려는 경우에 유용합니다.

Weave op에 Scorer를 적용하려면 오퍼레이션의 결과와 tracking 정보에 모두 접근할 수 있는 `.call()` 방법을 사용하세요. 이렇게 하면 Scorer 결과를 Weave 데이터베이스의 특정 Call과 연결할 수 있습니다.

자체 인프라에서 실행되는 Scorer로 프로덕션 Call을 평가하려면 [원격 Scorer](/ko/weave/guides/evaluation/remote-scorers)를 monitor에 연결하세요.

`.call()` 방법 사용에 대한 자세한 내용은 [Calling Ops](../tracking/tracing#getting-a-handle-to-the-call-object-during-execution) 가이드를 참조하세요.
...
### `preprocess_model_input` 사용 {#use-preprocess_model_input}
...
### 단일 Call의 점수 분석하기 {#analyze-a-single-calls-scores}

#### 단일 Call API {#single-call-api}
...
### 여러 Call의 점수 분석하기 {#analyze-multiple-calls-scores}

#### 여러 Call API {#multiple-calls-api}
...
### 특정 Scorer가 점수를 매긴 모든 Call 분석하기 {#analyze-all-calls-scored-by-a-specific-scorer}

#### Scorer별 모든 Call API {#all-calls-by-scorer-api}
...
#### Scorer UI에서 Scorer별 모든 call 보기 {#all-calls-by-scorer-ui}
