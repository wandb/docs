...
<div id="when-to-use-signals-and-custom-monitors">
  ### signals와 맞춤형 모니터를 사용해야 하는 경우
</div>

프로덕션 트레이스용으로 미리 설정된 자동 Scorer인 [signals](/ko/weave/guides/evaluation/monitors)을 사용해 프로덕션 모니터링을 빠르게 시작한 다음, 애플리케이션별 평가 기준에 맞는 맞춤형 모니터를 추가하세요.
...
2. Weave side-nav에서 **Monitors**를 선택한 다음 **+ New Monitor** 버튼을 클릭합니다. 그러면 **Create new monitor** modal dialog가 열립니다.
...
3. Create new monitor 메뉴에서 다음 필드를 설정합니다.
   ...
   * **Calls to monitor**:
     * **오퍼레이션**: 모니터링할 `@weave.op`를 하나 이상 선택합니다. 사용 가능한 op 목록에 표시되려면 해당 op를 사용하는 트레이스를 최소 하나 이상 log해야 합니다.
     * **Filter** (선택): 점수화 대상이 될 call을 좁힙니다(예: `max_tokens` 또는 `top_p` 기준).
     * **Sampling rate**: 점수화할 call의 비율입니다(0%~100%).
       <Tip>
         각 점수화 call마다 비용이 발생하므로 sampling rate를 낮추면 비용을 줄일 수 있습니다.
       </Tip>
   * **LLM-as-a-judge configuration**:
     ...
     * **Judge model**: op를 점수화할 모델을 선택합니다. 이 메뉴에는 W&amp;B 계정에 설정한 상용 LLM 모델과 [Serverless Inference models](/ko/inference/models)이 포함됩니다. 오디오 지원 모델은 이름 옆에 **Audio Input** 레이블이 표시됩니다. 선택한 모델에 대해 다음 설정을 구성합니다.
       * **Configuration name**: 이 모델 설정의 이름입니다.
       * **System prompt**: 평가 모델의 역할과 페르소나를 정의합니다. 예를 들어 &quot;You are an impartial AI judge.&quot;와 같습니다.
...
4. 모니터 필드 설정을 완료한 후 **Create monitor**를 선택합니다. 그러면 모니터가 Weave 프로젝트에 추가됩니다. 코드가 트레이스를 생성하기 시작하면 **Traces** 탭에서 모니터 이름을 선택하고, 표시되는 panel의 데이터를 검토해 점수를 확인할 수 있습니다.

또한 Weave UI에서 모니터의 트레이스 데이터를 [비교](/ko/weave/guides/tools/comparison)하고 시각화하거나, Traces 탭의 다운로드 버튼(<Icon icon="download" iconType="regular" />)을 사용해 CSV나 JSON 등 다양한 형식으로 다운로드할 수 있습니다.

Weave는 모든 Scorer 결과를 [Call](/ko/weave/guides/tracking/tracing#calls) object의 `feedback` 필드에 자동으로 저장합니다.
...
3. W&amp;B UI에서 Weave 프로젝트를 열고 side-nav에서 **Monitors**를 선택하세요. 그런 다음 **New Monitor**를 선택하세요.
