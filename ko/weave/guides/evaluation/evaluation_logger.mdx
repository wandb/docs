...
`EvaluationLogger`는 Python 또는 TypeScript 코드에서 평가 데이터를 직접 로깅할 수 있는 유연하고 점진적인 방법을 제공합니다. Weave의 내부 데이터 타입을 깊이 알 필요는 없습니다. 로거 인스턴스를 생성한 뒤, 제공되는 메서드들(`log_prediction`, `log_score`, `log_summary`)을 사용해 평가 단계를 기록하면 됩니다.

이 접근 방식은 전체 데이터셋이나 모든 스코어러가 처음부터 정의되어 있지 않을 수 있는 복잡한 워크플로우에서 특히 유용합니다.

미리 정의된 `Dataset`과 `Scorer` 객체 목록이 필요한 표준 `Evaluation` 객체와 달리, `EvaluationLogger`를 사용하면 개별 예측과 해당 점수를 사용 가능해지는 대로 점진적으로 로깅할 수 있습니다.

<Info>
**더 구조화된 평가를 선호하나요?**

미리 정의된 데이터셋과 스코어러가 있는, 보다 의견이 반영된 평가 프레임워크를 선호한다면 [Weave의 표준 Evaluation 프레임워크](../core-types/evaluations)를 참고하세요. 

`EvaluationLogger`는 유연성을 제공하는 반면, 표준 프레임워크는 구조와 가이드를 제공합니다.
</Info>
...
1. _로거 초기화:_ `EvaluationLogger` 인스턴스를 생성하고, 필요하다면 `model` 및 `dataset`에 대한 메타데이터를 선택적으로 제공할 수 있습니다. 생략하면 기본값이 사용됩니다.
    <Note>
    LLM 호출(예: OpenAI)의 토큰 사용량 및 비용을 수집하려면, 어떤 LLM을 호출하기 **이전에** `EvaluationLogger`를 초기화하세요. 
    먼저 LLM을 호출한 뒤에 예측 결과를 로깅하면 토큰 및 비용 데이터는 수집되지 않습니다.
    </Note>
...
4. _예측 완료:_ 예측에 대한 점수 로깅을 마친 후에는 항상 `finish()`를 호출하여 예측을 최종 확정합니다.
5. _요약 로깅:_ 모든 예측이 처리된 후 `log_summary`를 호출하여 점수를 집계하고 선택적으로 커스텀 메트릭을 추가합니다.

<Warning>
하나의 예측에 대해 `finish()`를 호출한 이후에는 해당 예측에 더 이상 점수를 로깅할 수 없습니다.
</Warning>

위에서 설명한 워크플로를 보여주는 Python 코드 예시는 [기본 예제](#basic-example)를 참조하세요.

<div id="basic-example">
  ## 기본 예제
</div>

다음 예제는 기존 코드 흐름 안에서 `EvaluationLogger`를 사용해 예측값과 점수를 함께 로깅하는 방법을 보여줍니다.

<Tabs>
  <Tab title="Python">
    `user_model` 모델 함수가 정의되어 입력 목록에 적용됩니다. 각 예제에 대해 다음을 수행합니다:

    * 입력과 출력이 `log_prediction`을 사용해 로그에 기록됩니다.
    * 단순 정확도 점수(`correctness_score`)가 `log_score`를 통해 로그에 기록됩니다.
    * `finish()`가 해당 예측에 대한 로깅을 마무리합니다.
      마지막으로 `log_summary`가 집계 메트릭을 기록하고, Weave에서 점수를 자동으로 요약하도록 트리거합니다。
  ...
  <Tab title="TypeScript">
    ...
    1. **Fire-and-forget API** (대부분의 경우 권장): 동기식이면서 논블로킹 방식으로 로깅할 때 `await` 없이 `logPrediction()`을 사용합니다.
    2. **Awaitable API**: 이후 로직으로 넘어가기 전에 작업이 완료되었는지 반드시 보장해야 할 때 `await`와 함께 `logPredictionAsync()`를 사용합니다.

    다음과 같은 경우에는 **fire-and-forget** 방식을 권장합니다:

    * **높은 처리량**: 각 로깅 작업을 기다리지 않고 여러 예측을 병렬로 처리
    * **코드 변경 최소화**: 기존 async/await 흐름을 재구성하지 않고 평가 로깅을 추가
    * **단순성**: 대부분의 평가 시나리오에서 보일러플레이트 코드를 줄이고 더 깔끔한 문법 제공

    fire-and-forget 패턴은 안전합니다. `logSummary()`가 결과를 집계하기 전에 모든 보류 중인 작업이 완료될 때까지 자동으로 기다리기 때문입니다.

    다음 예제는 fire-and-forget 패턴으로 모델 예측을 평가합니다. evaluation logger를 설정하고, 세 개의 테스트 샘플에 대해 간단한 모델을 실행한 뒤, `await`를 사용하지 않고 예측을 로깅합니다:
    ...
    // 예시를 순회하며 예측하고 fire-and-forget 패턴으로 로깅
    ...
    각 작업이 완료된 뒤에야 다음 단계로 진행해야 하는 경우, 예를 들어 오류 처리나 순차적 의존성을 관리해야 할 때는 awaitable API를 사용하세요.

    다음 예제에서는 `await` 없이 `logPrediction()`을 호출하는 대신, `await`와 함께 `logPredictionAsync()`를 사용하여 각 작업이 다음 작업으로 넘어가기 전에 완료되도록 보장합니다:
...
`EvaluationLogger`는 기본 워크플로를 넘어서는 유연한 패턴을 제공하여 더 복잡한 평가 시나리오를 처리할 수 있도록 합니다. 이 섹션에서는 자동 리소스 관리를 위한 컨텍스트 매니저 사용, 모델 실행과 로깅의 분리, 풍부한 미디어 데이터 처리, 여러 모델 평가를 나란히 비교하기 등의 고급 기법을 다룹니다.

<div id="using-context-managers">
  ### 컨텍스트 매니저 사용하기
</div>

`EvaluationLogger`는 예측과 점수 모두에 대해 컨텍스트 매니저(`with` 구문)를 지원합니다. 이를 사용하면 코드가 더 깔끔해지고, 리소스가 자동으로 정리되며, LLM 판정 호출과 같은 중첩 연산을 더 잘 추적할 수 있습니다.

이 문맥에서 `with` 구문을 사용하면 다음과 같은 장점이 있습니다:
...
<Tab title="TypeScript">
TypeScript에는 컨텍스트 매니저를 위한 Python의 `with` 구문 패턴이 없습니다. 대신 명시적인 `finish()` 호출이 포함된 fire-and-forget 패턴을 사용합니다.

다음 예제에서는 예측을 로깅하고, 단순 점수와 LLM 판정 점수를 추가한 다음, `finish()`로 예측을 마무리합니다:
...
<Note>
TypeScript에는 컨텍스트 매니저를 통한 자동 정리 기능이 없지만, `logSummary()`는 결과를 집계하기 전에 완료되지 않은 모든 예측을 자동으로 finish 처리합니다. `finish()`를 명시적으로 호출하지 않으려는 경우 이 동작에 의존해도 됩니다.
</Note>
</Tab>
</Tabs>
...
원시 데이터셋을 `inputs`로 `log_prediction`에 전달하면 Weave는 평가 run이 실행될 때마다 데이터를 다시 수집합니다. 이로 인해 중복 데이터가 저장되며, 데이터셋이 크거나 많은 평가에서 이를 재사용하는 경우 저장 공간이 낭비될 수 있습니다.

이러한 중복을 피하려면 평가를 실행하기 전에 데이터셋을 Weave에 게시한 다음, 게시된 데이터셋의 행을 `inputs`로 전달하세요. 그러면 Weave는 데이터를 다시 수집하는 대신 내부 참조를 사용해 게시된 행에 대한 참조를 확인합니다. 이 기법을 사용하면 표준 [Evaluation 프레임워크](../core-types/evaluations)와 동일한 연결 방식을 사용할 수 있으며, 각 prediction이 Weave UI에서 특정 데이터셋 행에 연결됩니다.

다음 예제에서는 데이터셋을 게시하고 `EvaluationLogger`에서 해당 데이터셋에 연결한 뒤, 다른 데이터셋과 마찬가지로 이를 조회하고 반복 처리합니다.
...
// OpenAI 호출과 같은 모델 출력은 토큰 추적을 위해 로거 초기화 이후에 수행해야 함
...
// logSummary는 모든 보류 중인 작업이 완료될 때까지 대기
await ev.logSummary();
```
...
<Tab title="TypeScript">
TypeScript SDK는 `weaveImage` 및 `weaveAudio` 함수를 사용해 이미지와 오디오 로깅을 지원합니다. 다음 예제는 이미지와 오디오 파일을 로드한 뒤 모델을 통해 처리하고, 점수와 함께 결과를 로깅합니다.
...
for (const inputs of richMediaDataset) {
  ...
  // 입력과 출력 모두에 리치 미디어가 포함된 prediction 로깅
...
- 단일 예측에 묶여 있지 않은 메트릭(예: 전체 레이턴시)을 수집하려면 `log_summary`를 사용하세요.
- 리치 미디어 로깅 기능은 정성적 분석에 매우 유용합니다.
</Tab>
<Tab title="TypeScript">
- **자동 종료 동작**: 명확성을 위해 각 예측마다 `finish()`를 명시적으로 호출하는 것을 권장하지만, `logSummary()`는 종료되지 않은 예측을 자동으로 종료합니다. 단, 스크립트에서 한 번 `finish()`를 호출하면 더 이상 스코어를 로깅할 수 없습니다.
