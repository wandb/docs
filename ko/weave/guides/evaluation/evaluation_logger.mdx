...
다음 예제는 기존 코드 안에서 바로 `EvaluationLogger`를 사용해 예측과 점수를 기록하는 방법을 보여줍니다.
...
<Tabs>
  <Tab title="Python">
    ...
    * `finish()`는 해당 prediction에 대한 로깅을 마무리합니다.
    ...
    for sample in eval_samples:
        ...
        pred_logger.log_score(
            scorer="correctness", # scorer의 문자열 이름
  ...
  <Tab title="TypeScript">
    ...
    * **단순성**: 대부분의 평가 시나리오에서 상용구 코드가 더 적고 문법이 더 깔끔함
    ...
    // logSummary는 내부적으로 모든 대기 중인 오퍼레이션이 완료될 때까지 대기합니다
...
* prediction 컨텍스트 내에서 모델 실행 후 output 설정
...
<Tabs>
  <Tab title="Python">
    ...
    # prediction에 컨텍스트 매니저 사용 - finish()를 호출할 필요 없음
    ...
    이 패턴을 사용하면 모든 중첩 오퍼레이션이 추적되고 상위 prediction에 연결되므로, Weave UI에서 정확한 token 사용량과 비용 데이터를 확인할 수 있습니다.
  </Tab>

  <Tab title="TypeScript">
    TypeScript에는 컨텍스트 매니저를 위한 Python의 `with` 문 패턴이 없습니다. 대신 `finish()`를 명시적으로 호출하는 fire-and-forget 패턴을 사용하세요.

    다음 예제에서는 prediction을 log하고, 단순한 점수와 LLM judge 점수를 추가한 다음, `finish()`로 prediction을 마무리합니다.
    ...
    // LLM judge 점수의 경우, call을 수행하고 결과를 log
    ...
    <Note>
      TypeScript에는 컨텍스트 매니저를 통한 자동 정리 기능이 없지만, `logSummary()`는 결과를 집계하기 전에 완료되지 않은 prediction을 자동으로 finish합니다. `finish()`를 명시적으로 호출하지 않으려면 이 동작에 의존해도 됩니다.
    </Note>
  </Tab>
</Tabs>
...
이 중복을 방지하려면 평가를 실행하기 전에 먼저 데이터셋을 Weave에 게시한 다음, 게시된 데이터셋의 행을 `inputs`로 전달하세요. 그러면 Weave는 데이터를 다시 수집하는 대신 내부 참조를 사용해 게시된 행 참조를 해석합니다. 이 방식을 사용하면 표준 [Evaluation 프레임워크](../core-types/evaluations)와 동일하게 연결된 환경을 사용할 수 있으며, 각 prediction은 Weave UI의 특정 데이터셋 행에 다시 연결됩니다.
...
<Tabs>
  <Tab title="Python">
    ```python lines
    # token tracking을 위해 모델을 호출하기 BEFORE에 EvaluationLogger를 초기화합니다
    ev = EvaluationLogger(
        model="example_model",
        dataset="example_dataset"
    )

    # token tracking을 위해 모델 출력(예: OpenAI call)은 logger init 이후에 발생해야 합니다
  ...
  <Tab title="TypeScript">
    ...
    // token tracking을 위해 OpenAI call과 같은 모델 출력은 logger init 이후에 발생해야 합니다
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    for (const inputs of richMediaDataset) {
      ...
      // 입력과 출력 모두에 리치 미디어가 포함된 prediction 기록
  ...
  <Tab title="TypeScript">
    ...
    for (const model of models) {
      ...
      for (const inputs of yourDataset) {
        ...
        // 간결하고 효율적인 로깅을 위한 fire-and-forget 패턴
        const pred = ev.logPrediction(inputs, output);
...
<Frame>
  ![Compare 뷰](/weave/guides/evaluation/img/comparison.png)
</Frame>
