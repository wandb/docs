...
keywords: ["EvaluationLogger", "log_prediction", "log_score", "점진적 평가", "리치 미디어 기록"]
---
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    * **단순성**: 대부분의 평가 시나리오에서 상용구 코드가 더 적고 문법이 더 깔끔합니다.
    ...
    다음 예제는 fire-and-forget 패턴으로 모델 예측을 평가합니다. 평가 로거를 설정하고, 세 개의 테스트 샘플에 대해 모델을 실행한 다음, await를 사용하지 않고 예측을 로깅합니다:
...
<div id="simplified-logging-with-log_example">
  ## `log_example()`를 사용한 간소화된 로깅 {#simplified-logging-with-log_example}
</div>
...
<div id="use-context-managers">
  ### 컨텍스트 매니저 사용 {#use-context-managers}
</div>

`EvaluationLogger`는 예측과 점수 모두에 대해 컨텍스트 매니저(`with` 문)를 지원합니다. 이를 사용하면 코드를 더 깔끔하게 작성할 수 있고, 리소스를 자동으로 정리할 수 있으며, LLM judge Call과 같은 중첩된 오퍼레이션도 더 효과적으로 추적할 수 있습니다.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    TypeScript에는 컨텍스트 매니저를 위한 Python의 `with` 문 패턴이 없습니다. 대신 명시적으로 `finish()`를 호출하는 fire-and-forget 패턴을 사용하세요.
    ...
    <Note>
      TypeScript에는 컨텍스트 매니저를 통한 자동 정리 기능이 없지만, `logSummary()`는 결과를 집계하기 전에 완료되지 않은 예측을 자동으로 마무리합니다. `finish()`를 명시적으로 호출하지 않으려면 이 동작을 활용할 수 있습니다.
    </Note>
  </Tab>
</Tabs>

<div id="link-agent-traces-to-evaluations">
  ### 에이전트 트레이스를 평가에 연결하기 {#link-agent-traces-to-evaluations}
</div>
...
<div id="link-spans-you-instrument-yourself">
  #### 직접 계측한 span 연결 {#link-spans-you-instrument-yourself}
</div>
...
<div id="link-an-agent-that-runs-in-a-separate-service">
  #### 별도의 서비스에서 실행되는 에이전트 연결 {#link-an-agent-that-runs-in-a-separate-service}
</div>
...
<div id="view-linked-agent-spans-from-your-evaluations">
  #### evaluation에서 연결된 에이전트 span 보기 {#view-linked-agent-spans-from-your-evaluations}
</div>
...
### 기존 데이터셋에 연결하기 {#link-to-an-existing-dataset}
...
<div id="get-outputs-before-logging">
  ### 로깅 전에 출력 먼저 계산하기 {#get-outputs-before-logging}
</div>
...
### 여러 평가 기록 및 비교 {#log-and-compare-multiple-evaluations}
...
4. **Compare**를 클릭합니다. Compare 뷰에서 다음 작업을 할 수 있습니다.
   * 추가하거나 제거할 평가 선택.
   * 표시하거나 숨길 메트릭 선택.
   * 특정 예시를 페이지별로 살펴보며, 주어진 데이터셋에서 동일한 입력에 대해 서로 다른 모델이 어떻게 수행되었는지 확인.
...
<Frame>
  ![평가 run 목록을 표시하는 Evals 탭](/weave/guides/evaluation/img/evals_tab.png)
</Frame>

<Frame>
  ![여러 평가 run의 메트릭을 표시하는 Compare 뷰](/weave/guides/evaluation/img/comparison.png)
</Frame>
