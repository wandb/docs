...
`EvaluationLogger`는 Python 또는 TypeScript 코드에서 평가 데이터를 직접 로깅할 수 있는 유연하고 점진적인 방법을 제공합니다. Weave의 내부 데이터 타입을 자세히 알 필요는 없습니다. 로거 인스턴스를 생성한 뒤, 해당 방법(`log_prediction`, `log_score`, `log_summary`)를 사용해 평가 step을 기록하면 됩니다.

이 방식은 전체 데이터셋이나 모든 스코어러가 처음부터 정의되어 있지 않을 수 있는 복잡한 워크플로에서 특히 유용합니다.

미리 정의된 `Dataset`과 `Scorer` 객체 목록이 필요한 표준 `Evaluation` 객체와 달리, `EvaluationLogger`를 사용하면 개별 예측과 해당 점수를 준비되는 대로 점진적으로 로깅할 수 있습니다.

<Info>
  **더 구조화된 평가를 선호하나요?**

  미리 정의된 데이터셋과 스코어러를 사용하는, 더 규범적인 평가 프레임워크를 원한다면 [Weave의 표준 Evaluation 프레임워크](../core-types/evaluations)를 참조하세요.
...
1. *로거 초기화:* `EvaluationLogger` 인스턴스를 생성하고, 필요에 따라 `model` 및 `dataset`에 대한 메타데이터를 선택적으로 제공할 수 있습니다. 생략하면 기본값이 사용됩니다.
   <Note>
     LLM Call(예: OpenAI)의 토큰 사용량 및 비용을 캡처하려면, LLM을 호출하기 **이전에** `EvaluationLogger`를 초기화하세요.
     먼저 LLM을 Call한 다음 나중에 예측을 로깅하면 토큰 및 비용 데이터는 캡처되지 않습니다.
   </Note>
2. *예측 로깅:* 시스템의 각 입력/출력 쌍에 대해 `log_prediction`을 호출합니다.
3. *점수 로깅:* 반환된 `ScoreLogger`를 사용해 해당 예측에 대해 `log_score`를 호출합니다. 하나의 예측에 대해 여러 개의 점수를 기록할 수 있습니다.
4. *예측 완료:* 예측의 점수 로깅을 마친 후에는 항상 `finish()`를 호출하여 예측을 최종 확정합니다.
5. *요약 로깅:* 모든 예측이 처리된 후 `log_summary`를 호출하여 점수를 집계하고 선택적으로 커스텀 메트릭을 추가합니다.

<Warning>
  하나의 예측에 대해 `finish()`를 호출한 이후에는 해당 예측에 더 이상 점수를 로깅할 수 없습니다.
</Warning>

설명한 워크플로를 보여주는 Python 코드 예시는 [기본 예시](#basic-example)를 참조하세요.

<div id="basic-example">
  ## 기본 예시
</div>

다음 예시는 기존 코드 흐름 안에서 `EvaluationLogger`를 사용해 예측과 점수를 로깅하는 방법을 보여줍니다.

<Tabs>
  <Tab title="Python">
    `user_model` 모델 함수가 정의되고 입력 목록에 적용됩니다. 각 예시에 대해:
    ...
    * `finish()`는 해당 예측의 로깅을 마무리합니다.
    ...
    # 예시 입력 데이터 (원하는 데이터 구조를 사용할 수 있습니다)
    eval_samples = [
    ...
    def user_model(a: int, b: int) -> int:
        ...
        # Response를 활용합니다 (여기서는 단순하게 a + b를 반환합니다)
        return a + b

    # 예시를 순회하며 예측하고 로깅합니다
    for sample in eval_samples:
        inputs = sample["inputs"]
        model_output = user_model(**inputs) # 입력값을 kwargs로 전달합니다
        ...
        pred_logger.log_score(
            scorer="correctness", # scorer의 단순한 string 이름
  ...
  <Tab title="TypeScript">
    ...
    1. **Fire-and-forget API**(대부분의 경우 권장): 동기식 비차단 로깅을 위해 `await` 없이 `logPrediction()`을 사용하세요.
    2. **Awaitable API**: 진행하기 전에 오퍼레이션이 완료되도록 보장해야 하는 경우 `await`와 함께 `logPredictionAsync()`를 사용하세요.

    다음과 같은 경우에는 **fire-and-forget**을 권장합니다.

    * **높은 처리량**: 각 로깅 오퍼레이션을 기다리지 않고 여러 예측을 병렬로 처리합니다.
    * **최소한의 코드 변경**: 기존 async/await 흐름을 재구성하지 않고 Evaluation 로깅을 추가합니다.
    * **단순함**: 대부분의 Evaluation 시나리오에서 상용구 코드가 더 적고 구문이 더 깔끔합니다.

    fire-and-forget 패턴이 안전한 이유는 `logSummary()`가 결과를 집계하기 전에 보류 중인 모든 오퍼레이션이 완료될 때까지 자동으로 기다리기 때문입니다.

    다음 예시는 fire-and-forget 패턴으로 모델 예측을 Evaluate합니다. Evaluation logger를 설정하고, 세 개의 테스트 샘플에 대해 단순한 모델을 실행한 다음, await를 사용하지 않고 예측을 로깅합니다.
    ...
    // 예시를 순회하며 예측하고 fire-and-forget 패턴으로 로깅합니다
    ...
    // logSummary는 내부적으로 모든 대기 중인 오퍼레이션이 완료될 때까지 기다립니다
    ...
    오류 handling 또는 순차적 의존성을 관리할 때처럼, 각 오퍼레이션이 진행하기 전에 완료되도록 보장해야 하는 경우 awaitable API를 사용하세요.

    다음 예시에서는 `await` 없이 `logPrediction()`을 호출하는 대신, 각 오퍼레이션이 다음 오퍼레이션으로 진행하기 전에 완료되도록 보장하기 위해 `await`와 함께 `logPredictionAsync()`를 사용합니다:
...
`EvaluationLogger`는 기본 워크플로를 넘어, 더 복잡한 평가 시나리오에 대응할 수 있는 유연한 패턴을 제공합니다. 이 섹션에서는 자동 리소스 관리를 위한 컨텍스트 매니저 사용, 모델 실행과 로깅 분리, 리치 미디어 데이터 활용, 그리고 여러 모델 평가를 나란히 비교하는 방법 등 고급 기법을 다룹니다.

<div id="using-context-managers">
  ### 컨텍스트 매니저 사용하기
</div>

`EvaluationLogger`는 예측과 점수 모두에 대해 컨텍스트 매니저(`with` 구문)를 지원합니다. 이를 사용하면 코드를 더 깔끔하게 작성할 수 있고, 리소스를 자동으로 정리할 수 있으며, LLM judge Call과 같은 중첩 오퍼레이션도 더 효과적으로 추적할 수 있습니다.

이 컨텍스트에서 `with` 구문을 사용하면 다음과 같은 이점이 있습니다:

* 컨텍스트를 벗어날 때 `finish()`가 자동으로 호출됨
* 중첩된 LLM Call의 토큰/비용 추적 개선
* 예측 컨텍스트 내에서 모델 실행 후 output 설정
...
<Tabs>
  <Tab title="Python">
    ...
    with ev.log_prediction(inputs={"user_prompt": user_prompt}) as pred:
        ...
        # LLM Call이 필요한 점수에는 중첩 컨텍스트 매니저 사용
    ...
    이 패턴을 사용하면 모든 중첩 오퍼레이션이 상위 예측에 연결되어 추적되므로, Weave UI에서 정확한 토큰 사용량과 비용 데이터를 확인할 수 있습니다.
  </Tab>

  <Tab title="TypeScript">
    TypeScript에는 컨텍스트 매니저를 위한 Python의 `with` 구문 패턴이 없습니다. 대신 명시적으로 `finish()`를 호출하는 fire-and-forget 패턴을 사용하세요.

    다음 예시에서는 예측을 로깅하고, 단순 점수와 LLM judge 점수를 추가한 다음, `finish()`로 예측을 마무리합니다:
    ...
    // LLM judge 점수의 경우, 호출을 수행하고 결과를 로깅
    ...
    <Note>
      TypeScript에는 컨텍스트 매니저를 통한 자동 정리 기능이 없지만, `logSummary()`는 결과를 집계하기 전에 완료되지 않은 모든 예측을 자동으로 finish 처리합니다. `finish()`를 명시적으로 호출하지 않으려는 경우 이 동작을 활용할 수 있습니다.
    </Note>
  </Tab>
</Tabs>
...
원시 데이터셋을 `log_prediction`에 `inputs`로 전달하면, Weave는 평가 런이 실행될 때마다 데이터를 다시 수집합니다. 이렇게 하면 중복 데이터가 저장되며, 데이터셋이 크거나 많은 평가에서 이를 재사용하는 경우 공간이 낭비될 수 있습니다.

이 중복을 피하려면, 평가를 실행하기 전에 데이터셋을 Weave에 게시한 다음 게시된 데이터셋의 행을 `inputs`로 전달하세요. Weave는 데이터를 다시 수집하는 대신 내부 레퍼런스를 사용해 게시된 행에 대한 레퍼런스를 확인합니다. 이 기법을 사용하면 표준 [Evaluation 프레임워크](../core-types/evaluations)와 동일한 연결 경험을 제공하며, 각 예측은 Weave UI에서 특정 데이터셋 행으로 다시 연결됩니다.

다음 예시는 데이터셋을 게시하고 `EvaluationLogger`에서 이에 연결한 뒤, 다른 데이터셋과 마찬가지로 이를 조회하고 반복 처리합니다.
...
먼저 모델 출력을 계산한 다음, 예측과 점수를 별도로 로깅할 수 있습니다. 이렇게 하면 평가 로직과 로깅 로직을 더 잘 분리할 수 있습니다.
...
<Tabs>
  <Tab title="Python">
    ...
    # 토큰 추적을 위해 모델 출력(예: OpenAI Call)은 로거 초기화 이후에 수행해야 함
  ...
  <Tab title="TypeScript">
    ...
    다음 예시는 여러 개의 동시 `EvaluationLogger` 인스턴스를 생성하여 평가를 병렬로 배치 처리합니다:
    ...
    // OpenAI Call과 같은 모델 출력은 토큰 추적을 위해 로거 초기화 이후에 수행해야 함
    ...
    // logSummary는 모든 보류 중인 오퍼레이션이 완료될 때까지 대기
    await ev.logSummary();
    ```

    `fire-and-forget` 패턴을 사용하면 사용 가능한 컴퓨트 리소스가 허용하는 한 많은 평가를 병렬로 처리할 수 있습니다.
  </Tab>
</Tabs>
...
입력, 출력, 점수에는 이미지, 비디오, 오디오, 구조화된 테이블 등의 리치 미디어를 포함할 수 있습니다. `log_prediction` 또는 `log_score` 방법에 dict나 미디어 객체를 전달하기만 하면 됩니다.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    TypeScript SDK는 `weaveImage` 및 `weaveAudio` 함수를 사용해 이미지와 오디오를 로깅할 수 있도록 지원합니다. 다음 예시에서는 이미지와 오디오 파일을 로드해 모델로 처리한 후, 점수와 함께 결과를 로깅합니다.
...
`EvaluationLogger`를 사용하면 여러 평가를 로깅하고 비교할 수 있습니다.

1. 아래에 표시된 코드 예시를 실행합니다.
...
4. **Compare** 버튼을 클릭합니다. Compare 뷰에서 다음을 수행할 수 있습니다.
   ...
   * 특정 예시를 넘겨 보면서 동일한 데이터셋의 동일한 입력에 대해 서로 다른 모델이 어떻게 동작했는지 확인할 수 있습니다.
   ...
   비교에 대한 자세한 내용은 [비교(Comparisons)](../tools/comparison)을 참조하세요.
...
<Tabs>
  <Tab title="Python">
    * 각 예측 후에 `finish()`를 즉시 호출하세요.
    * 단일 예측과 직접 연결되지 않은 메트릭(예: 전체 레이턴시)을 캡처하려면 `log_summary`를 사용하세요.
    * 리치 미디어 로깅은 정성적 분석에 매우 유용합니다.
  </Tab>

  <Tab title="TypeScript">
    * **자동 종료 동작**: 명확성을 위해 각 예측마다 `finish()`를 명시적으로 호출하는 것을 권장하지만, `logSummary()`는 미완료된 예측을 자동으로 종료합니다. 그러나 스크립트에서 `finish()`를 호출한 후에는 더 이상 점수를 로깅할 수 없습니다.
    * **설정 옵션**: `name`, `description`, `dataset`, `model`, `scorers`, `attributes`와 같은 설정 옵션을 사용하여 Weave UI에서 평가를 구성하고 필터링하세요.
  </Tab>
</Tabs>