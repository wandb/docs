...
description: "OpenTelemetry를 사용해 Weave에서 Google Agent Development Kit (ADK) 에이전트 및 도구 호출 추적하기"
---

[Google Agent Development Kit (ADK)](https://google.github.io/adk-docs/) 에이전트와 도구 호출을 [OpenTelemetry (OTEL)](https://opentelemetry.io/)을 사용해 Weave에서 추적할 수 있습니다. ADK는 AI 에이전트를 개발하고 배포하기 위한 유연하고 모듈식 프레임워크입니다. Gemini 및 Google 생태계에 최적화되어 있지만, ADK는 모델 및 배포 환경에 종속되지 않습니다. 단순한 작업부터 복잡한 워크플로까지 다양한 에이전트 아키텍처를 생성, 배포, 오케스트레이션하는 데 필요한 도구를 제공합니다.

이 가이드는 OTEL을 사용해 ADK 에이전트와 도구 호출을 추적하고, 해당 트레이스를 Weave에서 시각화하는 방법을 설명합니다. 필요한 의존성을 설치하고, Weave로 데이터를 전송하는 OTEL 트레이서를 구성하고, ADK 에이전트와 도구에 계측을 추가하는 방법을 학습하게 됩니다.
...
ADK에서 Weave로 trace를 전송하려면 OTEL에 `TracerProvider`와 `OTLPSpanExporter`를 설정해야 합니다. exporter의 엔드포인트와 HTTP 헤더를 [인증 및 프로젝트 식별을 위한 올바른 값](#required-configuration)으로 설정하십시오.

<Warning>
  API 키와 프로젝트 정보 같은 민감한 환경 변수는 `.env`와 같은 환경 파일에 저장하고, `os.environ`을 사용해 로드할 것을 권장합니다. 이렇게 하면 자격 증명을 더 안전하게 보호하고 코드베이스에 노출되지 않도록 할 수 있습니다.
</Warning>
...
* **Endpoint:** `https://trace.wandb.ai/otel/v1/traces`. 전용 Weave 인스턴스를 사용하는 경우에는 대신 다음 패턴의 URL을 사용합니다: `{YOUR_WEAVE_HOST}/traces/otel/v1/traces`
* **Headers:**
...
다음 코드 스니펫은 OTLP span exporter와 tracer provider를 구성하여 ADK 애플리케이션에서 Weave로 OTEL 트레이스를 전송하는 방법을 보여줍니다.

<Warning>
  Weave가 ADK를 정확히 트레이싱하도록 하려면 코드에서 ADK 컴포넌트를 사용하기 *전에* 전역 tracer provider를 설정해야 합니다.
</Warning>
...
트레이서 프로바이더를 설정한 후, 자동 추적이 활성화된 ADK 에이전트를 생성하고 실행할 수 있습니다. 다음 예제는 도구가 포함된 간단한 LLM 에이전트를 생성하고 인메모리 러너로 실행하는 방법을 보여줍니다:
...
async def run_agent():
    ...
    agent = LlmAgent(
        name="MathAgent",
        model="gemini-2.0-flash",  # 필요에 따라 다른 모델로 변경할 수 있습니다
        instruction=(
...
모든 에이전트 작업은 자동으로 트레이싱되어 Weave로 전송되며, 실행 흐름을 시각화할 수 있습니다. 여기에서 모델 호출, 추론 단계, 그리고 도구 호출을 확인할 수 있습니다.
...
ADK로 도구를 정의하고 사용할 때, 해당 도구 호출도 트레이스에 함께 캡처됩니다. OTEL 통합은 에이전트의 추론 과정과 개별 도구 실행을 모두 자동으로 계측하여, 에이전트의 동작을 포괄적으로 파악할 수 있도록 해 줍니다.

다음은 여러 도구를 사용하는 예제입니다:
...
ADK는 더 복잡한 시나리오를 위한 다양한 [*workflow agents*](https://google.github.io/adk-docs/agents/workflow-agents/)를 제공합니다. 일반적인 LLM 에이전트와 마찬가지로 워크플로 에이전트도 추적할 수 있습니다. 다음은 [`SequentialAgent`](https://google.github.io/adk-docs/agents/workflow-agents/sequential-agents/)를 사용하는 예입니다:
...
이 워크플로 에이전트 트레이스는 Weave에서 두 에이전트가 순차적으로 실행되는 과정을 보여 주며, 데이터가 멀티 에이전트 시스템을 통해 어떻게 흐르는지에 대한 가시성을 제공합니다.

<Frame>
  ![Sequential 워크플로 에이전트 트레이스 시각화](/weave/guides/integrations/imgs/google_adk/adk_workflow_trace.png)
</Frame>
