...
3. [Weave에서 OTEL 추적 설정](#weave에서-otel-추적-설정)을 수행합니다.

### Weave에서 OTEL 추적 설정

ADK에서 Weave로 트레이스를 보내려면 `TracerProvider` 및 `OTLPSpanExporter`로 OTEL을 구성해야 합니다. [인증 및 프로젝트 식별을 위한 올바른 엔드포인트와 HTTP 헤더](#필수-설정)로 exporter를 설정하세요.
