...
Weave는 [OpenAI Python 라이브러리](https://developers.openai.com/api/docs/libraries)에 대한 트레이스를 자동으로 캡처할 수 있습니다.

`weave.init(<project-name>)`을(를) 호출해 원하는 프로젝트 이름을 지정하면 캡처가 시작됩니다. OpenAI는 언제 임포트하더라도 자동으로 패치됩니다.

`weave.init()`을(를) 호출할 때 W&amp;B 팀을 지정하지 않으면 기본 엔티티가 사용됩니다. 기본 엔티티를 찾거나 변경하려면 W&amp;B Models 문서의 [User Settings](/ko/platform/app/settings-page/user-settings/#default-team)를 참고하세요.
...
Weave는 OpenAI API 호출에 대한 비용을 자동으로 추적합니다. Weave UI에서 비용 내역을 확인할 수 있습니다.
...
이제 OpenAI에 대한 트레이싱을 설정했으므로 다음을 할 수 있습니다.

1. **Weave UI에서 트레이스 뷰**: Weave 프로젝트로 이동하여 OpenAI 호출의 트레이스를 확인합니다.
2. **평가 생성**: 트레이스를 사용해 평가 데이터셋을 구축합니다.
3. **성능 모니터링**: 지연 시간, 비용 및 기타 메트릭을 추적합니다.
4. **문제 디버그**: 트레이스를 사용해 LLM 애플리케이션에서 어떤 일이 일어나고 있는지 파악합니다.
