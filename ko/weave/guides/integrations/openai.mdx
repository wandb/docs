...
<Note>
  별도의 설정 없이 Weave에서 OpenAI 모델을 실험해 보고 싶나요? [LLM 플레이그라운드](../tools/playground)를 사용해 보세요.
</Note>
...
LLM 애플리케이션의 트레이스를 개발 중은 물론 프로덕션 환경에서도 중앙 데이터베이스에 저장하는 것이 중요합니다. 이러한 트레이스는 디버깅에 활용할 수 있으며, 애플리케이션을 개선하는 동안 평가에 사용할 까다로운 예시 데이터셋을 구축하는 데도 도움이 됩니다.

Weave는 [OpenAI Python 라이브러리](https://developers.openai.com/api/docs/libraries)의 트레이스를 자동으로 캡처할 수 있습니다.

원하는 프로젝트 이름으로 `weave.init(<project-name>)`을 호출하면 캡처가 시작됩니다. OpenAI는 임포트 시점과 관계없이 자동으로 패치됩니다.

`weave.init()`을 호출할 때 W&amp;B 팀을 지정하지 않으면 기본 엔티티가 사용됩니다. 기본 엔티티를 찾거나 업데이트하려면 W&amp;B Models 문서의 [User Settings](https://docs.wandb.ai/platform/app/settings-page/user-settings/#default-team)를 참고하세요.

**자동 패치**

Weave는 `weave.init()` 호출 전이나 후에 OpenAI를 임포트했는지와 관계없이 자동으로 OpenAI를 패치합니다:
...
더 세밀하게 제어하고 싶다면 명시적으로 패치할 수도 있습니다:
...
<Tip>
  [OpenAI Functions](https://platform.openai.com/docs/guides/function-calling) 및 [OpenAI Assistants](https://platform.openai.com/docs/assistants/overview)의 함수 호출 도구도 캡처합니다.
</Tip>
...
Weave는 OpenAI와 함께 구조화된 출력도 지원합니다. 이는 LLM Response가 특정 형식을 따르도록 보장하는 데 유용합니다.
...
Weave는 OpenAI의 스트리밍 Response도 지원합니다.
...
Weave는 여러 요청을 처리하기 위해 OpenAI Batch API도 지원합니다.
...
Weave는 OpenAI API 호출 비용을 자동으로 추적합니다. Weave UI에서 비용 내역을 확인할 수 있습니다.

<Note>
  비용 추적은 모든 OpenAI 모델에서 사용 가능하며, 최신 OpenAI 가격을 기준으로 계산됩니다.
</Note>
...
`@weave.op` 데코레이터로 OpenAI를 사용하는 맞춤형 함수도 트레이싱할 수 있습니다.
...
이제 OpenAI용 트레이싱을 설정했으므로 다음 작업을 수행할 수 있습니다.

1. **Weave UI에서 트레이스 보기**: Weave 프로젝트로 이동하여 OpenAI 호출의 트레이스를 확인합니다.
2. **평가 생성**: 트레이스를 사용해 평가용 데이터셋을 구축합니다.
3. **성능 모니터링**: 지연 시간, 비용 및 기타 메트릭을 추적합니다.
4. **문제 디버깅**: 트레이스를 사용해 LLM 애플리케이션에서 무슨 일이 일어나고 있는지 파악합니다.

이러한 주제에 대한 자세한 내용은 [평가 가이드](../evaluation/scorers)와 [모니터링 가이드](../tracking)를 확인하세요.