---
title: "Not Diamond ¬◇"
description: "Not Diamond를 사용해 요구사항에 맞는 적합한 모델로 프롬프트를 라우팅하여 정확도를 극대화하고 비용을 절감하세요"
---

복잡한 LLM 워크플로를 구축할 때는 정확도, 비용 또는 호출 지연 시간에 따라 서로 다른 모델에 프롬프트해야 할 수 있습니다.
이러한 워크플로에서 [Not Diamond][nd]를 사용하면 프롬프트를 요구사항에 맞는 적합한 모델로 라우팅하여
모델 비용을 절감하면서 정확도를 극대화하는 데 도움이 됩니다.

<div id="getting-started">
  ## 시작하기
</div>

먼저 [계정을 생성][account]하고 [API 키를 발급][keys]한 후, API 키를 환경 변수에 `NOTDIAMOND_API_KEY`로 추가하세요.

![Create an API key](/weave/guides/integrations/imgs/notdiamond/api-keys.png)

여기서 다음을 할 수 있습니다.

* [퀵스타트 가이드][quickstart guide]를 사용해 보거나,
* W&amp;B Weave와 Not Diamond를 사용해 [맞춤형 라우터][custom router]를 구축하거나,
* [Not Diamond와 채팅][chat]하여 라우팅이 실제로 작동하는지 확인할 수 있습니다
...
워크플로 시작 시 `weave.init()`만 실행한 후, 평소처럼 라우팅된
프로바이더를 계속 사용하면 됩니다:
...
[Evaluations][evals]에서 직접 [맞춤형 라우터]를 트레이닝하면, Not Diamond가 특정 사용 사례에 맞춰 평가 성능에 따라 프롬프트를 라우팅할 수 있습니다.
...
이 preference ID를 모든 `model_select` Request에서 재사용하면, 프롬프트를
평가 데이터에서 성능은 최대화하고 비용은 최소화하도록 라우팅할 수 있습니다:
...
추가 지원을 위해 [docs]를 방문하거나 [문의 메일을 보내세요][support].
