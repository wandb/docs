...
이 가이드는 서드파티 라이브러리(예를 들어, OpenAI)를 Weave TypeScript SDK에 통합하는 방법을 설명합니다. Weave는 자동 계측을 지원하므로 설정이 더 간편해지고 수동 설정의 필요도 줄어듭니다.
...
ESM 프로젝트를 사용 중인데 CLI 플래그를 전달할 수 없는 경우(예를 들어 CLI 도구나 프레임워크의 제약 때문)에는 `NODE_OPTIONS` 환경 변수를 설정하세요:
