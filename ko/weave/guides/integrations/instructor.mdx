---
title: Instructor
description: "Weave의 Instructor 인테그레이션으로 LLM의 구조화된 데이터 추출을 트레이스하고, Pydantic 검증과 재시도 로직을 포착합니다."
keywords: ["Instructor", "Pydantic", "구조화된 추출", "response_model", "FastAPI 서빙"]
---
...
| ![구조화된 출력 추출 워크플로가 포함된 Weave의 Instructor LM 트레이스](/weave/guides/integrations/imgs/instructor/instructor_lm_trace.gif) |
| --------------------------------------------------------------------------------------------------------------------- |
| Weave는 Instructor를 사용해 이루어지는 모든 LLM Call을 추적하고 로깅합니다. Weave 웹 인터페이스에서 해당 트레이스를 확인할 수 있습니다.                           |

## 직접 만든 ops 추적하기 {#track-your-own-ops}
...
## `Model`로 더 쉽게 실험하기 {#create-a-model-for-easier-experimentation}
...
## Weave Model 서빙하기 {#serve-a-weave-model}
...
| [![FastAPI 서버 설정 및 모델 서빙 옵션이 포함된 Instructor 서빙 인터페이스](/weave/guides/integrations/imgs/instructor/instructor_serve.png)](https://wandb.ai/geekyrakshit/instructor-test/weave/objects/PersonExtractor/versions/xXpMsJvaiTOjKafz1TnHC8wMgH5ZAAwYOaBMvHuLArI) |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
