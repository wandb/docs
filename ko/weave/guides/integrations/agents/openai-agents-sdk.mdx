...
스크립트가 실행된 후에는 Weave에서 캡처된 활동을 검토할 수 있습니다. `weave.init()`가 실행되면 프로젝트 링크가 출력됩니다. 다음 항목을 확인하려면 **Agents** 뷰를 여세요:

* `Research assistant`에 대한 **Agents** 탭의 행 하나
* 세 개의 턴으로 이루어진 conversation
* 중첩된 모델 Call과 도구 Call이 포함된 `invoke_agent` span으로 렌더링된 각 턴
* 각 step의 전체 입력, 모델, 출력, 토큰 사용량, 그리고 Wikipedia 결과
