...
Weave SDK는 OpenAI Agents SDK와 자동으로 패치되어, 최소한의 설정만으로 에이전트의 트레이스를 캡처할 수 있습니다. 이 문서에서는 Weave를 초기화한 다음 OpenAI Agents SDK로 구축한 멀티턴 리서치 에이전트를 실행하여, Weave가 Session 전반에서 모든 에이전트 호출, 모델 Call, 도구 Call을 캡처하도록 설정하는 방법을 설명합니다.
...
`weave.init()`가 실행되면 프로젝트 링크가 출력됩니다. **Agents** 뷰를 열어 다음 항목을 확인하세요.
