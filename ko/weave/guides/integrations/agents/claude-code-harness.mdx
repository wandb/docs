...
<Steps>
  ...
  <Step title="Claude Code 시작:">
    ...
    이 시점부터 세션이 자동으로 트레이스됩니다. 프롬프트를 한두 개 입력한 다음 Weave 프로젝트를 열어 트레이스가 표시되는지 확인하세요.
  </Step>
</Steps>
...
멀티턴 대화는 서버 측에서 Session ID로 연결되므로, 여러 트레이스에 걸친 전체 대화를 따라갈 수 있습니다. 각 span에는 token 사용량, 모델 이름, 도구 입력 및 출력, 타이밍, 그리고 프롬프트와 응답의 텍스트 내용이 포함됩니다. 자세한 내용은 GitHub의 [What Gets Traced](https://github.com/wandb/weave-claude-code/blob/main/README.md#what-gets-traced)를 참조하세요.
...
| ------- | ---------------------- | --------------------------------------------------------------------------------------------------------- |
| Install | `/weave:weave-install` | 전체 설치 및 설정 과정을 대화형으로 안내합니다. 새 머신에서 사용하거나 문제가 있는 설정을 진단할 때 사용하세요.                                          |
...
플러그인은 시작 시 `WANDB_BASE_URL`을 읽는 백그라운드 데몬을 실행합니다. 변수를 설정할 때 데몬이 이미 실행 중이면 해당 변경 사항을 반영하지 않습니다. 데몬을 다시 시작하려면 다음과 같이 하세요.
...
3. Claude Code를 다시 실행합니다. 데몬이 자동으로 다시 시작되며 올바른 URL을 반영합니다.
