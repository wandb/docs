...
<div id="see-your-agent-traces-in-the-agents-view">
  ### Agents 뷰에서 에이전트 트레이스 보기
</div>

스크립트가 실행된 후 프로젝트의 **Agents** 뷰를 열어 Weave가 대화를 캡처했는지 확인하세요. `weave.init()`는 실행 시 프로젝트 링크를 출력합니다. **Agents** 뷰에서 다음을 확인하세요.

* **Agents** 탭에 `research_assistant`에 대한 행 1개
* 세 개의 턴이 포함된 conversation 1개
* 각 턴이 중첩된 모델 Call 및 도구 Call을 포함한 `invoke_agent` span으로 표시됨
* 각 단계의 전체 입력, 모델, 출력, 토큰 사용량, Wikipedia 결과
