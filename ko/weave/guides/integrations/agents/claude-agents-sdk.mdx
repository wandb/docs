...
이 문서에서는 Weave를 초기화한 다음 `ClaudeSDKClient`를 통해 MCP 도구와 함께 Claude 에이전트를 실행하여, Weave가 대화, 모델 Call, 도구 Call을 전체 흐름에 걸쳐 자동으로 트레이스하는 방법을 설명합니다.
...
프로젝트에 `weave.init`를 추가하고, W&amp;B 팀 이름과 프로젝트 이름을 업데이트한 뒤, 평소처럼 에이전트를 구축하세요.
...
`weave.init()`가 실행되면 프로젝트 링크를 출력합니다.
