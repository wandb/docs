---
title: Agno
description: "OpenTelemetry를 사용해 Weave에서 Agno 멀티 에이전트 시스템을 트레이싱하여 에이전트 상호작용, 도구 호출, 공유 메모리 작업, 멀티모달 워크플로를 캡처하고 AI 에이전트 시스템에 대한 종합적인 가시성을 확보할 수 있습니다."
---

[Agno](https://docs.agno.com/) 에이전트와 도구 호출을 [OpenTelemetry (OTEL)](https://opentelemetry.io/)을 사용해 Weave에서 트레이싱할 수 있습니다. Agno는 공유 메모리, 지식, 추론 기능을 갖춘 멀티 에이전트 시스템을 구축하기 위한 Python 프레임워크입니다. 이 프레임워크는 경량이고 모델 비종속적(model-agnostic)이며 고성능으로 설계되었고, 텍스트, 이미지, 오디오, 비디오 처리를 포함한 멀티모달 기능을 지원합니다.

이 가이드에서는 OTEL을 사용해 Agno 에이전트와 도구 호출을 트레이싱하고, 해당 트레이스를 Weave에서 시각화하는 방법을 설명합니다. 필요한 의존성을 설치하고, Weave로 데이터를 전송할 OTEL 트레이서를 구성하고, Agno 에이전트와 도구에 계측을 추가하는 방법을 배우게 됩니다.
...
2. OpenAI API 키(또는 다른 모델 제공자의 키)를 환경 변수로 설정합니다:
...
Agno에서 Weave로 트레이스를 전송하려면 `TracerProvider`와 `OTLPSpanExporter`를 사용해 OTEL을 구성하십시오. exporter를 [인증 및 프로젝트 식별을 위한 적절한 엔드포인트와 HTTP 헤더](#required-configuration)에 설정하십시오.

<note>
  API 키와 프로젝트 정보 같은 민감한 환경 변수는 `.env`와 같은 환경 파일에 저장하고, `os.environ`을 사용해 로드하는 것을 권장합니다. 이렇게 하면 자격 증명이 안전하게 보호되고 코드베이스에 노출되지 않습니다.
</note>
...
* **헤더(Headers):**
  * `Authorization`: W&amp;B API 키를 사용하는 Basic 인증 방식
...
[사전 준비](#prerequisites)를 완료했다면 Agno에서 Weave로 OTEL 트레이스를 전송할 수 있습니다. 다음 코드 예제는 OTLP span exporter와 tracer provider를 구성하여 Agno 애플리케이션에서 Weave로 OTEL 트레이스를 전송하는 방법을 보여 줍니다.

<Warning>
  Weave가 Agno를 올바르게 추적할 수 있도록, 코드에서 Agno 컴포넌트를 사용하기 *전에* 전역 tracer provider를 설정하세요.
</Warning>
...
# W&B 엔터티/프로젝트 이름 예: "myteam/myproject"
...
<div id="trace-agno-agents-with-otel">
  ## OTEL로 Agno 에이전트 추적하기
</div>

트레이서 프로바이더(tracer provider)를 설정한 후, 자동 추적이 활성화된 Agno 에이전트를 생성하고 실행할 수 있습니다. 다음 예시는 도구를 사용하는 간단한 에이전트를 생성하는 방법을 보여줍니다.
...
모든 에이전트 동작은 자동으로 트레이싱되어 Weave로 전송되며, 이를 통해 실행 흐름, 모델 호출, 추론 단계, 도구 호출을 시각화할 수 있습니다.
...
Agno에서 도구를 정의하고 사용할 때, 이러한 도구 호출도 트레이스에 함께 캡처됩니다. OTEL 통합은 에이전트의 추론 과정과 개별 도구 실행을 모두 자동으로 계측하여 에이전트의 동작을 종합적으로 파악할 수 있게 해 줍니다.
...
<div id="trace-multi-agent-teams-with-otel">
  ## OTEL로 멀티 에이전트 팀 트레이싱하기
</div>

Agno의 강력한 멀티 에이전트 아키텍처를 사용하면 협업하고 컨텍스트를 공유할 수 있는 에이전트 팀을 구성할 수 있습니다. 이러한 팀 상호작용도 모두 트레이싱됩니다:
...
이 멀티 에이전트 트레이스는 Weave 내 여러 에이전트 간의 조율을 보여 주어, 작업이 에이전트 팀 전체에 어떻게 분배되고 실행되는지 한눈에 파악할 수 있게 해줍니다.
...
Agno는 에이전트가 문제를 단계별로 생각해 나갈 수 있도록 돕는 내장 추론 기능을 제공합니다. 이러한 추론 과정은 트레이스에도 함께 기록됩니다:
...
추론 과정이 trace에 표시되어, 에이전트가 복잡한 문제를 어떻게 분해하고 어떤 결정을 내리는지 보여 줍니다.
...
Agno 에이전트는 메모리를 유지하고 지식 베이스에 액세스할 수 있습니다. 이러한 작업도 모두 추적됩니다:
...
대화 기록의 저장 및 조회를 포함한 메모리 관련 작업이 트레이스에 표시됩니다.
...
* [OpenInference Agno 계측 도구](https://pypi.org/project/openinference-instrumentation-agno/)