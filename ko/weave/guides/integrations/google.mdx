...
이 페이지에서는 Google GenAI 애플리케이션을 평가하고, 모니터링하고, 반복적으로 개선할 수 있도록 W&amp;B Weave를 Google Vertex AI API 및 Google Gemini API와 함께 사용하는 방법을 설명합니다.

Weave는 다음의 트레이스를 자동으로 캡처합니다:

* Python SDK, Node.js SDK, Go SDK 및 REST를 통해 액세스할 수 있는 [Google GenAI SDK](https://github.com/googleapis/python-genai).
* Google의 Gemini 모델과 [파트너 모델](https://cloud.google.com/vertex-ai/generative-ai/docs/partner-models/use-partner-models)에 대한 액세스를 제공하는 [Google Vertex AI API](https://cloud.google.com/vertex-ai/docs).

<Note>
  Weave는 사용 중단된 [Google AI Python SDK for the Gemini API](https://github.com/google-gemini/deprecated-generative-ai-python)도 지원합니다. 이 지원 역시 사용 중단되었으며 향후 버전에서 제거될 예정입니다.
</Note>

## 시작하기 {#get-started}

다음 예시에서는 지원되는 각 SDK에서 트레이싱을 활성화하는 방법을 보여줍니다. 두 경우 모두 `weave.init`를 호출하는 것만이 Weave에만 필요한 단계입니다. 기존 Google GenAI 또는 Vertex AI 코드는 그대로 유지됩니다.
...
## 직접 ops 추적하기 {#track-your-own-ops}
...
[`@weave.op`](/ko/weave/guides/tracking/ops)으로 데코레이팅된 함수를 만드세요.

다음 예제에서 함수 `recommend_places_to_visit`는 `@weave.op`으로 감싸져 있으며 도시에서 방문할 장소를 추천합니다.
...
다음 예시에서는 `CityVisitRecommender`를 실험해 볼 수 있습니다. 이 중 하나를 변경할 때마다 `CityVisitRecommender`의 새 *버전*이 생성됩니다. 이렇게 하면 시도한 각 설정에 대한 버전 관리 기록이 남아, 나중에 이를 비교하거나 평가할 수 있습니다.
