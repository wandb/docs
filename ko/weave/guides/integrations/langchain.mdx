...
자동 트레이싱 외에도 `WeaveTracer` 콜백이나 `weave_tracing_enabled` 컨텍스트 매니저를 사용해 호출을 수동으로 트레이싱할 수 있습니다. 이러한 방법은 LangChain 애플리케이션의 개별 부분에서 요청 콜백을 사용하는 것과 비슷합니다.

**Note:** Weave는 기본적으로 LangChain Runnables를 트레이싱하며, 이는 `weave.init()`을 호출할 때 활성화됩니다. `weave.init()`을 호출하기 전에 환경 변수 `WEAVE_TRACE_LANGCHAIN`을 `"false"`로 설정하여 이 동작을 비활성화할 수 있습니다. 이렇게 하면 애플리케이션에서 특정 체인이나 개별 요청의 트레이싱 동작을 제어할 수 있습니다.
...
또는 `weave_tracing_enabled` 컨텍스트 매니저를 사용하여 특정 코드 블록에 대해 트레이싱을 활성화할 수 있습니다.
...
with weave_tracing_enabled():  # <-- 이 체인 invoke에 대해서만 트레이싱을 활성화합니다.
    output = llm_chain.invoke({"number": 2})
