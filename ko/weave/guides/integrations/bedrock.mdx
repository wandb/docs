---
title: Bedrock
description: "Weave로 Amazon Bedrock LLM 호출을 추적 및 모니터링하고, 파운데이션 모델 상호작용과 Converse API 사용을 캡처합니다."
---

Weave는 선도적인 AI 기업들의 파운데이션 모델을 통합 API를 통해 제공하는 AWS의 완전 관리형 서비스인 Amazon Bedrock를 통한 LLM 호출을 자동으로 추적하고 로그로 남깁니다.

Amazon Bedrock에서 Weave로 LLM 호출을 로그로 남기는 방법은 여러 가지가 있습니다. `weave.op`을 사용해 Bedrock 모델에 대한 모든 호출을 추적하기 위한 재사용 가능한 작업을 만들 수 있습니다. 또한 Anthropic 모델을 사용하는 경우, Weave에 내장된 Anthropic 통합을 사용할 수 있습니다.

<Tip>
  최신 튜토리얼은 [Weights &amp; Biases on Amazon Web Services](https://wandb.ai/site/partners/aws/)를 방문하세요.
</Tip>
