...
Weave의 [OpenAI Realtime API](https://developers.openai.com/api/docs/guides/realtime/)와의 인테그레이션을 통해 애플리케이션의 음성-음성 상호작용을 발생하는 즉시 자동으로 트레이스할 수 있습니다. 이를 통해 에이전트와 사용자 간 대화를 캡처하여 에이전트 성능을 검토하고 평가할 수 있습니다.

<div id="integrate-realtime-traces">
  ## 실시간 트레이스 통합하기
</div>

Weave는 OpenAI Realtime API에 자동으로 패치되므로, 애플리케이션의 오디오 상호작용을 캡처하기 시작하려면 코드 몇 줄만 추가하면 됩니다. 다음 코드는 Weave와 Realtime API 인테그레이션을 임포트합니다:
...
코드에 임포트하여 실행하면 Weave가 사용자와 OpenAI Realtime API 간의 상호작용을 자동으로 트레이스합니다.
...
이 예제는 마이크 Audio를 OpenAI의 Realtime API로 스트리밍하고, AI의 음성 Response를 로컬 머신의 스피커를 통해 재생하는 실시간 음성 어시스턴트를 실행합니다. 이 애플리케이션은 `RealtimeAgent` 및 `RealtimeRunner`와 함께 OpenAI Agents SDK를 사용하며, `patch_openai_realtime()` 패치를 통해 Weave 트레이싱을 활성화합니다.

예제를 실행하려면:

1. Python 환경을 시작하고 다음 라이브러리를 설치하세요:
...
2. `weave_voice_assistant.py`라는 이름의 ファイル을 만들고 여기에 다음 코드를 추가하세요.

   강조 표시된 줄은 애플리케이션에서 Weave의 인테그레이션을 나타냅니다. 나머지 코드는 기본 음성 비서 앱을 생성합니다.
   ...
   <Accordion title="weave_voice_assistant.py">
     ...
     # 오디오 큐를 비우고 백그라운드 스레드에서 스피커에 씁니다.
     ...
     async def main(*, input_device_index: int | None = None, output_device_index: int | None = None):
         ...
         # 정리
         audio_output_queue.put(None)  # 재생 스레드에 종료 시그널을 보냅니다.
         mic.close()
         speaker.close()
         p.terminate()
...
다음 예시는 WebSockets를 통해 OpenAI Realtime API에 직접 연결합니다. 마이크 오디오를 API로 스트리밍하고, 음성 Response를 재생하며, 도구 호출(날씨 조회, 수학 평가, 코드 실행, 파일 쓰기)을 지원합니다. Weave는 `weave.init()`과 `patch_openai_realtime()`을 사용해 세션을 트레이스합니다.
...
2. `tool_definitions.py`라는 제목의 파일을 만들고 여기에 다음 도구 정의를 추가하세요. 기본 애플리케이션은 이 모듈에서 임포트합니다.
   ...
   <Accordion title="tool_definitions.py">
     ...
     def run_python_code(code: str) -> str:
         ...
         try:
             ...
             return "오류: 스크립트가 30초 후 시간 초과되었습니다."
         finally:
...
3. 같은 디렉터리에 `weave_ws_voice_assistant.py`라는 이름의 파일을 만들고 다음 코드를 추가하세요.
   ...
   <Accordion title="weave_ws_voice_assistant.py">
     ...
     def play_audio(output_stream: pyaudio.Stream, audio_output_queue: queue.Queue):
         ...
         진행 중인 쓰기가 완료될 때까지 기다리지 않고 인터럽트 시 큐를 비울 수 있습니다."""
         while True:
             data = audio_output_queue.get()
             if data is None:
                 break
     ...
     async def receive_events(ws, audio_output_queue: queue.Queue) -> None:
         ...
         async for raw_message in ws:
             ...
             if event_type == "session.created":
                 ...
                 # 사용자의 말을 덮어쓰지 않도록 대기 중인 AI 오디오를 비웁니다.
                 while not audio_output_queue.empty():
                     try:
                         audio_output_queue.get_nowait()
                     except queue.Empty:
...
4. `weave.init()` 호출을 팀 이름과 프로젝트 이름으로 업데이트하세요.
