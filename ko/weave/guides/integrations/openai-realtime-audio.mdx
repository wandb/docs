...
keywords: ["음성-대-음성", "음성 어시스턴트", "WebSockets", "Agents SDK", "오디오 트레이싱", "TypeScript"]
---
...
이 가이드에서는 애플리케이션에서 Realtime API 트레이싱을 활성화하는 방법을 보여주고, 로컬에서 실행할 수 있는 Python 및 TypeScript 엔드투엔드 예시를 안내합니다.

## 실시간 트레이스 통합하기 {#integrate-realtime-traces}
...
<Tabs>
  ...
  <Tab title="TypeScript">
    TypeScript에서 Weave는 [OpenAI Agents SDK](https://openai.github.io/openai-agents-js/)를 통해 Realtime API를 트레이스합니다. Weave를 임포트하면 `@openai/agents-realtime`의 모든 `RealtimeSession`이 자동으로 계측되므로 패치 함수를 호출할 필요가 없습니다. Weave 프로젝트 초기화만 수행하면 됩니다:
    ...
    동적 임포트 또는 특정 번들러처럼 설정에서 Weave가 자동 계측에 사용하는 Node.js 모듈 로더 훅을 우회하는 경우, 애플리케이션에서 세션을 생성하기 전에 시작 시 [`patchRealtimeSession()`](/ko/weave/reference/typescript-sdk/functions/patchrealtimesession)을 한 번 호출하세요:
...
### OpenAI Agents SDK로 실시간 에이전트 실행하기 {#run-a-realtime-agent-with-the-openai-agents-sdk}

<Tabs>
  <Tab title="Python">
    이 예제는 마이크 오디오를 OpenAI의 Realtime API로 스트리밍하고 AI의 음성 응답을 로컬 머신 스피커로 재생하는 실시간 음성 어시스턴트를 실행합니다. 이 애플리케이션은 OpenAI Agents SDK의 `RealtimeAgent`와 `RealtimeRunner`를 사용하며, `patch_openai_realtime()`으로 패치하여 Weave 트레이싱을 활성화합니다. 상위 수준의 Agents SDK가 Realtime 세션을 대신 관리하도록 하려면 이 방식을 사용하세요.

    예시를 실행하려면 다음 step을 완료하세요:
    ...
    어시스턴트가 실행되면 키보드에서 `T` 키를 눌러 마이크를 음소거하거나 음소거를 해제하세요. 어시스턴트는 서버 측 음성 활동 감지를 사용하여 차례 전환과 끼어들기를 처리합니다.

    assistant와 대화하는 동안 Weave는 세션의 오디오를 포함한 트레이스를 캡처하며, 이를 Weave UI에서 살펴볼 수 있습니다.
  </Tab>

  <Tab title="TypeScript">
    이 예제에서는 `RealtimeAgent` 및 `RealtimeSession`으로 실시간 에이전트를 만들고, 날씨 도구를 제공한 후 텍스트 메시지를 전송합니다. 임포트하면 Weave가 세션을 자동으로 계측하고, 세션, 생성, 도구 Call 및 오디오 출력을 트레이스로 캡처합니다.

    예제를 실행하려면 다음 단계를 완료하세요:

    1. 새 디렉터리에서 TypeScript를 사용해 Node.js 프로젝트를 설정하고 다음 라이브러리를 설치하세요:
    ...
    2. `weave_realtime_agent.ts` 파일을 만들고 다음 코드를 추가하세요.

       강조 표시된 줄은 애플리케이션에 Weave가 인테그레이션된 부분을 나타냅니다. 나머지 코드는 기본 실시간 에이전트를 구현합니다.
       ...
       <Accordion title="weave_realtime_agent.ts">
         ...
         // 에이전트가 호출할 수 있는 시뮬레이션된 날씨 조회 도구입니다.
         ...
         async function main() {
           ...
           // Weave는 임포트 시 `@openai/agents-realtime`을 자동으로 계측합니다.
           // 설정에서 모듈 로더 훅을 우회하는 경우 다음 줄의 주석 처리를 해제하세요.
           ...
           // 대화 이력이 업데이트될 때 assistant의 텍스트 응답을 출력합니다.
    ...
    3. `WEAVE_PROJECT` 값을 팀 이름과 프로젝트 이름으로 업데이트하세요.

    4. `OPENAI_API_KEY` 및 `WANDB_API_KEY` 환경 변수를 설정하세요.
    ...
    첫 번째 `--import` 플래그는 Weave 계측을 미리 로드하고, 두 번째 `--import` 플래그는 Node가 TypeScript를 실행할 수 있도록 `tsx`를 미리 로드합니다.

    에이전트는 Realtime API에 연결해 San Francisco의 날씨를 묻고 날씨 도구를 호출한 후 연결을 종료합니다. Weave UI에서 세션, 생성, 도구 Call 및 assistant의 오디오 출력을 포함한 결과 트레이스를 탐색할 수 있습니다.
  </Tab>
</Tabs>

### WebSockets를 사용해 실시간 음성 어시스턴트 실행 {#run-a-realtime-voice-assistant-with-websockets}

다음 Python 예제는 WebSockets를 통해 OpenAI Realtime API에 직접 연결합니다. 마이크 오디오를 API로 스트리밍하고, 음성 응답을 재생하며, 도구 호출 기능(날씨 조회, 수식 계산, 코드 실행, 파일 쓰기)을 지원합니다. Weave는 `weave.init()`와 `patch_openai_realtime()`을 사용해 세션을 트레이스합니다. Agents SDK에 의존하지 않고 Realtime Session을 완전히 제어하려는 경우 이 방식을 사용하세요.
