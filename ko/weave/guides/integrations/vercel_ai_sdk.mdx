...
계측을 추가한 후에는 AI SDK의 모든 함수 호출에서 Vercel의 `experimental_telemetry` 옵션을 사용해 OTel spans를 내보내세요:
...
telemetry가 활성화된 모든 `generateText` 호출은 Weave로 내보내지는 OTel span을 생성합니다. 이제 Next.js 앱이 Vercel AI SDK 트레이스를 Weave로 전송하도록 설정되었습니다.
