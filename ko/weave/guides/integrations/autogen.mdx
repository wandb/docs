...
{/* TODO: 이 가이드용 Colab 노트북을 추가하고 여기에 배지 링크를 넣으세요. */}

AutoGen은 Microsoft에서 제공하는 AI 에이전트 및 애플리케이션 구축용 프레임워크입니다. 대화형 AI용 컴포넌트(`AgentChat`), 핵심 멀티 에이전트 기능(`Core`), 외부 서비스 인테그레이션(`Extensions`)을 제공하여 복잡한 멀티 에이전트 시스템을 더 쉽게 만들 수 있도록 해줍니다. 또한 코드 없이 에이전트를 프로토타이핑할 수 있는 `Studio`도 제공합니다. 자세한 내용은 [공식 AutoGen 문서](https://microsoft.github.io/autogen/stable//index.html)를 참조하세요.

<Note>
  이 가이드는 [AutoGen](https://microsoft.github.io/autogen/stable/index.html)에 대한 기본 이해가 있다고 가정합니다.
</Note>

Weave는 [AutoGen](https://microsoft.github.io/autogen/stable/index.html)과 통합되어 멀티 에이전트 애플리케이션의 실행을 트레이스하고 시각화할 수 있도록 지원합니다. Weave를 초기화하기만 하면 `autogen_agentchat`, `autogen_core`, `autogen_ext` 내 상호작용을 자동으로 추적할 수 있습니다. 이 가이드에서는 AutoGen과 함께 Weave를 사용하는 다양한 예제를 안내합니다.

<div id="prerequisites">
  ## 사전 요구 사항
</div>

시작하기 전에 AutoGen과 Weave가 설치되어 있는지 확인하세요. 또한 사용할 LLM 제공업체(OpenAI, Anthropic 등)의 SDK도 필요합니다.
...
트레이스 수집을 시작하려면 스크립트 맨 앞에서 Weave를 초기화하세요.
...
Weave는 AutoGen 내에서 모델 클라이언트에 직접 수행되는 Call을 트레이스할 수 있습니다.

<div id="tracing-a-client-create-call">
  ### 클라이언트 create Call 트레이싱
</div>

이 예제는 `OpenAIChatCompletionClient`의 단일 Call을 트레이싱하는 방법을 보여줍니다.
...
<div id="tracing-a-client-create-call-with-streaming">
  ### 스트리밍이 포함된 클라이언트 create Call 트레이싱
</div>

Weave는 스트리밍되는 Response의 트레이싱도 지원합니다.
...
AutoGen의 `ChatCompletionCache`를 사용할 수 있으며, Weave는 이러한 상호작용을 트레이스하여 Response가 캐시에서 왔는지 아니면 새로운 Call에서 왔는지 보여줍니다.
...
<div id="tracing-an-agent-with-tool-calls">
  ## 도구 Call을 포함한 에이전트 트레이싱
</div>

Weave는 에이전트와 해당 도구 사용을 트레이스하여, 에이전트가 도구를 어떻게 선택하고 실행하는지 가시성을 제공합니다.
...
<div id="tracing-a-groupchat-round-robin">
  ## GroupChat 트레이싱 - round robin
</div>

`RoundRobinGroupChat`와 같은 그룹 채팅 내 상호작용은 Weave에서 트레이싱되어, 에이전트 간의 대화 흐름을 따라갈 수 있습니다.
...
# 전체 그룹 채팅을 트레이싱하기 위해 여기에 weave op를 추가합니다
...
AutoGen의 메모리 컴포넌트는 Weave로 트레이싱할 수 있습니다. `@weave.op()`을 사용하면 메모리 오퍼레이션을 하나의 트레이스로 묶어 가독성을 높일 수 있습니다.

```python lines

from autogen_core.memory import ListMemory, MemoryContent, MemoryMimeType

# 단일 트레이스 아래에서 memory add Call과 memory get Call을 함께 추적하기 위해
# 여기에 weave op를 추가합니다
# 완전히 선택 사항이지만 사용하는 것을 강력히 권장합니다
...
문서 인덱싱과 `ChromaDBVectorMemory` 같은 메모리 시스템을 활용한 검색을 포함한 검색 증강 생성(RAG) 워크플로는 트레이싱할 수 있습니다. RAG 프로세스를 `@weave.op()`로 데코레이팅하면 전체 흐름을 시각화하는 데 도움이 됩니다.

<Note>
  이 RAG 예시를 실행하려면 `chromadb`가 필요합니다. `pip install chromadb`로 설치하세요.
</Note>
...
Weave는 `SingleThreadedAgentRuntime`와 같은 AutoGen의 에이전트 런타임 내부 오퍼레이션을 트레이싱할 수 있습니다. 런타임 실행 함수를 `@weave.op()`으로 감싸면 관련 트레이스를 그룹화할 수 있습니다.
...
# 전체 에이전트 런타임 Call을 단일 트레이스로 추적하기 위해
# 여기에 weave op를 추가합니다
...
에이전트 간 상호작용의 순서를 정의하는 복잡한 에이전트 워크플로를 트레이스할 수 있습니다. 전체 워크플로에 대한 고수준 트레이스를 제공하려면 `@weave.op()`를 사용하세요.
...
class ConceptExtractorAgent(RoutedAgent):
    def __init__(self, model_client: ChatCompletionClient) -> None:
        ...
        self._system_message = SystemMessage(
            content=(
                "당신은 마케팅 분석가입니다. 제품 설명이 주어지면 다음을 파악하세요:\n"
                "- 주요 기능\n"
                "- 타겟 고객\n"
                "- 차별화 포인트\n\n"
            )
...
class WriterAgent(RoutedAgent):
    def __init__(self, model_client: ChatCompletionClient) -> None:
        ...
        self._system_message = SystemMessage(
            content=(
                "당신은 마케팅 카피라이터입니다. 기능, 타겟 고객, USP를 설명하는 텍스트 블록이 주어지면 "
                "이러한 포인트를 강조하는 설득력 있는 마케팅 카피(예: 뉴스레터 섹션)를 작성하세요. "
                "출력은 짧게(약 150단어) 작성하고, 카피만 하나의 텍스트 블록으로 출력하세요."
            )
        )
        self._model_client = model_client
...
class FormatProofAgent(RoutedAgent):
    def __init__(self, model_client: ChatCompletionClient) -> None:
        super().__init__("형식 및 교정 에이전트입니다.")
        self._system_message = SystemMessage(
            content=(
                "당신은 편집자입니다. 초안 카피가 주어지면 문법을 교정하고, 명확성을 높이고, 톤의 일관성을 유지하며, "
                "형식을 정돈해 매끄럽게 다듬으세요. 최종적으로 개선된 카피를 하나의 텍스트 블록으로 출력하세요."
            )
        )
        self._model_client = model_client
...
class UserAgent(RoutedAgent):
    ...
    async def handle_final_copy(self, message: WorkflowMessage, ctx: MessageContext) -> None:
        print(f"\\n{'-'*80}\\n{self.id.type}가 최종 카피를 받았습니다:\\n{message.content}")

# 전체 에이전트 워크플로를 단일 트레이스로 추적하려고
# 여기에 weave op를 추가합니다.
...
async def run_agent_workflow(model_name="gpt-4o"):
    ...
    await runtime.publish_message(
        WorkflowMessage(
            content="음료를 24시간 차갑게 유지해 주는 친환경 스테인리스 스틸 물병"
        ),
...
<Warning>
  **Docker 필요**
  이 예제는 Docker를 사용해 코드를 실행하므로 모든 환경(예: Colab에서 직접 실행)에서 동작하지 않을 수 있습니다. 이 예제를 실행해 보려면 로컬에서 Docker가 실행 중인지 확인하세요.
</Warning>

Weave는 AutoGen 에이전트가 코드를 생성하고 실행하는 과정을 트레이스합니다.
...
# 전체 코드 생성 워크플로를 단일 트레이스로 추적하기 위해
# 여기에 weave op를 추가합니다
# 완전히 선택 사항이지만 사용하는 것을 강력히 권장합니다
...
이 가이드는 Weave를 AutoGen과 통합하기 위한 출발점을 제공합니다. 에이전트 상호작용, 모델 Call, 도구 사용에 대한 자세한 트레이스를 보려면 Weave UI를 탐색하세요.