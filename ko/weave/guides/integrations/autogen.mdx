...
Weave는 AutoGen 내에서 모델 클라이언트에 직접 수행되는 호출을 트레이싱할 수 있습니다.
...
에이전트 간 상호작용의 순서를 정의하는 복잡한 에이전트 워크플로도 트레이싱할 수 있습니다. 전체 워크플로에 대한 상위 수준 트레이스를 남기려면 `@weave.op()`을(를) 사용할 수 있습니다.
