---
title: "AutoGen"
description: "Weave를 사용하여 AutoGen 에이전트와 멀티 에이전트 시스템을 추적 및 모니터링하세요"
---

{/* TODO: 이 가이드용 Colab 노트북을 추가하고 여기에 배지 링크를 달 것. */}

AutoGen은 Microsoft에서 제공하는 AI 에이전트와 애플리케이션을 구축하기 위한 프레임워크입니다. 이 프레임워크는 복잡한 멀티 에이전트 시스템의 구성을 단순화하고, 대화형 AI(`AgentChat`), 핵심 멀티 에이전트 기능(`Core`), 외부 서비스와의 인테그레이션(`Extensions`)을 위한 컴포넌트를 제공합니다. AutoGen은 또한 코드 작성 없이 에이전트를 프로토타이핑할 수 있는 `Studio`를 제공합니다. 자세한 내용은 [공식 AutoGen 문서](https://microsoft.github.io/autogen/stable//index.html)를 참조하세요.

<Note>
  이 가이드는 독자가 [AutoGen](https://microsoft.github.io/autogen/stable/index.html)에 대해 기본적인 이해를 갖추고 있다고 가정합니다.
</Note>

Weave는 [AutoGen](https://microsoft.github.io/autogen/stable/index.html)과 인테그레이션되어 멀티 에이전트 애플리케이션의 실행 과정을 추적하고 시각화할 수 있도록 도와줍니다. Weave를 간단히 초기화하기만 하면 `autogen_agentchat`, `autogen_core`, `autogen_ext` 내의 상호작용을 자동으로 추적할 수 있습니다. 이 가이드는 AutoGen과 함께 Weave를 사용하는 다양한 예제를 통해 이를 단계별로 안내합니다.

<div id="prerequisites">
  ## 사전 준비 사항
</div>

시작하기 전에 AutoGen과 Weave가 설치되어 있는지 확인하세요. 또한 사용하려는 LLM 제공 업체(OpenAI, Anthropic 등)의 SDK도 준비해야 합니다.
...
Weave는 AutoGen 내에서 모델 클라이언트에 직접 수행되는 호출을 트레이스할 수 있습니다.

<div id="tracing-a-client-create-call">
  ### 클라이언트 생성 호출 트레이싱
</div>

이 예제는 `OpenAIChatCompletionClient`에 대한 한 번의 호출을 트레이싱하는 방법을 보여줍니다.
...
Weave는 스트리밍된 응답에 대한 트레이싱도 지원합니다.
...
AutoGen의 `ChatCompletionCache`를 사용할 수 있으며, Weave는 이러한 상호작용을 추적하여 응답이 캐시에서 반환된 것인지 새 호출에서 생성된 것인지 보여줍니다.
...
Weave는 에이전트와 그 도구 사용을 추적하여, 에이전트가 어떤 도구를 선택해 어떻게 실행하는지에 대한 가시성을 제공합니다.
...
<div id="tracing-a-groupchat-round-robin">
  ## GroupChat 추적 - round robin
</div>

`RoundRobinGroupChat`와 같은 그룹 채팅 내 상호작용은 Weave에서 트레이싱되므로, 에이전트 간 대화 흐름을 추적할 수 있습니다.
...
AutoGen의 메모리 컴포넌트는 Weave로 트레이싱할 수 있습니다. `@weave.op()`을 사용해 메모리 연산을 하나의 트레이스로 묶어서 가독성을 높일 수 있습니다.
...
Retrieval-Augmented Generation(RAG) 워크플로(문서 인덱싱 및 `ChromaDBVectorMemory` 같은 메모리 시스템을 활용한 검색 포함)는 추적 가능합니다. RAG 프로세스에 `@weave.op()` 데코레이터를 적용하면 전체 흐름을 시각화하는 데 도움이 됩니다.

<Note>
  이 RAG 예시는 `chromadb`가 필요합니다. `pip install chromadb`로 설치하세요.
</Note>
...
Weave는 `SingleThreadedAgentRuntime`와 같은 AutoGen 에이전트 런타임 내에서 수행되는 작업을 트레이싱할 수 있습니다. 런타임 실행 함수를 `@weave.op()`으로 감싸면 관련 트레이스를 하나로 묶을 수 있습니다.
...
에이전트 간 상호작용의 순서를 정의하는 복잡한 에이전트 워크플로도 트레이스할 수 있습니다. 전체 워크플로에 대한 상위 수준 트레이스를 제공하려면 `@weave.op()`을(를) 사용하세요.
...
* **Weave**:
  * [트레이싱 가이드](/ko/weave/guides/tracking/tracing)
  * [Op 데코레이터](/ko/weave/guides/tracking/ops)
* **AutoGen**:
  * [공식 문서](https://microsoft.github.io/autogen/stable//index.html)
  * [AutoGen GitHub](https://github.com/microsoft/autogen)

이 가이드는 Weave를 AutoGen과 통합하기 위한 출발점입니다. Weave UI에서 에이전트 상호작용, 모델 호출, 도구 사용 내역에 대한 상세 트레이스를 확인해 보세요.