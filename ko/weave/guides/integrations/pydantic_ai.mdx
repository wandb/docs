...
```
그런 다음, [Weave에서 OTEL 트레이싱 설정](#weave에서-otel-트레이싱-설정)을 진행하세요.

### Weave에서 OTEL 트레이싱 설정

PydanticAI에서 Weave 로 트레이스를 보내려면 `TracerProvider` 및 `OTLPSpanExporter`로 OTEL을 설정하세요. 익스포터를 [인증 및 프로젝트 식별을 위한 올바른 엔드포인트와 HTTP 헤더](#필수-설정)로 설정합니다.
