...
keywords: ["Verifiers", "강화 학습", "RL 트레이닝", "롤아웃", "GSM8K"]
---
...
이 가이드는 Verifiers 기반 RL 워크플로에 관측성을 추가하려는 ML 엔지니어와 연구자를 위한 것입니다. 이 가이드에서는 Verifiers, W&amp;B, Weave를 설치하는 방법을 설명하고, Verifiers를 Weave 및 W&amp;B와 함께 사용하는 두 가지 예제를 제공합니다. 하나는 평가 중 롤아웃을 트레이싱하는 예제이고, 다른 하나는 실험 추적과 트레이싱을 활성화한 상태로 모델을 파인튜닝하는 예제입니다.
...
<div id="trace-rollouts-and-evaluate">
  ### 롤아웃을 트레이스하고 평가하기 {#trace-rollouts-and-evaluate}
</div>
...
<div id="fine-tune-a-model-with-experiment-tracking-and-tracing">
  ### 실험 추적 및 트레이싱으로 모델 파인튜닝하기 {#fine-tune-a-model-with-experiment-tracking-and-tracing}
</div>
