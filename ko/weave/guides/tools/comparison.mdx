...
기본적으로 Comparison view의 각 객체는 바로 왼쪽에 있는 객체와 비교됩니다. 하지만 객체를 &#95;베이스라인&#95;으로 설정할 수 있으며, 이렇게 설정하면 모든 객체가 뷰의 가장 왼쪽 객체를 기준으로 비교됩니다.

객체를 베이스라인으로 설정하려면 다음과 같이 하세요.

1. 비교 막대에서 베이스라인으로 설정하려는 객체 위에 포인터를 올립니다.
2. ID 오른쪽의 줄임표(<Icon icon="ellipsis-vertical" iconType="solid" />)를 클릭합니다.
   ![비교 막대에서 선택한 객체의 Make baseline 옵션을 보여주는 Comparison view.](/weave/guides/tools/imgs/comparison-2objs-baseline.png)
3. 목록에서 **Make baseline**을 선택합니다. 그러면 UI가 새로 고침되고, 비교 막대의 가장 왼쪽에 베이스라인 객체가 배치되며 ID 옆에 `Baseline`이 표시됩니다.
   ![비교 막대의 가장 왼쪽에 선택한 베이스라인 객체가 표시된 Comparison view.](/weave/guides/tools/imgs/comparison-2objs-baseline-set.png)
