...
예를 들어, 다음 스크립트는 Weave 트레이스가 W&amp;B 트레이닝 runs와 어떻게 통합되는지 보여줍니다. 이 스크립트는 각 트레이닝 step이 W&amp;B에 메트릭을 로깅하는 동시에 상세한 Weave 트레이스를 생성하는 머신 러닝 트레이닝 루프를 시뮬레이션합니다. 함수에 적용된 `@weave.op` 데코레이터는 입력, 출력, 실행 세부 정보를 자동으로 캡처합니다. 트레이닝 루프가 `wandb.log()`로 메트릭을 로깅하면 Weave 트레이스 정보도 함께 프로젝트에 로깅됩니다.
...
<Note>`Model` 클래스는 현재 Weave Python SDK에서만 지원됩니다.</Note>