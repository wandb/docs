---
title: "저장된 뷰 생성 및 관리"
description: "트레이스된 함수 호출 및 평가와의 상호작용 방식을 사용자 지정하세요"
---

Weave에서 &#95;저장된 뷰&#95;를 사용하면 트레이스된 함수 호출과 평가와의 상호작용 방식을 사용자 지정할 수 있습니다. 저장된 뷰를 정의하면 필터, 정렬, 열 표시 여부를 설정하여 관련 데이터에 빠르게 액세스할 수 있습니다.

Weave Python SDK나 UI에서 직접 뷰를 생성, 수정, 저장할 수 있습니다. Python SDK는 프로그래밍 방식의 필터링과 쿼리에 대한 세밀한 제어를 제공하고, UI는 **트레이스** 및 **Evals** 탭에서 다양한 테이블 설정을 쉽게 탐색하고 저장할 수 있게 해 줍니다.
...
Weave의 `SavedView` 클래스는 트레이스 및 evals 데이터에 대한 뷰를 저장하고, 필터링하고, 정렬하고, 사용자 지정할 수 있는 기능을 제공합니다. 
...
`.to_grid()`를 사용해 저장된 뷰를 그리드 형태로 표시합니다. `limit`으로 표시할 최대 행 수를 지정합니다.
...
`.set_columns()`를 사용해 뷰에서 표시할 열을 설정합니다. 표시할 열을 하나 이상 지정하세요.
...
Weave에서는 모든 트레이스 또는 eval이 오퍼레이션 이름에 연결되어 있습니다.
`.filter_op()`을 사용하여 해당 오퍼레이션이 실행된 Call만 포함하도록 `SavedView`를 필터링하세요.
...
`.add_filter()`를 사용해 뷰에 맞춤형 필터를 적용합니다. 필터는 [지원되는 필터 Operator](#filter-operators) 중 하나와 조건을 사용해 정의합니다.
...
`.save()`를 사용해 저장된 뷰를 Weave에 게시합니다.
...
저장된 뷰에 설정된 필터와 일치하는 함수 호출을 조회하려면 `.get_calls()`를 사용합니다. `limit` 및 `offset`과 같은 선택 파라미터를 지정할 수 있습니다.
...
Weave UI에서 저장된 뷰를 생성하고, 불러오고, 이름을 변경하고, 편집할 수 있습니다. 더 세밀하게 제어하려면 [Python SDK](#saved-views-in-the-python-sdk)를 사용하세요.

<div id="create-a-saved-view">
  ### 저장된 뷰 만들기
</div>

1. **트레이스** 또는 **Evals** 탭으로 이동하세요.
2. 테이블 설정에서 다음 항목 중 하나를 조정하세요:
   * 필터
   * 정렬 순서
   * 페이지 크기
   * 열 표시 여부
   * 열 고정
3. 다음 두 가지 옵션 중 하나를 사용해 뷰를 저장하세요:
   * 오른쪽 상단에서 **Save view**를 클릭하세요.
   * **Save view** 왼쪽의 햄버거 메뉴를 클릭하세요. 드롭다운 메뉴에서 **+ Save as new view**를 클릭하세요.
...
3. 액세스하려는 뷰를 클릭합니다. 선택한 저장된 뷰가 **트레이스** 또는 **Evals** 탭에 표시됩니다. 
...
2. **트레이스** 또는 **Evals** 탭의 왼쪽 상단에서 뷰 이름을 클릭하세요.
3. 새 뷰 이름을 입력하세요.
...
1. [저장된 뷰 불러오기](#load-a-saved-view)에 설명된 step을 따릅니다.
2. 테이블 설정을 변경합니다.
...
<Warning>
  더 이상 본인과 팀에 유용하지 않다고 판단되면 뷰를 삭제할 수 있습니다. 이 작업은 되돌릴 수 없습니다.
</Warning>

1. **트레이스** 또는 **Evals** 탭으로 이동하세요.
2. 삭제하려는 뷰를 [로드](#load-a-saved-view)하세요.
3. **Save view** 왼쪽의 햄버거 메뉴를 클릭하세요.
4. 드롭다운 메뉴에서 **Delete view**를 클릭하세요.
5. 팝업 모달에서 **Delete view**를 클릭하여 확인하세요. 또는 삭제를 중단하려면 **Cancel**을 클릭하세요.
...
1. **트레이스** 또는 **Evals** 탭으로 이동합니다.
2. **트레이스** 또는 **Evals** 탭 오른쪽의 햄버거 메뉴를 클릭합니다. 저장된 모든 뷰가 표시된 드롭다운 메뉴가 나타납니다.
