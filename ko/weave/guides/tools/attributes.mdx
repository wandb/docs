...
* **Per-call attributes(호출 단위 속성)**: `weave.attributes()`를 사용하여 특정 연산 또는 코드 블록에 메타데이터를 추가합니다.
...
Weave TypeScript SDK를 사용하면 `client.getCurrentAttributes()`를 통해 현재 속성을 가져올 수 있습니다. Weave Python SDK와 달리 TypeScript에서는 op 내부뿐만 아니라 `withAttributes()` 컨텍스트 내라면 어디에서든 속성에 접근할 수 있습니다.
