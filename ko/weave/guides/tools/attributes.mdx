...
Weave의 속성을 사용하면 트레이스와 평가에 맞춤형 메타데이터를 첨부할 수 있습니다. 이 메타데이터에는 환경 이름, 모델 버전, 실험 ID, 사용자 ID 또는 Weave 데이터를 정리·필터링·분석하는 데 도움이 되는 다른 맥락 정보가 포함될 수 있습니다.

Weave에서는 속성을 추가하는 두 가지 방법을 제공합니다:

* **Per-call attributes(호출 단위 속성)**: `weave.attributes()`를 사용해 특정 오퍼레이션 또는 코드 블록에 메타데이터를 추가합니다.
* **Global attributes(전역 속성)**: 초기화 시 `global_attributes` 필드를 사용해 프로젝트의 모든 트레이스와 평가에 적용되는 속성을 설정합니다.

트레이스와 평가에서 로깅된 모든 속성을 UI에서 확인할 수 있습니다. 이 속성을 사용해 데이터를 필터링하고 그룹화할 수 있습니다.

<Tip>
  `call.attributes`는 호출이 시작된 이후에는 수정할 수 없습니다. op를 호출하기 전에 이 컨텍스트 매니저를 사용해 필요한 메타데이터를 설정하세요.
</Tip>
...
`weave.attributes()` 컨텍스트 매니저를 사용하면 특정 트레이스된 오퍼레이션에 메타데이터를 추가할 수 있습니다. 이를 통해 특정 함수 호출이나 평가 run에 컨텍스트 정보를 태그로 지정할 수 있습니다.
...
이 함수는 컨텍스트 매니저 블록(Python) 또는 콜백 함수(TypeScript) 내에서 트레이스되는 모든 오퍼레이션에 해당 속성을 연결합니다.
...
<Tabs>
  <Tab title="Python">
    ...
    # 이제 global_attributes 사전이 이후의 모든 오퍼레이션에 이 속성들을 적용합니다
    @weave.op
    def my_function():
        return "Hello"
  ...
  <Tab title="TypeScript">
    ...
    async function main() {
        ...
        // 이제 globalAttributes 객체가 이후의 모든 오퍼레이션에 이 속성들을 적용합니다
...
<div id="combine-global-and-per-call-attributes">
  ### 전역 속성과 호출 단위 속성 결합하기
</div>

전역 속성과 호출 단위 속성을 함께 사용할 수 있습니다. 이때 같은 키를 가진 호출 단위 속성은 전역 속성보다 우선 적용됩니다:
...
<Tabs>
  <Tab title="Python">
    다음 예시는 Weave 데코레이터를 사용해 `process_data` 함수를 로깅하고, 로깅할 속성을 설정한 뒤, 실행 시 해당 속성을 반환합니다.
    ...
    <Note>
      `weave.get_current_call()`은 `@weave.op` 데코레이터가 적용된 함수 **내부에서만** 동작합니다. op 외부에서는 `None`을 반환합니다.
    </Note>
  </Tab>

  <Tab title="TypeScript">
    Weave TypeScript SDK를 사용하면 `client.getCurrentAttributes()`를 통해 현재 속성을 조회할 수 있습니다. Weave Python SDK와 달리 TypeScript에서는 op 내부뿐만 아니라 `withAttributes()` 컨텍스트 내라면 어디에서든 속성에 액세스할 수 있습니다.
