...
* **call별 속성**: `weave.attributes()`를 사용해 특정 오퍼레이션이나 코드 블록에 메타데이터를 추가합니다
...
<Tip>
  call이 시작되면 `call.attributes`는 수정할 수 없습니다. op를 호출하기 전에
...
`weave.attributes()` 컨텍스트 관리자를 사용하면 특정 트레이스된 오퍼레이션에 메타데이터를 추가할 수 있습니다. 이를 통해 특정 함수 호출이나 평가 run에 컨텍스트 정보를 태그할 수 있습니다.
