...
모델에 새 메시지를 보내려면 **Message composer**를 사용하세요.

메시지 역할을 선택하고, 실행할 메시지를 제출할 수 있습니다. 대부분의 상호작용은 **User** 메시지로 작성됩니다. 테스트 중인 지침이 변경될 때 **System** 또는 **Assistant** 메시지를 추가할 수 있습니다.

<Frame>
![상단에는 프롬프트 정의 영역, 중간에는 메시지 이력, 하단에는 새 메시지를 보내기 위한 메시지 작성기가 있는 Weave Playground 화면.](/weave/guides/tools/imgs/playground_chat_input.png)
</Frame>
...
Playground에서는 LLM을 서로 비교할 수 있습니다. 비교를 수행하려면 오른쪽 Playground 툴바에서 **Add Chat (<Icon icon="plus" iconType="solid" />)** 버튼을 클릭합니다. 그러면 원래 채팅 옆에 두 번째 채팅이 열립니다.
...
5. **Update model**을 클릭합니다. 이렇게 하면 모델이 업데이트되고 **Select a model** 드롭다운의 **Saved Models**에서 사용할 수 있게 됩니다. 저장된 모델의 버전은 자동으로 증가합니다.