...
Weave는 세션, LLM 호출, 도구 실행을 포함한 에이전트 대화의 전체 라이프사이클 전반에 대한 관측성을 제공합니다.
...
개별 함수 호출, 애플리케이션 코드 또는 맞춤형 로직을 트레이스하려면 Weave Ops와 Calls를 사용하세요. 함수에 한 줄만 추가하면 입력, 출력, 비용, 토큰 수, 지연 시간을 추적할 수 있습니다.

* 데이터가 LLM 애플리케이션 전반에서 어떻게 흐르는지 엔드 투 엔드로 추적하세요.
* LLM 피드백을 생성하는 데 사용된 원본 문서를 확인하세요.
* 특정 프롬프트와 답변이 생성되는 방식을 자세히 살펴보세요.

개별 함수를 트레이스하려면 Weave [Op 트레이싱 퀵스타트](/ko/weave/quickstart)를 따르거나 Weave [Ops and Calls](/ko/weave/guides/tracking/tracing) 사용법을 알아보세요.
...
평가를 통해 LLM 애플리케이션의 성능을 체계적으로 벤치마크하고 모니터링하여 품질과 안정성을 반복적으로 개선할 수 있습니다.
