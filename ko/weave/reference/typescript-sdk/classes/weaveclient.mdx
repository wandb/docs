...
description: "TypeScript SDK 참조 문서"
---
...
호출에 scorer 결과(예: scorer 출력값)를 추가합니다.
명령형 Evaluation에서 scorer 결과를 predict 호출에 연결할 때 사용합니다.
...
| 이름 | 타입 | 설명 |
| :------ | :------ | :------ |
| `predictCallId` | `string` | 피드백을 연결할 predict 호출의 ID |
| `scorerCallId` | `string` | 피드백을 생성한 scorer 호출의 ID |
