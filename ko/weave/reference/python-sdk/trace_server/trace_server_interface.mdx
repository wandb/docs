...
description: "weave.trace_server.trace_server_interface에 대한 Python SDK 참조"
---
...
주석 큐에 호출을 일괄 추가하기 위한 요청입니다. 

내부 API에서 사용하기 위해 `queue_id` 필드를 추가하여 `AnnotationQueueAddCallsBody`를 확장한 것입니다. 
...
annotation queue 생성 요청에 대한 응답입니다. 
...
annotation 큐의 항목을 조회하기 위한 요청입니다. 

내부 API에서 사용하기 위해 `queue_id`를 추가하여 `AnnotationQueueItemsQueryBody`를 확장한 것입니다. 
...
annotation queue 항목 조회 결과에 대한 응답입니다. 
...
특정 annotation queue를 조회하기 위한 요청입니다. 
...
annotation queue 응답에 대한 스키마입니다. 
...
프로젝트의 annotation queue를 조회하는 요청입니다. 
...
여러 annotation 큐에 대한 통계 정보를 가져오기 위한 요청입니다. 
...
여러 annotation queue의 통계 정보가 담긴 응답입니다. 
...
현재 annotator에 대해 큐 항목의 annotation 상태를 업데이트하기 위한 요청입니다. 

유효한 상태 전환은 다음과 같습니다. 

- (absence) -> 'in_progress': 항목을 진행 중으로 표시합니다 (레코드가 없을 때만) 
- (absence) -> 'completed' 또는 'skipped': 항목을 바로 완료/건너뜁니다 
- 'in_progress' 또는 'unstarted' -> 'completed' 또는 'skipped': 시작된 항목을 완료/건너뜁니다 
...
### <kbd>메서드</kbd> `serialize_typed_dicts`
...
이 모델은 RESTful 엔드포인트에서 URL 경로로 전달되는 project_id와 evaluation_run_id를 포함하지 않습니다. 
...
이 프로토콜은 레거시 V1 엔드포인트와 최신 Object 엔드포인트를 모두 포함한 전체 API 세트를 지원하는 트레이스 서버 구현을 나타냅니다. 두 API 버전을 모두 지원해야 하는 구현에는 이 타입을 사용하세요. 

---

<SourceLink url="https://github.com/wandb/weave/blob/v0.52.24/weave/trace_server/trace_server_interface.py#L2211" />

### <kbd>메서드</kbd> `actions_execute_batch`
...
### <kbd>메서드</kbd> `annotation_queue_add_calls`
...
### <kbd>메서드</kbd> `annotation_queue_create`
...
### <kbd>메서드</kbd> `annotation_queue_read`
...
### <kbd>메서드</kbd> `annotation_queues_query_stream`
...
### <kbd>메서드</kbd> `annotator_queue_items_progress_update`
...
### <kbd>메서드</kbd> `completions_create`
...
### <kbd>메서드</kbd> `completions_create_stream`
...
### <kbd>메서드</kbd> `ensure_project_exists`
...
### <kbd>메서드</kbd> `evaluation_run_create`
...
### <kbd>메서드</kbd> `evaluation_run_delete`
...
### <kbd>메서드</kbd> `evaluation_run_finish`
...
### <kbd>메서드</kbd> `evaluation_run_list`
...
### <kbd>메서드</kbd> `evaluation_run_read`
...
### <kbd>메서드</kbd> `feedback_create_batch`
...
### <kbd>메서드</kbd> `table_create_from_digests`
...
### <kbd>메서드</kbd> `table_query_stats_batch`
...
### <kbd>메서드</kbd> `table_query_stream`
...
Trace Server용 Object API 엔드포인트입니다. 

이 프로토콜에는 더 깔끔하고 RESTful한 인터페이스를 제공하는 객체 관리 API가 포함되어 있습니다. 구현은 하위 호환성을 유지하기 위해 이 프로토콜과 TraceServerInterface를 모두 지원해야 합니다. 
...
이 모델에는 RESTful 엔드포인트의 URL 경로에서 전달되는 project_id가 포함되지 않습니다. 
...
내부 API에서 사용하기 위해 `project_id`를 추가하여 `OpCreateBody`를 확장한 모델입니다. 
...
Op 객체를 읽을 때 사용하는 응답 모델입니다. 

`code` 필드에는 해당 Op의 실제 소스 코드가 포함됩니다. 
...
이 모델에는 RESTful 엔드포인트에서 URL 경로로 전달되는 project_id가 포함되지 않습니다. 
...
내부 API에서 사용하기 위해 project_id를 추가하여 PredictionCreateBody를 확장합니다. 
...
RESTful 엔드포인트에서는 URL 경로에서 전달되므로 이 모델에는 `project_id`가 포함되지 않습니다. 
...
내부 API에서 사용하기 위해 `project_id`를 추가해 `ScoreCreateBody`를 확장한 모델입니다. 
...
턴 호출(turn call)을 기준으로 집계된 통계를 사용해 thread를 조회합니다. 

턴 호출은 thread 컨텍스트의 직접적인 자식 호출이며(여기서 call.id == turn_id), 모든 중첩된 구현 세부 정보까지 포함하는 대신 보다 의미 있는 대화 단위 수준의 통계를 제공합니다. 
...
### <kbd>메서드</kbd> `threads_query_stream`
