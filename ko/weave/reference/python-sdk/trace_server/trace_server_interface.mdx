...
description: "weave.trace_server.trace_server_interface에 대한 Python SDK 레퍼런스"
---
...
어노테이션 큐에 Call을 일괄 추가하기 위한 Request입니다.
...
큐에 Call을 추가한 결과에 대한 Response입니다.
...
어노테이션 큐의 항목을 쿼리하기 위한 Request입니다.

내부 API 사용을 위해 queue&#95;id를 추가하여 AnnotationQueueItemsQueryBody를 확장한 것입니다.
...
특정 어노테이션 큐를 조회하기 위한 Request입니다.
...
프로젝트의 어노테이션 큐를 쿼리하기 위한 Request입니다.
...
여러 어노테이션 큐의 통계를 조회하기 위한 Request입니다.
...
여러 어노테이션 큐에 대한 통계 정보를 담은 Response입니다.
...
현재 어노테이터의 큐 항목 어노테이션 상태를 업데이트하기 위한 Request입니다.

유효한 상태 전이:

* (없음) -&gt; &#39;in&#95;progress&#39;: 레코드가 없을 때에만 항목을 진행 중으로 표시
* (없음) -&gt; &#39;completed&#39; 또는 &#39;skipped&#39;: 항목을 바로 완료 또는 건너뜀 처리
* &#39;in&#95;progress&#39; 또는 &#39;unstarted&#39; -&gt; &#39;completed&#39; 또는 &#39;skipped&#39;: 시작했거나 아직 시작하지 않은 항목을 완료 또는 건너뜀 처리
...
어노테이션 상태를 업데이트한 결과로 반환되는 Response입니다.
...
REST API를 통해 evaluation run을 완료하기 위한 Request 본문입니다.
...
V1 API와 Object API를 모두 지원하는 전체 트레이스 서버 인터페이스입니다.

이 프로토콜은 레거시 V1 엔드포인트와 최신 Object 엔드포인트를 모두 포함한 전체 API 세트를 지원하는 트레이스 서버 구현을 나타냅니다. 두 API 버전을 모두 지원해야 하는 구현에는 이 유형을 사용하세요.
...
Trace Server의 객체 API 엔드포인트입니다.

이 프로토콜에는 더 깔끔한 RESTful 인터페이스를 제공하는 객체 관리 API가 포함되어 있습니다. 이전 버전과의 호환성을 유지하려면 구현체가 이 프로토콜과 TraceServerInterface를 모두 지원해야 합니다.
...
REST API를 통해 Op 객체를 생성하기 위한 Request 본문입니다.

이 모델은 `project_id`를 제외합니다. `project_id`는 RESTful 엔드포인트의 URL 경로에서 가져오기 때문입니다.
...
Op 객체를 생성하기 위한 Request 모델입니다.

내부 API 사용을 위해 `project_id`를 추가하여 OpCreateBody를 확장합니다.
...
Op 객체를 조회할 때 사용하는 Response 모델입니다.

`code` 필드에는 op의 실제 소스 코드가 포함되어 있습니다.
...
REST API를 통해 Prediction을 생성하기 위한 Request 본문입니다.

이 모델은 RESTful 엔드포인트의 URL 경로에서 `project_id`를 가져오므로 이를 제외합니다.
...
Prediction을 생성하기 위한 Request 모델입니다.

내부 API 사용을 위해 `project_id`를 추가하여 PredictionCreateBody를 확장합니다.
...
REST API를 통해 Score를 생성할 때 사용하는 Request 본문입니다.

이 모델은 `project_id`를 제외합니다. 이 값은 RESTful 엔드포인트의 URL 경로에서 가져오기 때문입니다.
...
Score를 생성하기 위한 Request 모델입니다.

내부 API 사용을 위해 `project_id`를 추가한 ScoreCreateBody의 확장 모델입니다.
...
turn Call만을 기준으로 집계된 통계와 함께 스레드를 쿼리합니다.

turn Call은 스레드 컨텍스트의 직계 하위 항목입니다(`call.id == turn_id`인 경우). 이를 통해 모든 중첩된 구현 세부 정보를 포함하는 대신 의미 있는 대화 수준 통계를 제공합니다.
