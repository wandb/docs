...
description: "weave.trace_server_bindings.remote_http_trace_server용 Python SDK 참조 문서"
---
...
### <kbd>메서드</kbd> `get_call_processor`

```python
get_call_processor() → AsyncBatchProcessor | None
```

내부 호출 프로세서를 노출하기 위한 커스텀 메서드로, 정식 TraceServerInterface 에는 정의되어 있지 않습니다. 클라이언트 측 인터페이스에 정식으로 포함되어야 합니다.
...
### <kbd>메서드</kbd> `get_feedback_processor`

```python
get_feedback_processor() → AsyncBatchProcessor | None
```

내부 피드백 프로세서를 노출하기 위해 공식 `TraceServerInterface`에 정의되지 않은 커스텀 메서드입니다. 클라이언트 측 인터페이스에서 정식으로 규격화해야 합니다.
...
`calls/batch_upsert`와 유사하게, 테이블 업데이트를 일련의 업데이트로 나눌 수 있는 특성 덕분에 페이로드 크기를 동적으로 조정할 수 있습니다.
