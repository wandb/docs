...
description: "weave.trace_server_bindings.remote_http_trace_server에 대한 Python SDK 참고 문서"
---
...
내부 호출 프로세서를 노출하기 위해 공식 TraceServerInterface에 정의되어 있지 않은 커스텀 메서드입니다. 클라이언트 측 인터페이스에 정식으로 포함되어야 합니다.
...
하위 피드백 프로세서를 노출하기 위해 공식 TraceServerInterface에 정의되지 않은 커스텀 메서드입니다. 클라이언트 측 인터페이스에 정식으로 포함되어야 합니다.
...
실제 행 데이터 대신 행 다이제스트를 지정하여 테이블을 생성합니다.
...
`calls/batch_upsert`와 유사하게, 테이블 업데이트는 일련의 업데이트로 분해될 수 있는 특성이 있어 페이로드 크기를 동적으로 조정할 수 있습니다.
