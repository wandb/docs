...
이것은 `threading.Thread`를 바로 대체해 사용할 수 있는 구현으로, 스레드 내부에서 call이 예상대로 동작하도록 보장합니다. Weave는 특정 contextvars가 설정되어 있어야 하지만(`call&#95;context.py` 참조), 새 스레드는 부모의 컨텍스트를 자동으로 복사하지 않기 때문에 call 컨텍스트가 사라질 수 있습니다 -- 이는 바람직하지 않습니다! 이 클래스는 contextvar 복사를 자동화하므로, 이 스레드를 사용하면 사용자가 기대하는 대로 &quot;그냥 동작합니다&quot;.
