...
Retrieval Augmented Generation (RAG)는 맞춤형 지식 베이스에 액세스할 수 있는 생성형 AI 애플리케이션을 구축할 때 널리 사용하는 방법입니다.
...
* 지식 베이스를 구축하는 방법
* 관련 문서를 찾는 검색 step을 포함한 RAG 애플리케이션을 만드는 방법
* Weave로 검색 step을 추적하는 방법
* LLM 기반 평가자를 사용해 컨텍스트 정밀도를 측정하여 RAG 애플리케이션을 평가하는 방법
* 맞춤형 스코어링 함수를 정의하는 방법
...
다음으로, 검색 함수 `get_most_relevant_document`에 `weave.op()` 데코레이터를 적용하고 `Model` 클래스를 생성합니다. `weave.init('<team-name>/rag-quickstart')`를 호출하여 나중에 확인할 수 있도록 함수의 모든 입력과 출력을 추적하세요. 팀 이름을 지정하지 않으면 출력은 [W&amp;B 기본 팀 또는 entity](/ko/platform/app/settings-page/user-settings#default-team)에 기록됩니다.
...
애플리케이션을 평가할 단순한 방법이 없을 때는, LLM을 사용해 그 일부 측면을 평가하는 방법이 있습니다. 다음은 주어진 답변에 도달하는 데 컨텍스트가 유용했는지 확인하도록 프롬프트하여 컨텍스트 정밀도를 측정해 보려고 LLM 기반 평가자를 사용하는 예시입니다. 이 프롬프트는 널리 사용되는 [RAGAS 프레임워크](https://docs.ragas.io/en/stable/)를 바탕으로 확장되었습니다.

<div id="defining-a-scoring-function">
  ### 스코어링 함수 정의하기
</div>

[Build an Evaluation pipeline 튜토리얼](/ko/weave/tutorial-eval)에서와 마찬가지로, 앱을 테스트할 예시 행 집합과 스코어링 함수를 정의하세요. 스코어링 함수는 한 행을 받아 평가합니다. 입력 인수는 해당 행의 키와 일치해야 하므로, 여기서 `question`은 행 딕셔너리에서 가져옵니다. `output`은 모델의 출력입니다. 모델 입력도 입력 인수를 기준으로 예시에서 가져오므로, 여기서도 `question`이 사용됩니다. 이 예제는 `async` 함수를 사용하므로 여러 호출을 병렬로 빠르게 실행할 수 있습니다. async에 대한 간단한 소개가 필요하다면 [여기](https://docs.python.org/3/library/asyncio.html)에서 확인할 수 있습니다.
...
일부 애플리케이션에서는 맞춤형 평가 클래스를 만들고 싶을 수 있습니다. 예를 들어, 특정 파라미터(예: 채팅 모델, 프롬프트), 각 행에 대한 특정 채점 방식, 집계 점수의 특정 계산 방식을 포함하는 표준화된 `LLMJudge` 클래스를 만들 수 있습니다. Weave는 바로 사용할 수 있는 `Scorer` 클래스 목록을 제공하며, 맞춤형 `Scorer`를 쉽게 만들 수도 있습니다. 다음 예시는 맞춤형 `class CorrectnessLLMJudge(Scorer)`를 만드는 방법을 보여줍니다.

개략적으로 보면, 맞춤형 Scorer를 만드는 step는 비교적 단순합니다:

1. `weave.flow.scorer.Scorer`를 상속하는 맞춤형 클래스를 정의합니다.
2. `score` 함수를 오버라이드하고, 함수의 각 호출을 추적하려면 `@weave.op()`을 추가합니다.
   * 이 함수는 모델의 예측 결과가 전달될 `output` 인수를 정의해야 합니다. 모델이 &quot;None&quot;을 반환할 가능성이 있다면 유형을 `Optional[dict]`로 정의합니다.
   * 나머지 인수들은 일반적인 `Any` 또는 `dict`로 정의할 수도 있고, 모델을 평가하는 데 사용되는 데이터셋의 특정 열을 선택할 수도 있습니다. 이 경우 `weave.Evaluate` 클래스를 사용할 수 있으며, (사용 중이라면) `preprocess_model_input`을 거친 뒤 단일 행의 열 이름 또는 키와 정확히 동일한 이름을 사용해야 합니다.
3. *선택 사항:* 집계 점수 계산 방식을 사용자 지정하려면 `summarize` 함수를 오버라이드합니다. 기본적으로 Weave는 사용자 정의 함수를 정의하지 않으면 `weave.flow.scorer.auto_summarize` 함수를 사용합니다.
...
이를 Scorer로 사용하려면, 다음과 같이 초기화한 후 `Evaluation`의 `scorers` 인수로 전달합니다:
...
* LLM 호출 및 검색 step 함수를 `weave.op()`으로 래핑하세요
* (선택) `predict` 함수와 앱 정보를 포함하는 `Model` 서브클래스를 생성하세요
* 평가할 예시를 수집하세요
* 하나의 예시에 점수를 매기는 스코어링 함수를 생성하세요
* `Evaluation` 클래스를 사용해 예시에 대해 평가를 실행하세요

**참고:** 경우에 따라 Evaluation이 비동기적으로 실행되면 OpenAI, Anthropic 등의 모델에서 요청 속도 제한을 트리거할 수 있습니다. 이를 방지하려면 병렬 워커 수를 제한하는 환경 변수를 설정할 수 있습니다. 예: `WEAVE_PARALLELISM=3`.
...
<Tabs>
  ...
  <Tab title="TypeScript">
        ...
        async predict(question: string): Promise<{
            ...
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    ...
                    { 
                        role: "user", 
                        content: `다음 정보를 사용하여 아래 질문에 답하세요. 답을 찾을 수 없는 경우 "모르겠습니다"라고 작성하세요.
                        Context:
                        """
                        ${context}
                        """
    ...
    const contextPrecisionScore = weave.op(async function(args: {
        ...
        const prompt = `주어진 질문, 답변, 컨텍스트를 바탕으로 해당 컨텍스트가 답변 도출에 유용했는지 확인하세요. 유용한 경우 "1", 그렇지 않은 경우 "0"을 JSON 출력으로 제공하세요.
        유효한 JSON 형식으로만 출력하세요.
    ...
    async function main() {
        ...
        const model = new RAGModel({
            systemMessage: "당신은 금융 전문가로서 금융, 금융 서비스 및 금융 시장과 관련된 질문에 답합니다. 제공된 정보를 바탕으로 응답할 때는 반드시 출처를 인용하세요.",
            articleEmbeddings
        });
...
이 튜토리얼에서는 이 예제에서 살펴본 검색 step처럼 애플리케이션의 여러 step에 관측 가능성을 어떻게 구축하는지 다루었습니다. 또한 애플리케이션 Response를 자동으로 평가하기 위해 LLM 기반 평가자와 같은 더 복잡한 스코어링 함수를 구축하는 방법도 배웠습니다.

<div id="next-steps">
  ## 다음 단계
</div>

엔지니어를 위한 실전 RAG 기법을 더 깊이 있게 다루는 [RAG++ 강좌](https://www.wandb.courses/courses/rag-in-production?utm_source=wandb_docs\&utm_medium=code\&utm_campaign=weave_docs)를 살펴보세요. 이 과정에서는 Weights &amp; Biases, Cohere, Weaviate의 프로덕션 수준 솔루션을 통해 성능을 최적화하고 비용을 절감하며 애플리케이션의 정확도와 관련성을 높이는 방법을 배웁니다.