...
**참고:** 경우에 따라 `Evaluation`이 비동기로 실행되면서 OpenAI, Anthropic 및 기타 모델에서 요청 속도 제한이 트리거될 수 있습니다. 이를 방지하려면 병렬 worker 수를 제한하는 환경 변수를 설정하면 됩니다. 예를 들어 `WEAVE_PARALLELISM=3`입니다.
