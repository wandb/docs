...
이 가이드는 W&amp;B Weave를 [W&amp;B Inference](https://docs.wandb.ai/inference/)와 함께 사용하는 방법을 설명합니다. W&amp;B Inference를 사용하면 자체 인프라를 구축하거나 여러 제공자의 API 키를 관리하지 않고도 실시간 오픈 소스 모델을 활용해 LLM 애플리케이션을 구축하고 추적할 수 있습니다. W&amp;B API 키만 있으면 [W&amp;B Inference에서 호스팅하는 모든 모델](https://docs.wandb.ai/inference/models/)과 상호작용할 수 있습니다.
...
* Weave와 W&amp;B Inference 설정
* 자동 트레이싱이 적용된 기본 LLM 애플리케이션 구축
* 여러 모델 비교
* 데이터셋에서의 모델 성능 평가
* Weave UI에서 결과 확인
...
먼저 아래 코드 예제를 복사해 붙여넣으세요. 이 예제 코드는 W&amp;B Inference의 Llama 3.1-8B를 사용합니다.

이 코드를 실행하면 Weave가 다음을 수행합니다:

* LLM 호출을 자동으로 추적합니다.
* 입력, 출력, 지연 시간(latency), 토큰 사용량을 기록합니다.
* Weave UI에서 해당 추적(trace)을 볼 수 있는 링크를 제공합니다.
...
<Tabs>
  <Tab title="Python">
    ...
    # Weave 초기화 - your-team/your-project를 실제 값으로 바꾸세요
    ...
    # 함수를 데코레이트해 추적을 활성화합니다; 표준 OpenAI 클라이언트를 사용
    @weave.op()
  ...
  <Tab title="TypeScript">
    ...
    // Weave 초기화 - "<>"로 둘러싸인 값을 본인 값으로 교체하세요.
...
다음으로 이 코드를 실행해 보세요. Weave가 중첩된 연산을 어떻게 추적하는지 보여 주는 기본적인 요약 애플리케이션입니다:
...
W&amp;B Inference를 사용하면 여러 모델을 사용할 수 있습니다. 다음 코드를 사용해 Llama와 DeepSeek의 응답 성능을 비교하세요:
...
<Tabs>
  <Tab title="Python">
    ...
    # 서로 다른 LLM을 비교하기 위한 모델 클래스를 정의합니다
...
Weave의 내장 `EvaluationLogger`를 사용해 모델이 Q&amp;A 태스크에서 얼마나 잘 동작하는지 평가하세요. 이를 통해 자동 집계, 토큰 사용량 캡처, UI에서 제공되는 다양한 비교 기능이 포함된 구조화된 평가 추적이 가능합니다.
...
<Tabs>
  <Tab title="Python">
    ...
    def evaluate_model(model: InferenceModel, dataset: list[dict]):
        """Weave의 내장 평가 프레임워크를 사용하여 데이터셋을 평가합니다."""
        ...
        print(f"{model.model_name} 평가 완료 (로깅 이름: {safe_model_name}). Weave UI에서 결과를 확인하세요.")
...
이 예제들을 실행하면 터미널에 trace 링크가 출력됩니다. 원하는 링크를 클릭해 Weave UI에서 해당 trace를 확인하세요.
...
* 개별 예제를 넘겨 보면서 동일한 입력에 대해 서로 다른 모델이 어떻게 동작했는지 확인

<div id="available-models">
  ## 사용 가능한 모델
</div>

사용 가능한 모델의 전체 목록은 W&amp;B Inference 문서의 [사용 가능한 모델](https://docs.wandb.ai/inference/models/) 섹션을 참조하세요.
...
* **Evaluation 생성**: LLM 애플리케이션의 [체계적인 평가](/ko/weave/guides/core-types/evaluations)에 대해 알아보기
* **다른 통합 사용해 보기**: Weave는 [OpenAI, Anthropic 등 다양한 서비스](/ko/weave/guides/integrations)와 연동됩니다
...
<details>
  ...
  W&amp;B Inference에는 프로젝트별 동시 요청 한도가 있습니다. 요청 속도 제한에 걸린 경우:

  * 동시에 보내는 요청 수를 줄이세요.
  * 호출 간에 지연 시간을 추가하세요.
  * 더 높은 한도가 필요한 경우 요금제를 업그레이드하는 것을 고려하세요.

  자세한 내용은 [W&amp;B Inference 사용 한도 문서](https://docs.wandb.ai/inference/usage-limits/)를 참고하세요.
</details>

<details>
  <summary>크레딧 부족</summary>

  무료 요금제에는 크레딧이 제한적으로 제공됩니다. 자세한 내용은 [사용량 및 한도 문서](https://docs.wandb.ai/inference/usage-limits/)를 참고하세요.
</details>