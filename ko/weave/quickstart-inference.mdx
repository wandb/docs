...
description: "W&B Inference를 사용해 모델 Call을 트레이스하고, 출력을 비교하고, 평가를 실행하면서 Weave의 기본 사항을 알아보세요."
---
...
이 가이드에서는 [W&amp;B Inference](https://docs.wandb.ai/inference/)와 함께 W&amp;B Weave를 사용하는 방법을 설명합니다. 자체 인프라를 설정하거나 여러 제공자의 API 키를 관리하지 않고도 실시간 오픈 소스 모델을 사용해 LLM 애플리케이션을 구축하고 트레이스하려면 W&amp;B Inference를 사용하세요. W&amp;B API 키를 사용하면 [W&amp;B Inference에서 호스팅된 모든 모델](https://docs.wandb.ai/inference/models/)과 상호 작용할 수 있습니다.
...
먼저 아래 코드 예시를 복사해 붙여넣으세요. 이 코드 예시는 W&amp;B Inference의 Llama 3.1-8B를 사용합니다.
...
<Tabs>
  <Tab title="Python">
    ...
    # 함수를 데코레이트하여 트레이싱을 활성화합니다. 표준 OpenAI 클라이언트를 그대로 사용하세요
    @weave.op()
  ...
  <Tab title="TypeScript">
    ...
    const client = new OpenAI({
        ...
        apiKey: process.env.WANDB_API_KEY || 'YOUR_WANDB_API_KEY', // 본인의 API 키로 교체하거나 WANDB_API_KEY 환경 변수를 설정하세요
    });
...
이제 다음 코드를 실행해 보세요. Weave가 중첩된 오퍼레이션을 어떻게 트레이스하는지 보여주는 기본 텍스트 요약 애플리케이션입니다.
...
W&amp;B Inference는 여러 모델에 액세스할 수 있도록 제공합니다. 다음 코드를 사용하여 Llama와 DeepSeek의 각 Response 성능을 비교하세요:
...
<Tabs>
  <Tab title="Python">
    ...
    # 서로 다른 LLM을 비교하기 위한 Model class를 정의합니다
  ...
  <Tab title="TypeScript">
    ...
    // weave.op을 사용하여 모델 함수를 생성합니다(TypeScript에서는 weave.Model이 지원되지 않음)
...
Weave의 내장 `EvaluationLogger`를 사용해 Q&amp;A 작업에서 모델이 얼마나 잘 작동하는지 평가하세요. 이를 통해 자동 집계, 토큰 사용량 캡처, 그리고 UI에서의 다양한 비교 기능을 포함한 구조화된 평가 추적을 제공할 수 있습니다.
...
이 예시들을 실행하면 터미널에 트레이스 링크가 출력됩니다. 링크를 클릭해서 Weave UI에서 트레이스를 확인하세요.

Weave UI에서는 다음 작업을 수행할 수 있습니다:

* 모든 LLM Call의 타임라인 검토
* 각 오퍼레이션의 inputs 및 출력 검사
* token 사용과 예상 비용 확인 (EvaluationLogger에서 자동으로 캡처)
* 지연 시간과 성능 메트릭 분석
* **Evals** 탭으로 이동해 집계된 Evaluation 결과 확인
* **Compare** 기능을 사용해 서로 다른 모델 간 성능 비교
* 특정 예시를 하나씩 살펴보며 서로 다른 모델이 동일한 inputs에서 어떻게 동작했는지 확인
...
* **플레이그라운드 사용하기**: Weave 플레이그라운드에서 [모델을 대화형으로 사용해 보세요](/ko/weave/guides/tools/playground#access-the-playground)
* **평가 구축하기**: LLM 애플리케이션의 [체계적인 평가](/ko/weave/guides/core-types/evaluations)에 대해 알아보세요
* **다른 인테그레이션 시도하기**: Weave는 [OpenAI, Anthropic 등](/ko/weave/guides/integrations)과 함께 작동합니다
...
<details>
  ...
  W&amp;B Inference에는 프로젝트별 동시성 한도가 있습니다. 요청 속도 제한에 걸린 경우:

  * 동시에 보내는 요청 수를 줄이세요.
  * Call 사이에 지연 시간을 추가하세요.
  * 더 높은 한도가 필요하다면 플랜 업그레이드를 고려하세요.
...
<details>
  ...
  무료 요금제에는 제한된 크레딧이 포함되어 있습니다. 자세한 내용은 [사용 및 한도 관련 문서](https://docs.wandb.ai/inference/usage-limits/)를 참조하세요.
</details>