...
description: Weave SDK로 멀티턴 에이전트를 트레이스하세요. Session, 턴, LLM calls, 도구 Call이 프로젝트의 Agents 뷰에 표시됩니다.
---
...
[Colab에서 사용해 보기](https://colab.research.google.com/github/wandb/weave/blob/master/docs/weave/cookbooks/source/agents-quickstart.ipynb) · [GitHub 원본](https://github.com/wandb/weave/blob/master/docs/weave/cookbooks/source/agents-quickstart.ipynb)

Weave SDK를 사용하면 널리 사용되는 SDK나 맞춤형 하니스로 구축한 에이전트를 트레이스할 수 있습니다. 이 퀵스타트에서는 OpenTelemetry span을 생성하고 캡처할 수 있도록, 맞춤형 멀티턴 에이전트에 Weave를 수동으로 통합하는 방법을 안내합니다. 에이전트용 Weave의 개념을 이해하려면 [에이전트 트레이싱](/ko/weave/guides/tracking/trace-agents)을 참조하세요.

Claude Agent SDK나 Codex처럼 널리 사용되는 SDK 또는 하니스에 Weave를 통합하려는 경우 [에이전트 인테그레이션 트레이싱](/ko/weave/guides/tracking/trace-agent-integrations)을 참조하세요. Weave는 빠르게 통합할 수 있도록 여러 널리 사용되는 에이전트 구축 SDK와 에이전트 하니스에 자동으로 패치됩니다.

<div id="what-youll-learn">
  ## 학습할 내용
</div>

이 가이드의 코드는 Wikipedia에서 정보를 조회할 수 있는 간단한 리서치 에이전트를 설정합니다. 세 가지 질문(세 번의 턴)을 하고, 답을 찾기 위해 언제 Wikipedia를 검색할지 AI가 결정하도록 하며, Weave를 사용해 모든 단계(대화, 각 질문, 각 AI 응답, 각 Wikipedia 조회)를 기록합니다. 이를 통해 Weave Agents 뷰에서 정확히 어떤 일이 일어났는지 확인할 수 있습니다.

이 가이드에서는 다음 방법을 설명합니다:

* `weave.init()`를 사용해 에이전트 Tracing용 Weave를 초기화하기
* `start_session` / `startSession` 및 `start_turn` / `startTurn`으로 Session과 턴 시작하기
* `start_llm` / `startLLM`으로 LLM calls를 래핑하고 사용량 기록하기
* `start_tool` / `startTool`로 도구 실행을 래핑하고 결과 기록하기
* 생성된 Session, 턴, 도구 Call을 Agents 뷰에서 보기
...
다음 코드는 에이전트의 Wikipedia 검색 도구와, 도구를 언제 어떻게 사용할지 판단하기 위한 OpenAI 도구 스키마를 정의합니다.
...
<div id="run-a-traced-multi-turn-agent">
  ## 트레이스가 적용된 멀티턴 에이전트 실행하기
</div>

아래 예시에서는 하나의 Session에서 세 번의 턴을 실행합니다. 각 턴에서는 다음을 수행합니다.

1. `chat` span을 열고, LLM이 도구를 호출할지 결정하도록 합니다
2. LLM이 도구 호출을 요청한 경우, 해당 호출을 `execute_tool` span으로 감싸고 그 결과를 다시 LLM에 전달합니다
3. 최종 답변을 생성하기 위해 두 번째 `chat` span을 엽니다
...
* [Weave로 에이전트를 트레이스하는 방법](weave/guides/tracking/trace-agents)과 Weave SDK에서 사용 가능한 특성과 옵션을 더 잘 이해하세요.
