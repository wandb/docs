---
title: "PII 데이터 처리"
description: "W&B Weave의 데이터 마스킹 및 필터링 도구를 사용하여 LLM 애플리케이션에서 개인 식별 정보(PII)를 처리합니다."
keywords: ["PII 마스킹", "Presidio", "Faker", "autopatch_settings", "데이터 익명화"]
---
...
### `weave.op`을 사용하여 입력 및 출력 로깅 사용자 지정하기 {#customize-input-and-output-logging-using-weaveop}
...
## 마스킹 방법 Overview {#redaction-methods-overview}
...
### 방법 1: 정규 표현식을 사용한 필터링 {#method-1-filter-using-regular-expressions}
...
### 방법 2: Microsoft Presidio를 사용해 PII 마스킹하기 {#method-2-redact-using-microsoft-presidio}
...
### 방법 3: Faker와 Presidio를 사용해 대체값으로 익명화하기 {#method-3-anonymize-with-replacement-using-faker-and-presidio}
...
### 방법 4: `autopatch_settings` 사용 {#method-4-use-autopatch_settings}
...
## 메서드를 Weave Call에 적용하기 {#apply-the-methods-to-weave-calls}
...
### Presidio 마스킹 방법 {#presidio-redaction-method}

다음으로, Presidio를 사용해 원본 텍스트에서 PII 데이터를 파악하고 마스킹하세요.

<Frame>
  ![파악된 PII entity와 마스킹된 텍스트 출력을 보여주는 Presidio PII 마스킹 프로세스](/media/pii/redact.png)
</Frame>
...
### Faker 및 Presidio 대체 방법 {#faker-and-presidio-replacement-method}

이 예제에서는 Faker를 사용해 익명화된 대체 PII 데이터를 생성하고, Presidio를 사용해 원본 텍스트에서 PII 데이터를 파악해 대체합니다.

<Frame>
  ![원본 텍스트, 파악된 PII, 익명화된 대체 값이 포함된 Faker 및 Presidio PII 대체 과정](/media/pii/replace.png)
</Frame>
...
### `autopatch_settings` 방법 {#autopatch_settings-method}
...
### 선택 사항: 데이터를 암호화하세요 {#optional-encrypt-your-data}

<Frame>
  ![암호화된 텍스트 출력 및 암호화 키 관리가 포함된 PII 데이터 암호화 프로세스](/media/pii/encrypt.png)
</Frame>
