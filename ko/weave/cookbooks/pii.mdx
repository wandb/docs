---
title: "PII 데이터 처리"
description: "W&B Weave에서 PII 데이터 처리를 사용하는 방법을 알아보세요"
---
...
이 가이드에서는 W&amp;B Weave를 사용할 때 개인식별정보(PII) 데이터가 비공개로 유지되도록 보호하는 방법을 설명합니다. 이 가이드는 PII 데이터를 식별, 마스킹(비노출 처리) 및 익명화하는 다음과 같은 방법을 보여줍니다:

1. PII 데이터를 식별하고 마스킹하기 위한 **정규 표현식(regular expressions)**.
2. Python 기반 데이터 보호 SDK인 **Microsoft의 [Presidio](https://microsoft.github.io/presidio/)**. 이 도구는 마스킹 및 치환 기능을 제공합니다.
3. 가짜 데이터를 생성하는 Python 라이브러리인 **[Faker](https://faker.readthedocs.io/en/master/)**와 Presidio를 함께 사용하여 PII 데이터를 익명화하는 방법.

또한 *`weave.op` 입력/출력 로깅 커스터마이징* 및 *`autopatch_settings`* 를 사용해 PII 마스킹 및 익명화를 워크플로우에 통합하는 방법도 배우게 됩니다. 자세한 내용은 [로그된 입력과 출력을 커스터마이즈](/ko/weave/guides/tracking/ops/#customize-logged-inputs-and-outputs)를 참고하세요.
...
4. [이 방법들을 Weave 호출에 적용](#apply-the-methods-to-weave-calls)합니다.

<div id="overview">
  ## 개요
</div>

다음 섹션에서는 `weave.op`을 사용한 입력 및 출력 로깅의 개요와 Weave에서 PII 데이터를 처리할 때의 모범 사례를 소개합니다.
...
다음 예제에서는 두 개의 후처리 함수를 정의하고, 이를 `weave.op()`에 인자로 전달합니다.
...
<div id="best-practices-for-using-weave-with-pii-data">
  ### Weave에서 PII 데이터를 사용할 때의 모범 사례
</div>

PII 데이터를 Weave에서 사용하기 전에, 해당 모범 사례를 먼저 검토하세요.

<div id="during-testing">
  #### 테스트 환경에서
</div>

* PII 탐지를 검증하기 위해 익명화된 데이터를 로깅하세요
* Weave Traces로 PII 처리 절차를 추적하세요
...
* 원본 PII를 절대 로그에 기록하지 마세요
* 로그에 기록하기 전에 민감한 필드를 암호화하세요
...
* 암호화된 상태로도 분석해야 하는 데이터에는 전용 암호화 방식 사용을 고려하세요
...
4. 10개의 텍스트 블록이 포함된 데모용 PII 데이터셋을 로드합니다.
...
[사전 설정](#setup)을 완료했다면, 이제 다음 방법을 사용할 수 있습니다.

PII 데이터를 탐지하고 보호하기 위해, 다음 방법을 사용하여 PII 데이터를 식별하고 마스킹하며, 필요에 따라 익명화합니다:

1. **정규 표현식**을 사용해 PII 데이터를 식별하고 마스킹합니다.
2. **Microsoft [Presidio](https://microsoft.github.io/presidio/)**: 마스킹 및 치환 기능을 제공하는 Python 기반 데이터 보호 SDK입니다.
3. **[Faker](https://faker.readthedocs.io/en/master/)**: 가상의 데이터를 생성하는 Python 라이브러리입니다.

<div id="method-1-filter-using-regular-expressions">
  ### 방법 1: 정규식을 사용한 필터링
</div>

[정규 표현식(regex)](https://docs.python.org/3/library/re.html)은 PII 데이터를 식별하고 비식별화(마스킹)하는 가장 간단한 방법입니다. 정규식을 사용하면 전화번호, 이메일 주소, 사회보장번호와 같은 다양한 형식의 민감 정보를 식별할 수 있는 패턴을 정의할 수 있습니다. 정규식을 활용하면 더 복잡한 NLP 기법 없이도 대량의 텍스트를 스캔하여 해당 정보를 대체하거나 마스킹할 수 있습니다.
...
다음 방법에서는 [Microsoft Presidio](https://microsoft.github.io/presidio/)를 사용하여 PII 데이터를 완전히 제거합니다. Presidio는 PII를 마스킹하고 해당 PII 유형을 나타내는 플레이스홀더로 대체합니다. 예를 들어, Presidio는 `"My name is Alex"`에서 `Alex`를 `<PERSON>`으로 대체합니다.

Presidio는 [일반 엔티티](https://microsoft.github.io/presidio/supported_entities/)에 대해 기본적으로 지원을 제공합니다. 아래 예시에서는 `PHONE_NUMBER`, `PERSON`, `LOCATION`, `EMAIL_ADDRESS`, `US_SSN`인 모든 엔티티를 마스킹합니다. Presidio 처리 과정은 하나의 함수로 캡슐화되어 있습니다.
...
텍스트를 마스킹하는 대신, MS Presidio를 사용해 이름, 전화번호 같은 PII를 [Faker](https://faker.readthedocs.io/en/master/) Python 라이브러리가 생성한 가짜 데이터로 치환해 익명화할 수 있습니다. 예를 들어, 다음과 같은 데이터가 있다고 가정해 봅시다:
...
Presidio와 Faker를 효과적으로 함께 사용하려면, 사용자 정의 오퍼레이터(custom operator)에 대한 참조를 제공해야 합니다. 이러한 오퍼레이터는 Presidio가 PII를 가짜 데이터로 치환하는 데 사용할 적절한 Faker 함수로 연결되도록 합니다.
...
# 위에서 정의한 operators를 익명화 엔진에 반드시 전달하세요
...
코드를 하나의 클래스로 통합하고, 앞에서 식별한 추가 엔티티까지 포함하도록 목록을 확장해 봅시다.
...
`autopatch_settings`를 사용하면 지원되는 하나 이상의 LLM 통합에 대해 초기화 시점에 PII 처리 방식을 직접 구성할 수 있습니다. 이 방법의 장점은 다음과 같습니다:

1. PII 처리 로직이 초기화 단위로 중앙집중식 관리되어, 여기저기 흩어진 커스텀 로직의 필요성을 줄여 줍니다.
2. 특정 통합에 대해 PII 처리 워크플로를 맞춤 설정하거나 완전히 비활성화할 수 있습니다.

PII 처리를 구성하기 위해 `autopatch_settings`를 사용할 때에는, 지원되는 LLM 통합 중 하나의 `op_settings`에서 `postprocess_inputs` 및/또는 `postprocess_output`을 정의하면 됩니다.
...
다음 예제에서는 PII 마스킹 및 비식별화 메서드를 Weave Models에 통합하고, Weave Traces에서 결과를 미리 확인합니다.

먼저 [Weave Model](https://docs.wandb.ai/weave/guides/core-types/models)을 생성합니다. Weave Model은 구성 설정, 모델 가중치, 코드 등 모델이 어떻게 동작하는지를 정의하는 정보의 조합입니다.

이 모델에는 Anthropic API를 호출하는 `predict` 함수를 포함합니다. Anthropic의 Claude Sonnet은 [Traces](https://docs.wandb.ai/weave/quickstart)를 사용해 LLM 호출을 추적하면서 감성 분석을 수행하는 데 사용됩니다. Claude Sonnet은 텍스트 블록을 입력으로 받아 *positive*, *negative*, *neutral* 중 하나의 감성 분류를 출력합니다. 추가로, PII 데이터가 LLM으로 전송되기 전에 마스킹되거나 비식별화되도록 후처리 함수들도 포함합니다.

이 코드를 실행하면 Weave 프로젝트 페이지에 대한 링크와, 실행한 특정 trace(LLM 호출)에 대한 링크를 각각 받게 됩니다.

<div id="regex-method">
  ### 정규식 방식
</div>

가장 간단한 방법으로는 정규식을 사용해 원본 텍스트에서 PII 데이터를 식별하고 비식별화할 수 있습니다.
...
<div id="presidio-redaction-method">
  ### Presidio 마스킹(redaction) 방식
</div>

다음으로 Presidio를 사용해 원본 텍스트에서 PII 데이터를 식별하고 마스킹합니다.
...
이 예제에서는 Faker를 사용해 익명화된 대체 PII 데이터를 생성하고, Presidio를 사용해 원본 텍스트에서 PII 데이터를 식별하여 치환합니다.

<Frame>
  ![원본 텍스트, 식별된 PII, 익명화된 대체 값이 포함된 Faker 및 Presidio PII 교체 프로세스](/media/pii/replace.png)
</Frame>
...
다음 예제에서는 초기화 시 `anthropic`에 대한 `postprocess_inputs`를 `postprocess_inputs_regex()` 함수로 설정합니다. `postprocess_inputs_regex` 함수는 [방법 1: 정규식 필터링](#method-1-regular-expression-filtering)에 정의된 `redact_with_regex` 메서드를 적용합니다. 이제 `redact_with_regex`가 모든 `anthropic` 모델의 모든 입력에 적용됩니다.
...
PII를 익명화하는 것에 더해, `cryptography` 라이브러리의 [Fernet](https://cryptography.io/en/latest/fernet/) 대칭 암호화를 사용해 데이터에 추가 보안 계층을 적용할 수 있습니다. 이 방법을 사용하면 익명화된 데이터가 가로채더라도 암호화 키 없이는 내용을 읽을 수 없습니다.
