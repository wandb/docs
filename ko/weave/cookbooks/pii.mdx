---
title: "PII 데이터 처리"
description: "LLM 애플리케이션에서 W&B Weave의 데이터 마스킹 및 필터링 도구를 사용해 개인 식별 정보(PII)를 처리하세요."
---

<Note>
  대화형 노트북입니다. 로컬에서 실행하거나 아래 링크를 사용하세요:
...
이 가이드에서는 Personally Identifiable Information (PII) 데이터를 비공개로 유지하면서 W&amp;B Weave를 사용하는 방법을 알아봅니다. 이 가이드에서는 PII 데이터를 파악하고, 마스킹하고, 익명화하는 다음 방법을 설명합니다:

1. PII 데이터를 파악하고 마스킹하기 위한 **정규 표현식**
...
또한 *`weave.op` 입력/출력 로깅 사용자 지정* 및 *`autopatch_settings`* 를 사용하여 워크플로에 PII 마스킹 및 익명화를 통합하는 방법도 알아봅니다. 자세한 내용은 [로깅된 입력 및 출력 사용자 지정](/ko/weave/guides/tracking/ops/#customize-logged-inputs-and-outputs)을 참조하세요.

시작하려면 다음을 수행하세요:

1. [Overview](#overview) 섹션을 검토하세요.
2. [사전 요구 사항](#prerequisites)을 완료하세요.
3. PII 데이터를 파악하고, 마스킹하고, 익명화하는 데 사용할 수 있는 [사용 가능한 방법](#redaction-methods-overview)을 검토하세요.
4. [방법을 Weave Call에 적용하세요](#apply-the-methods-to-weave-calls).

<div id="overview">
  ## Overview
</div>

다음 섹션에서는 `weave.op`를 사용한 입력 및 출력 로깅의 Overview와, Weave에서 PII 데이터로 작업할 때의 모범 사례를 제공합니다.

<div id="customize-input-and-output-logging-using-weaveop">
  ### `weave.op`을 사용하여 입력 및 출력 로깅 사용자 지정하기
</div>

Weave Ops를 사용하면 입력 및 출력 후처리 함수를 정의할 수 있습니다. 이러한 함수를 사용하면 LLM 호출에 전달되거나 Weave에 로깅되는 데이터를 수정할 수 있습니다.

다음 예제에서는 두 개의 후처리 함수가 정의되며, `weave.op()`에 인수로 전달됩니다.
...
* PII 탐지를 확인하려면 익명화된 데이터를 로깅하세요
* Weave 트레이스로 PII 처리 과정을 추적하세요
* 실제 PII를 노출하지 않고도 익명화 성능을 측정하세요
...
* 되돌릴 필요가 없는 고유 ID에는 단방향 해싱을 적용하세요
...
!pip install weave                          # 트레이스를 활용하기 위해
...
PII 데이터를 탐지하고 보호하기 위해, 다음 방법을 사용하여 PII 데이터를 파악하고 마스킹하며 선택적으로 익명화합니다:
...
<div id="method-1-filter-using-regular-expressions">
  ### 방법 1: 정규 표현식을 사용한 Filter
</div>

[정규 표현식(regex)](https://docs.python.org/3/library/re.html)은 PII 데이터를 파악하고 마스킹하기 위한 가장 단순한 방법입니다. Regex를 사용하면 전화번호, 이메일 주소, 주민등록번호와 같은 다양한 형식의 민감한 정보와 일치하는 패턴을 정의할 수 있습니다. Regex를 사용하면 더 복잡한 NLP 기법이 없어도 대량의 텍스트를 훑어보며 정보를 대체하거나 마스킹할 수 있습니다.
...
def redact_with_regex(text):
    ...
    # 이메일 패턴
    # \b         : 단어 경계
    # [A-Za-z0-9._%+-]+ : 이메일 사용자 이름에 사용할 수 있는 하나 이상의 문자
    # @          : 리터럴 @ 기호
    # [A-Za-z0-9.-]+ : 도메인 이름에 사용할 수 있는 하나 이상의 문자
    # \.         : 리터럴 점
    # [A-Z|a-z]{2,} : 두 개 이상의 대문자 또는 소문자 (TLD)
    # \b         : 단어 경계
    text = re.sub(
...
<div id="method-2-redact-using-microsoft-presidio">
  ### 방법 2: Microsoft Presidio를 사용해 마스킹하기
</div>

다음 방법은 [Microsoft Presidio](https://microsoft.github.io/presidio/)를 사용하여 PII 데이터를 완전히 제거하는 것입니다. Presidio는 PII를 마스킹하고, 이를 PII 유형을 나타내는 플레이스홀더로 대체합니다. 예를 들어 Presidio는 `"My name is Alex"`에서 `Alex`를 `<PERSON>`으로 바꿉니다.

Presidio는 [일반적인 엔터티](https://microsoft.github.io/presidio/supported_entities/)를 기본으로 지원합니다. 아래 예제에서는 `PHONE_NUMBER`, `PERSON`, `LOCATION`, `EMAIL_ADDRESS`, `US_SSN`에 해당하는 모든 엔터티를 마스킹합니다. Presidio 프로세스는 함수로 캡슐화되어 있습니다.
...
텍스트를 redaction하는 대신, [Faker](https://faker.readthedocs.io/en/master/) Python 라이브러리로 생성한 가짜 데이터를 사용해 이름이나 전화번호 같은 PII를 바꾸도록 MS Presidio를 사용해 익명화할 수 있습니다. 예를 들어, 다음과 같은 데이터가 있다고 가정해 보겠습니다.

`"My name is Raphael and I like to fish. My phone number is 212-555-5555"`

Presidio와 Faker를 사용해 데이터를 처리하고 나면 다음과 같이 보일 수 있습니다.

`"My name is Katherine Dixon and I like to fish. My phone number is 667.431.7379"`

Presidio와 Faker를 함께 효과적으로 사용하려면 맞춤형 Operator에 대한 레퍼런스를 제공해야 합니다. 이러한 Operator는 PII를 가짜 데이터로 바꾸는 역할을 하는 Faker 함수로 Presidio를 안내합니다.
...
# 위에서 정의한 Operator를 anonymizer에 전달하는 것을 잊지 마세요
...
코드를 하나의 클래스로 통합하고, 앞서 식별한 추가 엔터티를 포함하도록 엔터티 목록을 확장해 보겠습니다.
...
class MyFaker(Faker):
    # faker 함수 생성 (값을 받아야 한다는 점에 유의하세요)
...
하나 이상의 지원되는 LLM 인테그레이션에 대해 초기화 중에 직접 PII 처리를 설정하려면 `autopatch_settings`를 사용할 수 있습니다. 이 방법의 장점은 다음과 같습니다.

1. PII 처리 로직이 초기화 시점에 중앙에서 관리되고 범위가 지정되므로, 여기저기 흩어진 맞춤형 로직의 필요성이 줄어듭니다.
2. 특정 인테그레이션에 대해 PII 처리 워크플로를 사용자 지정하거나 완전히 비활성화할 수 있습니다.

`autopatch_settings`를 사용하여 PII 처리를 설정하려면, 지원되는 LLM 인테그레이션 중 하나의 `op_settings`에서 `postprocess_inputs` 및/또는 `postprocess_output`을 정의하세요.
...
<div id="apply-the-methods-to-weave-calls">
  ## 방법을 Weave Call에 적용하기
</div>

다음 예제에서는 PII 마스킹 및 익명화 방법을 Weave Models에 통합하고, 결과를 Weave 트레이스에서 미리 확인합니다.

먼저 [Weave Model](https://docs.wandb.ai/weave/guides/core-types/models)을 생성하겠습니다. Weave Model은 설정값, 모델 가중치, 그리고 모델의 동작 방식을 정의하는 코드 같은 정보의 조합입니다.

모델에는 Anthropic API를 호출하는 `predict` 함수를 포함합니다. Anthropic의 Claude Sonnet은 [Traces](https://docs.wandb.ai/weave/quickstart)를 사용해 LLM Call을 트레이스하면서 감성 분석을 수행하는 데 사용됩니다. Claude Sonnet은 텍스트 블록을 입력으로 받아 다음 감성 분류 중 하나를 출력합니다: *positive*, *negative* 또는 *neutral*. 또한 PII 데이터가 LLM으로 전송되기 전에 마스킹되거나 익명화되도록 후처리 함수도 포함합니다.

이 코드를 실행하면 Weave 프로젝트 페이지 링크와 함께, 실행한 특정 트레이스(LLM Call)에 대한 링크도 받게 됩니다.

<div id="regex-method">
  ### 정규식 방법
</div>

가장 단순한 경우에는 정규식을 사용해 원본 텍스트의 PII 데이터를 파악하고 마스킹할 수 있습니다.
...
# 모델 예측 Weave Op에 regex 마스킹을 적용하는 입력 후처리 함수를 정의합니다
...
다음으로, Presidio를 사용해 원본 텍스트에서 PII 데이터를 파악하고 마스킹하겠습니다.

<Frame>
  ![식별된 PII 엔터티와 마스킹된 텍스트 출력이 포함된 Presidio PII 마스킹 프로세스](/media/pii/redact.png)
</Frame>
...
이 예제에서는 Faker를 사용해 익명화된 대체 PII 데이터를 생성하고, Presidio를 사용해 원본 텍스트의 PII 데이터를 파악하고 대체합니다.

<Frame>
  ![원본 텍스트, 파악된 PII, 익명화된 대체 값이 포함된 Faker 및 Presidio PII 대체 프로세스](/media/pii/replace.png)
</Frame>
...
model = SentimentAnalysisFakerPiiModel(
    name="claude-3-sonnet",
    model_name="claude-3-5-sonnet-20240620",
    system_prompt='당신은 감성 분석 분류기입니다. 텍스트를 감성을 기준으로 분류합니다. 입력은 텍스트 블록입니다. 다음 평가 옵션 중 하나로 답하세요["positive", "negative", "neutral"]. 답변은 json 형식의 한 단어여야 합니다: {classification}. 유효한 JSON인지 확인하세요.',
    temperature=0,
)
...
다음 예제에서는 초기화할 때 `anthropic`의 `postprocess_inputs`를 `postprocess_inputs_regex()` 함수()로 설정합니다. `postprocess_inputs_regex` 함수는 [방법 1: 정규식 필터링](#method-1-regular-expression-filtering)에 정의된 `redact_with_regex` 방법을 적용합니다. 이제 모든 `anthropic` 모델의 모든 입력에 `redact_with_regex`가 적용됩니다.
...
<Frame>
  ![암호화된 텍스트 출력과 암호화 키 관리가 포함된 PII 데이터 암호화 프로세스](/media/pii/encrypt.png)
</Frame>

PII를 익명화하는 것에 더해, cryptography 라이브러리의 [Fernet](https://cryptography.io/en/latest/fernet/) 대칭 암호화를 사용해 데이터에 보안 계층을 한 겹 더 추가할 수 있습니다. 이 방식은 익명화된 데이터가 가로채이더라도 암호화 키가 없으면 내용을 읽을 수 없도록 보장합니다.
...
# 새로운 EncryptedSentimentAnalysisInput을 사용하도록 sentiment_analysis_model을 수정합니다
class sentiment_analysis_model(weave.Model):
    ...
    async def predict(self, encrypted_input: EncryptedSentimentAnalysisInput) -> dict:
        ...
        decrypted_text = encrypted_input.decrypt() # 맞춤형 클래스를 사용하여 텍스트를 복호화합니다
