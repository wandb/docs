---
title: "Weave로 오디오 사용하기"
description: "W&B Weave에서 오디오를 사용하는 방법을 알아보세요"
---

<Note>
이 노트북은 인터랙티브 노트북입니다. 로컬에서 실행하거나 아래 링크를 사용하세요:
...
이 데모에서는 OpenAI Chat Completions API와 GPT-4o Audio Preview를 사용하여 텍스트 프롬프트에 대한 오디오 응답을 생성하고, 이를 Weave에서 추적합니다.

<img src="https://i.imgur.com/OUfsZ2x.png" alt="GPT-4o Audio Preview 통합 및 오디오 응답 생성 워크플로가 포함된 OpenAI Chat Completions API 인터페이스" />

고급 예제에서는 OpenAI Realtime API를 활용해 오디오를 실시간으로 스트리밍합니다. 비디오 데모를 보려면 아래 썸네일을 클릭하거나, [여기](https://www.youtube.com/watch?v=lnnd73xDElw)를 클릭하세요.
...
먼저 OpenAI(`openai`)와 Weave(`weave`) 의존성을 설치하고, API 키 관리를 위한 `set-env` 의존성도 함께 설치합니다.
...
다음으로 OpenAI와 Weave에 필요한 API 키를 로드합니다. 여기서는 Google Colab의 시크릿 키 매니저와 호환되는 set&#95;env를 사용하며, Colab의 전용 기능인 `google.colab.userdata`의 대안으로 사용할 수 있습니다. 사용 방법은 [여기](https://pypi.org/project/set-env-colab-kaggle-dotenv/)를 참고하세요.
...
이제 오디오 모달리티를 활성화한 상태에서 OpenAI completions 엔드포인트를 호출하도록 설정합니다. 먼저 OpenAI 클라이언트를 생성하고 Weave 프로젝트를 초기화합니다.
...
여기에서 `prompt_endpont_and_log_trace` 함수를 정의합니다. 이 함수는 세 가지 주요 단계를 가집니다:

1. 텍스트와 오디오 입력 및 출력을 지원하는 `GPT 4o Audio Preview` 모델을 사용해서 completion 객체를 만듭니다.

   * 모델에 다양한 억양으로 천천히 13까지 세도록 프롬프트를 줍니다.
   * completion을 &quot;stream&quot;으로 설정합니다.

2. 스트리밍된 데이터가 청크 단위로 기록되도록 새 출력 파일을 엽니다.

3. Weave가 트레이스에서 오디오 데이터를 로깅할 수 있도록 오디오 파일에 대한 열린 파일 핸들(파일 핸들러)을 반환합니다.
...
다음 셀을 실행하세요. 시스템 프롬프트와 사용자 프롬프트, 그리고 출력 오디오는 Weave 트레이스에 함께 저장됩니다.
셀을 실행한 후, &quot;🍩&quot; 이모지 옆의 링크를 클릭해서 트레이스를 확인하세요.
...
<img src="https://i.imgur.com/ZiW3IVu.png" alt="Weave와 Realtime Audio API 통합 및 스트리밍 오디오 대화 인터페이스" />

OpenAI의 Realtime API는 실시간 오디오 및 텍스트 어시스턴트를 구축하기 위한 고기능·고신뢰성 대화형 API입니다.

다음 사항을 참고하세요:

- [Microphone Configuration](#microphone-configuration)의 셀을 검토하세요.
- Google Colab 실행 환경의 한계로 인해, **반드시 호스트 머신에서** Jupyter Notebook으로 실행해야 합니다. 브라우저에서 직접 실행할 수 없습니다.
  - MacOS에서는 Pyaudio가 동작하도록 `portaudio`를 Brew로 설치해야 합니다([여기](https://formulae.brew.sh/formula/portaudio) 참고).
- OpenAI의 Python SDK는 아직 Realtime API를 지원하지 않습니다. 가독성을 높이기 위해 Pydantic에서 전체 OAI Realtime API 스키마를 구현했으며, 공식 지원이 출시되면 이 구현은 폐기(deprecate)될 수 있습니다.
- `enable_audio_playback` 토글을 활성화하면 어시스턴트가 출력한 오디오가 재생됩니다. **이 기능을 활성화한 경우 반드시 헤드폰을 사용해야** 하며, 에코 감지에는 매우 복잡한 구현이 필요하다는 점에 유의하세요.
...
다음 셀을 실행해 사용 가능한 모든 오디오 장치를 확인하세요. 그런 다음 표시된 장치 목록을 기준으로 `INPUT_DEVICE_INDEX`와 `OUTPUT_DEVICE_INDEX` 값을 설정하세요. 입력 장치는 최소 1개 이상의 입력 채널이 있어야 하고, 출력 장치는 최소 1개 이상의 출력 채널이 있어야 합니다.
...
enable_audio_playback = True  # @param {type:"boolean"}                           # 어시스턴트 오디오 재생을 켭니다. 헤드폰이 필요합니다.
...
OpenAI Python SDK는 아직 Realtime API를 지원하지 않습니다. 우리는 가독성을 높이기 위해 전체 OAI Realtime API 스키마를 Pydantic으로 구현했으며, 공식 지원이 제공되면 이 구현은 사용 중단(Deprecated)될 수 있습니다.
...
def parse_server_event(event_data: dict) -> ServerEvent:
    ...
    if not event_type:
        raise ValueError("이벤트 데이터에 'type' 필드가 없습니다")

    model_class = EVENT_TYPE_TO_MODEL.get(event_type)
    if not model_class:
        raise ValueError(f"알 수 없는 이벤트 유형: {event_type}")

    try:
        return model_class(**event_data)
    except ValidationError as e:
        raise ValueError(f"{event_type} 유형의 이벤트를 파싱하는 데 실패했습니다: {str(e)}") from e
```

<div id="audio-stream-writer-to-disk-and-in-memory">
  ## 오디오 스트림 기록기(디스크 및 메모리 기록)
</div>
...
class StreamingWavWriter:
    ...
    def get_wav_buffer(self):
        assert self.in_memory, "버퍼는 스트림이 메모리 내에 있을 때만 사용할 수 있습니다."
        return self.buffer
```
...
실시간(RT) 오디오 모델은 WebSocket을 사용해 이벤트를 OpenAI의 Realtime 오디오 API로 전송합니다. 동작 방식은 다음과 같습니다:

1.  **init:** 로컬 버퍼(입력 오디오)와 스트림(assistant 재생 스트림, 사용자 오디오 디스크 기록 스트림)을 초기화하고 Realtime API에 대한 연결을 엽니다.
2.  **receive_messages_thread**: 하나의 스레드가 API로부터 메시지를 수신합니다. 네 가지 주요 이벤트 타입을 처리합니다: - RESPONSE_AUDIO_TRANSCRIPT_DONE:

            서버는 assistant의 응답이 완료되었음을 나타내고, 해당 전사본(transcript)을 제공합니다.

        - CONVERSATION_ITEM_INPUT_AUDIO_TRANSCRIPTION_COMPLETED:

            서버는 사용자의 오디오가 전사되었음을 나타내고, 사용자의 오디오 전사본을 전송합니다. 이 전사본을 Weave에 기록(log)하고, 사용자에게 출력합니다.

        - RESPONSE_AUDIO_DELTA:

            서버는 assistant 응답 오디오의 새로운 청크를 전송합니다. 이 청크를 response ID를 기준으로 진행 중인 응답 데이터에 이어 붙이고, 재생을 위해 출력 스트림에 추가합니다.

        - RESPONSE_DONE:

            서버는 assistant 응답이 완료되었음을 나타냅니다. 해당 응답과 연관된 모든 오디오 청크와 전사본을 가져와 Weave에 기록합니다.

    3.**send_audio**: 핸들러는 사용자 오디오 청크를 버퍼에 추가하고, 오디오 버퍼가 특정 크기에 도달하면 오디오 청크를 전송합니다.
...
class RTAudioModel(weave.Model):
    ...
    ##### 일반 유틸리티 함수 #####
    def log_ws_message(self, message, direction="수신"):
...
`RTAudio` 모델의 `send_audio` 메서드에 연결된 핸들러가 있는 `pyaudio` 입력 스트림을 사용합니다. 이 스트림은 프로그램 종료 시 안전하게 종료할 수 있도록 메인 스레드로 반환됩니다.
...
메인 스레드는 Weave와 통합된 실시간 오디오 모델을 초기화합니다. 이후 녹음을 시작하고, 사용자가 키보드 인터럽트를 입력할 때까지 기다립니다.
...
if realtime_model.ws and realtime_model.ws.connected:
    ...
    print(
        "WebSocket 연결에 실패했습니다. API 키와 인터넷 연결을 확인하세요."
    )
```
