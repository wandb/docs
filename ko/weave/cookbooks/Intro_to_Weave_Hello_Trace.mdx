...
아래 코드 샘플은 `@weave.op` 데코레이터를 사용해 Weave에서 트레이스를 캡처하고 시각화하는 방법을 보여줍니다. 이 샘플은 `extract_fruit`라는 함수를 정의하고, 문장에서 구조화된 데이터(과일, 색, 맛)를 추출하기 위해 OpenAI의 GPT-4o에 프롬프트를 보냅니다. 함수에 `@weave.op` 데코레이터를 적용하면 Weave가 입력, 출력, 중간 과정 등을 포함한 함수 실행을 자동으로 추적합니다. 이 함수를 예시 문장으로 호출하면 전체 트레이스가 저장되며 Weave UI에서 확인할 수 있습니다.
