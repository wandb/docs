...
<div id="custom-routing-for-llm-prompts-with-not-diamond">
  # Not Diamond를 활용한 LLM 프롬프트 맞춤형 라우팅
</div>

이 노트북에서는 Weave를 [Not Diamond의 맞춤형 라우팅](https://docs.notdiamond.ai/docs/router-training-quickstart)과 함께 사용하여 평가 결과를 기준으로 LLM 프롬프트를 가장 적합한 모델로 라우팅하는 방법을 보여줍니다.
...
프롬프트용 맞춤형 라우터를 트레이닝하려면 세 가지가 필요합니다:

1. LLM 프롬프트 집합: 프롬프트는 문자열이어야 하며, 애플리케이션에서 사용하는 프롬프트를 대표할 수 있어야 합니다.
2. LLM 응답: 각 입력에 대한 후보 LLM의 응답입니다. 후보 LLM에는 지원되는 LLM과 자체 맞춤형 모델이 모두 포함될 수 있습니다.
3. 후보 LLM의 입력별 응답에 대한 평가 점수: 점수는 숫자이며, 필요에 맞는 어떤 메트릭이든 사용할 수 있습니다.
...
실제로는 자체 Evaluations를 사용해 맞춤형 라우터를 트레이닝합니다. 하지만 이 예시 노트북에서는 코딩 작업용 맞춤형 라우터를 트레이닝하기 위해
[HumanEval 데이터셋](https://github.com/openai/human-eval)에 대한 LLM 응답을 사용합니다.

먼저 이 예제를 위해 미리 준비한 데이터셋을 다운로드한 다음, 각 모델의 LLM 응답을 EvaluationResults로 파싱합니다.
...
이제 EvaluationResults가 있으므로 맞춤형 라우터를 트레이닝할 수 있습니다. [계정을 생성](https://app.notdiamond.ai/keys)하고
...
preference_id = train_router(
    ...
    maximize=True,
    api_key=api_key,
    # 첫 번째 맞춤형 라우터를 트레이닝할 때는 이 줄을 주석 처리된 상태로 두세요
    # 맞춤형 라우터를 재트레이닝하려면 주석을 해제하세요
    # preference_id=preference_id,
)
```

그런 다음 Not Diamond 앱에서 맞춤형 라우터의 트레이닝 진행 과정을 확인할 수 있습니다.
...
맞춤형 라우터 트레이닝을 마친 뒤에는 다음 두 가지 방식으로 성능을 평가할 수 있습니다.

* 트레이닝 프롬프트를 제출해 in-sample 성능을 평가하거나
* 새로운 프롬프트 또는 홀드아웃된 프롬프트를 제출해 out-of-sample 성능을 평가할 수 있습니다.
...
Weave를 통해 맞춤형 라우터를 트레이닝하면 자동으로 평가가 실행되고 결과가 Weave UI에 업로드됩니다. 맞춤형 라우터 프로세스가 완료되면 Weave UI에서 결과를 검토할 수 있습니다.
