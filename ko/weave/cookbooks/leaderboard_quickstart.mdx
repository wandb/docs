---
title: "리더보드 빠른 시작"
description: "W&B Weave에서 리더보드 빠른 시작을 사용하는 방법을 알아봅니다"
---

<Note>
이 노트북은 대화형입니다. 로컬에서 실행하거나 아래 링크를 사용해 보세요:
...
이 노트북에서는 Weave 리더보드를 사용하여 서로 다른 데이터셋과 스코어링 함수에서 모델 성능을 비교하는 방법을 알아봅니다. 구체적으로 다음 작업을 수행합니다:

1. 가짜 우편번호 데이터셋을 생성합니다.
2. 몇 가지 스코어링 함수를 작성하고 기준(baseline) 모델을 평가합니다.
3. 이러한 기법을 사용해 모델과 Evaluation 조합의 매트릭스를 평가합니다.
4. Weave UI에서 리더보드를 확인합니다.

<div id="step-1-generate-a-dataset-of-fake-zip-code-data">
  ## 1단계: 가짜 우편번호 데이터셋 생성
</div>

먼저 가짜 우편번호 데이터셋의 행 목록을 생성하는 함수 `generate_dataset_rows`를 정의합니다.
...
다음으로 세 가지 스코어링 함수를 작성합니다:

1. `check_concrete_fields`: 모델 출력이 예상된 도시와 주와 일치하는지 확인합니다.
2. `check_value_fields`: 모델 출력이 예상 인구와 중위 소득의 10% 이내인지 확인합니다.
3. `check_subjective_fields`: LLM을 사용해 모델 출력이 예상된 &quot;known for&quot; 필드와 일치하는지 확인합니다.
...
<div id="step-3-create-a-simple-evaluation">
  ## 3단계: 간단한 Evaluation 생성
</div>

다음으로, 예시 데이터와 스코어링 함수들을 사용해 간단한 Evaluation을 정의합니다.
...
이제 항상 동일한 응답을 반환하는 베이스라인 모델을 평가합니다.
...
이제 베이스라인과 비교하기 위해 모델 두 개를 더 생성합니다.
...
<div id="step-6-create-more-evaluations">
  ## 6단계: Evaluation 더 생성하기
</div>

이제 모델과 Evaluation의 조합(매트릭스)을 평가해 보겠습니다.
...
Python 코드에서 바로 리더보드를 생성할 수도 있습니다:
