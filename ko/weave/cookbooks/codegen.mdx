...
keywords: ["HumanEval", "구조화된 출력", "Pydantic", "코드 생성", "OpenAI"]
---
...
<Frame>
  ![코드 생성 Runs를 비교하는 Weave 평가 대시보드](/media/codegen/eval_dash.png)
</Frame>

## 왜 Weave를 사용해야 하나요 {#why-use-weave}
...
## 구조화된 출력과 Pydantic 모델 {#structured-outputs-and-pydantic-models}
...
## 코드 포매터 구현 {#implement-a-code-formatter}
...
## `CodeGenerationPipeline` 정의 {#define-the-codegenerationpipeline}

<Frame>
  ![코드 생성 파이프라인 run의 Weave 트레이스](/media/codegen/codegen_trace.png)
</Frame>
...
<Frame>
  ![생성된 코드를 평가하는 HumanEval Scorer의 Weave 트레이스](/media/codegen/eval_trace.png)
</Frame>

## Weave 데이터셋 만들기 및 평가 실행 {#create-a-weave-dataset-and-run-evaluation}
...
<Frame>
  ![HumanEval Scorer 결과를 보여주는 Weave 평가 대시보드](/media/codegen/eval_dash.png)
</Frame>
