---
title: "Codegen"
description: "W&B Weave를 사용하여 코드 생성 파이프라인을 구축하고 평가하며, 프롬프트, 출력, 품질 메트릭을 트레이스합니다."
---
...
적절한 구조, 문서화, 테스트를 갖춘 고품질 코드를 생성하는 것은 어려운 작업입니다. 이 가이드는 코드 생성 파이프라인을 구현하는 방법을 안내합니다. HumanEval 테스트 스위트를 대상으로 고품질 Python 함수를 생성하는 코드 생성 파이프라인을 만드는 방법을 배울 수 있습니다.

이 가이드에서는 평가 비교 및 추적을 위해 Weave를 사용하고, 구조화된 출력을 활용한 코드 생성을 위해 OpenAI의 GPT 모델을 사용합니다.
...
이 튜토리얼에서는 Weave를 사용하여 코드 생성 파이프라인을 구현하고 평가합니다. 다음 내용을 학습합니다:

1. **LLM 파이프라인 추적**: 코드 생성 프로세스의 inputs, outputs, 중간 step을 log합니다.
2. **LLM 출력 평가**: 풍부한 디버깅 도구와 시각화를 활용하여 생성된 코드의 evaluations를 생성하고 비교합니다.
...
<Note>
  Weave는 inputs, outputs 및 메타데이터를 포함하여 OpenAI API Call을 자동으로 추적합니다. 즉, OpenAI 상호작용에 대한 추가적인 로깅 코드를 작성할 필요가 없습니다. Weave가 백그라운드에서 원활하게 처리합니다.
</Note>
...
이 코드 생성 파이프라인에서는 OpenAI의 [구조화된 출력 모드](https://platform.openai.com/docs/guides/structured-outputs)와 Pydantic 모델을 활용하여 언어 모델로부터 일관되고 형식이 잘 갖춰진 Response를 보장합니다. 이 접근 방식은 다음과 같은 여러 장점을 제공합니다:

1. **유형 안전성**: 예상 출력에 대한 Pydantic 모델을 정의함으로써 생성된 코드, 프로그램 실행기, 단위 테스트에 엄격한 구조를 강제 적용합니다.
2. **간편한 파싱**: 구조화된 출력 모드를 사용하면 모델의 Response를 미리 정의된 Pydantic 모델로 직접 파싱할 수 있어 복잡한 후처리 작업이 줄어듭니다.
3. **향상된 신뢰성**: 예상하는 정확한 형식을 지정함으로써 언어 모델에서 예기치 않거나 잘못된 형식의 출력이 발생할 가능성을 줄입니다.

다음은 Pydantic 모델을 정의하고 OpenAI의 구조화된 출력과 함께 사용하는 방법의 예시입니다:
...
일관되고 깔끔한 코드 출력을 보장하기 위해 Weave 오퍼레이션을 사용하여 `CodeFormatter` 클래스를 구현합니다. 이 포매터는 생성된 코드, 프로그램 실행기, 단위 테스트에 다양한 린팅 및 스타일링 규칙을 적용합니다.
...
이 `CodeFormatter` 클래스는 생성된 코드를 정리하고 포맷하기 위한 여러 Weave 오퍼레이션을 제공합니다:
...
`weave.Model`을 사용하면 변경 시 자동으로 버전이 관리됩니다. 또한 `model_name`을 속성으로 유지하여 실험하고 Weave에서 손쉽게 비교 및 차이를 확인할 수 있습니다. `@weave.op`로 함수 호출을 추적하면 입력 및 출력이 로깅되어 오류 추적 및 디버깅에 도움이 됩니다.
    ...
    def generate_code(self, prompt: str) -> GeneratedCode:
        completion = client.beta.chat.completions.parse(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert Python code generator.",  # 당신은 전문 Python 코드 생성기입니다.
                },
...
1. 자동 실험 추적: Weave는 모델의 각 실행에 대한 입력, 출력 및 파라미터를 캡처합니다.
2. 버전 관리: 모델의 속성이나 코드 변경 사항은 자동으로 버전이 지정되어, 코드 생성 파이프라인이 시간에 따라 어떻게 발전하는지에 대한 명확한 이력을 생성합니다.
3. 재현성: 버전 관리 및 추적 기능을 통해 코드 생성 파이프라인의 이전 결과나 설정을 쉽게 재현할 수 있습니다.
4. 하이퍼파라미터 관리: 모델 속성(예: `model_name`)이 명확하게 정의되고 여러 실행에 걸쳐 추적되어 실험을 용이하게 합니다.
5. Weave 에코시스템과의 인테그레이션: `weave.Model`을 사용하면 평가 및 서빙 기능과 같은 다른 Weave 도구와 원활하게 인테그레이션할 수 있습니다.

<div id="implement-evaluation-metrics">
  ## 평가 메트릭 구현
</div>

생성된 코드의 품질을 평가하기 위해 `weave.Scorer` 서브클래스를 사용하여 간단한 평가 메트릭을 구현합니다. 이를 통해 데이터셋의 모든 `model_output`에 대해 `score`가 실행됩니다. `model_output`은 `weave.Model`의 `predict` 함수 출력에서 가져오며, `prompt`는 데이터셋 `human-eval`에서 가져옵니다.
...
이 평가 함수들은 생성된 코드를 실행하고, 데이터셋에서 제공된 테스트를 코드가 통과했는지 여부를 나타내는 불리언 값을 반환합니다.
...
파이프라인을 평가하기 위해 Weave 데이터셋을 생성하고 평가를 실행합니다.
...
이 예제에서는 Weave와 OpenAI의 언어 모델을 사용하여 코드 생성 파이프라인을 구현하는 방법을 살펴보았습니다. 다음 내용을 다루었습니다:

1. 코드 생성 프로세스의 각 step에 대한 Weave 오퍼레이션 생성
2. 손쉬운 추적 및 평가를 위해 파이프라인을 Weave Model로 래핑
3. Weave 오퍼레이션을 사용한 맞춤형 평가 메트릭 구현
4. 데이터셋 생성 및 파이프라인 평가 실행

Weave의 원활한 인테그레이션을 통해 코드 생성 프로세스 전반에 걸쳐 입력, 출력, 중간 step을 추적할 수 있어 LLM 애플리케이션의 디버깅, 최적화, 평가가 더욱 용이해집니다.

Weave 및 그 기능에 대한 자세한 내용은 [Weave 문서](/ko/weave)를 참조하십시오. 이 예제를 확장하여 더 큰 데이터셋을 처리하거나, 더 정교한 평가 메트릭을 구현하거나, 다른 LLM 워크플로와 통합할 수 있습니다.