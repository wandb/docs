...
<Note>
  이 노트북은 대화형 노트북입니다. 로컬에서 실행하거나 아래 링크를 이용할 수 있습니다:
...
핵심 세부 사항을 유지하면서 복잡한 기술 문서를 요약하는 것은 쉽지 않은 작업입니다. Chain of Density(CoD) 요약 기법은 요약을 반복적으로 다듬어 더 간결하면서도 정보 밀도가 높게 만드는 방식으로 이 문제를 해결합니다. 이 가이드에서는 애플리케이션을 추적하고 평가하기 위해 Weave로 CoD를 구현하는 방법을 보여줍니다.
...
이 접근 방식은 세부 정보를 보존하는 것이 중요한 과학 논문이나 기술 문서를 요약할 때 특히 유용합니다.

<div id="why-use-weave">
  ## 왜 Weave를 사용하나요?
</div>

이 튜토리얼에서는 Weave를 사용해 ArXiv 논문용 Chain of Density 요약 파이프라인을 구현하고 평가합니다. 다음 내용을 배우게 됩니다.

1. **LLM 파이프라인 추적**: Weave를 사용해 요약 과정의 입력, 출력, 중간 step을 자동으로 기록합니다.
2. **LLM 출력 평가**: Weave의 기본 제공 도구를 사용해 요약을 엄격하고 일관된 기준으로 평가합니다.
3. **조합 가능한 오퍼레이션 구축**: 요약 파이프라인의 여러 부분에서 Weave 오퍼레이션을 조합하고 재사용합니다.
4. **원활하게 통합하기**: 기존 Python 코드에 Weave를 최소한의 오버헤드로 손쉽게 추가합니다.
...
실험을 추적하기 위해 Weave를 사용하고, 텍스트 생성을 위해 Anthropic의 Claude 모델을 사용합니다. `weave.init(<project name>)` 호출은 요약 작업을 위한 새 Weave 프로젝트를 설정합니다.
...
이 클래스는 ArXiv 논문의 메타데이터와 본문을 캡슐화하며, 요약 파이프라인의 입력으로 사용됩니다.
...
논문 전체 내용으로 작업할 수 있도록 PDF에서 텍스트를 불러오고 추출하는 함수를 추가하겠습니다:
...
`@weave.op()` 데코레이터를 사용하면 Weave가 이 함수들의 입력, 출력, 실행 과정을 추적할 수 있습니다.
...
이제 요약 파이프라인을 Weave Model로 감싸겠습니다:
...
2. 버전 관리: 모델의 속성이나 코드 변경 사항이 자동으로 버전 관리되므로, 시간이 지남에 따라 요약 파이프라인이 어떻게 발전해 왔는지 명확한 이력을 남길 수 있습니다.
3. 재현성: 버전 관리와 추적 덕분에 요약 파이프라인의 이전 결과나 설정을 쉽게 재현할 수 있습니다.
4. 하이퍼파라미터 관리: `model` 및 `density_iterations` 같은 모델 속성이 서로 다른 run 전반에서 명확하게 정의되고 추적되므로, 실험을 더 쉽게 수행할 수 있습니다.
5. Weave 생태계와의 인테그레이션: `weave.Model`을 사용하면 evaluations 및 서빙 기능과 같은 다른 Weave 도구와 매끄럽게 인테그레이션할 수 있습니다.
...
이 평가 함수들은 관련성, 간결성, 기술적 정확성을 기준으로 생성된 요약의 품질을 평가하기 위해 Claude 모델을 사용합니다.
...
파이프라인을 평가하기 위해 Weave 데이터셋을 만들고 평가를 실행합니다:
...
이번 평가에서는 LLM-as-a-judge 접근 방식을 사용합니다. 이 기법은 언어 모델을 사용해 다른 모델이나 시스템이 생성한 출력물의 품질을 평가하는 방식입니다. 특히 기존 메트릭으로는 충분히 평가하기 어려운 작업에서, LLM의 이해력과 추론 능력을 활용해 더 정교한 평가를 제공합니다.
...
이 코드는 샘플 ArXiv 논문으로 데이터셋을 만들고, 품질 평가기를 정의한 다음, 요약 파이프라인을 평가합니다.
...
이 예제에서는 Weave를 사용해 ArXiv 논문용 Chain of Density 요약 파이프라인을 구현하는 방법을 살펴보았습니다. 구체적으로 다음을 보여주었습니다.

1. 요약 프로세스의 각 step에 대한 Weave 오퍼레이션 생성
2. 손쉽게 추적하고 평가할 수 있도록 파이프라인을 Weave Model로 래핑
3. Weave 오퍼레이션을 사용한 맞춤형 평가 메트릭 구현
4. 데이터셋을 생성하고 파이프라인 평가 실행

Weave의 원활한 인테그레이션을 통해 요약 프로세스 전반에서 입력, 출력, 중간 step을 추적할 수 있으므로 LLM 애플리케이션을 더 쉽게 디버그하고, 최적화하고, 평가할 수 있습니다.
