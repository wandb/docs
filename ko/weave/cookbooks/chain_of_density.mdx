...
<Frame>
![Chain of Density 요약 결과, 메트릭 및 성능 비교가 포함된 Weave 평가 대시보드](/media/summarization/eval_dash.png)
</Frame>
...
전체 논문 내용을 다루기 위해 PDF에서 텍스트를 불러오고 추출하는 함수를 추가합니다:
...
이제 Weave 연산을 사용해 CoD 요약의 핵심 로직을 구현해 보겠습니다:
...
<Frame>
  ![모델 설정과 파라미터가 표시된 Chain of Density 요약용 Weave Model 설정 인터페이스](/media/summarization/model.png)
</Frame>
...
요약의 품질을 평가하기 위해 간단한 평가 메트릭을 구현하겠습니다:
...
이러한 평가 함수는 Claude 모델을 사용해 관련성, 간결성, 기술적 정확성을 기준으로 생성된 요약의 품질을 평가합니다.
...
파이프라인을 평가하기 위해 Weave Dataset을 만들고 평가를 실행해 보겠습니다.

<Frame>
  ![데이터셋 선택 및 설정 옵션이 있는 평가용 Weave Dataset 설정 인터페이스](/media/summarization/dataset.png)
</Frame>
...
평가에는 LLM-as-a-judge 접근 방식을 사용하겠습니다. 이 기법은 한 언어 모델을 사용해 다른 모델이나 시스템이 생성한 출력의 품질을 평가하는 방식입니다. 특히 기존 메트릭만으로는 충분하지 않을 수 있는 작업에서 LLM의 이해력과 추론 능력을 활용해 더 세밀한 평가를 제공합니다.
...
이 코드는 샘플 ArXiv 논문을 사용해 데이터셋을 생성하고, 품질 스코어러를 정의한 다음, 요약 파이프라인을 평가합니다.
...
Weave의 매끄러운 통합을 통해 요약 프로세스 전반의 입력, 출력, 중간 단계를 추적할 수 있으므로 LLM 애플리케이션을 더 쉽게 디버깅하고, 최적화하고, 평가할 수 있습니다.
