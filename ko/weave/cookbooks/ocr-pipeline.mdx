...
[`@weave.op()` 데코레이터](/ko/weave/reference/python-sdk/trace/op)를 사용하여 W&B UI에서 함수 실행을 자동으로 추적하고 기록하세요.
...
VLM을 사용하여 NER을 수행하는 파이프라인을 만들었으므로, 이제 Weave를 사용하여 이를 체계적으로 평가하고 성능을 확인할 수 있습니다. Weave의 평가에 대한 자세한 내용은 [Evaluations 개요](/ko/weave/guides/core-types/evaluations)에서 확인할 수 있습니다.

Weave Evaluation의 핵심 요소는 [Scorers](/ko/weave/guides/evaluation/scorers)입니다. Scorer는 AI 출력을 평가하고 평가 메트릭을 반환하는 데 사용됩니다. AI의 출력을 받아 분석하고 결과 사전을 반환합니다. Scorer는 필요한 경우 입력 데이터를 참조로 사용할 수 있으며, 평가 결과에 대한 설명이나 추론과 같은 추가 정보를 출력할 수도 있습니다.
