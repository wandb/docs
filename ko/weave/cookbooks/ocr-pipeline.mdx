...
description: "W&B Weave를 사용해 컴퓨터 비전 파이프라인을 트레이스하고 평가하는 방법을 알아보세요"
---
...
시작하기 전에 필요한 라이브러리를 설치하고 임포트한 다음, W&amp;B API 키를 가져오고 Weave 프로젝트를 초기화하세요.
...
좋은 프롬프트 엔지니어링은 모델이 엔터티를 올바르게 추출하도록 유도하는 데 매우 중요합니다. 먼저 이미지 데이터에서 무엇을 추출하고 어떤 형식으로 지정할지에 대한 지침을 모델에 제공하는 기본 프롬프트를 생성합니다. 그런 다음 추적과 반복 개선을 위해 해당 프롬프트를 Weave에 저장합니다.
...
다음으로, 출력의 오류를 줄이는 데 도움이 되도록 더 많은 지침과 검증 규칙을 추가하여 프롬프트를 개선하세요.
...
다음으로, OCR 파이프라인의 입력으로 사용할 손글씨 메모 데이터셋을 조회하세요.

데이터셋의 이미지는 이미 `base64`로 인코딩되어 있으므로, 별도의 전처리 없이 LLM에서 사용할 수 있습니다.
...
1. 데이터셋의 PIL 이미지를 받아 VLM에 전달할 수 있는 `base64` 인코딩 문자열 표현으로 반환하는 `encode_image` 함수
2. 이미지와 system 프롬프트를 받아 system 프롬프트의 설명에 따라 해당 이미지에서 엔티티를 추출해 반환하는 `extract_named_entities_from_image` 함수
...
이제 `named_entity_recognation`이라는 함수를 만들어 다음을 수행하도록 하세요:

* 이미지 데이터를 NER 파이프라인에 전달합니다
* 결과를 올바른 형식의 JSON으로 반환합니다

W&amp;B UI에서 함수 실행을 자동으로 추적하고 트레이스하려면 [`@weave.op()` 데코레이터](/ko/weave/reference/python-sdk/trace/op)를 사용하세요.

`named_entity_recognation`가 실행될 때마다 전체 트레이스 결과를 Weave UI에서 확인할 수 있습니다. 트레이스를 보려면 Weave 프로젝트의 **Traces** 탭으로 이동하세요.
...
이제 VLM을 사용해 NER를 수행하는 파이프라인을 만들었으므로, Weave를 사용해 이를 체계적으로 평가하고 성능이 얼마나 좋은지 확인할 수 있습니다. Weave의 Evaluations에 대한 자세한 내용은 [Evaluations Overview](/ko/weave/guides/core-types/evaluations)에서 확인하세요.

Weave 평가의 핵심 구성 요소 중 하나는 [Scorers](/ko/weave/guides/evaluation/scorers)입니다. Scorers는 AI 출력을 평가하고 평가 메트릭을 반환하는 데 사용됩니다. AI의 출력을 받아 분석한 뒤 결과 사전을 반환합니다. 필요한 경우 입력 데이터를 참조로 사용할 수 있으며, 평가에 대한 설명이나 추론과 같은 추가 정보도 출력할 수 있습니다.

이 섹션에서는 파이프라인을 평가하기 위해 두 개의 Scorers를 생성합니다:
...
프로그래매틱 스코어러 `check_for_missing_fields_programatically`는 모델 출력(`named_entity_recognition` 함수의 출력)을 받아, 결과에서 어떤 `키`가 누락되었거나 비어 있는지 파악합니다.
...
평가의 다음 step에서는 평가가 실제 NER 성능을 반영하도록 이미지 데이터와 모델의 출력이 모두 제공됩니다. 모델 출력만이 아니라 이미지 콘텐츠를 명시적으로 참조합니다.

이 step에 사용되는 스코어러인 `check_for_missing_fields_with_llm`는 scoring을 수행하기 위해 LLM(구체적으로 OpenAI의 `gpt-4o`)을 사용합니다. `eval_prompt`의 내용에 지정된 대로, `check_for_missing_fields_with_llm`는 `Boolean` 값을 출력합니다. 모든 필드가 이미지의 정보와 일치하고 형식이 올바르면 스코어러는 `true`를 반환합니다. 어떤 필드든 누락되었거나, 비어 있거나, 올바르지 않거나, 일치하지 않으면 결과는 `false`이며, 스코어러는 문제를 설명하는 메시지도 함께 반환합니다.

```python lines
# LLM-as-a-judge를 위한 system 프롬프트
...
마지막으로, 전달된 `dataset`을 자동으로 순회하며 결과를 Weave UI에 함께 로깅하는 평가 호출을 정의하세요.

다음 코드는 평가를 시작하고 NER 파이프라인의 각 출력에 두 개의 스코어러를 적용합니다. 결과는 Weave UI의 **Evals** 탭에서 확인할 수 있습니다.
...
위 코드를 실행하면 Weave UI의 평가 테이블로 연결되는 링크가 생성됩니다. 해당 링크를 클릭해 결과를 확인하고, 원하는 모델, 프롬프트, 데이터셋 전반에 걸친 파이프라인의 서로 다른 iteration을 비교하세요. Weave UI는 팀을 위해 아래 예시와 같은 시각화를 자동으로 생성합니다.
