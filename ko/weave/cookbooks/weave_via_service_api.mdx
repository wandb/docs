---
title: "Weave Service API를 사용하여 트레이싱하기"
description: "W&B Weave에서 서비스 API를 통해 트레이싱을 사용하는 방법을 알아보세요"
---
...
<div id="use-the-service-api-to-log-and-query-traces">
  # Service API를 사용해 Trace를 기록하고 조회하기
</div>

이 가이드에서는 Weave Service API를 사용해 trace를 기록하고 조회하는 방법을 설명합니다. 특히 Service API를 사용해 다음 작업을 수행합니다:

1. [단순한 LLM 호출 및 응답의 mock을 생성하고, 이를 Weave에 기록합니다.](#simple-trace)
2. [더 복잡한 LLM 호출 및 응답의 mock을 생성하고, 이를 Weave에 기록합니다.](#complex-trace)
3. [기록된 trace에서 샘플 조회 쿼리를 실행합니다.](#run-a-lookup-query)

> **기록된 trace 보기**
>
> 이 가이드의 코드를 실행할 때 생성되는 모든 Weave trace는 Weave 프로젝트(`team_id\project_id`로 지정)의 **Traces** 탭으로 이동한 후, trace 이름을 선택하여 확인할 수 있습니다.

시작하기 전에 [사전 준비 사항](#prerequisites-set-variables-and-endpoints)을 완료하세요.

<div id="prerequisites-set-variables-and-endpoints">
  ## 전제 조건: 변수와 엔드포인트 설정
</div>

다음 코드는 Service API를 호출하는 데 사용할 URL 엔드포인트를 설정합니다:
...
다음 섹션에서는 간단한 트레이스를 생성하는 방법을 단계별로 안내합니다.
...
다음 코드는 샘플 LLM 호출인 `payload_start`를 생성하고 `url_start` 엔드포인트를 사용해 Weave에 기록합니다. `payload_start` 객체는 `Why is the sky blue?`라는 쿼리로 OpenAI의 `gpt-4o`를 호출하는 상황을 모방합니다.
...
if response.status_code == 200:
    ...
    print(f"호출이 시작되었습니다. ID: {call_id}, Trace ID: {trace_id}")
else:
    print("시작 요청이 실패했습니다. 상태 코드:", response.status_code)
    print(response.text)
    exit()
```
...
간단한 trace를 종료하려면, 다음 코드는 예시 LLM 호출인 `payload_end`를 생성하고 `url_end` 엔드포인트를 사용해 Weave에 기록합니다. `payload_end` 객체는 `Why is the sky blue?`라는 질의에 대해 OpenAI의 `gpt-4o`가 반환하는 응답을 모방합니다. 이 객체는 요금 요약 정보와 chat completion이 Weave Dashboard의 trace view에서 생성되도록 포맷되어 있습니다.

성공 시 이 코드는 trace가 완료되었음을 알리는 메시지를 출력합니다:
...
다음 섹션에서는 여러 연산으로 구성된 RAG 조회와 유사하게, 자식 스팬을 포함한 더 복잡한 트레이스를 생성하는 방법을 단계별로 설명합니다.
...
다음 코드는 여러 개의 스팬으로 구성된 더 복잡한 트레이스를 생성하는 방법을 보여 줍니다. 예를 들어, Retrieval-Augmented Generation(RAG) 조회 후 LLM 호출이 이어지는 경우입니다. 첫 번째 단계에서는 전체 작업을 나타내는 부모 트레이스(`payload_parent_start`)를 초기화합니다. 이 예시에서 작업은 사용자 쿼리 `Can you summarize the key points of this document?`를 처리하는 것입니다.

`payload_parent_start` 객체는 다단계 워크플로의 초기 단계를 나타내며, `url_start` 엔드포인트를 사용해 Weave에 해당 작업을 로깅합니다.
...
다음 코드는 이전 단계에서 시작한 복잡한 상위 trace에 자식 span을 추가하는 방법을 보여줍니다. 이 단계는 전체 워크플로에서 RAG 문서 조회 하위 연산을 모델링합니다.

자식 trace는 `payload_child_start` 객체로 시작하며, 여기에는 다음이 포함됩니다:

* `trace_id`: 이 자식 span을 상위 trace에 연결합니다.
* `parent_id`: 자식 span을 상위 연산과 연결합니다.
* `inputs`: 검색 쿼리를 기록합니다. 예:
  `"This is a search query of the documents I'm looking for."`

`url_start` 엔드포인트 호출이 성공하면, 코드가 자식 호출이 시작되고 완료되었음을 나타내는 메시지를 출력합니다:
...
<div id="add-a-child-span-for-an-llm-completion-call">
  ### LLM completion 호출을 위한 자식 span 추가
</div>

아래 코드는 복잡한 상위 trace에 추가 자식 span을 만들어, 이전 RAG 작업에서 가져온 문서 컨텍스트를 기반으로 하는 LLM completion 호출을 나타내는 방법을 보여준다. 이 단계에서는 문서 컨텍스트를 활용한 AI의 응답 생성 과정을 모델링한다.

LLM completion trace는 다음을 포함하는 `payload_child_start` 객체로 시작된다:

* `trace_id`: 이 자식 span을 상위 trace에 연결한다.
* `parent_id`: 자식 span을 전체 워크플로와 연결한다.
* `inputs`: 사용자 쿼리와 추가된 문서 컨텍스트를 포함해 LLM 입력 메시지를 기록한다.
* `model`: 작업에 사용된 모델을 지정한다 (`gpt-4o`).

성공 시, 코드는 LLM 자식 span trace가 시작되고 종료되었음을 나타내는 메시지를 출력한다:

```
Child call started. ID: 0245acdf-83a9-4c90-90df-dcb2b89f234a
```

작업이 완료되면 `payload_child_end` 객체가 `output` 필드에 LLM이 생성한 응답을 로깅하여 trace를 종료합니다. 사용량 요약 정보도 함께 로깅됩니다.

성공 시, 코드는 LLM child span trace가 시작되고 종료되었음을 나타내는 메시지를 출력합니다:
...
다음 코드는 상위 trace를 종료하여 전체 워크플로가 완료되었음을 표시하는 방법을 보여줍니다. 이 단계에서는 모든 자식 span(예: RAG lookup 및 LLM completion)의 결과를 집계하고 최종 출력과 메타데이터를 로깅합니다.

trace는 다음을 포함하는 `payload_parent_end` 객체를 사용해 종료합니다:

* `id`: 초기 상위 trace를 시작할 때의 `parent_call_id`.
* `output`: 전체 워크플로의 최종 출력을 나타냅니다.
* `summary`: 전체 워크플로에 대한 사용량 데이터를 통합합니다.
* `prompt_tokens`: 모든 prompt에 사용된 토큰의 총합.
* `completion_tokens`: 모든 응답에서 생성된 토큰의 총합.
* `total_tokens`: 워크플로 전체에 대한 토큰 수의 합계.
* `requests`: 발생한 요청의 총 개수(이 경우 `1`).
...
다음 코드는 이전 예제에서 생성한 trace를 조회하되, `inputs.model` 필드가 `gpt-4o`인 trace만 필터링하는 방법을 보여줍니다.

`query_payload` 객체에는 다음이 포함됩니다.

* `project_id`: 조회할 팀과 프로젝트를 지정합니다.
* `filter`: 쿼리 결과에 **trace 루트**(최상위 trace)만 포함되도록 보장합니다.
* `query`: `$expr` 연산자를 사용해 필터 로직을 정의합니다.
  * `$getField`: `inputs.model` 필드를 가져옵니다.
  * `$literal`: `inputs.model`이 `"gpt-4o"`와 같은 trace를 매칭합니다.
