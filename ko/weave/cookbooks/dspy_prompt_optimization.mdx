...
- LLM 워크플로우 추적 및 프롬프팅 전략 평가를 위한 [Weave](/ko/weave/quickstart)
...
Weave는 현재 DSPy와 인테그레이션되어 있으며, 코드 시작 부분에 [`weave.init`](/ko/weave/reference/python-sdk/trace/weave_client#method-init)을 포함하면 Weave UI에서 탐색할 수 있는 DSPy 함수를 자동으로 추적(trace)할 수 있습니다. 자세한 내용은 [DSPy를 위한 Weave 인테그레이션 가이드](/ko/weave/guides/integrations/dspy)를 확인하세요.
...
이 튜토리얼에서는 메타데이터를 관리하기 위해 [`weave.Object`](/ko/weave/reference/python-sdk#class-object)를 상속받은 메타데이터 클래스를 사용합니다.
...
HuggingFace Hub에서 이 데이터셋을 로드하여 트레이닝 및 검증 세트로 나누고, Weave에 [게시(publish)](/ko/weave/guides/core-types/datasets)할 것입니다. 이를 통해 Datasets의 버전을 관리하고, [`weave.Evaluation`](/ko/weave/guides/core-types/evaluations)을 사용하여 프롬프팅 전략을 평가할 수 있습니다.
...
이제 베이스라인 프롬프팅 전략이 준비되었으므로, 예측된 답변과 그라운드 트루스를 매칭하는 간단한 메트릭을 사용하여 검증 세트에서 [`weave.Evaluation`](/ko/weave/guides/core-types/evaluations)으로 평가해 보겠습니다. Weave는 각 예시를 가져와 애플리케이션을 통해 전달하고 여러 커스텀 스코어링 함수로 출력 점수를 매깁니다. 이를 통해 애플리케이션의 성능을 한눈에 파악하고, 풍부한 UI를 통해 개별 출력과 점수를 자세히 조사할 수 있습니다.
