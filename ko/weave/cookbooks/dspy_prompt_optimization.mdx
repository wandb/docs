...
[GPT3.5 Turbo](https://platform.openai.com/docs/models/gpt-3.5-turbo)에 LLM 호출을 수행하기 위해 [`dspy.OpenAI`](https://dspy.ai/learn/programming/language_models/#__tabbed_1_1) 추상화를 사용합니다.
...
먼저 베이스라인 모듈의 출력에서 나온 답변이 정답과 같은지 여부를 판별하는 단순한 Weave evaluation 채점 함수를 만들어야 합니다. 채점 함수에는 `model_output` 키워드 인수가 필요하지만, 나머지 인수는 모두 사용자가 정의하며 데이터셋 예시에서 가져옵니다. 인수 이름에 따라 딕셔너리 키를 사용해 필요한 키만 가져옵니다.
...
다음으로 Evaluation을 간단히 정의하고 실행하면 됩니다.
...
<Warning>
  causal reasoning 데이터셋으로 평가를 실행하면 OpenAI 크레딧이 약 $0.24 소요됩니다.
</Warning>
...
이제 베이스라인 DSPy 프로그램이 있으니, 지정된 메트릭을 최대화하도록 DSPy 프로그램의 매개변수를 조정할 수 있는 [BootstrapFewShot](https://dspy.ai/api/optimizers/BootstrapFewShot/) teleprompter를 사용해 인과 추론 성능을 개선해 보겠습니다.
