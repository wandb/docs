...
<div id="using-huggingface-datasets-in-evaluations-with-preprocess_model_input">
  # `preprocess_model_input`와 함께 HuggingFace 데이터셋로 평가 수행하기
</div>
...
> 이 가이드는 HuggingFace 데이터셋를 Weave 평가에 사용하기 위한 임시 해결 방법을 보여 줍니다.<br /><br />
...
* 이 인덱스 방식을 사용하면 원본 데이터셋에 대한 참조를 유지할 수 있습니다.

> **Note:**<br />
> 인덱스에서는 각 행에 고유 식별자가 생기도록 `hf_hub_name`과 `hf_id`를 함께 인코딩합니다.
...
* `hf_eval`: 모델 출력의 점수를 매기는 방식을 정의합니다
