---
title: 샌드박스 라이프사이클
description: 상태, 상태 변경을 기다리는 방법, 샌드박스를 중지하는 방법을 포함해 Serverless Sandbox의 라이프사이클에 대해 알아보세요.
...
다음 섹션에서는 샌드박스 상태, 준비 완료 또는 완료 시점까지 대기하는 방법, 그리고 샌드박스를 중지하는 방법을 설명합니다. 샌드박스를 생성하고 명령어를 실행하는 방법에 대한 자세한 내용은 [샌드박스 생성](/ko/sandboxes/create-sandbox) 및 [명령어 실행](/ko/sandboxes/run-commands)을 참조하세요.

## 샌드박스 상태 {#sandbox-states}
...
## 준비 완료 또는 완료 시점까지 기다리기 {#wait-for-readiness-or-completion}
...
### 샌드박스가 시작될 때까지 기다리기 {#wait-for-a-sandbox-to-start}
...
### 샌드박스 완료까지 기다리기 {#wait-for-a-sandbox-to-complete}
...
## 명시적 라이프사이클 제어 {#explicit-lifecycle-control}
