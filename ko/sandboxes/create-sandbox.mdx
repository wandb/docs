...
<Info>
  ...
  사용하기 전에 이미지와 태그가 공개 레지스트리에 있는지 확인하세요. 샌드박스가 이미지를 가져올 수 없으면 시작되지 않으며 클라이언트에서 `SandboxFailedError` 예외가 발생합니다.

  <Info>
    `SandboxFailedError`는 여러 이유로 발생할 수 있으며, 사용 불가능한 이미지를 구체적으로 식별하지는 않습니다. 이 오류가 발생하면 이미지와 태그를 사용할 수 있는지 확인하세요.
  </Info>

  다음 코드 스니펫은 `python:3.14` 이미지를 사용해 샌드박스를 만들고, 그 안에서 `python --version`을 실행합니다.
...
### 메인 명령 없이 샌드박스 시작하기 {#start-a-sandbox-without-a-main-command}
...
### 메인 명령으로 샌드박스 시작하기 {#start-a-sandbox-with-a-main-command}
...
## Session을 사용해 여러 샌드박스 만들기 {#create-multiple-sandboxes-with-a-session}
