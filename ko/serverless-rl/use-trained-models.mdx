...
* [Serverless RL API](/ko/serverless-rl/api-reference)의 base URL, `https://api.training.wandb.ai/v1/`
* 모델의 엔드포인트
...
* 모델에 연결된 프로젝트 이름
* 트레이닝된 모델의 이름
* 배포하려는 모델의 트레이닝 step. 일반적으로는 평가에서 모델 성능이 가장 좋았던 step입니다.

예를 들어 W&amp;B 팀 이름이 `email-specialists`이고, 프로젝트 이름이 `mail-search`이며, 트레이닝된 모델 이름이 `agent-001`이고, step 25에서 배포하려는 경우 Endpoint는 다음과 같습니다.
