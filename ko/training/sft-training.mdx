...
* **자동 배포**: Serverless SFT는 트레이닝한 모든 체크포인트를 자동으로 배포하여, 호스팅 인프라를 수동으로 구성할 필요를 없애줍니다. 트레이닝된 모델은 로컬, 스테이징, 프로덕션 환경에서 즉시 액세스하고 테스트할 수 있습니다.

<div id="how-serverless-sft-uses-wb-services">
  ## Serverless SFT가 W&amp;B 서비스를 사용하는 방식
</div>

Serverless SFT는 다음 W&amp;B 컴포넌트를 조합하여 작동합니다:

* [Inference](/ko/inference): 모델을 실행하는 데 사용합니다
* [Models](/ko/models): LoRA 어댑터 트레이닝 중 성능 메트릭을 추적하는 데 사용합니다
* [Artifacts](/ko/models/artifacts): LoRA 어댑터를 저장하고 버전을 관리하는 데 사용합니다
* [Weave (선택)](/ko/weave): 트레이닝 루프의 각 step에서 모델이 어떻게 응답하는지 관찰하는 데 사용합니다

Serverless SFT는 현재 퍼블릭 프리뷰입니다. 프리뷰 기간 동안에는 Inference 사용량과 아티팩트 저장에 대해서만 요금이 청구됩니다. W&amp;B는 프리뷰 기간 동안 어댑터 트레이닝에 대해서는 요금을 청구하지 않습니다.