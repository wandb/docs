...
keywords: ["에어갭 설치", "오프라인 배포", "분리된 클러스터", "프라이빗 레지스트리", "OpenShift"]
---
...
### SSL/TLS 요구 사항 {#ssltls-requirements}
...
### 에어갭 환경별 요구 사항 {#air-gapped-specific-requirements}
...
## 에어갭 환경 준비 {#prepare-your-air-gapped-environment}

다음 단계에서는 W&amp;B 컨테이너 이미지와 Helm 차트를 호스팅할 에어갭 환경을 준비합니다. Operator를 설치하거나 플랫폼을 배포하기 전에 다음 단계를 완료하세요.

### Step 1: 내부 컨테이너 레지스트리 설정 {#step-1-set-up-internal-container-registry}
...
#### 핵심 W&amp;B 구성 요소 컨테이너 {#core-wb-component-containers}
...
#### 종속성 컨테이너 {#dependency-containers}
...
#### 전체 이미지 목록 조회 {#get-the-complete-image-list}
...
#### 이미지를 에어갭 레지스트리로 전송 {#transfer-images-to-air-gapped-registry}
...
### Step 2: 내부 Helm 차트 저장소 설정 {#step-2-set-up-internal-helm-chart-repository}
...
### step 3: Helm 저장소 접근 설정 {#step-3-configure-helm-repository-access}
...
## 에어갭 환경에서 W&amp;B 배포 {#deploy-wb-in-air-gapped-environment}

내부 레지스트리와 Helm 저장소를 준비했다면 이제 Kubernetes Operator를 설치하고 외부 서비스를 구성한 다음 W&amp;B Platform을 배포할 수 있습니다.

### Step 4: Kubernetes Operator 설치 {#step-4-install-the-kubernetes-operator}
...
### Step 5: MySQL 데이터베이스 설정 {#step-5-set-up-mysql-database}
...
### 6단계: W&amp;B Custom Resource 설정 {#step-6-configure-wb-custom-resource}
...
### 7단계: W&amp;B Platform 배포 {#step-7-deploy-the-wb-platform}
...
## OpenShift 설정 {#openshift-configuration}
...
### OpenShift 보안 컨텍스트 제약 조건 {#openshift-security-context-constraints}
...
#### 옵션 1: restricted SCC 사용(권장) {#option-1-use-restricted-scc-recommended}
...
#### 옵션 2: 맞춤형 SCC 만들기(필요한 경우) {#option-2-create-custom-scc-if-required}
...
### OpenShift 이미지 풀 설정 {#openshift-image-pull-configuration}
...
### OpenShift 전체 예시 {#openshift-complete-example}
...
## 설치를 확인하세요 {#verify-your-installation}
...
### 추가 에어갭 검증 {#additional-air-gapped-verification}
...
### OpenShift SCC 오류 {#openshift-scc-errors}
...
### Helm chart를 찾을 수 없음 {#helm-chart-not-found}
...
### 다른 인그레스 클래스를 사용할 수 있나요? {#can-i-use-a-different-ingress-class}
...
### 여러 인증서가 포함된 인증서 번들은 어떻게 처리하나요? {#how-do-i-handle-certificate-bundles-with-multiple-certificates}
...
### 자동 업데이트를 방지하려면 어떻게 해야 하나요? {#how-do-i-prevent-automatic-updates}
...
### 배포는 공개 리포지토리에 연결하지 않아도 작동하나요? {#does-the-deployment-work-with-no-connection-to-public-repositories}

예. Operator 설정에 `airgapped: true`를 지정하면 Kubernetes Operator는 내부 리소스만 사용하고 공개 리포지토리에 연결을 시도하지 않습니다.

### 에어갭 환경에서 W&amp;B를 어떻게 업데이트하나요? {#how-do-i-update-wb-in-an-air-gapped-environment}
