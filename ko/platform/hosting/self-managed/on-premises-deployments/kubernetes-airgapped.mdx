...
2. 승인된 방법(USB 드라이브, 보안 파일 전송 등)을 사용해 `.tar` 파일을 air-gapped 환경으로 전송합니다.

3. air-gapped 환경에서 이미지를 로드한 뒤 내부 레지스트리에 push합니다:
...
<Note>
  보안 요구 사항에 맞는 포괄적인 OpenShift 설정 예시가 필요하면 [W&amp;B 지원팀](mailto:support@wandb.com) 또는 배정된 W&amp;B 지원 엔지니어에게 문의하세요.
</Note>
