...
<Tabs>
  ...
  <Tab title="Terraform">
    ...
    * [Google Cloud](https://github.com/wandb/terraform-google-wandb/blob/49ddc3383df4cefc04337a2ae784f57ce2a2c699/main.tf#L189)
    ...
    <div id="cloud-terraform-modules">
      #### 클라우드 Terraform 모듈
    </div>
...
<Tabs>
  ...
  <Tab title="Google Cloud">
    ...
    * Pub/Sub 메시징 시스템
    ...
    <div id="recommended-deployment-2">
      ### 권장 배포
    </div>
    ...
    <div id="enable-message-broker-queue-2">
      ### 메시지 브로커(큐) 활성화
    </div>
  ...
  <Tab title="Azure">
    ...
    * Azure Event Grid
    ...
    <div id="prerequisite-permissions-3">
      ### 사전 필요 권한
    </div>
    ...
    <div id="recommended-deployment-3">
      ### 권장 배포
    </div>
    ...
    <div id="enable-message-broker-queue-3">
      ### 메시지 브로커(큐) 활성화
    </div>
...
<div id="clickhouse-compatibility-for-upgrades">
  ## 업그레이드 전 ClickHouse 호환성
</div>

외부 ClickHouse 클러스터를 사용하는 Self-Managed 배포 환경은 W&amp;B Server를 업그레이드하기 전에 ClickHouse 호환성을 확인해야 합니다.

<div id="unsupported-clickhouse-versions">
  ### 지원되지 않는 ClickHouse 버전
</div>

W&amp;B Weave 또는 Models OLAP 특성을 사용하는 W&amp;B Self-Managed 배포에서는 ClickHouse Server와 ClickHouse Keeper가 모두 지원되는 버전이어야 합니다.

* Weave는 ClickHouse 26.1 또는 26.2에서 작동하지 않습니다.
* 다음 섹션에서 설명하는 Models OLAP 특성을 사용하려면 ClickHouse 26.2+가 필요합니다.
* Weave와 Models OLAP 특성을 모두 사용하는 배포에는 ClickHouse 26.3이 필요합니다.

W&amp;B Self-Managed 배포에서 Weave와 Models OLAP 특성을 모두 사용하지 않는 경우 ClickHouse는 필요하지 않습니다.

<div id="models-olap-features">
  ### Models OLAP 특성
</div>

W&amp;B Server 0.81.0+부터는 ClickHouse 26.2+가 필요한 스키마 마이그레이션이 도입되었습니다. 이러한 마이그레이션은 Helm values에서 다음 Models OLAP 특성 중 하나라도 활성화되어 있으면 실행됩니다:
...
***W&amp;B Self-Managed를 Server v0.81+로 업그레이드하기 전에***, 반드시 *다음 중 하나를* 수행하세요:

* Helm values에서 해당 설정을 `false`로 지정하거나,
* ClickHouse Server와 ClickHouse Keeper를 모두 26.2 이상으로 업그레이드하세요(배포 환경에서 Weave도 사용하는 경우 26.3 이상).

이 특성들이 비활성화되어 있거나 Models OLAP 워크로드에 ClickHouse를 사용하지 않는 경우에는 업그레이드 시 이 요구 사항이 적용되지 않습니다.
