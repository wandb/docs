...
keywords: ["W&B Operator", "Helm chart", "Kubernetes CR", "operator 설치", "커스텀 리소스"]
---

import SelfManagedMysqlRequirements from "/snippets/ko/_includes/self-managed-mysql-requirements.mdx";
import SelfManagedMysqlEolUpgrade from "/snippets/ko/_includes/self-managed-mysql-eol-upgrade.mdx";
...
MySQL 8.0.x에서 업그레이드하는 경우 [MySQL을 8.4.x로 업그레이드](#upgrade-mysql-to-84x)를 참조하세요.
...
### OpenShift Kubernetes 클러스터 {#openshift-kubernetes-clusters}
...
#### 컨테이너를 비특권 사용자로 실행 {#run-the-container-as-an-un-privileged-user}
...
## W&amp;B Server 애플리케이션 배포 {#deploy-wb-server-application}
...
#### Helm Terraform Module {#helm-terraform-module}
...
#### 클라우드 Terraform 모듈 {#cloud-terraform-modules}
...
### 설치 확인하기 {#verify-the-installation}

<SelfManagedVerifyInstallation />

## MCP 서버 활성화 {#enable-the-mcp-server}
...
### MCP 서버 확인 {#verify-the-mcp-server}
...
## 환경별 고려 사항 {#environment-specific-considerations}
...
### 온프레미스 및 베어메탈 {#on-premises-and-bare-metal}

온프레미스 또는 베어메탈 Kubernetes에 배포할 때는 다음 사항에 유의하세요.

#### 로드 밸런서 설정 {#load-balancer-configuration}
...
#### DNS 및 인증서 관리 {#dns-and-certificate-management}
...
#### 온프레미스 및 S3 호환 환경용 객체 저장소 {#object-storage-for-on-premises-and-s3-compatible}
...
### Terraform을 사용한 퍼블릭 클라우드 {#public-cloud-with-terraform}
...
## Terraform으로 퍼블릭 클라우드에 배포하기 {#deploy-with-terraform-on-public-cloud}
...
### 사전 필요 권한 {#prerequisite-permissions}
...
### 메시지 브로커(큐) 활성화 {#enable-message-broker-queue}
...
### 권장 배포 {#recommended-deployment-2}
...
### 메시지 브로커(큐) 활성화 {#enable-message-broker-queue-2}
...
### 사전 필요 권한 {#prerequisite-permissions-3}
...
### 권장 배포 {#recommended-deployment-3}
...
### 메시지 브로커(큐) 활성화 {#enable-message-broker-queue-3}
...
### 기타 배포 옵션 {#other-deployment-options}
...
## W&amp;B 관리 콘솔에 액세스 {#access-the-wb-management-console}
...
## W&amp;B Kubernetes Operator 업데이트 {#update-the-wb-kubernetes-operator}
...
## W&amp;B Server 애플리케이션 업데이트 {#update-the-wb-server-application}
...
## MySQL을 8.4.x로 업그레이드 {#upgrade-mysql-to-84x}

MySQL 8.0.x는 수명 종료되었습니다. Self-Managed 배포에서는 보안 패치와 중요한 버그 수정을 제공받는 지원되는 MySQL 버전을 실행해야 합니다. 커뮤니티 MySQL을 사용 중이라면 **MySQL 8.4.x**를 설치하거나 업그레이드하세요. 관리형 서비스를 사용하는 경우에는 공급자가 지원되며 패치가 적용된다고 문서에 명시한 엔진 버전(예: Amazon RDS for MySQL, Google Cloud SQL for MySQL, Azure Database for MySQL)을 사용하세요. W&amp;B는 MySQL 8.4.0 및 현재 8.4.x 릴리스에 대해 플랫폼 호환성을 검증했습니다.

다음 단계는 W&amp;B 관점에서의 진행 순서를 설명합니다. 백업 및 버전 업그레이드 경로를 포함해 MySQL 자체를 업그레이드하는 방법은 사용 중인 MySQL 배포판 또는 클라우드 제공업체의 문서를 따르세요. 동일한 순서는 [standard](/ko/platform/hosting/self-managed/operator) 및 [air-gapped](/ko/platform/hosting/self-managed/on-premises-deployments/kubernetes-airgapped) Operator 배포에 적용됩니다. 에어갭 환경에서는 데이터베이스를 업그레이드하기 전에 내부 배포 프로세스를 통해 MySQL 8.4.x 소프트웨어를 획득하세요.

<Note>
  시작하기 전에 유지 관리 시간을 계획하고 사용자에게 미리 알려주세요. 호환성이나 배포 토폴로지와 관련해 궁금한 점이 있으면 [Customer Support](mailto:support@wandb.com) 또는 담당 W&amp;B 팀에 문의하세요.
</Note>

1. 대상 버전과 그 사이의 각 버전에 대한 MySQL 릴리스 노트 및 문서를 검토하여 요구 사항과 기타 세부 정보를 확인하세요.
2. 유지 관리를 준비하세요.

   업그레이드를 시작하기 전에 데이터베이스에 대해 [MySQL Shell upgrade checker](https://dev.mysql.com/doc/mysql-shell/8.4/en/mysql-shell-utilities-upgrade.html)를 실행해 대상 버전과의 호환성 문제를 찾아 수정할 수 있습니다. 진행하기 전에 검사기 출력의 오류나 경고를 모두 해결하세요. 사용 중인 MySQL 배포판의 문서를 참고하세요.
3. MySQL을 종료하고, 사용 중인 MySQL 배포판의 문서에 따라 MySQL 데이터베이스의 전체 백업을 수행하세요.

   업그레이드 중에는 MySQL을 사용할 수 없습니다. 데이터베이스를 사용할 수 없는 동안 W&amp;B 클라이언트 애플리케이션은 연결할 수 없으며 일시적인 오류가 발생합니다.
4. 사용 중인 MySQL 배포판의 문서에 따라 MySQL을 8.4.x로 업그레이드하세요.
5. MySQL을 다시 시작하고 정상적으로 작동하는지 확인하세요.
6. MySQL이 실행되면 `wandb verify`를 실행하여 W&amp;B 배포를 검증하세요. 이 명령은 일련의 검사를 실행하고 결과를 `STDOUT`에 출력합니다. 문제가 보고되면 필요한 조치를 취한 후 다시 실행하세요. 설정 및 로그인 단계는 [설치 확인](#verify-the-installation)을 참고하세요.
7. 검증이 완료되면 사용자는 정상 작업을 재개할 수 있습니다.

## 업그레이드 전 ClickHouse 호환성 {#clickhouse-compatibility-for-upgrades}

외부 ClickHouse 클러스터를 사용하는 Self-Managed 배포 환경은 W&amp;B Server를 업그레이드하기 전에 ClickHouse 호환성을 확인해야 합니다.

### 지원되는 ClickHouse 버전 {#supported-clickhouse-versions}

W&amp;B Weave는 ClickHouse Server와 ClickHouse Keeper 모두 지원되는 버전을 사용해야 합니다.

* Weave는 ClickHouse 25.8부터 25.12까지, 그리고 26.3 이상을 지원합니다.
* Weave는 ClickHouse 26.1 또는 26.2에서는 동작하지 않습니다.

**W&amp;B Self-Managed를 업그레이드하기 *전에***, ClickHouse Server와 ClickHouse Keeper가 모두 Weave에서 지원하는 버전으로 실행 중인지 확인하세요. ClickHouse 버전을 변경해야 하는 경우, 두 컴포넌트를 함께 업그레이드하세요.

W&amp;B Self-Managed 배포 환경에서 Weave를 사용하지 않는 경우 ClickHouse는 필요하지 않으며, 업그레이드 시 이 요구 사항이 적용되지 않습니다.

Dedicated Cloud 및 Multi-tenant Cloud 배포 환경은 이미 호환되는 ClickHouse 버전에서 실행되므로 영향을 받지 않습니다.

버전별 릴리스 노트는 [지원되는 W&amp;B Server 릴리스](/ko/release-notes/server-releases)에서 확인하세요.

## Self-Managed 인스턴스를 W&amp;B Operator로 마이그레이션하기 {#migrate-self-managed-instances-to-wb-operator}
...
### Operator 기반 AWS Terraform 모듈로 이전 {#migrate-to-operator-based-aws-terraform-modules}

이전 절차에 대한 자세한 설명은 [operator-wandb 차트 문서](https://github.com/wandb/helm-charts/tree/main/charts/operator-wandb)를 참조하세요.

### Operator 기반 Google Cloud Terraform 모듈로 마이그레이션 {#migrate-to-operator-based-google-cloud-terraform-modules}

질문이 있거나 도움이 필요하면 [Customer Support](mailto:support@wandb.com) 또는 담당 W&amp;B 팀에 문의하세요.

### Operator 기반 Azure Terraform 모듈로 마이그레이션 {#migrate-to-operator-based-azure-terraform-modules}

질문이 있거나 도움이 필요하면 [Customer Support](mailto:support@wandb.com) 또는 담당 W&amp;B 팀에 문의하세요.

### Operator 기반 Helm chart로 마이그레이션 {#migrate-to-operator-based-helm-chart}
...
### Operator 기반 Terraform Helm chart로 마이그레이션 {#migrate-to-operator-based-terraform-helm-chart}
...
## W&amp;B Server 설정 레퍼런스 {#configuration-reference-for-wb-server}
...
### 객체 저장소 (버킷) {#object-storage-bucket}
...
### 맞춤형 Kubernetes 서비스 계정 {#custom-kubernetes-service-accounts}
...
### 요청 속도 제한 설정 {#set-rate-limits}

Operator를 사용하는 Dedicated Cloud 및 Self-Managed 배포에서는 필요에 따라 `spec.values.global.extraEnv`에서 환경 변수를 설정해 요청 속도 제한을 구성하세요. 참고로 이 예시에서는 각 요청 속도 제한을 기본값으로 명시적으로 설정합니다. 실제로는 기본값을 재정의할 경우에만 요청 속도 제한을 정의하면 됩니다.

<Important>요청 속도 제한을 활성화하려면 `GORILLA_LIMITER`를 Redis 서버 주소로 설정해야 합니다.</Important>
...
| 환경 변수                                                  | 기본값   | 설명                          |
| ------------------------------------------------------ | ----- | --------------------------- |
| `GORILLA_DEFAULT_RATE_LIMITS_FILESTREAM`               | `5`   | 기본 Filestream 요청 수입니다.      |
| `GORILLA_DEFAULT_RATE_LIMITS_FILESTREAM_COUNT`         | `5`   | 초당 Filestream 요청 수입니다.      |
| `GORILLA_DEFAULT_RATE_LIMITS_FILESTREAM_PER_RUN_COUNT` | `0.8` | run별 초당 Filestream 요청 수입니다. |
| `GORILLA_DEFAULT_RATE_LIMITS_FILESTREAM_SIZE`          | `10`  | 초당 Filestream 수집 한도(MB)입니다. |
| `GORILLA_DEFAULT_RATE_LIMITS_RUN_UPDATE_COUNT`         | `10`  | 초당 run 메타데이터 업데이트 요청 수입니다.  |

### 맞춤형 인증 기관 {#custom-certificate-authority}
...
### 맞춤형 보안 컨텍스트 {#custom-security-context}
...
## W&amp;B Operator용 설정 레퍼런스 {#configuration-reference-for-wb-operator}
...
### 각 파드의 목적과 역할 {#purpose-and-role-of-each-pod}
...
### W&amp;B Operator Console 비밀번호를 조회하는 방법 {#how-to-get-the-wb-operator-console-password}

[W&amp;B 관리 콘솔에 접속하기](#access-the-wb-management-console)를 참조하세요.

### 인그레스가 작동하지 않을 때 W&amp;B Operator Console에 접속하는 방법 {#how-to-access-the-wb-operator-console-if-ingress-doesnt-work}
...
### W&amp;B Server 로그를 확인하는 방법 {#how-to-view-wb-server-logs}
...
### Kubernetes 인그레스 클래스를 파악하는 방법 {#how-to-identify-the-kubernetes-ingress-class}
