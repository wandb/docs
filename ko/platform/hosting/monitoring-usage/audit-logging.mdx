...
| [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud) | Enterprise 요금제에서만 사용할 수 있습니다. [API](#fetch-audit-logs-using-api)를 통해서만 사용할 수 있습니다.
...
- [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)의 경우 API 엔드포인트는 항상 PII를 포함하여 감사 로그에 대한 관련 필드를 반환합니다. 이는 구성할 수 없습니다.
...
- 여러 Enterprise [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud) 조직의 관리자인 경우 감사 로깅 API 요청이 전송되는 조직을 구성해야 합니다. 프로필 이미지를 클릭한 다음 **User Settings**를 클릭합니다. 설정 이름은 **Default API organization**입니다.
</Note>
...
1. 인스턴스에 적합한 API 엔드포인트를 결정합니다.
    ...
    - [SaaS Cloud (Enterprise 필요)](/ko/platform/hosting/hosting-options/saas_cloud): `https://api.wandb.ai/audit_logs`
...
2. 기본 엔드포인트에서 전체 API 엔드포인트를 구성하고 선택적으로 URL 파라미터를 포함합니다.
    ...
    - `startDate`: `YYYY-MM-DD` 형식의 선택적 날짜입니다. [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)에서만 지원됩니다.
...
### 감사 로그를 가져올 때 PII 제외
[자체 관리](/ko/platform/hosting/hosting-options/self-managed) 및 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)의 경우 W&B 조직 또는 인스턴스 관리자는 감사 로그를 가져올 때 PII를 제외할 수 있습니다. [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)의 경우 API 엔드포인트는 항상 PII를 포함하여 감사 로그에 대한 관련 필드를 반환합니다. 이는 구성할 수 없습니다.
...
<a id="1">1</a>: [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)에서 감사 로그는 다음에 대해 수집되지 않습니다.
- 공개 또는 퍼블릭 프로젝트.
- `report:read` action.
