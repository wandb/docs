...
description: "배포 유형 전반에서 W&B 감사 로그에 액세스하고, 가져오고, 분석하는 방법과 로그 스키마 및 추적되는 작업을 설명합니다."
---
...
| ----------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud)       | <ul><li>[Instance-level BYOB](/ko/platform/hosting/data-security/secure-storage-connector): 인스턴스 수준 버킷(BYOB)으로 10분마다 동기화됩니다. [API](#fetch-audit-logs-using-api)를 사용해 가져올 수도 있습니다.</li><li>기본 인스턴스 수준 스토리지: [API](#fetch-audit-logs-using-api)로만 가져올 수 있습니다.</li></ul> |
...
* [1996년 의료보험 양도 및 책임에 관한 법(HIPAA)](https://hhs.gov/hipaa/for-professionals/index.html)의 적용을 받는 경우, 감사 로그는 필수 보존 기간이 끝나기 전에 내부 또는 외부의 어떤 주체에 의해서도 삭제되거나 수정될 수 없는 환경에서 최소 6년 동안 보존되어야 합니다. HIPAA를 준수하는 [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud) 인스턴스에서 [BYOB](/ko/platform/hosting/data-security/secure-storage-connector)를 사용하는 경우, 장기 보존 스토리지를 포함한 관리형 스토리지 전반에 대해 보호 장치(guardrail)를 구성해야 합니다.
...
* [Self-Managed](/ko/platform/hosting/hosting-options/self-managed) 및
  [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud)의 경우, 조직 관리자가 감사 로그를 가져올 때 [PII를 제외](#exclude-pii)하도록 설정할 수 있습니다.
...
2. (선택 사항) 엔드포인트에 추가할 쿼리 파라미터를 구성한다. 다음 단계에서 `<parameters>`를 이 과정에서 생성된 문자열로 바꿔서 사용한다.
   ...
   * `numdays`와 `startDate`를 조합해서 가져올 로그의 날짜 범위를 설정한다. 각 파라미터는 선택 사항이며, 서로 상호작용한다.
     ...
     * `numDays`: 로그를 가져올 기준 날짜인 `startDate`로부터 며칠 전까지의 로그를 가져올지 나타내는 정수이다. 생략되었거나 `0`으로 설정된 경우, `startDate` 하루의 로그만 가져온다. **Multi-tenant Cloud**에서는 `numDays`를 더 큰 값으로 설정하더라도 최대 7일치 감사 로그만 가져올 수 있다.
     * `startDate`: **Multi-tenant Cloud**에서만 지원된다. `startDate=YYYY-MM-DD` 형식으로, 가져올 가장 최신 로그의 기준 날짜를 지정한다. 생략되었거나 오늘 날짜로 명시적으로 설정된 경우, 오늘부터 `numDays`까지의 로그를 가져온다.
3. `<API-endpoint>?<parameters>` 형식으로 완전한 엔드포인트 URL을 구성한다.
4. 웹 브라우저 또는 [Postman](https://www.postman.com/downloads/), [HTTPie](https://httpie.io/), cURL과 같은 도구를 사용해 완전한 API 엔드포인트에 대해 HTTP `GET` 요청을 실행한다.
...
[Self-Managed](/ko/platform/hosting/hosting-options/self-managed) 및 [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud)의 경우, W&amp;B 조직 또는 인스턴스 관리자는 감사 로그를 가져올 때 PII를 제외할 수 있습니다. [Multi-tenant Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)의 경우, API 엔드포인트는 항상 PII를 포함하여 감사 로그에 해당하는 필드를 반환하며, 이는 설정을 변경할 수 없습니다.
...
| `team:uninvite`               | 사용자 또는 서비스 계정이 팀 초대에서 제외됩니다.                               |
...
| `user:update`                 | 사용자 정보가 업데이트됩니다. <sup><a href="#1">1</a></sup>             |
