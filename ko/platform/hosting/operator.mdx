...
[참조 아키텍처](/ko/platform/hosting/self-managed/ref-arch/#infrastructure-requirements)를 참조하세요. 또한 [유효한 W&B Server 라이선스를 획득하세요](/ko/platform/hosting/hosting-options/self-managed#obtain-your-wb-server-license).

자체 관리 설치를 설정하고 구성하는 방법에 대한 자세한 설명은 [이](/ko/platform/hosting/self-managed/bare-metal/) 가이드를 참조하세요.
...
### 에어 갭 설치
에어 갭 환경에 W&B Kubernetes Operator를 설치하는 방법은 [Kubernetes를 사용하여 에어 갭 환경에 W&B 배포](/ko/platform/hosting/self-managed/operator-airgapped/) 튜토리얼을 참조하세요.
...
이러한 모듈을 사용하는 방법에 대한 자세한 설명은 문서의 [자체 관리 설치 섹션](/ko/platform/hosting/hosting-options/self-managed#deploy-wb-server-within-self-managed-cloud-accounts)을 참조하세요.

### 설치 확인

설치를 확인하기 위해 W&B는 [W&B CLI](/ko/models/ref/cli/)를 사용하는 것이 좋습니다. 확인 코맨드는 모든 구성 요소와 구성을 확인하는 여러 테스트를 실행합니다.
...
마이그레이션 프로세스에 대한 자세한 설명은 [여기](/ko/platform/hosting/operator/#deploy-wb-with-helm-cli)를 계속 진행하세요.
...
설명서에는 [기본](/ko/platform/hosting/env-vars/) 및 [고급](/ko/platform/hosting/iam/advanced_env_vars/)의 두 가지 환경 변수 목록이 있습니다. 필요한 설정 옵션이 Helm Chart를 사용하여 노출되지 않은 경우에만 환경 변수를 사용하세요.
