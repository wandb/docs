...
description: W&B가 객체 저장소를 어떻게 사용하는지, 삭제가 버킷 바이트 사용량에 어떻게 반영되는지, 그리고 저장소 사용량과 비용을 줄이는 방법을 알아보세요.
keywords: ["저장소 정리", "가비지 컬렉션", "아티팩트 삭제", "버킷 바이트", "S3 비용"]
---
...
## 버킷 공간을 차지하는 항목 {#what-uses-bucket-space}
...
## W&amp;B가 저장소에서 데이터를 제거하는 방법 {#how-wb-removes-data-from-storage}
...
### run 데이터 및 run 파일 {#run-data-and-run-files}
...
## 백그라운드 정리 시 예상할 사항 {#what-to-expect-from-background-cleanup}
...
## 버킷 사용량 줄이기 {#reduce-bucket-usage}
