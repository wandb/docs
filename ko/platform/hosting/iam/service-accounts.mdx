...
<Note>
서비스 계정은 [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated_cloud), 엔터프라이즈 라이선스가 포함된 [Self-Managed 인스턴스](/ko/platform/hosting/hosting-options/self-managed), 그리고 [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)의 엔터프라이즈 계정에서 사용할 수 있습니다.
</Note>

## 조직 범위 서비스 계정

조직 범위의 서비스 계정은 [제한된 프로젝트(restricted projects)](/ko/platform/hosting/iam/access-management/restricted-projects/#visibility-scopes)를 제외하고, 팀에 관계없이 조직 내의 모든 Projects에 대해 읽기 및 쓰기 권한을 가집니다. 조직 범위 서비스 계정이 제한된 프로젝트에 엑세스하려면, 해당 프로젝트의 관리자가 서비스 계정을 프로젝트에 명시적으로 추가해야 합니다.
...
팀 범위 서비스 계정은 해당 팀 내의 [제한된 프로젝트(restricted projects)](/ko/platform/hosting/iam/access-management/restricted-projects/#visibility-scopes)를 제외한 모든 Projects에서 읽고 쓸 수 있습니다. 팀 범위 서비스 계정이 제한된 프로젝트에 엑세스하려면, 해당 프로젝트의 관리자가 서비스 계정을 프로젝트에 명시적으로 추가해야 합니다.
...
<Warning>
팀 범위 서비스 계정은 상위 팀이 아닌 다른 팀의 [팀 또는 제한된 범위 프로젝트(team or restricted-scoped project)](/ko/platform/hosting/iam/access-management/restricted-projects/#visibility-scopes)에는 Runs를 로깅할 수 없지만, 다른 팀 내의 공개 가시성 프로젝트(open visibility project)에는 Runs를 로깅할 수 있습니다.
</Warning>

### 외부 서비스 계정

기본 제공 서비스 계정 외에도, W&B는 JSON Web Tokens(JWT)을 발행할 수 있는 ID 공급자(IdP)와 [ID 연동(Identity federation)](./identity_federation#외부-서비스-계정)을 사용하여 W&B SDK 및 CLI에서 팀 범위의 외부 서비스 계정을 지원합니다.
