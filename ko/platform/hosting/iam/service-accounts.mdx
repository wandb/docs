---
description: 조직 및 팀 범위의 서비스 계정을 사용하여 자동화 또는 비대화형 워크플로를 관리합니다
displayed_sidebar: default
title: 서비스 계정을 사용해 워크플로 자동화하기
keywords: ["봇 계정", "CI/CD 인증", "자동화된 워크플로", "비대화형 로그인", "서비스 계정 API 키"]
---
...
<Note>
  조직 범위 서비스 계정은 [조직 API 키](/ko/platform/hosting/iam/api-keys#types-of-api-keys)와 다릅니다. 서비스 계정은 자체 키를 보유하는 사람이 아닌 ID입니다. 조직 API 키는 개인에게 속하며 단일 조직 내에서 해당 개인을 인증합니다.
</Note>
...
## 조직 범위 서비스 계정 {#organization-scoped-service-accounts}
...
### 조직 범위 서비스 계정 생성 {#create-an-organization-scoped-service-account}
...
## 팀 범위 서비스 계정 {#team-scoped-service-accounts}
...
<Note>
  [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud) 및 [Self-Managed](/ko/platform/hosting/hosting-options/self-managed) v0.83.0+에서는 관리자가 인스턴스에서 `GORILLA_DISABLE_TEAM_SERVICE_ACCOUNT_CREATION` 환경 변수를 `true`로 설정하여 팀 범위 서비스 계정 생성을 방지할 수 있습니다. [고급 IAM 설정](/ko/platform/hosting/iam/advanced_env_vars)을 참조하세요.
</Note>

### 팀 범위 서비스 계정 생성하기 {#create-a-team-scoped-service-account}

<TeamServiceAccountCreate />

<ApiKeyViewOnceWarning />

### 서비스 계정용 추가 API 키 만들기 {#create-additional-api-keys-for-a-service-account}

<ServiceAccountApiKeyCreateAdditional />

<ApiKeyViewOnceWarning />

### 서비스 계정 API 키 삭제 {#delete-a-service-account-api-key}
...
### 외부 서비스 계정 {#external-service-accounts}
