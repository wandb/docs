...
<Note>
  SCIM 사용 예시를 보여주는 [동영상](https://www.youtube.com/watch?v=Nw3QBqV0I-o)을 시청하세요 (12분)
</Note>
...
조직 수준의 역할을 사용자에게 할당합니다.

<Note>이 작업은 서비스 계정이 아닌 사용자에게만 적용됩니다. 서비스 계정에는 커스텀 역할이 지원되지 않습니다.</Note>
...
IAM에서 SCIM 그룹을 생성하면 해당 그룹에 매핑되는 W&amp;B Team이 생성되며, 이후 SCIM 그룹에 대한 다른 작업은 이 팀을 기준으로 수행됩니다. 팀 생성 시 사용자 지정 저장소를 설정하려면 요청에 `storageBucket`을 포함하세요.
...
`storageBucket` 객체를 포함하면 팀 생성 시 팀 수준의 [사용자 소유 버킷(BYOB)](/ko/platform/hosting/data-security/secure-storage-connector)을 구성할 수 있습니다. 이를 생략하면 팀은 기본 저장소 또는 인스턴스 수준 저장소를 사용합니다. BYOB 가이드를 참고하여 버킷(정책, CORS, 자격 증명)을 프로비저닝하고, provider별 저장소 주소 형식을 확인하세요. `storageBucket` 객체에는 다음 하위 필드가 있습니다:

* **Required**: `name`(버킷 이름), `provider`(`coreweave`, `aws`, `gcp`, `azure`, `minio` 중 하나). 대소문자를 구분합니다.
* **Optional**: `path`(버킷 내 경로 접두사), `kmsKeyId`(암호화를 위한 KMS 키, 예: AWS), `awsExternalId`(AWS 교차 계정 액세스), `azureTenantId`(Azure 테넌트 ID), `azureClientId`(Azure 관리 ID 클라이언트 ID).

W&amp;B는 팀을 생성하기 전에 버킷이 존재하고 접근 가능한지 검증합니다. 검증에 실패하면 SCIM 요청이 실패하고 팀이 생성되지 않습니다.

유효하지 않은 `provider` 값은 허용된 값 목록이 포함된 SCIM 오류와 함께 `400 Bad Request`를 반환합니다.

<div id="examples">
  #### 예시
</div>

다음 예시에서는 사용자 지정 저장소 없이 팀을 생성하는 방법과, 특정 제공자에서 BYOB 저장소를 사용해 팀을 생성하는 방법을 보여줍니다. 예시 요청을 보려면 원하는 저장소 설정의 탭을 선택하고, 예시 응답을 보려면 **응답** 탭을 선택하세요.
