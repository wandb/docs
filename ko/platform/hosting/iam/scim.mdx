...
<Note>
  여러 Enterprise [Multi-tenant Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud) 조직의 관리자라면, API 키를 사용해 전송한 SCIM API 요청이 올바른 조직에 적용되도록 SCIM API 요청을 보낼 조직을 반드시 설정해야 합니다. 프로필 이미지를 클릭한 다음 **User Settings**를 클릭하고 **Default API organization** 설정을 확인하세요.
...
SCIM 사용자 리소스는 W&amp;B 사용자와 서비스 계정에 매핑됩니다. 조직의 사용자와 서비스 계정을 관리하려면 이 엔드포인트를 사용하세요.
...
조직에서 팀 범위 또는 조직 범위의 서비스 계정을 생성합니다. 대신 일반 사용자를 생성하려면 `accountType`을 생략하세요. [사용자 생성](#create-user)을 참조하세요.
...
서비스 계정과 해당 조직 멤버십을 영구 삭제합니다. 이는 하드 삭제이므로 SCIM을 통해 계정을 다시 활성화할 수 없습니다.
...
| 서비스 계정 프로비저닝 (`POST` with `accountType`) | ✓                  | ✓ (v0.81.0+)                     |
| 서비스 계정 프로비저닝 해제 (`DELETE` by id)         | ✓                  | ✓ (v0.81.0+)                     |
