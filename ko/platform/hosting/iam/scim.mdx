...
주요 차이를 검토한 후 사용자 ID 또는 서비스 계정으로 인증할지 선택하세요.

<div id="key-differences">
  ### 주요 차이
</div>

* 사용 대상: Users는 대화형의 일회성 관리자 작업에 가장 적합하며, 서비스 계정은 자동화 및 인테그레이션(CI/CD, 프로비저닝 도구)에 가장 적합합니다.
* 자격 증명: Users는 사용자 이름과 API 키를 전송하고, 서비스 계정은 API 키만 전송합니다(사용자 이름 없음).
* Authorization 헤더 페이로드: Users는 `username:API-KEY`를 인코딩하고, 서비스 계정은 `:API-KEY`를 인코딩합니다(앞에 콜론이 붙음).
* 범위 및 권한: 둘 다 관리자 권한이 필요합니다. 서비스 계정은 조직 범위로 제한되며 헤드리스 방식이므로, 자동화에 대해 더 명확한 감사 추적을 제공합니다.
* 자격 증명을 가져오는 위치: Users는 User Settings에서 API 키를 복사하고, 서비스 계정 키는 조직의 Service account 탭에서 확인할 수 있습니다.
...
<Note>
  ...
  * `full` 시트 대신 Models `viewer` 시트가 부여됩니다. 그러면 Models에는 보기 전용 액세스가, 레지스트리에는 전체 액세스가 허용됩니다. 사용 가능한 Models 시트가 없으면 `Seat limit reached` 오류가 로깅되고, 해당 멤버는 Models 액세스 없이 추가됩니다. 시트를 사용할 수 있게 되면 나중에 업데이트할 수 있습니다.
  * `full` 시트 대신 Weave `viewer` 시트가 부여됩니다. 그러면 Weave에는 보기 전용 액세스가 허용됩니다. 사용 가능한 Weave 시트가 없으면 `Seat limit reached` 오류가 로깅되고, 해당 멤버는 Weave 액세스 없이 추가됩니다. 시트를 사용할 수 있게 되면 나중에 업데이트할 수 있습니다.
...
* **필수**: `name` (bucket 이름), `provider` (다음 중 하나: `coreweave`, `aws`, `gcp`, `azure`, `minio`). 대소문자를 구분합니다.
...
이 예시에서는 맞춤형 저장소 없이 팀을 생성하는 방법과 특정 provider에서 BYOB 저장소를 사용해 팀을 생성하는 방법을 보여줍니다. 예시 요청을 보려면 원하는 저장소 설정 탭을 선택하고, 예시 응답을 보려면 **응답** 탭을 선택하세요.
...
<Tabs>
  ...
  <Tab title="응답">
    ...
    ```json
    {
        ...
        "id": "Um9sZTo3",
        "inheritedFrom": "member", // 사전 정의된 역할임을 나타냄
        "meta": {
            "resourceType": "Role",
       ...
       "Resources": [
            {
                "description": "A sample custom role for example",
                "id": "Um9sZTo3",
                "inheritedFrom": "member", // 커스텀 역할이 상속하는 사전 정의 역할을 나타냅니다
                "meta": {
                    "resourceType": "Role",
                    "created": "2023-11-20T23:10:14Z",
                    "lastModified": "2023-11-20T23:31:23Z",
                ...
                "permissions": [
                    {
                        "name": "artifact:read",
                        "isInherited": true // 사전 정의 member 역할에서 상속됨
                    },
                    ...
                    ...
                    {
            ...
            },
            {
                ...
                "id": "Um9sZToxMg==",
                "inheritedFrom": "viewer", // 커스텀 역할이 상속하는 사전 정의 역할을 나타냅니다
                "meta": {
                    "resourceType": "Role",
                ...
                "permissions": [
                    {
                        ...
                        "isInherited": true // 사전 정의 viewer 역할에서 상속됨
                    },
                    ...
                    ...
                    {
