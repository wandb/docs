...
description: "Okta, Azure AD, AWS Cognito와 같은 ID 공급자를 사용해 W&B 인스턴스에서 OpenID Connect로 SSO를 구성합니다."
---
...
W&amp;B는 외부 ID 공급자(IdP)와의 통합을 위해 다음 OIDC 인증 플로우를 지원합니다.
...
W&amp;B의 맥락에서 액세스 토큰은 사용자 대신 API Request를 인가하지만, W&amp;B의 주된 관심사는 사용자 인증과 ID이므로 ID 토큰만 필요합니다.

환경 변수를 사용해 [IAM 옵션을 구성](/ko/platform/hosting/iam/advanced_env_vars)하여 [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud) 또는 [Self-Managed](/ko/platform/hosting/hosting-options/self-managed) 인스턴스를 설정할 수 있습니다.

[Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud) 또는 [Self-Managed](/ko/platform/hosting/hosting-options/self-managed) W&amp;B 설치에서 ID 공급자를 구성하는 데 도움이 되도록, 다양한 IdP에 대해 아래 지침을 따르십시오. W&amp;B의 SaaS 버전을 사용하는 경우, 조직용 Auth0 테넌트 구성을 지원받으려면 [support@wandb.com](mailto:support@wandb.com)으로 문의하십시오.

<div id="configure-your-idp">
  ## IdP 구성하기
</div>

이 섹션에서는 OIDC용 ID 공급자(IdP)를 구성하는 방법을 설명합니다. 사용하는 IdP에 해당하는 탭을 선택해 자세한 내용을 확인하세요.
...
<Tabs>
  ...
  <Tab title="Entra">
    ...
    * [공용 클라이언트](#public-client): 클라이언트 시크릿 없이 PKCE를 사용합니다. 설정이 더 간단하고 대부분의 배포 환경에 적합합니다.
    * [기밀 클라이언트](#confidential-client): 추가 보안을 위해 클라이언트 시크릿과 함께 PKCE를 사용합니다. `GORILLA_OIDC_SECRET` 환경 변수를 설정해야 한다면 필요합니다.

    <Warning>
      설정을 혼용하지 마십시오. Azure AD에서 &quot;Single-page application&quot;을 선택한 경우 클라이언트 시크릿을 지정하지 마십시오. 클라이언트 시크릿이 필요하다면 플랫폼 유형으로 &quot;Web&quot;을 선택해야 합니다.
    </Warning>

    <AccordionGroup>
      <Accordion title="공용 클라이언트" defaultOpen="true">
      ...
      <Accordion title="기밀 클라이언트">
        ...
        7. 클라이언트 시크릿을 생성합니다:
           ...
           * 시크릿에 대한 설명을 추가합니다.
           * 만료 기간을 선택합니다.
           * **Add**를 클릭합니다. <Warning>시크릿의 **Value**를 즉시 복사해 저장하세요(Secret ID가 아닙니다)</Warning>.
           <Frame>
