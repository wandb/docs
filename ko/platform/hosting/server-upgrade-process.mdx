...
<Warning>
`wandb` Helm 차트는 더 이상 지원되지 않으며 사용이 중단되었습니다. 이 차트는 단일 Pod를 배포하는 방식으로, [W&B Kubernetes Operator](/platform/hosting/self-managed/operator)로 대체되었습니다. 이 차트를 사용 중이라면 [마이그레이션 가이드](/platform/hosting/self-managed/operator#migrate-self-managed-instances-to-wb-operator)를 따라 Operator로 마이그레이션하세요.
</Warning>
