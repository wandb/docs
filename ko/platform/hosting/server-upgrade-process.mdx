...
| 릴리스 유형 | 설명 |
| ---------------- | ------------------ |
| [Terraform](#terraform으로-업데이트하기) | W&amp;B는 클라우드 배포를 위해 세 가지 공용 Terraform 모듈을 지원합니다: [AWS](https://registry.terraform.io/modules/wandb/wandb/aws/latest), [Google Cloud](https://registry.terraform.io/modules/wandb/wandb/google/latest), [Azure](https://registry.terraform.io/modules/wandb/wandb/azurerm/latest). |
| [Helm](#helm으로-업데이트하기) | [Helm Chart](https://github.com/wandb/helm-charts)를 사용하여 기존 Kubernetes 클러스터에 W&amp;B를 설치할 수 있습니다. |
...
<Warning>
  `wandb` Helm 차트는 사용 중단되었으며 더 이상 지원되지 않습니다. 이 차트는 단일 Pod를 배포했으며, 현재는 [W&amp;B Kubernetes Operator](/ko/platform/hosting/self-managed/operator)로 대체되었습니다. 아직 이 차트를 사용 중이라면 [마이그레이션 가이드](/ko/platform/hosting/self-managed/operator#migrate-self-managed-instances-to-wb-operator)를 따라 Operator로 이전하세요.
</Warning>

<Warning>
  `wandb` Helm 차트는 더 이상 지원되지 않으며 사용이 중단되었습니다. 이 차트는 단일 Pod를 배포하는 방식으로, [W&amp;B Kubernetes Operator](/ko/platform/hosting/self-managed/operator)로 대체되었습니다. 이 차트를 사용 중이라면 [마이그레이션 가이드](/ko/platform/hosting/self-managed/operator#migrate-self-managed-instances-to-wb-operator)를 따라 Operator로 마이그레이션하세요.
</Warning>
...
이 방법은 W&amp;B 서버 컨테이너에서 환경 변수로 설정되지 않은 라이선스를 업데이트할 때만 사용할 수 있으며, 일반적으로 Self-Managed Docker 설치에 해당합니다.

1. [W&amp;B Deployment Page](https://deploy.wandb.ai/)에서 새 라이선스를 발급받고, 업그레이드하려는 배포의 올바른 organization 및 deployment ID와 일치하는지 확인합니다.
2. W&amp;B App의 **License** 페이지로 이동합니다. **Settings** &gt; **License**를 클릭하거나 `<host-url>/console/settings/license`로 이동합니다.
