...
Bring your own bucket(BYOB)을 사용하면 W&B 아티팩트 및 기타 관련 민감한 데이터를 자체 클라우드 또는 온프레미스 인프라에 저장할 수 있습니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 또는 [SaaS Cloud](/platform/hosting/hosting-options/saas_cloud)의 경우 버킷에 저장하는 데이터는 W&B 관리 인프라에 복사되지 않습니다.
...
<Note>
팀 수준 BYOB에 대한 S3 호환 스토리지 연결은 [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)에서 사용할 수 없습니다. 또한 팀 수준 BYOB에 대한 AWS 버킷 연결은 해당 인스턴스가 GCP에 있으므로 [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)에서 크로스 클라우드입니다. 해당 크로스 클라우드 연결은 이전에 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 및 [자체 관리](/ko/platform/hosting/hosting-options/self-managed) 인스턴스에 대해 설명한 대로 엑세스 키 및 환경 변수 기반 메커니즘을 사용하지 않습니다.
</Note>
...
1. KMS 키 프로비저닝
    ...
    [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud) 또는 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 `<aws_principal_and_role_arn>`을 해당 값으로 바꿉니다.

    * [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud): `arn:aws:iam::725579432336:role/WandbIntegration`
...
2. [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud) 또는 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 W&B 플랫폼에 연결된 GCP 서비스 계정에 `Storage Admin` 역할을 부여합니다.

    * [SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)의 경우 계정은 `wandb-integration@wandb-production.iam.gserviceaccount.com`입니다.
...
9. ([SaaS Cloud](/ko/platform/hosting/hosting-options/saas_cloud)에서 선택 사항) 팀을 만들 때 팀 멤버를 초대할 수도 있습니다.
10. **팀 만들기** 버튼을 누릅니다.
