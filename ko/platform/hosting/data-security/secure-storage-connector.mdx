...
Bring your own bucket(BYOB)을 사용하면 W&B 아티팩트 및 기타 관련 민감한 데이터를 자체 클라우드 또는 온프레미스 인프라에 저장할 수 있습니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 또는 [SaaS Cloud](/platform/hosting/hosting-options/multi_tenant_cloud)의 경우 버킷에 저장하는 데이터는 W&B 관리 인프라에 복사되지 않습니다.

<Note>
* W&B SDK / CLI / UI와 버킷 간의 통신은 [사전 서명된 URL](/ko/platform/hosting/data-security/presigned-urls/)을 사용하여 이루어집니다.
* W&B는 가비지 컬렉션 프로세스를 사용하여 W&B Artifacts를 삭제합니다. 자세한 내용은 [Artifacts 삭제](/ko/models/artifacts/delete-artifacts/)를 참조하세요.
...
<Note>
팀 수준 스토리지 버킷은 특히 다양한 사업부 및 부서가 인프라 및 관리 리소스를 효율적으로 활용하기 위해 인스턴스를 공유할 때 [자체 관리](/ko/platform/hosting/hosting-options/Self-Managed) 인스턴스에 대해 동일한 이점을 제공합니다. 이는 별도의 고객 참여에 대한 AI 워크플로우를 관리하는 별도의 프로젝트 팀이 있는 회사에도 적용됩니다.
</Note>
...
[전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 또는 [자체 관리](/ko/platform/hosting/hosting-options/Self-Managed) 인스턴스에서 팀 수준 BYOB에 대해 다른 클라우드의 클라우드 네이티브 스토리지 버킷 또는 [MinIO](https://github.com/minio/minio)와 같은 S3 호환 스토리지 버킷에 연결할 수 있습니다.
...
<Note>
팀 수준 BYOB에 대한 S3 호환 스토리지 연결은 [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)에서 사용할 수 없습니다. 또한 팀 수준 BYOB에 대한 AWS 버킷 연결은 해당 인스턴스가 GCP에 있으므로 [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)에서 크로스 클라우드입니다. 해당 크로스 클라우드 연결은 이전에 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 및 [자체 관리](/ko/platform/hosting/hosting-options/Self-Managed) 인스턴스에 대해 설명한 대로 엑세스 키 및 환경 변수 기반 메커니즘을 사용하지 않습니다.
</Note>
...
1. KMS 키 프로비저닝
    ...
    [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud) 또는 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 `<aws_principal_and_role_arn>`을 해당 값으로 바꿉니다.

    * [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud): `arn:aws:iam::725579432336:role/WandbIntegration`
    * [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud): `arn:aws:iam::830241207209:root`
...
2. S3 버킷 프로비저닝
    ...
    5. 클라우드 인프라 또는 사용자 브라우저의 AI 워크로드가 버킷에 엑세스하는 데 사용하는 [사전 서명된 URL](/ko/platform/hosting/data-security/presigned-urls/)을 생성하는 데 필요한 권한인 W&B 플랫폼을 호스팅하는 AWS 계정에 필요한 S3 권한을 부여합니다.
        ...
        `<wandb_bucket>`을 적절하게 바꾸고 버킷 이름을 기록해 둡니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 인스턴스 수준 BYOB의 경우 버킷 이름을 W&B 팀과 공유합니다. 모든 배포 유형에서 팀 수준 BYOB의 경우 [팀을 만드는 동안 버킷을 구성합니다](#configure-byob-in-wb).
  ...
  자세한 내용은 [AWS 자체 관리 호스팅 가이드](/ko/platform/hosting/Self-Managed/aws-tf/)를 참조하십시오.
</Tab>
<Tab title="GCP">
1. GCS 버킷 프로비저닝
...
2. [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud) 또는 [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 W&B 플랫폼에 연결된 GCP 서비스 계정에 `Storage Admin` 역할을 부여합니다.

    * [SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)의 경우 계정은 `wandb-integration@wandb-production.iam.gserviceaccount.com`입니다.
    * [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)의 경우 계정은 `deploy@wandb-production.iam.gserviceaccount.com`입니다.

    버킷 이름을 기록해 둡니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 인스턴스 수준 BYOB의 경우 버킷 이름을 W&B 팀과 공유합니다. 모든 배포 유형에서 팀 수준 BYOB의 경우 [팀을 만드는 동안 버킷을 구성합니다](#configure-byob-in-wb).
</Tab>
<Tab title="Azure">
...
2. 스토리지 계정 엑세스 키를 생성하고 스토리지 계정 이름과 함께 기록해 둡니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 보안 공유 메커니즘을 사용하여 스토리지 계정 이름과 엑세스 키를 W&B 팀과 공유합니다.

    팀 수준 BYOB의 경우 W&B는 필요한 엑세스 메커니즘 및 권한과 함께 Azure Blob Storage 버킷을 프로비저닝하기 위해 [Terraform](https://github.com/wandb/terraform-azurerm-wandb/tree/main/modules/secure_storage_connector)을 사용하는 것이 좋습니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud)를 사용하는 경우 인스턴스에 대한 OIDC 발급자 URL을 제공합니다. [팀을 만드는 동안 버킷을 구성](#configure-byob-in-wb)하는 데 필요한 세부 정보를 기록해 둡니다.
...
<Tab title="팀 수준">
<Note>
[전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 또는 [자체 관리](/ko/platform/hosting/hosting-options/Self-Managed) 인스턴스에서 팀 수준 BYOB에 대해 다른 클라우드의 클라우드 네이티브 스토리지 버킷 또는 [MinIO](https://github.com/minio/minio)와 같은 S3 호환 스토리지 버킷에 연결하는 경우 [팀 수준 BYOB에 대한 크로스 클라우드 또는 S3 호환 스토리지](#cross-cloud-or-s3-compatible-storage-for-team-level-byob)를 참조하십시오. 이러한 경우 아래 지침을 사용하여 팀에 대해 구성하기 전에 W&B 인스턴스에 대한 `GORILLA_SUPPORTED_FILE_STORES` 환경 변수를 사용하여 스토리지 버킷을 지정해야 합니다.
</Note>
...
5. **이름** 필드에 스토리지 버킷 이름을 입력합니다. [전용 클라우드](/ko/platform/hosting/hosting-options/dedicated_cloud) 또는 Azure의 [자체 관리](/ko/platform/hosting/hosting-options/Self-Managed) 인스턴스가 있는 경우 **계정 이름** 및 **컨테이너 이름** 필드에 값을 입력합니다.
...
9. ([SaaS Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)에서 선택 사항) 팀을 만들 때 팀 멤버를 초대할 수도 있습니다.
10. **팀 만들기** 버튼을 누릅니다.
