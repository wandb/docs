...
description: "사용자 소유 버킷(BYOB) 기능을 사용하여 W&B 아티팩트와 데이터를 사용자의 클라우드 저장소 버킷에 저장합니다."
---
...
사용자 소유 버킷(BYOB)을 사용하면 W&amp;B Artifacts 및 기타 관련 민감한 데이터를 자체 클라우드 또는 온프레미스 인프라에 저장할 수 있습니다. [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud) 또는 [Multi-tenant Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)의 경우, 버킷에 저장한 데이터는 W&amp;B에서 관리하는 인프라로 복사되지 않습니다.
...
<Note>
  ...
  * 버킷을 구성할 때 하위 경로(sub-path)를 지정하여, 버킷 루트의 폴더에 W&amp;B가 파일을 저장하지 않도록 할 수 있습니다. 이는 조직의 버킷 거버넌스 정책을 더 잘 준수하는 데 도움이 될 수 있습니다.
</Note>
...
| Scope          | Description                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Instance level | [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud) 및 [Self-Managed](/ko/platform/hosting/hosting-options/self-managed) 환경에서는 조직 또는 인스턴스 내에서 필요한 권한을 가진 모든 사용자가 인스턴스 스토리지 버킷에 저장된 파일에 접근할 수 있습니다. [Multi-tenant Cloud](/ko/platform/hosting/hosting-options/multi_tenant_cloud)에는 적용되지 않습니다.                                                                                            |
...
<Tabs>
  ...
  <Tab title="Azure">
    ...
    **인스턴스 수준 BYOB**:
    ...
    2. Storage account access key를 생성하고, 해당 키 이름과 storage account 이름을 기록해 둡니다. [Dedicated Cloud](/ko/platform/hosting/hosting-options/dedicated-cloud)를 사용하는 경우, 보안성이 확보된 공유 메커니즘을 사용해 storage account 이름과 access key를 W&amp;B 팀과 공유하세요.
...
<Note>
  인스턴스 수준의 CoreWeave AI Object Storage에 대해서는 아래 지침을 따르지 말고 [W&amp;B support](mailto:support@wandb.com)에 문의하세요. 셀프서비스 설정은 아직 지원되지 않습니다.
</Note>

**Dedicated Cloud**의 경우: 버킷 세부 정보를 W&amp;B 팀과 공유하면 Dedicated Cloud 인스턴스를 설정해 줍니다.
...
W&amp;B App에서 팀을 생성할 때 또는 [SCIM API](/ko/platform/hosting/iam/scim#create-team)를 사용하여(`storageBucket` 선택 항목이 포함된 POST Groups) 팀 수준 BYOB를 설정할 수 있습니다. 다음 두 가지 옵션 중에서 선택할 수 있습니다:
