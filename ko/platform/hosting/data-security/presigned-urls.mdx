...
각 사전 서명된 URL은 W&B platform의 [팀 레벨 엑세스 제어](/ko/platform/hosting/iam/access-management/manage-organization#add-and-manage-teams)를 기반으로 특정 버킷으로 제한됩니다. 사용자가 [보안 스토리지 커넥터](/ko/platform/hosting/data-security/secure-storage-connector/)를 사용하여 blob storage 버킷에 매핑된 팀에 속하고 해당 사용자만 해당 팀에 속한 경우, 해당 요청에 대해 생성된 사전 서명된 URL은 다른 팀에 매핑된 blob storage 버킷에 엑세스할 수 있는 권한이 없습니다.
