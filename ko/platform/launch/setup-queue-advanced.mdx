...
keywords: ["큐 템플릿", "리소스 가드레일", "동적 매크로", "최소/최대 리소스"]
---
...
## 큐 설정 템플릿 설정 {#set-up-queue-config-templates}
...
### 큐 템플릿 설정 {#configure-queue-template}
...
<Frame>
  <img src="/images/launch/aws_template_example.png" alt="AWS 인스턴스 유형 옵션이 있는 Launch queue 템플릿 타일" />
</Frame>

## launch 작업 동적 구성 {#dynamically-configure-launch-jobs}
...
## Launch agent를 사용하여 가속기 또는 GPU에서 실행할 이미지를 구축하세요 {#use-the-launch-agent-to-build-images-that-execute-on-accelerators-or-gpus}
...
### TensorFlow에서 GPU 사용 {#use-gpus-with-tensorflow}
