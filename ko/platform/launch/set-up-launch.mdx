---
title: Launch 설정
description: "큐를 구성하고 에이전트를 배포하여 컴퓨트 인프라에 작업을 제출할 수 있도록 W&B Launch를 설정합니다."
---

이 페이지에서는 W&amp;B Launch를 설정하는 데 필요한 상위 수준의 단계를 설명합니다:

1. **큐 설정**: 큐는 선입선출(FIFO) 방식이며, 큐 구성을 가집니다. 큐의 구성은 대상 리소스에서 작업이 어디서 어떻게 실행되는지를 제어합니다.
2. **에이전트 설정**: 에이전트는 사용자의 머신/인프라에서 실행되며, 하나 이상의 큐를 폴링하여 Launch 작업을 가져옵니다. 작업이 가져와지면 에이전트는 이미지가 빌드되어 사용 가능한 상태인지 확인합니다. 그런 다음 에이전트는 작업을 대상 리소스에 제출합니다.
