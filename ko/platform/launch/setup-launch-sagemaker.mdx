...
keywords: ["SageMaker 트레이닝", "CreateTrainingJob", "AWS launch", "ECR 레지스트리", "SageMaker 큐"]
---
...
### Launch 에이전트가 도커 이미지를 빌드할지 결정합니다 {#decide-if-you-want-the-launch-agent-to-build-a-docker-image}
...
<div id="set-up-aws-resources">
  ### AWS 리소스 설정 {#set-up-aws-resources}
</div>
...
### Launch 에이전트용 IAM 정책 만들기 {#create-an-iam-policy-for-the-launch-agent}
...
<div id="create-an-iam-role-for-launch-agent">
  ### Launch 에이전트용 IAM 역할 생성 {#create-an-iam-role-for-the-launch-agent}
</div>
...
## SageMaker용 Launch 큐 설정 {#configure-the-launch-queue-for-sagemaker}
...
## Launch 에이전트 설정 {#set-up-the-launch-agent}
...
### Launch 에이전트를 어디에서 실행할지 결정하세요 {#decide-where-to-run-the-launch-agent}
...
### Launch 에이전트 설정 {#configure-a-launch-agent}
...
## 선택 사항: Launch 작업 도커 이미지를 Amazon ECR에 푸시 {#optional-push-your-launch-job-docker-image-to-amazon-ecr}
