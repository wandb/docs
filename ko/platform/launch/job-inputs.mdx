...
keywords: ["작업 설정", "하이퍼파라미터 재정의", "설정 파일 입력", "JSON 스키마", "Pydantic 스키마"]
---
...
## `Run` 객체 재구성하기 {#reconfigure-the-run-object}
...
### run config 입력값에 액세스하기 {#access-run-config-inputs}
...
### 설정 파일 입력값에 액세스 {#access-config-file-inputs}
...
## 작업의 Launch drawer UI 맞춤 설정 {#customize-a-jobs-launch-drawer-ui}
