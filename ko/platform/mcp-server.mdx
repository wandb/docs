...
keywords: ["MCP 도구", "Cursor MCP", "Claude MCP", "AI 도우미 인테그레이션", "자연어 쿼리"]
---
...
* W&amp;B는 `mcp` SDK 버전을 릴리스 `2024-11-05`의 [1.14.0](https://pypi.org/project/mcp/1.14.0/)으로 고정합니다. 클라이언트는 `mcp` SDK 1.14.x를 사용하여 연결해야 합니다. W&amp;B Dedicated Cloud에서 스트리밍 가능 HTTP를 지원하려면 릴리스 `2025-03-26` 이상의 `mcp` SDK 1.14.x가 필요합니다.

## W&amp;B 호스팅 서버 사용 {#use-the-hosted-server}

W&amp;B는 모든 배포 유형에 대해 관리형 MCP 서버를 운영합니다. 별도로 설치할 필요는 없습니다. `Authorization` 헤더에 W&amp;B API 키를 포함해 HTTP로 연결하도록 클라이언트를 설정하세요.

### 연결 URL {#connection-url}
...
### 로컬 사전 요구 사항 {#local-prerequisites}
...
### 클라이언트 설정 {#configure-your-client}
...
### HTTP 전송으로 서버 실행 {#run-the-server-with-http-transport}
...
## W&amp;B MCP 서버 기능 {#wb-mcp-server-capabilities}
...
### 스키마 우선 트레이스 쿼리 {#schema-first-trace-queries}
...
### 트레이스 중심 워크플로의 경우 {#for-trace-heavy-workflows}
...
### run 중심 워크플로의 경우 {#for-run-heavy-workflows}
...
### Dedicated Cloud 및 Self-Managed의 경우 {#for-dedicated-cloud-and-self-managed}
...
### 낯선 프로젝트 살펴보기 {#explore-an-unfamiliar-project}
...
### 실패한 LLM calls 선별하기 {#triage-failing-llm-calls}
...
### 문제가 있는 트레이닝 run 진단하기 {#diagnose-a-bad-training-run}
...
### 평가를 요약하고 모델 버전 비교하기 {#summarize-evals-and-compare-model-versions}
