---
title: "W&B SDK 릴리스"
description: "새 기능, 버그 수정 및 버전별 주요 변경 사항을 포함한 W&B Python SDK 릴리스 노트를 확인합니다."
rss: true
---

이 페이지에서는 W&amp;B Models와 함께 사용하는 W&amp;B Python SDK(`wandb` 패키지)의 릴리스 노트를 제공합니다. Weave Python SDK(`weave` 패키지)는 [Weave SDK 릴리스](/ko/release-notes/weave-sdk-releases)를 참조하세요. W&amp;B Server는 [W&amp;B Server 릴리스 노트](/ko/release-notes/server-releases)를 참조하세요.

<Update label="0.30.0" description="2026년 9월 8일">
  <div id="notable-changes">
    ## 주요 변경 사항
  </div>

  * `wandb.InternalApi`, `wandb.PublicApi` 및 `wandb.apis.PublicApi` 별칭, 그리고 `wandb.apis.internal`과 `wandb.sdk.internal.internal_api` 모듈이 제거되었습니다. 대신 `wandb.Api()`를 사용하세요.
  * `wandb.api`와 `wandb.ensure_configured()`는 사용이 중단되었습니다. 사용자의 로그인 여부를 확인하려면 `wandb.login(prompt=False)`를 사용하고, 그 외의 모든 경우에는 `wandb.Api()`를 사용하세요.
  * 레거시 `wandb sync` 옵션이 제거되었습니다. `wandb sync --sync-all` 대신 인수 없이 `wandb sync`를 실행하고, `--job_type` 대신 `--job-type`을 사용하세요. 현재 사용할 수 있는 옵션 목록을 확인하려면 `wandb sync --help`를 실행하세요.

  <div id="added">
    ## Added
  </div>

  * `wandb.login(prompt=False)`는 설정된 credential이 있으면 이를 사용하고, 없으면 API 키를 요청하거나 offline mode로 전환하지 않고 `False`를 반환합니다. 흐름을 차단하지 않고 사용자의 로그인 여부를 확인하려면 이 방식을 사용하세요.
  * Automations API는 `SendPromptToAria`를 사용해 ARIA에 prompt를 전송하는 자동화 액션을 지원합니다. 자세한 내용은 [ARIA 자동화 만들기](/ko/models/automations/create-automations/aria)를 참조하세요.
  * W&amp;B LEET TUI 특성:
    * 차트는 `run.define_metric()`으로 설정하는 맞춤형 x축을 지원하며, `run.define_metric("train/*", step_metric="train/step")`과 같은 glob 정의도 사용할 수 있습니다. 차트 헤더에는 축 이름이 `[x: train/step]` 형식으로 표시됩니다. 이는 로컬 `.wandb` 파일에서 여는 run에도 적용됩니다.
    * 콘솔 로그를 필터링하려면 로그 패널에 포커스를 두고 `/`를 누른 다음 정규 표현식을 입력하거나, **Tab**을 누르고 glob 패턴을 입력하세요. 일치하는 줄만 표시되며, 이후 새로 들어오는 일치 항목도 계속 추적됩니다. 필터를 지우려면 `Ctrl+/`를 누르세요.
    * 메트릭, 시스템 메트릭, Runs 필터는 `wandb` 디렉토리별로 저장되어, 다음에 해당 디렉토리를 열 때 Workspace와 단일 run 뷰 모두에 다시 적용됩니다. 필터는 해당 디렉토리 내의 `.wandb-leet.json`에 저장됩니다. 필터를 지우면 저장된 필터에서도 제거됩니다.
    * 패널이나 사이드바 크기를 조정하기 위해 드래그하는 경계선 또는 구분선은 드래그하는 동안 강조 표시됩니다.

  <div id="changed">
    ## 변경 사항
  </div>

  * `wandb status --settings`는 더 이상 API 키를 출력하지 않으며, `wandb projects`는 더 이상 프로젝트 설명을 출력하지 않습니다.
  * Apple Silicon Mac에서 system metrics가 run 시작 후 약 1.5초 더 빨리 수집을 시작합니다.
  * W&amp;B LEET TUI가 긴 run에서 더 빨라졌습니다. 메트릭 100개와 400,000 step으로 이루어진 run이 10배 빠르게 열리고, frame 렌더링이 약 30% 빨라졌으며, 실시간 chart 업데이트 시 모든 점을 다시 그리지 않습니다.

  <div id="deprecated">
    ## 사용 중단
  </div>

  * `wandb.api`와 `wandb.ensure_configured()`는 사용 중단되었으며 향후 릴리스에서 제거될 예정입니다. 이제 `wandb.api`는 `api_key`, `default_entity`, `viewer()`만 제공합니다. 사용자의 로그인 여부를 확인하려면 `wandb.login(prompt=False)`를 사용하고, 그 외의 모든 작업에는 `wandb.Api()`를 사용하세요.
  * `wandb.sandbox`와 `wandb beta sandbox` 명령어는 사용 중단되었으며 향후 릴리스에서 제거될 예정입니다. `cwsandbox` 패키지를 직접 사용하세요.
  ...
  * `wandb.InternalApi`, `wandb.PublicApi` 및 `wandb.apis.PublicApi` 별칭, 그리고 `wandb.apis.internal`과 `wandb.sdk.internal.internal_api` 모듈이 제거되었습니다. 대신 `wandb.Api()`를 사용하세요.
  * 레거시 `wandb sync` 옵션이 제거되었습니다. `wandb sync --sync-all` 대신 인수 없이 `wandb sync`를 실행하고, `--job_type` 대신 `--job-type`을 사용하세요.

  <div id="fixed">
    ## 수정 사항
  </div>

  * 20초 이상 걸리는 파일 업로드 및 다운로드가 더 이상 `CommError: Failed to execute API request: the service process is busy and did not respond in time` 오류로 실패하지 않습니다. v0.29.0에서 발생한 회귀 문제였습니다.
  * W&amp;B 서버의 응답이 느릴 때 `wandb.init()`과 `wandb.Api()`가 더 이상 최대 10초간 멈추지 않습니다. v0.29.0에서 발생한 회귀 문제였습니다.
  * 아티팩트 사용량 한도와 같은 플랜 또는 쿼터 한도를 나타내는 HTTP `402 Payment Required`로 거부된 요청은 더 이상 재시도하지 않습니다. 오류가 즉시 보고됩니다.
  * 아티팩트가 이미 업로드된 run을 동기화할 때, 해당 아티팩트 파일이 디스크에 남아 있지 않아도 더 이상 실패하지 않습니다. v0.29.0에서 발생한 회귀 문제였습니다.
  * 아티팩트 업로드가 실패해도 업로드 중이던 파일을 더 이상 삭제하지 않으므로 `wandb sync`로 재시도할 수 있습니다. 업로드에 성공한 후에는 아티팩트의 스테이징 디렉터리에 있는 파일만 정리됩니다.
  * `.wandb` 파일이 불완전한 레코드로 끝나는 경우(run이 아직 기록 중이거나 기록 도중에 run이 갑작스럽게 중단되면 발생), 이제 `wandb sync`가 내부 오류를 보고하는 대신 어떻게 해야 하는지 안내합니다.
  * Public API에서 `Run.log_artifact()`에 `aliases` 또는 `tags`를 전달해도 더 이상 서버 오류로 실패하지 않습니다.
  * [0, 255] 범위를 벗어난 값이 포함된 부동소수점 배열로 전달한 `wandb.Image` 마스크는 이제 정수 배열과 마찬가지로 `TypeError`를 발생시키며, 더 이상 올바르지 않은 클래스 ID로 저장되지 않습니다.
  * 시스템 메트릭이 기본값 15초인 샘플링 간격이 한 번 지난 후가 아니라 run이 시작되는 즉시 수집됩니다. 이제 짧은 run도 시스템 메트릭을 기록하며, 시스템 차트가 빈 구간으로 시작하지 않습니다.
  * `wandb.init()`이 GPU 및 TPU 메트릭 collector가 시작될 때까지 최대 5초간 기다리지 않습니다.
  * Jupyter Notebook에서 시스템 메트릭은 셀이 실행되는 동안 수집되고 셀 사이에는 일시 중지됩니다. v0.19.10부터는 이 둘이 뒤바뀌어 노트북이 유휴 상태일 때만 시스템 메트릭이 수집되었습니다.
  * AMD GPU가 장착된 머신에서 시스템 메트릭 모니터링이 1~3초가 걸리던 `rocm-smi`를 미리 실행하지 않으며, `rocm-smi`가 GPU를 보고하지 않을 때 더 이상 크래시가 발생하지 않습니다.
  * Apple Silicon Mac의 시스템 메트릭:
    * CPU 사용량, 주파수, 전력이 이제 약 10밀리초 스냅샷이 아니라 샘플링 간격 전체를 기준으로 측정됩니다. 기존 스냅샷 방식에서는 사용량과 주파수가 불안정하게 나타나고 전력이 약 25% 과다 보고되었습니다.
    * RAM이 256 GB 이상인 Mac에서 메모리 크기가 0 GB로 보고되던 문제가 해결되었습니다.
    * CPU 및 GPU 온도가 잘못된 센서 측정값으로 인해 왜곡되던 문제가 해결되었습니다.
    * M5 Pro 및 M5 Max Mac에서 효율 코어 수가 0으로 보고되던 문제가 해결되었습니다.
    * MacBook Neo의 CPU 주파수가 약 1,000배 높게 보고되던 문제가 해결되었습니다.
    * 그동안 누락되어 있던 M5 Mac의 효율 코어와 M5 Pro 및 M5 Max Mac의 성능 코어에 대해서도 이제 CPU 사용량과 주파수를 수집합니다.
  * W&amp;B LEET TUI:
    * `wandb leet`는 시작할 수 없는 경우(예: 사용 가능한 터미널이 없는 경우) 출력 없이 상태 1로 종료하지 않고 오류를 출력합니다. 디버그 로그(`WANDB_DEBUG=true`)는 현재 디렉토리가 아니라 LEET 설정 파일과 같은 위치에 기록됩니다.
    * `wandb leet`는 SDK의 다른 기능과 마찬가지로 오프라인 또는 disabled 모드(`WANDB_MODE=offline` 또는 `WANDB_MODE=disabled`)에서 더 이상 사용 telemetry를 전송하지 않습니다.
    * 좁은 터미널에서는 차트 너비가 24열 미만이 되는 경우 Workspace가 run 개요 사이드바를 숨기고, 그래도 부족하면 Runs 목록까지 숨깁니다. 이전에는 80열 터미널에서 차트가 두 사이드바 사이에 한 열짜리 조각으로 표시되었습니다.
    * 차트의 최상단 y축 레이블이 다른 레이블보다 길어도 더 이상 잘리지 않습니다. 예를 들어 이제 레이블이 `+03` 대신 `1.09e+03`으로 표시됩니다.
</Update>

<Update label="0.29.0" description="2026년 8월 26일">
  <div id="notable-changes-2">
    ## 주요 변경 사항
  </div>

  * macOS에서 `wandb`를 사용하려면 이제 macOS 13(Ventura) 이상이 필요합니다.
  * `wandb.Image`는 더 이상 픽셀 값을 정규화하지 않습니다. 데이터가 이미 [0, 255] 범위 내에 있는지 확인하세요.
  * NumPy 배열 또는 IO 객체로 `wandb.Video()`를 초기화할 때 이제 `format` 인수가 필요합니다.
  * `Api.runs()`, `Api.sweeps()`, `run.files()`와 같은 페이지 매김된 객체의 `.length` 속성이 제거되었습니다. 대신 `len(...)`을 사용하세요.
  * 다음 사용 중단된 API가 제거되었습니다. `start_method` 설정, `run.define_metric()`의 `summary="best"` 옵션 및 `goal` 인수, `run.finish()` 및 `wandb.finish()`의 `quiet` 인수, `run.project_name()`, `run.get_url()`, `run.get_project_url()`, `run.get_sweep_url()` 메서드입니다.

  <div id="added-2">
    ## 추가됨
  </div>

  * 새로운 Public API 메서드 `run.console_logs()`는 완료된 run과 활성 run 모두에서 W&amp;B가 캡처한 콘솔 출력을 반환합니다. 마지막 N줄만 반환하려면 `last=N`을 전달하세요. 로그의 처음부터 읽으려면 W&amp;B Server v0.77 이상이 필요합니다.
  * Automations API는 팀 및 조직 범위를 지원합니다.
  * Automations API는 `Registry` 객체 범위로 지정된 자동화를 생성하고 편집할 수 있도록 지원합니다.
  * `wandb.Artifact`는 `"MD5"`(기본값) 또는 `"XXH128"` 중 하나를 새 `digest_algorithm` 인수로 허용합니다. 가능한 경우 아티팩트에 XXH128 해싱을 적용하려면 `digest_algorithm="XXH128"`을 전달하세요. `digest_algorithm="XXH128"`로 생성된 아티팩트의 경우 SDK 버전이 v0.29.0보다 이전이면 `artifact.verify()`가 실패합니다.
  * 새 속성 `Artifact.linked_at`은 버전이 해당 컬렉션에 연결된 시간을 반환합니다. 연결된 버전에만 적용되며, 소스 아티팩트에는 `None`을 반환합니다.
  * Registry 버전 쿼리(`Api.registries().versions()`, `Api.registries().collections().versions()`, `Registry.versions()`, and `Registry.collections().versions()`)는 고급 검색을 사용하는 조직에서 선택적 `order` string을 키워드 인수로 허용합니다. `created_at`, `artifact_size` 또는 `linked_at`을 기준으로 아티팩트 버전을 정렬할 수 있습니다.
  * W&amp;B LEET TUI 특성:
    * 새 명령어 `wandb leet inspect [PATH]`로 run의 `.wandb` 트랜잭션 로그에 있는 원시 레코드를 탐색할 수 있습니다. 필터링 가능한 레코드 목록과 선택한 레코드의 텍스트 뷰를 함께 표시하며, 실행 중인 run에 기록되는 내용도 실시간으로 확인합니다. 단일 run 뷰에서 `i`를 눌러 현재 run을 검사하세요. 대신 레코드를 출력하려면 `wandb leet inspect | less`와 같이 출력을 파이프하거나 리디렉션하세요.
    * `g`를 눌러 선 차트 뒤에 점선 배경 또는 축 눈금에 맞춘 가로선 가이드를 표시하세요. 선택한 설정은 저장되며, `wandb leet config`에서 `chart_guides`로도 설정할 수 있습니다.
    * run 개요 섹션 사이의 구분선을 드래그해 마우스로 크기를 조절하세요. 비율은 각 뷰에 저장됩니다. 다른 패널 크기와 함께 재설정하려면 `0`을 누르세요.

  <div id="changed-2">
    ## 변경됨
  </div>

  * SDK가 내부 프로세스 간 통신에 사용하는 메시지 형식이 약간 변경되었습니다. `wandb beta core`를 사용하는 경우 SDK와 서비스의 버전이 다르면 일부 오퍼레이션이 실패할 수 있으므로 `wandb`를 업그레이드한 후 서비스를 다시 시작하세요.
  * 이제 `wandb.sandbox`는 클라이언트에서 `resources.gpu`를 거부하는 대신 샌드박스의 GPU 리소스 요청을 허용합니다.
  * 레지스트리 검색 메서드인 `Api.registries()`, `.collections()`, `.versions()`는 이제 필터 필드 이름을 검증하고 지원되지 않는 이름을 거부합니다.
  * 이제 `Api.create_automation()` 및 `Api.update_automation()`은 서버가 지정된 자동화를 지원하지 않는 경우 `CommError` 대신 `UnsupportedError`를 발생시킵니다.
  * 이제 macOS wheel에는 macOS 13(Ventura) 이상이 필요합니다. macOS 12는 2024년에 지원이 종료되었습니다.
  * W&amp;B LEET TUI에서 run 개요 섹션(**환경**, **설정**, **요약**)은 이제 사이드바 높이를 비례적으로 나누어 사용 가능한 공간을 모두 활용합니다. 이전에는 고정된 크기 이상으로 늘어나지 않아 높이가 큰 터미널의 하단이 비어 있었습니다.

  <div id="removed-2">
    ## 제거됨
  </div>

  * `wandb.tensorboard.log()` 및 `wandb.tensorflow.log()`를 제거했습니다.
  * 이미 아무런 효과가 없었던 사용 중단된 `start_method` 설정을 제거했습니다.
  * `wandb.apis.paginator.SizedPaginator`에서 사용 중단된 `.length` 속성을 제거했습니다. 대신 `len(...)`을 사용하세요.
  * 사용 중단된 `summary="best"` 옵션과 `run.define_metric()`의 `goal` 인수를 제거했습니다. 대신 `summary="min"` 또는 `summary="max"`를 사용하세요.
  * `run.finish()` 및 `wandb.finish()`의 사용 중단된 `quiet` 인수를 제거했습니다. 대신 `wandb.Settings(quiet=...)`를 사용하세요.
  * 사용 중단된 `run.project_name()`, `run.get_url()`, `run.get_project_url()`, `run.get_sweep_url()` 메서드를 제거했습니다. 대신 `run.project`, `run.url`, `run.project_url`, `run.sweep_url` 속성을 사용하세요.
  * `wandb.Image`에서 사용 중단된 `normalize` 인수를 제거했습니다. NumPy 배열 및 PyTorch 텐서의 픽셀 값이 이미 [0, 255] 범위에 있는지 확인하세요.

  <div id="fixed-2">
    ## 수정 사항
  </div>

  * 이제 `Artifact.new_file()`은 `wandb sync`로 업로드된 아티팩트에서도 작동합니다.
  * 이제 `Run.scan_history(keys=...)`는 중첩된 값을 가진 Runs에서도 실패하지 않습니다.
  * 사이드바 가장자리를 드래그하는 경우처럼 특정 패널 너비에서 W&amp;B LEET TUI가 사용 가능한 공간보다 넓게 메트릭 차트를 그려 오른쪽 사이드바가 화면 밖으로 밀려나던 문제가 있었습니다.
  * W&amp;B LEET TUI:
    * `wandb leet config`가 이제 짧은 터미널 창에서도 작동합니다. 필드 목록은 선택 항목에 맞춰 스크롤되며, **Space**를 눌러 불리언 설정을 표시하거나 숨길 수 있습니다.
    * 미디어 패널은 더 이상 ANSI 뷰에서 이미지 주변에 검은 막대를 표시하지 않으며, 전체 해상도 뷰로 전환할 때 검은색으로 깜박이지 않습니다. 이제 이미지 주변에는 터미널 배경이 표시되며, 모드를 전환해도 이미지가 깜박이거나 이동하지 않습니다.

  <div id="security">
    ## 보안
  </div>

  * 이제 `artifact.files()` 또는 `artifact.checkout()`을 사용해 아티팩트를 다운로드할 때 상대 경로를 포함하는 파일 및 아티팩트 이름은 허용되지 않습니다. 이를 통해 [CVE-2026-91771](https://www.tenable.com/cve/CVE-2026-91771)에 설명된 경로 탐색 취약점이 수정되었습니다.
</Update>

<Update label="0.28.2" description="2026년 8월 12일">
  <div id="notable-changes-3">
    ## 주요 변경 사항
  </div>

  * 이제 `wandb sync --clean` 명령은 종료 코드 1로 종료되며, 대신 새 `wandb clean` 명령을 사용하라는 안내를 출력합니다.
  * 이제 `wandb login`은 기본적으로 자격 증명을 확인합니다. 확인을 비활성화하려면 명령줄에서 `--no-verify`를 전달하거나 `wandb.login()`에 `verify=False`를 전달하세요.

  <div id="added-3">
    ## 추가됨
  </div>

  * Filestream 요청은 gzip 압축을 지원하므로 메트릭을 기록할 때 네트워크 트래픽을 줄일 수 있습니다. 압축은 선택적으로 사용할 수 있으며 서버 지원이 필요합니다. 사용하려면 `wandb.Settings`에서 `x_file_stream_no_gzip=False`를 설정하세요. 향후 릴리스에서는 압축이 기본값이 됩니다.
  * `wandb agent` 명령에 새 `--term-timeout` 플래그가 추가되었습니다.
  * 새 속성 `run.sync_dir`는 run의 동기화 디렉터리를 반환합니다.

  <div id="changed-3">
    ## 변경됨
  </div>

  * 새 `wandb clean` 명령이 `wandb sync --clean`을 대체합니다.
  * 이제 `wandb.init()`는 시간 초과 시 일반적인 시간 초과 메시지 대신 네트워크 오류와 같이 재시도하던 오류를 보고합니다. 또한 `init_timeout` 설정은 run 초기화 중 백엔드의 재시도 시간도 제한합니다.
  * 이제 `wandb.Api().create_run_queue()`, `wandb.Api().create_custom_chart()`, `wandb.Api().upsert_run_queue()`는 백엔드에서 오퍼레이션이 실패하면 `WandbApiFailedError`를 발생시킵니다.
  * 페이지 매김된 아티팩트 및 레지스트리 쿼리 메서드는 결과를 가져오기 전에 클라이언트에서 페이지 매김 인수를 검증합니다. 이는 `Api.artifacts()`, `Api.artifact_collections()`, `Api.registries()`, `Registries.collections()`, `Registries.versions()`, `Collections.versions()`, `Registry.collections()`, `Registry.versions()`, `Project.collections()`에 적용됩니다.
  * 정렬된 레지스트리 검색은 레지스트리별 컬렉션 및 버전 쿼리의 범위를 현재 레지스트리의 내부 ID로 제한하여 페이지 매김과 필터링의 신뢰성을 높입니다.
  * 아티팩트 및 레지스트리 쿼리를 포함한 여러 `wandb.Api` 오퍼레이션의 응답 파싱 속도가 빨라졌습니다.
  * `local-process` 리소스에 대한 `wandb launch`의 인수 처리가 강화되어 작업에서 제공한 값은 항상 셸 인용 처리됩니다.
  * Launch 에이전트는 이제 작업의 Git 소스 URL을 HTTPS 및 SSH 원격 URL로 제한하고, 소스를 가져오고 하위 모듈을 업데이트할 때 Git의 프로토콜 허용 목록을 고정합니다.

  <div id="deprecated-2">
    ## 사용 중단
  </div>

  * `wandb sync --sync-tensorboard`는 사용 중단되었으며 향후 릴리스에서 제거될 예정입니다.

  <div id="removed-3">
    ## Removed
  </div>

  * 릴리스에는 더 이상 32비트 Windows(`win32`)용 휠이 포함되지 않습니다. Windows에서는 64비트 Python을 사용하세요.

  <div id="fixed-3">
    ## 수정 사항
  </div>

  * 이제 `registries(order=...).collections(order=...).versions()`를 사용한 레지스트리 검색은 레지스트리 순서, 컬렉션 순서 또는 둘 다에 따라 아티팩트 버전을 반환합니다.
  * 이제 macOS x86&#95;64 wheel에는 `wandb-xpu` 바이너리 및 Rust parquet 라이브러리의 x86&#95;64 빌드가 포함됩니다. 이전에는 arm64 빌드로 제공되어 Intel Mac에서 실행하거나 로드할 수 없었습니다.
  * 이제 `wandb.login(verify=True)` 및 `wandb login --verify`는 이전에 확인하지 않았던 페더레이션 ID(ID 토큰) 자격 증명을 확인합니다.
  * 이제 `wandb login`은 API 키를 `.netrc` 파일에 저장하기 전에 검증합니다.
  * 이제 `wandb login`은 `.netrc` 파일에서 다른 머신의 자격 증명을 제거하거나 손상시키지 않습니다.
  * 이제 `wandb login` 및 `wandb verify`는 로그인 시도가 실패해도 시스템 호스트 설정을 업데이트하지 않습니다.
  * 이제 `wandb verify`는 재시도 후에도 오퍼레이션이 계속 실패하면 충돌하는 대신 검사 실패를 보고합니다.
  * 지정한 서버가 Dedicated Cloud 배포와 같이 기본 서버가 아닌 경우, `WANDB_BASE_URL`도 설정하지 않으면 페더레이션 ID(ID 토큰) 자격 증명으로 인증하지 못하던 `wandb.Api(overrides={"base_url": ...})` 문제를 수정했습니다.
  * 페더레이션 ID를 사용하는 경우, 이제 ID 토큰 파일을 시작 시에만 읽는 대신 각 액세스 토큰 교환 시 다시 읽습니다. 동일한 경로에 다시 발급되는 수명이 짧은 ID 토큰은 해당 토큰보다 오래 지속되는 run에서도 계속 작동합니다.
  * 페더레이션 ID를 사용 중이고 서버가 잘못되었거나 만료된 토큰 등의 이유로 ID 토큰 교환을 거부하면, 이제 요청은 서버의 오류 메시지와 함께 즉시 실패합니다. 이전에는 거부된 교환을 요청이 일반 오류로 시간 초과될 때까지 재시도했습니다.
  * 이제 스윕 에이전트는 JSON 출력을 작성할 때 맞춤형 `WANDB_DIR`을 준수합니다.
  * TensorBoard 파일을 Sync할 때 생성되는 `global_step` 메트릭에는 더 이상 `train/global_step`과 같은 접두사가 붙지 않아 트레이닝 및 검증 메트릭을 더 쉽게 비교할 수 있습니다.
  * 이제 TensorBoard 인테그레이션은 가능한 경우 동일한 `global_step`의 데이터를 하나의 W&amp;B step으로 병합하여 W&amp;B step을 더 적게 생성합니다.
  * 이제 `wandb sync`는 시작 후 이름, 태그 또는 메모를 설정한 run에서 더 이상 멈추지 않습니다.
  * Python 로거에서 캡처한 로그 메시지는 더 이상 run의 로그에 빈 줄을 추가하지 않습니다.
  * 이제 `del run.summary[key]`는 중첩된 값으로 로깅된 메트릭도 제거합니다. 이전에는 이를 무시했습니다.

  <div id="security-2">
    ## 보안
  </div>

  * 디버그 로그에 더 이상 CoreWeave 클러스터 자격 증명이 포함되지 않습니다.
  * `wandb docker` 및 `wandb docker-run`은 동일한 머신의 다른 사용자가 읽을 수 있는 명령줄을 통해 더 이상 API 키를 전달하지 않습니다.
</Update>

<Update label="0.28.1" description="2026년 7월 16일">
  <div id="notable-changes-4">
    ## 주요 변경 사항
  </div>

  * 이 버전에는 protobuf v5+가 필요하며 protobuf v4 지원이 중단됩니다.
  * 이 버전에서는 W&amp;B Server v0.69.x 이하와의 호환성이 중단됩니다.

  <div id="added-4">
    ## 추가됨
  </div>

  * `Api().project().sweeps()`는 `Api().runs()`와 동일한 필터 형식을 사용하는 선택적 `filters` 인수를 허용합니다.
  * 새 Public API 방법 `Run.stop()`(`wandb.Api().run(...).stop()`)는 W&amp;B App의 **run 중지** 버튼과 마찬가지로 활성 run이 정상적으로 중지되도록 요청합니다.
  * 새 방법 `wandb.Api().organization()`은 이름으로 `Organization`을 반환합니다. 이름을 생략하면 현재 기본 조직을 반환합니다.

  <div id="changed-4">
    ## 변경됨
  </div>

  * 이제 `wandb sync`는 지원되는 매개변수 조합에 대해 `wandb beta sync`로 라우팅됩니다. 이전 구현을 사용하려면 `--legacy`를 전달하세요.
  * 이제 `wandb beta sync`는 `wandb sync`와 동일하게 기본적으로 온라인 run을 건너뜁니다. 온라인 run을 포함하려면 `--no-skip-online`을 전달하세요.
  * 이제 `wandb.sandbox`는 Serverless Sandboxes의 최대 수명을 기본적으로 12시간(`max_lifetime_seconds=43200`)으로 설정합니다. 기본값을 재정의하려면 개별 샌드박스에 `max_lifetime_seconds`를 전달하거나 `SandboxDefaults`에서 설정하세요.
  * 이제 `wandb.Api().runs()` 및 `wandb.Api().run()`은 run 데이터 로드가 완료되기 전에 run이 삭제되는 경우처럼 run 데이터를 불러오지 못하면 `RunNotFoundError`를 발생시킵니다.
  * `run.save()`는 새 `glob` 인수를 허용합니다. `glob=False`를 전달하면 glob 확장(`*`, `?`, `[...]`)을 비활성화하고 경로를 리터럴 경로로 처리합니다. 파일 이름에 glob 메타문자가 포함된 경우에 유용합니다.
  * 이제 임시 Unix 소켓 파일과 디렉터리는 종료 시 제거됩니다.
  * protobuf v4는 더 이상 지원되지 않습니다.

  <div id="removed-4">
    ## 제거됨
  </div>

  * v0.70.0 이전 W&amp;B Server 릴리스에서 아티팩트 파일을 다운로드하기 위한 레거시 폴백 구현을 제거했습니다. 이러한 서버에서는 `Artifact.files()` 및 `Artifact.download()`가 더 이상 어떤 아티팩트에도 작동하지 않으며, `Artifact.get_entry()` 및 `Artifact.get_path()`를 통한 파일 다운로드는 더 이상 비참조 아티팩트에 작동하지 않습니다. 이러한 오퍼레이션을 계속 사용하려면 W&amp;B Server를 v0.70.0 이상으로 업그레이드하세요.
  * v0.70.0 이전 W&amp;B Server 릴리스에서 이름으로 아티팩트를 가져오기 위한 레거시 폴백 구현을 제거했습니다. 이러한 서버에서는 `wandb.Api().artifact()` 및 경로로 아티팩트를 가져오는 다른 방법이 더 이상 작동하지 않습니다. 이러한 오퍼레이션을 계속 사용하려면 W&amp;B Server를 v0.70.0 이상으로 업그레이드하세요.
  * `GitPython` 종속성을 제거했습니다. 이제 Git 메타데이터는 `git` 실행 파일을 직접 호출하여 수집됩니다. `GIT_PYTHON_GIT_EXECUTABLE` 환경 변수는 실행 파일의 위치를 찾는 데 계속 적용됩니다.

  <div id="fixed-4">
    ## 수정됨
  </div>

  * 별칭을 추가하는 경우처럼 연결된 레지스트리 아티팩트를 저장할 때 소스 프로젝트에 대한 쓰기 액세스 권한이 없어도 더 이상 실패하지 않습니다.
  * `Run.log()`로 로깅된 `np.float16` 및 `np.float32` `NaN` 값은 이제 자동으로 삭제되지 않고 `NaN`으로 기록되며, `np.float64` 및 기본 `float`의 동작과 일치합니다.
  * 이제 `Run.upload_file()`(`wandb.Api().run(...)`을 통해)은 Self-Managed 배포에서 업로드한 파일을 run에 등록합니다. 이전에는 파일의 바이트가 업로드되었지만 커밋되지 않았으므로 객체 스토어 알림이 없는 배포에서는 파일이 run에 표시되지 않았습니다.
  * 이제 스윕 에이전트는 스윕이 삭제되거나 `Api()`가 404 오류를 반환할 때 종료하기 전에 진행 중인 run이 완료되도록 합니다.
  * SDK v0.25.1에서 도입된 교착 상태를 수정했습니다. 이러한 교착 상태는 기본적으로 `fork` `multiprocessing` 시작 방법을 사용하는 Python 3.14 이전 버전에서 PyTorch를 사용할 때 보고되었습니다. 일부 `wandb.Api` 기능을 사용할 때 가비지 컬렉션 중에도 발생할 수 있었습니다.
</Update>

<Update label="0.28.0" description="2026년 6월 22일">
  <div id="notable-changes-5">
    ## 주요 변경 사항
  </div>

  * 이 버전부터는 0.65.0 미만 서버 버전과의 호환성이 지원되지 않습니다.
  ...
  * W&amp;B LEET TUI 기능:
    * 미디어 패널이 Kitty 프로토콜을 지원하는 터미널에서 고해상도 이미지 렌더링을 지원하며, 지원되지 않는 경우 ANSI 폴백을 사용합니다.
    * 미디어 패널이 동기화 스크러빙을 지원합니다. `l`을 눌러 스크러빙을 연결한 다음 화살표 키, Home, End 키로 이동하면 통합된 step 타임라인 위에서 공유 커서가 움직이고 모든 이미지 타일이 이에 맞춰 따라갑니다.
    * `wandb leet [run-url]`을 통한 기본적인 원격 run 지원.
  * 다음 페이지 매김된 아티팩트 및 레지스트리 API 메서드는 이제 선택적 `order` string을 키워드 인수로 받을 수 있습니다: `Api.artifacts()`, `Api.artifact_collections()`, `Api.registries()`, `Api.registries().collections()`, `Registry.collections()`.
  ...
  * W&amp;B LEET TUI 변경 사항:
    * W&amp;B LEET TUI가 이제 정식 출시되었으며, 명령어가 `wandb beta leet`에서 `wandb leet`로 변경되었습니다. 기존 명령어도 계속 별칭으로 사용할 수 있습니다.
    * 이제 단일 키로 로깅된 이미지 목록이 W&amp;B LEET 미디어 패널에 이미지당 하나의 타일로 표시됩니다.
  * 쿼리 성능을 개선하기 위해 `wandb.Api().runs()`는 이제 기본적으로 각 run의 Sweeps를 로드하지 않습니다. 스윕 데이터는 `sweep` 속성에 처음 액세스할 때 로드됩니다.

  <div id="fixed-5">
    ## Fixed
  </div>

  * `File.download()`는 더 이상 하드코딩된 5초 타임아웃 이후 실패하지 않으며, 다운로드는 파일 전송 설정을 따릅니다.
  * 0.27.1에서 도입된 회귀 문제를 수정하여, `wandb.Api().viewer`, `Api().user()`, `Api().users()` 호출이 `WandbApiFailedError: relogin required for some API keys` 오류와 함께 실패할 수 있던 문제를 해결했습니다.
  * 서로 다른 `class_labels`를 가진 여러 박스 또는 마스크 레이어가 포함된 `wandb.Image`를 `wandb.Table` 안에 로깅하면, 이제 각 레이어의 레이블이 `table.json`의 새 `box_class_maps` 또는 `mask_class_maps` 필드에 보존됩니다. 이전에는 단일 `class_map`이 각 class label 집합에 의해 잘못 덮어써졌습니다.
  * 이제 아티팩트 파일 오퍼레이션에는 항상 정규화된 상대 경로가 필요합니다.
  * `$TMPDIR`이 없거나 쓰기 불가능할 때 조용히 실패하던 문제를 해결하기 위해, 이제 `WandbLogger` 또는 `run.log_artifact`를 사용해 아티팩트를 로깅하면 매니페스트 파일을 OS 임시 디렉터리가 아니라 아티팩트의 스테이징 디렉터리에 기록합니다.
  * 공유 모드(`wandb.init(mode="shared")`)에서 코드 저장이 활성화된 상태로 아티팩트를 로깅할 때 오류가 발생하던 버그를 수정했습니다.
  * 이제 `diff.patch` 파일을 만들 때 기본적으로 `git_root` 설정을 사용하며, 없으면 `root_dir` 설정을 대신 사용합니다.
  * 이제 GPU, CPU, 전력, 온도와 같은 Apple 시스템 메트릭이 Apple M5 Mac에서 수집됩니다.
  * 이제 `wandb.Api().run(...).download_history_exports`를 사용해 이력을 내보낼 때 파일 다운로드 진행률이 표시됩니다.
  * 0.27.1에서 도입된 회귀 문제를 수정하여, 활성 run처럼 이력은 존재하지만 Parquet으로 내보내지지 않은 run에 대해 `Run.scan_history()`가 행을 반환하지 않던 문제를 해결했습니다.
</Update>

<Update label="0.27.2" description="2026년 6월 6일">
  <div id="fixed-6">
    ## Fixed
  </div>

  * `Run.scan_history()`는 더 이상 run 이력을 가져올 때 중복되거나 누락된 step을 반환하지 않습니다.
</Update>

<Update label="0.27.1" description="2026년 6월 4일">
  <div id="added-6">
    ## Added
  </div>

  * 이제 Automations API가 아티팩트 태그, 컬렉션 태그 및 아티팩트 연결 해제 이벤트를 지원합니다.

  <div id="changed-6">
    ## Changed
  </div>

  * 이제 `wandb.Api` GraphQL 오퍼레이션은 레거시 Python GraphQL 클라이언트 대신 `wandb-core`를 통해 라우팅됩니다.
    * 이제 `wandb.Api` GraphQL 오퍼레이션의 실패는 `requests` HTTP 예외 대신 `WandbApiFailedError`를 발생시킵니다.
  * 이제 `Run.scan_history()`는 사용 가능한 경우 내보낸 Parquet 이력에서 읽어오므로, 이력이 큰 run의 처리량을 크게 개선할 수 있습니다. 이 기능은 SDK `v0.23.1`에서 `beta_scan_history`로 도입되었습니다.

  <div id="removed-5">
    ## Removed
  </div>

  * 지원되지 않는 `wandb.apis.importers` API를 제거했습니다.
  * 레거시 자동 로깅 및 Tracing API를 포함하여 오래된 OpenAI, Cohere 및 LangChain LLM 인테그레이션을 제거했습니다.
  * 사용 중단된 Keras `WandbCallback` 및 레거시 `wandb.integration.yolov8` callback 패키지를 제거했습니다.

  <div id="fixed-7">
    ## Fixed
  </div>

  * 이제 `wandb.sandbox`는 placement, GPU 및 egress에 유효하지 않은 인수 값을 거부합니다.
</Update>

<Update label="0.27.0" description="2026년 5월 13일">
  <div id="notable-changes-6">
    ## 주요 변경 사항
  </div>

  이 버전에서는 Python 3.9 및 Pydantic v1 지원이 중단됩니다. 이제 Pydantic v2.6 이상이 필요합니다.

  <div id="added-7">
    ## 추가됨
  </div>

  * [Serverless Sandboxes](/ko/sandboxes)를 사용하기 위한 새 패키지 `wandb.sandbox`와 `wandb beta sandbox` CLI.
  * 메트릭 업로드를 불가능하게 만드는 치명적 오류가 발생하면 run을 중지하는 새 설정 `stop_on_fatal_error` 및 환경 변수 `WANDB_STOP_ON_FATAL_ERROR`.
  * 새 설정 `finish_timeout`과 해당 환경 변수 `WANDB_FINISH_TIMEOUT`을 사용하면 run 종료 시 데이터 업로드를 위해 몇 초 동안 대기할지 지정할 수 있습니다. run이 시간 초과되면 경고 메시지가 출력됩니다. 경고 메시지를 출력하는 것에 더해 오류를 발생시키려면 불리언 설정 `finish_timeout_raises` 또는 해당 환경 변수 `WANDB_FINISH_TIMEOUT_RAISES`를 사용하세요.
  * 새 설정 `capture_loggers`를 사용하면 로거 이름에서 최소 로그 수준으로의 매핑을 사용해 run의 **Logs** 탭에 캡처할 Python 로거를 지정할 수 있습니다. 해당 로거에서 출력된 로그 레코드는 `stdout`/`stderr` 캡처와 비슷하게 run에 콘솔 출력으로 게시됩니다.
  * 새 메서드 `run.write_logs()`는 `stdout`/`stderr`의 자동 캡처에 의존하지 않고 텍스트를 run의 **Logs** 탭에 직접 기록합니다.

  <div id="changed-7">
    ## 변경됨
  </div>

  * Linux 컨테이너의 run에 대한 시스템 메트릭은 이제 호스트 전체값 대신 컨테이너의 cgroup v2 limits를 기준으로 CPU 및 메모리 사용량을 보고합니다.
  * Python 3.9는 더 이상 지원되지 않습니다.
  * 이제 Pydantic 2.6 이상이 필요합니다. Pydantic v1은 더 이상 지원되지 않습니다.
</Update>

<Update label="0.26.1" description="2026년 4월 23일">
  <div id="added-8">
    ## 추가됨
  </div>

  * 아티팩트, 레지스트리, 자동화 및 관련 페이지네이터를 반환하는 `wandb.Api` 메서드는 선택적 `start` 인수를 받아 저장된 커서에서 반복을 재개할 수 있습니다.
  * `wandb.Settings`의 `stop_fn` 설정은 run 중지 요청이 있을 때 SDK가 Python 프로세스에 중지를 요청하는 방식을 사용자 지정합니다(기본값은 SIGINT를 보냄). `wandb.setup(settings=...)` 또는 `wandb.init(settings=...)`로 이 설정을 전달하세요. 값이 callable이므로 환경 변수로는 설정할 수 없습니다.

  <div id="changed-8">
    ## 변경됨
  </div>

  * `wandb beta leet`에서는 각 패널 내에서 포커스를 이동할 때 WASD 키와 화살표 키를 서로 바꿔 사용할 수 있습니다(차트 그리드, 목록 내비게이션). **Home**, **End**, **Page Up**, **Page Down** 키도 일관되게 동작합니다. 미디어 패널은 여전히 화살표 키로 X축을 스크럽하고 WASD 키로 이미지 타일 사이를 이동합니다.

  <div id="fixed-8">
    ## 수정됨
  </div>

  * 이제 `wandb.init(id=run_id, reinit="create_new")`는 동일한 스크립트에서 같은 `run_id`를 가진 다른 run이 아직 활성 상태이면 오류를 발생시킵니다.
  * `WANDB_MODE` 환경 변수 또는 `mode` 설정으로 오프라인 모드가 활성화된 경우, 이제 `wandb.Api`는 일부 오퍼레이션에서 더 이상 예외를 발생시키지 않습니다.
</Update>

<Update label="0.26.0" description="2026년 4월 13일">
  ...
  * `Sweep.agent().Runs()` 및 `Sweep.agents().Runs()`는 지정한 에이전트 또는 모든 에이전트에서 실행 중인 스윕의 Runs 세부 정보를 반환합니다.
  * Google TPU 인프라에서는 SDK가 `duty_cycle_pct` 및 `hbm_capacity_total`/`hbm_capacity_usage` 외에도 `tensorcore_util` 및 지연 시간 분포 같은 새로운 메트릭을 수집하려고 시도합니다. 자세한 내용은 Google Cloud 문서의 [TPU monitoring library: Supported metrics](https://docs.cloud.google.com/tpu/docs/tpu-monitoring-library#list-all-supported-metric-names)를 참조하세요.
  ...
  * `wandb beta leet` TUI 업데이트:
    ...
    * 색각 이상 친화적인 새 색상 구성표:
      ...
      * `clear-signal`: 인접한 시리즈를 최대한 구분할 수 있도록 색상을 교차 배치한 시에나/파랑/회색 테마입니다.
      ...
      `wandb beta leet config`로 설정하세요.

  <div id="changed-9">
    ## 변경됨
  </div>

  * 이제 JSON 직렬화 및 역직렬화에 성능 향상을 위해 `orjson`을 사용합니다.
  * `wandb beta leet` TUI에서 다중 시리즈 오버레이, 검사, 실시간/이력 확대 기능으로 시스템 메트릭의 가시성이 향상되었습니다.
  * `wandb beta leet`에서 run 기본 색상 충돌 가능성을 줄였습니다. 이제 workspace 범위의 run 색상 할당기가 해시된 팔레트 선호도를 유지하면서, 인접한 HSL 변형으로 충돌을 해결합니다.

  <div id="fixed-9">
    ## 수정됨
  </div>

  * 업스트림 tracking 브랜치가 설정되지 않은 경우, SDK가 로컬 브랜치와 가장 가까운 업스트림 브랜치 사이의 `git diff` `.patch` 파일을 생성하지 않던 버그를 수정했습니다. 이 수정 사항을 적용하려면 `wandb.Settings(disable_git_fork_point=False)` 속성을 전달하세요. 자세한 내용은 [Settings()](/ko/models/ref/python/experiments/settings)를 참조하세요.
  * 새 이벤트가 제공될 때 `update_automation()`이 `OnAddArtifactAlias`의 별칭 조건과 같은 이벤트 필터를 조용히 누락할 수 있던 버그를 수정했습니다.
  * `fork()` 후 fast ID 생성기를 다시 시드하여, 포크된 자식 프로세스에서 아티팩트 클라이언트 ID 충돌이 발생할 수 있던 버그를 수정했습니다.
  * 환경 변수 `WANDB__EXTRA_HTTP_HEADERS`가 사전 서명된 오브젝트 스토어 업로드 및 다운로드 요청에 적용되지 않던 버그를 수정했습니다.
...
<Update label="0.24.0" description="2026년 1월 13일">
  ...
  * 이제 `wandb agent`와 `wandb.agent()`는 `forward_signals` 플래그(CLI: `--forward-signals/-f`)를 지원합니다. 이 플래그를 사용하면 에이전트에서 스윕 하위 run으로 SIGINT/SIGTERM 및 기타 처리 가능한 시그널을 전달할 수 있어, 에이전트 프로세스를 중단할 때 더 깔끔하게 종료할 수 있습니다.
  ...
  * 사용 중단된 `wandb.beta.workflows` 모듈과 그 안의 `log_model()`, `use_model()`, `link_model()` 함수를 제거했습니다. 각각의 최신 대체 항목은 `Run.log_artifact`, `Run.use_artifact`, `Run.link_artifact` 방법입니다.
...
<Update label="v0.23.1" description="2025년 12월 2일">
  ...
  * `wandb beta leet` 명령어의 메트릭 및 run 개요 필터에서 정규식을 지원합니다.
  ...
  * `wandb.Api`에서 더 빠르게 이력을 스캔할 수 있도록 `Run` 객체에 `beta_history_scan` 방법이 추가되었습니다.
...
<Update label="v0.23.0" description="2025년 11월 11일">
  ...
  * 이제 `Artifact.link()`는 오류를 발생시키는 대신 저장되지 않은 아티팩트를 로깅하며, `Run.link_artifact()`의 동작과 일치합니다.
  * 이제 VSCode의 Jupyter notebook 확장에서 ipython notebook을 실행할 때 자동 코드 저장이 동작합니다.
  * 이제 `Artifact.metadata`에 무한대 float가 포함된 아티팩트를 로깅하면 request 재시도가 시간 초과될 때까지 기다리는 대신 초기에 `ValueError` 예외를 발생시킵니다.
</Update>
...
<Update label="v0.22.3" description="2025년 10월 28일">
  ...
  * `console_capture.py`의 드문 교착 상태 문제를 수정했습니다.

    교착 상태 중에 스레드 트레이스백을 덤프했을 때 `wandb-AsyncioManager-main` 스레드가 `console_capture.py`의 특정 줄에서 멈춰 있는 것이 보였다면, 이제 수정되었습니다.
  * tfevents 파일이 실시간으로 기록되는 경우 TensorBoard sync가 간헐적으로 중단되던 문제를 수정했습니다.
  * 이제 `Artifact.manifest`는 아티팩트 manifest를 처음 사용할 때까지 다운로드와 다운로드 URL 생성 **모두**를 지연합니다. manifest가 로컬에서 수정되지 않았다면 `Artifact.size`와 `Artifact.digest`는 전체 manifest를 가져오기를 기다리지 않고 반환될 수 있습니다.
  * `artifact.add_reference`를 통한 GCS 폴더 레퍼런스 업로드 문제를 수정했습니다.
  * 이제 SDK가 Jupyter 세션에서 서버의 루트 디렉터리를 사용해 notebook 경로를 올바르게 추론하므로, 하위 디렉터리(예: `code/nested/<notebook>.ipynb`)에서도 코드 저장이 동작합니다.
...
<Update label="v0.22.1" description="2025년 9월 29일">
  ...
  * 체크섬 캐싱을 사용해 아티팩트 다운로드 재검증을 최적화했습니다.
  ...
  * 아티팩트 생성 시 `storage_region` 옵션을 추가했습니다. 사용자는 wandb.ai를 사용할 때 `wandb.Artifact(storage_region="coreweave-us")`를 지정해 [CoreWeave AI Object Storage](https://docs.coreweave.com/docs/products/storage/object-storage)를 사용할 수 있으며, 이를 통해 CoreWeave 인프라에서 아티팩트 업로드/다운로드를 더 빠르게 수행할 수 있습니다.
...
<Update label="v0.21.2" description="2025년 8월 28일">
  ...
  * 아티팩트를 연결하는 동안 발생한 오류가 더 이상 억제되거나 숨겨지지 않으며, `Artifact.link()`와 `Run.link_artifact()`는 더 이상 `None`을 반환하지 않습니다.
...
<Update label="v0.20.1" description="2025년 6월 4일">
  ...
  * wandb.Table: 새 생성자 파라미터 `log_mode`가 추가되었으며, 옵션은 `"IMMUTABLE"` 및 `"MUTABLE"`입니다. `IMMUTABLE` log mode(기본값)는 테이블을 한 번만 로깅할 수 있도록 하는 기존 동작입니다. `MUTABLE` log mode는 테이블이 변경된 경우 다시 로깅할 수 있게 합니다. (@domphan-wandb in [https://github.com/wandb/wandb/pull/9758](https://github.com/wandb/wandb/pull/9758))

  * wandb.Table: 새 `log_mode` `"INCREMENTAL"`이 추가되었으며, 새로 추가된 테이블 데이터를 증분 방식으로 로깅합니다. (@domphan-wandb in [https://github.com/wandb/wandb/pull/9810](https://github.com/wandb/wandb/pull/9810))
...
<Update label="v0.19.10" description="2025년 4월 22일">
  ...
  * 아티팩트의 source run에서 이력 메트릭이 로깅된 가장 가까운 run step을 반환하는 `Artifact.history_step`이 추가되었습니다.
  ...
  * 이제 `Artifact.download()`는 `WANDB_MODE=offline`을 사용하거나 오프라인 run이 있는 경우 오류를 발생시키지 않습니다.
  ...
  * 다음 `wandb.Run` 메서드는 프로퍼티로 대체되어 사용 중단되었으며, 향후 릴리스에서 제거될 예정입니다(@kptkin in [https://github.com/wandb/wandb/pull/8925](https://github.com/wandb/wandb/pull/8925)):
  ...
  * 이제 `wandb.Html()`은 사용자 시스템의 디렉터리와 일치하는 값을 사용할 때 `IsADirectoryError`를 발생시키지 않습니다.

  GitHub에서 [v0.19.10 changelog](https://github.com/wandb/wandb/releases/tag/v0.19.10)을 확인하세요.
</Update>
...
<Update label="v0.19.5" description="2025년 1월 29일">
  ...
  * `wandb login --base-url {host_url}`를 `wandb login --host {host_url}`의 login 별칭으로 추가했습니다.
  ...
  * run rewind 설정 `fork_from` 및 `resume_from`을 올바르게 처리합니다.

  GitHub에서 [v0.19.5 changelog](https://github.com/wandb/wandb/releases/tag/v0.19.5)을 확인하세요.
</Update>
...
<Update label="v0.19.4" description="2025년 1월 16일">
  ...
  * 시스템 모니터에서 연속되지 않는 TPU 디바이스 ID를 처리합니다.
  ...
  GitHub에서 [v0.19.4 changelog](https://github.com/wandb/wandb/releases/tag/v0.19.4)을 확인하세요.
</Update>
...
<Update label="v0.19.3" description="2025년 1월 13일">
  ...
  * `wandb.integration.metaflow`에서 `wandb.Settings` 업데이트 회귀 문제를 수정했습니다.
  ...
  GitHub에서 [v0.19.3 changelog](https://github.com/wandb/wandb/releases/tag/v0.19.3)을 확인하세요.
</Update>
...
<Update label="v0.19.0" description="2024년 12월 05일">
  ...
  이 버전에서는 Python 3.7 지원을 중단하고 `wandb.Run.plot_table` method를 제거했습니다.
  이 버전에서는 pydantic&gt;=2.6,&lt;33을 종속성으로 추가했습니다.

  <div id="changed-26">
    ## 변경됨
  </div>

  * 기본 동작을 W&amp;B 작업을 생성하지 않도록 설정했습니다.
  * pydantic&gt;=2.6,&lt;33을 종속성으로 추가했습니다 (@dmitryduev [https://github.com/wandb/wandb/pull/8649](https://github.com/wandb/wandb/pull/8649)649 &amp; estellazx.
  ...
  * `wandb.Run.plot_table` method를 제거했습니다. 기능은 계속 사용 가능하며, 이제 이 기능의 권장 사용 방법인 `wandb.plot_table`을 통해 액세스해야 합니다.
...
<Update label="v0.18.4" description="2024년 10월 17일">
  ...
  * `Run` 클래스의 `logged_artifacts` 및 `used_artifacts` 메서드의 docstring을 업데이트합니다.
  ...
  * 사용 중단 경고를 해결하기 위해 `sentry-sdk`의 최소 버전을 2.0.0으로 업데이트했습니다.
  ...
  GitHub에서 [v0.18.4 changelog](https://github.com/wandb/wandb/releases/tag/v0.18.4)을 확인하세요.
</Update>
...
<Update label="v0.18.3" description="2024년 10월 01일">
  ...
  * rdkit의 미디어 종속성을 `rdkit-pypi`에서 `rdkit`로 업데이트했습니다.
  ...
  * `artifact = run.log_artifact()` 후에는 아티팩트 업로드 완료를 전제로 하는 오퍼레이션을 수행하기 전에 `artifact.wait()`를 사용해야 합니다. 이전에는 `run.log_artifact()`가 run의 다른 오퍼레이션을 차단했기 때문에 일부 경우에는 이것이 필요하지 않았습니다.
  ...
  GitHub에서 [v0.18.3 changelog](https://github.com/wandb/wandb/releases/tag/v0.18.3)을 확인하세요.
</Update>
...
<Update label="v0.18.2" description="2024년 9월 27일">
  ...
  * `wandb.Api`에 `upsert_run_queue` 메서드를 추가합니다.
  * 태그로 아티팩트를 필터링할 수 있도록 `wandb.Api.artifacts()`에 `tags` 매개변수를 추가합니다.
  ...
  * 마운트된 파일 시스템의 파일 경로로 `Artifact.add_file()`을 호출할 때 발생하는 `OSError`를 수정합니다.
...
<Update label="v0.18.1" description="2024년 9월 16일">
  ...
  * `WANDB_ERROR_REPORTING=false`를 올바르게 적용합니다. 이는 0.18.0에서 도입된 회귀를 수정합니다.
...
<Update label="v0.17.7" description="2024년 8월 15일">
  ...
  * core를 사용할 때 `libnvidia-ml.so`를 사용할 수 없더라도 Nvidia GPU 메트릭이 캡처되도록 보장합니다.
...
<Update label="v0.17.6" description="2024년 8월 08일">
  ...
  * 작업 시작 시 더 나은 UI를 제공할 수 있도록, `manage_config_file` 또는 `manage_wandb_config`를 호출할 때 작업 입력 스키마를 지정합니다.
  ...
  * [https://github.com/wandb/wandb/pull/7675](https://github.com/wandb/wandb/pull/7675)에서 누락된, 기본 정렬 순서와 관련된 `api.runs()`의 docstring을 수정합니다.
...
<Update label="v0.17.0" description="2024년 5월 07일">
  ...
  * 이제 `wandb-core`는 @moredatarequired가 [https://github.com/wandb/wandb/pull/7364](https://github.com/wandb/wandb/pull/7364) 및 [https://github.com/wandb/wandb/pull/7366](https://github.com/wandb/wandb/pull/7366)에서 추가한 Artifact 파일 캐싱을 지원합니다.
...
<Update label="v0.16.6" description="2024년 4월 03일">
  ...
  * @KyleGoyette의 [https://github.com/wandb/wandb/pull/7285](https://github.com/wandb/wandb/pull/7285) 작업으로 W&amp;B Launch 사용 시 job artifact 다운로드가 조용히 실패하고 run 충돌을 일으키던 문제를 수정했습니다.
...
<Update label="v0.16.4" description="2024년 3월 05일">
  ...
  * Wandb-core가 백엔드로 전송되는 요청에 속도 제한을 적용하고 RateLimit-* 헤더를 준수합니다.
    작성자: @timoffex [https://github.com/wandb/wandb/pull/7065](https://github.com/wandb/wandb/pull/7065)065
...
<Update label="v0.16.3" description="2024년 2월 6일">
  ...
  * 선택적 캐시 복사와 스레드를 통해 아티팩트 다운로드 지연 시간을 줄입니다.
  ...
  * nexus에서 아티팩트 파일 업로드 진행률을 개선합니다.
  * AMD GPU가 없는 시스템에 rocm-smi가 설치된 경우 시스템 모니터가 신중하게 처리하도록 합니다.
  * cli.py의 오타를 수정합니다.
  * accelerator build step에서 deadsnakes를 제거합니다.

  GitHub에서 [v0.16.3 변경 로그](https://github.com/wandb/wandb/releases/tag/v0.16.3)을 확인하세요.
</Update>
...
<Update label="v0.16.2" description="2024년 1월 9일">
  ...
  * core를 사용하지 않는 경우 아티팩트 다운로드를 위한 run 생성을 제거합니다.
  ...
  * 활성 run 없이 wandb-core로 아티팩트를 다운로드합니다.
  ...
  * NETRC 환경 변수로 기본 .netrc 위치를 override할 수 있도록 허용합니다.
  ...
  GitHub에서 [v0.16.2 변경 로그](https://github.com/wandb/wandb/releases/tag/v0.16.2)을 확인하세요.
</Update>
...
<Update label="v0.16.0" description="2023년 11월 07일">
  ...
  * sweep scheduler 가상 에이전트를 비동기로 등록합니다.
  ...
  * 에이전트가 여러 큐 간의 균형을 더 잘 맞추도록 개선합니다.
...
<Update label="v0.15.12" description="2023년 10월 03일">
  ...
  * ultralytics classification 작업의 채널 스와핑 문제를 수정합니다.
  ...
  * 에이전트 thread 정리 과정의 race condition을 수정합니다.
...
<Update label="v0.15.2" description="2023년 5월 05일">
  ...
  * [https://github.com/wandb/wandb/pull/5467](https://github.com/wandb/wandb/pull/5467)에서 @parambharat @tssweeney가 새 langchain 릴리스에 맞게 WandbTracer를 업데이트했습니다.
...
<Update label="v0.15.0" description="2023년 4월 19일">
  ...
  * @dmitryduev @parambharat @kptkin @raubitsj가 [https://github.com/wandb/wandb/pull/5362](https://github.com/wandb/wandb/pull/5362)에서 OpenAI Python 라이브러리에 대한 autolog를 추가했습니다.
  ...
  * @dmitryduev @kptkin @raubitsj가 [https://github.com/wandb/wandb/pull/5381](https://github.com/wandb/wandb/pull/5381)에서 사용자 경험 개선을 위해 OpenAI autolog Public API를 조정했습니다.
...
<Update label="v0.8.10" description="2019년 9월 13일">
  ...
  * 이제 jupyter 안에서 또는 python에서 직접 Sweeps를 실행할 수 있습니다! [https://docs.wandb.com/sweeps/python](https://docs.wandb.com/sweeps/python).
...
<Update label="v0.6.34" description="2019년 1월 23일">
  ...
  * 이제 `wandb sync`가 wandb run 디렉터리를 순회하여 모든 Runs를 동기화합니다.
...
<Update label="v0.6.34" description="2019년 1월 23일">
  ...
  * 로깅을 위해 tensorboard를 패치하는 **tensorboard**를 wandb.init()에 추가했습니다.
