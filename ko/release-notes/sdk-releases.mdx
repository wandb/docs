...
<Update label="0.28.1" description="2026년 7월 16일">
  ...
  * 새 Public API 메서드 `Run.stop()`(`wandb.Api().run(...).stop()`)는 W&amp;B App의 **run 중지** 버튼과 마찬가지로 활성 run이 정상적으로 중지되도록 요청합니다.
  * 새 메서드 `wandb.Api().organization()`은 이름으로 `Organization`을 반환합니다. 이름을 생략하면 현재 기본 조직을 반환합니다.
  ...
  * v0.70.0 이전 W&amp;B Server 릴리스에서 이름으로 아티팩트를 가져오기 위한 레거시 폴백 구현을 제거했습니다. 이러한 서버에서는 `wandb.Api().artifact()` 및 경로로 아티팩트를 가져오는 다른 메서드가 더 이상 작동하지 않습니다. 이러한 오퍼레이션을 계속 사용하려면 W&amp;B Server를 v0.70.0 이상으로 업그레이드하세요.
  ...
  * SDK v0.25.1에서 도입된 교착 상태를 수정했습니다. 이러한 교착 상태는 기본적으로 `fork` `multiprocessing` 시작 방법을 사용하는 Python 3.14 이전 버전에서 PyTorch를 사용할 때 보고되었습니다. 일부 `wandb.Api` 기능을 사용할 때 가비지 수집 중에도 발생할 수 있었습니다.
</Update>
