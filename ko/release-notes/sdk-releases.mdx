---
title: "W&B SDK 릴리스"
description: "W&B Models와 함께 사용하는 W&B Python SDK의 버전별 새 기능, 버그 수정, 호환성이 깨지는 변경 사항이 포함된 릴리스 노트를 확인하세요."
rss: true
---

이 페이지에는 W&amp;B Models와 함께 사용하는 W&amp;B Python SDK(`wandb` 패키지)의 릴리스 노트가 포함되어 있습니다. Weave Python SDK(`weave` 패키지)에 대해서는 [Weave SDK releases](/ko/release-notes/weave-sdk-releases)를 참조하세요. W&amp;B Server에 대해서는 [W&amp;B Server release notes](/ko/release-notes/server-releases)를 참조하세요.
...
<Update label="0.28.0" description="2026년 6월 22일">
  ...
  * 이제 GPU, CPU, 전력, 온도와 같은 Apple system metrics가 Apple M5 Mac에서 수집됩니다.
...
<Update label="0.27.1" description="2026년 6월 4일">
  ...
  * 이제 Automations API는 아티팩트 태그, 컬렉션 태그, 아티팩트 연결 해제 이벤트를 지원합니다.

  ## 변경됨

  * 이제 `wandb.Api` GraphQL 오퍼레이션은 레거시 Python GraphQL 클라이언트 대신 `wandb-core`를 통해 라우팅됩니다.
    * 이제 `wandb.Api` GraphQL 오퍼레이션 실패 시 `requests` HTTP 예외 대신 `WandbApiFailedError`가 발생합니다.
  * `Run.scan_history()`는 이제 사용 가능한 경우 내보낸 parquet 이력에서 조회하므로, 이력이 큰 run의 처리량을 크게 개선할 수 있습니다. 이 기능은 SDK `v0.23.1`에서 `beta_scan_history`로 도입되었습니다.

  ## 제거됨

  * 지원되지 않는 `wandb.apis.importers` API가 제거되었습니다.
  * 레거시 autologging 및 Tracing API를 포함한 오래된 OpenAI, Cohere, LangChain LLM 인테그레이션이 제거되었습니다.
  * 사용 중단된 Keras `WandbCallback`와 레거시 `wandb.integration.yolov8` 콜백 패키지가 제거되었습니다.

  ## 수정됨

  * 이제 `wandb.sandbox`는 placement, GPU, egress에 대해 잘못된 인수 값을 거부합니다.
</Update>
