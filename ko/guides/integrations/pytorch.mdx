...
gradient를 자동으로 기록하려면 [`wandb.watch`](/ko/models/ref/python/watch)를 호출하고 PyTorch model을 전달하면 됩니다.
...
동일한 스크립트에서 여러 model을 추적해야 하는 경우 각 model에서 `wandb.watch`를 개별적으로 호출할 수 있습니다. 이 함수에 대한 참조 문서는 [여기](/ko/models/ref/python/watch)에 있습니다.
...
이미지 데이터가 포함된 PyTorch `Tensors`를 [`wandb.Image`](/ko/models/ref/python/data-types/image)로 전달할 수 있으며, [`torchvision`](https://pytorch.org/vision/stable/index.html)의 유틸리티가 자동으로 이미지를 변환하는 데 사용됩니다.
...
PyTorch 및 기타 프레임워크에서 W&B에 rich media를 로깅하는 방법에 대한 자세한 내용은 [미디어 로깅 가이드](/ko/models/track/log/media/)를 확인하세요.
...
데이터셋 및 model 로깅 및 시각화에 대한 자세한 내용은 [W&B Tables 가이드](/ko/models/tables/)를 확인하세요.
...
W&B는 [PyTorch Kineto](https://github.com/pytorch/kineto)의 [Tensorboard plugin](https://github.com/pytorch/kineto/blob/main/tb_plugin/README.md)과 직접 통합되어 PyTorch 코드 프로파일링, CPU 및 GPU 통신 세부 정보 검사, 병목 현상 및 최적화 식별을 위한 툴을 제공합니다.
