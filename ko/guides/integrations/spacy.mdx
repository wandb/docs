...
| `model_log_interval`   | `Optional int`. 기본적으로 `None`입니다. 설정되면 [Artifacts](/ko/models/artifacts/)를 사용하여 [모델 버전 관리](/ko/models/core/registry/)를 활성화합니다. 모델 체크포인트를 로깅하는 사이의 단계 수를 전달합니다. 기본적으로 `None`입니다. |
...
트레이닝이 시작되면 트레이닝 run의 [W&B page](/ko/models/runs/) 링크가 출력되어 Weights & Biases 웹 UI에서 이 run의 실험 추적 [대시보드](/ko/models/track/workspaces/)로 이동합니다.
