...
1. API 키로 `WANDB_API_KEY` [환경 변수](/ko/models/track/environment-variables/)를 설정합니다.
...
wandb의 [`define_metric`](/ko/models/ref/python/run#define_metric) 함수를 사용하여 W&B 요약 메트릭에 해당 메트릭의 최소, 최대, 평균 또는 최적값을 표시할지 여부를 정의할 수 있습니다. `define`_`metric` _이(가) 사용되지 않으면 로깅된 마지막 값이 요약 메트릭에 나타납니다. `define_metric` [참조 문서(여기)](/ko/models/ref/python/run#define_metric)와 [가이드(여기)](/ko/models/track/log/customize-logging-axes/)를 참조하십시오.
...
모델 체크포인트를 W&B [Artifacts](/ko/models/artifacts/)로 저장하려면 Lightning [`ModelCheckpoint`](https://pytorch-lightning.readthedocs.io/en/stable/api/pytorch_lightning.callbacks.ModelCheckpoint.html#pytorch_lightning.callbacks.ModelCheckpoint) 콜백을 사용하고 `WandbLogger`에서 `log_model` 인수를 설정합니다.
...
_latest_ 및 _best_ 에일리어스는 W&B [Artifact](/ko/models/artifacts/)에서 모델 체크포인트를 쉽게 검색할 수 있도록 자동으로 설정됩니다.
...
로깅하는 모델 체크포인트는 [W&B Artifacts](/ko/models/artifacts/) UI를 통해 볼 수 있으며 전체 모델 계보를 포함합니다(UI의 예제 모델 체크포인트는 [여기](https://wandb.ai/wandb/arttest/artifacts/model/iv3_trained/5334ab69740f9dda4fed/lineage?_gl=1*yyql5q*_ga*MTQxOTYyNzExOS4xNjg0NDYyNzk1*_ga_JH1SJHJQXJ*MTY5MjMwNzI2Mi4yNjkuMS4xNjkyMzA5NjM2LjM3LjAuMA..) 참조).

최고의 모델 체크포인트를 북마크하고 팀 전체에서 중앙 집중화하려면 [W&B Model Registry](/ko/models)에 연결하면 됩니다.

여기에서 작업별로 최고의 모델을 구성하고, 모델 수명 주기를 관리하고, ML 수명 주기 전반에 걸쳐 쉬운 추적 및 감사를 용이하게 하고, 웹후크 또는 작업을 통해 다운스트림 작업을 [자동화](/ko/models/core/automations/)할 수 있습니다.
...
모델 체크포인트를 W&B에 저장하여 볼 수 있거나 향후 run에서 사용하기 위해 다운로드할 수 있습니다. 또한 GPU 사용량 및 네트워크 I/O와 같은 [시스템 메트릭](/ko/models/ref/system-metrics/), 하드웨어 및 OS 정보와 같은 환경 정보, [코드 상태](/ko/models/app/features/panels/code/) (git 커밋 및 차이 패치, 노트북 콘텐츠 및 세션 기록 포함) 및 표준 출력에 인쇄된 모든 항목을 캡처합니다.
