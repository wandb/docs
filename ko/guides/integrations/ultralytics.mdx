...
선택한 `YOLO` 모델을 초기화하고 모델로 추론을 수행하기 전에 `add_wandb_callback` 함수를 호출합니다. 이렇게 하면 트레이닝, 파인튜닝, 검증 또는 추론을 수행할 때 experiment 로그와 이미지가 자동으로 저장되고, [컴퓨터 비전 작업을 위한 대화형 오버레이](/ko/models/track/log/media/#image-overlays-in-tables)를 사용하여 각각의 예측 결과와 함께 W&B의 [`wandb.Table`](/ko/models/tables/)에 추가 인사이트와 함께 오버레이됩니다.
...
다음으로 `wandb.init`을 사용하여 W&B [run](/ko/models/runs/)을 초기화합니다.
...
다음으로 원하는 `YOLO` 모델을 초기화하고 모델로 추론을 수행하기 전에 `add_wandb_callback` 함수를 호출합니다. 이렇게 하면 추론을 수행할 때 [컴퓨터 비전 작업을 위한 대화형 오버레이](/ko/models/track/log/media/#image-overlays-in-tables)와 함께 [`wandb.Table`](/ko/models/tables/)에 추가 인사이트와 함께 이미지가 자동으로 기록됩니다.
...
W&B 이미지 오버레이에 대한 자세한 내용은 [여기](/ko/models/track/log/media/#image-overlays)에서 확인할 수 있습니다.
