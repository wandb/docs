...
<Note>
W&B 사용법을 배우는 중이라면 계속 읽기 전에 [실험 추적](/ko/models/track/)과 같은 이 문서의 다른 W&B 가이드를 살펴보는 것이 좋습니다.
</Note>
...
구성 파일을 정의하는 방법에 대한 자세한 내용은 [실험 구성](/ko/models/track/config/)을 참조하세요.

### W&B에 로깅하기

#### 메트릭 로깅

키 값이 메트릭 이름인 사전을 만듭니다. 이 사전 오브젝트를 [`run.log`](/ko/models/track/log/)에 전달합니다.
...
[`run.log`에 대해 자세히 알아보세요](/ko/models/track/log/).
...
미디어, 오브젝트, 플롯 등을 [로깅하는 방법에 대해 자세히 알아보세요](/ko/models/track/log/).
...
사용자 지정 에일리어스를 만드는 방법에 대한 자세한 내용은 [사용자 지정 에일리어스 만들기](/ko/models/artifacts/create-a-custom-alias/)를 참조하세요.
...
분산 환경 또는 간단한 추론과 같이(`wandb.init`을 통해) `wandb` Run을 만들지 않고 Artifact를 다운로드하려면 대신 [wandb API](/ko/models/ref/python/public-api)로 아티팩트를 참조할 수 있습니다.
...
자세한 내용은 [Artifacts 다운로드 및 사용](/ko/models/artifacts/download-and-use-an-artifact/)을 참조하세요.

### 하이퍼파라미터 튜닝

라이브러리에서 W&B 하이퍼파라미터 튜닝을 활용하려면 [W&B Sweeps](/ko/models/sweeps/)를 라이브러리에 추가할 수도 있습니다.
