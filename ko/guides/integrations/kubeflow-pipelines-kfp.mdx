...
[환경 변수](/ko/models/track/environment-variables/)를 컨테이너에 명시적으로 전달해야 할 수 있습니다. 양방향 연결을 위해서는 환경 변수 `WANDB_KUBEFLOW_URL`을 Kubeflow Pipelines 인스턴스의 기본 URL로 설정해야 합니다. 예를 들어, `https://kubeflow.mysite.com`과 같습니다.
...
웹 앱 UI는 Kubeflow Pipelines의 `Visualizations` 탭과 동일한 콘텐츠를 가지고 있지만 공간이 더 넓습니다. [여기에서 웹 앱 UI에 대해 자세히 알아보세요](/ko/models/app/).
...
* 프로그래밍 방식 엑세스의 경우 [Public API를 참조하세요](/ko/models/ref/python/public-api).
...
| ------------------ | --- | --------------- |
| Input Scalar | [`config`](/ko/models/track/config/) | [Overview 탭](/ko/models/runs/#overview-tab) |
| Output Scalar | [`summary`](/ko/models/track/log/) | [Overview 탭](/ko/models/runs/#overview-tab) |
