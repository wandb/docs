...
<Note>
W&B를 사용하여 파인튜닝 Experiments, Models, Datasets을 추적하고 결과를 동료와 공유하는 방법에 대한 자세한 내용은 [OpenAI 파인튜닝 인테그레이션](/ko/guides/integrations/openai-fine-tuning/)을 참조하세요.
</Note>
...
선택적으로 `wandb.init()`이 허용하는 인수가 있는 사전을 `autolog`에 전달할 수 있습니다. 여기에는 프로젝트 이름, 팀 이름, Entity 등이 포함됩니다. [`wandb.init`](/ko/models/ref/python/init)에 대한 자세한 내용은 API Reference Guide를 참조하세요.
...
**1단계**에서 `autolog`에 의해 생성된 W&B [run](/ko/models/runs/) 링크를 클릭합니다. 그러면 W&B 앱의 프로젝트 Workspace로 리디렉션됩니다.
