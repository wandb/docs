...
[OpenAI Gym](https://github.com/openai/gym)을 사용하는 경우, Weights & Biases는 `gym.wrappers.Monitor`에 의해 생성된 환경 비디오를 자동으로 기록합니다. `monitor_gym` 키워드 인수를 [`wandb.init`](/ko/models/ref/python/init)에 `True`로 설정하거나 `wandb.gym.monitor()`를 호출하기만 하면 됩니다.

저희 gym integration은 매우 가볍습니다. `gym`에서 기록된 [비디오 파일의 이름을 확인](https://github.com/wandb/wandb/blob/master/wandb/integration/gym/__init__.py#L15)하여 그에 따라 이름을 지정하거나 일치하는 항목을 찾지 못하면 `"videos"`로 대체합니다. 더 많은 제어를 원하시면 언제든지 수동으로 [비디오를 기록](/ko/models/track/log/media/)할 수 있습니다.
