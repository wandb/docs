...
[Metaflow](https://docs.metaflow.org)는 ML 워크플로우를 생성하고 실행하기 위해 Netflix 에서 만든 프레임워크입니다.
...
[`wandb` 클라이언트 라이브러리](/ko/models/ref/python/)를 사용하여 기록 중인 원래 Python 프로세스 내부, [웹 앱 UI](/ko/models/track/workspaces/) 또는 [Public API](/ko/models/ref/python/public-api/)를 사용하여 프로그램 방식으로 캡처한 정보에 액세스할 수 있습니다. `Parameter`는 W&B의 [`config`](/ko/models/track/config/)에 저장되며 [Overview 탭](/ko/models/runs/#overview-tab)에서 찾을 수 있습니다. `datasets`, `models` 및 `others`는 [W&B Artifacts](/ko/models/artifacts/)에 저장되며 [Artifacts 탭](/ko/models/runs/#artifacts-tab)에서 찾을 수 있습니다. 기본 Python 유형은 W&B의 [`summary`](/ko/models/track/log/) 사전에 저장되며 Overview 탭에서 찾을 수 있습니다. API를 사용하여 외부에서 이 정보를 프로그램 방식으로 가져오는 방법에 대한 자세한 내용은 [Public API 가이드](/ko/models/track/public-api-guide/)를 참조하십시오.
