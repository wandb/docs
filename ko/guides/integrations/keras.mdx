...
- **`WandbMetricsLogger`** : [Experiment 추적](/ko/models/track/)에 이 콜백을 사용하세요. 트레이닝 및 검증 메트릭과 시스템 메트릭을 Weights & Biases에 기록합니다.

- **`WandbModelCheckpoint`** : 모델 체크포인트를 Weight and Biases [Artifacts](/ko/models/artifacts/)에 기록하려면 이 콜백을 사용하세요.

- **`WandbEvalCallback`**: 이 기본 콜백은 모델 예측값을 대화형 시각화를 위해 Weights and Biases [Tables](/ko/models/tables/)에 기록합니다.
...
<Note>
W&B [Artifact 페이지](/ko/models/artifacts/explore-and-traverse-an-artifact-graph/)에는 **Workspace** 페이지가 아닌 Table 로그가 기본적으로 포함되어 있습니다.
</Note>
...
W&B 라이브러리 [`WandbCallback`](/ko/models/ref/python/integrations/keras/wandbcallback) 클래스를 사용하여 `model.fit`에서 추적된 모든 메트릭 및 손실 값을 자동으로 저장합니다.
...
자세한 내용은 [`keras.WandbCallback`에 대한 참조 문서](/ko/models/ref/python/integrations/keras/wandbcallback)를 확인하세요.
