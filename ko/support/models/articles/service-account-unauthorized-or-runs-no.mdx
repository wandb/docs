---
title: "서비스 계정 오류(`Unauthorized` 또는 Runs 누락 등)는 어떻게 해결하나요?"
keywords: ["Administrator", "Security"]
---

서비스 계정을 사용해 W&amp;B 워크플로를 자동화할 때 권한 관련 오류가 발생하거나 Runs가 예상한 위치에 표시되지 않을 수 있습니다. 이 문서에서는 일반적으로 발생하는 서비스 계정 문제와 이를 해결하는 방법을 설명하여, 자동화된 Runs가 올바른 entity에 적절한 권한으로 로깅되도록 돕습니다.

## &quot;Unauthorized&quot; 오류 {#unauthorized-errors}

API 키가 올바르게 설정되었고 서비스 계정이 대상 프로젝트에 액세스할 수 있는지 확인하세요. 제한된 프로젝트의 경우 서비스 계정을 프로젝트의 액세스 목록에 명시적으로 추가해야 합니다.

## Runs가 나타나지 않음 {#runs-not-appearing}

`WANDB_ENTITY` 환경 변수가 올바른 팀 이름으로 설정되어 있는지 확인하세요. `[TEAM_NAME]`을 팀의 이름으로 바꾸세요:

```bash
export WANDB_ENTITY=[TEAM_NAME]
```

`WANDB_ENTITY`가 설정되지 않으면 Runs가 잘못된 entity로 로깅될 수 있습니다.

## 사용자 귀속이 작동하지 않는 경우 {#user-attribution-not-working}

자동화된 Runs를 실제 사용자에게 귀속시키기 위해 `WANDB_USERNAME`을 사용하는 경우, 지정한 사용자가 해당 팀의 멤버인지 확인하세요. `[USERNAME]`을 W&amp;B 사용자 이름으로 바꾸세요:

```bash
export WANDB_USERNAME=[USERNAME]
```

## 제한된 프로젝트에 액세스할 수 없음 {#access-denied-to-restricted-projects}

서비스 계정을 제한된 프로젝트의 액세스 목록에 명시적으로 추가하세요. 서비스 계정은 팀에 속해 있더라도 제한된 프로젝트에 자동으로 액세스할 수 없습니다.

자세한 내용은 [서비스 계정을 사용하여 워크플로 자동화](/ko/platform/hosting/iam/service-accounts)를 참조하세요.

***

{/* AUTO-GENERATED: tab badges */}

<Badge stroke shape="pill" color="orange" size="md">[관리자](/ko/support/models/tags/administrator)</Badge><Badge stroke shape="pill" color="orange" size="md">[보안](/ko/support/models/tags/security)</Badge>

{/* END AUTO-GENERATED: tab badges */}