...
## 콘솔 캡처가 비활성화되어 있습니다 {#console-capture-is-disabled}
...
## 분산 트레이닝 (DDP / multiprocessing) {#distributed-training-ddp-multiprocessing}
...
## Run은 아직 활성 상태이지만 Files 탭에 파일이 없음 {#run-still-active-but-no-file-on-files-tab}
...
## 플러시 전에 run이 중단됨 {#run-crashed-before-flush}
...
## 재개된 Runs에서는 이전 콘솔 출력이 사라집니다 {#resumed-runs-lose-earlier-console-output}
...
## Logs 탭에 예상보다 적은 줄이 표시됨 {#logs-tab-shows-fewer-lines-than-expected}
