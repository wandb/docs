---
title: "특정 문자가 포함된 메트릭은 왜 정렬하거나 필터링할 수 없나요?"
keywords: ["Experiments", "메트릭"]
---

W&amp;B에서 메트릭 이름은 UI에서 정렬하고 필터링할 수 있도록 GraphQL 명명 규칙을 따라야 합니다.

## 유효한 메트릭 이름 {#valid-metric-names}

메트릭 이름은 다음 규칙을 따라야 합니다.

* **허용되는 문자**: 영문자(`A-Z`, `a-z`), 숫자(`0-9`), 밑줄(`_`)
* **시작 문자**: 이름은 영문자 또는 밑줄로 시작해야 합니다.
* **패턴**: 메트릭 이름은 `/^[_a-zA-Z][_a-zA-Z0-9]*$/` 패턴과 일치해야 합니다.

메트릭이 이 규칙을 따르지 않으면 W&amp;B UI에서 정렬하거나 필터링할 수 없을 수 있습니다.

## 예시 {#examples}

다음 예시는 이러한 규칙을 준수하는 메트릭 이름과 그렇지 않은 이름을 보여줍니다.

유효한 메트릭 이름:

```python
with wandb.init() as run:
  run.log({"accuracy": 0.9, "val_loss": 0.1, "epoch_5": 5})
  run.log({"modelAccuracy": 0.95, "learning_rate": 0.001})
```

유효하지 않은 메트릭 이름(다음은 피하세요):

```python
with wandb.init() as run:
  run.log({"acc,val": 0.9})  # 쉼표 포함
  run.log({"loss-train": 0.1})  # 하이픈 포함
  run.log({"test acc": 0.95})  # 공백 포함
  run.log({"5_fold_cv": 0.8})  # 숫자로 시작
```

## 권장 해결 방법 {#recommended-solution}

유효하지 않은 문자를 밑줄(&#95;)과 같은 유효한 문자로 바꾸세요:

* `"test acc"` 대신 `"test_acc"`를 사용하세요.
* `"loss-train"` 대신 `"loss_train"`을 사용하세요.
* `"acc,val"` 대신 `"acc_val"`을 사용하세요.

자세한 내용은 [메트릭 이름 지정 제약 사항](/ko/models/track/log/#metric-naming-constraints)을 참조하세요.

***

{/* AUTO-GENERATED: tab badges */}

<Badge stroke shape="pill" color="orange" size="md">[Experiments](/ko/support/models/tags/experiments)</Badge><Badge stroke shape="pill" color="orange" size="md">[메트릭](/ko/support/models/tags/metrics)</Badge>

{/* END AUTO-GENERATED: tab badges */}