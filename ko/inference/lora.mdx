...
<Tabs>
  <Tab title="다른 곳에서 트레이닝한 LoRA 업로드하기">
    ...
    아래 Python 코드는 로컬에 저장된 LoRA 가중치를 버전 관리되는 아티팩트로 W&amp;B에 업로드합니다. 필요한 메타데이터(기본 모델과 스토리지 리전)를 포함한 `lora` 타입 아티팩트를 생성하고, 로컬 디렉토리의 LoRA 파일을 추가한 뒤, 이를 추론에 사용할 수 있도록 W&amp;B 프로젝트에 로그합니다.
    ...
    * LoRA는 [지원되는 기본 모델 섹션](#supported-base-models)에 나열된 모델 중 하나를 사용해 트레이닝해야 합니다.
    * PEFT 포맷으로 저장되어 있고, W&amp;B 계정에 `lora` 타입 아티팩트로 저장되어 있어야 합니다.
    * LoRA는 낮은 레이턴시를 위해 `storage_region="coreweave-us"`에 저장되어야 합니다.
    * 업로드 시 트레이닝에 사용한 기본 모델 이름(예: `meta-llama/Llama-3.1-8B-Instruct`)을 포함해야 합니다. 이렇게 해야 W&amp;B가 올바른 모델로 로드할 수 있습니다.
  </Tab>
...
현재 Inference는 다음 LLM을 지원합니다 (`wandb.base_model`에는 정확한 문자열을 사용해야 합니다). 더 많은 모델이 곧 추가될 예정입니다.
