...
## LoRAs에 Serverless Inference를 사용하는 이유 {#why-use-serverless-inference-for-loras}
...
## LoRA를 추가하고 사용하기 {#add-and-use-loras}
...
## 지원되는 기본 모델 {#supported-base-models}

LoRA는 다음 기본 모델 중 하나를 기반으로 트레이닝되어야 합니다. Inference 시 W&amp;B가 어댑터를 올바른 기본 모델과 연결할 수 있도록 `wandb.base_model`을 설정할 때는 정확한 모델 ID 문자열을 사용하세요.
