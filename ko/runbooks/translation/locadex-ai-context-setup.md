...
# Agent 프롬프트: W&amp;B 문서용 Locadex AI 컨텍스트 설정(한국어, 이후 일본어) {#agent-prompt-configure-locadex-ai-context-for-wb-docs-korean-and-later-japanese}
...
## Agent 사전 요구 사항 {#agent-prerequisites}
...
### 레거시 도구 (wandb_docs_translation) {#legacy-tooling-wandb_docs_translation}
...
### Locadex/GT 플랫폼 동작 방식 {#locadexgt-platform-behavior}
...
## step별 절차 {#step-by-step-process}

### 1. 용어 출처 수집 {#1-gather-terminology-sources}
...
### 2. Glossary CSV를 빌드하거나 획득하기 {#2-build-or-obtain-a-glossary-csv}
...
### 3. 콘솔에서 Locadex 프로젝트 설정하기 {#3-configure-the-locadex-project-in-the-console}
...
### 4. Glossary 용어 업로드 또는 추가 {#4-upload-or-add-glossary-terms}
...
### 5. 한국어용 Locale Context 설정 {#5-set-locale-context-for-korean}
...
### 6. Style Controls 설정(프로젝트 전체에 적용) {#6-set-style-controls-project-wide}
...
### 7. 필요한 경우 다시 번역하기 {#7-retranslate-if-needed}
...
### 문제: CSV upload가 Glossary에 매핑되지 않음 {#issue-csv-upload-does-not-map-to-glossary}
...
### 문제: 영어로 유지해야 하는 용어가 계속 번역됨 {#issue-terms-still-translated-when-they-should-stay-in-english}
...
### 문제: 일본어(또는 다른 로캘)에는 별도의 규칙이 필요합니다 {#issue-japanese-or-another-locale-needs-different-rules}
...
### 향후 생성을 위한 CSV 형식 {#csv-formatting-for-future-generation}
