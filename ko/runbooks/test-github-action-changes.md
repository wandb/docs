...
* **W&amp;B 직원 액세스**: 내부 W&amp;B 시스템에 액세스할 수 있는 W&amp;B 직원이어야 합니다.
* **GitHub 포크**: 워크플로 변경 사항을 테스트하기 위한 wandb/docs의 개인 포크가 필요합니다. 이 포크에서는 기본 브랜치에 푸시할 수 있어야 하며, 브랜치 보호 규칙을 우회할 수 있는 권한도 있어야 합니다.
...
1. **GitHub 사용자 이름** - 먼저 `git remote -v`를 통해 포크 원격 저장소를 확인한 후, `git config`를 통해 사용자 이름을 확인하세요. 어느 쪽에서도 찾을 수 없으면 사용자에게 물어보세요.
2. **포크 상태** - wandb/docs의 포크를 가지고 있으며, 기본 브랜치에 푸시하고 브랜치 보호를 우회할 권한이 있는지 확인하세요.
...
* **테스트용 포크**: `<username>/docs` (fork 원격 저장소) - `git remote -v` 출력만으로 명확하지 않다면 사용자에게 포크 엔드포인트를 물어보세요.
* **중요**: PR의 GitHub Actions는 항상 PR 브랜치가 아니라 베이스 브랜치(main)에서 실행됩니다.
* **Mintlify 배포 제한 사항**: Mintlify 배포와 `link-rot` 확인은 포크가 아니라 기본 `wandb/docs` 저장소에서만 빌드됩니다. 포크에서는 `validate-mdx` GitHub Action이 포크 PR에서 `mint dev` 및 `mint broken-links` 명령의 상태를 확인합니다.

**에이전트 메모**: 다음을 수행해야 합니다:

1. `git remote -v`를 확인해 기존 포크 원격 저장소가 있는지 확인하고, 있으면 URL에서 username을 추출합니다.
2. remotes에서 username을 찾지 못하면 `git config`에서 GitHub 사용자 이름을 확인합니다.
3. 두 위치 어디에서도 찾지 못한 경우에만 사용자에게 GitHub 사용자 이름을 물어봅니다.
4. 테스트에 사용할 수 있는 wandb/docs 포크가 있는지 확인합니다.
5. 포크에 직접 푸시할 수 없으면 사용자가 푸시할 수 있도록 wandb/docs에 임시 브랜치를 만듭니다.
...
1. 포크의 `main`을 메인 리포지토리의 `main`과 Sync하고, 모든 임시 커밋을 버리세요.
2. 변경 사항을 포크의 main 브랜치에 적용하세요(기능 브랜치에만 적용하지 마세요).
3. 워크플로를 트리거할 수 있도록 콘텐츠 변경이 포함된 테스트 PR을 포크의 `main`을 대상으로 생성하세요.
...
# 포크 원격 저장소가 있으면 포크 URL에서 사용자 이름 확인
# 포크 원격 저장소가 없으면 git config에서 사용자 이름 확인
git config user.name  # or git config github.user

# 원격 저장소나 config에서 찾을 수 없는 경우에만 사용자에게 GitHub 사용자 이름 또는 포크 정보를 요청
# 예시 질문: "테스트에 사용할 포크의 GitHub 사용자 이름이 무엇인가요?"
...
# 포크에 강제 푸시하여 동기화 (포크의 임시 커밋 제거)
git push fork main --force

# 워크플로 변경사항용 테스트 브랜치 생성
git checkout -b test-[설명]-[date]
```
...
* 변경 사항이 포크의 main 브랜치에 있는지 확인하세요.
...
* [ ] 사용자에게 GitHub 사용자 이름과 포크 관련 세부 정보를 요청했는가
* [ ] origin과 포크 두 원격 저장소가 모두 설정되었는가
* [ ] 워크플로 변경 사항이 해당하는 두 파일에 모두 적용되었는가
* [ ] 변경 사항이 포크의 main 브랜치에 푸시되었는가(직접 또는 사용자에게 요청하여)
* [ ] 콘텐츠 변경 사항으로 테스트 PR이 생성되었는가
* [ ] 미리보기 댓글이 성공적으로 생성되었는가
* [ ] GitHub Actions 로그에 오류가 없는가
* [ ] 테스트 후 포크의 main 브랜치가 재설정되었는가
* [ ] wandb/docs에서 임시 브랜치가 정리되었는가(생성된 경우)