...
- **Organize and filter sessions**: Attach key/value labels and tags to a session, then narrow the Sessions page to what you care about. Six `hivemind.*` labels are derived from every session automatically, so filtering is useful before anyone tags anything by hand. Run `hivemind tag` to tag the session you're in, edit tags from the Labels card in the session insights sidebar, or install the `hivemind-tag` skill so a coding agent can tag its own session. For details, see [Labels and tags](https://hivemind.wandb.tools/docs/labels) in the HiveMind documentation.
