---
title: "Will wandb slow down my training?"
keywords: ["Experiments"]
---

W&B has a minimal impact on training performance under normal usage conditions. Normal use includes logging at a rate of less than once per second and limiting data to a few megabytes per step. W&B operates in a separate process with non-blocking function calls, so brief network outages or intermittent disk read/write issues don't disrupt performance. However, excessive logging of large data volumes can lead to disk I/O issues. For more information, contact support.

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Experiments](/support/models/tags/experiments)</Badge>
{/* END AUTO-GENERATED: tab badges */}