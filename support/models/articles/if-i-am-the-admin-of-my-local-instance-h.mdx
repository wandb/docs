---
title: "If I am the admin of my local instance, how should I manage it?"
keywords: ["Administrator"]
---

If you're the admin for your instance, review the [User Management](/platform/hosting/iam/access-management/manage-organization) section for instructions on adding users and creating teams.

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Administrator](/support/models/tags/administrator)</Badge>
{/* END AUTO-GENERATED: tab badges */}