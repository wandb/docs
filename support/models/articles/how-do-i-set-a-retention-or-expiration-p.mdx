---
title: "How do I set a retention or expiration policy on my artifact?"
keywords: ["Artifacts"]
---

To manage artifacts that contain sensitive data or to schedule the deletion of artifact versions, set a TTL (time-to-live) policy. For instructions, see the [TTL guide](/models/artifacts/ttl).

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Artifacts](/support/models/tags/artifacts)</Badge>
{/* END AUTO-GENERATED: tab badges */}