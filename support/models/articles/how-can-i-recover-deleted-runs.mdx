---
title: "How can I recover deleted runs?"
description: "Recover runs deleted within the last 7 days from the Project Overview page in W&B."
keywords: ["Runs"]
description: "Learn how to recover recently deleted runs in the W&B App for up to 7 days."
---

To recover deleted runs, complete the following steps:

- Navigate to the Project Overview page.
- Click the **action (<Icon icon="ellipsis" iconType="solid"/>)** menu in the top right corner.
- Select **Undelete recently deleted runs**.

**Notes**:
- You can only restore runs deleted within the last 7 days.
- You can manually upload logs using the W&B API if undelete is not an option.

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Runs](/support/models/tags/runs)</Badge>
{/* END AUTO-GENERATED: tab badges */}
