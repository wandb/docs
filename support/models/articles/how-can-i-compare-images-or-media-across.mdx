---
title: "How can I compare images or media across epochs or steps?"
keywords: ["Experiments"]
---

To compare images or other media logged at different epochs or steps, expand the **image panel** and use the **step slider** to navigate between them. This lets you see how a model's output changes during training.

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Experiments](/support/models/tags/experiments)</Badge>
{/* END AUTO-GENERATED: tab badges */}