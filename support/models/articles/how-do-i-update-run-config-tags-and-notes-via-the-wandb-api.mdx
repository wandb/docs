...
You cannot add new logged metrics (summary or history) to a finished run via the Public API. To attach post-hoc metrics, start a new run (for example `job_type="evaluation"`), log there, and link via tags or artifact lineage.
