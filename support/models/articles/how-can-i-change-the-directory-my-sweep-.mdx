---
title: "How can I change the directory my sweep logs to locally?"
description: "Learn how to change the directory your sweep logs to locally in W&B: Set the logging directory for W&B run data by configuring the environment variable..."
keywords: ["Sweeps"]
---

Set the logging directory for W&B run data by configuring the environment variable `WANDB_DIR`. For example:

```python
os.environ["WANDB_DIR"] = os.path.abspath("your/directory")
```

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Sweeps](/support/models/tags/sweeps)</Badge>
{/* END AUTO-GENERATED: tab badges */}