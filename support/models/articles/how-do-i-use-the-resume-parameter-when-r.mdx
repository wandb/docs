---
title: "How do I use the resume parameter when resuming a run in W&B?"
description: "Learn how to use the resume parameter when resuming a run in W&B: To use the `resume` parameter in W&B , set the `resume` argument in `wandb.init()` with..."
keywords: ["Resuming"]
---

To use the `resume` parameter in W&B , set the `resume` argument in `wandb.init()` with `entity`, `project`, and `id` specified. The `resume` argument accepts values of `"must"` or `"allow"`. 

  ```python
  run = wandb.init(entity="your-entity", project="your-project", id="your-run-id", resume="must")
  ```

---

{/* AUTO-GENERATED: tab badges */}
<Badge stroke shape="pill" color="orange" size="md">[Resuming](/support/models/tags/resuming)</Badge>
{/* END AUTO-GENERATED: tab badges */}