...
The **Logs** tab caps display for performance: a run stores up to 100,000 lines total, and the W&B UI displays up to 10,000 lines at once. Scroll through the logs to view older lines. The full log is in `output.log` or multipart chunks. Download it from the **Files** tab of the run details, or through the API.
