# /// script
# dependencies = ["pandas", "plotly", "streamlit", "weave"]
# ///
...
@app.cell
def _():
    # packages added via marimo's package management: streamlit pandas plotly weave !pip install streamlit pandas plotly weave
    return
...
def _(datetime, pd):
    ...
    def process_calls(calls):
        ...
        for call in calls:
            ...
            records.append({'Call ID': call.id, 'Trace ID': call.trace_id, 'Display Name': call.display_name, 'Latency (ms)': latency, 'Thumbs Up': thumbs_up, 'Thumbs Down': thumbs_down, 'Started At': pd.to_datetime(getattr(call, 'started_at', datetime.min)), 'Inputs': json.dumps(call.inputs, default=str), 'Outputs': json.dumps(call.output, default=str)})
        return pd.DataFrame(records)  # this is a unique ID for the trace that can be used to retrieve it  # this is an optional name you can set in the UI or programatically

    return (process_calls,)


@app.cell
def _(calls, process_calls):
...
def _():
    ...
    return plot_feedback_pie_chart, plot_model_cost_distribution


@app.cell
def _(df_calls, plot_feedback_pie_chart):
    plot_feedback_pie_chart(df_calls["Thumbs Up"].sum(), df_calls["Thumbs Down"].sum())
    return


@app.cell
def _(df_costs, plot_model_cost_distribution):
