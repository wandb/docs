# /// script
# dependencies = ["autoflake", "autopep8", "datasets", "isort", "openai", "set-env-colab-kaggle-dotenv", "weave"]
# ///
...
@app.cell
def _():
    # packages added via marimo's package management: autopep8 autoflake weave isort openai set-env-colab-kaggle-dotenv datasets !pip install -qU autopep8 autoflake weave isort openai set-env-colab-kaggle-dotenv datasets
    return
...
def _(BaseModel):
    ...
    return FormattedGeneratedCode, GeneratedCode
...
@app.cell
def _(CodeFormatter, GeneratedCode, client, weave):
...
@app.cell
def _(CODE_TEMPLATE, os, re, subprocess, tempfile, traceback, weave):
    @weave.op()
...
def _(selected_examples):
    ...
    return (formatted_selected_examples,)


@app.cell
def _(Dataset, formatted_selected_examples, weave):
