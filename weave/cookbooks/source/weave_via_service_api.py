...
def _(
    ...
    ## ------------
    ## Start trace
    payload_start = {'start': {'project_id': f'{team_id}/{project_id}', 'op_name': 'simple_trace', 'started_at': datetime.datetime.now().isoformat(), 'inputs': {'messages': [{'role': 'user', 'content': 'Why is the sky blue?'}], 'model': 'gpt-4o'}, 'attributes': {}}}
    _response = requests.post(url_start, headers=headers, json=payload_start, auth=('api', wandb_token))
    if _response.status_code == 200:
        _data = _response.json()
        call_id = _data.get('id')
        trace_id = _data.get('trace_id')
        print(f'Call started. ID: {call_id}, Trace ID: {trace_id}')
    else:  # Use this "messages" style to generate the chat UI in the expanded trace.
        print('Start request failed with status:', _response.status_code)
...
def _(
    ...
    ## ------------
    ## End trace
    payload_end = {'end': {'project_id': f'{team_id}/{project_id}', 'id': call_id, 'ended_at': datetime.datetime.now().isoformat(), 'output': {'choices': [{'message': {'content': 'It’s due to Rayleigh scattering, where shorter blue wavelengths of sunlight scatter in all directions.'}}]}, 'summary': {'usage': {'gpt-4o': {'prompt_tokens': 10, 'completion_tokens': 20, 'total_tokens': 30, 'requests': 1}}}}}
    _response = requests.post(url_end, headers=headers, json=payload_end, auth=('api', wandb_token))
    if _response.status_code == 200:
        print('Call ended.')
    else:
        print('End request failed with status:', _response.status_code)
        print(_response.text)  # Use this "choices" style to add the completion to the chat UI in the expanded trace.  # Format the summary like this to generate the pricing summary information in the traces table.
    return
...
def _(
    ...
    payload_parent_start = {'start': {'project_id': f'{team_id}/{project_id}', 'op_name': 'complex_trace', 'started_at': datetime.datetime.now().isoformat(), 'inputs': {'question': 'Can you summarize the key points of this document?'}, 'attributes': {}}}
    _response = requests.post(url_start, headers=headers, json=payload_parent_start, auth=('api', wandb_token))
    if _response.status_code == 200:
        _data = _response.json()
        parent_call_id = _data.get('id')
        trace_id_1 = _data.get('trace_id')
        print(f'Parent call started. ID: {parent_call_id}, Trace ID: {trace_id_1}')
    else:
        print('Parent start request failed with status:', _response.status_code)
        print(_response.text)
        exit()
    return parent_call_id, trace_id_1
...
def _(
    ...
    _payload_child_start = {'start': {'project_id': f'{team_id}/{project_id}', 'op_name': 'rag_document_lookup', 'trace_id': trace_id_1, 'parent_id': parent_call_id, 'started_at': datetime.datetime.now().isoformat(), 'inputs': {'document_search': "This is a search query of the documents I'm looking for."}, 'attributes': {}}}
    _response = requests.post(url_start, headers=headers, json=_payload_child_start, auth=('api', wandb_token))
    if _response.status_code == 200:
        _data = _response.json()
        _child_call_id = _data.get('id')
        print(f'Child call started. ID: {_child_call_id}')
    else:
        print('Child start request failed with status:', _response.status_code)
        print(_response.text)
        exit()
    _payload_child_end = {'end': {'project_id': f'{team_id}/{project_id}', 'id': _child_call_id, 'ended_at': datetime.datetime.now().isoformat(), 'output': {'document_results': "This will be the RAG'd document text which will be returned from the search query."}, 'summary': {}}}
    _response = requests.post(url_end, headers=headers, json=_payload_child_end, auth=('api', wandb_token))
    if _response.status_code == 200:
        print('Child call ended.')
    else:
        print('Child end request failed with status:', _response.status_code)
        print(_response.text)
    return
...
def _(
    ...
    _payload_child_start = {'start': {'project_id': f'{team_id}/{project_id}', 'op_name': 'llm_completion', 'trace_id': trace_id_1, 'parent_id': parent_call_id, 'started_at': datetime.datetime.now().isoformat(), 'inputs': {'messages': [{'role': 'user', 'content': 'With the following document context, could you help me answer:\n Can you summarize the key points of this document?\n [+ appended document context]'}], 'model': 'gpt-4o'}, 'attributes': {}}}
    _response = requests.post(url_start, headers=headers, json=_payload_child_start, auth=('api', wandb_token))
    if _response.status_code == 200:
        _data = _response.json()
        _child_call_id = _data.get('id')
        ...
        print(_response.text)
        exit()
    _payload_child_end = {'end': {'project_id': f'{team_id}/{project_id}', 'id': _child_call_id, 'ended_at': datetime.datetime.now().isoformat(), 'output': {'choices': [{'message': {'content': 'This is the response generated by the LLM.'}}]}, 'summary': {'usage': {'gpt-4o': {'prompt_tokens': 10, 'completion_tokens': 20, 'total_tokens': 30, 'requests': 1}}}}}
    _response = requests.post(url_end, headers=headers, json=_payload_child_end, auth=('api', wandb_token))
    if _response.status_code == 200:
        print('Child call ended.')
    else:
...
def _(
    ...
    ## ------------
    ## End trace
    payload_parent_end = {'end': {'project_id': f'{team_id}/{project_id}', 'id': parent_call_id, 'ended_at': datetime.datetime.now().isoformat(), 'output': {'choices': [{'message': {'content': 'This is the response generated by the LLM.'}}]}, 'summary': {'usage': {'gpt-4o': {'prompt_tokens': 10, 'completion_tokens': 20, 'total_tokens': 30, 'requests': 1}}}}}
    _response = requests.post(url_end, headers=headers, json=payload_parent_end, auth=('api', wandb_token))
    # Parent call: End
    if _response.status_code == 200:
        print('Parent call ended.')
    else:
        print('Parent end request failed with status:', _response.status_code)
        print(_response.text)
    return
...
def _(
    ...
    query_payload = {'project_id': f'{team_id}/{project_id}', 'filter': {'trace_roots_only': True}, 'query': {'$expr': {'$eq': [{'$getField': 'inputs.model'}, {'$literal': 'gpt-4o'}]}}, 'limit': 10000, 'offset': 0, 'sort_by': [{'field': 'started_at', 'direction': 'desc'}], 'include_feedback': False}
    _response = requests.post(url_stream_query, headers=headers, json=query_payload, auth=('api', wandb_token))
    if _response.status_code == 200:
        ...
        try:
            ...
            json_objects = _response.text.strip().split('\n')
            parsed_data = [json.loads(obj) for obj in json_objects]
            print(parsed_data)
    else:
        print(f'Query failed with status code: {_response.status_code}')
        print(_response.text)  # Alternate decode
    return
