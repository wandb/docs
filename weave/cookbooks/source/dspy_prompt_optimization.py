# /// script
# dependencies = ["datasets", "dspy-ai", "weave"]
# ///
...
@app.cell
def _():
    # packages added via marimo's package management: dspy-ai weave datasets !pip install -qU dspy-ai weave datasets
    return
...
def _(Metadata, metadata, weave):
    ...
    return dspy, dspy_train_examples
...
def _(dspy, weave):
    ...
    return CausalReasoningModule, Output
...
@app.cell
def _(CausalReasoningModule, dspy_train_examples):
    import rich
...
def _(Output, weave):
    ...
    return (weave_evaluation_scorer,)
...
@app.cell
async def _(baseline_module, metadata, weave, weave_evaluation_scorer):
...
@app.cell
def _(Metadata, baseline_module, dspy, dspy_train_examples, metadata, weave):
...
async def _(
    ...
    evaluation_1 = weave.Evaluation(name='optimized_causal_reasoning_module', dataset=validation_dataset, scorers=[weave_evaluation_scorer])
    await evaluation_1.evaluate(optimized_module.forward)
    return
