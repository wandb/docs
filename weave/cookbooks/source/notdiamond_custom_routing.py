...
def _(subprocess):
    ...
    subprocess.call(['curl', '-L', 'https://drive.google.com/uc?export=download&id=1q1zNZHioy9B7M-WRjsJPkfvFosfaHX38', '-o', 'humaneval.csv'])
    return
...
def _():
    ...
    return model_test, model_train, weave
...
@app.cell
def _(api_key, preference_id, weave):
    from notdiamond import NotDiamond
    weave.init('notdiamond-quickstart')
    llm_configs = ['anthropic/claude-3-5-sonnet-20240620', 'openai/gpt-4o-2024-05-13', 'google/gemini-1.5-pro-latest', 'openai/gpt-4-turbo-2024-04-09', 'anthropic/claude-3-opus-20240229']
    ...
    session_id, routing_target_model = client.model_select(messages=[{'role': 'user', 'content': new_prompt}], preference_id=preference_id)
...
@app.cell
def _(api_key, model_test, preference_id):
...
@app.cell
async def _(best_provider_model, nd_model, weave):
    @weave.op()
