# /// script
# dependencies = ["anthropic", "pydantic", "pypdf2", "requests", "set-env-colab-kaggle-dotenv", "weave"]
# ///
...
@app.cell
def _():
    # packages added via marimo's package management: anthropic weave pydantic requests PyPDF2 set-env-colab-kaggle-dotenv !pip install -qU anthropic weave pydantic requests PyPDF2 set-env-colab-kaggle-dotenv
    return
...
@app.cell
def _(BaseModel, datetime, timezone):
...
@app.cell
def _(PdfReader, io, requests, weave):
    @weave.op()
...
def _(anthropic_client, weave):
    ...
    return (chain_of_density_summarization,)
...
@app.cell
def _(ArxivPaper, chain_of_density_summarization, load_pdf, weave):
    # Weave Model
    ...
    return (ArxivChainOfDensityPipeline,)
...
@app.cell
async def _(ArxivChainOfDensityPipeline, dataset, quality_scorer, weave):
    # Run evaluation
