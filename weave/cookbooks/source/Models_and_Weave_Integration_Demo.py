...
def _():
    ...
    return ENTITY, PROJECT, pd, wandb, weave
...
def _(PROJECT, wandb, weave):
    ...
    class UnslothLoRAChatModel(weave.Model):
        ...
        def model_post_init(self, __context):
            ...
            self._model, self._tokenizer = FastLanguageModel.from_pretrained(model_name=model_path, max_seq_length=self.cm_max_new_tokens, dtype=self.dtype, load_in_4bit=self.cm_quantize)
            FastLanguageModel.for_inference(self._model)
      # we can simply paste this from the "Use" tab from the registry
        @weave.op()
        async def predict(self, query: list[str]) -> dict:
            input_ids = self._tokenizer.apply_chat_template(query, tokenize=True, add_generation_prompt=True, return_tensors='pt').to('cuda')
            output_ids = self._model.generate(input_ids=input_ids, max_new_tokens=64, use_cache=True, temperature=1.5, min_p=0.1)
            decoded_outputs = self._tokenizer.batch_decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)  # unsloth version (enable native 2x faster inference)
            return ''.join(decoded_outputs).strip()  # add_generation_prompt = true - Must add for generation

    return (UnslothLoRAChatModel,)


@app.cell
def _(UnslothLoRAChatModel):
...
@app.cell
async def _(RagModel):
    await RagModel.predict("When was the first conference on climate change?")
    return


@app.cell
def _(RagModel, new_chat_model):
...
def _(weave):
    ...
    return (climate_rag_eval,)


@app.cell
async def _(RagModel, climate_rag_eval, wandb, weave):
...
def _(PUB_REFERENCE):
    ...
    return MODELS_OBJECT_NAME, MODELS_OBJECT_VERSION


@app.cell
def _(ENTITY, MODELS_OBJECT_NAME, MODELS_OBJECT_VERSION, PROJECT, wandb):
