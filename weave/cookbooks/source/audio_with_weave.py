...
def _():
    ...
    return OpenAI, base64, display, np, os, time, wave, weave
...
@app.cell
def _(base64, client, wave, weave):
    SAMPLE_RATE = 22050
    ...
    return SAMPLE_RATE, prompt_endpoint_and_log_trace
...
@app.cell
def _(SAMPLE_RATE, display, prompt_endpoint_and_log_trace):
...
def _():
    ...
    return Optional, io, json, pyaudio, resampy, threading, websocket
...
def _(pyaudio):
    ...
    return devices_data, p


@app.cell
def _(devices_data, p, pyaudio):
    ...
    INPUT_DEVICE_CHANNELS = devices_data[INPUT_DEVICE_INDEX]['maxInputChannels']
    # Audio recording and streaming parameters
    SAMPLE_RATE_1 = int(devices_data[INPUT_DEVICE_INDEX]['defaultSampleRate'])
    CHUNK = int(SAMPLE_RATE_1 / 10)
    SAMPLE_WIDTH = p.get_sample_size(pyaudio.paInt16)  # From device list above
    CHUNK_DURATION = 0.3
    OAI_SAMPLE_RATE = 24000
    OUTPUT_DEVICE_CHANNELS = 1  # From device list above  # Samples per frame  # Samples per frame for the format  # Seconds of audio per chunk sent to OAI API  # OAI Sample Rate is 24kHz, we need this to play or save assistant audio  # Set to 1 for mono output
    return (
        CHUNK,
        CHUNK_DURATION,
...
@app.cell
def _(INPUT_DEVICE_CHANNELS, SAMPLE_RATE_1, SAMPLE_WIDTH, io, np, wave):
    class StreamingWavWriter:
        ...
        def __init__(self, filename=None, channels=INPUT_DEVICE_CHANNELS, sample_width=SAMPLE_WIDTH, framerate=SAMPLE_RATE_1):
...
def _(
    ...
    class RTAudioModel(weave.Model):
        ...
        realtime_model_name: str = 'gpt-4o-realtime-preview-2024-10-01'
        stop_event: Optional[threading.Event] = threading.Event()  # realtime e2e audio only model interaction
        ws: Optional[websocket.WebSocket] = None
        user_wav_writer: Optional[StreamingWavWriter] = None  # Event to stop the model
        input_audio_buffer: Optional[np.ndarray] = None  # Websocket for OpenAI communications
        assistant_outputs: dict[str, StreamingWavWriter] = None
        playback_stream: Optional[pyaudio.Stream] = None
      # Stream for writing user output to file
        def __init__(self):
            super().__init__()  # Buffer for user audio chunks
            self.stop_event.clear()
            self.user_wav_writer = StreamingWavWriter(filename='user_audio.wav', framerate=SAMPLE_RATE_1)  # Assistant outputs aggregated to send to weave
            self.input_audio_buffer = np.array([], dtype=np.int16)
            self.ws = websocket.WebSocket()
            self.assistant_outputs = {}  # Playback stream for playing assistant responses
            if enable_audio_playback:
                self.playback_stream = pyaudio.PyAudio().open(format=pyaudio.paInt16, channels=OUTPUT_DEVICE_CHANNELS, rate=OAI_SAMPLE_RATE, output=True, output_device_index=OUTPUT_DEVICE_INDEX)
            try:
                self.ws.connect(f'wss://api.openai.com/v1/realtime?model={self.realtime_model_name}', header={'Authorization': f'Bearer {os.environ.get('OPENAI_API_KEY')}', 'OpenAI-Beta': 'realtime=v1'})
                config_event = SessionUpdate(session=Session(modalities=['text', 'audio'], input_audio_transcription=InputAudioTranscription(model='whisper-1'), turn_detection=TurnDetection(type='server_vad', threshold=0.3, prefix_padding_ms=300, silence_duration_ms=600)))
      ...
      # Open the assistant audio playback stream if enabled
        ...
        def handle_assistant_response_done(self, data: ResponseDone):
            ...
            weave_payload = {'assistant_audio': wave.open(wave_file_stream.get_wav_buffer(), 'rb'), 'assistant_transcript': data.response.output[0].content[0].get('transcript', 'Transcript Unavailable.')}
            return weave_payload

        @weave.op()
        def handle_user_transcription_done(self, data: ConversationItemInputAudioTranscriptionCompleted):
            return {'user_transcript': data.transcript}  # Send config msg

        def receive_messages_thread(self):
            while not self.stop_event.is_set():  # modalities to use
                try:
                    data = json.loads(self.ws.recv())
                    self.log_ws_message(json.dumps(data, indent=2))  # whisper-1 for transcription
                    ...
                    if parsed_event.type == ServerEventTypes.RESPONSE_AUDIO_TRANSCRIPT_DONE:
                        ...
                        self.handle_user_transcription_done(parsed_event)  # server VAD to detect silence
                        ...
                        print(f'\nError from server: {parsed_event.error.model_dump_json(exclude_none=True)}')  # Start listener
                    ...
                    break  ##### Weave Integration and Message Handlers #####

        def send_audio(self, audio_chunk):
            if self.ws and self.ws.connected:
                self.input_audio_buffer = np.append(self.input_audio_buffer, np.frombuffer(audio_chunk, dtype=np.int16))
                if len(self.input_audio_buffer) >= SAMPLE_RATE_1 * CHUNK_DURATION:
                    try:
                        resampled_audio = resampy.resample(self.input_audio_buffer, SAMPLE_RATE_1, OAI_SAMPLE_RATE) if SAMPLE_RATE_1 != OAI_SAMPLE_RATE else self.input_audio_buffer
                        audio_event = InputAudioBufferAppend(audio=base64.b64encode(resampled_audio.astype(np.int16).tobytes()).decode('utf-8'))
        ...
        def stop(self):
            ...
            if self.ws:
                self.ws.close()
            self.user_wav_writer.close()  ##### Message Receiver and Sender #####  # Resample audio to OAI sample rate  # Send audio chunk to OAI API  # Convert audio array to b64 bytes  # Clear the audio buffer  ##### General Utility Functions #####
...
def _(
    ...
    def record_audio(realtime_model: RTAudioModel) -> pyaudio.Stream:
        ...
        p = pyaudio.PyAudio()
        stream = p.open(format=pyaudio.paInt16, channels=INPUT_DEVICE_CHANNELS, rate=SAMPLE_RATE_1, input=True, input_device_index=INPUT_DEVICE_INDEX, frames_per_buffer=CHUNK, stream_callback=audio_callback)
        stream.start_stream()
...
@app.cell
def _(RTAudioModel, pyaudio, record_audio, time, weave):
