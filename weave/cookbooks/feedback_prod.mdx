...
Automatically evaluating a generated LLM response is often hard. Depending on your risk tolerance, you can gather direct user feedback to find areas to improve.

In this tutorial, you use a custom chatbot as an example app from which to collect user feedback. You use Streamlit to build the interface and capture the LLM interactions and feedback in W&B Weave. By the end, you have a running chatbot that logs each response to Weave alongside thumbs-up or thumbs-down reactions and free-text feedback from your users, so you can review production interactions and identify areas for improvement.
...
To attach feedback, you need the `call` object, which you obtain by using the `.call()` method _instead of calling the function as normal_:
