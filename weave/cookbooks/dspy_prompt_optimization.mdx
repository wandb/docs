...
- [Open in Google Colab](https://colab.research.google.com/github/wandb/weave/blob/master/docs/notebooks/dspy_prompt_optimization.ipynb)
- [View source on GitHub](https://github.com/wandb/weave/blob/master/docs/notebooks/dspy_prompt_optimization.ipynb)
</Note>
...
This tutorial demonstrates how we can improve the performance of our LLM workflow implemented  on the **causal judgement task** from the BIG-bench Hard benchmark and evaluate our prompting strategies. We will use [DSPy](https://dspy-docs.vercel.app/) for implementing our LLM workflow and optimizing our prompting strategy. We will also use [Weave](https://docs.wandb.ai/guides/weave) to track our LLM workflow and evaluate our prompting strategies.
...
- [Weave](https://docs.wandb.ai/guides/weave) to track our LLM workflow and evaluate our prompting strategies.
...
Weave is currently integrated with DSPy, and including [`weave.init`](https://docs.wandb.ai/guides/weave/tracking) at the start of our code lets us automatically trace our DSPy functions which can be explored in the Weave UI. Check out the [Weave integration docs for DSPy](https://docs.wandb.ai/guides/weave/integrations/dspy) to learn more.
...
In this tutorial, we use a metadata class inherited from [`weave.Object`](/weave/guides/tracking/objects) to manage our metadata.
...
We will load this dataset from HuggingFace Hub, split into training and validation sets, and [publish](https://docs.wandb.ai/guides/weave/core-types/datasets) them on Weave, this will let us version the datasets, and also use [`weave.Evaluation`](https://docs.wandb.ai/guides/weave/core-types/evaluations) to evaluate our prompting strategy.
...
Now that we have a baseline prompting strategy, let's evaluate it on our validation set using [`weave.Evaluation`](https://docs.wandb.ai/guides/weave/core-types/evaluations) on a simple metric that matches the predicted answer with the ground truth. Weave will take each example, pass it through your application and score the output on multiple custom scoring functions. By doing this, you'll have a view of the performance of your application, and a rich UI to drill into individual outputs and scores.
