...
description: "Learn how to use Weave tracing through the Service API with W&B Weave"
keywords: [Weave Service API, trace logging, RAG tracing, stream query, LLM observability]
---
...
In the following guide, you learn how to use the W&B Weave Service API to log traces. Specifically, you use the Service API to:
...
Before logging traces, you must configure the Service API endpoints and authenticate with your W&B credentials.

The following code sets the URL endpoints used to access the Service API:
...
- `project_id`: the name of the W&B project where you want to log your traces.
...
Now that you've logged a simple, single-step trace, the following sections walk you through creating a more complex trace with child spans, similar to a multi-operation RAG lookup:
...
The following code demonstrates how to add a child span to the complex parent trace started in the previous step. This step models the RAG document lookup sub-operation in the workflow.
...
- `inputs`: logs the search query, for example, 
...
The following code demonstrates how to add another child span to the complex parent trace, representing an LLM completion call. This step models the LLM response generation based on document context retrieved in the previous RAG operation.

The LLM completion trace is initiated with the `payload_child_start` object, which includes:
- `trace_id`: links this child span to the parent trace.
- `parent_id`: associates the child span with the parent workflow.
...
On success, the code outputs a message indicating the LLM child span trace has started:

```
Child call started. ID: 0245acdf-83a9-4c90-90df-dcb2b89f234a
```

After the operation completes, the `payload_child_end` object ends the trace by logging the LLM-generated response in the `output` field. The code also logs usage summary information.
...
The following code demonstrates how to finalize the parent trace, marking the completion of the workflow. This step aggregates the results of all child spans (for example, RAG lookup and LLM completion) and logs the final output and metadata.
...
- `output`: represents the final output of the workflow.
- `summary`: consolidates usage data for the workflow.
...
With traces logged to Weave, you can use the Service API to query them programmatically. The following code demonstrates how to query the traces created in previous examples, filtering only for traces where the `inputs.model` field is equal to `gpt-4o`.
...
On a successful query, the response includes trace data matching the query parameters:
