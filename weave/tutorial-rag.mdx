...
In some applications you may want to create custom evaluation classes - where for example a standardized `LLMJudge` class should be created with specific parameters (for example, chat model and prompt), specific scoring of each row, and specific calculation of an aggregate score. Weave defines a list of ready-to-use `Scorer` classes and also makes it easy to create a custom `Scorer` - the following example shows how to create a custom `class CorrectnessLLMJudge(Scorer)`.
...
**NOTE:** Sometimes the async execution of Evaluations will trigger a rate limit on the models of OpenAI, Anthropic, and others. To prevent that you can set an environment variable to limit the amount of parallel workers, for example, `WEAVE_PARALLELISM=3`.
