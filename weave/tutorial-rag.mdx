...
import { GitHubLink } from '/snippets/en/_includes/github-link.mdx';
...
First, compute the embeddings for our articles. You would typically do this once with your articles and put the embeddings & metadata in a database, but here we're doing it every time we run our script for simplicity.
...
Next, we wrap our retrieval function `get_most_relevant_document` with a `weave.op()` decorator and we create our `Model` class. We call `weave.init('<team-name>/rag-quickstart')` to begin tracking all the inputs and outputs of our functions for later inspection. If you do not specify a team name, the output is recorded to your [W&B default team or entity](/platform/app/settings-page/user-settings#default-team).
