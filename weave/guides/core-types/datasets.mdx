...
<Tabs>
  ...
  Then, use the `from_hf` method. If you provide a `DatasetDict` with multiple splits (like 'train', 'test', 'validation'), Weave will automatically use the 'train' split and issue a warning. If the 'train' split is not present, it will raise an error. You can provide a specific split directly (for example, `hf_dataset_dict['test']`).
...
![Agent conversation details panel header showing table icon to add items to a dataaset.](./imgs/agent-conversation-details-header-add-dataset.png)
