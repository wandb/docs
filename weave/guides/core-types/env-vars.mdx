...
| `WEAVE_ENABLE_WAL` | `bool` | `false` | Enable the [write-ahead log](/weave/guides/tracking/write-ahead-log). When set to `true`, API requests are persisted to disk before being sent to the server, improving resilience against data loss. |
| `WEAVE_DISABLE_WAL_SENDER` | `bool` | `false` | Disable the write-ahead log sender. When set to `true`, requests are written to disk but not flushed to the server. Useful for debugging. See [Write-ahead log](/weave/guides/tracking/write-ahead-log). |
