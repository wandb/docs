...
| `WEAVE_ENABLE_WAL` | `bool` | `false` | Enable the Weave [Write-Ahead Log (WAL)](/weave/guides/tracking/write-ahead-log). When set to `true`, Weave API requests are persisted to disk before being sent to the server, improving resilience against data loss. |
