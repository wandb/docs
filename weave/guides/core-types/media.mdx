...
The examples in this guide use annotations. We recommend annotations because they're the most direct way to start logging your media. For more advanced configurations, see the [Content API section](#using-the-contents-api).
...
<Note>
The TypeScript SDK supports logging images and audio. To log videos, documents, or HTML, use the Weave Python SDK instead.
</Note>
...
<Note>
To render media that already lives in your own cloud bucket without uploading the bytes to Weave, see [Reference media in your own bucket (BYOB)](/weave/guides/tracking/byob-references).
</Note>
...
<Frame>
![Weave trace view showing a generated image of a cat wearing a pumpkin hat](/weave/guides/core-types/imgs/cat-pumpkin-trace.png)
</Frame>
...
### Advanced example: Log an image generated with the OpenAI DALL-E API

The following example logs an image generated with the OpenAI DALL-E API using the `weaveImage` function:
...
Weave automatically handles `pdf`, `csv`, `md`, `text`, `json`, and `xml` file types. You can also log using file paths with `Annotated[str, Content]`.
...
<Frame>
![Weave trace view showing a logged audio file with an inline audio player](/weave/guides/core-types/imgs/audio-trace.png)
</Frame>
...
The following example generates audio using OpenAI's text-to-speech (TTS) API and logs it to Weave:
...
The following example creates a minimal HTML page and logs it to Weave:
...
This example generates self-contained HTML pages using [Serverless Inference](/inference) and logs the pages to Weave:
...
You can use the Content API in two primary ways: type annotations and direct initialization.

Type annotations automatically detect the proper constructor to use, while direct initialization provides more fine-grained control and lets you take advantage of the Content API's runtime features in your code.
...
For a comprehensive list of class attributes and methods, see the [Content reference docs](/weave/reference/python-sdk#class-content).
...
| `extension` | `str`            | File extension (for example, `"jpg"`, `"mp3"`) |
| `mimetype`  | `str`            | MIME type (for example, `"image/jpeg"`)        |
