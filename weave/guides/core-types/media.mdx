...
Use type annotations like `Annotated[bytes, Content]` or `Annotated[str, Content]` as input or return types in your ops to log media to Weave. You can also annotate path arguments with `Annotated[str, Content]` to have Weave automatically open, detect and display the media within your trace.
...
<Note>
The TypeScript SDK currently supports images and audio only. Video, documents, and HTML logging are available in the Weave Python SDK.
</Note>

</Tab>
</Tabs>

Each media section demonstrates how to log that media type with practical examples.
...
Weave logs the image and returns a link to view the image in the terminal.
...
Weave logs the resized image and returns a link to view it in the terminal.
...
async function main() {
    await weave.init('media-logging');
...
async function main() {
    await weave.init('image-example');
...
<Note>
The TypeScript SDK currently only supports PNG images via `weaveImage()`. For other formats, convert to PNG first.
</Note>
  </Tab>
</Tabs>
...
Weave logs the video and returns a link to view the video in the terminal.
...
<Note>
The TypeScript SDK currently only supports WAV audio via `weaveAudio()`. For other formats, convert to WAV first.
</Note>
