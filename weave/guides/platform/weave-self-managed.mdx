...
- `{installation-name}` = The `metadata.name` from ch-keeper.yaml (for example, `wandb`)
- `{cluster-name}` = The `configuration.clusters[0].name` from ch-keeper.yaml (for example, `keeper`)
- `{cluster-index}` = Cluster index, typically `0` for single cluster
- `{replica-index}` = Replica number: `0`, `1`, `2` for 3 replicas
- `{namespace}` = Kubernetes namespace (for example, `clickhouse`)
