...
<Steps>
  <Step title="Install the package:">
    ...
    Use the full name `weave-openclaw` (`weave` on its own is the W&B SDK, not this plugin). The plugin is loaded by the OpenClaw gateway through its config. You don't import it from application code.
  </Step>
  ...
  <Step title="Restart the gateway and verify:">
    Restart the OpenClaw gateway, then run `/weave status` in any OpenClaw chat surface to confirm the plugin is active. Traces will appear at `https://wandb.ai/[YOUR-TEAM]/[YOUR-PROJECT]` within a few seconds of your first agent run.
  </Step>
</Steps>
...
2. Check you haven't set `diagnostics.enabled: false` in the gateway config.
