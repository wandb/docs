...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    ```bash
    pnpm install weave @openai/agents zod
    ```
    ...
    The following code defines a `get_weather` tool and a `weather_assistant` agent, then runs a single question while Weave captures the trace.
    ...
    - A tile in the **Agents** tab for `weather_assistant`.
