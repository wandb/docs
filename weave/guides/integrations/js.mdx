...
For CommonJS projects, no special configuration is required. Automatic instrumentation works out of the box.  In the folder where your code is:
...
2. Compile your TypeScript file.
   ...
   This assumes your `tsconfig.json` outputs compiled files to a `dist` directory:
   ...
   After compiling, for an example local file `test.ts`, the file will be available as `dist/test.js`.

3. Run the compiled file with Node.js:
