...
description: "Trace activity between your MCP client and MCP server with Weave"
---
...
The Model Context Protocol (MCP) is a standardized communication protocol that enables AI applications to exchange information with large language models (LLMs). MCP provides an interface for LLMs to access data sources and interact with external tools, without requiring custom integrations for each new service.

The Weave integration lets you trace activity between your MCP client and MCP server. It gives you detailed visibility into tool calls, resource access, and prompt generation across MCP-based systems, so you can debug, audit, and optimize your MCP applications.

This guide explains how the integration works, how to enable tracing on the server and client sides, and walks through a full example you can run yourself.

## How it works

<Warning>
The integration captures client-side and server-side operations separately, but doesn't provide end-to-end visibility into their interaction. An ongoing proposal aims to add OpenTelemetry trace support to MCP to enable end-to-end observability. For more information, see [GitHub discussion #269](https://github.com/modelcontextprotocol/modelcontextprotocol/discussions/269).
</Warning>
...
The Weave integration works with both the MCP server and client. Once installed, you can enable tracing with two additional lines of code: one to import `weave`, and another to initialize it.
...
Configure the MCP integration through the `MCP_TRACE_LIST_OPERATIONS` environment variable. Set it to `true` to trace list operations (`list_tools`, `list_resources`, and `list_prompts`) on both server and client sides.

### Server-side integration

Use this section if you're building or instrumenting an MCP server. To trace an MCP server, add two lines to your existing `FastMCP` setup: one to import Weave and one to initialize the client. Once added, Weave automatically traces tool, resource, and prompt operations.
...
Use this section if you're building or instrumenting an MCP client. On the client side, tracing also requires two changes: import Weave and initialize it. Weave automatically traces all tool calls, resource accesses, and prompt requests.
...
The `mcp_demo` example demonstrates an integration between MCP and Weave for tracing. It showcases how to instrument both the client and server components to capture detailed traces of their interactions. By running through this tutorial, you'll see traces from both sides of an MCP application in the Weave UI and gain a concrete reference you can adapt to your own projects.
...
| `demo`               | Run a full demo of all available features. Runs each feature in sequence and produces a full trace timeline of interactions in the Weave UI. |
| `q`                  | Quit the session                         |
...
Weave automatically traces all server-side operations when you initialize the client with `weave.init()`.
...
- Show usage of [`weave.op()`](../tracking/ops) with custom methods and functions.

Weave traces all client-side calls to provide a complete view of interactions between the client and server.

## FAQ

The following section answers common questions about why and how to use Weave's MCP tracing.

### Why MCP tracing is needed

As an LLM application developer, you fall into one of three categories:

- _MCP server-side developer_: You want to expose multiple tools, resources, and prompts to the MCP client. You expose your existing application's tools and resources, or you've built agents or have multiple agents orchestrated by an orchestrator agent.
...
- _MCP server and client developer_: You're developing both the server and the client.
...
If you're developing both the server and client, a unified trace timeline helps you iterate through both server and client-side logic.

In any case, an observability layer lets you:

- Iterate through your application.
