---
title: Verifiers
description: "Track Verifiers RL environments and LLM agent training with Weave, capturing multi-round conversations and evaluation rollouts."
keywords: ["Verifiers", "reinforcement learning", "RL training", "rollouts", "GSM8K"]
---
...
This guide is for ML engineers and researchers who want to add observability to their Verifiers-based RL workflows. It shows you how to install Verifiers, W&B, and Weave, and provides two examples of how to use Verifiers with Weave and W&B: tracing rollouts during evaluation, and fine-tuning a model with experiment tracking and tracing enabled.
...
Weave enables [implicit patching](/weave/guides/integrations#automatic-implicit-patching) for the library by default. This lets you use Weave with Verifiers without requiring explicit call patch functions.

With the libraries installed, you're ready to run the examples in the following sections.
...
Weave provides insight into how your models perform during RL fine-tuning. When you use it alongside W&B, you get observability across the workflow. W&B tracks training metrics and performance charts, while Weave captures detailed traces of each interaction during the training process.
...
<Note>
This example was tested on 2xH100s with the following environment variables set for increased stability:
...
Once training begins, you can [view the traces](/weave/guides/tools/weave-in-workspaces) logged during your run in the UI. W&B captures the training metrics while Weave records the per-interaction traces, giving you both high-level performance trends and detailed step-by-step visibility into your training run.
...
Verifiers has first-class integration with W&B Models. For more information, see the [Verifiers repository](https://github.com/willccbb/verifiers).
