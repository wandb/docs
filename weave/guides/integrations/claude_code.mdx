...
For CI, bootstrap scripts, or other automation where you must not be prompted, set the `--non-interactive` flag. The installer still configures the plugin and registers Claude Code the same way. It uses the environment variables when they are set and warns if either is missing:
