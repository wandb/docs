...
- You can use `wandb login` with the `host` flag to persist the base URL across sessions. This writes the host URL to `$HOME/.config/wandb/settings`, which the Weave client reads automatically without requiring the environment variable to be set.

```bash lines
wandb login --host https://your-instance.wandb.io
```

If you ever need to change `WANDB_BASE_URL` or the `host` flag after the plugin's daemon is already running, restart the daemon for the change to take effect:
