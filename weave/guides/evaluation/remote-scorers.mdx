...
The [`examples/remote_scorer`](https://github.com/wandb/weave/tree/master/examples/remote_scorer) directory in the `weave` repository is the reference implementation of this contract and the source of truth for the sample code. It's deployment independent: the Python service shows the contract, not a hosting recommendation. For Calls, these files apply:
