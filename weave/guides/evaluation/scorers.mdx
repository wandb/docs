...
This guide is for developers who want to measure the quality of their AI system's outputs. It explains how Scorers fit into Weave evaluations, how to build your own Scorers, how to apply Scorers to individual calls, and how to analyze the resulting scores.
...
<Tabs>
  ...
  <Tab title="TypeScript">
    These are functions wrapped with `weave.op` that accept an object with `modelOutput` and optionally `datasetRow`. They work well for straightforward evaluations like the following example:
    ```typescript lines
...
To score production calls with a scorer that runs on your own infrastructure, attach a [remote scorer](/weave/guides/evaluation/remote-scorers) to a monitor.
