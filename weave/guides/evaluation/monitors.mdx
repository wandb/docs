...
Monitors passively score production traffic to surface trends and issues without requiring code changes. They run asynchronously in the background and have no impact on your application's control flow.
...
Monitors are ideal for ongoing observation and analysis. You can set them up entirely in the W&B UI without changing your code. All scorer results are automatically stored in Weave's database, allowing you to analyze historical trends and patterns.
...
3. In the Create new monitor menu, configure the following fields:
   ...
   - **LLM-as-a-judge configuration**:
     ...
     - **Judge model**: Select the model to score your ops. The menu contains any commercial LLM models you have configured in your W&B account and [W&B Inference models](/inference/models). Audio-enabled models have an **Audio Input** label beside their names. For the selected model, configure the following settings:
