---
title: "Set up guardrails and monitors"
...
Weave's powerful scoring framework can help you develop guardrails and monitors that improve your LLM application's output quality and ensure they're generating safe, appropriate content. This guide shows you how to use custom Weave scorers to build guardrails and monitors that can:

* Block toxic content in responses
* Redact PII and sensitive data from being shared in responses
* Track and ensure accuracy of information
* Sample and monitor only a certain volume of calls to your application to reduce costs

## Guardrails vs. Monitors

This document teaches how to use Weave's [Scorer](/weave/guides/evaluation/scorers) component to create:

- **Guardrails**: Block or modify unsafe content before it reaches users
- **Monitors**: Track quality metrics over time to identify trends and improvements
...
For example, you could use a toxicity scorer both as a guardrail to block toxic content immediately, and as a monitor to track toxicity levels over time.

Every scorer result is automatically stored in Weave's database. This means your guardrails double as monitors without any extra work and you can always analyze historical scorer results, regardless of how they were originally used.

## Getting Started

The following example demonstrates a basic scorer used assess and log a response's length:
...
def generate_text(prompt: str) -> str:
    return "Hello, world!"  # <-- This return value...

class LengthScorer(Scorer):
    @weave.op
    def score(self, output: str) -> dict:
        #              ^^^^^^
        #              ...automatically gets passed here as 'output'
        print(f"Received output: {output}")  # Would print "Hello, world!"
        return {
            "length": len(output),
...
# result = "Hello, world!"
# call contains trace info + the result

await call.apply_scorer(LengthScorer())
# Weave automatically passes "Hello, world!" as the output parameter
```

When run, the code uses `@weave.op` to trace the `generate_text()` function. The `.call()` method returns two values: the function's result and a Call object containing trace metadata. 

When you invoke `call.apply_scorer(LengthScorer())`, Weave automatically:
1. Extracts the function's output from the Call object
2. Passes it to the scorer's `score()` method as the `output` parameter
3. Associates the resulting score with the traced call in the database

This association between scores and traces is a fundamental part of Weave's value. Without the Call object, scores would be isolated data points with no connection to the original function execution, making them impossible to query or analyze over time.
...
For detailed information about querying calls and their scorer results, see our [Score Analysis Guide](./scorers#score-analysis) and our [Data Access Guide](/weave/guides/tracking/tracing#querying--exporting-calls).
...
- Explore [Available Scorers](./scorers)
