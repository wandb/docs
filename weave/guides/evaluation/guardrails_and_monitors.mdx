...
The foundation of Weave's evaluation system is the [**Scorer**](./scorers) - a component that evaluates your function's inputs and outputs to measure quality, safety, or any other metric you care about. Scorers are versatile and can be used in two ways:
...
While this guide shows you how to create custom scorers, Weave comes with a variety of [predefined scorers](./builtin_scorers) that you can use right away, including:
...
For more details about Call objects, see the [Calls guide section on Call objects](../tracking/tracing#getting-a-handle-to-the-call-object-during-execution).
</Tip>
...
You can create monitors to observe and judge both text and audio. For example, you can create a monitor that checks the text responses of a support chatbot for toxicity. You can also create a monitor that passes an MP3 or WAV file to an LLM judge to check for the quality of an AI drive-thru assistant's responses during customer orders.
...
3. In the drawer, configure the monitor:
   ...
   - **LLM-as-a-Judge configuration**: 
        ...
        - **Judge model**: Select the model to score your ops. The menu contains any commercial LLM models you have configured in your W&B account and [W&B Inference models](/inference/models). To filter for audio-enabled models that you can audio files to, type **audio** into the search bar.
