...
<Note>
The Weave TypeScript SDK does not support the tools required to set up inline guardrails.
</Note>

### Optimize your Weave guardrail performance

Because guardrails can interrupt your application's control flow and change the course of its responses, they can negatively impact its performance if written too complexly. For optimal performance, we recommend:

* Keeping guardrail logic simple and fast
* Caching common results
* Avoiding heavy external API calls
* Initializing guards outside of your main functions to avoid repeated initialization costs
...
The following example sends user prompts to OpenAI's GPT-4o mini model. The model's response is then passed to Weave's [OpenAI's moderation API](https://platform.openai.com/docs/guides/moderation) integration to assess whether the LLM's response contains harmful or toxic content.
