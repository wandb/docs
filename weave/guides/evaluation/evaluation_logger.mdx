...
Send the span to the same Weave project as the evaluation through the `/agents/otel/v1/traces` endpoint. OTel span attributes don't propagate from parent spans to child spans, so set the attributes on every span that you want to link.
