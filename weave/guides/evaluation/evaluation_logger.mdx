...
keywords: ["EvaluationLogger", "log_prediction", "log_score", "incremental evaluation", "rich media logging"]
---

This guide shows you how to use `EvaluationLogger` to record predictions and scores from your existing Python or TypeScript code, so you can evaluate model performance in Weave without first defining a full dataset and scorer suite.
...
For a Python example that demonstrates this workflow, see the [Basic example](#basic-example). If the output and all scores are available at once, Python users can combine steps 2 through 4 into a single call using [`log_example()`](#simplified-logging-with-log_example).
...
The following example evaluates model predictions with the fire-and-forget pattern. It sets up an evaluation logger, runs a model on three test samples, and then logs the prediction without using await:
...
Use the awaitable API when you need to ensure each operation completes before proceeding, such as when managing error handling or sequential dependencies. 
...
The following example logs a prediction, adds scores and an LLM judge score, then finalizes the prediction with `finish()`:
...
In Python, keep each traced agent call inside its `log_prediction()` context. `EvaluationLogger` stamps spans created in that context with the evaluation run, example, and trial metadata that Weave uses to link the trace to its evaluation result.

The following example uses the [OpenAI Agents SDK](../integrations/agents/openai-agents-sdk). The same pattern applies to other agent frameworks that Weave traces. Replace `[TEAM]/[PROJECT]` with your W&B entity and project.
...
If the agent runs before the prediction context starts or after it finishes, Weave records the trace but does not link it to the evaluation result.

#### Set evaluation attributes on raw spans

You don't need to set evaluation attributes on spans created inside the `log_prediction()` context. `EvaluationLogger` sets them automatically. If you send spans with your own OpenTelemetry (OTel) instrumentation, or create spans in another process, set the attributes directly on each span that you want to link. OTel span attributes do not propagate from parent spans to child spans.

The span must be sent to the same Weave project as the evaluation. For information about configuring the destination project, see [Send OpenTelemetry traces to Weave](/weave/guides/tracking/otel).
...
| `weave.eval.kind` | string | The evaluation category. Weave uses `agent` for agent evaluations and `standard` for standard evaluations. |
| `weave.eval.row_digest` | string | A stable digest that identifies the evaluated dataset row. `EvaluationLogger` derives this value from the prediction inputs unless you provide one. |
| `weave.eval.example_id` | string | A caller-provided identifier for the evaluated example. |
| `weave.eval.trial_index` | integer | The zero-based trial number for the dataset row. |
| `weave.eval.evaluation_name` | string | The evaluation's human-readable name. |
| `weave.eval.project_id` | string | Project context stamped by the Weave SDK. This attribute does not route or link the span; configure the destination project on the OTel resource instead. |

Only `weave.eval.run_id` and `weave.eval.predict_and_score_call_id` establish the evaluation and result links. The row digest, example ID, trial index, kind, and evaluation name add context and support filtering, but do not create a link by themselves. Use Weave call IDs for the two linkage attributes, not OTel trace or span IDs.
...
The following examples assume that `span` is the OpenTelemetry span for the agent operation. Replace each bracketed value with metadata from the evaluation run and result that the span belongs to. You can omit optional attributes that you don't have.
...
#### Propagate evaluation IDs to another service

Entering the `log_prediction()` context creates the `Evaluation.predict_and_score` call before the context body runs. Use the returned `ScoreLogger` to get both call IDs, then send them to the service that runs the agent. Keep the context open until the service returns so that you can log its output and scores on the same evaluation result.

In the evaluation process, replace `[AGENT-SERVICE-URL]` with the endpoint that runs your agent:
...
In the agent service, copy the received attributes onto every agent span that you want to associate with the result. The following function demonstrates the receiving side with a raw OTel span. Configure the service to export spans to the same `[TEAM]/[PROJECT]` as the evaluation.
...
The wrapper span in this example is linked to the evaluation result. If the agent framework creates additional spans, copy `eval_context` onto those spans as well; OTel does not inherit span attributes from the wrapper.

To inspect linked traces in the UI:

1. Open the evaluation run in **Evals**.
2. In **Results**, click **View spans** to open **Agents > Spans** filtered to that evaluation.
3. Open an example preview and click **View spans** to inspect the agent spans linked to that example.

### Link to an existing dataset

When you pass raw datasets as `inputs` to `log_prediction`, Weave reimports the data with every evaluation run. This stores duplicate data, which can waste space if the dataset is large or if many evaluations reuse it.
...
For more information about comparisons, see [Comparisons](../tools/comparison).
...
<Tab title="TypeScript">
- **Auto-finish behavior**: While we recommend explicitly calling `finish()` on each prediction for clarity, `logSummary()` automatically finishes any unfinished predictions. However, once the script calls `finish()`, it can't log any more scores.
