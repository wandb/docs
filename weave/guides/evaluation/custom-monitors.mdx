...
To score calls with an HTTP endpoint that you host instead of an LLM judge, see [Score calls with remote scorers](/weave/guides/evaluation/remote-scorers).
...
You can also [compare](/weave/guides/tools/comparison) and visualize the monitor's trace data in the Weave UI, or download it in formats such as CSV and JSON using the download button (<Icon icon="download" iconType="regular" />) in the **Traces** tab. 
