...
keywords: ["automations", "monitors", "Slack notification", "webhook", "alerts"]
---

Automations let you define event-driven rules that trigger actions when your project meets specific conditions. Instead of manually watching dashboards for metric changes, you configure an automation once, and W&B Weave runs an action for you when the event occurs. Use cases include:
...
You manage automations in the Weave UI, with no code changes required. This guide walks you through creating an automation from the Automations page, creating one from an existing monitor, and viewing and managing automations after you create them.
...
Before you create the automation that connects them, the triggering event (such as an Op or a monitor) and the action (such as a Slack integration) must exist.
...
4. In the **Create automation** dialog, configure the event:
    ...
    - **Source**: Select the automation source to be either an **Operation** or a **Monitor** from your project.
        - **Operation**: Select the **Op name** whose traces the monitor evaluates. The list contains Ops that have logged at least one trace in the project.
...
5. Review the **Alert preview** graph to confirm the thresholds alert the way you expect.
6. Click **Next step** to proceed to action configuration.

### Configure the action

The action configuration defines what action happens when the event meets the condition.
...
1. Configure the automation metadata:
    - **Automation name**: Provide a name for the automation that appears in the Automations table.
...
5. In the **Create automation** panel, Weave pre-fills the event configuration with the selected monitor's details. Adjust the configuration as needed:
    - **Metric**: Select the metric to measure for the alert.
    - **Metric Threshold**: Define the condition that triggers the automation. Select a comparator (such as "is above") and provide the threshold value (such as `0.9`).
    - **Window**: Configure the window used to trigger the automation to be either **Time-based** or **Count-based**. Select the aggregation function (such as **Average**) and the rolling period (such as **1 day**).
    - **Aggregation**: Set the aggregation function (such as mean, median, minimum) to apply to metric values within the window before comparing to the threshold.
...
The **Automations** page lists all automations in your project. The table displays the following information for each automation:

- **Automation**: The automation name and description.
- **Event type**: The type of event that triggers the automation.
- **Action type**: The action that the automation triggers, such as generating a Slack notification.
- **Date created**: When you created the automation.
...
The History tab displays a record of all triggered automations. When an automation triggers, Weave creates an automation instance that records what happened. You can inspect these instances to see when and why an automation triggered.
