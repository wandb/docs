...
1. In the upper right corner of the Playground, click the **Chat settings (<Icon icon="sliders" iconType="regular" />)** icon to open the chat settings panel.
...
To view message history, click the **History (<Icon icon="clock-rotate-left" iconType="light" />)** icon to the upper right of the Playground.  This opens a History panel showing all messages sent for the current project.  
...
1. To the upper right of the Playground, click **Add Chat (<Icon icon="plus" iconType="solid" />)**. A second chat opens next to the original chat.
