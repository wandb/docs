...
keywords: ["LLM playground", "prompt engineering", "model comparison", "custom runtimes", "custom providers", "saved models"]
---
...
Evaluating LLM prompts and responses is challenging. The Weave Playground simplifies the process of iterating on LLM prompts and responses, helping you experiment with different models and prompts. With features like prompt editing, message retrying, and model comparison, Playground helps you test and improve your LLM applications. Playground is intended for developers and prompt engineers who want to test, compare, and refine LLM prompts before integrating them into applications. Playground supports models from providers such as OpenAI, Anthropic, and Google, as well as [custom runtimes](#custom-runtimes).
...
- **Custom runtimes:** Test OpenAI-compatible API endpoints for custom models.
...
In addition to the built-in providers, the Playground supports custom runtimes: OpenAI-compatible API endpoints that you register to your project. Custom runtimes were previously called *custom providers*. Use a custom runtime to test models such as an agent prototype your team serves, a local model, or an older version of a supported provider's model.

To register a custom runtime in the UI or with the Python SDK, see [Custom runtimes](/weave/guides/tools/custom-runtimes). After you register a custom runtime, select its models from the **Select a model** dropdown and test them using standard Playground features.

### Use a local model as a custom runtime
...
3. In a separate terminal, create an ngrok tunnel with the required CORS headers:
   ...
   ```
4. After ngrok starts, it displays a public URL, such as `https://xxxx-xxxx.ngrok-free.app`. Use this URL as the **Base URL** when you [register a custom runtime](/weave/guides/tools/custom-runtimes#register-a-custom-runtime-in-the-ui).
