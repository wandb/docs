...
By default, Weave automatically detects and associates traces with the active `wandb.run`. If you need to associate traces with a specific run that is not the global `wandb.run`, use `set_wandb_run_context` on the Weave client:
...
client.set_wandb_run_context(run_id="<your-run-ID>", step=5)
