...
2. Click the three dots to the right of the ID.
   ![Make baseline option displayed.](/weave/guides/tools/imgs/comparison-2objs-baseline.png)
3. In the dropdown, select **Make baseline**. The UI refreshes so that the baseline object is furthest left in the topbar, and `Baseline` displays next to the ID.
    ![Baseline set.](/weave/guides/tools/imgs/comparison-2objs-baseline-set.png)
...
2. Click and hold the six dots to the left of the ID and drag it to the left or the right as needed.
   ![Setting the order.](/weave/guides/tools/imgs/comparison-2objs-reorder.png)
3. Place the ID in the desired ordering. The UI refreshes with an updated comparison ordering.
...
1. In the right hand corner of either the Side-by-side or Unified Comparison view, click the dropdown. Depending on your current view configuration, the dropdown is either titled **Compare with previous** or **Compare with baseline**.
...
When comparing 3 or more objects, you can compare a single object to its adjacent object or a baseline. This changes the Comparison table view so that the view is identical to a two-object comparison. To compare a pair of objects from a multi-object comparison, do the following:

1. In the Comparison view topbar, find the ID that you want to compare.  If you have a baseline set, the comparison will be made to that object. Otherwise, it will compare to the prior ID (the ID to the left) in the list of selected IDs.
2. To select the item, click the ID. The UI refreshes with a two-way comparison table.
    ![Comparing a pair from a multi-object comparison.](/weave/guides/tools/imgs/comparsion-7objs-diffonly-subset.png)
