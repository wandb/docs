...
For custom agents, you can instrument any agent code by hand using `weave.start_session`, `weave.start_turn`, `weave.start_llm`, and `weave.start_tool`. 

See the [Trace an agent quickstart](/weave/agents-quickstart) for a walkthrough on how to trace custom agents.
