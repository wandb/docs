...
For an agent turn, `schema_version` is `2` and the scored data is under `scoring_target`, a tagged union with three fields:
...
| The endpoint returns `400` for agent turns | The endpoint dispatches on the top-level `schema_version` and accepts `scoring_target.type` `agent_turn` with inner `schema_version` `1`. |
