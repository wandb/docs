...
Dispatch on the envelope version first, then on the pair of `scoring_target.type` and `scoring_target.schema_version`. Return `4xx` for a pair your endpoint doesn't implement, such as `call` if you score only agent turns.
...
| The endpoint returns `400` for agent turns | The endpoint dispatches on the top-level `schema_version` and accepts `scoring_target.type` `agent_turn` at payload version `1`. |
