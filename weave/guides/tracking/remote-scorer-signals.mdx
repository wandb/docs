...
The sample requires Weave `0.53.0` or later. A local run verifies only the contract. To test with Weave, deploy the endpoint at an HTTPS URL on the allowed hosts and register it as previously described in [Create a remote scorer signal](#create-a-remote-scorer-signal).
