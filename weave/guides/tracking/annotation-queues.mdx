...
keywords: ["annotation queue", "human feedback", "trace review", "dataset labeling", "domain experts"]
---
...
This guide shows you how to set up annotation queues in Weave so you can collect structured feedback from domain experts on selected traces. Use annotation queues when you need expert-labeled data for evaluation, failure analysis, or dataset creation. Annotation queues also let reviewers focus on a curated set of traces rather than the full Weave UI.
...
The following workflow summarizes how to use annotation queues to obtain reviews. The following sections walk through each step in detail.
...
Before you create an annotation queue, define your annotation fields so they can be selected during queue setup. Annotation fields define the feedback that the annotator provides for each trace item. Fields are reusable across queues and projects.
...
The new field appears in the **Fields** tab and is available to add to any annotation queue in the project.

<Note>
Fields can't be edited after creation to ensure annotation consistency.
</Note>

## Create an annotation queue

With your annotation fields defined, the next step is to create the queue that annotators work from. An annotation queue consists of:
...
Creating an annotation queue defines the fields and guidelines for evaluation, but you still need to add traces to the queue to identify the data to evaluate.

### Add traces to a queue

Once a queue exists, you populate it with the specific traces you want reviewed. Add traces to an annotation queue directly from the **Traces** page.
...
5. In the **Add to annotation queue** modal dialog, configure:
    ...
    - **Select trace data to display**: Select the Weave trace data elements to display to the annotator as they evaluate the results.
        - **Inputs**: Select which trace input fields to show during annotation.
        - **Outputs**: Select which trace output fields to show during annotation.
6. Click **Add `[COUNT]` traces to annotation queue** to assign these traces as a part of the annotation queue review.


When you add traces, you control which trace inputs and which outputs or model responses are reviewed, so annotators see only the context needed to make accurate judgments.


## Monitor review progress

After traces are added, annotators can begin work and you can track how the review progresses. Share the queue name with your annotator so they can begin their review. See [Review items in an annotation queue](/weave/guides/tracking/annotation-review) for details on the review process.
...
After annotators have submitted reviews, you can use the resulting feedback for analysis, evaluation, or training. Weave stores completed annotations as structured metadata on traces.
...
4. To also filter on 'completed' queue items only, click **+ Add Filter**:
    - For **Column**, type "feedback". A dialog populates with annotations and includes your annotation field names. Choose a required field from your queue.
...
5. Filter rows are automatically applied. Click elsewhere in the page for the filter entry to close.
6. Optional: Save as a view for quick access. In the Traces table header, click **Save View**.
...
In addition to the UI workflow, you can read annotation data programmatically. This is useful for automating analysis, building custom reports, or integrating annotations into downstream pipelines. To integrate annotations programmatically, you must know your project name and queue ID:
