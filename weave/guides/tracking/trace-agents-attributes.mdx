...
![The Spans tab with a span selected that has events. The detail panel on the right shows the Raw data tab with the events array expanded, listing the events recorded on the span.](./imgs/agent-view-spans-attributes-events.png)
