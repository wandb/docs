...
When you enable the WAL, the client writes each API request to a local JSONL file first, then flushes the file to the server. If a process crashes or the server is unreachable, the data remains on disk and is automatically sent when the client runs again.
...
| `WEAVE_DISABLE_WAL_SENDER` | `bool` | `false` | Disables the WAL sender. When set to `true`, Weave writes the requests to disk locally but does not flush them to the server. This is useful for debugging. |
