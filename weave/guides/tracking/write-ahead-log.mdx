...
The write-ahead log (WAL) addresses this by persisting requests to disk before sending them. When you enable the WAL, the client writes each API request to a local JSONL file first, then flushes the file to the server. If a process crashes or the server is unreachable, the data remains on disk and is automatically sent when the client runs again.
...
These variables are also listed on the [environment variables](/weave/guides/core-types/env-vars) reference page.
