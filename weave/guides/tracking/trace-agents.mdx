...
| Session | `Session` | *(no span, turns are grouped by the `conversation_id` attribute)* | A conversation or run that contains one or more turns. | [Python](/weave/reference/typescript-sdk/classes/session) <br /> [TypeScript](/weave/reference/typescript-sdk/classes/session) |  
