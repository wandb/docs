...
| Conversation | `Conversation` | *(no span, turns are grouped by the `conversation_id` attribute)* | A conversation or run that contains one or more turns. | [Python](/weave/reference/python-sdk#class-conversation) <br /> [TypeScript](/weave/reference/typescript-sdk/classes/conversation) |  
...
```typescript TypeScript lines twoslash
// @noErrors
...
```python Python lines highlight="13,14,17,25,29"
import weave
...
```typescript TypeScript lines highlight="11,13,16,24,35" twoslash
// @noErrors
import * as weave from 'weave';
...
```python Python lines highlight="1,2,4,9,15"
...
```typescript TypeScript lines highlight="1,2,4,9,15" twoslash
// @noErrors
const conversation = weave.startConversation({ agentName: 'weather-bot' });
const turn = conversation.startTurn({ agentName: 'weather-bot' });
