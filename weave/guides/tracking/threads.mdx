...
keywords: [threads, multi-turn conversations, thread_id, turns, agent loops]
---
...
- Is a direct child of a thread context and may include nested lower-level calls (not shown in thread-level stats)
...
A _Trace_ captures the full Call stack for a single operation. Threads group traces together that are part of the same logical conversation or session. In other words, a thread is made up of multiple turns, each representing one part in the conversation. For more information about Traces, see the [Tracing overview](/weave/guides/tracking/tracing).

## UI overview

In the Weave project sidebar, select **Threads** to access the [Threads list view](#threads-list-view).
...
- To view the full execution of a turn, you can open it from the thread detail drawer. This lets you drill into all nested operations that occurred during that turn.
- If a turn includes messages extracted from LLM calls, they appear in the chat pane. These messages typically come from calls made by supported integrations (for example, `openai.ChatCompletion.create`) and must meet certain criteria to display. For more information, see [Chat view behavior](#chat-view-behavior).
...
<Frame>
![The Threads chat pane showing structured LLM messages](/weave/guides/tracking/imgs/threads-chat-view.png)
</Frame>

#### What qualifies as a message

Weave extracts messages from Calls within a turn that represent direct interactions with LLM providers (for example, sending a prompt and receiving a response). Only Calls that aren't further nested inside other Calls appear as messages. This avoids duplicating intermediate steps or aggregated internal logic.

Typically, automatically patched third-party SDKs emit messages, for example:
- `openai.ChatCompletion.create`
- `anthropic.Anthropic.completion`

#### What happens when no messages are present

If a turn doesn't emit any messages, the chat pane shows an empty message section for that turn. The chat pane may still include messages from other turns in the same thread.

#### Turn and chat interactions

- Clicking a turn scrolls the chat pane to that turn's message location (pinning behavior).
- Scrolling the chat pane highlights the corresponding turn in the turn list.

#### Navigate to and from the trace view

You can open the full trace for a turn by clicking into it.

A back button appears in the upper left corner to return to the thread detail view. Weave doesn't preserve UI state (such as scroll position) across the transition.

<Frame>
![The Threads detail drawer with the back button to return to the thread view](/weave/guides/tracking/imgs/threads-drawer.png)
</Frame>

## SDK usage

The following sections describe how to create and manage threads programmatically using the Weave SDK. Each example demonstrates a different strategy for organizing turns and threads in your application. For most examples, you should provide your own LLM call or system behavior inside the stub functions.
...
Nested `weave.thread()` contexts always start a new thread unless you reuse the same `thread_id`. Ending a child context doesn't interrupt or overwrite the parent context. This allows for forked thread structures or layered thread orchestration, depending on your app logic.

### Basic thread creation

The following code sample demonstrates how to use `weave.thread()` to group one or more operations under a shared `thread_id`. This is a straightforward way to start using Threads in your application.
...
The following sections describe the Threads query endpoint, its request and response schemas, and common query patterns you can use to retrieve thread data programmatically.
