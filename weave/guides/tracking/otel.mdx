...
## Organizing traces into threads

You can organize your OpenTelemetry traces into [Weave threads](/weave/guides/tracking/threads) by adding specific span attributes. This allows you to group related operations (like multi-turn conversations or user sessions) and visualize them in Weave's thread UI.
...
# Create a multi-turn conversation in a thread
thread_id = "conversation_123"

# First turn
with tracer.start_as_current_span("process_user_message") as span:
    span.set_attribute("wandb.thread_id", thread_id)
    span.set_attribute("wandb.is_turn", True)
    # Your logic here...
...
## Organizing OTEL traces into threads
...
The following code shows four examples of how to organize OTEL traces into Weave threads using `wandb.thread_id` (to group related operations) and `wandb.is_turn` (to mark high-level operations that appear as rows in the thread view). The examples perform the following:

* **Example 1**: Traces a basic single-turn thread
* **Example 2**: Traces a multi-turn conversation sharing one thread ID
* **Example 3**: Traces deeply nested operations where only the outermost span is marked as a turn
* **Example 4**: Traces background operations that belong to a thread but aren't turns

<Accordion title="Click to open the code example">
...
def main():
    """Run all examples"""
    print("Starting OpenTelemetry Threads Examples")
    print("======================================")
    ...
    print("1. wandb.thread_id groups related spans into threads")
    print("2. wandb.is_turn=true marks top-level operations as turns")
    print("3. Nested spans within turns should have is_turn unset or false")
    print("4. Multiple turns can share the same thread_id")
    print("5. Non-turn operations can be part of a thread")
