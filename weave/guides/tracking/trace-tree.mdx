...
keywords: ["trace tree", "flame graph", "call stack", "debugging traces", "scrubbers"]
---
...
This guide is for developers who use Weave to instrument LLM or agentic applications and want to inspect the resulting traces. It describes how to move through the trace stack, filter and search for functions called by your code, switch between visual representations, and more.

The following sections describe how to open the Trace view, orient yourself within the Traces page, navigate and filter a trace, switch between alternative visualizations, and inspect details for a specific op.
...
After you complete these steps, you are in the Trace Details view for your selected trace and can use the rest of this guide to navigate it.
...
The trace tree shows a hierarchy of all the methods tracked within the trace. To move up or down the tree, use `Cmd` (macOS) or `Alt` (Windows/Linux) + Up Arrow (↑) or Down Arrow (↓).

Several scrubbers below the trace tree provide rapid navigation across states within it. You can use the sliders to navigate through your trace.
...
At the top of the panel, you can switch between multiple visual representations of the trace tree based on your debugging needs. Use **code composition view** to understand call logic, **flame graph view** to understand performance over time, and **graph view** to understand structure.
