...
<Warning>
Because generators yield values lazily, the outputs are only logged when the generator is fully consumed (such as by converting it to a list).
To ensure that outputs are captured in the trace, fully consume the generator (for example, by using `list()`).
</Warning>
