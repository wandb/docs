...
## Apply and organize ops with kinds and colors

To better organize your ops in the Weave UI, you can apply custom kinds and colors to them by adding the `kind` and `color` arguments to the `@weave.op` decorators. For example, the following code applies a `LLM` kind and a `blue` color to the parent function, and a `sync` kind and a `red` color to a nested function:
...
def llm_func():
    @weave.op(kind="sync", color="red")
    def sync_func():
        return "sync result"

    sync_result = sync_func()
    
    return f"llm result with {sync_result}"
...
<Frame>
![The Weave UI, showing a parent call with a `LLM` kind and a `blue` color, and a nested call with a `sync` kind and a `red` color.](/images/weave/weave_colors_kinds.png)
</Frame>
