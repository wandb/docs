...
You can store metrics or other post-call values in the `summary` dictionary of a call. Modify `call.summary` during execution. Any values you add will be merged with Weave's computed summary data when the call finishes.

### 2. Decorate and wrap functions
