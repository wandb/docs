...
Use the `summary` property to attach custom, post-execution data to a Call so you can analyze it later alongside Weave's built-in metrics. It's a dictionary you can write to during a Call's execution. When the Call finishes, Weave deep-merges your values with its own computed data and stores the result.
...
print(weave_summary.get("latency_ms"))   # Null if the Call is still running.
...
console.log(weaveSummary?.latency_ms);   // Undefined if the Call is still running.
