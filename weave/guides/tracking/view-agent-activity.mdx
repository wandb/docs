...
A turn is one back-and-forth exchange with the agent. Use this selector to restrict which turns the signal scores, such as a specific `Agent name`, `Operation name`, `Tool name`, or `Request model`. Multiple filters are combined with `AND` logic.
