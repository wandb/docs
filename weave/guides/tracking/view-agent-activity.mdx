...
- **Tags**: Tags-based scorers assign a tag to each matching turn that meets your criteria (for example, detecting user frustration). Weave creates a Signals table row only when a turn matches the tag criteria. If you create a tag signal and see no rows, the scorer likely hasn't found a match yet.
