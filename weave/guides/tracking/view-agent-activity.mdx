...
At the top of the detail panel, a flame graph shows the wall-clock
position and relative duration of every span in the selected trace. Each span
appears as a colored block scaled to its actual duration, positioned at its start
time in milliseconds from the beginning of the trace. Use the timeline to:

- **Spot the longest operation at a glance.** Wide blocks indicate spans that
  dominated total latency.
- **See parallelism.** Overlapping blocks indicate spans that executed
  concurrently rather than sequentially.
- **Inspect any span inline.** Click a block in the timeline to load that
  span's details into view, including its input messages and output messages,
  token counts, and other metadata.

![The Spans tab with a trace selected, highlighting a group of related spans in the table and showing the flame graph in the detail panel on the right.](./imgs/agent-view-spans-detail.png)
...
Each row represents one signal score applied to a turn. The following columns appear by default.

| Column | Description |
|---|---|
| **Type** | Linked to the flame graph details of the turn that generated the signal result. |
...
| **Scores** | The numeric score from 0 to 1, or a tag if matched. Scores near 0 indicate a detected issue. Scores near 1 indicate no issue detected. |
| **Trend (24h)** | How scores for this signal have changed over the past 24 hours. |
...
- **Rating**: Ratings-based scorers assign a numeric value between 0 and 1 to each matching turn. Weave creates one Signals table row per matching turn.
- **Tags**: Tags-based scorers assign a tag to each matching turn that meets your criteria (for example, detecting user frustration). Weave creates a Signals table row only when a turn matches the tag criteria. If you create a tag signal and see no rows, the scorer likely hasn't found a match yet.  Multiple tags can be assigned per scorer.

#### Only score turns matching

A turn is one back-and-forth exchange with the agent. Use this selector to restrict which turns the signal scores, such as a specific `Agent name`, `Operation name`, `Tool name`, or `Request model`. Multiple filters are combined with `AND` logic. To score every turn, select the **x** at the end of the filter row to remove it.

#### Prompt template

Starter templates are provided for common evaluation goals, such as `User Frustration` or `Malicious Intent`. After you select a template, the prompt appears inline as **Scorer prompt** for immediate editing. The scorer prompt supports template variables (`{input_messages}`, `{output_messages}`, `{system_instructions}`, and `{agent_name}`) that Weave fills in at score time.
...
- **Inference model**: Choose the model that runs the scorer prompt against agent turns. [Serverless Inference](/inference) is recommended and is the default when it is set up for your project. Otherwise, Weave falls back to the first configured third-party provider or any saved custom provider for the project.
...
Signal activity itself appears under **Traces** in the project sidebar. If you do not see signal matches you expect, use the Traces table to debug. Check the scorer name and the `Status` column for error conditions.  Scorer execution errors show a red indicator for `Status` and include details for the error.
