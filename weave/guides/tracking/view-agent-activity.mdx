...
- **Rating**: A numeric score between 0.0 and 1.0 that a signal monitor assigns to evaluate a matching span.
- **Tags**: Labels that a signal monitor assigns to categorize matching spans, such as detecting user frustration or NSFW content.
...
| Column | Description |
|---|---|
| **Type** | The part of the conversation being scored. Currently only "turn" is supported. |
...
| **Trend** | Displays an inline chart showing how this signal is trending over time. Shows either the average value (for ratings) or the count (for tags). |
...
Choose a starter template from the lists below, then adjust the prompt that appears inline as **Scorer prompt**. Weave resolves the template variables `{input_messages}`, `{output_messages}`, `{system_instructions}` during scoring.
...
| **Low Quality Response** | Agent output that is factually wrong, off-topic, evasive, repetitive, refuses without justification, or otherwise fails to address the user's request. |
