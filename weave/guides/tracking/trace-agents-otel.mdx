...
W&B Weave agent observability is built on [OpenTelemetry (OTel)](https://opentelemetry.io/docs/concepts/). Weave provides a dedicated OTLP endpoint that ingests spans into the **Agents** view directly, so if your application already emits OTel spans, you can send them to Weave by pointing your existing OTel pipeline at the endpoint. You don't need to install or import the Weave SDK.

This page covers the agents endpoint details, authentication, the span attributes that Weave uses to render agent activity, and an environment-variable configuration that requires no code changes at all.

Use this approach when:

- Your application is already instrumented with OTel, whether through your own `TracerProvider` or a third-party instrumentation library.
- You want to keep your existing observability pipeline and fan spans out to Weave as an additional backend.
- You're working in a language that the Weave SDK doesn't support.

If you're instrumenting an agent from scratch in Python or TypeScript, the Weave SDK's agent helpers manage span creation, attributes, and flushing for you. See [Trace your agents](/weave/guides/tracking/trace-agents) instead.
...
<Note>
This endpoint feeds the **Agents** view. Weave has a second OTLP endpoint, `/otel/v1/traces`, that feeds the **Traces** and **Threads** views. Spans sent to one endpoint don't appear in the views served by the other. To populate both, export the same spans to both endpoints. See [Send OpenTelemetry traces to Weave](/weave/guides/tracking/otel) for the Traces endpoint.
</Note>

## Authentication

Authenticate with your W&B API key, which you can create at [https://wandb.ai/settings](https://wandb.ai/settings). The endpoint accepts either of the following headers:
...
- **`project_id` header**: Set the value to `<your-team-name>/<your-project-name>`.
...
If your application or collector reads the standard OTel exporter environment variables, you can route spans to Weave without changing any code:
...
export OTEL_RESOURCE_ATTRIBUTES="wandb.entity=<your-team-name>,wandb.project=<your-project-name>"
```

<Warning>
`OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` must contain the full traces path shown above. The OTel SDK sends requests to this URL verbatim. If you use the generic `OTEL_EXPORTER_OTLP_ENDPOINT` variable instead, the SDK appends `/v1/traces` itself, so that variable must hold `https://trace.wandb.ai/agents/otel` or you get a doubled path and silently dropped spans.
</Warning>
...
Then run the following code with `WANDB_API_KEY` set in your environment:
...
Third-party instrumentation libraries such as [OpenInference](https://github.com/Arize-ai/openinference) and [OpenLLMetry](https://github.com/traceloop/openllmetry) emit OTel spans automatically for popular LLM providers and frameworks. Because the agents endpoint accepts spans from any OTel pipeline, you can point these libraries at it using the same exporter configuration shown above. Spans render in the Agents view when they carry the `gen_ai.operation.name` values described in [Shape spans for the Agents view](#shape-spans-for-the-agents-view).

You can also forward spans through an [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/) to centralize authentication and fan spans out to Weave alongside your other observability backends. The [collector setup for the Traces endpoint](/weave/guides/tracking/otel#use-an-opentelemetry-collector) applies to the agents endpoint as well. Change the exporter endpoint to `https://trace.wandb.ai/agents/otel`.

## View your traces

After your application exports spans, open the **Agents** tab of your Weave project at `https://wandb.ai/<your-team-name>/<your-project-name>/weave/agents`. Conversations are grouped under the `gen_ai.agent.name` you set, each turn appears as a row in the conversation timeline, and LLM and tool calls nest inside each turn. For details on navigating the view, see [View agent activity](/weave/guides/tracking/view-agent-activity).

## Combine with the Weave SDK

If part of your application uses the Weave SDK, note that `weave.init()` doesn't attach an exporter to a `TracerProvider` that your application already registered. Spans from your own provider only reach Weave if you add the Weave exporter to that provider yourself, using the exporter configuration shown in the example above.
