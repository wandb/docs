...
To conditionally disable tracing for a specific block of code, you can use a tracing context manager. Weave provides two equivalent options for this purpose:

- `with set_tracing_enabled(False)`
- `with tracing_disabled()`

Both context managers suppress tracing **only for the function calls executed inside the `with` block**. They are intended to be used in application code to scope which calls should not be logged.
...
Although tracing behavior is fixed when functions are defined, these context managers can still be used for runtime control when combined with application logic. For example, you can wrap the context manager in a conditional to dynamically enable or disable tracing based on a runtime value:
