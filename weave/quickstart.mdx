...
3. **(Optional)** Add the `@weave.op()` decorator to specific functions you want to track. If you do not add `@weave.op()`, Weave automatically tracks the inputs, outputs, and code of your top-level functions. The decorator has to following syntax in TypeScript: `weave.op(your_function)`

The following example code sends a request to OpenAI (requires [OpenAI API key](https://platform.openai.com/docs/quickstart/step-2-setup-your-api-key)). The system prompt asks it to extract dinosaur names from the input and identify whether each is an herbivore or carnivore.
